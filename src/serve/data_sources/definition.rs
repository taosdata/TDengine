...
pub enum DataSourceOptions {
    ...
    Endpoint {
        ...
        #[serde(skip_serializing_if = "Option::is_none")]
        security_mode: Box<Option<Param>>,
        #[serde(skip_serializing_if = "Option::is_none")]
        security_policy: Box<Option<Param>>,
        #[serde(skip_serializing_if = "Option::is_none")]
        certificate: Box<Option<Param>>,
        #[serde(skip_serializing_if = "Option::is_none")]
        private_key: Box<Option<Param>>,
        #[serde(skip_serializing_if = "Option::is_none")]
        connect_timeout: Box<Option<Param>>,
    },
    Uri {
        #[serde(default)]
        host: OptionDef,
...
mod tests {
    ...
    #[test]
    fn parse_data_source_options_test() -> anyhow::Result<()> {
        let endpoint = DataSourceOptions::Endpoint {
            endpoint: OptionDef::default(),
            username: None,
            password: None,
            security_mode: Box::new(Some(Param {
                name: "test".to_string(),
                display_order: Some(5),
                ..Default::default()
            })),
            security_policy: Box::new(None),
            certificate: Box::new(None),
            private_key: Box::new(None),
            connect_timeout: Box::new(None),
        };
        let serialize_endpoint = serde_json::to_string(&endpoint)?;
        let DataSourceOptions::Endpoint { security_mode, .. } =
            serde_json::from_str(&serialize_endpoint)?
        else {
            anyhow::bail!("not data source option endpoint")
        };
        let security_mode = security_mode.context("security mode not found")?;
        assert_eq!(&security_mode.name, "test");
        assert_eq!(security_mode.display_order, Some(5));
        Ok(())
    }
}
