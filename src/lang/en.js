...
export default {
  ...
  step3sub2linux:`Linux: <code>systemctl start taosx-agent</code> and use <code> systemctl status taosx-agent</code> to check the service status. `,
  step3sub2window:`Windows: <code>sc.exe start taosx-agent</code> and use <code>sc.exe query taosx-agent</code> to check the service status.`,
  step3sub3linux:`If the agent token is error, the service will exit directly, you can check the logs with: journalctl -u taosx-agent on Linux`,
  step3sub3window:`If the agent token is error, the service will exit directly, you can check the logs with or check the log file in <code>C:\\Program Files\\taosX\\log\\agent\\ on Windows.</code>`,
