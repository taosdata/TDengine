...
export default {
  ...
  dataIn: {
    ...
    needAgentTip:
      "如果数据源在私有网络，那么请配置 Agent 的安全链接，这样无需配置 VPN 或将数据源服务公开到外部网络。",
    runAgent: "运行代理",
    ...
    successTip: "数据源可用。",
    successVersionTip: "数据源可用，版本是 {version}，当前系统支持此版本。",
    unSupportTip:
      "数据源可用，版本是 {version}，当前系统不支持此版本，对于给您带来的不便，我们深表歉意，请联系 TDengine 团队，我们将在未来为您的数据源版本添加支持。",
    failTip:
  ...
  login: {
    ...
    loginLeftMessage: {
      title:
        "只需 200 美元，您就可以在一个月内管理 10,000 个智能电表生成的数据",
      list: [
        "从现在起将您的总拥有成本至少降低 50%",
        "不用担心可扩展性，不再需要维护",
        "专注于您的业务而不是数据管理",
    ...
    youRegistered: "您已经注册",
    registerVer:
      "您已经尝试注册TDengine  请在下方输入电子邮件以接收另一封验证电子邮件。",
    registerForgot:
      "您已经注册了 TDengine  服务。 如果您忘记了密码，请在下面输入电子邮件以重置您的密码。",
    goBack: "返回",
    ...
    servTaosdTip:
      "您的 taosd 服务处于异常，请尝试重启 taosd 服务并保证服务可用",
  },
  dashboard: {
  ...
  data: {
    ...
    bufferTip:
      "每个 vnode 的写入缓存大小，单位为 MB，默认为 96，最小为 3，最大为 16384",
    durationTip:
  ...
  connecter: {
    ...
    codeSample: "代码示例",
    codeSampleTip:
      "控制文档中的代码示例如何填充系统资源。 并非所有代码示例都使用系统资源。",
    createToken: "创建令牌",
  ...
  billing: {
    ...
    creditTip:
      "这是您的信用卡和借记卡的列表。 选择省略号 (...) 以编辑或删除卡片或将其设为默认卡片",
    nameOnCard: "卡上的名字",
    makeDefault: "设置为默认",
    streetAddress: "街道地址",
    creditNumber: "卡号",
    ...
    paymentEmails: "付款邮箱",
    paymentEmailTip:
      "此列表中的电子邮件将收到定期付款电子邮件，其中包含 PDF 附件形式的发票。",
    addNewEmail: "添加新邮箱",
    wallet: "钱包",
    paymentMethod: "付款方式",
    ...
    dueAt: "到期日期",
    paymentFailedTip:
      "付款失败，请{更新您的信用卡}信息。 如果在到期日之前未收到付款，服务将停止",
    monthMap: {
      1: "一",
      2: "二",
      3: "三",
  ...
  vpc: {
    ...
    limitTip:
      "通过使用 CIDR 块或单个 IP 地址创建流量过滤器来限制从特定 IP 地址对集群的访问。",
    configTip:
      "您可以在此处配置默认规则，然后在集群的连接到 TDengine 对话框中快速应用它们。",
    defaultIP: "默认规则：",
    ipAddress: "IP 地址",
    desc: "描述 (可选)",
    optional: "可选",
  ...
  topic: {
    ...
    advancedDatabaseFeatures: '数据库可选功能',
    ...
    `,
    learnMoreTip:
      "要了解更多关于数据订阅的信息，请查看<a target='_blank' href=\"docsUrl\">文档</a>",
    upTime: "在线时间",
    subscribeTime: "订阅时间",
    rebalanceTime: "平衡时间",
  ...
  datasource: {
    transformer: {
      ...
      更详细解析规则请访问<a href="/docs/enterprise/datain/transformer/#2-提取或拆分">企业版文档<a>。`,
      ...
      可使用 && 或 || 组合多个判断表达式，更详细过滤规则请访问<a href="/docs/enterprise/datain/transformer/#3-过滤">企业版文档<a>。`,
      ...
      更详细映射规则请访问<a href="/docs/enterprise/datain/transformer/#4-映射">企业版文档<a>。`,
      ...
      更详细拆分提取规则请访问<a href="/docs/enterprise/datain/transformer/#2-提取或拆分">企业版文档<a>。`,
    ...
    excutestart: '启动数据源"{name}"',
    excutestop: '停止数据源"{name}"',
    viewconfig: '查看"{name}" 数据源配置',
    editconfig: '修改配置',
    agent: "代理",
    agenttip: "请选择代理",
    typetip: "请选择数据类型",
  ...
  dataOut: {
    ...
    timeoutPlaceholder:
      "kafka ack_timeout，默认为 1 s。消息需要 ack，ack 级别=1",
  },
  docs: {
    taosxAgent: {
...
Windows： <code>C:\\TDengine\\cfg\\</code>`,
    ...
    connector: {
      ...
      python: {
        step1: "安装连接器",
        step1desc:
          "首先您需要安装最新的 taospy 模块，Python要求是 Python 3.6+。请在终端中执行下面的命令：",
        step2: "配置",
        step3: "建立连接",
        step3desc:
      ...
      rest: {
        ...
        step2: "插入",
        step2desc:
          "请按照下面的命令通过命令行工具 curl 往数据库 test 的表 d1001 中插入数据：",
        step3: "查询",
        step3desc:
          "请按照下面的命令通过命令行工具 curl 从数据库 information_schema 的表 ins_databases 中查询数据：",
      },
      r: {
        step1: "安装RJDBC库",
        step11desc:
          "首先该库需要依赖Java环境，请先从Oracle官方网站下载适合您操作系统的JDK，并按照安装指南进行安装。",
      ...
      odbc: {
        ...
        step23desc: '选择想要添加的数据源，然后选择【TDengine】，点击完成，进入 TDengine ODBC 数据源配置页面，填写如下必要信息：',
        step23desc1: '【DSN】：',
    ...
    party: {
      prometheus: {
        ...
        step2end:
          "然后 Prometheus 就会被安装到当前目录. 想了解更多 Prometheus 安装选型，请参考",
        step2doc: "官方文档",
        step3: "配置 Prometheus",
        step3desc:
          "可以通过编辑 Prometheus 配置文件 `prometheus.yml` 来设置 Prometheus （如果您完全按照上面的步骤执行，您可以在当前目录找到 prometheus.xml 文件）。",
        step3desc1:
          "配置完成后，Prometheus 会从自己的 HTTP 指标端点收集数据并存储到 TDengine  里面。",
        step4: "启动 Prometheus",
        step4desc:
          "之后 Prometheus 应该已经启动好。同时也启动了一个 Web 服务器",
        step4desc1:
      ...
      telegraf: {
        ...
        step3: "配置",
        step3desc:
          "在您的终端命令行里面执行下面的命令来保存 TDengine  的令牌和URL为环境变量：",
      ...
      influxdb: {
        ...
        step1: "配置",
        step1desc:
          "在您的终端命令行运行下面的命令来设置 TDengine  的令牌和URL为环境变量：",
        step2: "插入",
        step2desc:
    ...
    dataout: {
      dump: {
        ...
        step1desc2:
          "使用时可以指定数据备份的目录路径，如果不指定位置，taosdump 默认会将数据备份到当前目录。",
        step1desc3:
    ...
    virtual: {
      grafana: {
        desc: `${grafanagds} 能够与开源数据可视化系统  Grafana 快速集成搭建数据监测报警系统，整个过程无需任何代码开发，${grafanagds} 中数据表的内容可以在仪表盘(DashBoard)上进行可视化展现。关于 ${grafanagds} 插件的使用您可以在 GitHub`,
        topdesc: `${grafanagds} 能够与开源数据可视化系统`,
        topdesc1: `   快速集成搭建数据监测报警系统，整个过程无需任何代码开发。${grafanagds} 中数据表的内容可以在仪表盘(DashBoard)上进行可视化展现。关于 ${grafanagds} 插件的使用您可以在 GitHub. `,
        ...
        step3: "添加数据源",
        step3desc: `在Grafana数据源配置页面中，复制如下所示的主机和用户，并将其输入相应的输入框。`,
        step4: "使用 Grafana",
      ...
      gds: {
        ...
        topdesc: "使用",
        topconnector: "第三方连接器",
        topdesc1: `，Google Data Studio可以快速访问 ${grafanagds} 并且通过基于网页的报表功能可以快速创建交互式的报表和仪表盘。整个过程不需要任何的代码编写过程。可以分享报表和仪表盘给不同的个人，团队以及全世界，还可以跟其他人员实时协作，另外在任何的网页里面嵌入您的报表。`,
    ...
    tool: {
      cli: {
        ...
        step1: "安装",
        step1desc:
          "运行 TDengine CLI 来访问 TDengine  ，请首先下载和安装最新的 ",
        ...
        step2: "配置",
        step2desc:
          "在您的 Linux 终端里面执行下面的命令设置 TDengine 的 DSN 为环境变量：",
        step2desc1:
          "在您的 Windows CMD 里面执行下面的命令设置 TDengine  的 DSN 为环境变量：",
        step2desc2:
          "或者在您的 Windows PowerShell 里面执行下面的命令设置 TDengine  的 DSN 为环境变量：",
        step2desc3:
          "在您的 Mac 里面执行下面的命令设置 TDengine  的 DSN 为环境变量：",
        step3: "建立连接",
        step3desc:
          "如果您已经设置了环境变量，您只需要立即执行 `taos` 命令就可以访问 TDengine  实例。",
        step3desc1:
        ...
        step4desc1:
          "进入 TDengine CLI 以后，您就可以执行大量的 SQL 命令来进行插入，查询或者进行管理。",
        step4desc2: "官方文档",
        step4desc3: "。",
      },
      benchmark: {
        ...
        step32: "使用配置文件运行",
        step32desc:
          "taosBenchmark 安装包中提供了配置文件的示例，位于 `<install_directory>/examples` 下",
        step32desc1:
        ...
        step41desc2:
          "：TDengine 客户端配置文件所在的目录，默认路径是 /etc/taos 。",
        step41desc3:
        ...
        step44desc5:
          '：超级表和子表名称中是否包含转义字符，默认值为 "no"，可选值为 "yes" 或 "no"。',
        step44desc6:
        ...
        step44desc21:
          "：每个子表中插入数据的时间戳步长，单位与数据库的 `precision` 一致，默认值是 1。",
        ...
        step47desc2:
          "：预先建立的与 TDengine 服务端之间的连接的数量。若不配置，则与所指定的线程数相同。",
    ...
    topic: {
      ...
      python: {
        step1: "安装模块",
        step1desc:
          "首先您需要安装 `taos-ws-py` 模块，版本需要大于 `0.2.1` 。在您的终端执行下面的命令。",
        step1desc1: "您需要先安装 `Python3`。",
      },
      go: {
        step1: "初始化",
        step1desc:
          "您按照下面命令可以生成 Go 示例模块并加上 `driver-go` 依赖：",
      },
      rust: {
        step1: "创建项目",
      ...
      step5: "关闭消费者",
      step5desc:
        "如果您想从这个共享的主题 `{0}` 中取消订阅消息，您可以按照下面的代码关闭这个消费者：",
      step6: "完整实例",
    ...
    dashboard: {
      ...
      topdesc1: `。${grafanagds} 可以与 Grafana 顺利集成，无需一行代码。 `,
    ...
    tools: {
      seeq: {
        ...
        step1: "前置条件",
        step1desc:
          "安装 Seeq Server 和 Seeq Data Lab 软件，请从官方下载地址下载安装 ",
        step1desc1: " 。",
        step2: "安装 TDengine Java 连接器",
        step2desc:
          "获取 Seeq 数据地址配置。在 Linux 上，可以执行下面的命令获取：",
        step2desc11: "首先从 ",
        step2desc12: " 可以下载最新的 TDengine Java 连接器（目前的版本是 ",
        step2desc13:
          "），然后复制下载的 JAR 文件到这个文件目录 the_directory_found_in_step_1/plugins/lib/ 。",
        step2desc2: "重启 Seeq 服务器。在 Linux 上，可以执行下面的命令：",
        step3: "添加 TDengine 数据源",
        step3full: "把 TDengine 数据源添加到 Seeq 数据源",
        step3desc:
          "打开 Seeq，以 admin 用户登录，然后打开 Administration，点击“Add Data Source”",
        step3desc1: "对于连接器，请选择 SQL connector v2",
        step3desc2:
          "在“Additional Configuration”的输入框, 请复制和粘贴下面的内容：",
        step3desc3:
          "对于“QueryDefintions”，请参考下面的例子来完成您自己的查询定义。",
        step4: "智能电表样例",
      ...
      yonghongbi: {
        ...
        step43desc: '当勾选“数据库内计算”时，BI 工具将不再缓存 TDengine 的时序数据，并在处理查询时将 SQL 请求发送给 TDengine 直接处理。',
        step44desc:
        ...
        step44desc2: '。通过使用这些特色查询，当 BI 工具将 SQL 查询发送到 TDengine 数据库时，可以大大提高数据访问速度，减少网络传输带宽。',
        step45desc: '在 BI 工具中，您可以创建“参数”并在 SQL 语句中使用，通过手动、定时的方式动态执行这些 SQL 语句，即可实现可视化报告的刷新效果。如下 SQL 语句：',
        step45desc0: '可以从 TDengine 实时读取数据，其中：',
        step45desc1: '：表示时间窗口起始时间。',
        step45desc2: '：表示时间窗口内的聚合值。',
        step45desc3: '：表示在 SQL 语句中引入名称为 interval 的参数，当 BI 工具查询数据时，会给 interval 参数赋值，如果取值为 1m，则表示按照 1 分钟的时间窗口降采样数据。',
        step45desc4: '：该参数用来指定查询的数据表名称，当在 BI 工具中把某个“下拉参数组件”的 ID 也设置为 metric 时，该“下拉参数组件”的被选择项将会和该参数绑定在一起，实现动态选择的效果。',
        step45desc5: '：这两个参数用来表示查询数据集的时间范围，可以与“文本参数组件”绑定。',
        step45desc6: '可以在 BI 工具的“编辑参数”对话框中修改“参数”的数据类型、数据范围、默认取值，并在“可视化报告”中动态设置这些参数的值。',
        step5: '制作报告',
        step5full: '制作可视化报告',
  ...
  <code>C:\\Program Files\\taosX\\log\\agent\\</code>`,
    ...
    addagenttip:
      "如果数据源在私有网络，那么请配置Agent的安全链接，这样无需配置 VPN 或将数据源服务公开到外部网络。",
    edittitle: "编辑代理",
    deletetip: `确定删除代理 {id} ?`,
    eidtagent: "编辑代理",
    createnewagent: "创建新的代理",
