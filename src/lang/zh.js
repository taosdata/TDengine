...
export default {
  ...
  dataIn: {
    ...
    connectorTip:
      '使用您选择的编程语言-将数据输入 TDengine Cloud 的主要方法是<a href="https://docs.tdengine.com/cloud/data-in/insert-data/" target="_blank">，使用SQL插入</a>数据',
    thirdParty: "三方工具",
  ...
  docs: {
    connector: {
      ...
      python: {
        ...
        step42Title: "第二步：配置",
        step42Desc: "为了让 Jupyter 连接上 TDengine Cloud 的实例，您需要先启动 Jupyter，然后设置好环境变量。我们以 Linux 终端作为例子：",
    ...
    party: {
      prometheus: {
        desc: "配置 Prometheus 往 TDengine Cloud 写入和读取数据。",
        totaldesc1:
        ...
        step3desc1: "配置完成后，Prometheus 会从自己的 HTTP 指标端点收集数据并存储到 TDengine Cloud 里面。",
        step4: "启动 Prometheus",
        ...
        step5: "验证远程写入",
        step5desc: "登录 TDengine Cloud，然后点击左边导航栏的”数据浏览器“。您就会看见由 Prometheus 收集的指标数据。",
        step5desc1: "TDengine 会根据一定规则自动为子表名创建唯一的 IDs。",
      },
      telegraf: {
        desc: "配置 Telegraf 往 TDengine Cloud 写入指标。",
        totaldesc1:
        ...
        step2doc: "官方文档",
        step3: "配置",
        step3desc: "在您的终端命令行里面执行下面的命令来保存 TDengine Cloud 的令牌和URL为环境变量：",
      ...
      influxdb: {
        title: "InfluxDB 行协议",
        desc: "这一部分主要介绍如何通过REST接口的无模式 {0}往 TDengine Cloud 写入数据。",
        step1: "配置",
        step1desc: "在您的终端命令行运行下面的命令来设置 TDengine Cloud 的令牌和URL为环境变量：",
        step2: "插入",
        step2desc:
    ...
    virtual: {
      grafana: {
        ...
        step2desc: "请复制下面的脚本命令来为数据源安装设置 `TDENGINE_CLOUD_URL` 和 `TDENGINE_CLOUD_TOKEN` 的环境变量：",
      ...
      gds: {
        ...
        step1desc: "目前的",
        step1desc1: " 连接器",
        step1desc2: " 支持两种不同的数据源：TDengine Server 和 TDengine Cloud。首先选择”TDengine Cloud“类型然后点击”下一步“。",
        step2: "连接器配置",
        step21: "必须的配置",
    ...
    tool: {
      ...
      benchmark: {
        ...
        step3: "运行",
        step31: "配置和运行方式",
        step31desc: "运行下面命令来设置 TDengine Cloud 的 DSN 环境变量：",
