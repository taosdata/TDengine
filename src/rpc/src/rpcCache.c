...
void rpcAddConnIntoCache(void *handle, void *data, char *fqdn, uint16_t port, int8_t connType) {
  ...
  uint64_t time1 = taosGetTimestampMs();
  ...
  pCache->count[hash]++;
  rpcRemoveExpiredNodes(pCache, pNode->next, hash, time1);
...
void *rpcGetConnFromCache(void *handle, char *fqdn, uint16_t port, int8_t connType) {
  ...
  while (pNode) {
    if (time1 >= pCache->keepTimer + pNode->time) {
      rpcRemoveExpiredNodes(pCache, pNode, hash, time1);
      pNode = NULL;
      break;
    }
...
static void rpcRemoveExpiredNodes(SConnCache *pCache, SConnHash *pNode, int hash, uint64_t time1) {
  if (pNode == NULL || (time1 < pCache->keepTimer + pNode->time) ) return;
