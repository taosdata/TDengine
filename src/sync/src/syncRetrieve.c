...
static int64_t syncProcessLastWal(SSyncPeer *pPeer, char *wname, int64_t index1) {
  SSyncNode *pNode = pPeer->pSyncNode;
...
static int64_t syncRetrieveWal(SSyncPeer *pPeer) {
  ...
  while (1) {
    // retrieve wal info
    wname[0] = 0;
    code = (*pNode->getWalInfoFp)(pNode->vgId, wname, &index1);
    if (code < 0) {
    ...
    if (code == 0) {  // last wal
      code = syncProcessLastWal(pPeer, wname, index1);
