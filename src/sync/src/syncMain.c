...
int64_t syncStart(const SSyncInfo *pInfo) {
  ...
  for (int32_t index1 = 0; index1 < pCfg->replica; ++index1) {
    const SNodeInfo *pNodeInfo = pCfg->nodeInfo + index1;
    pNode->peerInfo[index1] = syncAddPeer(pNode, pNodeInfo);
    if (pNode->peerInfo[index1] == NULL) {
  ...
  for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
    syncStartCheckPeerConn(pNode->peerInfo[index1]);
  }
...
int32_t syncReconfig(int64_t rid, const SSyncCfg *pNewCfg) {
  ...
  for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
    syncStopCheckPeerConn(pNode->peerInfo[index1]);
  }
...
void syncBroadcastStatus(SSyncNode *pNode) {
  for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
    if (index1 == pNode->selfIndex) continue;
    SSyncPeer *pPeer = pNode->peerInfo[index1];
...
static void syncResetFlowCtrl(SSyncNode *pNode) {
  for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
    pNode->peerInfo[index1]->numOfRetrieves = 0;
  }
...
static void syncChooseMaster(SSyncNode *pNode) {
  ...
  if (onlineNum == pNode->replica) {
    ...
    for (int32_t i = 1; i < pNode->replica; ++i) {
      if (pNode->peerInfo[i]->version > pNode->peerInfo[index1]->version) {
        index1 = i;
      }
    }
    sDebug("vgId:%d, master:%s may be choosed, index1:%d", pNode->vgId, pNode->peerInfo[index1]->id, index1);
  } else {
  ...
  if (index1 < 0 && onlineNum > replica / 2.0) {
    ...
    for (int32_t i = 0; i < pNode->replica; ++i) {
      ...
      if (pPeer->role == TAOS_SYNC_ROLE_SLAVE || pPeer->role == TAOS_SYNC_ROLE_MASTER) {
        if (index1 < 0 || pPeer->version > pNode->peerInfo[index1]->version) {
          index1 = i;
        }
      }
    }
    ...
    if (index1 >= 0) {
      sDebug("vgId:%d, master:%s may be choosed, index1:%d onlineNum(arb):%d replica:%d", pNode->vgId,
             pNode->peerInfo[index1]->id, index1, onlineNum, replica);
    }
  }

  if (index1 >= 0) {
    if (index1 == pNode->selfIndex) {
...
static SSyncPeer *syncCheckMaster(SSyncNode *pNode) {
  ...
  for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
    if (pNode->peerInfo[index1]->role != TAOS_SYNC_ROLE_OFFLINE) {
      onlineNum++;
    }
  }
  ...
  if (onlineNum <= replica * 0.5) {
    ...
    for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
      SSyncPeer *pTemp = pNode->peerInfo[index1];
      if (pTemp->role != TAOS_SYNC_ROLE_MASTER) continue;
      if (masterIndex < 0) {
        masterIndex = index1;
        sDebug("vgId:%d, peer:%s is master, index1:%d", pNode->vgId, pTemp->id, index1);
        ...
        if (masterIndex == pNode->selfIndex) {
          ...
          sError("vgId:%d, peer:%s is master too, masterIndex:%d index1:%d", pNode->vgId, pTemp->id, masterIndex, index1);
        }
      }
    }
  }
...
static int32_t syncValidateMaster(SSyncPeer *pPeer) {
  ...
  if (nodeRole == TAOS_SYNC_ROLE_MASTER && nodeVersion < pPeer->version) {
    ...
    for (int32_t index1 = 0; index1 < pNode->replica; ++index1) {
      if (index1 == pNode->selfIndex) continue;
      syncRestartPeer(pNode->peerInfo[index1]);
    }
  }
...
static void syncCheckRole(SSyncPeer *pPeer, SPeerStatus* peersStatus, int8_t newPeerRole) {
  ...
  if (pMaster) {
    ...
    int32_t index1 = 0;
    if (peersStatus != NULL) {
      for (index1 = 0; index1 < pNode->replica; ++index1) {
        SSyncPeer *pTemp = pNode->peerInfo[index1];
        if (pTemp->role != peersStatus[index1].role) break;
        if ((pTemp->role != TAOS_SYNC_ROLE_OFFLINE) && (pTemp->version != peersStatus[index1].version)) break;
      }

      if (index1 >= pNode->replica) consistent = 1;
    } else {
...
static int32_t syncSaveFwdInfo(SSyncNode *pNode, uint64_t _version, void *mhandle) {
  SSyncFwds *pSyncFwds = pNode->pSyncFwds;
  int64_t    time1 = taosGetTimestampMs();
