...
int taosGetTableRecordInfo(char *table, STableRecordInfo *pTableRecordInfo, TAOS *taosCon) {
  ...
  if (code != 0) {
    errorPrint("failed to run command %s\n", tempCommand);
    free(tempCommand);
    taos_free_result(result);
    return -1;
  }
  ...
  errorPrint("invalid table/metric %s\n", table);
  free(tempCommand);
  return -1;
}
...
int taosDumpOut(struct arguments *arguments) {
  ...
  _dump_db_point:
    ...
    if (dbInfos[count] == NULL) {
      errorPrint("failed to allocate %"PRIu64" memory\n", sizeof(SDbInfo));
      goto _exit_failure;
    }
...
int taosGetTableDes(
  ...
  for (int i = 0 ; i < count; i++) {
    ...
    if (NULL == row) {
      errorPrint(" fetch failed to run command <%s>, reason:%s\n", sqlstr, taos_errstr(res));
      taos_free_result(res);
      return -1;
    }
