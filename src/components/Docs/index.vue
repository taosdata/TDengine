<template>
  <div class="view">
    <section class="view-header">
      <section class="left">
        <img
          class="image-contains"
          :src="getImg(config.name, config.icon)"
          alt=""
        />
      </section>
      <section class="right">
...
export default {
  ...
  mounted() {
    // 在这里保存元素
    this.$nextTick(() => {
      this.domList = this.steps.map((item) =>
        document.getElementById(item.dom)
      );
    });
    this.scoleFn = debounce((e) => {
      let top = e.target.scrollTop;
      let currentTop = Math.ceil(top + elementHeight);

      let activeTab = this.getOffsetTop().findLastIndex((item) => {
        return (
          (item.start <= top && item.end >= currentTop) ||
          (item.start <= currentTop && item.end >= currentTop)
        );
      });
      this.activeTab = activeTab == -1 ? this.domList.length : activeTab + 1;
    }, 100);
    ...
    this.element.addEventListener("scroll", this.scoleFn);
    this.$once("hook:beforeDestroy", () => {
      this.element.removeEventListener("scroll", this.scoleFn);
    });
