...
<script type="text/babel">
  ...
  const calcDefaultValue = (defaultValue) => {
    if (Array.isArray(defaultValue)) {
      ...
      return [switchTimezone(new Date(defaultValue)), nextDate(switchTimezone(new Date(defaultValue)), 1)];
    } else {
      return [switchTimezone(new Date()), nextDate(switchTimezone(new Date()), 1)];
    }
  };
  ...
  export default {
    ...
    computed: {
      ...
      enableMonthArrow() {
        ...
        return this.unlinkPanels && switchTimezone(new Date(this.leftYear + yearOffset, nextMonth)) < switchTimezone(new Date(this.rightYear, this.rightMonth));
      },
    ...
    data() {
      return {
        ...
        rightDate: nextMonth(switchTimezone(new Date())),
        rangeState: {
          endDate: null,
          selecting: false,
          row: null,
    ...
    methods: {
      ...
      handleDateInput(value, type) {
        ...
        if (parsedValue) {
          if (typeof this.disabledDate === 'function' &&
            this.disabledDate(switchTimezone(new Date(parsedValue)))) {
            return;
          }
          if (type === 'min') {
            this.minDate = modifyDate(this.minDate || switchTimezone(new Date()), parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate());
            this.leftDate = switchTimezone(new Date(parsedValue));
            if (!this.unlinkPanels) {
              this.rightDate = nextMonth(this.leftDate);
            }
          } else {
            this.maxDate = modifyDate(this.maxDate || switchTimezone(new Date()), parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate());
            this.rightDate = switchTimezone(new Date(parsedValue));
            if (!this.unlinkPanels) {
      ...
      handleMinTimePick(value, visible, first) {
        this.minDate = this.minDate || switchTimezone(new Date());
        if (value) {
        ...
        if (!this.maxDate || this.maxDate && this.maxDate.getTime() < this.minDate.getTime()) {
          this.maxDate = switchTimezone(new Date(this.minDate));
        }
      },
      ...
      handleMaxTimePick(value, visible, first) {
        ...
        if (this.maxDate && this.minDate && this.minDate.getTime() > this.maxDate.getTime()) {
          this.minDate = switchTimezone(new Date(this.maxDate));
        }
      },
