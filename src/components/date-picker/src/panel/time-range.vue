...
switchTimezone
  ...
  const advanceTime = function(date, amount) {
    return switchTimezone(new Date(Math.min(date.getTime() + amount, maxTimeOfDay(date).getTime())));
  };
  ...
  export default {
    ...
    data() {
      return {
        popperClass: '',
        minDate: switchTimezone(new Date()),
        maxDate: switchTimezone(new Date()),
        value: [],
        oldValue: [switchTimezone(new Date()), switchTimezone(new Date())],
        defaultValue: null,
        format: 'HH:mm:ss',
        visible: false,
        selectionRange: [0, 2],
    ...
    watch: {
      value(value) {
        if (Array.isArray(value)) {
          this.minDate = switchTimezone(new Date(value[0]));
          this.maxDate = switchTimezone(new Date(value[1]));
        } else {
          if (Array.isArray(this.defaultValue)) {
            this.minDate = switchTimezone(new Date(this.defaultValue[0]));
            this.maxDate = switchTimezone(new Date(this.defaultValue[1]));
          } else if (this.defaultValue) {
            this.minDate = switchTimezone(new Date(this.defaultValue));
            this.maxDate = advanceTime(switchTimezone(new Date(this.defaultValue)), 60 * 60 * 1000);
          } else {
            this.minDate = switchTimezone(new Date());
            this.maxDate = advanceTime(switchTimezone(new Date()), 60 * 60 * 1000);
          }
        }
      },
