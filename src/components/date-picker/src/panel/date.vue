        ...
        <div class="el-picker-panel__body">
          ...
          <div class="el-picker-panel__content">
            <date-table
              ...
              :value="value"
              :default-value="defaultValue ? switchTimezone(new Date(defaultValue)) : null"
              :date="date"
  ...
  export default {
    ...
    watch: {
      ...
      value(val) {
        ...
        if (isDate(val)) {
          this.date = switchTimezone(new Date(val));
        } else {
      ...
      defaultValue(val) {
        if (!isDate(this.value)) {
          this.date = val ? switchTimezone(new Date(val)) : switchTimezone(new Date());
        }
      },
    ...
    methods: {
      ...
      changeToNow() {
        ...
        if ((!this.disabledDate || !this.disabledDate(switchTimezone(new Date()))) && this.checkDateWithinRange(switchTimezone(new Date()))) {
          this.date = switchTimezone(new Date());
          this.emit(this.date);
        }
      },
      ...
      confirm() {
        if (this.selectionMode === 'dates') {
          ...
          const value = this.value
            ? this.value
            : modifyWithTimeString(this.getDefaultValue(), this.defaultTime);
          this.date = switchTimezone(new Date(value)); // refresh date
          this.emit(value);
        }
      },
      ...
      handleKeyControl(keyCode) {
        ...
        const newDate = switchTimezone(new Date(this.date.getTime()));
      ...
      getDefaultValue() {
        ...
        return this.defaultValue ? switchTimezone(new Date(this.defaultValue)) : switchTimezone(new Date());
      },
