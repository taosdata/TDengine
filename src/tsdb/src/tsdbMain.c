...
uint32_t tsdbGetFileInfo(STsdbRepo *repo, char *name, uint32_t *index1, uint32_t eindex, int64_t *size) {
  // TODO
  return 0;
#if 0
  STsdbRepo *pRepo = (STsdbRepo *)repo;
  ...
  tsdbDebug("vgId:%d name:%s index:%d eindex:%d", pRepo->config.tsdbId, name, *index1, eindex);
  ASSERT(*index1 <= eindex);

  if (name[0] == 0) {  // get the file from index or after, but not larger than eindex
    int fid = (*index1) / TSDB_FILE_TYPE_MAX;

    if (pFileH->nFGroups == 0 || fid > pFileH->pFGroup[pFileH->nFGroups - 1].fileId) {
      if (*index1 <= TSDB_META_FILE_INDEX && TSDB_META_FILE_INDEX <= eindex) {
        fname = tsdbGetMetaFileName(pRepo->rootDir);
        *index1 = TSDB_META_FILE_INDEX;
      ...
      if (pFGroup->fileId == fid) {
        SFile *pFile = &pFGroup->files[(*index1) % TSDB_FILE_TYPE_MAX];
        ...
        if ((pFGroup->fileId + 1) * TSDB_FILE_TYPE_MAX - 1 < (int)eindex) {
          ...
          fname = strdup(TSDB_FILE_NAME(pFile));
          *index1 = pFGroup->fileId * TSDB_FILE_TYPE_MAX;
          magic = pFile->info.magic;
          char *tfname = strdup(fname);
          sprintf(name, "tsdb/%s/%s", TSDB_DATA_DIR_NAME, basename(tfname));
          tfree(tfname);
    ...
    if (*index1 == TSDB_META_FILE_INDEX) {  // get meta file
