...
void taosCacheCleanup(SCacheObj *pCacheObj) {
  ...
  // TD-6446 core at if ((count % pCacheObj->checkTick) != 0) when pCacheObj is free. so expand the sleep time to 200s
  for (int i = 0; i < 4000&&atomic_load_8(&pCacheObj->deleting) != 0; i++) {
    taosMsleep(50);
  }
