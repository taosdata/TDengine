...
void* taosArrayGet(const SArray* pArray, size_t index1) {
  assert(index1 < pArray->size);
  return TARRAY_GET_ELEM(pArray, index1);
}

void* taosArrayGetP(const SArray* pArray, size_t index1) {
  assert(index1 < pArray->size);
  
  void* d = TARRAY_GET_ELEM(pArray, index1);
...
void* taosArrayInsert(SArray* pArray, size_t index1, void* pData) {
  if (pArray == NULL || pData == NULL) {
    return NULL;
  }

  if (index1 >= pArray->size) {
  ...
  void* dst = TARRAY_GET_ELEM(pArray, index1);

  int32_t remain = (int32_t)(pArray->size - index1);
...
void taosArraySet(SArray* pArray, size_t index1, void* pData) {
  assert(index1 < pArray->size);
  memcpy(TARRAY_GET_ELEM(pArray, index1), pData, pArray->elemSize);
}

void taosArrayRemove(SArray* pArray, size_t index1) {
  assert(index1 < pArray->size);
  
  if (index1 == pArray->size - 1) {
    taosArrayPop(pArray);
    return;
  }
  
  size_t remain = pArray->size - index1 - 1;
  memmove((char*)pArray->pData + index1 * pArray->elemSize, (char*)pArray->pData + (index1 + 1) * pArray->elemSize, remain * pArray->elemSize);
  pArray->size -= 1;
}
