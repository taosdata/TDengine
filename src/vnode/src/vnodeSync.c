...
uint32_t vnodeGetFileInfo(int32_t vgId, char *name, uint32_t *index1, uint32_t eindex, int64_t *size, uint64_t *fver) {
  ...
  *fver = pVnode->fversion;
  uint32_t ret = tsdbGetFileInfo(pVnode->tsdb, name, index1, eindex, size);
