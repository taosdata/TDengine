...
int32_t tNameSetDbName(SName* dst, const char* acct1, SStrToken* dbToken) {
  assert(dst != NULL && dbToken != NULL && acct1 != NULL);

  // too long account id or too long db name
  if (strlen(acct1) >= tListLen(dst->acctId) || dbToken->n >= tListLen(dst->dbname)) {
    return -1;
  }

  dst->type = TSDB_DB_NAME_T;
  tstrncpy(dst->acctId, acct1, tListLen(dst->acctId));
  tstrncpy(dst->dbname, dbToken->z, dbToken->n + 1);
  return 0;
}

int32_t tNameSetAcctId(SName* dst, const char* acct1) {
  assert(dst != NULL && acct1 != NULL);

  // too long account id or too long db name
  if (strlen(acct1) >= tListLen(dst->acctId)) {
    return -1;
  }
