...
static FORCE_INLINE void dataColSetNullAt(SDataCol *pCol, int index1) {
  if (IS_VAR_DATA_TYPE(pCol->type)) {
    pCol->dataOff[index1] = pCol->len;
    char *ptr = POINTER_SHIFT(pCol->pData, pCol->len);
    setVardataNull(ptr, pCol->type);
    pCol->len += varDataTLen(ptr);
  } else {
    setNull(POINTER_SHIFT(pCol->pData, TYPE_BYTES[pCol->type] * index1), pCol->type, pCol->bytes);
