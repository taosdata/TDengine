...
// todo handle the agg arithmetic expression
  ...
  if (cmd == TSDB_SQL_SELECT) {
    ...
    tscTrace(
        "%p new subquery:%p, tableIndex:%d, vgroupIndex:%d, type:%d, exprInfo:%"PRIu64", colList:%"PRIu64","
...
void tscTryQueryNextVnode(SSqlObj* pSql, __async_cb_func_t fp) {
  ...
  while (++pTableMetaInfo->vgroupIndex < totalVgroups) {
    tscTrace("%p results from vgroup index:%d completed, try next:%d. total vgroups:%d. current numOfRes:%"PRIu64"", pSql,
