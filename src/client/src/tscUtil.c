...
bool tscIsInsertData(char* sqlstr) {
  ...
  do {
    SStrToken t0 = tStrGetToken(sqlstr,  &index1, false);
    if (t0.type != TK_LP) {
...
SInternalField* tscFieldInfoInsert(SFieldInfo* pFieldInfo, int32_t index1, TAOS_FIELD* field) {
  pFieldInfo->numOfOutput++;
  struct SInternalField info = { .pExpr = NULL, .visible = true };

  info.field = *field;
  return taosArrayInsert(pFieldInfo->internalField, index1, &info);
}
...
SInternalField* tscFieldInfoGetInternalField(SFieldInfo* pFieldInfo, int32_t index1) {
  assert(index1 < pFieldInfo->numOfOutput);
  return TARRAY_GET_ELEM(pFieldInfo->internalField, index1);
}

TAOS_FIELD* tscFieldInfoGetField(SFieldInfo* pFieldInfo, int32_t index1) {
  assert(index1 < pFieldInfo->numOfOutput);
  return &((SInternalField*)TARRAY_GET_ELEM(pFieldInfo->internalField, index1))->field;
}

int16_t tscFieldInfoGetOffset(SQueryInfo* pQueryInfo, int32_t index1) {
  SInternalField* pInfo = tscFieldInfoGetInternalField(&pQueryInfo->fieldsInfo, index1);
...
SExprInfo* tscExprInsert(SQueryInfo* pQueryInfo, int32_t index1, int16_t functionId, SColumnIndex* pColIndex, int16_t type,
  ...
  taosArrayInsert(pQueryInfo->exprList, index1, &pExpr);
  return pExpr;
}
...
SExprInfo* tscExprUpdate(SQueryInfo* pQueryInfo, int32_t index1, int16_t functionId, int16_t srcColumnIndex,
                           int16_t type, int16_t size) {
  STableMetaInfo* pTableMetaInfo = tscGetMetaInfo(pQueryInfo, 0);
  SExprInfo* pExpr = tscExprGet(pQueryInfo, index1);
  if (pExpr == NULL) {
    return NULL;
  }
...
bool tscMultiRoundQuery(SQueryInfo* pQueryInfo, int32_t index1) {
  if (!UTIL_TABLE_IS_SUPER_TABLE(pQueryInfo->pTableMetaInfo[index1])) {
    return false;
  }
...
SExprInfo* tscExprGet(SQueryInfo* pQueryInfo, int32_t index1) {
  return taosArrayGetP(pQueryInfo->exprList, index1);
}
...
void tscGetSrcColumnInfo(SSrcColumnInfo* pColInfo, SQueryInfo* pQueryInfo) {
  ...
  for (int32_t i = 0; i < numOfExprs; ++i) {
    ...
    if (TSDB_COL_IS_TAG(pExpr->base.colInfo.flag)) {
      ...
      int16_t index1 = pExpr->base.colInfo.colIndex;
      pColInfo[i].type =  (index1 != -1) ? pTagSchema[index1].type : TSDB_DATA_TYPE_BINARY;
    } else {
...
STableMetaInfo* tscGetTableMetaInfoByUid(SQueryInfo* pQueryInfo, uint64_t uid, int32_t* index1) {
  int32_t k = -1;
...
void tscRemoveVgroupTableGroup(SArray* pVgroupTable, int32_t index1) {
  assert(pVgroupTable != NULL && index1 >= 0);

  size_t size = taosArrayGetSize(pVgroupTable);
  assert(size > index1);

  SVgroupTableInfo* pInfo = taosArrayGet(pVgroupTable, index1);
  taosArrayDestroy(pInfo->itemList);
  taosArrayRemove(pVgroupTable, index1);
}
...
static void tscSubqueryRetrieveCallback(void* param, TAOS_RES* tres, int code) {
  ...
  int32_t index1 = ps->subqueryIndex;
  bool ret = subAndCheckDone(pSql, pParentSql, index1);
  ...
  if (!ret) {
    tscDebug("0x%"PRIx64" sub:0x%"PRIx64" orderOfSub:%d completed, not all subquery finished", pParentSql->self, pSql->self, index1);
    return;
  }
