...
SJoinSupporter* tscCreateJoinSupporter(SSqlObj* pSql, int32_t index1) {
  ...
  pSupporter->subqueryIndex = index1;
  SQueryInfo* pQueryInfo = tscGetQueryInfo(&pSql->cmd);
  
  memcpy(&pSupporter->interval, &pQueryInfo->interval, sizeof(pSupporter->interval));
  pSupporter->limit = pQueryInfo->limit;

  STableMetaInfo* pTableMetaInfo = tscGetTableMetaInfoFromCmd(&pSql->cmd, index1);
...
static int32_t tscLaunchRealSubqueries(SSqlObj* pSql) {
  ...
  for (int32_t i = 0; i < pSql->subState.numOfSub; ++i) {
     ...
     */
    pSupporter->limit = pQueryInfo->limit;
    SColumnIndex index1 = {.tableIndex = 0, .columnIndex = PRIMARYKEY_TIMESTAMP_COL_INDEX};
    ...
    if ((pExpr->base.colInfo.colId != PRIMARYKEY_TIMESTAMP_COL_INDEX) ||
      ...
      tscAddFuncInSelectClause(pQueryInfo, 0, functionId, &index1, s, TSDB_COL_NORMAL, getNewResColId(&pNew->cmd));
...
static void issueTsCompQuery(SSqlObj* pSql, SJoinSupporter* pSupporter, SSqlObj* pParent) {
  ...
  tscAddFuncInSelectClause(pQueryInfo, 0, TSDB_FUNC_TS_COMP, &index1, &colSchema, TSDB_COL_NORMAL, getNewResColId(pCmd));
...
int32_t tscHandleFirstRoundStableQuery(SSqlObj *pSql) {
  ...
  for(int32_t i = 0; i < numOfExprs; ++i) {
    ...
    if (pExpr->base.functionId == TSDB_FUNC_TS && pQueryInfo->interval.interval > 0) {
      ...
      SExprInfo* p = tscAddFuncInSelectClause(pNewQueryInfo, index1++, TSDB_FUNC_TS, &colIndex, schema, TSDB_COL_NORMAL, getNewResColId(pCmd));
      ...
      SExprInfo* p = tscAddFuncInSelectClause(pNewQueryInfo, index1++, TSDB_FUNC_AVG, &colIndex, &schema, TSDB_COL_NORMAL, getNewResColId(pCmd));
      p->base.resColId = pExpr->base.resColId;  // update the result column id
      ...
      SExprInfo* p = tscAddFuncInSelectClause(pNewQueryInfo, index1++, TSDB_FUNC_TAG, &colIndex, schema, TSDB_COL_TAG, getNewResColId(pCmd));
      ...
      for(int32_t k = 0; k < num; ++k) {
        ...
        if (pExpr->base.colInfo.colId == pIndex->colId) {
          ...
          SExprInfo* p = tscAddFuncInSelectClause(pNewQueryInfo, index1++, TSDB_FUNC_PRJ, &colIndex, schema, TSDB_COL_NORMAL, getNewResColId(pCmd));
  ...
  if (pColList != NULL) {
    ...
    for (int32_t i = 0; i < s; ++i) {
      ...
      if (pCol->info.flist.numOfFilters > 0) {  // copy to the pNew->cmd.colList if it is filtered.
        int32_t index2 = tscColumnExists(pNewQueryInfo->colList, pCol->columnIndex, pCol->tableUid);
        if (index2 >= 0) {
          SColumn* x = taosArrayGetP(pNewQueryInfo->colList, index2);
  ...
  tscDebug(
      ...
      tscNumOfExprs(pNewQueryInfo), index1+1, pNewQueryInfo->fieldsInfo.numOfOutput, tNameGetTableName(&pTableMetaInfo->name));
...
char *getArithmeticInputSrc(void *param, const char *name, int32_t colId) {
  ...
  assert(index1 >= 0 && index1 < pSupport->numOfCols);
  return pSupport->data[index1] + pSupport->offset * pExpr->base.resBytes;
}
