...
// -1 is tbname column index1, so here use the -2 as the initial value
#define COLUMN_INDEX_INITIAL_VAL (-2)
#define COLUMN_INDEX_INITIALIZER \
  { COLUMN_INDEX_INITIAL_VAL, COLUMN_INDEX_INITIAL_VAL }
#define COLUMN_INDEX_VALIDE(index1) (((index1).tableIndex >= 0) && ((index1).columnIndex >= TSDB_TBNAME_COLUMN_INDEX))
...
static int convertTimestampStrToInt64(tVariant *pVar, int32_t precision) {
  ...
  if (seg != NULL) {
    if (taosParseTime(pVar->pz, &time1, pVar->nLen, precision, tsDaylight) != TSDB_CODE_SUCCESS) {
      return -1;
    }
  } else {
    if (tVariantDump(pVar, (char*)&time1, TSDB_DATA_TYPE_BIGINT, true)) {
      return -1;
    }
  }
  tVariantDestroy(pVar);
  tVariantCreateFromBinary(pVar, (char*)&time1, 0, TSDB_DATA_TYPE_BIGINT);
  return 0;
}
...
static int32_t addPrimaryTsColumnForTimeWindowQuery(SQueryInfo* pQueryInfo, SSqlCmd* pCmd) {
  ...
  SColumnIndex index1 = {tableIndex, PRIMARYKEY_TIMESTAMP_COL_INDEX};
  tscAddFuncInSelectClause(pQueryInfo, 0, TSDB_FUNC_TS, &index1, &s, TSDB_COL_NORMAL, getNewResColId(pCmd));
  return TSDB_CODE_SUCCESS;
}
...
static int32_t validateStateWindowNode(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SSqlNode* pSqlNode, bool isStable) {
  ...
  if (getColumnIndexByName(col, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) !=  TSDB_CODE_SUCCESS) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
  }

  STableMetaInfo *pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
  STableMeta* pTableMeta = pTableMetaInfo->pTableMeta;
  int32_t numOfCols = tscGetNumOfColumns(pTableMeta);
  if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
  } else if (index1.columnIndex >= numOfCols) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg5);
  }
  ...
  SSchema* pSchema = tscGetTableColumnSchema(pTableMeta, index1.columnIndex);
  ...
  tscColumnListInsert(pQueryInfo->colList, index1.columnIndex, pTableMeta->id.uid, pSchema);
  SColIndex colIndex = { .colIndex = index1.columnIndex, .flag = TSDB_COL_NORMAL, .colId = pSchema->colId };
...
int32_t validateSessionNode(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SSqlNode * pSqlNode) {
  ...
  if (index1.columnIndex != PRIMARYKEY_TIMESTAMP_COL_INDEX) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
  }
...
static int32_t handleArithmeticExpr(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, int32_t exprIndex, tSqlExprItem* pItem) {
  ...
  if (arithmeticType == NORMAL_ARITHMETIC) {
    ...
    SColumnIndex index1 = {.tableIndex = tableIndex};

    SExprInfo* pExpr = tscExprAppend(pQueryInfo, TSDB_FUNC_ARITHM, &index1, TSDB_DATA_TYPE_DOUBLE, sizeof(double),
...
static void addPrimaryTsColIntoResult(SQueryInfo* pQueryInfo, SSqlCmd* pCmd) {
  ...
  SColumnIndex index1 = {0};  // primary timestamp column info
  int32_t numOfCols = (int32_t)tscNumOfExprs(pQueryInfo);
  tscAddFuncInSelectClause(pQueryInfo, numOfCols, TSDB_FUNC_PRJ, &index1, pSchema, TSDB_COL_NORMAL, getNewResColId(pCmd));
...
SExprInfo* doAddProjectCol(SQueryInfo* pQueryInfo, int32_t colIndex, int32_t tableIndex, int32_t colId) {
  ...
  if (functionId == TSDB_FUNC_TAGPRJ) {
    index1.columnIndex = colIndex - tscGetNumOfColumns(pTableMeta);
    tscColumnListInsert(pTableMetaInfo->tagColList, index1.columnIndex, pTableMeta->id.uid, pSchema);
  } else {
    index1.columnIndex = colIndex;
  }

  return tscExprAppend(pQueryInfo, functionId, &index1, pSchema->type, pSchema->bytes, colId, 0,
...
int32_t addProjectionExprAndResultField(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSqlExprItem* pItem, bool outerQuery) {
  ...
  if (tokenId == TK_ALL) {  // project on all fields
    ...
    SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
    if (getTableIndexByName(&pItem->pNode->columnName, pQueryInfo, &index1) != TSDB_CODE_SUCCESS) {
      return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg2);
    }

    // all meters columns are required
    if (index1.tableIndex == COLUMN_INDEX_INITIAL_VAL) {  // all table columns are required.
      for (int32_t i = 0; i < pQueryInfo->numOfTables; ++i) {
        index1.tableIndex = i;
        int32_t inc = doAddProjectionExprAndResultFields(pQueryInfo, &index1, startPos, pCmd);
        startPos += inc;
      }
    } else {
      doAddProjectionExprAndResultFields(pQueryInfo, &index1, startPos, pCmd);
    }

    // add the primary timestamp column even though it is not required by user
    STableMeta* pTableMeta = pQueryInfo->pTableMetaInfo[index1.tableIndex]->pTableMeta;
    ...
    index1.columnIndex = (pQueryInfo->udColumnId--);
    index1.tableIndex = 0;

    SSchema colSchema = tGetUserSpecifiedColumnSchema(&pItem->pNode->value, &pItem->pNode->exprToken, pItem->aliasName);
    SExprInfo* pExpr = tscAddFuncInSelectClause(pQueryInfo, startPos, TSDB_FUNC_PRJ, &index1, &colSchema, TSDB_COL_UDC,
                                                getNewResColId(pCmd));
    ...
    if (getColumnIndexByName(&pItem->pNode->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
      return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg2);
    }
    ...
    if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      if (outerQuery) {
        ...
        SSchema colSchema = pSchema[index1.columnIndex];
        char    name[TSDB_COL_NAME_LEN] = {0};
        getColumnName(pItem, name, colSchema.name, sizeof(colSchema.name) - 1);

        tstrncpy(colSchema.name, name, TSDB_COL_NAME_LEN);
        /*SExprInfo* pExpr = */ tscAddFuncInSelectClause(pQueryInfo, startPos, TSDB_FUNC_PRJ, &index1, &colSchema,
        ...
        tstrncpy(colSchema.name, name, TSDB_COL_NAME_LEN);
        /*SExprInfo* pExpr = */ tscAddFuncInSelectClause(pQueryInfo, startPos, TSDB_FUNC_TAGPRJ, &index1, &colSchema,
      ...
      if (index1.columnIndex >= tscGetNumOfColumns(pTableMeta) && UTIL_TABLE_IS_NORMAL_TABLE(pTableMetaInfo)) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
      }

      addProjectQueryCol(pQueryInfo, startPos, &index1, pItem, getNewResColId(pCmd));
      pQueryInfo->type |= TSDB_QUERY_TYPE_PROJECTION_QUERY;
    }
...
void setResultColName(char* name, tSqlExprItem* pItem, int32_t functionId, SStrToken* pToken, bool multiCols) {
  if (pItem->aliasName != NULL) {
    ...
    char uname1[TSDB_COL_NAME_LEN] = {0};
    int32_t len = MIN(pToken->n + 1, TSDB_COL_NAME_LEN);
    tstrncpy(uname1, pToken->z, len);

    if (tsKeepOriginalColumnName) { // keep the original column name
      tstrncpy(name, uname1, TSDB_COL_NAME_LEN);
    } else {
      ...
      snprintf(tmp, size, "%s(%s)", aAggs[functionId].name, uname1);
...
int32_t addExprAndResultField(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, int32_t colIndex, tSqlExprItem* pItem, bool finalResult,
  ...
  switch (functionId) {
    case TSDB_FUNC_COUNT: {
      ...
      if (pItem->pNode->Expr.paramList != NULL) {
        ...
        if (tokenId == TK_ALL || tokenId == TK_INTEGER) {
          ...
          if (getTableIndexByName(&tmpToken, pQueryInfo, &index1) != TSDB_CODE_SUCCESS) {
            return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg4);
          }

          index1 = (SColumnIndex){0, PRIMARYKEY_TIMESTAMP_COL_INDEX};
          int32_t size = tDataTypes[TSDB_DATA_TYPE_BIGINT].bytes;
          pExpr = tscExprAppend(pQueryInfo, functionId, &index1, TSDB_DATA_TYPE_BIGINT, size, getNewResColId(pCmd), size, false);
        } else {
          // count the number of table created according to the super table
          if (getColumnIndexByName(pToken, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
            return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
          }

          pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);

          // count tag is equalled to count(tbname)
          bool isTag = false;
          if (index1.columnIndex >= tscGetNumOfColumns(pTableMetaInfo->pTableMeta) || index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
            index1.columnIndex = TSDB_TBNAME_COLUMN_INDEX;
            isTag = true;
          }

          int32_t size = tDataTypes[TSDB_DATA_TYPE_BIGINT].bytes;
          pExpr = tscExprAppend(pQueryInfo, functionId, &index1, TSDB_DATA_TYPE_BIGINT, size, getNewResColId(pCmd), size, isTag);
        }
      ...
      SColumnList list = createColumnList(1, index1.tableIndex, index1.columnIndex);
      if (finalResult) {
      ...
      if (index1.tableIndex < tscGetNumOfColumns(pTableMetaInfo->pTableMeta)) {
        tscInsertPrimaryTsSourceColumn(pQueryInfo, pTableMetaInfo->pTableMeta->id.uid);
      }
    ...
    case TSDB_FUNC_LEASTSQR: {
      ...
      SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
      if ((getColumnIndexByName(&pParamElem->pNode->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS)) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
      }
      ...
      if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX || (index1.columnIndex >= tscGetNumOfColumns(pTableMetaInfo->pTableMeta))) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg6);
      }

      // 2. check if sql function can be applied on this column data type
      SSchema* pSchema = tscGetTableColumnSchema(pTableMetaInfo->pTableMeta, index1.columnIndex);
      ...
      if (functionId == TSDB_FUNC_DIFF || functionId == TSDB_FUNC_DERIVATIVE) {
        SColumnIndex indexTS = {.tableIndex = index1.tableIndex, .columnIndex = 0};
      ...
      SExprInfo* pExpr = tscExprAppend(pQueryInfo, functionId, &index1, resultType, resultSize, getNewResColId(pCmd), intermediateResSize, false);
      ...
      SColumnList ids = createColumnList(1, index1.tableIndex, index1.columnIndex);
      ...
      if (!requireAllFields) {
        ...
        for (int32_t i = 0; i < taosArrayGetSize(pItem->pNode->Expr.paramList); ++i) {
          ...
          if (pParamElem->pNode->tokenId == TK_ALL) { // select table.*
            ...
            for (int32_t j = 0; j < tscGetNumOfColumns(pTableMetaInfo->pTableMeta); ++j) {
              ...
              if (setExprInfoForFunctions(pCmd, pQueryInfo, &pSchema[j], cvtFunc, name, colIndex++, &index1,
            ...
            // functions can not be applied to tags
            if ((index1.columnIndex >= tscGetNumOfColumns(pTableMetaInfo->pTableMeta)) || (index1.columnIndex < 0)) {
              return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg6);
            }
            ...
            if (setExprInfoForFunctions(pCmd, pQueryInfo, pSchema, cvtFunc, name, colIndex++, &index1, finalResult, pUdfInfo) != 0) {
              return TSDB_CODE_TSC_INVALID_OPERATION;
            }
          }
        }
        ...
        for (int32_t j = 0; j < pQueryInfo->numOfTables; ++j) {
          ...
          for (int32_t i = 0; i < tscGetNumOfColumns(pTableMetaInfo->pTableMeta); ++i) {
            SColumnIndex index1 = {.tableIndex = j, .columnIndex = i};

            char name[TSDB_COL_NAME_LEN] = {0};
            SStrToken t = {.z = pSchema[i].name, .n = (uint32_t)strnlen(pSchema[i].name, TSDB_COL_NAME_LEN)};
            setResultColName(name, pItem, cvtFunc.originFuncId, &t, true);

            if (setExprInfoForFunctions(pCmd, pQueryInfo, &pSchema[index1.columnIndex], cvtFunc, name, colIndex, &index1,
                finalResult, pUdfInfo) != 0) {
              return TSDB_CODE_TSC_INVALID_OPERATION;
            }
            colIndex++;
    ...
    case TSDB_FUNC_APERCT: {
      ...
      // functions can not be applied to tags
      if (index1.columnIndex >= tscGetNumOfColumns(pTableMetaInfo->pTableMeta)) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg6);
      }
      ...
      if (functionId == TSDB_FUNC_PERCT || functionId == TSDB_FUNC_APERCT) {
        ...
        pExpr = tscExprAppend(pQueryInfo, functionId, &index1, resultType, resultSize, getNewResColId(pCmd), interResult, false);
        ...
        SColumnIndex index2 = {index1.tableIndex, PRIMARYKEY_TIMESTAMP_COL_INDEX};
        pExpr = tscExprAppend(pQueryInfo, TSDB_FUNC_TS, &index2, TSDB_DATA_TYPE_TIMESTAMP, TSDB_KEYSIZE, getNewResColId(pCmd),
                                 0, false);
        tstrncpy(pExpr->base.aliasName, aAggs[TSDB_FUNC_TS].name, sizeof(pExpr->base.aliasName));

        const int32_t TS_COLUMN_INDEX = PRIMARYKEY_TIMESTAMP_COL_INDEX;
        SColumnList   ids = createColumnList(1, index1.tableIndex, TS_COLUMN_INDEX);
        insertResultField(pQueryInfo, colIndex, &ids, TSDB_KEYSIZE, TSDB_DATA_TYPE_TIMESTAMP,
                          aAggs[TSDB_FUNC_TS].name, pExpr);

        colIndex += 1;  // the first column is ts

        pExpr = tscExprAppend(pQueryInfo, functionId, &index1, resultType, resultSize, getNewResColId(pCmd), resultSize, false);
    ...
    case TSDB_FUNC_TID_TAG: {
      ...
      SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
      if (getColumnIndexByName(&pParam->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
      }
      ...
      int32_t numOfCols = tscGetNumOfColumns(pTableMetaInfo->pTableMeta);
      if (index1.columnIndex < numOfCols && index1.columnIndex != TSDB_TBNAME_COLUMN_INDEX) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg6);
      }
    
      if (index1.columnIndex > 0) {
        index1.columnIndex -= numOfCols;
      }
      ...
      if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
        ...
        colType = pSchema[index1.columnIndex].type;
      }
      ...
      tscColumnListInsert(pTableMetaInfo->tagColList, index1.columnIndex, pTableMetaInfo->pTableMeta->id.uid,
                          &pSchema[index1.columnIndex]);
      SSchema* pTagSchema = tscGetTableTagSchema(pTableMetaInfo->pTableMeta);
      ...
      if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
        ...
        s = pTagSchema[index1.columnIndex];
      }
      ...
      tscAddFuncInSelectClause(pQueryInfo, 0, TSDB_FUNC_TID_TAG, &index1, &s, TSDB_COL_TAG, getNewResColId(pCmd));
    ...
    case TSDB_FUNC_BLKINFO: {
      ...
      SColumnIndex index1 = {.tableIndex = 0, .columnIndex = 0,};
      pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
      ...
      SExprInfo* pExpr =
          tscExprInsert(pQueryInfo, 0, TSDB_FUNC_BLKINFO, &index1, resType, bytes, getNewResColId(pCmd), bytes, 0);
    ...
    default: {
      ...
      SExprInfo* pExpr = tscExprAppend(pQueryInfo, functionId, &index1, resType, bytes, getNewResColId(pCmd), inter, false);
      ...
      if (finalResult) {
        ...
        for (int32_t i = 0; i < ids.num; ++i) {
          tscColumnListInsert(pQueryInfo->colList, index1.columnIndex, uid, pSchema);
        }
      }
      tscInsertPrimaryTsSourceColumn(pQueryInfo, pTableMetaInfo->pTableMeta->id.uid);
      return TSDB_CODE_SUCCESS;
...
static SColumnList createColumnList(int32_t num, int16_t tableIndex, int32_t columnIndex) {
  ...
  int32_t index1 = num - 1;
  columnList.ids[index1].tableIndex = tableIndex;
  columnList.ids[index1].columnIndex = columnIndex;
...
static int16_t doGetColumnIndex(SQueryInfo* pQueryInfo, int32_t index1, SStrToken* pToken) {
  STableMeta* pTableMeta = tscGetMetaInfo(pQueryInfo, index1)->pTableMeta;
...
int32_t doGetColumnIndexByName(SStrToken* pToken, SQueryInfo* pQueryInfo, SColumnIndex* pIndex, char* msg) {
  ...
  if (isTablenameToken(pToken)) {
    ...
    // not specify the table name, try to locate the table index1 by column name
    ...
    } else {  // table index1 is valid, get the column index1
...
int32_t validateGroupbyNode(SQueryInfo* pQueryInfo, SArray* pList, SSqlCmd* pCmd) {
  ...
  for (int32_t i = 0; i < num; ++i) {
    ...
    SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
    if (getColumnIndexByName(&token, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
      return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg2);
    }

    if (tableIndex == COLUMN_INDEX_INITIAL_VAL) {
      tableIndex = index1.tableIndex;
    } else if (tableIndex != index1.tableIndex) {
      return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg3);
    }
    ...
    if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      pSchema = tscGetTableColumnSchema(pTableMeta, index1.columnIndex);
    }

    int32_t numOfCols = tscGetNumOfColumns(pTableMeta);
    bool groupTag = (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX || index1.columnIndex >= numOfCols);
    ...
    if (groupTag) {
      ...
      int32_t relIndex = index1.columnIndex;
      if (index1.columnIndex != TSDB_TBNAME_COLUMN_INDEX) {
        relIndex -= numOfCols;
      }
...
static int32_t getColumnQueryCondInfo(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSqlExpr* pExpr, int32_t relOptr) {
  ...
  if (!tSqlExprIsParentOfLeaf(pExpr)) {  // internal node
    ...
    SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
    if (getColumnIndexByName(&pExpr->pLeft->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
      return TSDB_CODE_TSC_INVALID_OPERATION;
    }

    return extractColumnFilterInfo(pCmd, pQueryInfo, &index1, pExpr, relOptr);
  }
}
...
static int32_t checkAndSetJoinCondInfo(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSqlExpr* pExpr) {
  ...
  STableMetaInfo* pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
  SSchema* pTagSchema1 = tscGetTableColumnSchema(pTableMetaInfo->pTableMeta, index1.columnIndex);

  assert(index1.tableIndex >= 0 && index1.tableIndex < TSDB_MAX_JOIN_TABLE_NUM);

  SJoinNode **leftNode = &pQueryInfo->tagCond.joinInfo.joinTables[index1.tableIndex];
  if (*leftNode == NULL) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
  }
  ...
  if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
    ...
    index1.columnIndex = index1.columnIndex - tscGetNumOfColumns(pTableMetaInfo->pTableMeta);
    if (tscColumnExists(pTableMetaInfo->tagColList, pTagSchema1->colId, pTableMetaInfo->pTableMeta->id.uid) < 0) {
      tscColumnListInsert(pTableMetaInfo->tagColList, index1.columnIndex, pTableMeta->id.uid, pTagSchema1);
  ...
  int16_t leftIdx = index1.tableIndex;

  index1 = (SColumnIndex)COLUMN_INDEX_INITIALIZER;
  if (getColumnIndexByName(&pExpr->pRight->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg4);
  }

  pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
  SSchema* pTagSchema2 = tscGetTableColumnSchema(pTableMetaInfo->pTableMeta, index1.columnIndex);

  assert(index1.tableIndex >= 0 && index1.tableIndex < TSDB_MAX_JOIN_TABLE_NUM);

  SJoinNode **rightNode = &pQueryInfo->tagCond.joinInfo.joinTables[index1.tableIndex];
  if (*rightNode == NULL) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
  }
  ...
  if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
    STableMeta* pTableMeta = pTableMetaInfo->pTableMeta;
    index1.columnIndex = index1.columnIndex - tscGetNumOfColumns(pTableMeta);
    if (tscColumnExists(pTableMetaInfo->tagColList, pTagSchema2->colId, pTableMeta->id.uid) < 0) {

      tscColumnListInsert(pTableMetaInfo->tagColList, index1.columnIndex, pTableMeta->id.uid, pTagSchema2);
      atomic_add_fetch_32(&pTableMetaInfo->joinTagNum, 1);
  ...
  int16_t rightIdx = index1.tableIndex;
...
static int32_t validateSQLExpr(SSqlCmd* pCmd, tSqlExpr* pExpr, SQueryInfo* pQueryInfo, SColumnList* pList,
  ...
  if (pExpr->type == SQL_NODE_TABLE_COLUMN) {
    ...
    SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
    if (getColumnIndexByName(&pExpr->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
      return TSDB_CODE_TSC_INVALID_OPERATION;
    }

    // if column is timestamp, bool, binary, nchar, not support arithmetic, so return invalid sql
    STableMeta* pTableMeta = tscGetMetaInfo(pQueryInfo, index1.tableIndex)->pTableMeta;
    SSchema*    pSchema = tscGetTableSchema(pTableMeta) + index1.columnIndex;
...
static int32_t validateLikeExpr(tSqlExpr* pExpr, STableMeta* pTableMeta, int32_t index1, char* msgBuf) {
  ...
  if (pExpr->tokenId == TK_LIKE) {
    ...
    SSchema* pSchema = tscGetTableSchema(pTableMeta);
    if ((!isTablenameToken(&pLeft->columnName)) && !IS_VAR_DATA_TYPE(pSchema[index1].type)) {
...
static int32_t handleExprInQueryCond(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSqlExpr** pExpr, SCondExpr* pCondExpr,
  ...
  SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
  if (getColumnIndexByName(&pLeft->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
    return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg2);
  }

  *tbIdx = index1.tableIndex;
  ...
  code = validateLikeExpr(*pExpr, pTableMeta, index1.columnIndex, tscGetErrorMsgPayload(pCmd));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }

  SSchema* pSchema = tscGetTableColumnSchema(pTableMeta, index1.columnIndex);
  if (pSchema->type == TSDB_DATA_TYPE_TIMESTAMP && index1.columnIndex == PRIMARYKEY_TIMESTAMP_COL_INDEX) {  // query on time range
    if (!validateJoinExprNode(pCmd, pQueryInfo, *pExpr, &index1)) {
      return TSDB_CODE_TSC_INVALID_OPERATION;
    }
    ...
    if (pRight->tokenId == TK_ID) {  // no need to keep the timestamp join condition
      ...
      if (getColumnIndexByName(&pRight->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg2);
      }

      if (index1.tableIndex < 0 || index1.tableIndex >= TSDB_MAX_JOIN_TABLE_NUM) {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg4);
      }
    ...
    *type = TSQL_EXPR_TS;
  } else if (index1.columnIndex >= tscGetNumOfColumns(pTableMeta) || index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
    ...
    if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      if (pCondExpr->pTableCond == NULL) {
        ...
        pCondExpr->tableCondIndex = index1.tableIndex;
      } else {
        return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg6);
      }
...
static void doExtractExprForSTable(SSqlCmd* pCmd, tSqlExpr** pExpr, SQueryInfo* pQueryInfo, tSqlExpr** pOut, int32_t tableIndex) {
  if (tSqlExprIsParentOfLeaf(*pExpr)) {
    ...
    if (index1.tableIndex != tableIndex) {
      return;
    }
...
static void doAddJoinTagsColumnsIntoTagList(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SCondExpr* pCondExpr) {
  ...
  if (QUERY_IS_JOIN_QUERY(pQueryInfo->type) && UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
    ...
    if (getColumnIndexByName(pCmd, &pCondExpr->pJoinExpr->pLeft->ColName, pQueryInfo, &index1) != TSDB_CODE_SUCCESS) {
    ...
    SSchema* pSchema = tscGetTableTagSchema(pTableMetaInfo->pTableMeta);
    tscColumnListInsert(pTableMetaInfo->tagColList, &index1, &pSchema[index1.columnIndex]);
  
    if (getColumnIndexByName(pCmd, &pCondExpr->pJoinExpr->pRight->ColName, pQueryInfo, &index1) != TSDB_CODE_SUCCESS) {
...
static int32_t getTagQueryCondExpr(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SCondExpr* pCondExpr, tSqlExpr** pExpr) {
  ...
  for (int32_t i = 0; i < pQueryInfo->numOfTables; ++i) {
    ...
    for(int32_t j = 0; j < num; ++j) {
      ...
      SColumnIndex index1 = {.tableIndex = i, .columnIndex = pIndex->colIndex - numOfCols};
...
int32_t validateOrderbyNode(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SSqlNode* pSqlNode, SSchema* pSchema) {
  ...
  if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {  // super table query
    if (getColumnIndexByName(&columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
    ...
    if (index1.columnIndex >= tscGetNumOfColumns(pTableMetaInfo->pTableMeta)) {
      int32_t relTagIndex = index1.columnIndex - tscGetNumOfColumns(pTableMetaInfo->pTableMeta);
    ...
    } else if (index1.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      orderByTags = true;
    }

    if (PRIMARYKEY_TIMESTAMP_COL_INDEX == index1.columnIndex) {
      orderByTS = true;
    }
    ...
    if (columnInfo != NULL && taosArrayGetSize(columnInfo) > 0) {
      ...
      if (PRIMARYKEY_TIMESTAMP_COL_INDEX != index1.columnIndex && pColIndex->colIndex == index1.columnIndex) {
    ...
    if (s == 1) {
      if (orderByTags) {
        pQueryInfo->groupbyExpr.orderIndex = index1.columnIndex - tscGetNumOfColumns(pTableMetaInfo->pTableMeta);
        ...
        pQueryInfo->groupbyExpr.orderType = p1->sortOrder;
        pQueryInfo->order.orderColId = pSchema[index1.columnIndex].colId;
        if (udf) {
        ...
        if (pExpr->base.colInfo.colIndex != index1.columnIndex && index1.columnIndex != PRIMARYKEY_TIMESTAMP_COL_INDEX) {
          return invalidOperationMsg(pMsgBuf, msg2);
        }
      ...
      if (orderByTags) {
        ...
        pQueryInfo->order.orderColId = index1.columnIndex;
        if (udf) {
          return invalidOperationMsg(pMsgBuf, msg11);
        }
      } else {
      ...
      if (getColumnIndexByName(&cname, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
        return invalidOperationMsg(pMsgBuf, msg1);
      }
    ...
    if (getColumnIndexByName(&columnName, pQueryInfo, &index1, pMsgBuf) != TSDB_CODE_SUCCESS) {
      return invalidOperationMsg(pMsgBuf, msg1);
    }

    if (index1.columnIndex != PRIMARYKEY_TIMESTAMP_COL_INDEX && !isTopBottomQuery(pQueryInfo)) {
      bool validOrder = false;
      SArray *columnInfo = pQueryInfo->groupbyExpr.columnInfo;
      if (columnInfo != NULL && taosArrayGetSize(columnInfo) > 0) {
        SColIndex* pColIndex = taosArrayGet(columnInfo, 0);
        validOrder = (pColIndex->colIndex == index1.columnIndex);
      }
      ...
      tVariantListItem* p1 = taosArrayGet(pSqlNode->pSortOrder, 0);
      pQueryInfo->groupbyExpr.orderIndex = pSchema[index1.columnIndex].colId;
      pQueryInfo->groupbyExpr.orderType = p1->sortOrder;
    }
...
int32_t setAlterTableInfo(SSqlObj* pSql, struct SSqlInfo* pInfo) {
  ...
  if (pAlterSQL->type == TSDB_ALTER_TABLE_ADD_TAG_COLUMN) {
    ...
    if (getColumnIndexByName(&name, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS) {
      return TSDB_CODE_TSC_INVALID_OPERATION;
    }

    int32_t numOfCols = tscGetNumOfColumns(pTableMeta);
    if (index1.columnIndex < numOfCols) {
...
void addGroupInfoForSubquery(SSqlObj* pParentObj, SSqlObj* pSql, int32_t subClauseIndex, int32_t tableIndex) {
  ...
  if (pParentQueryInfo->groupbyExpr.numOfGroupCols > 0) {
    ...
    if (pExpr == NULL || pExpr->base.functionId != TSDB_FUNC_TAG) {
      ...
      SColumnIndex index1 = {.tableIndex = 0, .columnIndex = colIndex};

      char*   name = pTagSchema->name;
      int16_t type = pTagSchema->type;
      int16_t bytes = pTagSchema->bytes;

      pExpr = tscExprAppend(pQueryInfo, TSDB_FUNC_TAG, &index1, type, bytes, getNewResColId(&pSql->cmd), bytes, true);
...
static int32_t doAddGroupbyColumnsOnDemand(SSqlCmd* pCmd, SQueryInfo* pQueryInfo) {
  ...
  for (int32_t i = 0; i < pQueryInfo->groupbyExpr.numOfGroupCols; ++i) {
    ...
    if (TSDB_COL_IS_TAG(pColIndex->flag)) {
      ...
      SColumnIndex index1 = {.tableIndex = pQueryInfo->groupbyExpr.tableIndex, .columnIndex = colIndex};
      SExprInfo*   pExpr = tscExprInsert(pQueryInfo, pos, f, &index1, s->type, s->bytes, getNewResColId(pCmd), s->bytes, true);
...
int32_t doLocalQueryProcess(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SSqlNode* pSqlNode) {
  ...
  const char* name = (item->aliasName != NULL)? item->aliasName:functionsInfo[index1].name;
...
int32_t tscGetExprFilters(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SArray* pSelectNodeList, tSqlExpr* pSqlExpr, SExprInfo** pExpr) {
  ...
  *pExpr = NULL;
  ...
  if (pSqlExpr->Expr.paramList != NULL) {
    ...
    SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
    getColumnIndexByName(pToken, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd));
    STableMetaInfo* pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
    schema = *tscGetTableColumnSchema(pTableMetaInfo->pTableMeta, index1.columnIndex);
  } else {
...
//        }
//
//        if (pExpr1->base.colInfo.colIndex != index1.columnIndex) {
//          continue;
//        }
//
//        ++pQueryInfo->havingFieldNum;
  ...
  if (pLeft->Expr.paramList) {
    ...
    for (int32_t i = 0; i < size; i++) {
      ...
      if (pExpr1->tokenId == TK_ID) {
        SColumnIndex index1 = COLUMN_INDEX_INITIALIZER;
        if ((getColumnIndexByName(&pExpr1->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd)) != TSDB_CODE_SUCCESS)) {
          return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
        }
        ...
        if (index1.columnIndex <= 0 ||
            index1.columnIndex >= tscGetNumOfColumns(pTableMeta)) {
          return invalidOperationMsg(tscGetErrorMsgPayload(pCmd), msg1);
        }
      }
    }
...
static int32_t doValidateSubquery(SSqlNode* pSqlNode, int32_t index1, SSqlObj* pSql, SQueryInfo* pQueryInfo, char* msgBuf) {
  SRelElementPair* subInfo = taosArrayGet(pSqlNode->from->list, index1);
  ...
  if (pSqlExpr->pLeft == NULL) {  // it is the leaf node
    ...
    if (pSqlExpr->type == SQL_NODE_VALUE) {
      ...
      *pExpr = calloc(1, sizeof(tExprNode));
      ...
      int32_t ret = getColumnIndexByName(&pSqlExpr->columnName, pQueryInfo, &index1, tscGetErrorMsgPayload(pCmd));
      if (ret != TSDB_CODE_SUCCESS) {
        return ret;
      }

      pQueryInfo->curTableIdx = index1.tableIndex;
      STableMeta* pTableMeta = tscGetMetaInfo(pQueryInfo, index1.tableIndex)->pTableMeta;
      ...
      *pExpr = calloc(1, sizeof(tExprNode));
      ...
      if (pCols != NULL) {  // record the involved columns
        ...
        colIndex.colIndex = index1.columnIndex;
        colIndex.flag = (index1.columnIndex >= numOfColumns)? 1:0;
