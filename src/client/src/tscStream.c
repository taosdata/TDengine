  ...
  if (numOfRows > 0) { // when reaching here the first execution of stream computing is successful.
    ...
    if (pStream->numOfRes == 0) {
      if (pQueryInfo->fillType == TSDB_FILL_SET_VALUE || pQueryInfo->fillType == TSDB_FILL_NULL) {
        ...
        tscError("%p stream:%p, retrieve no data, code:%d, retry in %dms", pSql, pStream, numOfRows, retry);
    ...
    tscTrace("%p stream:%p, query on:%s, fetch result completed, fetched rows:%"PRIu64"", pSql, pStream, pTableMetaInfo->name,
             pStream->numOfRes);
