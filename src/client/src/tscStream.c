...
static void tscSetNextLaunchTimer(SSqlStream *pStream, SSqlObj *pSql) {
  ...
  if (pStream->isProject) {
    ...
    int64_t stime1 = taosTimeTruncate(pStream->stime - 1, &pStream->interval, pStream->precision);
    if (stime1 >= pStream->etime) {
...
static int64_t tscGetStreamStartTimestamp(SSqlObj *pSql, SSqlStream *pStream, int64_t stime1) {
  SQueryInfo* pQueryInfo = tscGetQueryInfo(&pSql->cmd);
  ...
  if (pStream->isProject) {
    ...
    if (stime1 != INT64_MIN) {  // first projection start from the latest event timestamp
      assert(stime1 >= pQueryInfo->window.skey);
      stime1 += 1;  // exclude the last records from table
    } else {
      stime1 = pQueryInfo->window.skey;
    }
  } else {             // timewindow based aggregation stream
    if (stime1 == INT64_MIN) {  // no data in meter till now
      ...
      stime1 = taosTimeTruncate(stime1, &pStream->interval, pStream->precision);
    } else {
      int64_t newStime = taosTimeTruncate(stime1, &pStream->interval, pStream->precision);
      if (newStime != stime1) {
        tscWarn("0x%"PRIx64" stream:%p, last timestamp:%" PRId64 ", reset to:%" PRId64, pSql->self, pStream, stime1, newStime);
        stime1 = newStime;
      }
    }
  }
...
TAOS_STREAM *taos_open_stream_withname(TAOS *taos, const char* dstTable, const char *sqlstr, void (*fp)(void *param, TAOS_RES *, TAOS_ROW row),
                              int64_t stime1, void *param, void (*callback)(void *), void* cqhandle) {
...
TAOS_STREAM *taos_open_stream(TAOS *taos, const char *sqlstr, void (*fp)(void *param, TAOS_RES *, TAOS_ROW row),
                              int64_t stime1, void *param, void (*callback)(void *)) {  
  return taos_open_stream_withname(taos, "", sqlstr, fp, stime1, param, callback, NULL);
}
