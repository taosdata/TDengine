...
static int normalStmtBindParam(STscStmt* stmt, TAOS_BIND* pBind) {
  ...
  for (uint16_t i = 0; i < normal->numParams; ++i) {
    ...
    switch (tb->buffer_type) {
      ...
      default:
        tscError("0x%"PRIx64" pBind column%d: type mismatch or invalid", stmt->pSql->self, i);
        return invalidOperationMsg(tscGetErrorMsgPayload(&stmt->pSql->cmd), "pBind type mismatch or invalid");
    }
  }
    ...
    switch (param->type) {
      case TSDB_DATA_TYPE_BOOL: {
        switch (pBind->buffer_type) {
          ...
          case TSDB_DATA_TYPE_SMALLINT: {
            u.v1 = (int8_t)*(int16_t*)pBind->buffer;
            if (u.v1==0 || u.v1==1) break;
          } break;
          case TSDB_DATA_TYPE_INT: {
            u.v1 = (int8_t)*(int32_t*)pBind->buffer;
            if (u.v1==0 || u.v1==1) break;
          } break;
          case TSDB_DATA_TYPE_BIGINT: {
            u.v1 = (int8_t)*(int64_t*)pBind->buffer;
            if (u.v1==0 || u.v1==1) break;
          } break;
          case TSDB_DATA_TYPE_BINARY:
          case TSDB_DATA_TYPE_NCHAR: {
            // "0", "1" convertible
            if (strncmp((const char*)pBind->buffer, "0", *pBind->length)==0) {
              u.v1 = 0;
              break;
            }
            if (strncmp((const char*)pBind->buffer, "1", *pBind->length)==0) {
              u.v1 = 1;
              break;
            }
      ...
      case TSDB_DATA_TYPE_TINYINT: {
        switch (pBind->buffer_type) {
          case TSDB_DATA_TYPE_BOOL:
          case TSDB_DATA_TYPE_TINYINT: {
            int8_t v = *(int8_t*)pBind->buffer;
            u.v1 = v;
            if (v >= SCHAR_MIN && v <= SCHAR_MAX) break;
          } break;
          case TSDB_DATA_TYPE_SMALLINT: {
            int16_t v = *(int16_t*)pBind->buffer;
            u.v1 = (int8_t)v;
            if (v >= SCHAR_MIN && v <= SCHAR_MAX) break;
            return TSDB_CODE_TSC_INVALID_VALUE;
          }
          case TSDB_DATA_TYPE_INT: {
            int32_t v = *(int32_t*)pBind->buffer;
            u.v1 = (int8_t)v;
            if (v >= SCHAR_MIN && v <= SCHAR_MAX) break;
            return TSDB_CODE_TSC_INVALID_VALUE;
          }
          case TSDB_DATA_TYPE_BIGINT: {
            int64_t v = *(int64_t*)pBind->buffer;
            u.v1 = (int8_t)v;
            if (v >= SCHAR_MIN && v <= SCHAR_MAX) break;
            return TSDB_CODE_TSC_INVALID_VALUE;
          }
          ...
          case TSDB_DATA_TYPE_NCHAR: {
            int64_t v;
            int     n, r;
            r = sscanf((const char*)pBind->buffer, "%" PRId64 "%n", &v, &n);
            if (r == 1 && n == strlen((const char*)pBind->buffer)) {
              u.v1 = (int8_t)v;
              if (v >= SCHAR_MIN && v <= SCHAR_MAX) break;
            }
            return TSDB_CODE_TSC_INVALID_VALUE;
      ...
      case TSDB_DATA_TYPE_FLOAT: {
        switch (pBind->buffer_type) {
          case TSDB_DATA_TYPE_BOOL:
          case TSDB_DATA_TYPE_TINYINT: {
            u.f4 = *(int8_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_SMALLINT: {
            u.f4 = *(int16_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_INT: {
            u.f4 = (float)*(int32_t*)pBind->buffer;
            // shall we check equality?
          } break;
          case TSDB_DATA_TYPE_BIGINT: {
            u.f4 = (float)*(int64_t*)pBind->buffer;
            // shall we check equality?
          } break;
          case TSDB_DATA_TYPE_FLOAT: {
            u.f4 = *(float*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_DOUBLE: {
            u.f4 = *(float*)pBind->buffer;
            // shall we check equality?
          ...
          case TSDB_DATA_TYPE_NCHAR: {
            float v;
            int n,r;
            r = sscanf((const char*)pBind->buffer, "%f%n", &v, &n);
            if (r==1 && n==strlen((const char*)pBind->buffer)) {
              u.f4 = v;
              break;
            }
      ...
      case TSDB_DATA_TYPE_BIGINT: {
        switch (pBind->buffer_type) {
          case TSDB_DATA_TYPE_BOOL:
          case TSDB_DATA_TYPE_TINYINT: {
            u.v8 = *(int8_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_SMALLINT: {
            u.v8 = *(int16_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_INT: {
            u.v8 = *(int32_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_BIGINT: {
            u.v8 = *(int64_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_BINARY:
          case TSDB_DATA_TYPE_NCHAR: {
            int64_t v;
      ...
      case TSDB_DATA_TYPE_DOUBLE: {
        switch (pBind->buffer_type) {
          case TSDB_DATA_TYPE_BOOL:
          case TSDB_DATA_TYPE_TINYINT: {
            u.f8 = *(int8_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_SMALLINT: {
            u.f8 = *(int16_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_INT: {
            u.f8 = *(int32_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_BIGINT: {
            u.f8 = (double)*(int64_t*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_FLOAT: {
            u.f8 = *(float*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_DOUBLE: {
            u.f8 = *(double*)pBind->buffer;
          } break;
          case TSDB_DATA_TYPE_BINARY:
          case TSDB_DATA_TYPE_NCHAR: {
            double v;
            int n,r;
            r = sscanf((const char*)pBind->buffer, "%lf%n", &v, &n);
            if (r==1 && n==strlen((const char*)pBind->buffer)) {
              u.f8 = v;
              break;
            }
      ...
      case TSDB_DATA_TYPE_TIMESTAMP: {
        switch (pBind->buffer_type) {
          ...
          case TSDB_DATA_TYPE_NCHAR: {
            ...
            int32_t len = (int32_t)*pBind->length;
            if (taosParseTime(pBind->buffer, &u.v8, len, 3, tsDaylight) == TSDB_CODE_SUCCESS) {
              break;
            }
            return TSDB_CODE_TSC_INVALID_VALUE;
          } break;
...
static int insertStmtBindParam(STscStmt* stmt, TAOS_BIND* pBind) {
  ...
  for (uint32_t j = 0; j < pBlock->numOfParams; ++j) {
    ...
    if (code != TSDB_CODE_SUCCESS) {
      tscDebug("0x%"PRIx64" pBind column %d: type mismatch or invalid", pStmt->pSql->self, param->idx);
      return invalidOperationMsg(tscGetErrorMsgPayload(&stmt->pSql->cmd), "pBind column type mismatch or invalid");
    }
  }
  ...
  if (colIdx == -1) {
    for (uint32_t j = 0; j < pBlock->numOfParams; ++j) {
      ...
      if (pBind[param->idx].num != rowNum) {
        ...
        return invalidOperationMsg(tscGetErrorMsgPayload(&stmt->pSql->cmd), "pBind row num mismatch");
      }

      int code = doBindBatchParam(pBlock, param, &pBind[param->idx], pCmd->batchSize);
      if (code != TSDB_CODE_SUCCESS) {
        tscError("0x%"PRIx64" pBind column %d: type mismatch or invalid", pStmt->pSql->self, param->idx);
        return invalidOperationMsg(tscGetErrorMsgPayload(&stmt->pSql->cmd), "pBind column type mismatch or invalid");
      }
    }
...
static int insertStmtExecute(STscStmt* stmt) {
  SSqlCmd* pCmd = &stmt->pSql->cmd;
  if (pCmd->batchSize == 0) {
    tscError("no records pBind");
    return invalidOperationMsg(tscGetErrorMsgPayload(&stmt->pSql->cmd), "no records pBind");
  }
...
int taos_stmt_bind_param(TAOS_STMT* stmt, TAOS_BIND* pBind) {
  ...
  if (pStmt->isInsert) {
    ...
    tscDebug("tableId:%" PRIu64 ", try to pBind one row", pStmt->mtb.currentUid);
...
int taos_stmt_bind_single_param_batch(TAOS_STMT* stmt, TAOS_MULTI_BIND* pBind, int colIdx) {
  ...
  if (pBind == NULL || pBind->num <= 0 || pBind->num > INT16_MAX || colIdx < 0) {
    tscError("0x%"PRIx64" invalid parameter", pStmt->pSql->self);
    STMT_RET(invalidOperationMsg(tscGetErrorMsgPayload(&pStmt->pSql->cmd), "invalid bind param"));
  }
