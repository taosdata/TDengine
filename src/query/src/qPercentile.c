...
int32_t tBucketIntHash(tMemBucket *pBucket, const void *value) {
  ...
  if (span < pBucket->numOfSlots) {
    ...
    index1 = (delta % pBucket->numOfSlots);
  } else {
    double slotSpan = (double)span / pBucket->numOfSlots;
    index1 = (int32_t)(((double)v - pBucket->range.i64MinVal) / slotSpan);
    if (index1 == pBucket->numOfSlots) {
      index1 -= 1;
    }
  }

  assert(index1 >= 0 && index1 < pBucket->numOfSlots);
  return index1;
}
...
int32_t tBucketUintHash(tMemBucket *pBucket, const void *value) {
  ...
  if (span < pBucket->numOfSlots) {
    ...
    index1 = (int32_t) (delta % pBucket->numOfSlots);
  } else {
    double slotSpan = (double)span / pBucket->numOfSlots;
    index1 = (int32_t)((v - pBucket->range.u64MinVal) / slotSpan);
...
int32_t tBucketDoubleHash(tMemBucket *pBucket, const void *value) {
  ...
  if (span < pBucket->numOfSlots) {
    ...
    index1 = (int32_t)((v - pBucket->range.dMinVal) / slotSpan);
...
int32_t tMemBucketPut(tMemBucket *pBucket, const void *data, size_t size) {
  ...
  for (int32_t i = 0; i < size; ++i) {
    ...
    int32_t index1 = (pBucket->hashFunc)(pBucket, d);
    if (index1 < 0) {
      continue;
    }
    ...
    tMemBucketSlot *pSlot = &pBucket->pSlots[index1];
    tMemBucketUpdateBoundingBox(&pSlot->range, d, pBucket->type);

    // ensure available memory pages to allocate
    int32_t groupId = getGroupId(pBucket->numOfSlots, index1, pBucket->times);
    int32_t pageId = -1;
...
static bool isIdenticalData(tMemBucket *pMemBucket, int32_t index1);
 ...
 */
bool isIdenticalData(tMemBucket *pMemBucket, int32_t index1) {
  tMemBucketSlot *pSeg = &pMemBucket->pSlots[index1];
