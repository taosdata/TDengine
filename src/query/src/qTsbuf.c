...
static void TSBufUpdateGroupInfo(STSBuf* pTSBuf, int32_t index1, STSGroupBlockInfo* pBlockInfo);
...
int32_t tsBufMerge(STSBuf* pDestBuf, const STSBuf* pSrcBuf) {
  ...
  // src can only have one vnode index1
  ...
  if (id != tsBufGetLastGroupInfo(pDestBuf)->info.id) {
    ...
    // directly copy the vnode index1 information
...
// update prev vnode length info in file
static void TSBufUpdateGroupInfo(STSBuf* pTSBuf, int32_t index1, STSGroupBlockInfo* pBlockInfo) {
  int32_t offset = sizeof(STSBufFileHeader) + index1 * sizeof(STSGroupBlockInfo);
