...
static void doApplyFunctions(SQueryRuntimeEnv* pRuntimeEnv, SQLFunctionCtx* pCtx, STimeWindow* pWin, int32_t offset,
  ...
  for (int32_t k = 0; k < numOfOutput; ++k) {
    ...
    // keep it temporarily
    char *start, *udfStart[2];

    if (pCtx[k].functionId < 0) {
      udfStart[0] = (char *)pCtx[k].pUdfInput[0];
      udfStart[1] = (char *)pCtx[k].pUdfInput[1];
    } else {
      start = pCtx[k].pInput;
    }
    ...
    if (pCtx[k].functionId < 0) {
      if (pCtx[k].pUdfInput[0] != NULL) {
        pCtx[k].pUdfInput[0] = (char *)pCtx[k].pUdfInput[0] + pos * pCtx[k].udfInputBytes[0];
      }

      if (pCtx[k].pUdfInput[1] != NULL) {
        pCtx[k].pUdfInput[1] = (char *)pCtx[k].pUdfInput[1] + pos * pCtx[k].udfInputBytes[1];
      }
    } else {
    ...
    if (pCtx[k].functionId < 0) {
      pCtx[k].pUdfInput[0] = (void *)udfStart[0];
      pCtx[k].pUdfInput[1] = (void *)udfStart[1];
    } else {
      pCtx[k].pInput = start;
    }
  }
