...
static SQueryNode* doAddTableColumnNode(SQueryInfo* pQueryInfo, STableMetaInfo* pTableMetaInfo, SQueryTableInfo* info,
  ...
  if (pQueryInfo->projectionQuery) {
    ...
    for (int32_t i = 0; i < numOfCols; ++i) {
      ...
      SColumnIndex index1 = {.tableIndex = 0, .columnIndex = pCol->columnIndex};
      STableMetaInfo* pTableMetaInfo1 = tscGetMetaInfo(pQueryInfo, index1.tableIndex);
      SExprInfo*   p = tscExprCreate(pTableMetaInfo1, TSDB_FUNC_PRJ, &index1, pCol->info.type, pCol->info.bytes,
