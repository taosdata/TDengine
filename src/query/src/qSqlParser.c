...
void setCreateFuncInfo(SSqlInfo *pInfo, int32_t type, SStrToken *pName, SStrToken *pPath, TAOS_FIELD *output, SStrToken *bufSize, SStrToken *numOfParams, int32_t funcType) {
  pInfo->type = type;
  if (pInfo->pMiscInfo == NULL) {
    pInfo->pMiscInfo = calloc(1, sizeof(SMiscInfo));
  }
  ...
  if (numOfParams->n > 0) {
    pInfo->pMiscInfo->funcOpt.numOfParams = strtol(numOfParams->z, NULL, 10);
  } else {
    pInfo->pMiscInfo->funcOpt.numOfParams = 1;
  }
}
