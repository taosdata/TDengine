  ...
  if (pFillInfo->type == TSDB_FILL_PREV) {
    ...
    if (prev != NULL && !outOfBound) {
      for (int32_t i = 1; i < pFillInfo->numOfCols; ++i) {
        ...
        taosGetLinearInterpolationVal(&point, type, &point1, &point2, type, &exceedMax);
      }
    } else {
      setNullValueForRow(pFillInfo, data, pFillInfo->numOfCols, index);
    }
...
int32_t taosGetLinearInterpolationVal(SPoint* point, int32_t outputType, SPoint* point1, SPoint* point2, int32_t inputType, bool *exceedMax) {
  double v1 = -1, v2 = -1, vmax = -1;
