<template>
  <div class="tbCreate">
    ...
    <div class="formWrapper">
      <el-form ref="table_form" label-position="left" :rules="rules" :label-width="table_form.stbTmpl ? '170px' : '130px'" :model="table_form">
        <!-- Name -->
        <el-form-item prop="name" class="name_input">
          ...
          <el-input size="small" maxlength="192" :title="table_form.name" :disabled="isEdit" v-model="table_form.name" />
        </el-form-item>
        <template v-if="!table_form.stbTmpl">
          ...
          <div v-if="!isColumnsFold">
            ...
            <div v-for="(column, index) in table_form.columns" class="flexCenter input_row" :key="'column' + index">
              ...
              <el-input size="small" v-model="column.field" :placeholder="$t('data.columnNameTip')" :disabled="isEdit">
                <template slot="append">
                  <el-button icon="el-icon-minus" @click="minusColumn(index, column)"></el-button>
                  <el-button icon="el-icon-plus" v-if="!isEdit" @click="addColumn(index)"></el-button>
                  <el-button v-if="isEdit" icon="el-icon-check" @click="columnTypeChange(column)"></el-button>
                </template>
              </el-input>
            </div>
            <!-- 添加用的column -->
            <div class="flexCenter input_row" v-if="columnEdit && isEdit">
              ...
              <el-input size="small" v-model="currentData.field" :placeholder="$t('data.columnNameTip')">
                <template slot="append">
                  <el-button
                    icon="el-icon-close"
                    @click="
            ...
            <el-button v-if="isEdit" class="add-btn" size="small" plain icon="el-icon-plus" @click="addColumn()"></el-button>
          </div>
        </template>
        <template v-else>
          <!-- Tag Section -->
          ...
          <div v-if="!isTagsFold">
            <div v-for="(tag, index) in table_form.tags" :key="'tag' + index">
              <el-input size="small" v-model="tag.value" placeholder="Value" class="input_row" :title="tag.value">
                <p slot="prepend" class="columnPrependBtn nowrap" :title="tag.type">
                  {{ `${tag.field}:${tag.type}` }}
                </p>
                <el-button v-if="isEdit" slot="append" icon="el-icon-check" @click="tagValueChange(tag, index)"></el-button>
              </el-input>
            </div>
          </div>
        </template>
        ...
        <!-- Comfirm Btn -->
        <el-button v-if="!isEdit" class="submitBtn" size="small" :loading="loading" :disabled="loading" type="primary" @click="handleCreateTable">{{
          $t("create")
        }}</el-button>
        <el-button :disabled="loading" class="submitBtn" size="small" @click="cancel">{{ $t("cancel") }}</el-button>
      </el-form>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
      ...
      formTitle() {
        if (!this.isEdit) {
          if (this.$store.state.tables.formStatus == "createByStb") {
            return this.$t("data.createTableUse").replace(/\{\}/, this.table_form.stbTmpl);
          } else {
            return this.$t("data.createTable");
          }
        } else {
          return this.$t("edit") + ` ${this.table_form.name} ` + this.$t("data.table");
        }
      },
      rules() {
        return {
    ...
    mounted() {
      // if (JSON.stringify(this.table_form) == "{}") {
      //   this.$router.push("/data/databases");
      // }
    },
    watch: {
      table_form: {
    ...
    methods: {
      ...
      async updateData(params, tag) {
        ...
        this.loading = true;
        await changeTableStruct(params, this.selected_db + "." + this.table_form.name)
          .then(() => {
        ...
        if (tag) {
          tag.value = (await getTagValue([tag], this.selected_db, this.table_form.stbTmpl, this.table_form.name))[tag.field];
        } else {
          this.$store
      ...
      tagValueChange(tag) {
        let isString = VariableTableColumnType.some(item => tag.type.startsWith(item));
