<template>
  <div class="dbs-tree">
    <PanelHeader style="justify-content: space-between">
      ...
      <div>
        <el-button icon="el-icon-refresh" @click="refersh" size="mini"></el-button>
        <el-button @click="addDatabase" size="mini" icon="el-icon-plus" plain v-permission></el-button>
      </div>
    </PanelHeader>
    <div class="dbs-tree-container">
      ...
      >
        <el-tooltip slot-scope="{ node, data }" effect="dark" :content="node.label" placement="right" popper-class="el-tree-popper">
          <div class="tree-wrapper">
            <div class="console-custom-tree-node">
              <Icon :name="icon[data.typeName]" class="console-tree-icon"></Icon>
              <span>{{ node.label }}</span>
            </div>
            <span v-if="data.dataType" class="column-type">{{ data.dataType }}</span>
            <section class="operate-btn">
              <template v-if="!data.dataType">
                <i class="el-icon-view operate-icon" @click.stop="view(data)"></i>
                <template v-if="!data.noOperate">
                  <i v-permission v-if="data.typeName !== 'table'" class="el-icon-plus operate-icon" @click.stop="add(data, node)"></i>
                  <i v-permission class="el-icon-edit operate-icon" @click.stop="edit(data, node)"></i>
                  <i v-permission class="el-icon-delete operate-icon" @click.stop="del(data, node)"></i>
                </template>
              </template>
              <div v-if="data.typeName == 'table' || data.typeName == 'stable'" class="tablebutton" @click.stop="clickAdd(data, true)">
...
<script>
  ...
  import { getStableListReq, getStableStructReq } from "@/api/gateway/data/stables.js";
  import { getTableListReq, getMatrixStructReq } from "@/api/gateway/data/tables.js";
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        requesting: false,
        defaultExpandedKeys: [],
        pageSize: Math.min(Math.max(Math.floor((window.innerHeight - 240) / 30), 5), 10000),
        isRequest: false,
      };
    },
    computed: {
    ...
    methods: {
      ...
      async clickAdd(data, all) {
        if (all) {
          ...
          let sdata = await getStableStructReq({
            selected_db: db,
            stableName: data.typeName == "stable" ? data.name : data.parent.split(".")[1],
          }).catch(() => ({
            ts_field_name: "",
            columns: [],
            tags: [],
          }));
          let columns = [`\`${sdata.ts_field_name}\``].concat(sdata.columns.map(item => `\`${item.field}\``));
          this.$store.state.console.addSql = `${this.$store.state.console.sqlStr ? "\n" : ""}SELECT ${columns.join(",") || "*"} FROM ${
            db + "." + data.name
          } limit 200;`;
          this.$store.state.console.sqlStr += this.$store.state.console.addSql;
        } else {
          let code = data.parent ? data.parent.split(".")[0] + "." + data.name : data.name;
      ...
      async expandChange(data) {
        ...
        switch (data.typeName) {
          case "table":
            await this.$store
              .dispatch("console/sendConsoleSQL", `select * from ${data.parent.split(".")[0] + "." + data.name} order by _C0 desc limit 200`)
              .catch(() => false);
            break;
          case "database":
      ...
      async handleVar(data, node) {
        switch (data.typeName) {
          ...
          case "table":
            ...
            this.$store.commit("stables/SET_SELECTED_STB", data.parent.split(".")[1]);
      ...
      async view(data) {
        ...
        this.$store.commit("console/SET_TAB_NAME", this.$t(`console.${data.typeName}Info`));
        this.$store.state.console.partActive = "detail";
      },
      ...
      async del(data, node) {
        ...
        switch (data.typeName) {
          case "database":
            this.$confirm(this.$t("data.delDatabase") + ":" + data.name + "?", this.$t("tips"), {
            ...
            break;
          case "stable":
            this.$confirm(`${this.$t("del")} ${this.$t("data.stable")} ${data.name} ?`, this.$t("tips"), {
              confirmButtonText: this.$t("confirm"),
              cancelButtonText: this.$t("cancel"),
              type: "warning",
            }).then(async () => {
            ...
            break;
          case "table":
            this.$confirm(this.$t("del") + " " + data.name + " " + this.$t("data.table") + "?", this.$t("tips"), {
              confirmButtonText: this.$t("confirm"),
              cancelButtonText: this.$t("cancel"),
              type: "warning",
            }).then(async () => {
