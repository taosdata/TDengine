  ...
  <div class="dnode-block">
    <div class="flexEnd">
      ...
      <el-button plain @click="add" size="small" icon="el-icon-plus">{{
        $t("taosuser.addreplication")
      }}</el-button>
    </div>
    <el-table style="margin-top: 20px" :data="topicList" size="mini">
      <el-table-column label="ID" width="80" prop="id"></el-table-column>
      <el-table-column
        :label="$t('taosuser.fromdb')"
        prop="fromdb"
      ></el-table-column>
      <el-table-column
        :label="$t('taosuser.toinstance')"
        prop="hostport"
      ></el-table-column>
      <el-table-column :label="$t('taosuser.todb')" prop="db"></el-table-column>

      <el-table-column
        :label="$t('taosuser.status')"
        prop="status"
      ></el-table-column>
      <el-table-column
        :label="$t('taosuser.reason')"
        prop="reason"
      ></el-table-column>
      <el-table-column
        :label="$t('taosuser.finishat')"
        prop="finished_at"
      ></el-table-column>
      <el-table-column
        :label="$t('taosuser.createat')"
        prop="created_at"
      ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    switchOperation(val, data) {
      this.$confirm(
        `${this.$t(
          val ? this.$t("replication.start") : this.$t("replication.stop")
        )} ${this.$t("replication.theTaskWithId").replace("{id}", data.id)}?`,
        this.$t("warning"),
        {
          confirmButtonText: this.$t("confirm"),
          cancelButtonText: this.$t("cancel"),
    ...
    async getReplication() {
      try {
        let id = localStorage.getItem("local_clusterID");
        let result = await getReplicationList(id);
        if (result.message) {
