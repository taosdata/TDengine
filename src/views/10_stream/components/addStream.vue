<template>
  ...
  >
    ...
    <SQLEditor v-show="model == 'SQL'" :placeholder="sqlTip" ref="sqlStr" v-model="sqlStr"></SQLEditor>
    <template v-if="model == 'Wizard'">
      <el-form-item :label="$t('stream.streamName')" required prop="stream_name">
      ...
      <el-form-item :label="$t('topic.database')" required prop="target_db">
        <el-select class="w100" v-model="info.target_db" @change="info.target_stb = ''" placeholder="">
      ...
      <el-form-item :label="$t('topic.stable')" required prop="target_stb">
        <el-input :disabled="!info.target_db" v-model="info.target_stb"> </el-input>
      </el-form-item>
      <el-form-item :label="$t('stream.subtablePrefix')" prop="subtale">
        <el-input :disabled="!info.target_stb" v-model="info.subtale" placeholder=""> </el-input>
      </el-form-item>
        ...
        <template #db-bottom>
          <el-form-item :label="$t('type')" prop="source_type" required>
            <el-radio-group size="small" v-model="info.source_type">
              <el-radio-button :label="1">{{ $t("stream.stableUpper") }}</el-radio-button>
              <el-radio-button :label="2">{{ $t("stream.tableUpper") }}</el-radio-button>
            </el-radio-group>
          </el-form-item>
        </template>
      </Subquery>
      ...
      <el-form-item v-if="info.trigger == 'MAX_DELAY'" :label="$t('stream.maxDelayTime')">
        <el-input-number :min="0" v-model="info.max_delay_time"></el-input-number>
        <el-select style="margin-left: 20px" v-model="info.max_delay_unit" placeholder="">
      ...
      <el-form-item :label="$t('stream.delay')">
        ...
        <el-input-number :min="0" :max="watermarkMax" v-model="info.watermark"></el-input-number>
        <el-select style="margin-left: 20px" v-model="info.watermark_unit" @change="watermarkUnitChange" placeholder="">
          <el-option v-for="item in watermarkUnitList" :key="item.label" v-bind="item"></el-option>
        </el-select>
      </el-form-item>
    </template>
    ...
    <el-form-item v-if="model == 'Wizard'">
      <div class="flexBetween">
        <el-button style="width: 30%" :disabled="createBtn" type="primary" @click="handlecreateStream">{{ $t("create") }}</el-button>
        <el-button :disabled="previewBtn" @click="generateSql">SQL Preview</el-button>
      </div>
    </el-form-item>
    <div v-else class="flexCenter">
      <el-button size="small" style="width: 30%" :loading="requestIng" :disabled="createBtn" type="primary" @click="handlecreateStream">{{
        $t("create")
      }}</el-button>
    </div>
    <el-dialog custom-class="show-topic-sql" width="500px" append-to-body :visible.sync="dialog" title="SQL">
      <pre :key="previewSql" v-highlight>
          <code class="language-sql">{{previewSql}}</code>
        </pre>
      <section class="flexEnd">
        <el-button type="primary" size="mini" @click="dialog = false">{{ $t("confirm") }}</el-button>
      </section>
    </el-dialog>
  </el-form>
</template>
    ...
    computed: {
      ...
      previewBtn() {
        if (this.model === "Wizard") {
          if (!this.info.stream_name || !this.info.target_db || !this.info.db_name) return true;
      ...
      createBtn() {
        return this.requestIng || (this.model === "Wizard" && this.previewBtn) || (this.model === "SQL" && !this.sqlStr);
      },
    },
    watch: {},
    created() {},
            ...
            if (valid) {
              try {
                ...
                if (this.info.trigger === "MAX_DELAY") {
                  previewSql += this.info.max_delay_time + this.info.max_delay_unit;
                }
                if (this.info.watermark) {
                  previewSql += " WATERMARK " + this.info.watermark + this.info.watermark_unit;
                }
                previewSql += " INTO `" + this.info.target_db.toLowerCase() + "`.`" + this.info.target_stb + "`";
                ...
                if (this.info.window_type) {
                  previewSql += " ";
                  const ts_col = this.info.resultSet.find(item => item.type === "TIMESTAMP")?.field;
