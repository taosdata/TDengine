<template>
  <el-form :model="changeForm" :rules="rules" :status-icon="true" label-position="top" label-width="auto" size="small" ref="changeForm">
  ...
  export default {
    ...
    data() {
      ...
      var checkPassword = async (_, value, callback) => {
        this.err_msg = "";
        callback(validPassword(value) ? undefined : new Error(this.$t("passwordError")));
      };
      let cheakConfirmPassword = async (_, value, callback) => {
        this.err_msg = "";
        callback(value == this.changeForm.new_password ? undefined : new Error(this.$t("twoPassError")));
      };
      return {
        changeForm: {
