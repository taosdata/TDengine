<template>
  <div class="page-wrapper">
    <MainContentHeader :title="$t('dashboard.overview')"
      ><el-button slot="right" class="change-btn" @click="change" plain size="small">
        {{ btnTitle }}
...
<script>
  ...
  export default {
    components: { ClusterInfo, UsageTrend, panelHeader, Query },
    data() {
      return {
        isQuery: false,
      };
    ...
    computed: {
      title() {
        return this.isQuery
          ...
          : this.$t("dashboard.totalUsage").replace(/Date/, moment.utc().format("YYYY-MM") + "-01");
      },
      btnTitle() {
