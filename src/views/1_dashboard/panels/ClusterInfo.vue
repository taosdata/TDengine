<template>
  <div class="clusterInfo">
    <section class="usage-content">
      ...
      <div class="info_card block-border">
        <div :title="cluster_info.ingress" class="uptime-value flexCenter nowrap">
          {{ cluster_info.ingress | handleValue }}
        </div>
        <div class="info_label">{{ $t("dashboard.ingressVolume") }} (M)</div>
      </div>
      <div class="info_card block-border">
        <div :title="cluster_info.egress" class="uptime-value flexCenter nowrap">
          {{ cluster_info.egress | handleValue }}
        </div>
        <div class="info_label">{{ $t("dashboard.egressVolume") }} (M)</div>
      </div>
      <div class="info_card mini">
        <div class="block-border">
          <div :title="cluster_info.gbHourStorage" class="info_value">
            {{ cluster_info.gbHourStorage | handleValue }}
          </div>
...
<style lang="scss" scoped>
  ...
  .block-border {
    ...
    display: flex;
    flex-direction: column;
    box-shadow: 0 1px 1px 0 var(--awsui-color-shadow-medium, rgba(0, 28, 36, 0.3)), 0 1px 1px 0 var(--awsui-color-shadow-side, rgba(0, 28, 36, 0.15)),
