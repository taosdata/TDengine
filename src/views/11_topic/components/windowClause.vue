<template>
  <div>
    ...
    <el-form-item v-if="window_clause.window_type == 'SESSION'" :label="$t('sql.totalTime')">
      <el-input-number v-model="window_clause.tol_val" :min="0"></el-input-number>
      <el-select style="margin-left: 20px" v-model="window_clause.tol_unit" placeholder="">
        <el-option v-for="item in timeUnit" :key="item.value" v-bind="item"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item v-if="window_clause.window_type == 'STATE'" :label="$t('stream.column')" prop="state_column" required>
      <el-select class="w100" v-model="window_clause.state_column" placeholder="">
    ...
    <template v-if="window_clause.window_type == 'INTERVAL'">
      <el-form-item :label="$t('stream.intervalPeriod')">
        <el-input-number v-model="window_clause.interval_val" :min="1"></el-input-number>
        <el-select style="margin-left: 20px" v-model="window_clause.interval_unit" placeholder="">
      ...
      <el-form-item :label="$t('stream.slidingPeriod')">
        ...
        <el-input-number v-model="window_clause.sliding_val" :min="0"></el-input-number>
        <el-select style="margin-left: 20px" v-model="window_clause.sliding_unit" placeholder="">
          <el-option v-for="item in timeUnit" :key="item.value" v-bind="item"></el-option>
        </el-select>
      </el-form-item>
    </template>
...
<script>
  ...
  export default {
    ...
    computed: {
      stateColumn() {
        return this.columnList.filter(item => !stateColumnExculde.includes(item.type));
      },
    },
    watch: {},
    created() {},
