...
def get_install_path():
    ...
    if release_info.OnlyBuild:
        ...
        return os.path.join(
            taosx_dir,
            "release",
            "{0}-{1}-{2}-linux-{3}".format(
                product_name,
                target,
                release_info.TdengineVersion,
                release_info.CpuType.lower(),
            ),
        )
...
def init_build_info():
    ...
    parser.add_argument(
        ...
        "--connector_list",
        nargs="+",
        help="A list of connectors to package, all by default",
    )
    parser.add_argument(
        "-b", "--build_mode", help="Debug or Release, Release by default"
    )
    parser.add_argument(
        "-s",
        "--sub_version_mode",
    ...
    parser.add_argument(
        "-c",
        "--cpu_type",
        help="cpu [aarch32 | aarch64 | x64 | x86 | mips64 | loongarch64 ...] ",
    )
    parser.add_argument(
        "-o",
        "--objective",
        choices=["taosx", "agent"],
        help="target package type(taosx, agent)",
    )
    parser.add_argument(
        "-t", "--test_process", help="test single process(pi,opc,taosx,package)"
    )
    parser.add_argument(
        "-ob",
        "--only_build",
        nargs="?",
        const=get_install_path(),
        help="only build taosx into this path.)",
    )
    ...
    parser.add_argument(
        "-ua", "--upload_agent", help="upload taosx-agent to taosdata.com"
    )
    ...
    parser.add_argument(
        ...
        "--build_without_docs",
        action="store_true",
        help="build taosexplorer without docs zip",
    )
    parser.add_argument(
        "-bh",
        "--build_with_selfhost",
        action="store_true",
        help="build taosexplorer with selfhost  docs zip",
    )
    ...
    if args.upload_agent:
        ...
        sub_module.append(
            SubmoduleBuildInfo(taosx_agent_name, release_info.DefaultBuildMode)
        )
    if args.build_agent:
    ...
    if release_info.Target == "taosx":
        ...
        sub_module.append(
            SubmoduleBuildInfo(taos_explorer_name, release_info.DefaultBuildMode)
        )
    ...
    release_info.Commit = (
        subprocess.check_output(["git", "rev-list", "-1", branch])
        .strip()
        .decode("ascii")
    )
...
def change_piconnector_assemble_file():
    pi_connector_assembly_file_path = os.path.join(
        taosx_dir,
        "plugins",
        "pi",
        "src",
        "TDPIConnector",
        "TDPIConnector.Service",
        "Properties",
        "AssemblyInfo.cs",
    )
    change_assemble_file_key(
        pi_connector_assembly_file_path, "BuildTime", release_info.BuildTime
    )
    change_assemble_file_key(
        pi_connector_assembly_file_path, "Commit", release_info.Commit
    )

    pi_backfill_assembly_file_path = os.path.join(
        taosx_dir,
        "plugins",
        "pi",
        "src",
    ...
    change_assemble_file_key(
        pi_backfill_assembly_file_path, "BuildTime", release_info.BuildTime
    )
    change_assemble_file_key(
        pi_backfill_assembly_file_path, "Commit", release_info.Commit
    )
...
def build_and_install_pi(mode):
    ...
    connector_path = os.path.join(
        pi_connector_path, "TDPIConnector.Service", "bin", f"{mode}"
    )
...
def copy_taosx_cfg(taos_cfg_path):
    ...
    try:
        ...
        print(
            "Copy taosx cfg from {} to {} failed: {}".format(
...
def copy_taosx_agent_cfg(taos_cfg_path):
    ...
    try:
        ...
        print(
            "Copy taosx Agent cfg from {} to {} failed: {}".format(
...
def build_and_install_taosx(mode):
    ...
    if mode == "Release":
        os.system(
            f'cargo make -e VER_NUMBER={release_info.TdengineVersion} -e CUS_PROMPT={release_info.CustomPrompt} -e CUS_NAME="{release_info.CustomName}" -e CUS_EMAIL={release_info.CustomEmail} -e BUILD_PROFILE=release deploy-taosx'
        )
        os.system(
            f'cargo make -e VER_NUMBER={release_info.TdengineVersion} -e CUS_PROMPT={release_info.CustomPrompt} -e CUS_NAME="{release_info.CustomName}" -e CUS_EMAIL={release_info.CustomEmail} -e BUILD_PROFILE=release deploy-xnoded'
        )
    else:
        os.system(
            f'cargo make -e VER_NUMBER={release_info.TdengineVersion} -e CUS_PROMPT={release_info.CustomPrompt} -e CUS_NAME="{release_info.CustomName}" -e CUS_EMAIL={release_info.CustomEmail} -e BUILD_PROFILE=dev deploy-taosx'
        )
        os.system(
            f'cargo make -e VER_NUMBER={release_info.TdengineVersion} -e CUS_PROMPT={release_info.CustomPrompt} -e CUS_NAME="{release_info.CustomName}" -e CUS_EMAIL={release_info.CustomEmail} -e BUILD_PROFILE=dev deploy-xnoded'
        )
...
def build_and_install_taosx_agent(mode):
    ...
    if mode == "Release":
        os.system(
            f"set VER_NUMBER={release_info.TdengineVersion}&set CUS_PROMPT={release_info.CustomPrompt}&set CUS_NAME={release_info.CustomName}&set CUS_EMAIL={release_info.CustomEmail}&set BUILD_PROFILE=release&cargo make deploy-taosx-agent"
        )
    else:
        os.system(
            f"set VER_NUMBER={release_info.TdengineVersion}&set CUS_PROMPT={release_info.CustomPrompt}&set CUS_NAME={release_info.CustomName}&set CUS_EMAIL={release_info.CustomEmail}&set BUILD_PROFILE=dev&cargo make deploy-taosx-agent"
        )

    taosx_install_path = os.path.join(release_info.InstallPath, "bin")
    check_directory(taosx_install_path)
    taosx_agent_path = os.path.join(
        taosx_dir, "target", "deploy", get_taosx_agent_output_name()
    )
    try:
    ...
    if release_info.UploadAgent:
        ...
        os.system(
            f"scp {release_info.PackageName}.exe root@taosdata.com:/data/www/assets-download/3.0/"
        )
        print(f"upload {release_info.PackageName}.exe to tdengine.com")
        os.system(
            f"scp {release_info.PackageName}.exe ubuntu@tdengine.com:/data/www/assets-download/3.0/"
        )
...
def build_and_install_influxdb(mode):
    ...
    print(build)
    os.system(build)
    influxdb_install_path = os.path.join(
        release_info.InstallPath, "plugins", "influxdb"
    )
    init_directory(influxdb_install_path)
    influxdb_path = os.path.join(
        influxdb_connector_path, "target", "taosx-influxdb.jar"
    )
    try:
...
def build_and_install_opentsdb(mode):
    ...
    print(build)
    os.system(build)
    opentsdb_install_path = os.path.join(
        release_info.InstallPath, "plugins", "opentsdb"
    )
    init_directory(opentsdb_install_path)
    opentsdb_path = os.path.join(
        opentsdb_connector_path, "target", "taosx-opentsdb.jar"
    )
    try:
...
def build_and_install_pspace(mode):
    print("build_and_install_pspace on windows start...")
    pspace_connector_path = os.path.join(taosx_dir, "plugins", "pspace")
    os.chdir(pspace_connector_path)
    build = "mvn clean package"
    print(build)
    os.system(build)
    pspace_install_path = os.path.join(release_info.InstallPath, "plugins", "pspace")
    init_directory(pspace_install_path)
    pspace_path = os.path.join(pspace_connector_path, "target", "taosx-pspace.jar")
    try:
...
def build_and_install_hebeipower(mode):
    ...
    hebeipower_build_path = os.path.join(
        taosx_dir, "crates", "transform", "parsers", "hebeipower"
    )
    os.chdir(taosx_dir)
    build = "cargo make taosx-plugin-hebeipower"
    print(build)
    os.system(build)
    hebeipower_install_path = os.path.join(
        release_info.InstallPath, "plugins", "parsers"
    )
    init_directory(hebeipower_install_path)
    hebeipower_path = os.path.join(
        hebeipower_build_path, "target", "release", "hebeipower.dll"
    )
...
def build_taos_explorer(explorer_path, mode):
    ...
    if release_info.OS.lower() == "windows":
        os.chdir(taosx_dir)
        os.system(
            f'cargo make -e VER_NUMBER={release_info.TdengineVersion} -e CUS_PROMPT={release_info.CustomPrompt} -e CUS_NAME="{release_info.CustomName}" -e CUS_EMAIL={release_info.CustomEmail} deploy-explorer'
        )
    else:
        os.chdir(taosx_dir)
        os.system(
...
def copy_taos_explorer_on_windows(explorer_path):
    explorer_exe_path = os.path.join(
        taosx_dir, "target", "deploy", f"{release_info.CustomPrompt}-explorer.exe"
    )
    explorer_srv_path = os.path.join(
        taosx_dir, "target", "deploy", f"{release_info.CustomPrompt}-explorer-srv.exe"
    )
    explorer_srv_xml_path = os.path.join(
        taosx_dir, "target", "deploy", f"{release_info.CustomPrompt}-explorer-srv.xml"
    )
    ...
    try:
        ...
        print(
            "Copy taos-explorer to {} failed: {}".format(
...
def package_on_windows():
    ...
    if (
        ...
        target = f"{release_info.CustomPrompt}x-agent"
    cmd = (
        f'iscc /F"{release_info.PackageName}" '
...
def copy_docs_to_explorer(explorer_path):
    ...
    if release_info.CustomPrompt != "taos" and release_info.CustomName != "TDengine":
        zh_doc_zip_path = os.path.join(
            explorer_path, "..", f"docs-{release_info.CustomPrompt}.zip"
        )
        ...
        if release_info.CustomPrompt == "taos" or release_info.CustomName == "TDengine":
            if release_info.build_with_selfhost:
                if os.path.exists(en_doc_zip) and os.path.exists(zh_doc_zip):
                    ...
                    print(
                        f"ERROR: not found docs-en.zip or docs-zh.zip at {local_doc_zip_path}"
                    )
                    sys.exit(1)
            else:
            ...
            subprocess.run(
                f"scp root@192.168.0.30:{remote_doc_zip_path}/docs-{release_info.CustomPrompt}.zip {doc_zip_path}",
                shell=True,
            )
    except Exception as e:
...
def init_install_directory():
    ...
    influxdb_install_path = os.path.join(
        release_info.InstallPath, "plugins", influxdb_connector
    )
    init_directory(influxdb_install_path)

    opentsdb_install_path = os.path.join(
        release_info.InstallPath, "plugins", opentsdb_connector
    )
    init_directory(opentsdb_install_path)

    pspace_install_path = os.path.join(
        release_info.InstallPath, "plugins", pspace_connector
    )
    init_directory(pspace_install_path)
...
def test_handle_windows(process):
    ...
    elif process == pspace_connector:
        print("Calling pSpace function...")
        build_and_install_pspace("Debug")
    else:
...
if __name__ == "__main__":
    ...
    if release_info.OS.lower() == "linux":
        ...
        for task in sub_module:
            ...
            elif pspace_connector == task.Name:
                print("build pspace_connector")
                build_and_install_pspace(task.VersionMode)
