...
func (conn *Connector) SubscribeMultiple(topics map[string]int) error {
	...
	return conn.client.SubscribeMultiple(filter, conn.onMessage).Error()
}
...
func NewConnector(config *config.MQTT, logger logrus.FieldLogger, onConnect func(), onDisconnected func(err error), onMessage mqtt.MessageHandler) *Connector {
	conn := &Connector{
		conf:           config,
		onMessage:      onMessage,
		onConnect:      onConnect,
