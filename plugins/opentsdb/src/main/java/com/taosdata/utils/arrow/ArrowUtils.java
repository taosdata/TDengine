...
public class ArrowUtils {
     ...
     * 构造时初始化schema信息
     *
     * @param opentsdbMetricEntity
     * @throws Exception
     */
    public ArrowUtils(OpentsdbMetricEntity opentsdbMetricEntity) throws Exception {
        // 根据Metric获取arrow初始化信息
        ArrowInitDto arrowInitDto = getArrowInit(opentsdbMetricEntity);
        // 封装meta信息
        ...
        if (arrowInitDto == null || metaData.size() == 0 || fieldList.size() == 0) {
            throw new Exception("Illegal data, generating schema error, Metric=" + opentsdbMetricEntity.toString());
        }
    }
     ...
     * @return
     */
    private ArrowInitDto getArrowInit(OpentsdbMetricEntity opentsdbMetricEntity) {
        ...
        opentsdbMetricEntity.getTagSet().forEach(tag -> {
    ...
    public byte[] transform(List<OpentsdbDataEntity> opentsdbDataEntityList, boolean first) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < opentsdbDataEntityList.size(); i++) {
                ...
                List<OpentsdbDataPointEntity> opentsdbDataPointEntityList = opentsdbDataEntity.getDps();
                for (int j = 0; j < opentsdbDataPointEntityList.size(); j++) {
                    OpentsdbDataPointEntity opentsdbDataPointEntity = opentsdbDataPointEntityList.get(j);
                    /* __records__ */
            ...
            // 设置recordVector写数据结束
            recordVector.endValue(0, amount);
            // 这里固定传1
            vectorSchemaRoot.setRowCount(1);
            writer.writeBatch();
            // 计数
    ...
    private void setData(StructVector structVector, String dataName, Object dataValue, String dataType, int index) {
        ...
        switch (dataType) {
            ...
            case "timestamp": {
                ...
                // 如果是秒则转化为毫秒
                timeStampMilliVector.setSafe(index, Long.parseLong(StringUtils.rightPad(dataValue.toString(), 13, '0')));
                break;
            }
            case "string":
            case "binary(256)":
