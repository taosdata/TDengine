...
class TestIllegalType(TDCase):
    ...
    def int_illegal_type_value_check(self):
        ...
        for test_type in int_type_list:
            ...
            self.tdRest.request(f'create stable if not exists {dbname}.stb (col_ts timestamp, c1 {test_type}) tags (tag_ts timestamp, t1 {test_type})')
            ...
            for error_type_value in error_type_value_list:
                self.tdRest.error(f'create table if not exists {dbname}.tb_error using {dbname}.stb tags (now, {error_type_value})')
            self.tdRest.error(f'insert into {dbname}.tb values (now, {error_type_value})')
    ...
    def float_illegal_type_value_check(self):
        ...
        for test_type in float_type_list:
            ...
            self.tdRest.request(f'create database if not exists {dbname} precision "ms"')
            self.tdRest.request(f'create stable if not exists {dbname}.stb (col_ts timestamp, c1 {test_type}) tags (tag_ts timestamp, t1 {test_type})')
            self.tdRest.request(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now, 1.1)')
            for error_type_value in error_type_value_list:
                self.tdRest.error(f'create table if not exists {dbname}.tb_error using {dbname}.stb tags (now, {error_type_value})')
            self.tdRest.error(f'insert into {dbname}.tb values (now, {error_type_value})')
            self.tdRest.request(f'drop database if exists {dbname}')
    ...
    def bool_illegal_type_value_check(self):
        ...
        for test_type in bool_type_list:
            ...
            self.tdRest.request(f'create database if not exists {dbname} precision "ms"')
            self.tdRest.request(f'create stable if not exists {dbname}.stb (col_ts timestamp, c1 {test_type}) tags (tag_ts timestamp, t1 {test_type})')
            self.tdRest.request(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now, True)')
            for error_type_value in error_type_value_list:
                self.tdRest.error(f'create table if not exists {dbname}.tb_error using {dbname}.stb tags (now, {error_type_value})')
            self.tdRest.error(f'insert into {dbname}.tb values (now, {error_type_value})')
            self.tdRest.request(f'drop database if exists {dbname}')
    ...
    def desc(self) -> str:
        case_description = '''
            int_illegal_type_value_check <jayden>: [TD-12748] : int_illegal_type_value_check;\n
            float_illegal_type_value_check <jayden>: [TD-12748] : float_illegal_type_value_check;\n
            bool_illegal_type_value_check <jayden>: [TD-12748] : bool_illegal_type_value_check;\n
            binary_illegal_type_value_check <jayden>: [TD-12748] : binary_illegal_type_value_check;
        '''
