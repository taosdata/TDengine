...
class Timeline_100B(TDCase):
    ...
    def run(self):
        ...
        stb_into = [self.tdCom.setStbinfo(columns=column_info_list, tags=tag_info_list, childtable_count=self.childtable_count1, childtable_prefix=self.childtable_prefix1, insert_rows=self.insert_rows, interlace_rows=self.interlace_rows, batch_create_tbl_num=self.batch_create_tbl_num, insert_mode=self.insert_mode)]
        ...
        dbinfo = self.tdCom.setDBinfo(name=self.dbname, replica=self.replica, vgroups=self.vgroups, drop="no")
        stb_into = [self.tdCom.setStbinfo(columns=column_info_list, tags=tag_info_list, childtable_count=self.childtable_count2, childtable_prefix=self.childtable_prefix2, insert_rows=self.insert_rows, interlace_rows=self.interlace_rows, batch_create_tbl_num=self.batch_create_tbl_num, insert_mode=self.insert_mode)]
        database_info = [self.tdCom.setDatabases(dbinfo=dbinfo, super_tables=stb_into)]
        ...
        dbinfo = self.tdCom.setDBinfo(name=self.dbname, replica=self.replica, vgroups=self.vgroups, drop="no")
        stb_into = [self.tdCom.setStbinfo(columns=column_info_list, tags=tag_info_list, childtable_count=self.childtable_count3, childtable_prefix=self.childtable_prefix3, insert_rows=self.insert_rows, interlace_rows=self.interlace_rows, batch_create_tbl_num=self.batch_create_tbl_num, insert_mode=self.insert_mode)]
        database_info = [self.tdCom.setDatabases(dbinfo=dbinfo, super_tables=stb_into)]
