...
class TestBigintBoundary(TDCase):
    ...
    def run(self):
        ...
        host = self.get_component_by_name(
            "taosd")[0]['fqdn'][0]
        print(host)
        ret = self.envMgr._remote.cmd2(host ,["hostname",f"taosBenchmark -f  {case_dir}pre_datas_insert.json"])
        ...
        for _ in range(10):
            ...
            ret = self.envMgr._remote.cmd2(host ,["hostname",f"taosBenchmark -f  {case_dir}basic_agg_query.json"])
            ...
            ret = self.envMgr._remote.cmd2(host ,["hostname",f"taosBenchmark -f  {case_dir}basic_long_query.json"])
