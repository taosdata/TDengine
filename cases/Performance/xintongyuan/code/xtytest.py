...
class Remote:
    ...
    def cmd(self, host, cmd_list, password=""):
        ...
        try:
            # 如果是本地命令
            if host in ["localhost", "127.0.0.1", self._local_host]:
                # 使用 subprocess.run 替代 check_output
                result = subprocess.run(
                    ...
                    text=True,
                    env=dict(os.environ, PYTHONUNBUFFERED="1", COLUMNS="120")
                )
...
class Container:
    ...
    def run_container(self, config, container_name, host, net_ip):
        ...
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/localhost/{container_name}/g" /etc/taos/taos.cfg'])
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/firstEp                   hostname:6030/firstEp                   taostest_net_host_0:6030/g" /etc/taos/taos.cfg'])
        ...
        try:
            # 删除错误节点
            query_cmd = f'docker exec -ti taostest_net_host_0 sh -c \'taos -s \"select id from information_schema.ins_dnodes where support_vnodes =0\";\''
            query_result = self.remote.cmd(config["fqdn"][0], [query_cmd])
            
            # 只有当查询结果包含有效数据时才进行删除操作
            if query_result and "==============" in query_result:
                try:
                    dnode_id = str(query_result).split("==============")[1].split("|")[0].strip()
                    if dnode_id and dnode_id.isdigit():
                        self.remote.cmd(config["fqdn"][0], 
                            [f'docker exec -d taostest_net_host_0 sh -c \'taos -s \"drop dnode {dnode_id}";\''])
                except (IndexError, ValueError) as e:
                    print(f"Warning: Failed to parse dnode id: {e}")
                    # 继续执行，不中断程序
        except Exception as e:
            print(f"Warning: Failed to handle error nodes: {e}")
            # 继续执行，不中断程序
...
if  __name__ == '__main__':
    ...
    container_config = {
        "fqdn": [platform.node()],  
        "net_name": "taostest_net",
        "subnet": "172.12.0.0/16",
        "image": "tdengine/tdengine-amd64:3.3.6.6",
        "dnode_count": 100
    }
