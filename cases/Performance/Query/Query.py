    ...
    def run(self):
        ...
        for cases in cfg:
            ...
            for sqls in sqls_list:
                ...
                f.write("********query\t"+ str(cases)+":\t sql:\t" + sqls + "\twith\t" + str(cfg[cases]["specify_query"]["concurrent"]) + "\tthreads********\n")
                f.close()
                for i in range(len(taosBenchmark_iplist)):
                    json_data.append({})
                    sql_info = jfile.setSqlInfo(sql=sqls)
                    specify_query = jfile.setSpecifyQuery(concurrent=cfg[cases]["specify_query"]["concurrent"], sqls=[sql_info])
                    json_info = jfile.setJsoninfo(host=cfg[cases]["json_info"]["host"],
                                                  query_times=cfg[cases]["json_info"]["query_times"],
                                                  confirm_parameter_prompt=cfg[cases]["json_info"]["confirm_parameter_prompt"],
                                                  specified_table_query=specify_query,
                                                  databases="performance_insert_insertbase", reset_query_cache=cfg[cases]["json_info"]["reset_query_cache"])
                    json_info.update({"query_mode":cfg[cases]["json_info"]["query_mode"], "test_log": "/root/testlog/"})
                    json_data[i] = json_info
                    file_name.append("query" + str(i) + "_" + str(cfg[cases]["specify_query"]["concurrent"]) + ".json")
