...
class InsertTest(TDCase):
    ...
    def run(self):
        ...
        col = jfile.schemacfg(intcount=40, binarycount=(20, 16), doublecount=40)
        tag = jfile.schemacfg(intcount=1, binarycount=(1, 16))
        insert_mode = ["taosc","rest","stmt"]
        ...
        for rows in insert_rows:
            for k in insert_mode:
                ...
                f.write("-------- insert 100 columns ,\t" + str(
                    rows * 10000) + " rows with\t" + k + "\tinsert mode--------\n")
                f.close()
