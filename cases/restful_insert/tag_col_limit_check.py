...
class TestTagColLimit(TDCase):
    ...
    def tb_col_max_count_check(self):
        ...
        dbname = self.tdCom.get_long_name()
        self.tdRest.request(f'create database if not exists {dbname} precision "ms"')
        self.tdRest.error(f'create table if not exists {dbname}.tb (col_ts timestamp, {col_str_exceed})')
        self.tdRest.request(f'create table if not exists {dbname}.tb (col_ts timestamp, {col_str})')
    ...
    def tag_col_name_length_check(self):
        ...
        """
        dbname = self.tdCom.get_long_name()
        tag_key_name = self.tdCom.get_long_name(length=self.tdCom.Boundary.TAG_KEY_MAX_LENGTH, mode="letters")
        col_key_name = self.tdCom.get_long_name(length=self.tdCom.Boundary.TAG_KEY_MAX_LENGTH, mode="letters")
        self.tdRest.request(f'create database if not exists {dbname}')
        ...
        self.tdRest.error(f'create table if not exists {dbname}.stb_error (col_ts timestamp, {col_key_name}a int)')
        self.tdRest.request(f'create stable if not exists {dbname}.stb (col_ts timestamp, {col_key_name} int) tags ({tag_key_name} int)')
        self.tdRest.request(f'create table if not exists {dbname}.ctb using {dbname}.stb tags (1)')
        self.tdRest.request(f'create table if not exists {dbname}.tb (col_ts timestamp, {col_key_name} int)')
        self.tdRest.request(f'insert into {dbname}.ctb values (now, 1)')
        self.tdRest.request(f'insert into {dbname}.tb values (now, 1)')
