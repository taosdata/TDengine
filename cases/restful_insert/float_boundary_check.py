...
class TestFloatBoundary(TDCase):
    ...
    def float_boundary_check(self):
        ...
        self.tdSql.checkEqual(math.isclose(self.tdCom.Boundary.FLOAT_BOUNDARY[1], self.tdRest.resp[0][0], rel_tol=0.01), True)
        self.tdSql.checkEqual(math.isclose(self.tdCom.Boundary.FLOAT_BOUNDARY[0], self.tdRest.resp[0][1], rel_tol=0.01), True)
        self.tdRest.request(f'select t1, c1 from {dbname}.tb2')
        self.tdSql.checkEqual(math.isclose(self.tdCom.Boundary.FLOAT_BOUNDARY[0], self.tdRest.resp[0][0], rel_tol=0.01), True)
        self.tdSql.checkEqual(math.isclose(self.tdCom.Boundary.FLOAT_BOUNDARY[1], self.tdRest.resp[0][1], rel_tol=0.01), True)
        ...
        self.tdSql.checkEqual(abs(float(str(self.tdCom.Boundary.FLOAT_BOUNDARY[1]).replace("e+38",""))-float(str(self.tdRest.resp[0][0]).replace("e+38",""))) < 0.01, True)
        self.tdRest.request(f'select c1 from {dbname}.tb3 where c1<0')
        self.tdSql.checkEqual(abs(float(str(self.tdCom.Boundary.FLOAT_BOUNDARY[1]).replace("e+38",""))+float(str(self.tdRest.resp[0][0]).replace("e+38",""))) < 0.01, True)
