...
class TestDoubleBoundary(TDCase):
    ...
    def double_boundary_check(self):
        ...
        self.tdSql.checkEqual(float(self.tdRest.resp[0][0]), self.tdCom.Boundary.DOUBLE_BOUNDARY[1])
        self.tdSql.checkEqual(float(self.tdRest.resp[0][1]), self.tdCom.Boundary.DOUBLE_BOUNDARY[0])
        self.tdRest.request(f'select t1, c1 from {dbname}.tb2')
        self.tdSql.checkEqual(float(self.tdRest.resp[0][0]), self.tdCom.Boundary.DOUBLE_BOUNDARY[0])
        self.tdSql.checkEqual(float(self.tdRest.resp[0][1]), self.tdCom.Boundary.DOUBLE_BOUNDARY[1])
        ...
        self.tdSql.checkEqual(self.tdRest.resp[0][0], self.tdCom.Boundary.DOUBLE_BOUNDARY[1])
        self.tdRest.request(f'select c1 from {dbname}.tb3 where c1={self.tdCom.Boundary.DOUBLE_BOUNDARY[0]}')
        self.tdSql.checkEqual(self.tdRest.resp[0][0], self.tdCom.Boundary.DOUBLE_BOUNDARY[0])
