...
class TestUnsignedTinyintBoundary(TDCase):
    ...
    def utinyint_boundary_check(self):
        ...
        self.tdRest.request(f'create table if not exists {dbname}.tb1 using {dbname}.stb tags (254)')
        ...
        self.tdRest.request(f'insert into {dbname}.tb2 values (now, 254)')
        self.tdRest.request(f'select t1, c1 from {dbname}.tb1')
        self.tdSql.checkEqual(int(self.tdRest.resp["data"][0][0]), 254)
        ...
        self.tdSql.checkEqual(int(self.tdRest.resp["data"][0][1]), 254)
        self.tdRest.error(f'create stable if not exists {dbname}.stb_error1 (col_ts timestamp, c1 254) tags (t1 255)')
        self.tdRest.error(f'create stable if not exists {dbname}.stb_error2 (col_ts timestamp, c1 255) tags (t1 254)')
        self.tdRest.error(f'create stable if not exists {dbname}.stb_error3 (col_ts timestamp, c1 254) tags (t1 -1)')
        self.tdRest.error(f'create stable if not exists {dbname}.stb_error4 (col_ts timestamp, c1 -1) tags (t1 0)')
        self.tdRest.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, 255)')
        self.tdRest.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, -1)')
        self.tdRest.error(f'insert into {dbname}.tb values (now-1h, 255)')
