...
class TestTs2919(TDCase):
    ...
    def run(self):
        ...
        for i in range(20):
            self.tdSql.execute(f'create table test_ts2919.ctb{i} using test_ts2919.nginx tags ("/gdmall/auction_syn/{self.tdCom.get_long_name(200)}");')
        ...
        for i in range(25):
            for j in range(20):
                self.tdSql.execute(f'insert into test_ts2919.ctb{j} values (now+{i}m, "/gdmall/auction_syn/{self.tdCom.get_long_name(200)}", {i}.{i});')
