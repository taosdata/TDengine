...
class TestTs2920(TDCase):
    ...
    def run(self):
        ...
        self.tdSql.execute(f'create table test_ts2920.ctb using test_ts2920.nginx tags ("/gdmall/auction_syn/{self.tdCom.get_long_name(200)}");')
        self.tdSql.execute('create stream nginx_avg into test_ts2920.nginx_avg_output subtable(short_request) as select _wstart as start_time, _wend as end_time, short_request, short_request1, avg(upstream_response) as avg_response from test_ts2920.nginx partition by short_request,short_request1 interval(1m);')
        for i in range(self.rows_count):
            self.tdSql.execute(f'insert into test_ts2920.ctb values (now+{i}m, "/gdmall/auction_syn/{self.tdCom.get_long_name(200)}", {i}.{i});')
