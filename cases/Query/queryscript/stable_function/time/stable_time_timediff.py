...
class TDTestQuery(TDCase):
    ...
    #basic_param
    db = "stable_timediff"
    db_1 = "stable_timediff_1"
    db_2 = "stable_timediff_2"
    db_3 = "stable_timediff_3"
    service_host = ""
    ...
    def right_case_1_groupby(self):
        ...
        for i in (71,81,):
            ...
            try:
                ...
                self.tdSql.query('use %s;' %self.db_1)              
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        self.tdCreateData.data_matrix_equal('%s' %sql1 ,1,rows,1,1,'%s' %sql2 ,1,5,1,1)
                        self.time_check(sql1,sql2)
                        self.np_check(sql1,sql2)
                        cur1.execute(sql2)
                        self.tdCreateData.explain_sql(sql2)
