            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        sql2 = "select %s from (select * from %s where tbname in ('%s_1') and  %s %s %sorder by ts ) order by ts desc" %(func_desc,self.table,self.table,qt_where,qt_like_match,qt_in_where)
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        self.tdCreateData.data_matrix_equal('%s' %sql1 ,1,int('%d' %n),1,1,'%s' %sql2 ,1,int('%d' %n),1,1)
                        cur1.execute(sql2)
                        self.tdCreateData.explain_sql(sql2)
