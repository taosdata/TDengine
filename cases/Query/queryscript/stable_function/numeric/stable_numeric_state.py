...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support statecount && stateduration function [hanshu = 'statecount','stateduration']
        case2:
        '''
        return case_description

    #basic_param
    db = "stable_numeric_state"
    service_host = ""
    ...
    def right_case_1(self):
        ...
        # 1: support statecount && stateduration function [hanshu = 'statecount','stateduration']
        for i in (11,):
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        sql2 = "select %s from (select * from %s where tbname in ('%s_1') and  %s %s %sorder by ts desc ) order by ts desc" %(func_desc,self.table,self.table,qt_where,qt_like_match,qt_in_where)
                        self.tdSql.error(sql2)
                        sql= sql + sql2
