...
class TDTestQuery(TDCase):
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        for i in (6,7,8,9,):                        
                            ...
                            #TD-14965 self.tdCreateData.dataequal('%s' %sql1 ,1,2,'%s' %sql2 ,1,2)
                            #cur1.execute(sql2)
                            sql= sql + sql2
    ...
    def right_case_2(self):
        ...
        # 2: support all int type \ double type  [hanshu = ['AVG','SUM','MIN','MAX']]
        # 3: support all int type \ double type \ ts type [hanshu = ['SPREAD']]
        for i in (11,12,21,3):#13 last_row not support
            func = tdFunction.func_stable_tbname_all(i)
            try:
                self.tdCreateData.taos_f(self.service_host,self.testcasePath,self.testcaseFilename)                  
                cur1.execute('use %s;' %self.db)              
                ...
                stable_where = tdWhere.stable_where()
                sql1 = " select %s from (select * from %s where tbname in ('%s_1') order by ts desc);"  % (func,self.table,self.table)
                for i in range(2,len(stable_where[2])+1):
                    qt_where = list(combinations(stable_where[2],i))
                    for qt_where in qt_where:
                        qt_where = str(qt_where).replace("(","").replace(")","").replace("'","").replace("\"","").replace(",","")
                        ...
                        sql2 = "select %s from (select * from %s where tbname in ('%s_1') and  %s %s %s order by ts desc)" %(func,self.table,self.table,qt_where,qt_like_match,qt_in_where)
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)
                        self.tdCreateData.explain_sql(sql2)
                        sql= sql + sql2
                        ...
                        sql2 = "select %s from (select * from %s where tbname in ('%s_1') and  %s %s %s order by ts desc ) order by ts desc" %(func,self.table,self.table,qt_where,qt_like_match,qt_in_where)
                        self.tdSql.error(sql2)
                        sql= sql + sql2
    ...
    def right_case_2_2(self):
        ...
        # 2: support all int type \ double type  [hanshu = ['AVG','SUM','MIN','MAX']]
        # 3: support all int type \ double type \ ts type [hanshu = ['SPREAD']]
        for i in (22,):#13 last_row not support
            func = tdFunction.func_stable_tbname_all(i)
            try:
                self.tdCreateData.taos_f(self.service_host,self.testcasePath,self.testcaseFilename)                  
                cur1.execute('use %s;' %self.db)              
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        for i in (1,2,3,4,):                        
                            ...
                            sql1 = 'select %s from %s %s limit 10 ;'  % (func,self.table,time_window_new)
                        ...
                        for i in (6,7,8,9,):                        
                            time_window_new = tdWhere.time_window_new(i)
                            self.logger.info("\n\n\n=====right case========case3====time num = %d======interval======\n\n\n" %i)
                            sql1 = 'select %s from %s %s %s limit 10;'  % (func,self.table,interval_fill,time_window_new)
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        for i in (6,7,8,9,):                        
                            ...
                            sql2 = "select %s from %s where  %s %s %s group by tbname order by ts limit 10" %(func,self.table,qt_like_match,qt_in_where,time_window_new)
                            self.tdSql.error(sql2)
                            sql= sql + sql2

                            sql2 = "select * from (select %s from %s where  %s %s %s group by tbname order by ts limit 10)" %(func,self.table,qt_like_match,qt_in_where,time_window_new)
                            self.tdSql.error(sql2)
                            sql= sql + sql2
