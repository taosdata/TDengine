...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support binary and nchar type\ support str function [hanshu = ['LTRIM']
        case2:
        '''
    ...
    def np_check(self,sql1,sql2):  
        #通过第三方的numpy去校验 
        sql0 = sql1.replace("LTRIM","").replace("ltrim","")
        for j in range(0,self.np_check_num):
            self.tdSql.query(sql0)
            value = np.char.lstrip(self.tdSql.getData(j,0))
            self.tdCreateData.check_one_row_one_col_str_value('%s' %sql2 , j ,0,'LTRIM','%s' %value)  
    ...
    def right_case_1_groupby(self):
        ...
        # 1: support str function [hanshu = ['LTRIM']
        for i in (4,):
