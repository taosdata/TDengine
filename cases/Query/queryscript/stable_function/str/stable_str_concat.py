...
class TDTestQuery(TDCase):
    ...
    #basic_param
    db = "stable_concat"
    db_1 = "stable_concat_1"
    db_2 = "stable_concat_2"
    db_3 = "stable_concat_3"
    service_host = "ceph01"
    ...
    #     self.tdCreateData.dropandcreateDB_random_concat("%s" % self.db, 1) 
    ...
    def np_check(self,sql1,sql2):  
        ...
        sql0 = sql1.replace("CONCAT(","").replace(") from"," from")
        for j in range(0,self.np_check_num):
            ...
            self.tdCreateData.check_one_row_one_col_str_value('%s' %sql2 , j ,0,'CONCAT','%s' %value) 
