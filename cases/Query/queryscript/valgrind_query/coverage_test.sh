...
function buildTDengine() {
    ...
    rm -rf *
    makecmd="cmake -DCOVER=true -DBUILD_TEST=false -DBUILD_HTTP=false -DBUILD_TOOLS=true -DBUILD_GEOS=true -DBUILD_CONTRIB=false ../../"
    echo "$makecmd"
    $makecmd
...
function lcovFunc {
    ...
    /usr/local/bin/coveralls-lcov coverage.info -b $branch -t WOjivt0JCvDfqHDpyBQXtqhYbOGANrrps | tee -a $TDENGINE_COVERAGE_REPORT
