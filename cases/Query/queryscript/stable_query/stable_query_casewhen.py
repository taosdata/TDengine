...
class TDTestQuery(TDCase):
    ...
    def casewhen_list(self):
        casewhen_lists = ['case when 3 then 4 end' , 
                        'case when 0 then 4 end' ,
                        'case when null then 4 end ' ,
                        'case when 1 then 4+1 end ' ,
                        'case when 1-1 then 0 end ' ,
                        'case when 1+1 then 0 end ' ,   
                        'case when 1 then 1-1+2 end ' ,
                        'case when 1 > 0 then 1 < 2 end ' ,
                        'case when 1 > 2 then 1 < 2 end ' ,
                        'case when abs(3) then abs(-1) end ' ,
                        'case when abs(1+1) then abs(-1)+abs(3) end ' ,
                        'case when 0 then 1 else 3 end ' ,
                        'case when 0 then 1 when 1 then 0 else 3 end ' ,
                        'case when 0 then 1 when 1 then 0 when 2 then 3 end ' ,
                        'case when \'a\' then \'b\' when null then 0 end ' ,
                        'case when 2 then \'b\' when null then 0 end ' ,
                        'case when 0 then \'b\' else null end ',
                        'case when 0 then \'b\' else 2 end ',
                        'case when sum(2) then sum(2)-sum(1) end ' ,
                        'case when sum(2) then abs(-2) end ' ,
                        'case when q_int then ts end ' ,
                        'case when q_int then q_int when q_int + 1 then q_int + 1 else q_int is null end ' ,
                        'case when q_int then 3 when ts then ts end ' ,
                        'case when 3 then q_int end ' ,
                        'case when q_int then 3 when 1 then 2 end ' ,
                        'case when sum(q_int) then sum(q_int)-abs(-1) end ' ,
                        'case when q_int < 3 then 1 when q_int >= 3 then 2 else 3 end caseWhen' ,
                        'cast(case q_int when q_int then q_int + 1 else q_int is null end as double) ' ,
                        'sum(case q_int when q_int then q_int + 1 else q_int is null end + 1) ' ,
                        'case when q_int is not null then case when q_int <= 0 then q_int else q_int * 10 end else -1 end ' ,
                        'case 3 when 3 then 4 end ' ,
                        'case 3 when 1 then 4 end ' ,
                        'case 3 when 1 then 4 else 2 end ' ,
                        'case null when null then 4 when 3 then 1 end ' ,
                        'case q_double when q_int then q_int when q_int - 1 then q_int else 99 end ' ,
                        'case cast(q_double as int) when 0 then q_double when q_int then 11 else ts end ' ,
                        'case q_int + 1 when 1 then 1 when 2 then 2 else 3 end ' ,
                        'case when 3 then 4 end ' ,
                        'case when 3 then 4 end ' ,
                        'case when 0 then 4 end ' ,
                        'case when null then 4 end ' ,
                        'case when 1 then 4+1 end ' ,
                        ...
                        'case 3 when 1 then 4 else 2 end ' ,
                        'case null when null then 4 when 3 then 1 end ' ,
                        'q_double,q_int,case q_double when q_int then q_int when q_int - 1 then q_int else 99 end ' ,
                        'case cast(q_double as int) when 0 then q_double when q_int then 11 else ts end ' ,
                        'q_int, case q_int + 1 when 1 then 1 when 2 then 2 else 3 end ' ,
                        'distinct loc, case t_int when t_bigint then t_ts else t_smallint + 100 end ' ,
                        ]
    ...
    def casewhen_list_notsupport(self):
        casewhen_lists = ['case when 3 then 4 end' , 
                        ...
                        'case when 3 then q_int end ' ,
                        'case when q_int then 3 when 1 then 2 end ' ,
                        #need groupby: 'case when sum(q_int) then sum(q_int)-abs(-1) end ' ,
                        #need groupby: 'q_int, case sum(q_int) when 1 then q_int + 99 when q_int then q_int -99 else q_int end' ,
                        #need groupby: 'case when sum(q_int) then sum(q_int)-abs(q_int) end ' ,
                        #need groupby: 'case when q_int then sum(q_int) when q_int is not null then 9 else 8 end ' ,
                        ...
                        'case 3 when 1 then 4 end ' ,
                        'case 3 when 1 then 4 else 2 end ' ,
                        #'case 3 when null then 4 when '3' then 1 end ' ,
                        #'case '3' when null then 4 when 3 then 1 end ' ,
                        'case null when null then 4 when 3 then 1 end ' ,
                        #'case 3.0 when null then 4 when '3' then 1 end ' ,
                        #'case q_double when 'a' then 4 when '0' then 1 end ' ,
                        'case q_double when q_int then q_int when q_int - 1 then q_int else 99 end ' ,
                        'case cast(q_double as int) when 0 then q_double when q_int then 11 else ts end ' ,
                        'case q_int + 1 when 1 then 1 when 2 then 2 else 3 end ' ,
                        'case q_int when sum(q_int) then sum(q_int)-abs(q_int) end ' ,
                        'q_int, case sum(q_int) when 1 then q_int + 99 ' ,
                        'case when 3 then 4 end ' ,
                        'case when 3 then 4 end ' ,
                        'case when 0 then 4 end ' ,
                        'case when null then 4 end ' ,
                        ...
                        # 'case when '2' then 'b' when null then 0 end ' ,
                        # 'case when 0 then 'b' else null end ' ,
                        # 'case when 0 then 'b' else 2+abs(-2) end ' ,
                        'case 3 when 3 then 4 end ' ,
                        'case 3 when 1 then 4 end ' ,
                        'case 3 when 1 then 4 else 2 end ' ,
                        #'case 3 when null then 4 when '3' then 1 end ' ,
                        'case null when null then 4 when 3 then 1 end ' ,
                        #'case 3.0 when null then 4 when '3' then 1 end ' ,
                        #'q_double,case q_double when 'a' then 4 when '0' then 1 end ' ,
                        'q_double,q_int,case q_double when q_int then q_int when q_int - 1 then q_int else 99 end ' ,
                        'case cast(q_double as int) when 0 then q_double when q_int then 11 else ts end ' ,
                        'q_int, case q_int + 1 when 1 then 1 when 2 then 2 else 3 end ' ,
                        #need groupby: 'case q_int when sum(q_int) then sum(q_int)-abs(q_int) end ' ,
                        #need groupby: 'case sum(q_int) when 1 then q_int + 99 when q_int then q_int -99 else q_int end ' ,
                        'distinct tbname, case t_int when t_bigint then t_ts else t_smallint + 100 end ' ,
                        ]
