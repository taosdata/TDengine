...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support no type ,only () \ support time function [hanshu = ['TIMEZONE()']]
        case2:
        '''
    ...
    def np_check(self,sql1,sql2):  
        ...
        for j in range(0,self.np_check_num):
            value = get_localzone()
            self.tdCreateData.check_one_row_one_col_time_value('%s' %sql2 , j ,0,'SYS_TIMEZONE','%s' %value)  
    ...
    def time_check(self,sql1,sql2):  
        ...
        for j in range(0,self.np_check_num):
            ...
            self.tdCreateData.check_one_row_one_col_time_value('%s' %sql2 , j ,0,'TIMEZONE','%s' %value)
    ...
    def right_case_1_groupby(self):
        ...
        # 1: support time function [hanshu = ['TIMEZONE()']
        for i in (3,):
