...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support no type ,only () \ support time function [hanshu = ['NOW()']]
        case2:
        '''
    ...
    def np_check(self,sql1,sql2):  
        #和sys time去对比，适合多条时，选择checknum的条数对比
    ...
    def time_check_row(self,sql1,sql2,rows):  
        #因为查询时时间有差值,方便对比返回的行数
        sql0 = sql1
        for j in range(0,rows):
            self.logger.info("===============%d======"%j)
            self.tdSql.query(sql0)
            value = self.tdSql.getData(j,0)
            self.tdCreateData.check_one_row_one_col_time_value('%s' %sql2 , j ,0,'TIME','%s' %value)
    ...
    def right_case_1_groupby(self):
        ...
        # 1: support time function [hanshu = ['NOW()']
        for i in (1,):
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        # rows = self.tdSql.query(sql2).row_count #TD-16258
                        #self.time_check_row(sql1,sql2,rows)
                        # self.np_check_row(sql1,sql2,rows)
                        #cur1.execute(sql2)
                        # self.tdCreateData.explain_sql(sql2)
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        # self.time_check(sql1,sql2)##TD-16258
                        # self.np_check(sql1,sql2)
                        # cur1.execute(sql2)
                        # self.tdCreateData.explain_sql(sql2)
                        sql= sql + sql2
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        # self.time_check(sql1,sql2)#TD-16266
                        # self.np_check(sql1,sql2)
                        cur1.execute(sql2)
                        self.tdCreateData.explain_sql(sql2)
                        sql= sql + sql2
                        ...
                        # self.np_check(sql1,sql2)
                        # self.logger.info(sql2)
                        # cur1.execute(sql2) #TD-16272
                        # self.tdCreateData.explain_sql(sql2)
                        sql= sql + sql2
                        self.logger.info("====d11=========")
