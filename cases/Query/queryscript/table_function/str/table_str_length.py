...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support binary and nchar type\ support str function [hanshu = ['LENGTH','CHAR_LENGTH']]
        case2:
        '''
    ...
    def right_case_1_groupby(self):
        ...
        # 1: support str function [hanshu = ['LENGTH','CHAR_LENGTH']]
        for i in (6,):
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        #self.tdCreateData.check_mult_rows_one_col_value('%s' %sql2 ,0,100,0,'GE',-0)
                        cur1.execute(sql2)       
                        self.tdCreateData.explain_sql(sql2)
                        sql= sql + sql2
