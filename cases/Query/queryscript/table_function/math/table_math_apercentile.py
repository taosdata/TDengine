...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support int type\ support math function [hanshu = ['APERCENTILE']]
        case2:
        '''
    ...
    def data_check(self,sql1, throw=True) -> bool: 
        # apercentile(column_name, 50, "default") 与 apercentile(column_name, 50) 等价 ,t-digest是个概率值
        self.tdSql.query(sql1)
        value1 = self.tdSql.getData(0,0)
        self.logger.info("=============sqlsql1 %s"%sql1)
        
        sql0 = sql1.replace(',"default"',"").replace(',"t-digest"',"")
        self.logger.info("=============sqlsql0 %s"%sql0)
        self.tdSql.query(sql0)
        value0 = self.tdSql.getData(0,0)
        
        chazhi = abs(value1-value0)
        self.logger.info(chazhi)
        chazhi1 = abs(chazhi/value1)
        self.logger.info(chazhi1)

        if value1 == value0:  
                self.logger.debug(f"default= checkEqual success, elm={value1} expect_elm={value0}")                 
                return True 
        #t-digest和default做了差值比对，控制在20%之内，放开差值对比
            ...
            if throw:
                raise AssertionError(f"default=check  Equal error, elm={value1} expect_elm={value0}")
            else:
                self.logger.debug(f"default=check  Equal error, elm={value1} expect_elm={value0}")
            return False 
    ...
    def right_case_1_groupby(self):
        ...
        # 1: support math function [hanshu = ['APERCENTILE']]
        for i in (7,):
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        sql3 = "select %s from (select * from %s where  %s %s %s )" %(func_desc,self.table,qt_where,qt_like_match,qt_in_where)                        
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql3 ,1,1)
                        self.tdCreateData.data_matrix_equal('%s' %sql1 ,1,1,1,1,'%s' %sql3 ,1,1,1,1)
                        self.data_check(sql3)
                        cur1.execute(sql3)   
                        self.tdCreateData.explain_sql(sql3)
                        sql= sql + sql3
                        
                        sql3 = "select %s from %s where  %s %s %s ;" %(func_desc,self.table,qt_where,qt_like_match,qt_in_where)                        
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql3 ,1,1)
                        self.tdCreateData.data_matrix_equal('%s' %sql1 ,1,1,1,1,'%s' %sql3 ,1,1,1,1)
                        self.data_check(sql3)
                        cur1.execute(sql3)   
