...
class TDTestQuery(TDCase):
    ...
    def desc(self) -> str:
        case_description = '''
        case1:# support FLOOR,CEIL,ROUND function [hanshu = 'FLOOR','CEIL','ROUND']
        case2:
        '''
        return case_description

    #basic_param
    db = "table_numeric_floor"
    service_host = ""
    ...
    def right_case_1(self):
        ...
        # 1: support FLOOR,CEIL,ROUND function [hanshu = 'FLOOR','CEIL','ROUND']
        for i in (4,):
            ...
            try:
                ...
                for i in range(2,len(stable_where[2])+1):
                    ...
                    for qt_where in qt_where:
                        ...
                        sql2 = "select * from (select %s from %s where tbname in ('%s') and  %s %s %s order by ts)" %(func,self.table,self.table,qt_where,qt_like_match,qt_in_where)
                        self.tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        self.tdCreateData.data_matrix_equal('%s' %sql1 ,1,5,1,1,'%s' %sql2 ,1,5,1,1)
                        cur1.execute(sql2)
                        self.tdCreateData.explain_sql(sql2)
