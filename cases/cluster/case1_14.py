...
class Case12(ClusterCase):
    ...
    def loop_replica_backgroud(self, db_name, sleep_time , loop_times, endpoint):
        if not endpoint is None:
            client_0 = self.tdSql.get_connection(None, endpoint)
        else:
            client_0 = self.tdSql.get_connection(self._conf)
    ...
    def run(self):
        ...
        mthread=threading.Thread(target=self.alter_database_replica,args=(self.db_name, self.stable_name, self.table_name, self.table_num, self.row_num, self.replicas, self.slave_node))
        mthread.start()
