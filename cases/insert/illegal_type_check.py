...
class TestIllegalType(TDCase):
    ...
    def int_illegal_type_check(self):
        ...
        for test_type in int_type_list:
            ...
            for error_type in error_type_list:
                self.tdSql.error(f'create table if not exists {dbname}.tb_error using {dbname}.stb tags (now, {error_type})')
            self.tdSql.error(f'insert into {dbname}.tb values (now, {error_type})')
    ...
    def desc(self) -> str:
        case_description = '''
            int_illegal_type_check <jayden>: [TD-13419] : int_illegal_type_check;\n
            float_illegal_type_check <jayden>: [TD-13419] : float_illegal_type_check;\n
            bool_illegal_type_check <jayden>: [TD-13419] : bool_illegal_type_check;\n
            binary_illegal_type_check <jayden>: [TD-13419] : binary_illegal_type_check;
        '''
