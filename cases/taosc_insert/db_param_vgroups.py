...
class TestVgroups(TDCase):
    ...
    def vgroups_check(self):
        ...
        self.tdSql.error(f'create database if not exists {dbname} vgroups {self.copy_cfg["boundary"][-1] + 1} buffer {self.buffer_min}')
        # check logic
        if 'DATABASE_REPLICAS' not in os.environ.keys():
