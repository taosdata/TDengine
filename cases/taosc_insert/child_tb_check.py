...
class TestChildTb(TDCase):
    ...
    def child_tbname_without_backquote(self):
        ...
        for insert_str in symbol_list:
            ...
            for i in range(len(d_list)+1):
                ...
                self.tdSql.error(f'create table if not exists {self.dbname}.{tbname} using {stbname} tags (127)')
    ...
    def illegal_child_tbsql_check(self):
        ...
        base_sql = f'create table if not exists tb using {self.dbname}.{stbname} tags (now, 1, 2, 3, 4, 5, 6, 7, 8, 9.9, 10.1, True)'
