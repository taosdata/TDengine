...
class TestUnsignedBigintBoundary(TDCase):
    ...
    def ubigint_boundary_check(self):
        ...
        self.tdSql.execute(f'create table if not exists {dbname}.tb1 using {dbname}.stb tags ({self.tdCom.boundary_config["UBIGINT_MAX"]})')
        ...
        self.tdSql.execute(f'insert into {dbname}.tb2 values (now, {self.tdCom.boundary_config["UBIGINT_MAX"]})')
        ...
        self.tdSql.checkEqual(self.tdSql.query_data[0][1], self.tdCom.boundary_config["UBIGINT_MAX"])
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error1 (col_ts timestamp, c1 {self.tdCom.boundary_config["UBIGINT_MAX"]}) tags (t1 {self.tdCom.boundary_config["UBIGINT_MAX"]+1})')
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error2 (col_ts timestamp, c1 {self.tdCom.boundary_config["UBIGINT_MAX"]+1}) tags (t1 {self.tdCom.boundary_config["UBIGINT_MAX"]})')
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error3 (col_ts timestamp, c1 {self.tdCom.boundary_config["UBIGINT_MAX"]}) tags (t1 -1)')
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error4 (col_ts timestamp, c1 -1) tags (t1 0)')
        self.tdSql.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, {self.tdCom.boundary_config["UBIGINT_MAX"]+1})')
        self.tdSql.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, -1)')
        self.tdSql.error(f'insert into {dbname}.tb values (now-1h, {self.tdCom.boundary_config["UBIGINT_MAX"]+1})')
