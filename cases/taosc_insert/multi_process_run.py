...
class TestMultiProcessRun(TDCase):
    ...
    def check_default_shmsize(self):
        ...
        vnodeShmSize_ipcs = self._remote.cmd(self.fqdn, [f"ipcs -m | grep -w {vnodeShmid_infile} | awk \'{{print $5}}\'"])
        ...
        mnodeShmSize_ipcs = self._remote.cmd(self.fqdn, [f"ipcs -m | grep -w {self.mnodeShmSize_default} | grep -v {vnodeShmid_infile} | wc -l"])
    ...
    def check_shmsize_delivery(self):
        ...
        for mnodeShmSize in self.mnodeShmSize_list:
            for vnodeShmSize in self.vnodeShmSize_list:
                ...
                mnodeShmSize_ipcs = self._remote.cmd(self.fqdn, [f"ipcs -m | grep -w {mnodeShmSize} | grep -v {vnodeShmid_infile} | wc -l"])
                self.tdSql.checkEqual(int(mnodeShmSize_ipcs) >= 1, True)
        self.log_generation()
