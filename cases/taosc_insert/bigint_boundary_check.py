...
class TestBigintBoundary(TDCase):
    ...
    def bigint_boundary_check(self):
        ...
        self.tdSql.execute(f'create table if not exists {dbname}.tb2 using {dbname}.stb tags (-{self.tdCom.boundary_config["BIGINT_MAX"]+1})')

        self.tdSql.execute(f'insert into {dbname}.tb1 values (now, -{self.tdCom.boundary_config["BIGINT_MAX"]+1})')
        ...
        # self.tdSql.checkEqual(self.tdSql.query_data[0][1], -self.tdCom.boundary_config["BIGINT_MAX"]-1)
        # self.tdSql.query(f'select t1, c1 from {dbname}.tb2')
        # self.tdSql.checkEqual(self.tdSql.query_data[0][0], -self.tdCom.boundary_config["BIGINT_MAX"]-1)
        ...
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error3 (col_ts timestamp, c1 {self.tdCom.boundary_config["BIGINT_MAX"]}) tags (t1 -{self.tdCom.boundary_config["BIGINT_MAX"]+2})')
        self.tdSql.error(f'create stable if not exists {dbname}.stb_error4 (col_ts timestamp, c1 -{self.tdCom.boundary_config["BIGINT_MAX"]+2}) tags (t1 -{self.tdCom.boundary_config["BIGINT_MAX"]})')
        ...
        self.tdSql.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, -{self.tdCom.boundary_config["BIGINT_MAX"]+2})')
        self.tdSql.error(f'insert into {dbname}.tb values (now-1h, {self.tdCom.boundary_config["BIGINT_MAX"]+1})')
        self.tdSql.error(f'insert into {dbname}.tb values (now-1h, -{self.tdCom.boundary_config["BIGINT_MAX"]+2})')

        self.tdSql.execute(f'create table if not exists {dbname}.tb3 (ts timestamp, c1 bigint)')
        self.tdSql.execute(f'insert into {dbname}.tb3 values (now, {self.tdCom.boundary_config["BIGINT_MAX"]})')
        self.tdSql.execute(f'insert into {dbname}.tb3 values (now, -{self.tdCom.boundary_config["BIGINT_MAX"]+1})')
        self.tdSql.query(f'select c1 from {dbname}.tb3 where c1={self.tdCom.boundary_config["BIGINT_MAX"]}')
        self.tdSql.checkEqual(self.tdSql.query_data[0][0], self.tdCom.boundary_config["BIGINT_MAX"])
        self.tdSql.query(f'select c1 from {dbname}.tb3 where c1=-{self.tdCom.boundary_config["BIGINT_MAX"]+1}')
        self.tdSql.checkEqual(self.tdSql.query_data[0][0], -self.tdCom.boundary_config["BIGINT_MAX"]-1)
        self.tdSql.error(f'insert into {dbname}.tb3 values (now, {self.tdCom.boundary_config["BIGINT_MAX"]+1})')
        self.tdSql.error(f'insert into {dbname}.tb3 values (now, -{self.tdCom.boundary_config["BIGINT_MAX"]+2})')
