...
class TestBinaryBoundary(TDCase):
    ...
    def binary_length_check(self):
        ...
        self.tdSql.execute(f'insert into {dbname}.tb2 values (now, "{str_16374}")')
        self.tdSql.query(f'select c1 from {dbname}.tb2 where c1="{str_16374}"')
        self.tdSql.checkEqual(self.tdSql.query_data[0][0], str_16374)
        self.tdSql.error(f'insert into {dbname}.tb3 values (now, "{str_16375}")')
