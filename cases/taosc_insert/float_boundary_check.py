...
class TestFloatBoundary(TDCase):
    ...
    def float_boundary_check(self):
        ...
        self.tdSql.execute(f'insert into {dbname}.tb3 values (now, -{self.tdCom.boundary_config["FLOAT_MAX"]})')
        self.tdSql.query(f'select c1 from {dbname}.tb3 where c1={self.tdCom.boundary_config["FLOAT_MAX"]}')
        self.tdSql.checkEqual(self.tdSql.query_data[0][0], self.tdCom.boundary_config["FLOAT_MAX"])
        self.tdSql.query(f'select c1 from {dbname}.tb3 where c1=-{self.tdCom.boundary_config["FLOAT_MAX"]}')
        self.tdSql.checkEqual(self.tdSql.query_data[0][0], -self.tdCom.boundary_config["FLOAT_MAX"])
