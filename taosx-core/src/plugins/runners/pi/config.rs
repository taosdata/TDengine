...
use crate::runners::pi::{pi_datasets, PiError};
...
impl PiConfig {
    ...
    fn parse_server_name(dsn: &Dsn) -> anyhow::Result<String> {
        dsn.addresses.first().and_then(|addr| addr.host.clone())
            .ok_or(anyhow::anyhow!("PIServerName is required"))
    }

    fn parse_system_name(dsn: &Dsn, default_name: String) -> String {
        dsn.params.get("PISystemName").map(|v| v.to_string()).unwrap_or(default_name)
    }

    fn parse_database(dsn: &Dsn) -> anyhow::Result<String> {
        dsn.subject.clone().ok_or(anyhow::anyhow!("Database name is required"))
    }
    ...
    fn parse_pi_data_pipes_instances(dsn: &Dsn) -> anyhow::Result<Option<u32>> {
        dsn.params
            ...
            .map(|v| v.parse::<u32>()
                .map_err(|err| anyhow::anyhow!("invalid PIDataPipesInstances, cause: {}", err.to_string()))
            )
            .transpose()
    }
    ...
    fn parse_af_data_pipes_instances(dsn: &Dsn) -> anyhow::Result<Option<u32>> {
        dsn.params
            ...
            .map(|v| v.parse::<u32>()
                .map_err(|err| anyhow::anyhow!("invalid AFDataPipesInstances, cause: {}", err.to_string())))
            .transpose()
    }

    fn parse_max_wait_len(dsn: &Dsn) -> anyhow::Result<Option<u32>> {
        let max_wait_len = dsn.params.get("MaxWaitLen")
            .map(|v| v.parse::<u32>().map_err(|err| anyhow::anyhow!("invalid MaxWaitLen, cause: {}", err.to_string())))
            .transpose()?;
    ...
    fn parse_update_interval(dsn: &Dsn) -> anyhow::Result<Option<u32>> {
        let update_interval = dsn.params.get("UpdateInterval")
            .map(|v| v.parse::<u32>()
                .map_err(|err| anyhow::anyhow!("invalid UpdateInterval, cause: {}", err.to_string())))
            .transpose()?;
        if let Some(ui) = update_interval {
            if ui < 100 || ui > 60000 {
                return Err(anyhow::anyhow!("UpdateInterval should be in range 100..60000"));
            }
        }
        Ok(update_interval)
    }
    ...
    fn parse_max_backfill_range_days(dsn: &Dsn) -> anyhow::Result<Option<u32>> {
        dsn.params
            ...
            .map(|v| v.parse::<u32>()
                .map_err(|err| anyhow::anyhow!("invalid MaxBackfillRangeDays, cause: {}", err.to_string())))
            .transpose()
    }
    ...
    fn parse_point_list(dsn: &Dsn) -> anyhow::Result<Vec<String>> {
        Ok(get_string_from_param_or_file(&mut dsn.clone(), "PointList", false, Some(","))
    ...
    fn parse_from_tdengine_last_time(dsn: &Dsn) -> anyhow::Result<Option<bool>> {
        dsn.params
            ...
            .map(|v| {
                v.parse::<bool>()
                    .map_err(|err| anyhow::anyhow!("invalid FromTDengineLastTime, cause: {}",err.to_string()))
            })
            .transpose()
    }
    ...
    fn parse_to_tdengine_first_time(dsn: &Dsn) -> anyhow::Result<Option<bool>> {
        dsn.params
            ...
            .map(|v| {
                v.parse::<bool>()
                    .map_err(|err| anyhow::anyhow!("invalid ToTDengineFirstTime, cause: {}", err.to_string()))
            })
            .transpose()
    }
    ...
    fn parse_backfill_start_time(dsn: &Dsn) -> anyhow::Result<Option<Datetime>> {
        dsn.params
            ...
            .map(|v| {
                Self::parse_date_time(v.as_str())
                    .map_err(|err| anyhow::anyhow!("invalid BackfillStartTime, cause: {}", err.to_string()))
            })
            .transpose()
    }
    ...
    fn parse_backfill_end_time(dsn: &Dsn) -> anyhow::Result<Option<Datetime>> {
        dsn.params
            ...
            .map(|v| {
                Self::parse_date_time(v.as_str())
                    .map_err(|err| anyhow::anyhow!("invalid BackfillEndTime, cause: {}", err.to_string()))
            })
            .transpose()
    }
    ...
    fn parse_date_time(date_time: &str) -> anyhow::Result<Datetime> {
        let parsed_time = NaiveDateTime::parse_from_str(date_time, "%Y-%m-%d %H:%M:%S")
            .map_err(|err| {
                anyhow::anyhow!("failed to parse date time: {}, cause: {}", date_time, err.to_string())
            })?
    ...
    pub async fn new(
        ...
        let backfill_start_time = Self::parse_backfill_start_time(&dsn)?;
        ...
        match (from_tdengine_last_time, to_tdengine_first_time) {
            (Some(true), Some(true)) => {
                return Err(anyhow::anyhow!("Only one of the BackfillStartTime and BackfillEndTime can be automatically set"));
            }
            _ => {}
        }
...
mod tests {
    ...
    fn test_parse_pi_data_pipes_instances() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid PIDataPipesInstances, cause: invalid digit found in string", config.unwrap_err().to_string());
    }
    ...
    fn test_parse_af_data_pipes_instances() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid AFDataPipesInstances, cause: invalid digit found in string", config.unwrap_err().to_string());
    }
    ...
    fn test_parse_max_wait_len() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid MaxWaitLen, cause: invalid digit found in string", config.unwrap_err().to_string());

        let dsn = Dsn::from_str("pi:///?MaxWaitLen=0").unwrap();
        let config = PiConfig::parse_max_wait_len(&dsn);
        assert!(config.is_err());
        assert_eq!("MaxWaitLen should be in range 1..10000", config.unwrap_err().to_string());
    ...
    fn test_parse_update_interval() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid UpdateInterval, cause: invalid digit found in string", config.unwrap_err().to_string());

        let dsn = Dsn::from_str("pi:///?UpdateInterval=99").unwrap();
        let config = PiConfig::parse_update_interval(&dsn);
        assert!(config.is_err());
        assert_eq!("UpdateInterval should be in range 100..60000", config.unwrap_err().to_string());
    ...
    fn test_parse_max_backfill_range_days() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid MaxBackfillRangeDays, cause: invalid digit found in string", config.unwrap_err().to_string());
    }
    ...
    fn test_parse_from_tdengine_last_time() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid FromTDengineLastTime, cause: provided string was not `true` or `false`", config.unwrap_err().to_string());
    }
    ...
    fn test_parse_to_tdengine_first_time() {
        ...
        assert!(config.is_err());
        assert_eq!("invalid ToTDengineFirstTime, cause: provided string was not `true` or `false`", config.unwrap_err().to_string());
    }
    ...
    fn test_parse_date_time() {
        ...
        assert!(config.is_err());
        assert_eq!("failed to parse date time: 2021-01-01 00:00:00.000, cause: trailing input", config.unwrap_err().to_string());
    }
    ...
    async fn test_config() {
        ...
        let dsn: Dsn = Dsn::from_str("pi://WIN-2OA23UM12TN/Met1?PISystemName=other&point_file=app\napp\napp").unwrap();
