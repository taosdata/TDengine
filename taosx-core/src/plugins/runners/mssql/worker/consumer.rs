...
use crate::runners::mssql::worker::set_breakpoint;
...
#[cfg(test)]
mod tests {
    use crate::runners::mssql::{appender::to_schema, worker::producer::Producer};
    ...
    async fn test_consumer() {
        ...
        // query for schema
        let mut query = MssqlQuery::try_new(config.connect.clone(), config.task.time_zone.clone())
            .await
            .unwrap();
        let col_map = query
            .select_for_schema("select * from t_metric")
            .await
            .unwrap();
