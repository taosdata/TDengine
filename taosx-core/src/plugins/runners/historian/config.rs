    ...
    pub fn from_dsn(dsn: &Dsn) -> anyhow::Result<Self> {
        ...
        let config = SourceConfig {
            ...
            table: dsn
                .params
                .get("table")
                .unwrap_or(&"dbo.History".to_string())
                .to_string(),
            tags: dsn
                .params
                .get("tags")
...
mod tests {
    ...
    fn test_source_config() {
        ...
        assert_eq!(
            ...
            DateTime::parse_from_rfc3339("1970-01-01T00:00:00.0Z").unwrap()
        );
        assert_eq!(config.end_date_time, default_end_datetime());
        assert_eq!(config.retrieve_mode, "full");

        let dsn = Dsn::from_str(
            "historian://taosdata:taosdata@192.168.1.92:1234?\
        ...
        assert_eq!(
            config.begin_date_time,
            DateTime::parse_from_rfc3339("2023-08-01T08:01:02.52+08:00").unwrap()
        );
        assert_eq!(
            config.end_date_time,
            DateTime::parse_from_rfc3339("2023-08-30T08:01:02.52+08:00").unwrap()
        );
        assert_eq!(config.retrieve_mode, "full");
    }
}
