                    ...
                    Ok(rs) => {
                        ...
                        for (exist, (tagname, expect)) in rs.iter().zip(tags) {
                            if exist != expect {
                                ...
                                if let Err(err) =
                                    taos.exec_with_req_id(alter_set_sql, req_id.next()).await
                                {
                            ...
                            if err_str.contains("0x2653") {
                                ...
                                if alter_sqls.is_some() {
                                    for alter_sql in alter_sqls.unwrap() {
                                        ...
                                        taos.exec_with_req_id(alter_sql, req_id.next()).await?;
                                    }
                                }
                            } else {
...
async fn consume_point_record(
    ...
    for message in record.records() {
        // todo: handle value_transform, ts_transform, rts_transform
        // let message = handle_transform(message);
                            ...
                            if errstr.contains("[0x2603]") || errstr.contains("0x0200") {
                                    ...
                                    zip(child_table_create_sqls, child_table_counts_vec)
                                {
                                    ...
                                    match taos
                                        .as_ref()
                                        .unwrap()
                                        .exec_with_req_id(&create_child_sql, req_id.next())
                                        .await
                                    {
                                ...
                                for column_config in &table_config.column_configs {
                                    ...
                                    if need_add {
                                        ...
                                        let res = taos
                                            .as_ref()
                                            .unwrap()
                                            .exec_with_req_id(&add_column_sql, req_id.next())
                                            .await;
                                        if let Err(err) = res {
                                            tracing::warn!("describe error: {err:#}");
                                            let code: i32 = err.code().into();
