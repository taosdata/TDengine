                            ...
                            if errstr.contains("[0x2603]") || errstr.contains("0x0200") {
                                ...
                                for (create_child_sql, child_table_count) in zip(child_table_create_sqls, child_table_counts_vec) {
                                    ...
                                    .await
                                    {
                                        ...
                                        Ok(_n) => {
                                             counter!(METRIC_CHILD_TABLE_CREATED, child_table_count as u64);
                                        },
                                        Err(err) => {
                                ...
                                for column_config in &table_config.column_configs {
                                    ...
                                    if need_add {
                                        ...
                                        let res =
                                            taos.as_ref().unwrap().exec(&add_column_sql).await;
                                        if let Err(err) = res {
                                            tracing::warn!("describe error: {err:#}");
                                            let code: i32 = err.code().into();
                                            let _err_str = err.to_string();
                        ...
                        loop {
                            ...
                            if let Err(err) = taos.as_ref().unwrap().write_raw_block(&raw).await {
                                ...
                                *count += raw.nrows();
                                ...
                                counter!(METRIC_POINTS, (raw.nrows() * raw.column_views().len()) as u64);
                                if let Some(transferred) = transferred {
                                    transferred
                                        .records
