...
static int csvValidateParamTsFormat(const char* csv_ts_format) {
  ...
  struct tm test_tm = {
      .tm_year = 70, .tm_mon = 0, .tm_mday = 1, .tm_hour = 0, .tm_min = 0, .tm_sec = 0, .tm_isdst = -1};
  mktime(&test_tm);
...
static int csvParseParameter() {
  ...
  if (g_arguments->output_path[len - 1] != '/') {
    int n =
        snprintf(g_arguments->output_path_buf, sizeof(g_arguments->output_path_buf), "%s/", g_arguments->output_path);
...
int csvGenCreateStbSql(SDataBase* db, SSuperTable* stb, char* buf, int size) {
  ...
  pos +=
      snprintf(buf + pos, size - pos, g_arguments->escape_character ? "`%s`.`%s`" : "%s.%s", db->dbName, stb->stbName);
  if (pos <= 0 || pos >= size) return -1;
  ...
  for (size_t i = 0; i < stb->cols->size; ++i) {
    ...
    if (col->type == TSDB_DATA_TYPE_BINARY || col->type == TSDB_DATA_TYPE_NCHAR ||
        col->type == TSDB_DATA_TYPE_VARBINARY || col->type == TSDB_DATA_TYPE_GEOMETRY) {
  ...
  for (size_t i = 0; i < stb->tags->size; ++i) {
    ...
    if (tag->type == TSDB_DATA_TYPE_BINARY || tag->type == TSDB_DATA_TYPE_NCHAR ||
        tag->type == TSDB_DATA_TYPE_VARBINARY || tag->type == TSDB_DATA_TYPE_GEOMETRY) {
...
static int csvInitWriteMeta(SDataBase* db, SSuperTable* stb, CsvWriteMeta* write_meta) {
  ...
  if (ret < 0) {
    errorPrint("Failed to generate csv header data. database: %s, super table: %s, naming type: %d.\n", db->dbName,
               stb->stbName, write_meta->naming_type);
    return -1;
  }
...
static int csvGetFileFullname(CsvWriteMeta* write_meta, CsvThreadMeta* thread_meta, char* fullname, size_t size) {
  ...
  switch (write_meta->naming_type) {
    ...
    case CSV_NAMING_I_TIME_SLICE: {
      csvConvertTime2String(thread_meta->start_secs, write_meta->stb->csv_ts_format, start_time_buf,
                            sizeof(start_time_buf));
      csvConvertTime2String(thread_meta->end_secs, write_meta->stb->csv_ts_format, end_time_buf, sizeof(end_time_buf));
      ret = snprintf(fullname, size, "%s%s_%s_%s.csv%s", base_path, file_prefix, start_time_buf, end_time_buf,
                     gzip_suffix);
      break;
    }
    case CSV_NAMING_B_THREAD: {
    ...
    case CSV_NAMING_B_THREAD_TIME_SLICE: {
      ...
      csvConvertTime2String(thread_meta->start_secs, write_meta->stb->csv_ts_format, start_time_buf,
                            sizeof(start_time_buf));
      csvConvertTime2String(thread_meta->end_secs, write_meta->stb->csv_ts_format, end_time_buf, sizeof(end_time_buf));
      ret = snprintf(fullname, size, "%s%s_%s_%s_%s.csv%s", base_path, file_prefix, thread_buf, start_time_buf,
                     end_time_buf, gzip_suffix);
      break;
    }
    default: {
      ret = -1;
...
static int csvWriteFile(CsvFileHandle* fhdl, uint64_t ctb_idx, int64_t cur_ts, int64_t* ck, CsvWriteMeta* write_meta,
  ...
  if (ret <= 0) {
    errorPrint(
        "Failed to generate csv column data. database: %s, super table: %s, naming type: %d, thread index: %zu, ctb "
        "index: %" PRIu64 ".\n",
  ...
  if (thread_meta->output_header) {
    ...
    if (ret != CSV_ERR_OK) {
      errorPrint(
          "Failed to write csv header data. database: %s, super table: %s, naming type: %d, thread index: %zu, ctb "
          "index: %" PRIu64 ".\n",
          db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id, ctb_idx);
      return -1;
    }
  ...
  if (ret != CSV_ERR_OK) {
    errorPrint(
        "Failed to write csv column data. database: %s, super table: %s, naming type: %d, thread index: %zu, ctb "
        "index: %" PRIu64 ".\n",
        db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id, ctb_idx);
    return -1;
  }
  ...
  if (ret != CSV_ERR_OK) {
    errorPrint(
        "Failed to write csv tag data. database: %s, super table: %s, naming type: %d, thread index: %zu, ctb index: "
        "%" PRIu64 ".\n",
        db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id, ctb_idx);
    return -1;
  }
  ...
  if (ret != CSV_ERR_OK) {
    errorPrint(
        "Failed to write csv line break data. database: %s, super table: %s, naming type: %d, thread index: %zu, ctb "
        "index: %" PRIu64 ".\n",
        db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id, ctb_idx);
    return -1;
  }
...
static void* csvGenStbThread(void* arg) {
  ...
  if (!buf) {
    errorPrint(
        "Failed to malloc csv column buffer. database: %s, super table: %s, naming type: %d, thread index: %zu.\n",
        db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id);
    goto end;
  }

  CsvRowColsBuf cols_buf = {.buf = buf, .buf_size = buf_size, .length = 0};
  ...
  while (cur_ts < write_meta->end_ts) {
    ...
    if (ret < 0) {
      errorPrint(
          "Failed to generate csv filename. database: %s, super table: %s, naming type: %d, thread index: %zu.\n",
          db->dbName, stb->stbName, write_meta->naming_type, thread_meta->thread_id);
      goto end;
    }
    ...
    while (slice_cur_ts < slice_end_ts) {
      ...
      for (ctb_idx = 0; ctb_idx < thread_meta->ctb_count; ++ctb_idx) {
        for (slice_ctb_cur_ts = slice_cur_ts; slice_ctb_cur_ts < slice_batch_ts;
             slice_ctb_cur_ts += write_meta->stb->timestamp_step) {
...
static int csvGenStbProcess(SDataBase* db, SSuperTable* stb) {
  ...
  if (ts_elapse > 0) {
    succPrint("Spent %.6f seconds to insert rows: %" PRIu64 " with %zu thread(s) into %s, at a rate of %.2f rows/s.\n",
              ts_elapse / 1000.0, total_rows, write_meta->total_threads, g_arguments->output_path,
