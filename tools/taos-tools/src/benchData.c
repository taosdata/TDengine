...
const char *locations[] = {"California.SanFrancisco", "California.LosAngles", "California.SanDiego",
                           "California.SanJose",      "California.PaloAlto",  "California.Campbell",
                           "California.MountainView", "California.Sunnyvale", "California.SantaClara",
...
float funValueFloat(Field *field, int32_t angle, int32_t loop) {
  ...
  if (field->multiple != 0) funVal *= field->multiple;
...
char *genPrepareSql(SSuperTable *stbInfo, char *tagData, uint64_t tableSeq, char *db) {
  ...
  if (stbInfo->autoTblCreating) {
    ...
    n = snprintf(prepare + len, TSDB_MAX_ALLOWED_SQL_LEN - len,
                 "INSERT INTO ? USING `%s`.`%s` TAGS (%s) %s VALUES(?,%s)", db, stbInfo->stbName, tagQ, ttl, colQ);
  } else {
    if (workingMode(g_arguments->connMode, g_arguments->dsn) == CONN_MODE_NATIVE) {
      // native
      n = snprintf(prepare + len, TSDB_MAX_ALLOWED_SQL_LEN - len, "INSERT INTO ? VALUES(?,%s)", colQ);
    } else {
      // websocket
      bool ntb = stbInfo->tags == NULL || stbInfo->tags->size == 0;  // normal table
      colNames = genColNames(stbInfo->cols, !ntb);
      n = snprintf(prepare + len, TSDB_MAX_ALLOWED_SQL_LEN - len, "INSERT INTO `%s`.`%s`(%s) VALUES(%s,%s)", db,
                   stbInfo->stbName, colNames, ntb ? "?" : "?,?", colQ);
    }
  }
  len += n;
  ...
  if (g_arguments->prepared_rand < g_arguments->reqPerReq) {
    infoPrint(
        ...
        "auto set to %" PRId64 "\n",
        g_arguments->reqPerReq, g_arguments->prepared_rand, g_arguments->prepared_rand);
...
static bool getSampleFileNameByPattern(char *filePath, SSuperTable *stbInfo, int64_t child) {
  ...
  snprintf(filePath + offset, MAX_PATH_LEN - offset, "%s", stbInfo->childTblArray[child]->name);
  size_t len = strlen(stbInfo->childTblArray[child]->name);
  snprintf(filePath + offset + len, MAX_PATH_LEN - offset - len, "%s", pos + 4);
  return true;
}
...
int tmpStr(char *tmp, int iface, Field *field, int64_t k) {
  if (field->values) {
    ...
    if (g_arguments->chinese) {
      snprintf(tmp, field->length, "%s", locations_chinese[tmpRand % 10]);
    } else if (SML_IFACE == iface) {
      snprintf(tmp, field->length, "%s", locations_sml[tmpRand % 10]);
    } else {
      snprintf(tmp, field->length, "%s", locations[tmpRand % 10]);
    }
  } else {
...
int tmpBlobStr(char **t, int iface, Field *field, int64_t k) {
  ...
  if (field->values) {
    ...
    if (arraySize) {
      tools_cJSON *buf = tools_cJSON_GetArrayItem(field->values, taosRandom() % arraySize);
      snprintf(tmp, len, "%s", buf->valuestring);
    } else {
      errorPrint(
          "%s() cannot read correct value "
          "from json file. array size: %d\n",
    ...
    unsigned int tmpRand = taosRandom();
    int32_t      idx = tmpRand % 10;
    if (g_arguments->chinese) {
      snprintf(tmp, strlen(locations_chinese[idx]), "%s", locations_chinese[idx]);
    } else if (SML_IFACE == iface) {
      snprintf(tmp, strlen(locations_sml[idx]), "%s", locations_sml[idx]);
    } else {
      snprintf(tmp, strlen(locations[idx]), "%s", locations[idx]);
    }
  } else {
    if (field->gen == GEN_ORDER) {
      snprintf(tmp, len, "%" PRId64, k);
    } else {
      rand_string(tmp, len, g_arguments->chinese);
    }
  }
  *t = tmp;
  return 0;
  ...
  if (field->funType != FUNTYPE_NONE) {
    ...
    if (field->max != field->min) {
      if (field->gen == GEN_ORDER) {
        ...
        floatTmp += ((taosRandom() % (field->max - field->min)) + (taosRandom() % 1000) / 1000.0);
      }
    }
    if (g_arguments->demo_mode && i == 0) {
      floatTmp = (float)(9.8 + 0.04 * (taosRandom() % 10) + floatTmp / 1000000000);
    } else if (g_arguments->demo_mode && i == 2) {
      floatTmp = (float)((105 + taosRandom() % 10 + floatTmp / 1000000000) / 360);
    }
  }

  if (field->scalingFactor > 0) {
    if (field->scalingFactor > 1) floatTmp = floatTmp / field->scalingFactor;
...
double tmpDoubleImpl(Field *field, int32_t angle, int32_t k) {
  ...
  if (field->funType != FUNTYPE_NONE) {
    ...
    if (field->gen == GEN_ORDER) {
      ...
      doubleTmp += ((taosRandom() % (field->max - field->min)) + taosRandom() % 1000000 / 1000000.0);
    }
  }
...
static int tmpJson(char *sampleDataBuf, int bufLen, int64_t pos, int fieldsSize, Field *field) {
  ...
  for (int j = 0; j < fieldsSize; ++j) {
    // key
    n += snprintf(sampleDataBuf + pos + n, bufLen - pos - n, "\"k%d\":", j);
    if (n < 0 || n >= bufLen - pos) {
      errorPrint("%s() LN%d snprintf overflow\n", __func__, __LINE__);
      return -1;
    }
    ...
    n += snprintf(sampleDataBuf + pos + n, bufLen - pos - n, "\"%s\",", buf);
    if (n < 0 || n >= bufLen - pos) {
      errorPrint("%s() LN%d snprintf overflow\n", __func__, __LINE__);
      tmfree(buf);
      return -1;
  ...
  n += snprintf(sampleDataBuf + pos + n - 1, bufLen - pos - n, "}'");
  if (n < 0 || n >= bufLen - pos) {
    errorPrint("%s() LN%d snprintf overflow\n", __func__, __LINE__);
    return -1;
  }
...
static int generateRandDataSQL(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow,
    ...
    for (int i = 0; i < fieldsSize; ++i) {
      ...
      switch (field->type) {
        case TSDB_DATA_TYPE_BOOL: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s,", boolTmp ? "true" : "false");
          break;
        }
        case TSDB_DATA_TYPE_TINYINT: {
          int8_t tinyint = tmpInt8Impl(field, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", tinyint);
          break;
        }
        case TSDB_DATA_TYPE_UTINYINT: {
          uint8_t utinyint = tmpUint8Impl(field, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%u,", utinyint);
          break;
        }
        case TSDB_DATA_TYPE_SMALLINT: {
          int16_t smallint = tmpInt16Impl(field, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", smallint);
          break;
        }
        case TSDB_DATA_TYPE_USMALLINT: {
          uint16_t usmallint = tmpUint16Impl(field, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%u,", usmallint);
          break;
        }
        case TSDB_DATA_TYPE_INT: {
          int32_t intTmp = tmpInt32Impl(field, i, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", intTmp);
          break;
        }
        case TSDB_DATA_TYPE_BIGINT: {
          int64_t bigintTmp = tmpInt64Impl(field, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%" PRId64 ",", bigintTmp);
          break;
        }
        case TSDB_DATA_TYPE_UINT: {
          uint32_t uintTmp = tmpUint32Impl(field, i, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%u,", uintTmp);
          break;
        }
        case TSDB_DATA_TYPE_UBIGINT:
        case TSDB_DATA_TYPE_TIMESTAMP: {
          uint64_t ubigintTmp = tmpUint64Impl(field, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%" PRIu64 ",", ubigintTmp);
          break;
        }
        case TSDB_DATA_TYPE_FLOAT: {
          float floatTmp = tmpFloatImpl(field, i, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%f,", floatTmp);
          break;
        }
        case TSDB_DATA_TYPE_DOUBLE: {
          double double_ = tmpDoubleImpl(field, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%f,", double_);
          break;
        }
        case TSDB_DATA_TYPE_DECIMAL: {
          ...
          if (ret != 0) {
            errorPrint("%s() LN%d precision: %d, scale: %d, high: %" PRId64 ", low: %" PRIu64 "\n", __func__, __LINE__,
                       field->precision, field->scale, DECIMAL128_HIGH_WORD(&dec), DECIMAL128_LOW_WORD(&dec));
            return -1;
          }
        ...
        case TSDB_DATA_TYPE_DECIMAL64: {
          ...
          if (ret != 0) {
            errorPrint("%s() LN%d precision: %d, scale: %d, value: %" PRId64 "\n", __func__, __LINE__, field->precision,
                       field->scale, DECIMAL64_GET_VALUE(&dec));
            return -1;
          }
          size_t decLen = strlen(sampleDataBuf + pos);
          n = snprintf(sampleDataBuf + pos + decLen, bufLen - pos - decLen, ",");
        ...
        case TSDB_DATA_TYPE_BLOB: {
          ...
          if (tmpBlobStr(&tmp, stbInfo->iface, field, k) != 0) {
            errorPrint("Failed to generate blob data\n");
            return -1;
          }
...
static int fillStmt(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow, BArray *fields,
    ...
    for (int i = 0; i < fieldsSize; ++i) {
      ...
      switch (field->type) {
        case TSDB_DATA_TYPE_BOOL: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", boolTmp);
          break;
        }
        case TSDB_DATA_TYPE_TINYINT: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", tinyintTmp);
          break;
        }
        case TSDB_DATA_TYPE_UTINYINT: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%u,", utinyintTmp);
          break;
        }
        case TSDB_DATA_TYPE_SMALLINT: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%d,", smallintTmp);
          break;
        }
        case TSDB_DATA_TYPE_USMALLINT: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%u,", usmallintTmp);
          break;
        }
        case TSDB_DATA_TYPE_INT: {
          int32_t intTmp = tmpInt32Impl(field, i, angle, k);
        ...
        case TSDB_DATA_TYPE_DOUBLE: {
          ...
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%f,", doubleTmp);
          break;
        }
        case TSDB_DATA_TYPE_BINARY:
        case TSDB_DATA_TYPE_VARBINARY:
        ...
        case TSDB_DATA_TYPE_BLOB: {
          char *tmp = NULL;
          if (0 != tmpBlobStr(&tmp, stbInfo->iface, field, k)) {
            free(tmp);
            return -1;
          }
          if (childCol) {
  ...
  skip_stmt:
    if (pos > 0) *(sampleDataBuf + pos - 1) = 0;
    angle += stbInfo->timestamp_step / stbInfo->angle_step;
    if (angle > 360) {
      angle -= 360;
    }
...
static int generateRandDataStmtForChildTable(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow,
                                             BArray *fields, int64_t loop, BArray *childCols) {
  ...
  for (int i = 0; i < fields->size; ++i) {
    ...
    if (field->type == TSDB_DATA_TYPE_BINARY || field->type == TSDB_DATA_TYPE_NCHAR) {
      childField->stmtData.data = benchCalloc(1, loop * (field->length + 1), true);
    } else {
      childField->stmtData.data = benchCalloc(1, loop * field->length, true);
    }
  ...
  return fillStmt(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, false, childCols);
}

static int generateRandDataStmt(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow,
                                BArray *fields, int64_t loop, bool tag) {
    ...
    for (int i = 0; i < fieldsSize; ++i) {
      ...
      switch (field->type) {
        case TSDB_DATA_TYPE_BOOL: {
          bool boolTmp = tmpBool(field);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%s ", field->name, boolTmp ? "true" : "false");
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s ", boolTmp ? "true" : "false");
          }
          break;
        ...
        case TSDB_DATA_TYPE_TINYINT: {
          int8_t tinyint = tmpInt8Impl(field, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%di8 ", field->name, tinyint);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%di8 ", tinyint);
          }
          break;
        }
        case TSDB_DATA_TYPE_UTINYINT: {
          uint8_t utinyint = tmpUint8Impl(field, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%uu8 ", field->name, utinyint);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%uu8 ", utinyint);
          }
          break;
        }
        case TSDB_DATA_TYPE_SMALLINT: {
          int16_t smallint = tmpInt16Impl(field, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%di16 ", field->name, smallint);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%di16 ", smallint);
          }
          break;
        }
        case TSDB_DATA_TYPE_USMALLINT: {
          uint16_t usmallint = tmpUint16Impl(field, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%uu16 ", field->name, usmallint);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%uu16 ", usmallint);
          }
          break;
        }
        case TSDB_DATA_TYPE_INT: {
          int32_t intTmp = tmpInt32Impl(field, i, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%di32 ", field->name, intTmp);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%di32 ", intTmp);
          }
          break;
        }
        case TSDB_DATA_TYPE_BIGINT: {
          int64_t bigintTmp = tmpInt64Impl(field, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%" PRId64 "i64 ", field->name, bigintTmp);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%" PRId64 "i64 ", bigintTmp);
          }
          break;
        }
        case TSDB_DATA_TYPE_UINT: {
          uint32_t uintTmp = tmpUint32Impl(field, i, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%uu32 ", field->name, uintTmp);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%uu32 ", uintTmp);
          }
          break;
        }
        case TSDB_DATA_TYPE_UBIGINT:
        case TSDB_DATA_TYPE_TIMESTAMP: {
          uint64_t ubigintTmp = tmpUint64Impl(field, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%" PRIu64 "u64 ", field->name, ubigintTmp);
          } else {
        ...
        case TSDB_DATA_TYPE_FLOAT: {
          float floatTmp = tmpFloatImpl(field, i, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%ff32 ", field->name, floatTmp);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%ff32 ", floatTmp);
          }
          break;
        }
        case TSDB_DATA_TYPE_DOUBLE: {
          double double_ = tmpDoubleImpl(field, angle, k);
          if (tag) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%ff64 ", field->name, double_);
          } else {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%ff64 ", double_);
          }
          break;
        }
        case TSDB_DATA_TYPE_BINARY:
        ...
        case TSDB_DATA_TYPE_NCHAR: {
          ...
          if (field->type == TSDB_DATA_TYPE_BINARY || field->type == TSDB_DATA_TYPE_VARBINARY) {
            if (tag) {
              n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=L\"%s\" ", field->name, tmp);
            } else {
              n = snprintf(sampleDataBuf + pos, bufLen - pos, "\"%s\" ", tmp);
            }
            if (n < 0 || n >= bufLen - pos) {
            ...
            if (tag) {
              ...
              n = snprintf(sampleDataBuf + pos, bufLen - pos, "L\"%s\" ", tmp);
            }
            if (n < 0 || n >= bufLen - pos) {
              errorPrint("%s() LN%d snprintf overflow\n", __func__, __LINE__);
              tmfree(tmp);
...
static int generateRandDataSmlJson(SSuperTable *stbInfo, char *sampleDataBuf, int bufLen, int lenOfOneRow,
                                   BArray *fields, int64_t loop, bool tag) {
  int angle = stbInfo->startTimestamp % 360;  // 0 ~ 360
  for (int64_t k = 0; k < loop; ++k) {
    int64_t pos = k * lenOfOneRow;
...
static int generateRandDataSmlLine(SSuperTable *stbInfo, char *sampleDataBuf, int bufLen, int lenOfOneRow,
                                   BArray *fields, int64_t loop, bool tag) {
  int angle = stbInfo->startTimestamp % 360;  // 0 ~ 360
  for (int64_t k = 0; k < loop; ++k) {
    int64_t pos = k * lenOfOneRow;
    int     n = 0;
    if (tag) {
      n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s,", stbInfo->stbName);
      if (n < 0 || n >= bufLen - pos) {
        errorPrint("%s() LN%d snprintf overflow\n", __func__, __LINE__);
        return -1;
      } else {
    ...
    for (int i = 0; i < fieldsSize; ++i) {
      ...
      switch (field->type) {
        ...
        case TSDB_DATA_TYPE_DOUBLE: {
          double doubleTmp = tmpDoubleImpl(field, angle, k);
          n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=%ff64,", field->name, doubleTmp);
          break;
        }
        case TSDB_DATA_TYPE_BINARY:
        case TSDB_DATA_TYPE_VARBINARY:
        case TSDB_DATA_TYPE_NCHAR: {
          ...
          if (field->type == TSDB_DATA_TYPE_BINARY) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=\"%s\",", field->name, tmp);
          } else if (field->type == TSDB_DATA_TYPE_NCHAR) {
            n = snprintf(sampleDataBuf + pos, bufLen - pos, "%s=L\"%s\",", field->name, tmp);
          }
          tmfree(tmp);
        ...
        case TSDB_DATA_TYPE_BLOB: {
          char *tmp = NULL;
          benchCalloc(1, field->length + 1, false);
          if (0 != tmpBlobStr(&tmp, stbInfo->iface, field, k)) {
            free(tmp);
            return -1;
          }
...
static int generateRandDataSml(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow,
                               BArray *fields, int64_t loop, bool tag) {
  int protocol = stbInfo->lineProtocol;

  switch (protocol) {
    case TSDB_SML_LINE_PROTOCOL:
      return generateRandDataSmlLine(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
    case TSDB_SML_TELNET_PROTOCOL:
      return generateRandDataSmlTelnet(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
    default:
      return generateRandDataSmlJson(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
  }

  return -1;
}

int generateRandData(SSuperTable *stbInfo, char *sampleDataBuf, int64_t bufLen, int lenOfOneRow, BArray *fields,
                     int64_t loop, bool tag, BArray *childCols) {
  int iface = stbInfo->iface;
  switch (iface) {
    case TAOSC_IFACE:
      return generateRandDataSQL(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
    // REST
    case REST_IFACE:
      return generateRandDataSQL(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
    case STMT_IFACE:
    case STMT2_IFACE:
      if (childCols) {
        return generateRandDataStmtForChildTable(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, childCols);
      } else {
        return generateRandDataStmt(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
      }
    case SML_IFACE:
    case SML_REST_IFACE:  // REST
      return generateRandDataSml(stbInfo, sampleDataBuf, bufLen, lenOfOneRow, fields, loop, tag);
    default:
...
static BArray *initChildCols(int colsSize) {
  BArray *childCols = benchArrayInit(colsSize, sizeof(ChildField));
  for (int col = 0; col < colsSize; col++) {
    ChildField *childCol = benchCalloc(1, sizeof(ChildField), true);
...
int prepareSampleData(SDataBase *database, SSuperTable *stbInfo) {
  ...
  if (stbInfo->partialColNum != 0 && ((stbInfo->iface == TAOSC_IFACE || stbInfo->iface == REST_IFACE))) {
    // check valid
    ...
    if (stbInfo->partialColNum < stbInfo->cols->size) {
      stbInfo->partialColNameBuf = benchCalloc(1, TSDB_MAX_ALLOWED_SQL_LEN, true);
      int pos = 0;
      int n;
      n = snprintf(stbInfo->partialColNameBuf + pos, TSDB_MAX_ALLOWED_SQL_LEN - pos, TS_COL_NAME);
      ...
      for (int i = stbInfo->partialColFrom; i < stbInfo->partialColFrom + stbInfo->partialColNum; ++i) {
        ...
        n = snprintf(stbInfo->partialColNameBuf + pos, TSDB_MAX_ALLOWED_SQL_LEN - pos, ",%s", col->name);
        if (n < 0 || n > TSDB_MAX_ALLOWED_SQL_LEN - pos) {
  ...
  stbInfo->sampleDataBuf = benchCalloc(1, stbInfo->lenOfCols * g_arguments->prepared_rand, true);
  infoPrint(
  ...
  if (stbInfo->random_data_source) {
    if (g_arguments->mistMode) {
      infoPrint(
          "Each child table using different random prepare data pattern. need "
      ...
      for (int64_t child = 0; child < stbInfo->childTblCount; child++) {
        ...
        childTbl->sampleDataBuf = benchCalloc(1, stbInfo->lenOfCols * g_arguments->prepared_rand, true);
        if (generateRandData(stbInfo, childTbl->sampleDataBuf, stbInfo->lenOfCols * g_arguments->prepared_rand,
                             stbInfo->lenOfCols, stbInfo->cols, g_arguments->prepared_rand, false,
                             childTbl->childCols)) {
      ...
      if (generateRandData(stbInfo, stbInfo->sampleDataBuf, stbInfo->lenOfCols * g_arguments->prepared_rand,
                           stbInfo->lenOfCols, stbInfo->cols, g_arguments->prepared_rand, false, NULL)) {
        return -1;
      }
    }
    ...
    if (stbInfo->useSampleTs) {
      if (getAndSetRowsFromCsvFile(stbInfo->sampleFile, &stbInfo->insertRows)) {
        return -1;
      }
    }
    if (generateSampleFromCsv(stbInfo->sampleDataBuf, stbInfo->sampleFile, NULL, stbInfo->lenOfCols,
    ...
    if (stbInfo->childTblSample) {
      ...
      for (int64_t child = 0; child < stbInfo->childTblCount; child++) {
        ...
        if (getAndSetRowsFromCsvFile(sampleFilePath, &(childTbl->insertRows))) {
          errorPrint("Failed to get sample data rows for table %s\n", childTbl->name);
          return -1;
        }

        childTbl->sampleDataBuf = benchCalloc(1, stbInfo->lenOfCols * g_arguments->prepared_rand, true);
        if (generateSampleFromCsv(childTbl->sampleDataBuf, sampleFilePath, NULL, stbInfo->lenOfCols,
                                  g_arguments->prepared_rand)) {
          errorPrint(
              "Failed to generate sample from file "
  ...
  if (0 != convertServAddr(stbInfo->iface, stbInfo->tcpTransfer, stbInfo->lineProtocol)) {
    return -1;
  }
  return 0;
}
...
uint32_t bindParamBatch(threadInfo *pThreadInfo, uint32_t batch, int64_t startTime, int64_t pos, SChildTable *childTbl,
                        int32_t *pkCur, int32_t *pkCnt, int32_t *n, int64_t *delay2, int64_t *delay3) {
  ...
  // if (!pThreadInfo->stmtBind || stbInfo->interlaceRows > 0 ) {
  {
    ...
    for (int c = 0; c <= columnCount; c++) {
      ...
      if (c == 0) {
        ...
        debugPrint("col[%d]: type: %s, len: %d\n", c, convertDatatypeToString(data_type), col->length);
      }
    ...
    for (uint32_t k = 0; k < batch; k++) {
      ...
      if (stbInfo->disorderRatio) {
        *(pThreadInfo->bind_ts_array + k) =
            startTime + getTSRandTail(stbInfo->timestamp_step, *n, stbInfo->disorderRatio, stbInfo->disorderRange);
      } else {
  ...
  */
  ...
  if (taos_stmt_bind_param_batch(stmt, (TAOS_MULTI_BIND *)pThreadInfo->bindParams)) {
...
void generateSmlJsonTags(tools_cJSON *tagsList, char **sml_tags_json_array, SSuperTable *stbInfo,
  ...
  snprintf(tbName, TSDB_TABLE_NAME_LEN, "%s%" PRIu64, stbInfo->childTblPrefix, start_table_from + tbSeq);
  ...
  for (int i = 0; i < stbInfo->tags->size; i++) {
    ...
    switch (tag->type) {
      ...
      default: {
        ...
        tools_cJSON_AddNumberToObject(tags, tagName, tagTmp);
        break;
      }
    }
  }
...
void generateSmlTaosJsonTags(tools_cJSON *tagsList, SSuperTable *stbInfo, uint64_t start_table_from, int tbSeq) {
  tools_cJSON *tags = tools_cJSON_CreateObject();
  char        *tbName = benchCalloc(1, TSDB_TABLE_NAME_LEN, true);
  snprintf(tbName, TSDB_TABLE_NAME_LEN, "%s%" PRIu64, stbInfo->childTblPrefix, tbSeq + start_table_from);
...
void generateSmlJsonValues(char **sml_json_value_array, SSuperTable *stbInfo, int tableSeq) {
  char  *value_buf = NULL;
  ...
  switch (col->type) {
    case TSDB_DATA_TYPE_BOOL: {
      ...
      snprintf(value_buf, len_key + 6, "\"value\":%s,", boolTmp ? "true" : "false");
      break;
    }
    case TSDB_DATA_TYPE_FLOAT: {
      value_buf = benchCalloc(len_key + 20, 1, true);
      float floatTmp = tmpFloat(col);
      snprintf(value_buf, len_key + 20, "\"value\":%f,", floatTmp);
      break;
    }
    case TSDB_DATA_TYPE_DOUBLE: {
      value_buf = benchCalloc(len_key + 40, 1, true);
      double doubleTmp = tmpDouble(col);
      snprintf(value_buf, len_key + 40, "\"value\":%f,", doubleTmp);
      break;
    }
    case TSDB_DATA_TYPE_BINARY:
    case TSDB_DATA_TYPE_VARBINARY:
    case TSDB_DATA_TYPE_NCHAR: {
      ...
      snprintf(value_buf, len_key + col->length + 3, "\"value\":\"%s\",", buf);
      tmfree(buf);
      break;
    }
    case TSDB_DATA_TYPE_GEOMETRY: {
...
void generateSmlJsonCols(tools_cJSON *array, tools_cJSON *tag, SSuperTable *stbInfo, uint32_t time_precision,
                         int64_t timestamp) {
...
void generateSmlTaosJsonCols(tools_cJSON *array, tools_cJSON *tag, SSuperTable *stbInfo, uint32_t time_precision,
                             int64_t timestamp) {
  tools_cJSON *record = tools_cJSON_CreateObject();
...
bool generateTagData(SSuperTable *stbInfo, char *buf, int64_t cnt, FILE *csv, BArray *tagsStmt) {
  if (csv) {
    if (generateSampleFromCsv(buf, NULL, csv, stbInfo->lenOfTags, cnt)) {
      return false;
    }
  } else {
    if (generateRandData(stbInfo, buf, cnt * stbInfo->lenOfTags, stbInfo->lenOfTags,
                         tagsStmt ? tagsStmt : stbInfo->tags, cnt, true, NULL)) {
...
//
uint32_t bindVColsProgressive(TAOS_STMT2_BINDV *bindv, int32_t tbIndex, threadInfo *pThreadInfo, uint32_t batch,
                              int64_t startTime, int64_t pos, SChildTable *childTbl, int32_t *pkCur, int32_t *pkCnt,
                              int32_t *n) {
  SSuperTable *stbInfo = pThreadInfo->stbInfo;
  uint32_t     columnCount = stbInfo->cols->size;
...
// STMT2 bind cols param progressive
//
uint32_t bindVColsInterlace(TAOS_STMT2_BINDV *bindv, int32_t tbIndex, threadInfo *pThreadInfo, uint32_t batch,
                            int64_t startTime, int64_t pos, SChildTable *childTbl, int32_t *pkCur, int32_t *pkCnt,
                            int32_t *n) {
  // count
  bindv->count += 1;
