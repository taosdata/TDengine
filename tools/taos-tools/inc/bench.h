...
#define BENCH_MODE     "insert mode, default is taosc, options: taosc|rest|stmt|stmt2|sml"
...
#define BENCH_RANDOM                                                                                               \
  "Each child table generates different random data, this option need much memory. ( all memory = childs count * " \
  "prepared_rand)"
...
enum enum_TAOS_INTERFACE { TAOSC_IFACE, REST_IFACE, STMT_IFACE, STMT2_IFACE, SML_IFACE, SML_REST_IFACE, INTERFACE_BUT };

typedef enum enumQUERY_CLASS { SPECIFIED_CLASS, STABLE_CLASS, CLASS_BUT } QUERY_CLASS;
...
int postProcessSql(char *sqlstr, char *dbName, int precision, int iface, int protocol, uint16_t rest_port, bool tcp,
                   int sockfd, char *filePath);
int queryDbExecCall(SBenchConn *conn, char *command);
int queryDbExecRest(char *command, char *dbName, int precision, int iface, int protocol, bool tcp, int sockfd);
...
int         getAllChildNameOfSuperTable(TAOS *taos, char *dbName, char *stbName, char **childTblNameOfSuperTbl,
