...
int64_t    tools_user_mktime64(const unsigned int year0, const unsigned int mon0, const unsigned int day,
                               const unsigned int hour, const unsigned int min, const unsigned int sec, int64_t time_zone);
...
#ifdef RELEASE
#define TOOLS_ASSERT(x)                                                       \
  do {                                                                        \
    if (!(x)) errorPrint("%s() LN%d, %s\n", __func__, __LINE__, "assertion"); \
  } while (0)
#else
#include <assert.h>
#define TOOLS_ASSERT(x) \
