#!/bin/bash
# Wrapper that preserves the Linux start-model.sh entry point while delegating
...
parse_options() {
  ...
  if [ $# -gt 0 ]; then
    echo "ERROR: Extra arguments are no longer supported by start-model.sh." >&2
    echo "Use '$0 [model_name|all]' or call taosanode_service.py directly if new options are added." >&2
    exit 2
  fi
}
...
main() {
  ...
  cmd=("${python_cmd}" "${SERVICE_SCRIPT}" "model-start")
