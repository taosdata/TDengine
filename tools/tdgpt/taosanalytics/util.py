...
def is_stationary(input_list):
    ...
    AppLogger.get_instance().log_inst.info("adf is:%f critical value is:%s", adf, critical_values)
    return pvalue < 0.05
...
def do_initial_check(request):
    if not request.is_json:
        AppLogger.get_instance().log_inst.error('recv invalid request, only json allowed. %s', request.data)
        raise ValueError("invalid request format")

    try:
        req_json = request.json
    except Exception as e:
        AppLogger.get_instance().log_inst.error('recv invalid request, invalid json format:%s', request.data)
        raise ValueError(e)

    AppLogger.get_instance().log_inst.debug('req payload: %s', req_json)
    return req_json
...
def do_check_before_exec(request, check_rows=True):
    ...
    try:
        ...
        AppLogger.get_instance().log_inst.error('validate req json failed, %s', e)
        raise ValueError(e)
    ...
    if wn_check:
        ...
        try:
            ...
            AppLogger.get_instance().log_inst.error("failed to check white noise data, %s", str(e))
            raise

        if is_wn:
            AppLogger.get_instance().log_inst.debug("%s is %s", data, white_noise_error_msg())
