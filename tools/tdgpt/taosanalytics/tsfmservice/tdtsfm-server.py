...
class TaosTSGenerationMixin(GenerationMixin):
        ...
        **kwargs,
        ...
        if len(inputs.shape) == 2:
            ...
            if cur_len < self.config.Taos_input_token_len:
                raise ValueError(
                    f"Input length must be at least {self.config.Taos_input_token_len}"
                )
            elif cur_len % self.config.Taos_input_token_len != 0:
                new_len = (
                    cur_len // self.config.Taos_input_token_len
                ) * self.config.Taos_input_token_len
    ...
    def _update_model_kwargs_for_generation(
        ...
        model_kwargs["Taos_past_key_values"] = self._extract_past_from_model_output(
...
class TaosPatchEmbedding(nn.Module):
    def __init__(self, config: TaosConfig):
        ...
        self.Taos_emb = nn.Linear(
            config.Taos_input_token_len, config.Taos_hidden_size, bias=False
        )

    def forward(self, hidden_state: torch.Tensor):
        hidden_state = hidden_state.unfold(
            dimension=-1, size=self.Taos_input_token_len, step=self.Taos_input_token_len
        )
...
class TaosPointEmbedding(nn.Module):
    def __init__(self, config: TaosConfig):
        super().__init__()
        self.Taos_emb_layer = nn.Linear(
            config.Taos_input_token_len, config.Taos_hidden_size, bias=False
        )
        self.Taos_gate_layer = nn.Linear(
            config.Taos_input_token_len, config.Taos_hidden_size, bias=False
        )
...
class TaosRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=10000, base=10000, device=None):
        ...
        inv_freq = 1.0 / (
            self.Taos_base
            ** (
                torch.arange(0, self.Taos_dim, 2, dtype=torch.int64).float().to(device)
                / self.Taos_dim
            )
        )
        ...
        self._set_cos_sin_cache(
            ...
            device=self.Taos_inv_freq.device,
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = torch.arange(
            self.Taos_max_seq_len_cached, device=device, dtype=torch.int64
        ).type_as(self.Taos_inv_freq)
    ...
    def forward(self, x, seq_len=None):
        ...
        return (
            self.Taos_cos_cached[:seq_len].to(dtype=x.dtype),
            self.Taos_sin_cached[:seq_len].to(dtype=x.dtype),
        )


class TaosAttention(nn.Module):
    def __init__(self, config: TaosConfig, layer_idx: int | None = None):
        super().__init__()
        ...
        self.Taos_q_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_hidden_size, bias=True
        )
        self.Taos_k_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_hidden_size, bias=True
        )
        self.Taos_v_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_hidden_size, bias=True
        )
        self.Taos_o_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_hidden_size, bias=False
        )
        self.Taos_rotary_emb = TaosRotaryEmbedding(
            self.Taos_head_dim,
            max_position_embeddings=config.Taos_max_position_embeddings,
        )
    ...
    def forward(
        ...
        Taos_past_key_value: Cache | None = None,
        ...
        **kwargs,
        ...
        query_states = query_states.view(
            bsz, q_len, self.Taos_num_heads, self.Taos_head_dim
        ).transpose(1, 2)
        ...
        if Taos_past_key_value is not None:
            kv_seq_len += Taos_past_key_value.get_usable_length(
                kv_seq_len, self.Taos_layer_idx
            )
        ...
        if Taos_past_key_value is not None:
            key_states, value_states = Taos_past_key_value.update(
                key_states, value_states, self.Taos_layer_idx
            )
        attn_output = F.scaled_dot_product_attention(
            ...
            value_states,
            attention_mask,
            dropout_p=self.Taos_attention_dropout,
        )
...
class TaosMLP(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int, hidden_act: str):
        ...
        self.Taos_gate_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_intermediate_size, bias=False
        )
        self.Taos_up_proj = nn.Linear(
            self.Taos_hidden_size, self.Taos_intermediate_size, bias=False
        )
        self.Taos_down_proj = nn.Linear(
            self.Taos_intermediate_size, self.Taos_hidden_size, bias=False
        )
        self.Taos_act_fn = ACT2FN[hidden_act]

    def forward(self, hidden_state):
        return self.Taos_down_proj(
            self.Taos_act_fn(self.Taos_gate_proj(hidden_state))
            * self.Taos_up_proj(hidden_state)
        )
...
class TaosDecoderLayer(nn.Module):
    def __init__(self, config: TaosConfig, layer_idx: int):
        ...
        self.Taos_ffn_layer = TaosMLP(
            hidden_size=config.Taos_hidden_size,
            intermediate_size=config.Taos_intermediate_size,
            hidden_act=config.Taos_hidden_act,
        )
    ...
    def forward(
        ...
        attention_mask: torch.Tensor | None = None,
        position_ids: torch.LongTensor | None = None,
        Taos_past_key_value: tuple[torch.Tensor] | None = None,
...
class TaosModel(TaosPreTrainedModel):
    def __init__(self, config: TaosConfig):
        ...
        self.Taos_layers = nn.ModuleList(
            [
                TaosDecoderLayer(config, layer_idx)
                for layer_idx in range(config.Taos_num_hidden_layers)
            ]
        )
    ...
    def forward(
        ...
        attention_mask: torch.Tensor | None = None,
        position_ids: torch.LongTensor | None = None,
        Taos_past_key_values: list[torch.FloatTensor] | None = None,
        ...
        if use_cache:
            ...
            if use_legacy_cache:
                Taos_past_key_values = DynamicCache.from_legacy_cache(
...
class TaosForPrediction(TaosPreTrainedModel, TaosTSGenerationMixin):
    def __init__(self, config: TaosConfig):
        ...
        for i, output_token_len in enumerate(self.config.Taos_output_token_lens):
            lm_head_list.append(
                nn.Linear(self.config.Taos_hidden_size, output_token_len, bias=False)
            )
    ...
    def forward(
        ...
        if labels is not None:
            ...
            for lm_head, output_token_len in zip(
                self.Taos_lm_heads, self.config.Taos_output_token_lens
            ):
            ...
            if max_output_length is None:
                ...
                for h in self.config.Taos_output_token_lens[1:]:
                    if h > max_output_length:
                        ...
                        output_token_len = h
            lm_head = self.Taos_lm_heads[
                self.Taos_output_token_len_map[output_token_len]
            ]
    ...
    def calc_ar_loss(self, predictions, labels, loss_masks, output_token_len):
        ...
        labels = labels[
            :, : seq_len - self.config.Taos_input_token_len + output_token_len
        ]
        shift_labels = labels.unfold(
            dimension=-1, size=output_token_len, step=self.config.Taos_input_token_len
        )
        ...
        **kwargs,
        ...
        if Taos_past_key_values is not None:
            ...
            if attention_mask is not None and attention_mask.shape[1] > (
                input_ids.shape[1] // self.config.Taos_input_token_len
            ):
                input_ids = input_ids[:, -(attention_mask.shape[1] - past_length) :]
            elif past_length < (input_ids.shape[1] // self.config.Taos_input_token_len):
                input_ids = input_ids[
                    :, past_length * self.config.Taos_input_token_len :
                ]
            if (
    ...
    try:
        ...
        if len(set(input_data)) == 1:
            ...
            if Taos_model is None:
                return jsonify({"error": "not predict, load model failed"}), 500
        ...
        response = {"status": "success", "output": pred_y[-num_len:]}
...
def main():
    ...
    app.run(host="0.0.0.0", port=_port, threaded=True, debug=False)
