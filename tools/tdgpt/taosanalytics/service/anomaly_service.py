...
def handle_anomaly(request):
    ...
    try:
        ...
        AppLogger.get_instance().log_inst.error("failed to do anomaly-detection, %s", str(e))
    ...
    try:
        ...
        AppLogger.get_instance().log_inst.debug("anomaly-detection result: %s", str(result))
        return result
    except Exception as e:
