...
static bool tableHasDecimalCol(TAOS *conn, const char *dbName, const char *tbName) {
    ...
    if (code != 0) {
        logError("DESCRIBE failed(0x%08X %s): %s", code, taos_errstr(res), sql);
...
static int cmpFilePath(const void *a, const void *b) {
...
static char** findDataFiles(const char *dbName, const char *stbName, int *totalCount) {
    *totalCount = 0;
    ...
    // Sort file list by name for better sequential I/O locality across threads.
    if (*totalCount > 1) {
        taosSort(files, *totalCount, sizeof(char *), cmpFilePath);
    }
