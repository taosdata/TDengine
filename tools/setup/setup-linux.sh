#!/bin/bash
# ============================================================================
# setup-linux.sh — TSDB build environment setup for Linux
#
# Usage:
...
#   ./tools/setup/setup-linux.sh --yes --component adapter
#   ./tools/setup/setup-linux.sh --help
# ============================================================================
...
if [[ "$SETUP_OS" != "linux" ]]; then
    echo "ERROR: This script is for Linux. Use setup-macos.sh on macOS."
    exit 1
fi
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║        TSDB Build Environment Setup (Linux)                    ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo ""
echo "  OS:       $SETUP_OS ($SETUP_DISTRO) $SETUP_ARCH"
...
for lang in "${LANGS[@]}"; do
    ...
    # install + config (unless check-only)
    if [[ "$CHECK_ONLY" != "true" ]]; then
        "mod_${lang}_install"
        "mod_${lang}_config"
    fi
