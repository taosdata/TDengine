...
void init_dump_info(tsDumpInfo *dump_info, TAOS_RES *tres, const char *sql, bool vertical) {
  ...
  if (vertical) {
    ...
    // set an appropriate with for token and totp_secret display
    ...
    } else if (shellRegexMatch(sql, "^[\t ]*create[ \t]+user[ \t]+.*totp_secret[ \t]+.*", REG_EXTENDED | REG_ICASE)) {
      dump_info->width[0] = TMAX(dump_info->width[0], SHELL_SHOW_TOTP_SECRET_DISPLAY_WIDTH);
    } else if (shellRegexMatch(sql, "^[\t ]*alter[ \t]+user[ \t]+.*update[ \t]+totp_secret[; \t]+.*", REG_EXTENDED | REG_ICASE)) {
      dump_info->width[0] = TMAX(dump_info->width[0], SHELL_SHOW_TOTP_SECRET_DISPLAY_WIDTH);
    }
  }
}
