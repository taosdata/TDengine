...
use crate::{
    Action, QueryOpts, TargetOpts, TimeRange, split_table_into_time_range_chunks,
    sync_single_table_partial, sync_super_table_schema, transform_sql_with_remap,
    transform_tbname_with_actions, utils::breakpoints::BreakpointDb,
};

use taosx_core::core_metrics::{CoreMetrics, TaskMetrics};
                        ...
                        match breakpoint {
                            Ok(Some(breakpoint)) => {
                                    ...
                                    || time_range.start.unwrap() < breakpoint
                                {
                                    ...
                                    tracing::debug!(
                                        "load breakpoint success set time_range: {time_range} table: {table}"
                                    );
                                }
                                break;
                            }
                                ...
                                .await
                                {
                                    ...
                                    Err(err) => {
                                            ...
                                            && retries > 0
                                        {
                                            ...
                                            tracing::info!(
                                                "[worker:{worker}] sync table {table} error 0x263F: {err:?}, add column"
                                            );
