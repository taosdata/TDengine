...
/// migrate data
pub async fn migrate_history(config: MssqlConfig, cancel: CancellationToken) -> anyhow::Result<()> {
    ...
    // create an instance of MssqlQuery
    let query = MssqlQuery::try_new(config.connect.clone(), config.task.time_zone.clone()).await?;
    ...
    tokio::select! {
        res = future_migrate => {
            if let Err(e) = res {
                tracing::error!("migrate mssql from history data error: {e}");
            }
        }
    ...
    let future_sync = async move {
        ...
        while origin_end.is_none() || origin_end.unwrap() > now {
            ...
            if real_end - real_start > chrono::Duration::seconds(10) {
                tracing::trace!(
                    "migrate mssql from live data, start: {}, end: {}",
                    real_start,
                    real_end
                );
                // create a new window
                ...
                if let Err(e) = migrate_result {
                    tracing::error!("migrate mssql from live data error: {e}");
                }
                // move the window
                now = real_end;
            }
    ...
    tracing::info!("migrate mssql finished");
    Ok(())
}
...
pub async fn migrate_history_by_subtable(
        ...
        migrate_join_set.spawn(async move {
            ...
            for i in 1..4 {
                ...
                match migrate_result {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "migrate mssql, migrate history by interval failed, cause: {}, retrying {i} times...",
                            e
                        );
    ...
    tokio::select! {
        res = futures => {
            if let Err(err) = &res {
                tracing::error!("migrate mssql, do migrate runtime error: {err:#}");
            }
            return res;
        },
        _ = cancel.cancelled() => {}
...
pub async fn migrate_history_by_interval(
    mut config: MssqlConfig,
    cancel: CancellationToken,
    mut query: MssqlQuery,
) -> anyhow::Result<()> {
    // schema
    let sql = config.task.generate_sql()?;
    let col_map = query.select_for_schema(&sql).await?;
    let schema = to_schema(col_map)?;
    tracing::debug!("migrate mssql, schema: {:?}", schema);
                    ...
                    .map(|v| {
                        v.into_iter()
                            ...
                            .filter_map(|(col_cidx, col)| {
                                ...
                                match col {
                                    tiberius::ColumnData::U8(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::I16(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::I32(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::I64(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::F32(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::F64(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}={}", col_name, val))
                                    }),
                                    tiberius::ColumnData::String(val) => val.map(|val| {
                                        (col_name.clone(), format!("{}='{}'", col_name, val))
                                    }),
                                    _ => None,
                                }
...
mod tests {
    ...
    async fn test_migrate_history() {
        let dsn = Dsn::from_str("mssql://test:123456@192.168.1.66:1433/test_taosx?encryption=On&trust_cert=true&sql=select * from t_metric&start=2024-03-01T00:00:00Z&end=2024-04-01T00:00:00Z&interval=5d&delay=0")
            .unwrap();
    ...
    async fn test_set_breakpoint() {
        let dsn = Dsn::from_str("mssql://test:123456@192.168.1.66:1433/test_taosx?encryption=On&trust_cert=true&sql=select * from t_metric&start=2021-01-01T00:00:00Z&end=2021-02-01T00:00:00Z&interval=12h&delay=0")
            .unwrap();
        let mut config = MssqlConfig::from_dsn(&dsn).unwrap();
