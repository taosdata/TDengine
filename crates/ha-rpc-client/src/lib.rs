...
pub async fn create_client(
    ...
    let token = xnoded_jwt_encode(xnoded_id).context(JwtSnafu)?;

    let mut flight_client = FlightClient::new(channel);
    flight_client
        .add_header("x-client-type", "xnoded")
    ...
    tokio::spawn(async move {
        ...
        let mut flight_reqs = HashMap::<u64, oneshot::Sender<FlightResult>>::with_capacity(1000);
