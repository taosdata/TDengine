...
pub async fn migrate_history(
    ...
    // create an instance of MongoDBQuery
    let query = MongoDBQuery::try_new(config.connect.clone()).await?;
    ...
    tokio::select! {
        res = future_migrate => {
            if let Err(e) = res {
                tracing::error!("migrate mongodb from history data error: {e}");
            }
        }
        ...
        while origin_end.is_none() || origin_end.unwrap() > now {
            ...
            if real_end - real_start > chrono::Duration::seconds(10) {
                ...
                if let Err(e) = migrate_result {
                    tracing::error!("migrate mongodb from live data error: {e}");
                }
                // move the window
                now = real_end;
            }
...
pub async fn migrate_history_by_subtable(
        ...
        migrate_join_set.spawn(async move {
            ...
            for i in 1..4 {
                ...
                match migrate_result {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "migrate mongodb, migrate history by interval failed, cause: {}, retrying {i} times...",
                            e
                        );
    ...
    tokio::select! {
        res = futures => {
            if let Err(err) = &res {
                tracing::error!("migrate mongodb, do migrate runtime error: {err:#}");
            }
            return res;
        },
        _ = cancel.cancelled() => {}
...
pub async fn get_all_distinct_values(
    ...
    while window_start_with_tz < end_with_tz {
        ...
        if current_database != database || current_collection != collection {
            // get distinct values
            if !config.task.subtable_fields.is_empty() {
