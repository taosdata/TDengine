...
if [ -z $log_folder ]; then
    ...
    perf_summary_file=$log_folder/summary.txt
fi
...
function parse_perf() {
    local file="$1"
    local line=`grep -w "@@##@@##" $file`
    parse_perf_insert_rows "$line"
}
...
function run_thread() {
    ...
    while [ 1 ]; do
        ...
        local perf_full_path=$log_dir/${case_file}.perf.txt
        ...
        if [ $ret -ne 0 ]; then
            ...
            parse_perf "$log_full_path" >$perf_full_path
            if [ ! -z "$perf_summary_file" ]; then
                echo "${case_cmd}" >>$perf_summary_file
                cat $perf_full_path >>$perf_summary_file
            fi
            cat $perf_full_path
        fi
    done
