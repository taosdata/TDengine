case_file="cases_stream_replica1.txt"
...
def build_package(internal_root, new_version) {
    sh '''
        ...
        rm -rf /var/data/jenkins/workspace/TDinternal/enterprise/contrib/deps-download/CMakeCache.txt
...
pipeline {
    ...
    environment{
        ...
        CASE_BRANCH="${env.CASE_BRANCH}"
        BRANCH_3_0="${env.BRANCH_3_0}"
        NEW_VERSION='3.0.0.100'
        FILE_WEB_SERVER='http://192.168.1.53:8081/'
    }
    stages {
        stage ('build source') {
            when {
                ...
                stage('building') {
                    steps {
                        script {
                            ...
                            '''
                            if (check_cases_to_run() == 0) {
                                sync_source("${STREAM_BRANCH}", "${INTERNAL_ROOT}")
                ...
                script {
                    ...
                    if (check_cases_to_run() == 0) {
                        sh '''
                            ...
                            git reset --hard
                            git fetch
                            git checkout ${CASE_BRANCH} -f
                            git branch
                            git pull origin ${CASE_BRANCH}
                            git log -2
        ...
        stage('stream replica 1 test') {
            ...
            steps {
                script {
                    run_cases("cases_stream_replica1.txt", "/home/m.json", "1", "", "-s -o 3600")
                }
            }
        }
        stage('stream replica 3 test') {
            ...
            steps {
                script {
                    run_cases("cases_stream_replica3.txt", "/home/m.json", "3", "", "-s -o 10800")
                }
            }
        }
    }
