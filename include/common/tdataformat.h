...
// Failure contract: on any non-zero return, *ppTag is guaranteed to be
// NULL — if tTagNew() succeeded but the subsequent align failed, the
// pre-built STag is freed internally so callers can simply `goto end`
// without leaking it.
