...
typedef struct SDataSinkManager2 {
  ...
  int64_t   readDataFromMesTimes;
  int64_t   readDataFromFileTimes;
  SHashObj* DataSinkStreamList;  // hash <streamId + taskId, SSreamDataSinkManager>
} SDataSinkManager2;
 ...
 typedef struct SStreamDataSinkManager {
   int64_t            streamId;
   int64_t            taskId;
   int64_t            usedMemSize;
   int8_t             cleanMode;
   SHashObj*          DataSinkGroupList;  // hash <groupId, SSreamGroupDataSink>
   int64_t            resultMaxSize;
   DataSinkFileState* pFile;
 } SStreamDataSinkManager;

 typedef struct SSreamGroupDataSink {
   int64_t                groupId;
   SHashObj*              DataSinkTableList;  // hash <tableId, STableDataInfo>
   SStreamDataSinkManager* pSinkManager;
 } SStreamGroupDataSink;
 ...
 typedef struct STableDataInfo {
   ...
   bool                 hasDataInFile;
   SHashObj*            windowDataInMem;   // hash <wstart, SSDataBlock*>
   SHashObj*            windowDataInFile;  // hash <wstart, start block num in file>
   SStreamGroupDataSink* pSinkGroup;
 } STableDataInfo;

 typedef struct SResultIter {
  SStreamGroupDataSink* groupData;
   int64_t              tableId;
   STableDataInfo*      tableInfo;
   int8_t               dataPos;  // 0 - data in mem, 1 - data in file
   int64_t              blockNum;
 } SResultIter;
