...
1. 目的
  本文档的目的是对 taosX 高可用架构进行完整的设计阐述，对高可用功能涉及的架构变更、设计实现、安装部署、运行维护等方面的具体问题做[说明。
1. 范围
2. 受众
  taosX 开发者。
...
| 2025/12/09 | 1.0 | 霍琳贺 | 更新 SQL 命令，发布第一版本 |
...
![](./images/wb_T4mXwqQJqhWBdQbo5UTc0pGPnUh.png)
...
2. 所有元数据操作均通过 MNode 执行，包括：
   ...
   - 创建或删除 Agent：`CREATE XNODE AGENT 'name' WITH options`；
   - （内部使用）为指定数据接入任务创建负载均衡工作分片：`CREATE XNODE JOB ON <xnodeId>.<taskId> WITH '{json}'`
   - 为负载均衡工作分片手动切换运行节点：`ALTER XNODE JOB <jid> SET XNODE 1`
   - 手动结束负载均衡工作分片任务：`DROP XNODE JOB <jid>`
   - 重新平衡相关工作负载：`REBALANCE XNODE JOBS WHERE job_conditions`
...
![](./images/wb_Sjyewsz6ihcvRob00IHcnF23nBh.png)
...
<callout emoji="spiral_note_pad" background-color="light-orange" border-color="light-orange">
该用户将用于：
1. 与 MNode 保持连接；
2. 向 TDengine 写入数据；
3. 将 XNode 及任务运行指标写入 log 库；
解决的问题：
1. 旧版本用户密码变更后 taosX 写入会停止，新版本生产环境建议使用随机密码，这个密码创建后不会更改，且仅在 XNode 中使用；
2. 其他用户创建 XNode 数据接入任务后，自动赋予 XNode 用户数据库写入权限（相当于 root）。
未来的变化：
1. 如果之后的版本增加了 Auth Token 方式，可以迁移到 root 用户的授权 TOKEN 方式，就不需要单独的用户了；
</callout>
...
| url | varchar | 必填 | 通用的 `host:port` 形式的节点表示。 | 通信地址，xnode 开启的rpc服务 |
| status | int | 0 | 2: drain 1:online 0:offline | 节点状态 |
| createTime | datetime | 入库时间 |  | XNode 最近一次启动的时间。具体存储根据方便可以改为时间戳储存。 |
| updateTime | datetime | 入库时间 |  | XNode 最近一次启动的时间 |
...
删除流程如下：
![](./images/wb_B0QEwBmC0hw7qib1IomcjRk7nVq.png)

1. 管理员发起 DROP XNode 请求；
2. MNode 接收删除 XNode 请求后，需检查节点可用性：
   - 如果当前节点已经是最后一个可用的 XNode，需检查是否有任务正在运行：
      - 如果有正在运行的任务，则直接报错：请停止正在运行的任务后再删除 XNode！
      - 如果没有正在运行的任务，可在系统表中删除该节点，taosX 删除所有任务且不再接受任务变更。
   - 如果当前节点不是最后一个可用节点：
      - 先向 XNode 发起清理操作。XNode 接收删除请求后，按顺序执行以下操作：
         - 设置当前节点为 DRAIN 模式，表示该节点不接收新增任务执行请求；
         - 如果有正在运行的任务，获取任务列表，依次执行任务清理操作：
            - 循环停止所有任务，由 MNode 节点上的负载均衡器重新分发该分片任务到其他可用节点；
            - 任务清理完毕后，下线节点，表示节点已移除；
         - 如果没有任务，则直接下线该节点。
      - 确保 XNode 没有任务正在运行，可以在系统表中删除该节点。
...
| --- | --- | --- | --- | --- |
| id | int |  |  | 任务id， 从1开始计数 |
| name | varchar | NULL | 非空 | 任务名称，用于标识任务 |
| via | int | NULL | agent 的 id, 如果没有 agent 取值 -1 |  |
| from | varchar | NULL |  | dsn 格式 |
| to | varchar | NULL |  | dsn 格式 |
| parser | varchar | NULL |  |  |
| xnode_id | int | NULL | NULL 表示刚初始创建 或者 分片任务 非 NULL 表示非分片任务 | 需要查看 show jobs 查看具体分片信息 |
| status | str | NULL | Xnoded 自行维护 | Xnoded 自行维护 |
| reason | varchar | NULL |  | failed 需要填写此内容：最后出错消息，重试失败消息会覆盖此内容 |
| create_time | datetime |  |  |  |
| update_time | datetime |  |  | 最近一次更新时间(status完成状态时，updateTime就是完成时间) |
...
![](./images/wb_T9kmwuWeJhsEIdb5pQpcFljznLf.png)

1. 用户侧通过 API 发起新建任务请求：
   - `CreateNewTask`请求​：用户发起任务创建指令，请求体与单机 taosX 保持不变。
   - `Validation`自检​：`XNode`对请求格式、权限、License 等做基础校验：
      - ✅ 校验通过​：进入任务创建子流程。
      - ❌ 校验失败​：直接返回错误（`Invalid NewTask request`）
2. 任务创建流程：
   - 校验通过后，请求转发至`MNode`。
      - 任务转发失败或 MNode 状态异常，直接返回具体错误原因（`MNode is not ready`） 。
   - `MNode`获取元数据写锁，​避免并发冲突​。
   - 进行任务查重（`name` 字段是 UNIQUE 约束的）：
      - 任务不存在，初始化任务元数据，并进行持久化；
      - 任务已存在，返回`Task has been exist`，终止创建流程；
      - 持久化失败，返回具体错误信息，终止创建流程；
3. 任务调度与结果反馈
   - 如果创建任务要求即时启动，则触发任务执行流程（见下一节：启动任务）。
   - 成功反馈​：`MNode` → `XNode` → `User` 逐层返回创建成功。

##### 6.2.2.1 异常处理机制

| ​​异常场景​​ | ​​处理方式​​ | ​​反馈路径​​ |
| --- | --- | --- |
| 请求校验失败（Check error） | 直接终止流程 | XNode → User（红色错误信息） |
| 任务重复（Task exist） | 放弃创建，返回存在警告 | MNode → XNode → User（红色） |
| 并发冲突 | 通过写锁（WriteLock）规避竞争风险 | 隐含在锁机制中 |
...
![](./images/wb_QdMOwDMPuhfNIXbbYhhcx7eMnAf.png)

1. 发送请求
   - 用户发送任务之前请求，所有执行请求均转发到 MNode 执行；
   - MNode 根据任务 ID 标记任务为执行状态，执行状态无法进行任务修改、再执行、删除操作。
   - MNode taosX 调度器将指定任务加入执行队列：
      - 当执行队列不可用时，标记任务失败，并发送失败消息到 MNode。
      - 加入队列成功后，标记任务启动，并发送成功消息到 MNode。
   - 根据请求结果：
      - 如果请求失败，继续触发重试流程，直到达到最大重试次数。
      - 最终失败后，标记任务发送执行请求失败，发送系统通知和告警。
2. 分片生成
   - MNode 调度器后台开始执行任务，首先进行分片生成：
      - 根据数据特征，生成均匀的分片信息；
   - 根据负载均衡策略，将分片派发到不同的 XNode 上。
      - 任务派发时，使用异步发送，不阻塞派发线程；
      - 如果任务派发成功，标记该分片的执行节点、开始时间。
      - 如果任务派发失败，将分片派发到其他 XNode 节点，重试直到任务派发成功。
3. 分片执行
   - 运行分片过程中，持续、异步上报运行指标
   - 分片执行完毕，返回分片执行结果（shardId、指标、成功信息）
   - 分片执行失败，触发重试：
      - 达到最大重试次数，视为分片执行失败；
      - 返回分片执行信息（shardId、指标、失败原因）
4. 结果聚合
   - 主节点异步等待所有分片状态：
      - 成功：标记分片执行成功（shardId、指标），异步更新元数据，并同步存储到数据目录。
      - 失败：标记分片执行失败，记录失败原因，重新派发分片，直到达到最大重试次数后仍然失败，则停止其他分片任务，将该任务标记为运行失败（对应原 taosx 任务状态 `Failed`）。
...
![](./images/wb_AAEdwCKAuh9W01b43A8cH9WfnPc.png)
...
![](./images/wb_DjkZwtFmSho01hbUFb3cLSJWnmg.png)

1. 用户发起删除任务请求；
2. XNode 转发删除请求到 MNode:
   - 请求失败则直接报错退出；
3. MNode 申请写锁：
   - 超时无法申请到写锁，直接退出并向 XNode 回复错误；
4. 在写锁保护范围内，执行以下操作：
   - 锁定任务，停止接收新的任务调度操作；
   - 启动停止任务流程，直到所有子任务停止。
   - 删除任务元数据，删除失败则报错。
...
- RoundRobin
  ...
  ![](./images/wb_PH3uwZFLKhs5CcbUk4bctvpknDA.png)

- Range
  - 假设集群 3 个节点（X1-X3），任务 A 生成 7 个分片（A1 - A7），按照 Range 方式分配如下：
  ![](./images/wb_GFVgwh6V2hptXbb6EcnclYJUn7f.png)
...
1. 节点宕机的确认：
   - 节点宕机必须在 MNode 和至少一个 XNode 节点确认同时无法连接才会确认为宕机，触发任务重新平衡；
   - 当无法找到至少一个 XNode 节点时，视为系统异常，报警；
   - 当 XNode 无法连接到 MNode 时，不做任何操作，当前任务继续执行；
2. 重新平衡：
   - 首先将 MNode 中的节点状态置为 Offline，不再进行新任务的消费；
...
![](./images/img_W54xbzpqzoGWlbxtwNVcH91XnNc.png)
...
- 添加数据备份 CRON 表达式（`xNodeBackupCron`），taosX 将自动定时备份数据文件。
  ...
  - 示例：
    ...
    - `0 0/10 * * * ?` 表示每 10 分钟 执行任务
- 添加数据备份路径参数（`xNodeBackupDir`），作为数据备份存储路径（支持S3？）。

## 7. 待解决问题

### 7.1 Mnode 上的 taosX 管理模块是否采用独立进程？

独立线程的好处：
- 出现问题不阻塞 TDengine

### 7.2 taosX 各个数据源下分片如何实现？
