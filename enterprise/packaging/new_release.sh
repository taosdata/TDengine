...
while getopts "hb:n:l:v:d:N:P:M:D:G" arg
do
  case $arg in    
    b)
...
echo "execute new_release.sh -b ${branch} -n ${version} -D ${baseDir} -l ${pagMode} -v ${versionType} -d ${dockerMode} -M ${cusEmail} -G ${grantValue}"
...
function build_TDengine() {
    ...
    if [ "$versionType" == "enterprise" ];then
        ...
        echo "cmake ../ -DCMAKE_BUILD_TYPE=Release -DASSERT_NOT_CORE=true -DCPUTYPE=${os_arch} -DWEBSOCKET=true -DOSTYPE=${os_type} -DSOMODE=dynamic -DDBNAME=taos -DVERTYPE=stable -DVERDATE='${build_time}' -DGITINFO=${gitinfo} -DGITINFOI=${gitinfoOfInternal} -DVERNUMBER=${version} -DVERCOMPATIBLE=3.0.0.0 -DBUILD_HTTP=false -DBUILD_TOOLS=true -DGRANT_VALUE=${grantValue}"
        cmake ../ -DCMAKE_BUILD_TYPE=Release -DASSERT_NOT_CORE=true -DCPUTYPE=${os_arch}                       \
            -DWEBSOCKET=true -DOSTYPE=${os_type} -DSOMODE=dynamic -DDBNAME=taos -DVERTYPE=stable                  \
            -DVERDATE='${build_time}' -DGITINFO=${gitinfo} -DGITINFOI=${gitinfoOfInternal} -DVERNUMBER=${version}   \
            -DVERCOMPATIBLE=3.0.0.0 -DBUILD_HTTP=false -DBUILD_TOOLS=true -DGRANT_VALUE=${grantValue}
    else
...
function build_taoskeeper() {
    ...
    if [[ "${os_type}" != "Darwin" && "$versionType" == "community" ]]; then
...
function preparepkg() {
    ...
    wsheader_files="${communityDir}/include/taosws.h"
...
function make_linux_pkg() {    
    ...
    if [[ "${packageType}" == "all" || "${packageType}" == "server" ]]; then
        ...
        # copy bin files
        serverBin=(remove.sh ${prefix} ${prefix}d ${prefix}keeper TDinsight.sh set_core.sh ${prefix}adapter ${prefix}d-dump-cfg.gdb  tdengine-datasource.zip udfd startPre.sh  ${prefix}Benchmark ${prefix}dump tdengine-datasource.zip.md5 quick_deploy.sh)
        ...
        # copy cfg files
        serverCfg=(${prefix}adapter.service  ${prefix}adapter.toml  ${prefix}.cfg  ${prefix}d.service  ${prefix}keeper.service ${prefix}keeper.toml)
        ...
        cd ${install_dir}
        cp -r connector/ driver/ examples/ taosx/ share/ ${serverPackageName}/
    ...
    if [[ "${versionType}" == "community" && "${os_type}" == "Linux" && "${os_arch}" == "x64" ]]; then
        ...
        cp ${install_dir}/cfg/${prefix}.cfg ${install_dir}/cfg/${prefix}.service  ${install_dir}/${liteServerPackageName}/cfg/
        ...
        cp -r ${install_dir}/examples/c ${install_dir}/${liteServerPackageName}/
...
function make_mac_pkg() {
    if [ "${versionType}" == "community" ]; then
        ...
        sudo cp -f ${explorerDir}/server/example/explorer.toml /opt/tdengine/cfg/
        ...
        echo "enterprise version is not supported on macOS"
    fi
}
