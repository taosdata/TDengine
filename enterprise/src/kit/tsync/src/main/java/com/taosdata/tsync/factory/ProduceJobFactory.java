...
public class ProduceJobFactory {

    public static Job build(JSONObject configJSON, ConfigurationRepository configurationRepository) {
        Configuration configuration = ConfigurationFactory.build(ConfigurationType.PRODUCE_JOB, configJSON);
        configurationRepository.add(configuration);
        return new Job(ConfigurationType.PRODUCE_JOB, configuration.getId());
    }
}