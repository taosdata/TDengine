...
class DataLoader:
    ...
    def load_n_forecast(self):
        ...
        for key, value in self.fc_algos.items():
            ...
            mse = mean_squared_error(eval_list, r["res"][1])
            print(f"The mse of predicate is: {mse}")

            self.do_draw_fc_results(input_list, len(r["res"]) > 2, r["res"], self.rows, key)
            self.write_fc_results(rowIndex, key, value, mse, f'./{key}.png', elapsed_time=(end - start) * 1000)
            rowIndex += 1
    ...
    def open_fc_file(self):
        ...
        self.worksheet = self.workbook.add_worksheet()
        col_title = ['algorithm', 'params', 'mse', 'elapsed_time(ms.)']
        for col_num, data in enumerate(col_title):
            self.worksheet.write(0, col_num, data)


    def write_fc_results(self, row, name, param, mse, result_img, elapsed_time):
        self.worksheet.write(row, 0, name)
        self.worksheet.write(row, 1, param)
        self.worksheet.write(row, 2, mse)
        self.worksheet.write(row, 3, elapsed_time)
