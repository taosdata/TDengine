  ...
  for (int i = 0; i < pNode->replica; ++i) {
    ...
    if (retLen == fwdLen) {
      sTrace("vgId:%d peer:%s, forward is sent, ver:%d len:%d", 
              pNode->vgId, pPeer->ep, pWalHead->version, pWalHead->len);
    } else {
      sError("vgId:%d peer:%s, failed to forward, ver:%d retLen:%d", 
              pNode->vgId, pPeer->ep, pWalHead->version, retLen);
...
void syncConfirmForward(void *param, uint64_t version, int32_t code)
{
  ...
  if (retLen == msgLen) {
    sTrace("vgId:%d peer:%s, forward-rsp is sent, ver:%d ", pNode->vgId, pPeer->ep, version);
  } else {
    sTrace("vgId:%d peer:%s, failed to send forward ack, restart", pNode->vgId, pPeer->ep);
    syncRestartConnection(pPeer);
  }
}
...
static void syncRemovePeer(SSyncPeer *pPeer)
{
  ...
  sPrint("vgId:%d peer:%s, it is removed", pNode->vgId, pPeer->ep);
...
static SSyncPeer *syncAddPeer(SSyncNode *pNode, const SNodeInfo *pInfo) 
{
  ...
  sprintf(pPeer->ep, "%s:%d", pPeer->fqdn, pPeer->port);
  ...
  sPrint("vgId:%d peer:%s, it is configured, port:%d", pNode->vgId, pPeer->ep, pPeer->port);
  int ret = strcmp(pPeer->fqdn, tsNodeFqdn);
  if (pPeer->nodeId == 0 || (ret > 0) || (ret == 0 && pPeer->port > tsSyncPort)) {
    sTrace("vgId:%d peer:%s, start to check peer connection", pNode->vgId, pPeer->ep);
...
static void syncChooseMaster(SSyncNode *pNode) {
  ...
  if (index >= 0) {
    if (index == pNode->selfIndex) {
      ...
      sPrint("vgId:%d peer:%s, it shall work as master", pNode->vgId, pPeer->ep);
    }
  } else {
    ...
    for (int i=0; i<pNode->replica; ++i) {
      ...
      if ( index < 0 ) {
        ...
        if ( i == pNode->selfIndex ) {
          sError("vgId:%d peer:%s: is master, work as slave instead", pNode->vgId, pTemp->ep);
...
static void syncCheckRole(SSyncPeer *pPeer, SPeerStatus peersStatus[], int8_t newRole)
{
  ...
  sTrace("vgId:%d peer:%s, own role:%s, new peer role:%s", pNode->vgId, pPeer->ep, 
  ...
  if ( pMaster ) {
    ...
    sTrace("vgId:%d peer:%s, it is the master", pNode->vgId, pMaster->ep);
    
    if ( nodeRole == TAOS_SYNC_ROLE_UNSYNCED ) {
      if ( nodeVersion < pMaster->version) {
        syncRequired = 1;
      } else {
        sPrint("vgId:%d peer:%s is master, work as slave, ver:%d", pNode->vgId, pMaster->ep, pMaster->version);
        nodeRole = TAOS_SYNC_ROLE_SLAVE;
        (*pNode->notifyRole)(pNode->ahandle, nodeRole);
      }
...
void syncRestartConnection(SSyncPeer *pPeer)
{
  ...
  sTrace("vgId:%d peer:%s, restart connection", pNode->vgId, pPeer->ep);
  tclose(pPeer->peerFd);
  tclose(pPeer->syncFd);
  taosTmrStopA(&pPeer->timer);
...
{
  SSyncNode *pNode = pPeer->pSyncNode;
  sTrace("vgId:%d peer:%s, sync-req is received", pNode->vgId, pPeer->ep);

  if (pPeer->ip == 0) return;

  if (nodeRole != TAOS_SYNC_ROLE_MASTER) {
    sError("vgId:%d peer:%s, I am not master anymore", pNode->vgId, pPeer->ep);
    tclose(pPeer->syncFd);
    return;
  }

  if (pPeer->sstatus != TAOS_SYNC_STATUS_INIT) {
    sTrace("vgId:%d peer:%s, sync is already started", pNode->vgId, pPeer->ep);
  ...
  if (ret != 0) {
    sError("vgId:%d peer:%s, failed to create sync thread(%s)", pNode->vgId, pPeer->ep, strerror(errno));
  } else {
    pPeer->sstatus = TAOS_SYNC_STATUS_START;
    sTrace("vgId:%d peer:%s, thread is created to retrieve data", pNode->vgId, pPeer->ep);
  }
}
...
static void syncNotStarted(void *param, void *tmrId)
{
  ...
  pPeer->timer = NULL;
  sPrint("vgId:%d peer:%s, sync connection is still not up, restart", pNode->vgId, pPeer->ep);
  syncRestartConnection(pPeer);
}
...
static void syncRecoverFromMaster(void *param, void *tmrId)
{
  ...
  if ( nodeSStatus != TAOS_SYNC_STATUS_INIT) {
    sTrace("vgId:%d peer:%s, sync is already started, status:%d", pNode->vgId, pPeer->ep, nodeSStatus);
    return;
  } 

  taosTmrStopA(&pPeer->timer);
  if (tsSyncNum >= tsMaxSyncNum) {
    sPrint("vgId:%d peer:%s, %d syncs are in process, try later", pNode->vgId, pPeer->ep, tsSyncNum);
    taosTmrReset(syncRecoverFromMaster, 500, pPeer, syncTmrCtrl, &pPeer->timer);
    return;
  }

  sTrace("vgId:%d peer:%s, try to sync", pNode->vgId, pPeer->ep)
  ...
  if (write(pPeer->peerFd, &firstPkt, sizeof(firstPkt)) != sizeof(firstPkt) ) {
    sError("vgId:%d peer:%s, failed to send sync-req to peer", pNode->vgId, pPeer->ep);
  } else {
    nodeSStatus = TAOS_SYNC_STATUS_START;
    sPrint("vgId:%d peer:%s, sync-req is sent", pNode->vgId, pPeer->ep);
  }
...
static void syncProcessFwdResponse(char *cont, SSyncPeer *pPeer) 
{
  ...
  sTrace("vgId:%d peer:%s, forward-rsp is received, ver:%d ", pNode->vgId, pPeer->ep, pFwdRsp->version);
...
static void syncProcessForwardFromPeer(char *cont, SSyncPeer *pPeer)
{
  ...
  sTrace("vgId:%d peer:%s, forward is received, ver:%d ", pNode->vgId, pPeer->ep, pHead->version);
  ...
  if (nodeRole == TAOS_SYNC_ROLE_SLAVE) {
    ...
    if (nodeSStatus != TAOS_SYNC_STATUS_INIT) {
      ...
      sError("vgId:%d peer:%s, forward discarded, ver:%d", pNode->vgId, pPeer->ep, pHead->version);
    }
  }
...
static void syncProcessPeersStatusMsg(char *cont, SSyncPeer *pPeer)
{
  ...
  sTrace("vgId:%d peer:%s, status msg received, self:%s ver:%d peer:%s ver:%d",
         pNode->vgId, pPeer->ep, syncRole[nodeRole], nodeVersion,
...
static int syncProcessPeerMsg(void *param, void *buffer)
{
  ...
  if (hlen != sizeof(head)) {
    sTrace("vgId:%d peer:%s, failed to read msg, hlen:%d", pNode->vgId, pPeer->ep, hlen);
    return -1;
  }

  // head.len = htonl(head.len);
  if (head.len > TSDB_DEFAULT_PKT_SIZE || head.len <0) {
    sError("vgId:%d peer:%s, invalid pkt length, len:%d", pNode->vgId, pPeer->ep, head.len);
    return -1;
  } 

  bytes = taosReadMsg(pPeer->peerFd, cont, head.len);
  if (bytes != head.len) {
    sError("vgId:%d peer:%s, failed to read, bytes:%d len:%d", pNode->vgId, pPeer->ep, bytes, head.len);
    return -1;
  }
...
static void syncSendPeersStatusMsgToPeer(SSyncPeer *pPeer, char ack)
{
  ...
  if (retLen == statusMsgLen) {
    sTrace("vgId:%d peer:%s, status msg is sent", pNode->vgId, pPeer->ep);
  } else {
    sTrace("vgId:%d peer:%s, failed to send status msg, restart", pNode->vgId, pPeer->ep);
    syncRestartConnection(pPeer);
  }
...
static void syncCheckPeerConnection(void *param, void *tmrId) 
{
  ...
  SSyncNode *pNode = pPeer->pSyncNode;
  sTrace("vgId:%d peer:%s, check peer connection", pNode->vgId, pPeer->ep);

  taosTmrStopA(&pPeer->timer);
  if (pPeer->peerFd >= 0) {
    sTrace("vgId:%d peer:%s, send role version to peer", pNode->vgId, pPeer->ep);
    syncSendPeersStatusMsgToPeer(pPeer, 1);
    return;
  }
  ...
  if (connFd < 0) {
    sTrace("vgId:%d peer:%s, failed to open tcp socket(%s)", pNode->vgId, pPeer->ep, strerror(errno));
    taosTmrReset(syncCheckPeerConnection, tsSyncTimer *1000, pPeer, syncTmrCtrl, &pPeer->timer);
    return;
  }
  ...
  if ( write(connFd, &firstPkt, sizeof(firstPkt)) == sizeof(firstPkt)) {
    sTrace("vgId:%d peer:%s, connection to peer server is setup", pNode->vgId, pPeer->ep);
    pPeer->peerFd = connFd; 
...
static void syncCreateRestoreDataThread(SSyncPeer *pPeer) 
{
  ...
  if (ret < 0) {
    sError("vgId:%d peer:%s, failed to create sync thread(%s)", pNode->vgId, pPeer->ep);
    tclose(pPeer->syncFd);
  } else { 
    sPrint("vgId:%d peer:%s, sync connection is up", pNode->vgId, pPeer->ep);
  }
}
...
static void syncProcessIncommingConnection(int connFd, uint32_t sourceIp) 
{
  ...
  if (firstPkt.syncHead.type == TAOS_SMSG_SYNC_DATA) {
    ...
    if (pPeer->peerFd >= 0) {
      sTrace("vgId:%d peer:%s, TCP connection is already up, close one", pNode->vgId, pPeer->ep);
    ...
    pPeer->pThread = taosAllocateTcpThread(tsTcpPool, pPeer, connFd);
    syncAddPeerRef(pPeer);
    sTrace("vgId:%d peer:%s, ready to exchange data", pNode->vgId, pPeer->ep);
    syncSendPeersStatusMsgToPeer(pPeer, 1);
  }
...
static void syncProcessBrokenLink(void *param) {
  ...
  sTrace("vgId:%d peer:%s, TCP link is broken(%s)", pNode->vgId, pPeer->ep, strerror(errno));
