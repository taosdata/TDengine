...
void *processShellMsg(void *param) {
  static int num = 0;
  taos_qall  qall;
  SRpcMsg    rpcMsg;
...
int main(int argc, char *argv[]) {
  ...
  if (pthread_create(&thread, &thattr, (void *)processShellMsg, NULL) != 0) {
    tError("%s failed to create thread, reason:%s", label, strerror(errno));
    return -1;
  }
