...
void taosReadDataDirCfg(char *v1, char *v2, char *v3) {
  ...
  if (v2 != NULL) {
    ...
    if (level < 0 || level >= TSDB_MAX_TIERS) {
      uError("config option:dataDir, input value2:%s, not in range [0, %d), set default 0", v2, TSDB_MAX_TIERS);
      level = 0;
    }
  }
  ...
  if (v3 != NULL) {
    ...
    if (primary < 0 || primary > 1) {
      uError("config option:dataDir, input value3:%s, not in range [0, 1], set default 1", v3);
      primary = 0;
    }
  }
