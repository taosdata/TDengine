...
static int32_t genUniqActiveFromLegacy(SGrantUniqObj *pObj, SGrantStatus *pStatus) {
  ...
  ASSERTS(0 == grantUniqParseActiveCode(uniqObj, NULL), "invalid parse uniq active");
...
static int32_t mndProcessGrantRsp(SRpcMsg *pRsp) {
  ...
  int16_t nGrantRsp = atomic_add_fetch_16(&grantHandle.nGrantRsp, 1);
  ...
  if (pDnodeInfo) {
    uDebug("succeed to receive grant msg from dnode:%d, %s:%" PRIu16 ", nReq:%" PRIi16 ", nRsp:%" PRIi16 ", gtid:%s",
           grantMsgRsp.dnodeId, pDnodeInfo->ep.fqdn, pDnodeInfo->ep.port, grantHandle.nGrantReq, nGrantRsp, tbuf);
    mndProcessDnodeSGrantMsg(grantHandle.pMnode, pDnodeInfo, &grantMsgRsp, &gStatus);
  }

_exit:
  if (code != 0) {
    uWarn("receive grant msg from dnode, nReq:%" PRIi16 ", nRsp:%" PRIi16 " with error:%s, gtid:%s",
          grantHandle.nGrantReq, nGrantRsp, tstrerror(code), tbuf);
  }
...
static int32_t mndProcessGrantHBSyncInfo(SMnode *pMnode, int8_t type) {
  ...
  if (!pGrant) {
    SGrantState grantState = {.ts = curTime,
                              .lastState = GRANT_STATE_INIT,
                              .state = GRANT_STATE_UNGRANTED,
                              .reason = GRANT_STATE_REASON_INIT};
  ...
  if (pLastState->state == GRANT_STATE_REVOKED) {
    ...
    if (gStatus.revokedExpireSec > grantCurTime) {
      ...
      gStatus.expired = 1;
      uWarn("grant cluster expired at %s %u, curtime: %u, set to un-grant state", ts, gStatus.revokedExpireSec,
            grantCurTime);
    }
    goto _exit;
  }
...
static int32_t mndProcessGrantHBImpl(SMnode *pMnode, int8_t type) {
  ...
  for (int32_t i = 0; i < dnodeSize; ++i) {
    ...
    if (0 == mndSendGrantStatusToDnode(pMnode, info, clusterTime)) {
      atomic_add_fetch_16(&grantHandle.nGrantReq, 1);
    }
  }
...
static int32_t mndRetrieveGrantFull(SRpcMsg *pReq, SShowObj *pShow, SSDataBlock *pBlock, int32_t rows) {
  SMnode           *pMnode = pReq->info.node;
  int32_t           numOfRows = 0;
  SGrantUniqStatus *pStatus = &gStatus;
...
static int32_t mndRetrieveGrantLog(SRpcMsg *pReq, SShowObj *pShow, SSDataBlock *pBlock, int32_t rows) {
  ...
  if (pShow->numOfRows < 1) {
    ...
    for (int32_t i = 0; i < pGrant->nStates; ++i) {
      ...
      if (i == 0) {
        snprintf(tmp, 50, "%" PRIi64 ",%d,%d,%d", pState->ts, pState->reason, pState->lastState, pState->state);
      } else {
        snprintf(tmp, 50, ";%" PRIi64 ",%d,%d,%d", pState->ts, pState->reason, pState->lastState, pState->state);
      }
      tmpLen = strlen(tmp);
    ...
    for (int32_t i = 0; i < pGrant->nActives; ++i) {
      ...
      if (i == 0) {
        snprintf(tmp, 50, "%" PRIi64 ",%s", pActive->ts, pActive->active);
      } else {
        snprintf(tmp, 50, ";%" PRIi64 ",%s", pActive->ts, pActive->active);
      }
      tmpLen = strlen(tmp);
    ...
    for (int32_t i = 0; i < nMachines ; ++i) {
      ...
      if (i == 0) {
        snprintf(tmp, 50, "%" PRIi64 ",%s", pMachine->ts, pMachine->machine);
      } else {
        snprintf(tmp, 50, ";%" PRIi64 ",%s", pMachine->ts, pMachine->machine);
      }
      tmpLen = strlen(tmp);
