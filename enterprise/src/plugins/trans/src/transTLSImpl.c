...
void destroySSLCtxImpl(SSL_CTX* ctx) {
...
int32_t transTlsCtxCreateImpl(const SRpcInit* pInit, SSslCtx** ppCtx) {
  int32_t code = 0;
  int32_t lino = 0;
  ...
  pCtx->sslCtx = initSSLCtxImpl(pCtx->certfile, pCtx->keyfile, pCtx->cafile, cliMode);
  if (pCtx->sslCtx == NULL) {
...
int32_t sslInitImpl(SSslCtx* pCtx, STransTLS** ppTLs) {
  ...
  code = sslBufferInitImpl(&pTls->readBuf, 4096);
  TAOS_CHECK_GOTO(code, &lino, _error);
...
void sslSetModeImpl(STransTLS* pTls, int8_t cliMode) {
  if (cliMode) {
...
int32_t sslConnectImpl(STransTLS* pTls, uv_stream_t* stream, uv_write_t* req) {
  int32_t code = 0;
  int32_t lino = 0;

  sslSetModeImpl(pTls, 1);

  code = sslInitConnImpl(pTls);
  TAOS_CHECK_GOTO(code, NULL, _error);

  code = sslDoConnImpl(pTls);
  TAOS_CHECK_GOTO(code, NULL, _error);
...
int32_t sslWriteImpl(STransTLS* pTls, uv_stream_t* stream, uv_write_t* req, uv_buf_t* pBuf, int32_t nBuf,
  ...
  while ((nread = BIO_read(pTls->writeBio, buf, sizeof(buf))) > 0) {
    code = sslBufferAppendImpl(&pTls->sendBuf, buf, nread);
    TAOS_CHECK_GOTO(code, &lino, _error);
    total += nread;
  }
  if (total <= 0) {
  ...
  if (status == 0) {
    ...
    sslBufferUnrefImpl(&pTls->sendBuf);
    tError("Failed to write SSL data: %s", uv_err_name(status));
    TAOS_CHECK_GOTO(TSDB_CODE_THIRDPARTY_ERROR, &lino, _error);
  }
...
int32_t sslReadImpl(STransTLS* pTls, SConnBuffer* pBuf, int32_t nread, int8_t cliMode) {
  int32_t     code = 0;
  int32_t     lino = 0;
  SSslBuffer* sslBuf = &pTls->readBuf;
  int32_t     nwrite = 0;
...
int32_t sslBufferInitImpl(SSslBuffer* buf, int32_t cap) {
  if (buf == NULL) {
    return TSDB_CODE_INVALID_PARA;
  }
...
void sslBufferClearImpl(SSslBuffer* buf) {
  buf->len = 0;
  memset(buf->buf, 0, buf->cap);
}

void sslBufferDestroyImpl(SSslBuffer* buf) {
  if (buf == NULL) {
    return;
  }
...
int32_t sslBufferAppendImpl(SSslBuffer* buf, uint8_t* data, int32_t len) {
  int32_t code = 0;
  if (buf == NULL) {
    return TSDB_CODE_INVALID_PARA;
  }
...
int32_t sslBufferGetAvailableImpl(SSslBuffer* buf, int32_t* available) {
  if (buf == NULL) {
    return TSDB_CODE_INVALID_PARA;
  }
...
int32_t sslBufferReallocImpl(SSslBuffer* buf, int32_t newCap, uv_buf_t* uvbuf) {
  int32_t code = 0;
  int32_t lino = 0;
  if (buf == NULL) {
    return TSDB_CODE_INVALID_PARA;
...
int8_t transCheckTlsEnvImpl(const char* caPath, const char* certPath, const char* keyPath, const char* instName,
  ...
  if (!transCheckFileImpl(caPath, &flag[0], instName)) {
    return 0;
  }

  if (!transCheckFileImpl(certPath, &flag[1], instName)) {
    return 0;
  }

  if (!transCheckFileImpl(keyPath, &flag[2], instName)) {
    return 0;
  }
