...
int32_t translateView(STranslateContext* pCxt, SNode** pTable, SName* pName) {
   ...
   int32_t code = catalogChkGrant(pCxt->pParseCxt->pCatalog, TSDB_GRANT_VIEW);
   if (TSDB_CODE_SUCCESS != code) {
     code = generateSyntaxErrMsg(&pCxt->msgBuf, code, tstrerror(code));
     goto _exit;
   }
   code = getViewQuerySqlUser(pCxt, pName, &querySql, &user);
   if (TSDB_CODE_SUCCESS != code) {
