...
def ErrorInfo(conn, code, sql):
	...
	if(code == 0):
		...
		print sql + " failure, reason: " + so.taos_errstr(conn)
...
def InsertCount(tableId, rowsCount, printRows, recordPerRow, host, user, password, dbName, port, tablePrefix):
	...
	sql = ''
	newConn = ConnectTDengine(host, user, password, dbName, port)
	# newConn = 0
	...
	for row in range(rowsCount):
		...
		code = so.taos_query(newConn, sql)
		# if (code == 0):
		# 	ErrorInfo(newConn, code, sql)
...
def QueryExecute(tableId, conn, tablePrefix):
	...
	for i in range(fieldCount):
		...
		metas.append(p)
		filedNames.append(p.contents.name)
	...
	# 	ptr += i*ptrOffset
	# 	print ptr
	# 	fieldName = cast(ptr, POINTER(c_char))
	# 	print "name:" + fieldName[0:31]
	# 	fieldBytes = cast(ptr+32, POINTER(c_short))
	# 	print "bytes:" + str(fieldBytes[0])
	# 	fieldType = cast(ptr+34, POINTER(c_byte))
...
if __name__=="__main__":
	...
	# for tableId in range(tableCount):
	# 	p = pool.apply_async(InsertCount, args=(tableId, rowsCount, recordPerRow, printRows, host, user, password, dbName, port, tablePrefix))
