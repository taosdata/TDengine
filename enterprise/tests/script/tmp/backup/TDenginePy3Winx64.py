...
def ConnectTDengine(host, user, password, dbName, port):
	so.taos_connect.restype = c_uint64
	conn = so.taos_connect(host, user, password, dbName, port)
	if(conn == 0):
		print("Connect to TDengine failed")
		sys.exit(1)
	else:
		print("Connect to TDengine success")
	return(c_uint64(conn))
...
def UserResult(conn):
	if (conn != 0):
		so.taos_use_result.restype = c_uint64
...
def InsertCount(tableId, rowsCount, recordPerRow, printRows, host, user, password, dbName, port, tablePrefix):
...
def FieldValueExt(meta, fieldPtr):
	if (meta.contents.type == 1):
		filedValue = cast(fieldPtr, POINTER(c_bool))[0]
	elif (meta.contents.type == 2):
		filedValue = cast(fieldPtr, POINTER(c_byte))[0]
	elif (meta.contents.type == 3):
		filedValue = cast(fieldPtr, POINTER(c_short))[0]
	elif (meta.contents.type == 4):
		filedValue = cast(fieldPtr, POINTER(c_int))[0]
	elif (meta.contents.type == 5):
		filedValue = cast(fieldPtr, POINTER(c_ulong))[0]
	elif (meta.contents.type == 6):
		filedValue = cast(fieldPtr, POINTER(c_float))[0]
	elif (meta.contents.type == 7):
		filedValue = cast(fieldPtr, POINTER(c_double))[0]
	elif (meta.contents.type == 8):
		filedValue = str(cast(fieldPtr, POINTER(c_char))[0:(meta.contents.bytes)], "ascii")
	elif (meta.contents.type == 9):
		filedValue = cast(fieldPtr, POINTER(c_long))[0]
	return(filedValue)
...
def QueryExecute(tableId, conn, tablePrefix):
	...
	for i in range(fieldCount):
		p = so.taos_fetch_fields(c_uint64(result + i*(sizeof(TAOS_FIELD))))
		metas.append(p)
	...
	so.taos_fetch_row.restype = c_uint64
	rowPtr = so.taos_fetch_row(c_uint64(result))
	if (rowPtr == 0):
		...
		while (rowPtr > 0):
			...
			for fields in range(fieldCount):
				meta = metas[fields]
				fieldPtr = cast(rowPtr, POINTER(c_uint64))[0]
			...
			rowPtr = so.taos_fetch_row(c_uint64(result))
	FreeResult(c_uint64(result))


def MultipulTableInsertRead(tableCount, rowsCount, recordPerRow, printRows, host, user, password, dbName, port, tablePrefix):
	for tableId in range(tableCount):
		InsertCount(tableId, rowsCount, recordPerRow, printRows, host, user, password, dbName, port, tablePrefix)


if __name__=="__main__":
	host = bytes(str("192.168.100.128"), 'ascii')
	...
	# MultipulTableInsertRead(tableCount, rowsCount, recordPerRow, printRows, host, user, password, dbName, port, tablePrefix)
