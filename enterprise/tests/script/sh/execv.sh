...
TAOS_CFG=$NODE_DIR/cfg/tbase.cfg
...
if [ "$SHELL_OPTION" = "true" ]; then 
  if [ "$EXEC_OPTON" = "start" ]; then 
    echo "ExcuteCmd:" $EXE_DIR/tbase -c $CFG_DIR -u $USERS -p
    $EXE_DIR/tbase -c $CFG_DIR -u $USERS -p
  else
    #relative path
    RCFG_DIR=sim/$NODE_NAME/cfg
    PID=`ps -ef|grep -v taosd | grep tbase | grep $RCFG_DIR | grep -v grep | awk '{print $2}'`
    if [ -n "$PID" ]; then 
      sudo kill -9 $PID
    fi 
  fi
