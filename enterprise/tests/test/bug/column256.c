...
int main(int argc, char *argv[])
{
  ...
  tscPrint("tableNum:%d tableInterval:%d tableColumns:%d streamInterval:%d streamColumns:%d replica:%d createTable:%d"
    , tableNum, tableInterval, tableColumns, streamInterval, streamColumns, replica, createTable);
  ...
  if (createTable > 0) {
    ...
    if (streamInterval != 0) {
      tscPrint("sleep 2 seconds to wait table created");
      taosMsleep(2000);
