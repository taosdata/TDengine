...
void check_env(MYSQL *mysql, char *table_name) {
    ...
    const char *CREATE_PERF_TABLE = "create table if not exists %s(id int not null auto_increment,\
            ts timestamp, tag varchar(15), lat float, lon float, direction smallint, primary key(id))";
 ...
 */
void simple_insert(MYSQL *mysql, char *tname, entry_list *entries,
                   int32_t sample_interval, int32_t rec_size) {
    char qstr[256] = {0};
    ...
    int32_t total_sample_rec_cnt =
            (entries->cur_len + sample_interval - 1) / sample_interval;
    ...
    sampling_ele **sample_recs = (sampling_ele **) malloc(POINTER_BYTES * total_sample_rec_cnt);
    ...
    for (int32_t i = 0; i < entries->cur_len; ++i) {
        sprintf(qstr, "insert into %s (ts, tag, lat, lon, direction) values('%s', '%s', %f, %f, %d)",
                tname, el->ts, el->tag,
                el->lat, el->lon,
                el->direction);
        el = el->next;
        ...
        if (i % sample_interval == 0 && i != 0) {
            record_sampling_end(sample_recs[sampling_cnt - 1],
                                sample_interval, rec_size);
    ...
    record_sampling_end(sample_recs[sampling_cnt - 1],
                        entries->cur_len % sample_interval, rec_size);
 ...
 * insert into db
 */
void dynamic_bind_insert_operation(MYSQL *mysql, char *table_name, entry_list *entries,
                                   int32_t sample_interval, int32_t rec_size) {
    MYSQL_BIND bind[5] = {0};
    ...
    const char *INSERT_SAMPLE = "INSERT INTO %s (ts, tag, lat, lon, direction)\
                       VALUES(?,?,?,?,?)";
...
void main(int argc, char **argv) {
    if (argc == 1) {
        printf("pls specify the input data directory");
        exit(1);
    }
...
//    simple_insert(&mysql, table_name, entries, 10000, rec_size(entries->data));
...
void multi_simple_insert(MYSQL *mysql, char *tname, entry_list *entries,
                         int32_t sample_interval, int32_t rec_size, int32_t max_sql_len) {
