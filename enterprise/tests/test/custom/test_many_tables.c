...
int main(int argc, char * argv[]) {
    ...
    int    ntables = 30000;
	int    tablesOfVnode = 8000;
    int    nrecords_per_table = 5;
    int    nrecords_per_request = 150;
    int    nconnections = 5;
    long   time_interval = 60000;
	int    blocks = 16384;
    char  command[BUFFER_SIZE] = "\0";
	logFp = fopen("result.log", "w");
	int replica = 2;

	printf("usage: ntables rowNum replica cacheSize tablesOfVnode.\n\n");
	fprintf(logFp, "usage: ntables rowNum replica cacheSize tablesOfVnode.\n\n");
	
	// a simple way to parse input parameters
	if (argc >= 2) ntables = atoi(argv[1]);
	if (argc >= 3) nrecords_per_table = atoi(argv[2]);
	if (argc >= 4) replica = atoi(argv[3]);
	if (argc >= 5) blocks = atoi(argv[4]);
	if (argc >= 6) tablesOfVnode = atoi(argv[5]);
	
	printf("parameters: ntables:%d rowNum:%d replica:%d cacheSize:%d tablesOfVnode:%d\n", ntables, nrecords_per_table, replica, blocks, tablesOfVnode);
	fprintf(logFp, "parameters: ntables:%d rowNum:%d replica:%d cacheSize:%d tablesOfVnode:%d\n", ntables, nrecords_per_table, replica, blocks, tablesOfVnode);
	fflush(logFp);
    ...
    sprintf(command, "create database if not exists %s replica %d cache %d ablocks 4 tblocks 500 tables %d", db_name, replica, blocks, tablesOfVnode);
    taos_query(taos, command);

    sprintf(command, "use %s", db_name);
    taos_query(taos, command);

    sprintf(command, "create table mt (ts timestamp, v1 int, v2 int, v3 int, v4 int, v5 int, v6 int, v7 int, v8 int, v9 int, v10 int) tags (t int)");
    taos_query(taos, command);
...
void * sync_write(void * sarg) {
    ...
    for (int i = 0; i < winfo->nrecords_per_table; i++){
        ...
        for (int tID = winfo->start_table_id; tID <= winfo->end_table_id; tID++){
            ...
            int val = randValue();
            pStr += sprintf(pStr, "%s%d values (%ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d) ", winfo->tb_prefix, tID, time_counter, val+0, val+10, val+20, val+30, val+40, val+50, val+60, val+70, val+90);
