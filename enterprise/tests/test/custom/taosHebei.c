...
void gsParseDirectory(const char *directoryName, const char *prefix, char **fileArray, int totalFiles)
{
  fileArray[0] = "part-00000.csv";
  fileArray[1] = "part-00001.csv";
  fileArray[2] = "part-00002.csv";
  fileArray[3] = "part-00003.csv";
}
...
char gsTablePrefix[16] = "";

int  gsAblocks = 10;
int  gsTblocks = 2000;
int  gsCache = 204800; //512K
int  gsRows = 10000;      //2^24 / 900
int  gsMaxTables = 500;
bool gsSort = false;
...
void gsInit(int argc, char **argv)
{
  ...
  gsAddArgs("-cache",       &gsCache,             GS_ARG_TYPE_INT,    false, 1,   10000, 0,   "Cache block size of vnode, should large than rowsize*2048, default is 4M");
...
void gsParseSchemaLine(char *line, char *schemaLine)
{
  ...
  sprintf(sql + len, "%s", ") tags(city binary(10), station_code int)");
...
GsTable *gsCreateNewTable(char* city, int tableId)
{  
  GsTable tmp;
  ...
  if (gsCreateTable) {
    char sql[1024] = { 0 };
    sprintf(sql, "create table %s%s%d using %s tags('%s', %d)", gsTablePrefix, city, tableId, gsMetricsName, city, tableId);
    int code = taos_query(gsTaos, sql);
    if (code == TSDB_CODE_TABLE_ALREADY_EXIST) {
      gsPrint("table:%s.%s%d already exist", gsDatabaseName, gsTablePrefix, table->tableId);
    }
    else if (code != 0) {
      gsError("failed to create table:%s.%s%s%d, code:%d, error:%s, sql:%s", gsDatabaseName, gsTablePrefix, city, tableId, code, taos_errstr(gsTaos), sql);
      gsExit(EXIT_FAILURE);
    }
    else {
      //gsPrint("create table:%s.%s%s%d successed", gsDatabaseName, gsTablePrefix, city, tableId);
    }
  }
...
int gsParseDataSingleLine(char *line, GsSingleLine *data)
{
  int dotPostion[6] = {0};
  int len = 0;
  int pos = 0;
  ...
  strncpy(city, line, dotPostion[0]);
  station_code = atoi(line + dotPostion[0]+1);
  timestamp = atoi(line + dotPostion[1]+1);

  if (data->city[0] != 0) {
    if (data->station_code != station_code || data->timestamp != timestamp || strcmp(city, data->city) != 0) {
      return 1;
    }
  }
  ...
  strncpy(data->city, line, dotPostion[0]);
  data->city[dotPostion[0]] == '\0';

  if (line[dotPostion[2] + 1] != '-') {
    data->pctei = atof(line + dotPostion[2] + 1);
  }
  if (line[dotPostion[3] + 1] != '-') {
    data->prtei = atof(line + dotPostion[3] + 1);
  }
  if (line[dotPostion[4] + 1] != '-') {
    data->rctei = atof(line + dotPostion[4] + 1);
  }
  if (line[dotPostion[5] + 1] != '-') {
    data->rrtei = atof(line + dotPostion[5] + 1);
  }

  return 0;
}

void gsParseDataLine(GsSingleLine *data, int lineNum, char *csvfile)
{
  int tableId = data->station_code;
  
  GsTable *table = (GsTable*)taosGetIntHashData(gsTableHash, tableId);
  if (table == NULL) {
    table = gsCreateNewTable(data->city, data->station_code);
  }
  
  int64_t timeMs = data->timestamp;
  ...
  if (table->lastTimestamp > timeMs) {
    ...
    gsPrint("table:%s%d file:%s line:%d smaller than file:%s line:%d %s, try import"
      , gsTablePrefix, tableId, csvfile, lineNum, table->lastFile, table->lastLine, gsGenerateTime(table->lastTimestamp));
    ...
    if (false) {
      ...
      char *cache = table->cache[writepos];
      table->cacheFileName[writepos] = csvfile;
      table->cachePos += sprintf(cache, "import into %s.%s%s%d values(%d000,%lf,%lf,%lf,%lf)", gsDatabaseName, gsTablePrefix
        , data->city, data->station_code, data->timestamp, data->pctei, data->prtei, data->rctei, data->rrtei);
      table->cacheRows++;
    ...
    if (table->cacheRows == 0) {
      table->cachePos += sprintf(cache, "%s into %s.%s%s%d values(%d000,%lf,%lf,%lf,%lf)", gsOldData ? "import" : "insert", gsDatabaseName, gsTablePrefix
        , data->city, data->station_code, data->timestamp, data->pctei, data->prtei, data->rctei, data->rrtei);
      table->cacheRows++;
      ...
      table->cachePos += sprintf(cache + table->cachePos, "(%d000,%lf,%lf,%lf,%lf)", data->timestamp, data->pctei, data->prtei, data->rctei, data->rrtei);
      table->cacheRows++;
...
void gsParseCsvFile(char *csvfile)
{
  ...
  sprintf(line, "%s,%s,%s,%s,%s,%s,%s", "timestamp", "city", "station_code", "pctei", "prtei", "rctei", "rrtei");
  gsParseSchemaLine(line, NULL);
 
  GsSingleLine data;
  memset(&data, 0, sizeof(GsSingleLine));
  do {
    num = fscanf(fp, "%s", line, line + 11);
    if (num != 1) break;
    lineNum++;
    ...
    if (finished == -1) {
      ...
      gsParseDataLine(&data, lineNum, csvfile);
      memset(&data, 0, sizeof(GsSingleLine));
      if (gsParseDataSingleLine(line, &data) == -1) {
        memset(&data, 0, sizeof(GsSingleLine));
        break;
      }
    }
