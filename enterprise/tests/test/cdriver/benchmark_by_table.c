...
int64_t numOfTablesPerThread = 1000;
char    dbName[32] = "db";
...
void insertData() {
  ...
  dPrint("%sall threads:%ld finished, use %.1lf seconds, tables:%.ld rows:%ld points:%ld, speed RowsPerSecond:%.1lf PointsPerSecond:%.1lf%s",
...
void *syncTest(void *param)
{
  ...
  for (int64_t table = pInfo->tableBeginIndex; table < pInfo->tableEndIndex; ++table) {
    ...
    for (int64_t row = 0; row < pInfo->rowsPerTable; row++) {
      ...
      if (len > maxBytes) {
        if (taos_query(con, qstr)) {
          dError("thread:%d, failed to insert table:%s%ld row:%ld, reason:%s", pInfo->threadIndex, pInfo->stableName, table, row, taos_errstr(con));
        }
  ...
  dPrint("thread:%d, insert finished, use %.2f seconds, tables:%ld rows:%ld points:%ld",
          pInfo->threadIndex, (et-st)/1000.0/1000.0, totalTables, totalRows, totalPoints);
