...
namespace TDengineDriver
{
  class TDengineTest
  {
    ...
    private string stableName = "mt";
    private string tablePrefix = "t";

    private bool isInsertData = true;
    private bool isQueryData = true;

    private long tableCount = 1;
    private long totalRows = 10000;
    private long batchRows = 50;
    ...
    public long GetArgumentAsLong(String[] argv, String argName, int minVal, int maxVal, int defaultValue)
    {
      ...
      for (int i = 0; i < argc; ++i)
      {
        ...
        if (i < argc - 1)
        {
          ...
          {
            Console.WriteLine("option {0:G} should in range [{1:G}, {2:G}]", tmp, minVal, maxVal);
            ExitProgram();
          }
    ...
    public void ReadArgument(String[] argv)
    {
      ...
      stableName = this.GetArgumentAsString(argv, "-s", "mt");
      ...
      tableCount = this.GetArgumentAsLong(argv, "-n", 1, 100000, 10);
      batchRows = this.GetArgumentAsLong(argv, "-b", 1, 1000, 10);
      totalRows = this.GetArgumentAsLong(argv, "-i", 1, 1000000, 100);
    ...
    public void ExecuteInsert()
    {
      ...
      Console.Write("Total {0:G} rows inserted, {0:G} rows failed, time spend {0:G} seconds.\n"
        , this.rowsInserted, loopCount * this.batchRows - this.rowsInserted, ts.TotalSeconds);
    }
