...
class TDTestCase:
    ...
    def _import(self, threadId):
        conn = taos.connect(config=tdDnodes.getSimCfgPath())
        cursor = conn.cursor()
        importLog = open("%s/import%d.log" %(tdDnodes.getDnodesRootDir(), threadId), "a")
        try:
            sql = "use %s" %(self.db)
            importLog.write("%s %s\n" %(datetime.datetime.now(), sql))
            cursor.execute(sql)
        ...
        try:
            ...
            while l <= loop:
                ...
                if queryRows < lastQueryRows[tbId]:
                    tdLog.exit("sql:%s, queryRows:%d < lastQueryRows:%d" % (sql, queryRows, lastQueryRows[tbId]))
                else:
    ...
    def _selectAggFromTable(self):
        ...
        try:
            ...
            tdLog.exit("Failure during querying aggregation result from a single table: %s, loop: %d, sql: %s" %(str(e), l, sql))
        finally:
            cursor.close()
            conn.close()
    ...
    def run (self):
        ...
        queryThread3 = threading.Thread(target=self._selectAggFromTable, name="query-thread-3")
        queryThread3.start()
