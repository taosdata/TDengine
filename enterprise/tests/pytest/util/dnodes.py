...
class TDDnode:
	...
	def deploy(self):
		...
		self.cfg("qdebugFlag", "135")
		self.deployed = 1
		tdLog.debug("dnode:%d is deployed and configured by %s" % (self.index, self.cfgPath))		
	...
	def stop(self):
		if self.running != 0:
			cmd = "ps -ef|grep taosd | grep '%s' | grep -v grep | awk '{print $2}' | xargs kill -sigint" % (self.cfgDir)
			if os.system(cmd) != 0 :
				tdLog.exit(cmd)
	...
	def forcestop(self):
		if self.running != 0:
			cmd = "ps -ef|grep taosd | grep '%s' | grep -v grep | awk '{print $2}' | xargs kill -9" % (self.cfgDir)
			if os.system(cmd) != 0 :
				tdLog.exit(cmd)
