...
RESTful 接口不依赖于任何 TDengine TSDB 的库，因此客户端不需要安装任何 TDengine TSDB 的库，只要客户端的开发语言支持 HTTP 协议即可。TDengine TSDB 的 RESTful API 由 [taosAdapter](../../11-operations-and-tooling/03-components/03-taosadapter.md) 提供，在使用 RESTful API 之前需要确保 `taosAdapter` 正常运行。
...
HTTP 请求的 Header 里需带有身份认证信息，TDengine TSDB 支持 Basic 认证、Token 认证与自定义认证三种机制，后续版本将提供标准安全的数字签名机制来做身份验证。
...
错误码和错误描述请参考[错误码](../09-error-codes.md)
...
TDengine TSDB 从 3.4.0.0 版本开始支持 token 认证机制，token 认证机制与 Basic 认证机制类似，区别在于 token 认证机制使用的授权码 `<TOKEN>` 是由 TDengine TSDB 服务器端生成并分发给客户端的，而不是由客户端通过 Base64 编码生成的。

HTTP 请求的 Header 里需带有身份认证信息，Token 认证信息如下所示：

```text
Authorization: Bearer <TOKEN>
```

TOKEN 使用 SQL 语句 `CREATE TOKEN` 创建，具体见 [令牌管理](../../04-tdengine-sql/07-user-and-privilege/01-user.md#令牌管理)。
