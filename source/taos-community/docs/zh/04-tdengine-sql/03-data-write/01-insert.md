...
通过查询系统表 `INS_DISK_USAGE`，可以查看数据库的压缩率和磁盘空间，参见 [文档](../02-ddl/01-database.md#查看数据库的磁盘空间占用)。

```sql
select * from INFORMATION_SCHEMA.INS_DISK_USAGE where db_name = 'db_name';
```

通过如下命令，可以查看表的表的压缩率及具体分布情况，参见 [文档](../09-system-info/03-show.md#show-table-distributed)。
