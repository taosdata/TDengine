...
- 说明：RPC 收发数据线程数目
- 类型：整数
- 默认值：CPU 核数的一半（Linux/macOS 限制在 [2, 100] 范围内，Windows 限制在 [2, 4] 范围内）
- 最小值：1
- 最大值：1024
- 动态修改：不支持
- 支持版本：从 v3.0.0.0 版本开始引入
...
- 默认值：0
- 动态修改：不支持
- 支持版本：`≥ v3.3.6.32` 且 `< v3.3.7.0`，或 `≥ v3.3.8.12`
- 注意事项：`v3.3.6.*` 系列版本仅支持模式 0 和 1

#### wsTlsVersion

- 说明：TLS 协议版本列表，多个版本使用英文逗号分隔。
- 类型：字符串
...
- 说明：客户端用于验证服务端证书的 CA 证书文件路径或 PEM 格式的证书内容。该证书应为签发服务端证书的 CA 证书。
- 类型：字符串
- 动态修改：不支持
- 支持版本：`≥ v3.3.8.12`
...
请参考[连接器](../../09-developer-guide/08-connectors-reference/index.md)
