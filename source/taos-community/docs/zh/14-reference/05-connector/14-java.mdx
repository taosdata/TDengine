...
:::warning 原生连接和 REST 连接即将废弃

<font color="red">Java 的原生连接和 REST 连接将于 2027-01-01 下线</font>，请迁移到 WebSocket 连接。
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| 3.8.3                | 修复数据订阅令牌认证错误地要求用户名的 bug                                                                                                                                                                                                                               | -                  |
| 3.8.2                | 数据订阅支持令牌                                                                                                                                                                                                                               | -                  |
| 3.8.1                | Decimal 和 Blob 类型支持参数绑定写入                                                                                                                                                                                                                               | -                  |
| 3.8.0                | 1. 负载均衡采用最小连接数算法 <br/> 2. 支持故障节点恢复后的连接数重平衡 <br/> 3. 支持上报连接器类型和版本 <br/> 4. 支持令牌认证方式 <br/> 5. 解决了参数绑定带子表名时纳秒精度问题 <br/> 6. 日志脱敏优化                                                                                                                                                                                                                                      | -                  |
...
| VARBINARY              | byte[]               |                                                                                |
| GEOMETRY               | byte[]               |                                                                                |
| BLOB                   | byte[]               | 仅在 WebSocket 连接的列中支持                                        |
| DECIMAL                | java.math.BigDecimal | 仅在 WebSocket 连接的列中支持                                          |
...
对于 java 连接器，可以使用 jts 库来方便的创建 GEOMETRY 类型对象，序列化后写入 TDengine TSDB，这里有一个样例[Geometry 示例](https://github.com/taosdata/TDengine/blob/main/docs/examples/JDBC/JDBCDemo/src/main/java/com/taos/example/GeometryDemo.java)  
...
6. 如何从 REST 连接迁移到 WebSocket 连接？

**背景**：从 taos-jdbcdriver 3.4.0 版本开始，推荐使用 WebSocket 连接替代 REST 连接。

**迁移步骤**：  
  a. 将驱动类从 `com.taosdata.jdbc.rs.RestfulDriver` 修改为 `com.taosdata.jdbc.ws.WebSocketDriver`。  
  b. 将 JDBC URL 从 `jdbc:TAOS-RS://` 修改为 `jdbc:TAOS-WS://`。  
  c. 移除 `batchfetch=true` 参数，该参数已经不需要。  
  d. 测试验证功能正常。  

**迁移优势**：WebSocket 连接延迟更低、性能更好、支持自动重连和更丰富的功能。
...
- conmode：BI 模式，仅在 WebSocket 连接时生效，默认 0，可设置为 1。为 1 代表开启 BI 模式，元数据信息不统计子表，主要用在 BI 工具对接场景。
...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]：仅在使用 JDBC WebSocket 连接时生效，令牌信息。其认证优先级高于用户名密码。
