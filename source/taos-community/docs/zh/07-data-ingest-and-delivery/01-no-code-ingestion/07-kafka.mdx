...
本节讲述如何通过 Explorer 界面创建数据迁移任务，从 Kafka 迁移数据到当前 TDengine TSDB 集群。

## 功能概述

Apache Kafka 是一个用于流处理、实时数据管道和大规模数据集成的开源分布式流系统。

TDengine TSDB 可以高效地从 Kafka 读取数据并将其写入 TDengine TSDB，以实现历史数据迁移或实时数据流入库。
...
![kafka-04-sasl-plain.png](assets/kafka-04-sasl-plain.png)

#### 4.2. SCRAM(SCRAM-SHA-256) 认证

选择 `SCRAM-SHA-256` 认证机制，输入用户名和密码：

![kafka-04.png](assets/kafka-04.png)

#### 4.3. GSSAPI 认证

选择 `GSSAPI` ，将通过 [RDkafka 客户端](https://github.com/confluentinc/librdkafka) 调用 GSSAPI 应用 Kerberos 认证机制：

![kafka-04-sasl-gssapi.png](assets/kafka-04-sasl-gssapi.png)
...
![kafka-05.png](assets/kafka-05.png)
...
在 **Offset** 的下拉列表中选择从哪个 Offset 开始消费数据。有三个选项：`Earliest`、`Latest`。默认值为 Earliest。
...
![kafka-06.png](assets/kafka-06.png)

### 7. 配置 Payload 解析

在 **Payload 解析** 区域填写 Payload 解析相关的配置参数。

Kafka 支持针对不同的数据结构使用不同的解析规则，在对 payload 进行解析之后，添加多个规则块对不同的数据进行不同的解析。

1. 匹配表达式用于判断消息是否匹配当前规则块，填写规则同 `过滤` 规则，默认为 true，表示直接匹配
2. 从列中提取或拆分 / 过滤 / 映射 块的填写规则保持不变
3. 规则块可以添加/删除，但受限于表字段大小限制，不可无限添加

![kafka-multi-datastructures](assets/kafka-multi-datastructures.png)
...
![kafka-07.png](assets/kafka-07.png)

点击 **放大镜图标**  可查看预览解析结果。

![kafka-08.png](assets/kafka-08.png)
...
![kafka-09.png](assets/kafka-09.png)

点击 **放大镜图标** 可查看预览提取/拆分结果。

![kafka-10.png](assets/kafka-10.png)
...
![kafka-11.png](assets/kafka-11.png)

点击 **放大镜图标** 可查看预览过滤结果。

![kafka-12.png](assets/kafka-12.png)
...
![kafka-13.png](assets/kafka-13.png)

点击 **预览**，可以查看映射的结果。

![kafka-14.png](assets/kafka-14.png)
...
import Contributing from './resources/_03-exception-handling-strategy.mdx'

<Contributing />

### 10. 创建完成

点击 **提交** 按钮，完成创建 Kafka 到 TDengine TSDB 的数据同步任务，回到**数据源列表**页面可查看任务执行情况。
