...
- `TRUE_FOR(...)` 指定窗口过滤条件，以及（仅事件窗口）开窗/关窗连续满足门限。支持基于持续时间、记录条数或两者组合过滤，也可用 `start(...)` 指定开窗连续满足条件、`end(...)` 指定关窗连续满足条件。详见 [事件窗口 TRUE_FOR 参数](../14-reference/03-taos-sql/24-distinguished.md#事件窗口)。
