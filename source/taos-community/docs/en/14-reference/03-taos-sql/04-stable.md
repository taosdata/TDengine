...
- `ENCODE` and `COMPRESS`: See [Data Compression](12-compress.md).

- `COMMENT` and `SMA`: See [Tables](03-table.md).

- `KEEP`: See [Databases](02-database.md) for details. However, note the following differences between supertable-level `KEEP` and database-level `KEEP`:
