...
class TestAggFunction:
    ...
    def initdabase(self):
        ...
        sql = 'create table '
        sql += ' t1 using stb tags(1)'
        sql += ' t2 using stb tags(2)'
        sql += ' t3 using stb tags(3)'
        sql += ' t4 using stb tags(4)'
        sql += ' t5 using stb tags(5)'
        sql += ' t6 using stb tags(6)'
        tdSql.execute(sql)

    def insert_data(self):
        sql = "insert into "
        for i in range(5000):
            sql += f" t1 values({self.ts + i * 1000}, {i%5})"
            sql += f" t2 values({self.ts + i * 1000}, {i%5})"
            sql += f" t3 values({self.ts + i * 1000}, {i%5})"
            if i == 2500:
                tdSql.execute(sql)
                sql = "insert into "
        tdSql.execute(sql)        
