...
if [ $1 -eq 1 ];then
	docker ps | grep $2
        ...
        docker run -itd --name $2 -h $2 -e TaosadapterIp=$3 -e TaosadapterPort=$4 tcollector:v1 /bin/bash
else
		...
		docker ps | grep $perfix$i
                ...
                docker run -itd --name $perfix$i -h $perfix$i -e TaosadapterIp=$3 -e TaosadapterPort=$4 tcollector:v1 /bin/bash
        done
fi
#docker run -itd --name tcollector_agent1 -h tcollector_agent1 -e TaosadapterIp=172.26.10.86 -e TaosadapterPort=6049  tcollector:v1 /bin/bash
