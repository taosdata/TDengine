...
if [ $1 -eq 1 ];then
	docker ps | grep $2
        if [ $? -eq 0 ];then
       	...
       	docker run -itd --name $2 -h $2 -e TaosadapterIp=$3 -e TaosadapterPort=$4 -e TelegrafInterval=$5 telegraf:v1 /bin/bash
else
		...
		if [ $? -eq 0 ];then
                ...
                docker run -itd --name $perfix$i -h $perfix$i -e TaosadapterIp=$3 -e TaosadapterPort=$4 -e TelegrafInterval=$5 -e Dbname=$6 telegraf:v1 /bin/bash
        done
fi

#docker run -itd --name telegraf_agent1 -h telegraf_agent1 -e TaosadapterIp=172.26.10.86 -e TaosadapterPort=6041 -e TelegrafInterval=1s -e Dbname=telegraf telegraf:v1 /bin/bash
