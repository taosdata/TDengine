...
//    private static ConnectionFactory connectionFactory = new ConnectionFactory();
    ...
    public TSDataGenerator() {
    }

    public static void main(String[] args) {
        Timer mainTimer = new Timer();
        mainTimer.start();
        TSDataGenerator generator = new TSDataGenerator();
    ...
    private void createAllTables() {
        ...
        try {
            ...
            for (int i = 1; i <= tbNum; i++) {
                sql = SqlGenerator.getCreateTableUsingMetricSql(tbPrefix + i, mt, new String[]{String.valueOf(i), String.valueOf(i%5), cities[i%3]});
                stmt.executeUpdate(sql);
            }
            stmt.close();
                ...
                for (int t = 1; t <= tbNum; t++) {
                    ...
                    if ((t - 1) % threadNum + 1 == threadId) {
                        ...
                        for (int i = 0; i < batchNum; i++) {
                            StringBuilder insertSql = new StringBuilder("insert into ").append(tbName).append(" values ");
                            for (int j = 0; j < batchSize; j++) {
                                ts = ts + timestep;
                                insertSql.append("(").append(ts).append(", ");
                                int val = (int) ((Math.sin((double)(j + i * batchSize) * 2 * Math.PI / (double)totalRowsPerTable) + Math.random())* (double)batchSize);
//                                int val = (int) (j + i * batchSize);
                                String colValue = String.valueOf(val);
