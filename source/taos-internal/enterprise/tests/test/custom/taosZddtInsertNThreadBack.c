...
{
  if (argc == 1) {
    printf("usage: %s table_num thread_num loop_num rowsperfile cache ablocks tblocks tables   \n", argv[0]);
    exit(0);
  }
  ...
  if (argc >= 4) loop_num = atoi(argv[3]);
  if (argc >= 5) rowsperfile = atoi(argv[4]);
  if (argc >= 6) cache = atoi(argv[5]);
  if (argc >= 7) ablocks = atof(argv[6]);
  if (argc >= 8) tblocks = atoi(argv[7]);
  if (argc >= 9) tables = atoi(argv[8]);
  
  printf("table_num:%d thread_num:%d loop_num:%d rowsperfile:%d cache:%d ablocks:%f tblocks:%d tables:%d   \n"
	, table_num, thread_num, loop_num, rowsperfile, cache, ablocks, tblocks, tables);
...
void taos_execute(void *param)
{
  ...
  code = taos_query(taos, "create table if not exists db.st (ts timestamp "
    ", radix smallint, total int,FID binary(100), RR_FLAG binary(1), FILE_NAME binary(50), DEAL_TIME binary(14)"
  ...
  //    //  printf("==> thread:%d ts:%d table:%d insert finished\n", pThread->index, taosGetTimestampSec(), i);
  ...
  //  if (pThread->index == 0)
  //    printf("==> thread:%d ts:%d loop:%d insert finished\n", pThread->index, taosGetTimestampSec(), l);
  //}

  for (int i = tableBegin; i < tableEnd; i++) {
    int64_t ts = 1552559907000;
    for (int l = 0; l < loop_num; l+=50) {
      int len = sprintf(sql, "insert into");
      for (int j = 0; j < 50; j++) {
        //len += sprintf(sql + len, " db.t%d using db.st tags(%d) values(%lld, '%d', '%d', '04020118140116.79.234.11220.206.175.12484010602134203847124600176610767983GNET', '1', NULL, '0402013209', '11', NULL, '%d', '460017661076798', '116.79.234.11', NULL, '46868', NULL, '51981', '3420384712', '220.206.175.124', '3GNET', NULL, '01', NULL, NULL, '46001', '22', NULL, '0371', '0371', NULL, '1', NULL, NULL, '0', NULL, '3530500980812906', '20180402', '011814', '0', '840106021', NULL, '0', '40', '89', NULL, NULL, 'X', NULL, '1024', '1|0|3070000|3071101|10;1|7617101383192277|8132453|8132453|0', '0|1|201804|0|0|99997|129;0|1|201804|7617101383192277|0|40001|129;0|7|201804|0|0|10058|129|8132453:10058|0', '0', '10', '0', '0', '0', '0', NULL, '0760000GJSJ0099103900201804020121003azz.16.0.sn', '0', '7614103025031815', '7614103025820540', '99999829', '4G00', '51016', '766480', '0', '20040324114143', NULL, NULL, '002', '002', '0', '1', '0402013209', '744FEA0B', NULL, '1', '1', NULL, '1', NULL, '0', '76', NULL, NULL, '20180402012122')"
        //  , i, i, ts, (l+j) % 1000, l+j);
        len += sprintf(sql + len, " db.t%d values(%lld, '%d', '%d', '04020118140116.79.234.11220.206.175.12484010602134203847124600176610767983GNET', '1', NULL, '0402013209', '11', NULL, '%d', '460017661076798', '116.79.234.11', NULL, '46868', NULL, '51981', '3420384712', '220.206.175.124', '3GNET', NULL, '01', NULL, NULL, '46001', '22', NULL, '0371', '0371', NULL, '1', NULL, NULL, '0', NULL, '3530500980812906', '20180402', '011814', '0', '840106021', NULL, '0', '40', '89', NULL, NULL, 'X', NULL, '1024', '1|0|3070000|3071101|10;1|7617101383192277|8132453|8132453|0', '0|1|201804|0|0|99997|129;0|1|201804|7617101383192277|0|40001|129;0|7|201804|0|0|10058|129|8132453:10058|0', '0', '10', '0', '0', '0', '0', NULL, '0760000GJSJ0099103900201804020121003azz.16.0.sn', '0', '7614103025031815', '7614103025820540', '99999829', '4G00', '51016', '766480', '0', '20040324114143', NULL, NULL, '002', '002', '0', '1', '0402013209', '744FEA0B', NULL, '1', '1', NULL, '1', NULL, '0', '76', NULL, NULL, '20180402012122')"
          , i, ts, (l + j) % 1000, l + j);
        ts += 1000;
      }
	  ...
	  for (int i = 0; i < 100; ++i) {
		...
		fprintf(stderr, "thread:%d ts:%d TDengine error, code:%d reason:%s\n", pThread->index, taosGetTimestampSec(), code, taos_errstr(taos));
		taos_query(taos, "alter dnode 192.168.1.116 debugflag 135");
		cdebugFlag = 135;
		uDebugFlag = 135;
		rpcDebugFlag = 135;
		taosMsleep(100);
    ...
    if (pThread->index == 0)
      printf("==> thread:%d ts:%d table:%d rows:%d insert finished\n", pThread->index, taosGetTimestampSec(), i, loop_num);
  }

  int64_t end = taosGetTimestampMs();

  printf("\n thread:%d time spent: %d seconds\n   insert avg tables %f per second\n ", pThread->index, (end - start) / 1000, (float)table_num * loop_num / ((end - start) / 1000));
