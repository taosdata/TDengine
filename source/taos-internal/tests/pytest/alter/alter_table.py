...
class TDTestCase:
    def init(self, conn):
        ...
        self.types = ["int", "bigint", "float", "double", "smallint", "tinyint", "binary(10)", "nchar(10)", "timestamp"]
        self.rowNum = 300
        self.ts = 1537146000000L
        self.step = 1000L
    ...
    def addColumnAndCount(self):
        for colIdx in range(len(self.types)):
            tdSql.execute("alter table tb add column c%d %s" %(colIdx + 2, self.types[colIdx]))
    ...
    def run(self):
        ...
        # Alter tb and add a column of smallint type, then query tb to see if all added column are NULL
