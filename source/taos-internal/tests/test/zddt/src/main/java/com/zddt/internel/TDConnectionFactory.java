...
public class TDConnectionFactory {
    ...
    public static Connection getConnection() {
        ...
        try {
            ...
            TDLog.error(String.format("%s: failed to connect to %s", Thread.currentThread().getName(), DEFAULT_HOST));
            e.printStackTrace();
        } catch (Error error) {
            TDLog.error(String.format("%s: failed to connect to %s", Thread.currentThread().getName(), DEFAULT_HOST));
    ...
    public static Connection getConnection(Properties properties) {
        ...
        try {
            ...
            if (properties == null) {
                ...
                if (properties.getProperty(TSDBDriver.PROPERTY_KEY_USER) == null || properties.getProperty(TSDBDriver.PROPERTY_KEY_USER) == "") {
                    properties.setProperty(TSDBDriver.PROPERTY_KEY_USER, DEFAULT_USER);
                }
                if (properties.getProperty(TSDBDriver.PROPERTY_KEY_PASSWORD) == null || properties.getProperty(TSDBDriver.PROPERTY_KEY_PASSWORD) == "") {
                    properties.setProperty(TSDBDriver.PROPERTY_KEY_PASSWORD, DEFAULT_PASSWORD);
                }
                if (properties.getProperty(TSDBDriver.PROPERTY_KEY_PORT) == null || properties.getProperty(TSDBDriver.PROPERTY_KEY_PORT) == "") {
                ...
                TDLog.print(String.format("%s: connected to host: %s", Thread.currentThread().getName(), DEFAULT_HOST));
            }
     ...
     */
    public static Connection getConnection(String host, Properties properties) {
        ...
        try {
            ...
            TDLog.error(String.format("%s: failed to connect to %s", Thread.currentThread().getName(), host));
            e.printStackTrace();
        } catch (Error error) {
            TDLog.error(String.format("%s: failed to connect to %s", Thread.currentThread().getName(), host));
            error.printStackTrace();
    ...
    public static Connection getConnection(String host, String user, String password) {
        ...
        pty.setProperty("user", user);
        pty.setProperty("password", password);
        return getConnection(host, pty);
    }
}