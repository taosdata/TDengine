...
static void testQueryStr(SSchema *schema, int32_t numOfCols, char *sql, tSkipList *pSkipList,
                         ResultObj *expectedVal);
...
int32_t main(int32_t argc, char **argv) {
    const int32_t numOfCols = 8;

    SSchema schema[8] = {0};
    {
        initSchema(schema, numOfCols);

        tSkipList *pSkipList = createSkipList(schema, numOfCols);
        tSkipListPrint(pSkipList, 1);

        Left2RightTest(schema, numOfCols, pSkipList);
        Right2LeftTest(schema, numOfCols, pSkipList);
        IllegalExprTest(schema, numOfCols, pSkipList);
...

    {
        ...
        initSchema_binary(schema, numOfCols);

        tSkipList *pSkipList = createSkipList_binary(schema, numOfCols);
        tSkipListPrint(pSkipList, 1);

        Left2RightTest_binary(schema, numOfCols, pSkipList);
        Right2LeftTest_binary(schema, numOfCols, pSkipList);
//        IllegalExprTest_binary(schema, numOfCols, pSkipList);
...
static void testQueryStr(SSchema *schema, int32_t numOfCols, char *sql, tSkipList *pSkipList,
 ...
 /*   tSQLBinaryExpr *pExpr = NULL;
    ...
    tSQLBinaryExprTraverse(pExpr, pSkipList, schema, numOfCols, tSkipListNodeFilterCallback, &result);
    printf("the result is:%lld\n", result.num);

    assert(result.num == pResult->numOfResult);
