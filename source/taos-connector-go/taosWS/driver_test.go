...
var (
	...
	dataSourceNameWithParisTimezone    = fmt.Sprintf("%s:%s@ws(%s:%d)/?timezone=Europe%%2FParis", user, password, host, port)
	dataSourceNameWithShanghaiTimezone = fmt.Sprintf("%s:%s@ws(%s:%d)/?timezone=Asia%%2FShanghai", user, password, host, port)
)
...
func TestChinese(t *testing.T) {
	...
	defer func() {
		_, err = exec(db, "drop database if exists test_chinese_ws")
		if err != nil {
			t.Error(err)
			return
		}
	}()
	_, err = exec(db, "create database if not exists test_chinese_ws")
	if err != nil {
		t.Error(err)
		return
	}
	_, err = exec(db, "drop table if exists test_chinese_ws.chinese")
	if err != nil {
		t.Error(err)
		return
	}
	_, err = exec(db, "create table if not exists test_chinese_ws.chinese(ts timestamp,v nchar(32))")
	if err != nil {
		t.Error(err)
		return
	}
	_, err = exec(db, `INSERT INTO test_chinese_ws.chinese (ts, v) VALUES (?, ?)`, "1641010332000", "'阴天'")
	if err != nil {
		t.Error(err)
		return
	}
