...
int32_t tTagNewWithName(SArray *pTagVals, SArray *pTagName, const SSchema *pSchema, int32_t numOfTags,
  ...
  if (code != TSDB_CODE_SUCCESS) {
    // Align failed after STag was successfully built. Free the pre-built
    // STag here so callers can simply `goto end` (or return) on error
    // without leaking it, since they have no other handle to *ppTag.
    uDebug("tTagNewWithName: align tagName by cid failed, free pre-built STag, code:0x%x", code);
    tTagFree(*ppTag);
    *ppTag = NULL;
  }
  return code;
