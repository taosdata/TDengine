...
static int32_t tColDataMerge(SArray **colArr) {
  ...
  for (int32_t i = 0; i < taosArrayGetSize(src); i++) {
    ...
    tColDataInit(dstCol, srcCol->cid, srcCol->type, 0);
  }

  SColData *tsColData = (SColData *)TARRAY_DATA(src);
  int32_t   numRows = ((SColData *)TARRAY_DATA(src))->nVal;
  TSKEY     lastKey = 0;
  for (int32_t i = 0; i < numRows; i++) {
    TSKEY key = ((TSKEY *)tsColData[0].pData)[i];

    if (i == 0 || key != lastKey) {  // append new row
