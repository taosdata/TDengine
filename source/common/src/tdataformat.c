...
int tdEncodeSchema(void **buf, STSchema *pSchema) {
  ...
  for (int i = 0; i < schemaNCols(pSchema); i++) {
    ...
    tlen += taosEncodeFixedI8(buf, colFlag(pCol));
...
void *tdDecodeSchema(void *buf, STSchema **pRSchema) {
  ...
  for (int i = 0; i < numOfCols; i++) {
    ...
    buf = taosDecodeFixedI8(buf, &flag);
    buf = taosDecodeFixedI16(buf, &colId);
    buf = taosDecodeFixedI32(buf, &bytes);
    if (tdAddColToSchema(&schemaBuilder, type, flag, colId, bytes) < 0) {
...
int32_t tdAddColToSchema(STSchemaBuilder *pBuilder, int8_t type, int8_t flag, col_id_t colId, col_bytes_t bytes) {
