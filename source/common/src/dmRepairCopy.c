...
// Internal command execution for repair SSH/SCP commands.
...
static int64_t dmGetsCmd(DmCmdPtr p, int32_t maxSize, char *buf) {
  if (p == NULL) return -1;
  if (fgets(buf, maxSize, p) == NULL) return -1;
...
typedef struct SRepairFile {
  ...
  int32_t lcn;       // last chunk number (shared storage)
  int32_t fid;       // file set id
  int32_t mid;       // migration id (shared storage)
...
static int32_t dmParseSourceCfg(const char *cfgPath, SDiskCfg **ppDisks, int32_t *pNumDisks) {
  ...
  code = cfgAddDir(pCfg, "dataDir", "/tmp", CFG_SCOPE_SERVER, CFG_DYN_NONE, CFG_CATEGORY_LOCAL, 0);
  if (code != 0) {
...
static int32_t dmParseRepairFileJson(SJson *pJson, int32_t type, SRepairFile *pFile) {
  ...
  pFile->mid = 0;
  tjsonGetInt32ValueFromDouble(pJson, "mid", pFile->mid, code);
...
// Shared storage variant (mid!=0): ...ver{cid}.m{mid}.{suffix}
// Last chunk variant (lcn>0): ...ver{cid}.{lcn}.{suffix}
// Both (mid!=0 && lcn>0): ...ver{cid}.m{mid}.{lcn}.{suffix}
static void dmBuildTsdbFilePath(const char *diskPath, int32_t vnodeId, const SRepairFile *pFile, char *buf,
  ...
  if (pFile->lcn > 0) {
    if (pFile->mid != 0) {
      snprintf(buf, bufLen, "%s%svnode%svnode%d%stsdb%sv%df%dver%" PRId64 ".m%d.%d.%s", diskPath, TD_DIRSEP, TD_DIRSEP,
               vnodeId, TD_DIRSEP, TD_DIRSEP, vnodeId, pFile->fid, pFile->cid, pFile->mid, pFile->lcn, suffix);
    } else {
    ...
    if (pFile->mid != 0) {
      snprintf(buf, bufLen, "%s%svnode%svnode%d%stsdb%sv%df%dver%" PRId64 ".m%d.%s", diskPath, TD_DIRSEP, TD_DIRSEP,
               vnodeId, TD_DIRSEP, TD_DIRSEP, vnodeId, pFile->fid, pFile->cid, pFile->mid, suffix);
    } else {
...
static int32_t dmHardLinkRetainedFiles(STfs *pTgtTfs, int32_t vnodeId, const SArray *retainFids,
  ...
  for (int32_t r = 0; r < nRetain; r++) {
    ...
    for (int32_t f = 0; f < nFiles; f++) {
      ...
      if (pFile->lcn > 0) {
        if (pFile->mid != 0) {
          snprintf(fileName, sizeof(fileName), "v%df%dver%" PRId64 ".m%d.%d.%s", vnodeId, pFile->fid, pFile->cid,
                   pFile->mid, pFile->lcn, suffix);
        } else {
          snprintf(fileName, sizeof(fileName), "v%df%dver%" PRId64 ".%d.%s", vnodeId, pFile->fid, pFile->cid,
                   pFile->lcn, suffix);
        }
      } else {
        if (pFile->mid != 0) {
          snprintf(fileName, sizeof(fileName), "v%df%dver%" PRId64 ".m%d.%s", vnodeId, pFile->fid, pFile->cid,
                   pFile->mid, suffix);
        } else {
...
static int32_t dmCopyNonTsdbFiles(const SRepairTfs *pSrcTfs, STfs *pTgtTfs, const char *host, int32_t vnodeId) {
  ...
  char line[512];
  while (dmGetsCmd(pCmd, sizeof(line), line) > 0) {
...
static int64_t dmGetRemoteFileSize(const char *host, const char *remotePath) {
  ...
  int64_t size = -1;
  char    buf[64] = {0};
  if (dmGetsCmd(pCmd, sizeof(buf), buf) > 0) {
...
static int32_t dmGenerateCurrentJson(STfs *pTgtTfs, int32_t vnodeId, const SArray *retainFids,
  ...
  for (int32_t i = 0; i < totalSets; i++) {
    ...
    for (int32_t t = 0; t < 4; t++) {
      ...
      (void)tjsonAddDoubleToObject(pFileJson, "mid", pFile->mid);
...

    {
      ...
      for (int32_t sl = 0; sl < nSttLevels; sl++) {
        ...
        for (int32_t f = 0; f < nFiles; f++) {
          ...
          (void)tjsonAddDoubleToObject(pSttJson, "mid", pf->mid);
...
int32_t dmRepairCopyMode(const SRepairCopyOpt *pOpts) {
  ...
  if (isRemote) {
    ...
    if (dmSshFetchFile(pOpts->sourceHost, resolvedCfg, tmpCfgPath, sizeof(tmpCfgPath)) != 0) {
      ...
      if (dmSshFetchFile(pOpts->sourceHost, remoteCfgRetry, tmpCfgPath, sizeof(tmpCfgPath)) != 0) {
  ...
  for (int32_t v = 0; v < nVnodes; v++) {
    ...
    for (int32_t s = 0; s < nSets; s++) {
      ...
      for (int32_t f = 0; f < nFiles; f++) {
        SRepairFile *pf = taosArrayGet(pSet->files, f);
        if (pf->lcn > 1) vnResults[v].hasSsFile = true;
      }
    }
  ...
  bool    hasSsFiles = false;
  int32_t nSuccess = 0, nSkipped = 0, nFailed = 0;
  for (int32_t v = 0; v < nVnodes; v++) {
    int32_t vnodeId = *(int32_t *)taosArrayGet(pOpts->vnodeIds, v);
    if (vnResults[v].hasSsFile) hasSsFiles = true;
    if (vnResults[v].result == 1) {
      nFailed++;
  ...
  if (hasSsFiles) {
    uInfo("repair: INFO — some source files have lcn > 1 (shared storage multi-chunk).");
  }
  uInfo("repair: ================================");
