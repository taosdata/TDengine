...
int32_t buildCtbNameAddGroupId(const char* stbName, char* ctbName, uint64_t groupId, size_t cap) {
  ...
  if (prefixLen + suffixLen + 1 >= TSDB_TABLE_NAME_LEN) {
    ...
    tMD5Final(&context);
    suffixLen =
        snprintf(suffix, suffixCap, "%016lx%016lx", *(uint64_t*)context.digest, *(uint64_t*)(context.digest + 8));
    if (suffixLen >= suffixCap) {
      code = TSDB_CODE_INTERNAL_ERROR;
      TSDB_CHECK_CODE(code, lino, _end);
    }
  ...
  if (prefixLen + suffixLen + 1 >= TSDB_TABLE_NAME_LEN) {
    ...
    tMD5Final(&context);
    prefixLen = snprintf(ctbName, cap, "t_%016lx%016lx", *(uint64_t*)context.digest, *(uint64_t*)(context.digest + 8));
    if (prefixLen >= cap) {
      code = TSDB_CODE_INTERNAL_ERROR;
      TSDB_CHECK_CODE(code, lino, _end);
    }
