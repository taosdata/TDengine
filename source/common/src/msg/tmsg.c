...
static int32_t tEncodeSSubmitTbData(SEncoder *pCoder, const SSubmitTbData *pSubmitTbData) {
  ...
  if (pSubmitTbData->flags & SUBMIT_REQ_COLUMN_DATA_FORMAT) {
    ...
    uError("encode %d row data", nColData);
    ...
    uError("encode %d row data", TARRAY_SIZE(pSubmitTbData->aRowP));
    ...
    uError("encode row data size %d", TARRAY_SIZE(pSubmitTbData->aRowP));
  }
...
static int32_t tDecodeSSubmitTbData(SDecoder *pCoder, SSubmitTbData *pSubmitTbData, void* rawData) {
  ...
  if (pSubmitTbData->flags & SUBMIT_REQ_COLUMN_DATA_FORMAT) {
    ...
    uError("decode row data size %d", TARRAY_SIZE(pSubmitTbData->aRowP));
  }
