...
#define REPAIR_COPY_SSH_BIN_ENV    "TAOS_REPAIR_SSH_BIN"
#define REPAIR_COPY_SCP_BIN_ENV    "TAOS_REPAIR_SCP_BIN"
...
static bool tRepairValidateReplicaNodeEndpoint(const char *endpoint) {
  ...
  const char *sep = strchr(endpoint, ':');
  if (sep == NULL || sep == endpoint || sep[1] == '\0') {
    return false;
  }
...
static const char *tRepairResolveCopyBin(const char *envName, const char *defaultBin) {
  if (envName == NULL || envName[0] == '\0' || defaultBin == NULL || defaultBin[0] == '\0') {
    return NULL;
  }
...
static void tRepairTrimLineEnding(char *line) {
  if (line == NULL) {
    return;
  }
...
static int32_t tRepairRunShellCommand(const char *cmd, char *lastOutput, int32_t lastOutputSize) {
  if (cmd == NULL || cmd[0] == '\0' || (lastOutput != NULL && lastOutputSize <= 0) ||
      (lastOutput == NULL && lastOutputSize > 0)) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  char wrappedCmd[PATH_MAX * 8] = {0};
  int32_t wrappedLen =
      tsnprintf(wrappedCmd, sizeof(wrappedCmd), "%s 2>&1; printf '\\n%s%%d\\n' $?", cmd, REPAIR_COPY_CMD_EXIT_MARKER);
  if (wrappedLen <= 0 || wrappedLen >= (int32_t)sizeof(wrappedCmd)) {
    return TSDB_CODE_INVALID_PARA;
  }

  TdCmdPtr pCmd = taosOpenCmd(wrappedCmd);
  if (pCmd == NULL) {
    return terrno != 0 ? terrno : TSDB_CODE_FAILED;
  }

  int32_t code = TSDB_CODE_SUCCESS;
  int32_t exitCode = INT32_MIN;
  char    line[1024] = {0};
  int32_t markerLen = strlen(REPAIR_COPY_CMD_EXIT_MARKER);
  while (true) {
    ...
    tRepairTrimLineEnding(line);
    if (strncmp(line, REPAIR_COPY_CMD_EXIT_MARKER, markerLen) == 0) {
      errno = 0;
      char   *end = NULL;
      int32_t parsed = taosStr2Int32(line + markerLen, &end, 10);
      if (errno == 0 && end != NULL && end != line + markerLen && *end == '\0') {
        exitCode = parsed;
      } else {
    ...
    if (lastOutput != NULL && line[0] != '\0') {
      tstrncpy(lastOutput, line, lastOutputSize);
    }
  }
...
static int32_t tRepairCountRegularFilesRecursive(const char *dirPath, int32_t *pFiles) {
  if (dirPath == NULL || dirPath[0] == '\0' || pFiles == NULL) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  while ((pDirEntry = taosReadDir(pDir)) != NULL) {
    ...
    if (taosDirEntryIsDir(pDirEntry)) {
      ...
      code = tRepairCountRegularFilesRecursive(entryPath, pFiles);
      if (code != TSDB_CODE_SUCCESS) {
        break;
      }
      continue;
...
static int32_t tRepairCreateEmptyFile(const char *filePath) {
...
int32_t tRepairParseReplicaNodeEndpoint(const char *endpoint, char *host, int32_t hostSize, char *remoteDataDir,
  ...
  if (endpoint == NULL || host == NULL || hostSize <= 0 || remoteDataDir == NULL || remoteDataDirSize <= 0) {
    return TSDB_CODE_INVALID_PARA;
  }

  if (!tRepairValidateReplicaNodeEndpoint(endpoint)) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  if (pathLen <= 0 || pathLen >= remoteDataDirSize) {
    return TSDB_CODE_INVALID_PARA;
  }

  memcpy(host, endpoint, hostLen);
  host[hostLen] = '\0';
  tstrncpy(remoteDataDir, sep + 1, remoteDataDirSize);
  return TSDB_CODE_SUCCESS;
}
...
int32_t tRepairValidateCliArgs(const SRepairCliArgs *pCliArgs) {
  ...
  if (pCliArgs->mode == REPAIR_MODE_COPY) {
    ...
    if (!tRepairValidateReplicaNodeEndpoint(pCliArgs->replicaNode)) {
      return TSDB_CODE_INVALID_PARA;
    }
...
int32_t tRepairNeedRunReplicaRepair(const SRepairCtx *pCtx, bool *pNeedRun) {
  ...
  *pNeedRun = pCtx->nodeType == REPAIR_NODE_TYPE_VNODE && pCtx->mode == REPAIR_MODE_REPLICA;
  return TSDB_CODE_SUCCESS;
}
...
int32_t tRepairNeedRunCopyRepair(const SRepairCtx *pCtx, bool *pNeedRun) {
  ...
  *pNeedRun = pCtx->nodeType == REPAIR_NODE_TYPE_VNODE && pCtx->mode == REPAIR_MODE_COPY;
  return TSDB_CODE_SUCCESS;
}
...
int32_t tRepairBuildCopySshProbeCmd(const char *replicaHost, const char *remoteTargetPath, char *cmd, int32_t cmdSize) {
  ...
  int32_t len = tsnprintf(cmd, cmdSize, "%s -o BatchMode=yes -o StrictHostKeyChecking=no %s \"test -d '%s'\"",
...
int32_t tRepairBuildCopyScpCmd(const char *replicaHost, const char *remoteTargetPath, const char *localTargetPath,
  ...
  const char *scpBin = tRepairResolveCopyBin(REPAIR_COPY_SCP_BIN_ENV, REPAIR_COPY_DEFAULT_SCP);
  if (scpBin == NULL) {
    return TSDB_CODE_INVALID_PARA;
  }

  int32_t len = tsnprintf(
      cmd, cmdSize, "%s -q -o BatchMode=yes -o StrictHostKeyChecking=no -r %s:'%s/.' '%s'", scpBin, replicaHost,
      remoteTargetPath, localTargetPath);
  if (len <= 0 || len >= cmdSize) {
    return TSDB_CODE_INVALID_PARA;
  }
...
int32_t tRepairRollbackReplicaVnode(const SRepairCtx *pCtx, const char *dataDir, int32_t vnodeId) {
  ...
  int32_t markerPathLen =
      tsnprintf(markerPath, sizeof(markerPath), "%s%s%s", vnodeDir, TD_DIRSEP, "replica.degrade.marker.json");
  if (markerPathLen <= 0 || markerPathLen >= (int32_t)sizeof(markerPath)) {
    return TSDB_CODE_INVALID_PARA;
  }

  if (!taosCheckExistFile(markerPath)) {
    return TSDB_CODE_SUCCESS;
  }

  if (taosRemoveFile(markerPath) != 0) {
    return terrno != 0 ? terrno : TSDB_CODE_INVALID_PARA;
  }
...
int32_t tRepairWriteReplicaRestoreHint(const SRepairCtx *pCtx, const char *dataDir, char *hintPath,
  ...
  if (pCtx == NULL || !pCtx->enabled || dataDir == NULL || dataDir[0] == '\0' || hintPath == NULL ||
      hintPathSize <= 0) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  int32_t code = tRepairBuildSessionDir(pCtx, dataDir, sessionDir, sizeof(sessionDir));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }

  if (taosMulMkDir(sessionDir) != 0 && !taosDirExist(sessionDir)) {
    return terrno != 0 ? terrno : TSDB_CODE_INVALID_PARA;
  }

  int32_t hintPathLen = tsnprintf(hintPath, hintPathSize, "%s%s%s", sessionDir, TD_DIRSEP, "replica.restore.hint.json");
  if (hintPathLen <= 0 || hintPathLen >= hintPathSize) {
    return TSDB_CODE_INVALID_PARA;
  }

  char vnodeIds[PATH_MAX] = {0};
  if (pCtx->hasVnodeIdList && pCtx->vnodeIdList[0] != '\0') {
    tstrncpy(vnodeIds, pCtx->vnodeIdList, sizeof(vnodeIds));
  } else {
    int32_t offset = 0;
    for (int32_t i = 0; i < pCtx->vnodeIdNum; ++i) {
      int32_t left = (int32_t)sizeof(vnodeIds) - offset;
      if (left <= 0) {
        return TSDB_CODE_INVALID_PARA;
      }
      int32_t append = tsnprintf(vnodeIds + offset, left, i == 0 ? "%d" : ",%d", pCtx->vnodeIds[i]);
      if (append <= 0 || append >= left) {
        return TSDB_CODE_INVALID_PARA;
      }
      offset += append;
    }
  ...
  char hintContent[2048] = {0};
  int32_t hintContentLen = tsnprintf(
      hintContent, sizeof(hintContent),
      "{\"mnodeMsgType\":\"TDMT_MND_RESTORE_DNODE\",\"restoreType\":\"RESTORE_TYPE__VNODE\","
      "\"vgroupAction\":\"mndBuildRestoreAlterVgroupAction\","
      "\"restoreSqlHint\":\"RESTORE VNODE ON DNODE <dnode-id>\",\"sessionId\":\"%s\",\"vnodeIds\":\"%s\","
      "\"updatedAtMs\":%" PRId64 "}",
      pCtx->sessionId, vnodeIds, taosGetTimestampMs());
  if (hintContentLen <= 0 || hintContentLen >= (int32_t)sizeof(hintContent)) {
    return TSDB_CODE_INVALID_PARA;
  }

  return tRepairWriteFileAtomically(hintPath, hintContent, hintContentLen);
}
...
int32_t tRepairBuildMetaMissingFileMark(const SRepairMetaScanResult *pResult, char *mark, int32_t markSize) {
  if (pResult == NULL || mark == NULL || markSize <= 0) {
    return TSDB_CODE_INVALID_PARA;
  }

  mark[0] = '\0';
  if (pResult->missingRequiredFiles <= 0) {
    tstrncpy(mark, "none", markSize);
    return TSDB_CODE_SUCCESS;
  }

  int32_t offset = 0;
  for (int32_t i = 0; i < pResult->missingRequiredFiles && i < REPAIR_META_MAX_MISSING_FILES; ++i) {
    if (pResult->missingRequiredFileNames[i][0] == '\0') {
      continue;
    }

    int32_t remain = markSize - offset;
    if (remain <= 0) {
      return TSDB_CODE_INVALID_PARA;
    }

    int32_t written = tsnprintf(mark + offset, remain, "%s%s", offset == 0 ? "" : ",",
                                pResult->missingRequiredFileNames[i]);
  ...
  if (offset == 0) {
    tstrncpy(mark, "unknown", markSize);
  }
  return TSDB_CODE_SUCCESS;
}
...
int32_t tRepairInferMetaFromWalTsdb(const SRepairCtx *pCtx, const char *dataDir, int32_t vnodeId,
  ...
  code = tRepairBuildVnodeTargetPath(dataDir, vnodeId, REPAIR_FILE_TYPE_WAL, walPath, sizeof(walPath));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }
  if (taosDirExist(walPath) && taosIsDir(walPath)) {
    code = tRepairCountRegularFilesRecursive(walPath, &pReport->walEvidenceFiles);
    if (code != TSDB_CODE_SUCCESS) {
      return code;
    }
  }

  SRepairCtx tsdbCtx = *pCtx;
  tsdbCtx.fileType = REPAIR_FILE_TYPE_TSDB;
  SRepairTsdbBlockReport tsdbReport = {0};
  code = tRepairAnalyzeTsdbBlocks(&tsdbCtx, dataDir, vnodeId, &tsdbReport);
  if (code == TSDB_CODE_SUCCESS) {
    pReport->tsdbRecoverableBlocks = tsdbReport.recoverableBlocks;
  }

  if (pReport->walEvidenceFiles > 0) {
    ++pReport->inferredRules;
  }
  if (pReport->tsdbRecoverableBlocks > 0) {
    ++pReport->inferredRules;
  }

  pReport->recoverable = pReport->inferredRules > 0;
  return pReport->recoverable ? TSDB_CODE_SUCCESS : TSDB_CODE_INVALID_PARA;
}
...
int32_t tRepairRebuildMetaFiles(const SRepairCtx *pCtx, const char *dataDir, int32_t vnodeId, const char *outputDir,
  ...
  code = tRepairCopyDirRecursive(targetPath, outputDir);
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }
  ...
  for (int32_t i = 0; i < (int32_t)tListLen(kMetaRequiredFiles); ++i) {
    char filePath[PATH_MAX] = {0};
    code = tRepairBuildPathWithEntry(outputDir, kMetaRequiredFiles[i], filePath, sizeof(filePath));
    if (code != TSDB_CODE_SUCCESS) {
      return code;
    }
    ...
    if (taosCheckExistFile(filePath)) {
      ...
      code = tRepairCreateEmptyFile(filePath);
      if (code != TSDB_CODE_SUCCESS) {
        return code;
      }
  ...
  for (int32_t i = 0; i < (int32_t)tListLen(kMetaOptionalIndexFiles); ++i) {
    char filePath[PATH_MAX] = {0};
    code = tRepairBuildPathWithEntry(outputDir, kMetaOptionalIndexFiles[i], filePath, sizeof(filePath));
    if (code != TSDB_CODE_SUCCESS) {
      return code;
    }
...
int32_t tRepairPrecheck(const SRepairCtx *pCtx, const char *dataDir, int64_t minDiskAvailBytes) {
  ...
  for (int32_t i = 0; i < pCtx->vnodeIdNum; ++i) {
    ...
    if (pCtx->fileType == REPAIR_FILE_TYPE_TSDB) {
      ...
      if (code != TSDB_CODE_SUCCESS) {
        SRepairMetaInferenceReport inferReport = {0};
        int32_t inferCode = tRepairInferMetaFromWalTsdb(pCtx, dataDir, pCtx->vnodeIds[i], &inferReport);
        if (inferCode != TSDB_CODE_SUCCESS) {
          return code;
        }
      }
    }
...
int32_t tRepairMockCopyReplicaVnodeTarget(const SRepairCtx *pCtx, const char *replicaDataDir, const char *localDataDir,
                                          ...
                                          int32_t dstPathSize) {
  if (srcPath != NULL && srcPathSize > 0) {
    srcPath[0] = '\0';
  }
  if (dstPath != NULL && dstPathSize > 0) {
    dstPath[0] = '\0';
  }

  if (pCtx == NULL || !pCtx->enabled || replicaDataDir == NULL || replicaDataDir[0] == '\0' || localDataDir == NULL ||
      localDataDir[0] == '\0' || vnodeId < 0) {
    return TSDB_CODE_INVALID_PARA;
  }

  if (pCtx->nodeType != REPAIR_NODE_TYPE_VNODE || pCtx->mode != REPAIR_MODE_COPY) {
    return TSDB_CODE_INVALID_PARA;
  }

  if ((srcPath != NULL && srcPathSize <= 0) || (srcPath == NULL && srcPathSize > 0) || (dstPath != NULL && dstPathSize <= 0) ||
      (dstPath == NULL && dstPathSize > 0)) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  char replicaTargetPath[PATH_MAX] = {0};
  char localTargetPath[PATH_MAX] = {0};
  code = tRepairBuildVnodeTargetPath(replicaDataDir, vnodeId, pCtx->fileType, replicaTargetPath, sizeof(replicaTargetPath));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }
  code = tRepairBuildVnodeTargetPath(localDataDir, vnodeId, pCtx->fileType, localTargetPath, sizeof(localTargetPath));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }

  if (!taosDirExist(replicaTargetPath) || !taosIsDir(replicaTargetPath)) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  code = tRepairCopyDirRecursive(replicaTargetPath, localTargetPath);
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }

  if (srcPath != NULL) {
    tstrncpy(srcPath, replicaTargetPath, srcPathSize);
  }
  if (dstPath != NULL) {
    tstrncpy(dstPath, localTargetPath, dstPathSize);
  }
  return TSDB_CODE_SUCCESS;
}

int32_t tRepairSshScpCopyReplicaVnodeTarget(const SRepairCtx *pCtx, const char *replicaHost, const char *replicaDataDir,
                                            const char *localDataDir, int32_t vnodeId, char *srcPath, int32_t srcPathSize,
                                            char *dstPath, int32_t dstPathSize) {
  if (srcPath != NULL && srcPathSize > 0) {
    srcPath[0] = '\0';
  }
  if (dstPath != NULL && dstPathSize > 0) {
  ...
  if (pCtx == NULL || !pCtx->enabled || replicaHost == NULL || replicaHost[0] == '\0' || replicaDataDir == NULL ||
      replicaDataDir[0] == '\0' || localDataDir == NULL || localDataDir[0] == '\0' || vnodeId < 0) {
    return TSDB_CODE_INVALID_PARA;
  }
  ...
  char sshProbeCmd[PATH_MAX * 6] = {0};
  code = tRepairBuildCopySshProbeCmd(replicaHost, replicaTargetPath, sshProbeCmd, sizeof(sshProbeCmd));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }

  char lastOutput[PATH_MAX] = {0};
  code = tRepairRunShellCommand(sshProbeCmd, lastOutput, sizeof(lastOutput));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }
  ...
  char scpCmd[PATH_MAX * 6] = {0};
  code = tRepairBuildCopyScpCmd(replicaHost, replicaTargetPath, localTargetPath, scpCmd, sizeof(scpCmd));
  if (code != TSDB_CODE_SUCCESS) {
    return code;
  }
