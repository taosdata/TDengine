...
bool    tsWalPreserveForRestore = false;  // preserve WAL logs for node restore from logs instead of snapshot
...
static int32_t taosAddServerCfg(SConfig *pCfg) {
  ...
  TAOS_CHECK_RETURN(cfgAddBool(pCfg, "walPreserveForRestore", tsWalPreserveForRestore, CFG_SCOPE_SERVER, CFG_DYN_SERVER,CFG_CATEGORY_GLOBAL));
...
static int32_t taosSetServerCfg(SConfig *pCfg) {
  ...
  TAOS_CHECK_GET_CFG_ITEM(pCfg, pItem, "walPreserveForRestore");
  tsWalPreserveForRestore = pItem->bval;
...
static int32_t taosCfgDynamicOptionsForServer(SConfig *pCfg, const char *name) {
  ...
  {  //  'bool/int32_t/int64_t/float/double' variables with general modification function
    ...
    static OptionNameAndVar options[] = {{"audit", &tsEnableAudit},
                                         ...
                                         {"walPreserveForRestore", &tsWalPreserveForRestore}};
