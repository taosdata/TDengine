...
static int32_t taosAddServerCfg(SConfig *pCfg) {
  ...
  TAOS_CHECK_RETURN(cfgAddBool(pCfg, "refPrintStack", tsRefPrintStack, CFG_SCOPE_SERVER, CFG_DYN_SERVER));
...
static int32_t taosSetServerCfg(SConfig *pCfg) {
  ...
  TAOS_CHECK_GET_CFG_ITEM(pCfg, pItem, "refPrintStack");
  tsRefPrintStack = pItem->bval;
  // GRANT_CFG_GET;
  TAOS_RETURN(TSDB_CODE_SUCCESS);
}
...
static int32_t taosCfgDynamicOptionsForServer(SConfig *pCfg, const char *name) {
  ...
  {  //  'bool/int32_t/int64_t/float/double' variables with general modification function
    static OptionNameAndVar debugOptions[] = {
        ...
        {"tsRefPrintStack", &tsRefPrintStack}};
