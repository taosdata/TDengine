...
static int32_t vnodePrepareCreateTb(SVCreateTbReq *pTbReq, char *tbName, int64_t suid, SSchemaWrapper *pTagSchema,
  ...
  /*
      SValue sv = {0};
      sv.type = TSDB_DATA_TYPE_VARCHAR;
      sv.nData = strlen(tbName);
      sv.pData = tbName;
  ...
  */
  ...
  code = tTagNew(pTagVals, 1, false, &pTag);  // TODO dmchen version hardcode
  if (pTagVals != NULL) taosArrayDestroy(pTagVals);
  if (code != TSDB_CODE_SUCCESS) {
...
_exit:
  if (code != 0) {
    ...
    if (TagNames != NULL) taosArrayDestroy(TagNames);
    if (pTagVals != NULL) taosArrayDestroy(pTagVals);
    if (pTag != NULL) tTagFree(pTag);
  }
  return code;
}
...
_exit:
  ...
  // tDestroySubmitReq(pSubmitReq, TSDB_MSG_FLG_DECODE); //TODO dmchen try ci mem leak check
