...
SVnode *vnodeOpen(const char *path, int32_t diskPrimary, STfs *pTfs, SMsgCb msgCb, bool force) {
  ...
  if (tsEnableMonitor) {
    if (taos_collector_registry_get_metric(VNODE_METRIC_SQL_COUNT) == NULL) {
      ...
      if (taos_collector_registry_register_metric(counter) == 1) {
        ...
        vInfo("vgId:%d, failed to registry metric, counter already exist", TD_VID(pVnode));
      }
    } else {
      vInfo("vgId:%d, counter already exist", TD_VID(pVnode));
    }
  }
