...
static void destroyStreamNotifyHandle(void* ptr) {
  ...
  if ((*ppHandle)->curl) {
    ...
    CURLcode res = curl_ws_send((*ppHandle)->curl, &status, sizeof(status), &len, 0, CURLWS_CLOSE);
    if (res != CURLE_OK) {
      tqError("failed to send ws-close msg to %s for %d", (*ppHandle)->url ? (*ppHandle)->url : "", res);
    }
    // TODO: add wait mechanism for peer connection close response
    curl_easy_cleanup((*ppHandle)->curl);
  }
#endif
