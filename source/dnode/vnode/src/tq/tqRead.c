  ...
  while (1) {
    ...
    if (pTask != NULL) {
      ...
      if ((pTask->info.taskLevel == TASK_LEVEL__SOURCE) && (pTask->exec.pExecutor != NULL)) {
        int32_t code = qDeleteTableListForStreamScanner(pTask->exec.pExecutor, tbUidList);
        if (code != 0) {
...
int32_t tqAddTbUidList(STQ* pTq, SArray* tbUidList) {
  ...
  while (1) {
    ...
    if (pTask != NULL) {
      ...
      if ((pTask->info.taskLevel == TASK_LEVEL__SOURCE) && (pTask->exec.pExecutor != NULL)) {
        int32_t code = qAddTableListForStreamScanner(pTask->exec.pExecutor, tbUidList);
        if (code != 0) {
          tqError("vgId:%d, s-task:0x%x update qualified table error for stream task", vgId, taskId);
        }
      }
...
int32_t tqUpdateTbUidList(STQ* pTq, SArray* tbUidList, SArray* cidList) {
  ...
  while (1) {
    ...
    if (pTask != NULL) {
      ...
      if ((pTask->info.taskLevel == TASK_LEVEL__SOURCE) && (pTask->exec.pExecutor != NULL)) {
        int32_t code = qUpdateTableListForStreamScanner(pTask->exec.pExecutor, tbUidList);
        if (code != 0) {
          tqError("vgId:%d, s-task:0x%x update qualified table error for stream task", vgId, taskId);
        }
      }
