...

      {
        ...
        for (int32_t k = 0; k < pr->numOfCols; ++k) {
          ...
          if (tRowKeyCompare(&pColVal->rowKey, &p->rowKey) > 0) {
            ...
            if (p->colVal.value.type != pColVal->colVal.value.type || p->colVal.cid != pColVal->colVal.cid) {
              // check for type/cid mismatch
              tsdbError("last cache type mismatch, uid:%" PRIu64
                        ", schema-type:%d, schema-col:%d, cache-type:%d, cache-col:%d",
                        uid, p->colVal.value.type, p->colVal.cid, pColVal->colVal.value.type, pColVal->colVal.cid);
              taosArrayClearEx(pRow, tsdbCacheFreeSLastColItem);
