...
int32_t tsdbRowCachePutToLRU(STsdb *pTsdb, SLastRowKey *pLastRowKey, SLastRow *pLastRow, int8_t dirty) {
  ...
  SLastRow *pLRULastRow = NULL;

  pLRULastRow = taosMemoryCalloc(1, sizeof(SLastRow));
  if (!pLRULastRow) {
    code = terrno;
  ...
  *pLRULastRow = *pLastRow;
  ...
  // Success: LRU cache owns the memory now
  pLRULastRow = NULL;
...
int32_t tsdbRowCacheCreateTable(STsdb *pTsdb, int64_t uid, STSchema *pTSchema) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    colVals = taosArrayInit(pTSchema->numOfCols, sizeof(SColVal));
    if (!colVals) {
      code = terrno;
      TAOS_CHECK_GOTO(code, &lino, _exit);
    }
    ...
    for (int j = 0; j < pTSchema->numOfCols; j++) {
      ...
      if (NULL == taosArrayPush(colVals, &emptyCol)) {
        code = terrno;
        TAOS_CHECK_GOTO(code, &lino, _exit);
      }
    }
...
_exit:
  if (code != TSDB_CODE_SUCCESS) {
    if (colVals) taosArrayDestroy(colVals);
    if (colStatus) taosMemoryFree(colStatus);
...
int32_t tsdbRowCacheUpdate(STsdb *pTsdb, tb_uid_t suid, tb_uid_t uid, STsdbRowKey *pRowKey, SRow *pRow, int8_t lflag) {
  ...
  if (h) {
    SLastRow *pCacheLastRow = (SLastRow *)taosLRUCacheValue(pCache, h);
    // Always check for row cache updates - individual column status will be handled separately
    int32_t cmp_res = tRowKeyCompare(&pCacheLastRow->rowKey.key, &pRowKey->key);
    ...
    if (values_list && values_list[0]) {
      ...
      if (pLastRow) {
        ...
        if (cmp_res < 0 || (cmp_res == 0 && pRow != NULL)) {
          ...
          colStatus = lastRowTmp.colStatus;
...
_exit:
  ...
  if (pLastRow) {
    ...
    // Only free colStatus if pLastRow is NULL
    // If pLastRow is not NULL, colStatus was already freed above
    taosMemoryFree(colStatus);
  }
  if (pTSchema) {
...
static int32_t tsdbRowCacheUpdateLastCols(STsdb *pTsdb, tb_uid_t suid, tb_uid_t uid, SArray *updCtxArray,
  ...
  h = taosLRUCacheLookup(pCache, &key, ROCKS_ROW_KEY_LEN);
  ...
  if (h) {
    ...
    if (!pLastRow) {
      ...
      code = tsdbLastRowInitColStatus(pLastRow, pTSchema);
      TAOS_CHECK_GOTO(code, &lino, _exit);
    }
  }

  // Initialize lastColVals array with current values from pLastRow or NULL
  lastColVals = taosArrayInit(pTSchema->numOfCols, sizeof(SColVal));
  if (!lastColVals) {
    code = terrno;
    TAOS_CHECK_GOTO(code, &lino, _exit);
  }
...
int32_t tsdbRowCacheNewTableColumn(STsdb *pTsdb, int64_t uid, int16_t cid, int8_t col_type, int8_t lflag,
  ...
  SLastRow        *pLastRow = NULL;
  SRow            *pNewRow = NULL;
  SColCacheStatus *newColStatus = NULL;
  ...
  if (values_list && values_list[0] != NULL) {
    ...
    if (pLastRow && pLastRow->pRow) {
      ...
      if (pLastRow->colStatus) {
        newColStatus = taosMemoryMalloc(newLastRow.numCols * sizeof(SColCacheStatus));
        if (newColStatus == NULL) {
          code = TSDB_CODE_OUT_OF_MEMORY;
          TAOS_CHECK_GOTO(code, &lino, _exit);
        }
        // Copy existing status entries
        memcpy(newColStatus, pLastRow->colStatus, pLastRow->numCols * sizeof(SColCacheStatus));
        // Add new column status
        newColStatus[newLastRow.numCols - 1] = (SColCacheStatus){.cid = cid, .status = TSDB_LAST_CACHE_VALID};
        newLastRow.colStatus = newColStatus;
      } else {
        // Initialize colStatus for the first time
        code = tsdbLastRowInitColStatus(&newLastRow, pNewTSchema);
        TAOS_CHECK_GOTO(code, &lino, _exit);
        newColStatus = newLastRow.colStatus;
      }
  ...
  if (h) {
    SLastRow *pLRULastRow = (SLastRow *)taosLRUCacheValue(pTsdb->lruCache, h);
    if (pLRULastRow && pLRULastRow->pRow) {
      ...
      for (int j = 0; j < pOldTSchema->numOfCols; j++) {
        SColVal colVal = {0};
        code = tRowGetLastColVal(pLRULastRow->pRow, pOldTSchema, j, &colVal);
        TAOS_CHECK_GOTO(code, &lino, _exit_lru);
      ...
      SLastRow newLastRow = {.rowKey = pLRULastRow->rowKey,
                             .pRow = pNewRow,
                             .dirty = 1,
                             .numCols = pLRULastRow->numCols + 1,
                             .colStatus = NULL};
      ...
      if (pLRULastRow->colStatus) {
        ...
        // Copy existing status entries
        memcpy(newColStatus, pLRULastRow->colStatus, pLRULastRow->numCols * sizeof(SColCacheStatus));
        // Add new column status
        newColStatus[newLastRow.numCols - 1] = (SColCacheStatus){.cid = cid, .status = TSDB_LAST_CACHE_VALID};
        newLastRow.colStatus = newColStatus;
      } else {
...
_exit:
  if (colVals) taosArrayDestroy(colVals);
  if (pLastRow) tsdbFreeLastRow(pLastRow);
  if (pNewRow) taosMemoryFree(pNewRow);
  if (newColStatus) taosMemoryFree(newColStatus);
  if (values_list) {
#ifdef USE_ROCKSDB
    if (values_list[0]) {
      rocksdb_free(values_list[0]);
...
int32_t tsdbRowCacheDropTableColumn(STsdb *pTsdb, int64_t uid, int16_t cid, int8_t lflag,
  ...
  if (values_list && values_list[0] != NULL) {
    SLastRow *pRocksLastRow = NULL;
    code = tsdbRowCacheDeserialize(values_list[0], values_list_sizes[0], &pRocksLastRow);
    if (code != TSDB_CODE_SUCCESS) {
      tsdbFreeLastRow(pRocksLastRow);
      TAOS_CHECK_GOTO(code, &lino, _exit);
    }
    if (pRocksLastRow && pRocksLastRow->pRow) {
      ...
      if (columnExists) {
        ...
        for (int j = 0; j < pTOldSchema->numOfCols; j++) {
          if (pTOldSchema->columns[j].colId != cid) {
            SColVal colVal = {0};
            code = tRowGetLastColVal(pRocksLastRow->pRow, pTOldSchema, j, &colVal);
            if (code != TSDB_CODE_SUCCESS) {
              tsdbFreeLastRow(pRocksLastRow);
              TAOS_CHECK_GOTO(code, &lino, _exit);
            }
        ...
        // Build new row
        SRow             *pRocksNewRow = NULL;
        SRowBuildScanInfo scanInfo = {0};
        code = tRowBuild(colVals, pNewTSchema, &pRocksNewRow, &scanInfo);
        if (code == TSDB_CODE_SUCCESS && pRocksNewRow) {
          // Update the cached row
          SLastRow newLastRow = {.rowKey = pRocksLastRow->rowKey,
                                 .pRow = pRocksNewRow,
                                 .dirty = 1,
                                 .numCols = pRocksLastRow->numCols,
                                 .colStatus = pRocksLastRow->colStatus};
        ...
        for (int k = 0; k < pRocksLastRow->numCols; k++) {
          if (pRocksLastRow->colStatus && pRocksLastRow->colStatus[k].cid == cid) {
            // Remove this column status entry
            memmove(&pRocksLastRow->colStatus[k], &pRocksLastRow->colStatus[k + 1],
                    (pRocksLastRow->numCols - k - 1) * sizeof(SColCacheStatus));
            pRocksLastRow->numCols--;
            statusUpdated = true;
            break;
          }
        }
        ...
        if (statusUpdated) {
          pRocksLastRow->dirty = 1;
          code = tsdbRowCachePutToRocksdb(pTsdb, &rowKey, pRocksLastRow);
        }
      }
    }
  ...
  if (h) {
    ...
    if (pLRULastRow && pLRULastRow->pRow) {
      ...
      if (columnExists) {
        // Build new row without the dropped column
        SArray *lruColVals = taosArrayInit(pTOldSchema->numOfCols - 1, sizeof(SColVal));
        if (lruColVals) {
          // Extract all columns except the one to be dropped
          for (int j = 0; j < pTOldSchema->numOfCols; j++) {
            if (pTOldSchema->columns[j].colId != cid) {
              SColVal colVal = {0};
              code = tRowGetLastColVal(pLRULastRow->pRow, pTOldSchema, j, &colVal);
              if (code != TSDB_CODE_SUCCESS) {
                taosArrayDestroy(lruColVals);
                tsdbLRUCacheRelease(pTsdb->lruCache, h, false);
                TAOS_CHECK_GOTO(code, &lino, _exit);
              }
              if (!taosArrayPush(lruColVals, &colVal)) {
                code = terrno;
                taosArrayDestroy(lruColVals);
                tsdbLRUCacheRelease(pTsdb->lruCache, h, false);
                TAOS_CHECK_GOTO(code, &lino, _exit);
          ...
          // Build new row
          SRow             *pLRUNewRow = NULL;
          SRowBuildScanInfo scanInfo = {0};
          code = tRowBuild(lruColVals, pNewTSchema, &pLRUNewRow, &scanInfo);
          if (code != TSDB_CODE_SUCCESS) {
            taosArrayDestroy(lruColVals);
            tsdbLRUCacheRelease(pTsdb->lruCache, h, false);
            TAOS_CHECK_GOTO(code, &lino, _exit);
          ...
          if (pLRUNewRow) {
            ...
            SLastRow newLastRow = {.rowKey = pLRULastRow->rowKey,
                                   .pRow = pLRUNewRow,
                                   .dirty = 1,
                                   .numCols = pLRULastRow->numCols,
                                   .colStatus = pLRULastRow->colStatus};
        ...
        // Column doesn't exist in row, just delete the column status if present
        bool statusUpdated = false;
        for (int k = 0; k < pLRULastRow->numCols; k++) {
          if (pLRULastRow->colStatus && pLRULastRow->colStatus[k].cid == cid) {
            // Remove this column status entry
            memmove(&pLRULastRow->colStatus[k], &pLRULastRow->colStatus[k + 1],
                    (pLRULastRow->numCols - k - 1) * sizeof(SColCacheStatus));
            pLRULastRow->numCols--;
            statusUpdated = true;
            break;
          }
...
_exit:
  ...
  if (pLastRow) tsdbFreeLastRow(pLastRow);
  if (pNewRow) taosMemoryFree(pNewRow);
  if (pTOldSchema) taosMemoryFree(pTOldSchema);
  if (pNewTSchema) taosMemoryFree(pNewTSchema);
  if (values_list) {
#ifdef USE_ROCKSDB
    if (values_list[0]) {
      rocksdb_free(values_list[0]);
  ...
  for (int i = 0; i < numKeys; ++i) {
    ...
    if (NULL != pLastRow && (pLastRow->rowKey.key.ts <= eKey && pLastRow->rowKey.key.ts >= sKey)) {
      ...
      if (needUpdate) {
        ...
        code = tsdbRowCachePutToRocksdb(pTsdb, pRowKey, pLastRow);
        if (code != TSDB_CODE_SUCCESS) {
        ...
        code = tsdbRowCachePutToLRU(pTsdb, pRowKey, pLastRow, 0);
        if (code != TSDB_CODE_SUCCESS) {
