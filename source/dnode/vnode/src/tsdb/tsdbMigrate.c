...
int32_t tsdbUpdateS3MigrateState(STsdb* tsdb, SVnodeS3MigrateState* pState) {
  ...
  bool isLeader = vnodeIsLeader(tsdb->pVnode);
  bool updated = false;
  ...
  for( int32_t i = 0; i < taosArrayGetSize(pLocalState->pFileSetStates); i++) {
    ...
    // the remote state was not generated by this vnode, which means this vnode was
    // a follower at the time the remote state was generated, so if it is the leader
    // now, it means there's a leader change. because we don't know how the remote
    // file sets are, we set the local file set state to failed.
    if (isLeader) {
      pLocalFileSet->state = FILE_SET_MIGRATE_STATE_FAILED;
      updated = true;
      tsdbDebug("vgId:%d, fid:%d, set migration state to failed since leader changed", vid, pLocalFileSet->fid);
      continue;
    }
...
static int32_t tsdbMigrateFile(SRTNer* rtner, int32_t vid, STFileObj* fobj) {
  ...
  if (code != TSDB_CODE_SUCCESS) {
    tsdbError("vgId:%d, fid:%d, failed to migrate file %s since %s", vid, f->fid, fname, tstrerror(code));
    return code;
  }
...
static int32_t migrateDataFile(SRTNer* rtner, STFileObj* fobj) {
  ...
  for (int i = lcn; i <= totalChunks; ++i) {
    ...
    if (!vnodeIsLeader(rtner->tsdb->pVnode)) {
      tsdbError("vgId:%d, fid:%d, abort data migration since not the leader anymore", vid, f->fid);
...
static int32_t tsdbFollowerDoSsMigrate(SRTNer *rtner) {
  ...
  if (pState->state != FILE_SET_MIGRATE_STATE_SUCCEEDED) {
    tsdbInfo("vgId:%d, fid:%d, migration skipped because leader migration skipped or failed", vid, fset->fid);
    return TSDB_CODE_SUCCESS;
  }

  tsdbInfo("vgId:%d, fid:%d, follower migration started", vid, fset->fid);
  return 0; // TODO
  /*
