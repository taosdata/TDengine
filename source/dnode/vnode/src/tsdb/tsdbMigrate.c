...
static int32_t tsdbFollowerDoSsMigrate(SRTNer *rtner) {
  ...
  tsdbInfo("vgId:%d, fid:%d, leader migration succeeded, begin sync data", vid, fset->fid);
  return 0; // TODO
  /*
...
static int32_t tsdbLeaderDoSsMigrate(SRTNer *rtner) {
  ...
  tsdbInfo("vgId:%d, fid:%d, leader migration started", vid, fset->fid);
  ...
  tsdbInfo("vgId:%d, fid:%d, manifest file uploaded, migration succeeded", vid, fset->fid);
