...
static int32_t tsdbDoCopyFileLC(SRTNer *rtner, const STFileObj *from, const STFile *to) {
  ...
  TAOS_CHECK_GOTO(tsdbTFileLastChunkName(rtner->tsdb, from->f, fname_from), &lino, _exit);
  TAOS_CHECK_GOTO(tsdbTFileLastChunkName(rtner->tsdb, to, fname_to), &lino, _exit);
...
static int32_t tsdbDoCopyFile(SRTNer *rtner, const STFileObj *from, const STFile *to) {
  ...
  TAOS_CHECK_GOTO(tsdbTFileName(rtner->tsdb, to, fname), &lino, _exit);
...
static int32_t tsdbRetention(void *arg) {
  ...
  code = tsdbBeginTaskOnFileSet(pTsdb, rtnArg->fid, &fset);
  if (code == TSDB_CODE_SUCCESS && fset && (code = tsdbTFileSetInitCopy(pTsdb, fset, &rtner.fset))) {
...
static int32_t tsdbMigrateDataFileLCS3(SRTNer *rtner, const STFileObj *fobj, int64_t size, int64_t chunksize) {
  ...
  char fname[TSDB_FILENAME_LEN];
  TAOS_CHECK_GOTO(tsdbTFileName(rtner->tsdb, &op.nf, fname), &lino, _exit);
  char   *object_name = taosDirEntryBaseName(fname);
...
static int32_t tsdbDoS3Migrate(SRTNer *rtner) {
  ...
  if (/*lcn < 1 && */ taosCheckExistFile(fobj->fname)) {
    ...
    TAOS_CHECK_GOTO(tsdbTFileLastChunkName(rtner->tsdb, fobj->f, fname1), &lino, _exit);
