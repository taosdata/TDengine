...
int ttlMgrOpen(STtlManger **ppTtlMgr, TDB *pEnv, int8_t rollback) {
  int   ret;
  char *oldTbname = "ttl.idx";
  char *newTbname = "ttlv1.idx";
  ...
  *ppTtlMgr = NULL;
  ...
  if (!rollback && tdbTbExist(oldTbname, pEnv)) {
    ret = tdbTbOpen(oldTbname, sizeof(STtlIdxKey), 0, ttlIdxKeyCmpr, pEnv, &pTtlMgr->pOldTtlIdx, rollback);
    if (ret < 0) {
      metaError("failed to open meta old ttl index since %s", tstrerror(terrno));
      return ret;
    }
  }

  ret = tdbTbOpen(newTbname, TDB_VARIANT_LEN, TDB_VARIANT_LEN, ttlIdxKeyV1Cmpr, pEnv, &pTtlMgr->pTtlIdx, rollback);
  if (ret < 0) {
    metaError("failed to open %s since %s", newTbname, tstrerror(terrno));
...
int ttlMgrBegin(STtlManger *pTtlMgr, void *pMeta) {
  ...
  if (pTtlMgr->pOldTtlIdx) {
    ...
    ret = tdbTbDropByName("ttl.idx", meta->pEnv, meta->txn);
    if (ret < 0) {
  ...
  metaInfo("ttl mgr open end, hash size: %d, time consumption: %ld ns", taosHashGetSize(pTtlMgr->pTtlCache),
           endNs - startNs);
_err:
  return ret;
}
...
int ttlMgrConvert(TTB *pOldTtlIdx, TTB *pNewTtlIdx, void *pMeta) {
  ...
  metaWarn("ttlMgr convert ttl start.");
  ...
  metaWarn("ttlMgr convert ttl end.");
  return ret;
}
...
int ttlMgrDeleteTtl(STtlManger *pTtlMgr, const STtlDelTtlCtx *delCtx) {
  ...
  metaDebug("ttl mgr update ctime, uid: %ld", delCtx->uid);
...
int ttlMgrUpdateChangeTime(STtlManger *pTtlMgr, const STtlUpdCtimeCtx *pUpdCtimeCtx) {
  ...
  metaDebug("ttl mgr update ctime, uid: %ld, ctime: %ld", pUpdCtimeCtx->uid, pUpdCtimeCtx->changeTimeMs);
