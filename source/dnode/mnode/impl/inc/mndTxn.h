...
typedef struct SUserTxn {
  txn_id_t       txnId;      // 全局唯一 64 位事务 ID（高 32 位为 seq 段，低 32 位为 range 内偏移）
...
int8_t      mndTxnIsAlive(SMnode* pMnode, txn_id_t txnId);           // keepalive 查询：txn 是否仍在活跃阶段
void        mndTxnRefreshKeepalive(SMnode* pMnode, txn_id_t txnId);  // 客户端 HB 保活：刷新 lastActiveTime
int32_t     mndRollbackOrphanTxnOnVnode(SMnode* pMnode, txn_id_t txnId, int32_t vgId);  // Raft-safe orphan cleanup
