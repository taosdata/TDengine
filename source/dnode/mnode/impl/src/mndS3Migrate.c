...
static int32_t mndProcessS3MigrateTimer(SRpcMsg *pReq);
...
int32_t mndInitS3Migrate(SMnode *pMnode) {
  ...
  mndSetMsgHandle(pMnode, TDMT_MND_S3MIGRATE_DB_TIMER, mndProcessS3MigrateTimer);
...
static int32_t mndProcessQueryS3MigrateProgressRsp(SRpcMsg *pReq) {
  ...
  if (pReq->code != 0) {
    mError("received wrong s3migrate response, req code is %s", tstrerror(pReq->code));
    TAOS_RETURN(pReq->code);
  }
  code = tDeserializeSQueryS3MigrateProgressRsp(pReq->pCont, pReq->contLen, &rsp);
  if (code != 0) {
    mError("failed to deserialize vnode-query-s3migrate-progress-rsp, ret:%d, pCont:%p, len:%d", code, pReq->pCont,
           pReq->contLen);
    TAOS_RETURN(code);
  }
  ...
  code = mndUpdateS3MigrateProgress(pMnode, pReq, rsp.s3MigrateId, &rsp);
  if (code != 0) {
...
static int32_t mndProcessS3MigrateTimer(SRpcMsg *pReq) {
  mTrace("start to process s3migrate timer");
