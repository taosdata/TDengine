...
int32_t        mndFetchSdbStables(const char *mntName, const char *path, SArray **ppStbs) {
  ...
  SHashObj *hash = pSdb->hashObjs[SDB_STB];
  nStb = taosHashGetSize(hash);
  ...
  while ((pStb = taosHashIterate(hash, pStb))) {
    ...
    *(int32_t *)pVal = vlen;
    memcpy((char *)pVal + sizeof(int32_t), pStb, sizeof(SSdbRaw) + pStb->dataLen);
    TSDB_CHECK_NULL(taosArrayPush(pStbs, &pVal), code, lino, _exit, terrno);
  }
