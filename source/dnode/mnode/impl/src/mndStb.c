...
static void *mndBuildVCreateStbReq(SMnode *pMnode, SVgObj *pVgroup, SStbObj *pStb, int32_t *pContLen) {
  ...
  if (req.rollup) {
    ...
    if (pStb->ast1Len > 0) {
      if (mndConvertRsmaTask(&req.pRSmaParam.qmsg1, &req.pRSmaParam.qmsg1Len, pStb->pAst1, pStb->uid, 0, 0,
                             req.pRSmaParam.xFilesFactor) != TSDB_CODE_SUCCESS) {
        return NULL;
      }
    }
    if (pStb->ast2Len > 0) {
      if (mndConvertRsmaTask(&req.pRSmaParam.qmsg2, &req.pRSmaParam.qmsg2Len, pStb->pAst2, pStb->uid, 0, 0,
                             req.pRSmaParam.xFilesFactor) != TSDB_CODE_SUCCESS) {
        return NULL;
      }
    }
