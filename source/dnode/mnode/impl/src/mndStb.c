...
static int32_t mndRetrieveStb(SRpcMsg *pReq, SShowObj *pShow, SSDataBlock *pBlock, int32_t rows) {
  ...
  showAll = (0 == mndCheckObjPrivilegeRecF(pMnode, pOperUser, PRIV_CM_SHOW, PRIV_OBJ_TBL, pDb?pDb->ownerId:0 , pDb ? pDb->name : objFName,
                                           "*"));
  ...
  while (numOfRows < rows) {
    ...
    if (!showAll &&
        (mndCheckObjPrivilegeRecF(pMnode, pOperUser, PRIV_CM_SHOW, PRIV_OBJ_TBL, pStb->ownerId, pStb->db,
                                  &stbName[VARSTR_HEADER_SIZE]) ||
         mndCheckDbPrivilegeByName(pMnode, RPC_MSG_USER(pReq), RPC_MSG_TOKEN(pReq), MND_OPER_USE_DB, pStb->db))) {
      sdbRelease(pSdb, pStb);
      terrno = 0;
      continue;
    }
