...
static int32_t tokenCacheUpdate(const STokenObj* token) {
  ...
  (void)taosThreadRwlockWrlock(&tokenCache.rw);
  SCachedTokenInfo** pp = taosHashGet(tokenCache.tokens, token->token, TSDB_TOKEN_LEN);
  if (pp != NULL && *pp != NULL) {
    ti = *pp;
  } else {
...
int32_t mndGetUserActiveToken(const char* user, char* token) {
  ...
  while (pIter) {
    SCachedTokenInfo *ti = *(SCachedTokenInfo **)pIter;
    if (ti != NULL && ti->enabled && (ti->expireTime == 0 || ti->expireTime > now)) {
...
static int32_t mndProcessCreateTokenReq(SRpcMsg *pReq) {
  ...
  if (pToken != NULL) {
    TAOS_CHECK_GOTO(TSDB_CODE_MND_TOKEN_ALREADY_EXIST, &lino, _OVER);
  }
