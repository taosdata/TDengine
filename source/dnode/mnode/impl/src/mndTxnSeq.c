...
// currentTxnId has been moved into pMnode->txnMgmt.currentTxnId; no global variable is used here.
...
int32_t mndTxnSeqPrepare(SMnode *pMnode) {
  ...
  } else if (code != TSDB_CODE_TXN_SEQ_NOT_EXIST) {
...
_exit:
  if (code != TSDB_CODE_SUCCESS) {
    mError("txnSeq, failed at line %d to init txn seq since %s", lino, tstrerror(code));
  }
  TAOS_RETURN(code);
}
...
static int32_t triggerAllocateTxnSeq(SMnode *pMnode, txn_id_t nextRangeId, bool checkLeader) {
  ...
  // Fine-grained dedup: skip only when the in-flight request already covers nextRangeId.
  // A larger nextRangeId must be allowed through (range exhausted while previous alloc is slow).
  txn_id_t pending = atomic_load_64(&pMnode->txnMgmt.pendingRangeId);
  if (pending != 0 && nextRangeId <= pending) {
    int64_t elapsed = taosGetTimestampMs() - atomic_load_64(&pMnode->txnMgmt.txnSeqAllocTime);
    if (elapsed <= 30000) {
      mInfo("txnSeq, skip since in-flight alloc pendingRangeId:%" PRIu64 " already covers nextRangeId:%" PRIu64,
            pending, nextRangeId);
      TAOS_RETURN(0);
    }
    mWarn("txnSeq, in-flight alloc appears stuck (elapsed %" PRId64 "ms > 30s), re-triggering nextRangeId:%" PRIu64,
          elapsed, nextRangeId);
  }
  atomic_store_64(&pMnode->txnMgmt.pendingRangeId, nextRangeId);
  atomic_store_64(&pMnode->txnMgmt.txnSeqAllocTime, taosGetTimestampMs());
...
_exit:
  if (code < 0) {
    // Reset pending range on error so future allocations are not permanently blocked
    atomic_store_64(&pMnode->txnMgmt.pendingRangeId, 0);
...
int32_t mndAcquireTxnSeq(SMnode *pMnode, int32_t id, STxnSeqObj **ppObj) {
  ...
  if (pObj == NULL) {
    if (terrno == TSDB_CODE_SDB_OBJ_NOT_THERE) {
      terrno = TSDB_CODE_TXN_SEQ_NOT_EXIST;
    } else if (terrno == TSDB_CODE_SDB_OBJ_CREATING) {
      terrno = TSDB_CODE_MND_TXN_SEQ_IN_CREATING;
    } else if (terrno == TSDB_CODE_SDB_OBJ_DROPPING) {
      terrno = TSDB_CODE_MND_TXN_SEQ_IN_DROPPING;
    } else {
...
int32_t mndGenTxnId(SMnode *pMnode, txn_id_t *pTxnId) {
  ...
  mndReleaseTxnSeq(pMnode, pObj);
  if (nextId <= 0) {
    // Signed int64 wraparound — currentTxnId crossed 2^63. Engineering-impossible
    // at any realistic rate (range-allocated, ~10^4 ids per range) but if it ever
    // happens, returning to caller stops txn creation rather than corrupting state.
...
static int32_t mndProcessTxnSeqAllocRsp(SRpcMsg *pReq) {
  int32_t code = 0, lino = 0;
  mInfo("received txn seq alloc rsp, code:0x%x", pReq->code);
  SMnode     *pMnode = pReq->info.node;
  SMTxnSeqRsp txnRsp = {0};

  atomic_store_64(&pMnode->txnMgmt.pendingRangeId, 0);  // clear in-flight marker; allow next allocation when needed
  if (pReq->code != 0) {
    mError("txn seq alloc rsp failed since %s", tstrerror(pReq->code));
    code = pReq->code;
    goto _exit;
  }
  ...
  STxnSeqObj *pObj = NULL;
  TAOS_CHECK_EXIT(mndAcquireTxnSeq(pMnode, 0, &pObj));
  taosWLockLatch(&pObj->lock);
  txn_id_t oldMaxRangeId = pObj->maxRangeId;
  if (txnRsp.rangeId >= oldMaxRangeId) {
    // Accept responses that match or advance the current maxRangeId.
    // Only a strictly smaller rangeId is truly stale (would regress the range).
    if (txnRsp.rangeId > oldMaxRangeId) {
      pObj->maxRangeId = txnRsp.rangeId;
    }
    if (pMnode->txnMgmt.currentTxnId < 0 || pMnode->txnMgmt.currentTxnId >= oldMaxRangeId) {
      // currentTxnId is uninitialised (-1) or has exhausted the previous range;
      // reset it to the start of the current range so mndGenTxnId can proceed.
      pMnode->txnMgmt.currentTxnId = pObj->maxRangeId - TXN_ID_RANGE_STEP;
    }
  } else {
    mInfo("txnSeq, ignore stale alloc rsp rangeId:%" PRIu64 " < current maxRangeId:%" PRIu64, txnRsp.rangeId,
          oldMaxRangeId);
  }
  taosWUnLockLatch(&pObj->lock);
