...
/** @section xnode task slices handlers */
static int32_t mndProcessCreateXnodeSliceReq(SRpcMsg *pReq);
static int32_t mndProcessUpdateXnodeSliceReq(SRpcMsg *pReq);
static int32_t mndProcessDropXnodeSliceReq(SRpcMsg *pReq);
static int32_t mndRetrieveXnodeTaskJobs(SRpcMsg *pReq, SShowObj *pShow, SSDataBlock *pBlock, int32_t rows);
static void    mndCancelGetNextXnodeTaskJob(SMnode *pMnode, void *pIter);
...
int32_t mndInitXnode(SMnode *pMnode) {
  ...
  mndSetMsgHandle(pMnode, TDMT_MND_DROP_XNODE_TASK, mndProcessDropXnodeReq);
  ...
  mndSetMsgHandle(pMnode, TDMT_MND_CREATE_XNODE_SLICE, mndProcessCreateXnodeReq);
  mndSetMsgHandle(pMnode, TDMT_MND_UPDATE_XNODE_SLICE, mndProcessUpdateXnodeReq);
  mndSetMsgHandle(pMnode, TDMT_MND_DROP_XNODE_SLICE, mndProcessDropXnodeReq);
  mndAddShowRetrieveHandle(pMnode, TSDB_MGMT_TABLE_XNODE_TASK_JOBS, mndRetrieveXnodeTasks);
  mndAddShowFreeIterHandle(pMnode, TSDB_MGMT_TABLE_XNODE_TASK_JOBS, mndCancelGetNextXnodeTask);
...
static int32_t mndProcessCreateXnodeTaskJobReq(SRpcMsg *pReq) {
  printf("xnode create task slice request received, contLen:%d", pReq->contLen);
  SMnode              *pMnode = pReq->info.node;
  int32_t              code = -1;
  SXnodeTaskJobObj  *pObj = NULL;
  SMCreateXnodeTaskReq createReq = {0};
...
static int32_t mndProcessUpdateXnodeTaskJobReq(SRpcMsg *pReq) {
  printf("xnode update task slice request received, contLen:%d", pReq->contLen);
  return 0;
}
static int32_t mndProcessDropXnodeTaskJobReq(SRpcMsg *pReq) {
  printf("xnode drop task slice request received, contLen:%d", pReq->contLen);
  return 0;
}
static int32_t mndRetrieveXnodeTaskJobs(SRpcMsg *pReq, SShowObj *pShow, SSDataBlock *pBlock, int32_t rows) {
  SMnode             *pMnode = pReq->info.node;
  ...
  int32_t             cols = 0;
  SXnodeTaskJobObj *pObj = NULL;
  char                config[TSDB_XNODE_TASK_CONFIG_LEN + VARSTR_HEADER_SIZE];
  char                status[64];
  int32_t             code = 0;
  mInfo("show.type:%d, %s:%d: retrieve xnode tasks with rows: %d", pShow->type, __FILE__, __LINE__, rows);

  while (numOfRows < rows) {
    pShow->pIter = sdbFetch(pSdb, SDB_XNODE_TASK_JOB, pShow->pIter, (void **)&pObj);
    if (pShow->pIter == NULL) break;
    ...
    pColInfo = taosArrayGet(pBlock->pDataBlock, cols++);
    code = colDataSetVal(pColInfo, numOfRows, (const char *)config, false);
    if (code != 0) goto _end;
...
static void mndCancelGetNextXnodeTaskJob(SMnode *pMnode, void *pIter) {
  SSdb *pSdb = pMnode->pSdb;
  sdbCancelFetchByType(pSdb, pIter, SDB_XNODE_TASK_JOB);
}
