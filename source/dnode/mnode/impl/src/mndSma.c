...
static int32_t mndSetCreateTSMARedoLogs(SMnode *pMnode, STrans *pTrans, SSmaObj *pSma) {
  int32_t  code = 0;
...
static int32_t mndSetCreateTSMAUndoLogs(SMnode *pMnode, STrans *pTrans, SSmaObj *pSma) {
  int32_t  code = 0;
...
static int32_t mndSetCreateTSMACommitLogs(SMnode *pMnode, STrans *pTrans, SSmaObj *pSma) {
  int32_t  code = 0;
...
static int32_t mndCheckCreateTSMAReq(SMCreateSmaReq *pCreate) {
...
static int32_t mndSetDropTSMARedoLogs(SMnode *pMnode, STrans *pTrans, SSmaObj *pSma) {
  int32_t  code = 0;
  SSdbRaw *pRedoRaw = mndSmaActionEncode(pSma);
  if (pRedoRaw == NULL) {
    code = TSDB_CODE_MND_RETURN_VALUE_NULL;
...
static int32_t mndSetDropTSMACommitLogs(SMnode *pMnode, STrans *pTrans, SSmaObj *pSma) {
  int32_t  code = 0;
  SSdbRaw *pCommitRaw = mndSmaActionEncode(pSma);
  if (pCommitRaw == NULL) {
    code = TSDB_CODE_MND_RETURN_VALUE_NULL;
...
static void initSMAObj(SCreateTSMACxt *pCxt) {
  ...
  pCxt->pSma->uid = pCxt->pCreateSmaReq->uid;
...
static int32_t mndCreateTSMATxnPrepare(SCreateTSMACxt *pCxt) {
  ...
  newDb.tsmaVersion++;
  TAOS_CHECK_GOTO(mndSetCreateTSMARedoLogs(pCxt->pMnode, pTrans, pCxt->pSma), NULL, _OVER);
  TAOS_CHECK_GOTO(mndSetCreateTSMAUndoLogs(pCxt->pMnode, pTrans, pCxt->pSma), NULL, _OVER);
  TAOS_CHECK_GOTO(mndSetCreateTSMACommitLogs(pCxt->pMnode, pTrans, pCxt->pSma), NULL, _OVER);
...
static int32_t mndProcessCreateTSMAReq(SRpcMsg *pReq) {
  ...
  if ((code = mndCheckCreateTSMAReq(&createReq)) != 0) goto _OVER;
...
static int32_t mndDropTSMA(SCreateTSMACxt *pCxt) {
  ...
  memcpy(&newDb, pCxt->pDb, sizeof(SDbObj));
  newDb.tsmaVersion++;
  TAOS_CHECK_GOTO(mndSetDropTSMARedoLogs(pCxt->pMnode, pTrans, pCxt->pSma), NULL, _OVER);
  TAOS_CHECK_GOTO(mndSetDropTSMACommitLogs(pCxt->pMnode, pTrans, pCxt->pSma), NULL, _OVER);
...
int32_t mndDropTSMAsByDb(SMnode *pMnode, STrans *pTrans, SDbObj *pDb) {
  ...
  while (1) {
    ...
    if (pSma->dbUid == pDb->uid) {
      if ((code = mndSetDropTSMACommitLogs(pMnode, pTrans, pSma)) != 0) {
        sdbRelease(pSdb, pSma);
