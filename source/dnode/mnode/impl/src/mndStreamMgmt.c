...
int32_t msmUPAddScanTask(SStmGrpCtx* pCtx, SStreamObj* pStream, char* scanPlan, int32_t vgId, int64_t taskId) {
  ...
  TAOS_CHECK_EXIT(stmBuildAcquireAllocator(&pCtx->allocatorRefId, streamId));
...
void msmCleanStreamGrpCtx(SStreamHbMsg* pHb) {
  ...
  if (mStreamMgmt.tCtx) {
    ...
    stmReleaseDestroyAllocator(mStreamMgmt.tCtx[tidx].grpCtx[pHb->streamGId].allocatorRefId);
  }
}
