...
static int32_t mndProcessCreateQnodeReq(SRpcMsg *pReq) {
  ...
  auditRecord1(pReq, pMnode->clusterId, "createQnode", obj, "", createReq.sql, createReq.sqlLen);
_OVER:
  if (code != 0 && code != TSDB_CODE_ACTION_IN_PROGRESS) {
...
static int32_t mndProcessDropQnodeReq(SRpcMsg *pReq) {
  ...
  auditRecord1(pReq, pMnode->clusterId, "dropQnode", obj, "", dropReq.sql, dropReq.sqlLen);
