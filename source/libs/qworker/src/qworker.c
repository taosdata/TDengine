...
int32_t qwProcessQuery(QW_FPARAMS_DEF, SQWMsg *qwMsg, char *sql) {
  ...
  QW_SCH_TASK_PERF("qMsgToSubplan time:%", PRId64, "us", end - start);
  taosDisableMemPoolUsage();
  ...
  QW_SCH_TASK_PERF("qCreateExecTask time:%", PRId64, "us", end - start);
  taosDisableMemPoolUsage();
  ...
  if (!ctx->dynamicTask) {
    ...
    int64_t end = taosGetTimestampUs();
    QW_SCH_TASK_PERF("qwExecTask time:%", PRId64, "us", end - start);
  } else {
    ctx->queryExecDone = true;
    ctx->queryEnd = true;
  }
