...
static int32_t createFederatedScanPhysiNode(SPhysiPlanContext* pCxt, SSubplan* pSubplan,
  ...
  // Build pColTypeMappings: one entry per scan column, matching the SELECT column
  // order generated by nodesRemotePlanToSQL.  The executor passes these to
  // extConnectorFetchBlock for raw-value → TDengine-binary conversion.
  // The TDengine type is already in SColumnNode.dataType (resolved during parsing).
  if (pScan->pScanCols) {
    int32_t nScanCols = (int32_t)LIST_LENGTH(pScan->pScanCols);
    if (nScanCols > 0) {
      pScan->pColTypeMappings =
          (SExtColTypeMapping*)taosMemoryCalloc(nScanCols, sizeof(SExtColTypeMapping));
      if (!pScan->pColTypeMappings) {
        nodesDestroyNode((SNode*)pScan);
        return terrno;
      }
      int32_t mappingIdx = 0;
      SNode*  pColNode   = NULL;
      FOREACH(pColNode, pScan->pScanCols) {
        if (nodeType(pColNode) == QUERY_NODE_COLUMN) {
          SColumnNode* pCol = (SColumnNode*)pColNode;
          pScan->pColTypeMappings[mappingIdx].tdType = pCol->node.resType;
        } else {
          // Fallback for non-column nodes: VARCHAR
          pScan->pColTypeMappings[mappingIdx].tdType.type  = TSDB_DATA_TYPE_VARCHAR;
          pScan->pColTypeMappings[mappingIdx].tdType.bytes = TSDB_MAX_BINARY_LEN;
        }
        mappingIdx++;
      }
      pScan->numColTypeMappings = mappingIdx;
    }
  }
