...
static int32_t vstbAggSplit(SSplitContext* pCxt, SLogicSubplan* pSubplan) {
  ...
  if (info.needPartAgg) {
    ...
    PLAN_ERR_RET(splCreateExchangeNodeForSubplan(pCxt, info.pSubplan, (SLogicNode*)info.pAgg, info.pSubplan->subplanType, false));
    PLAN_ERR_RET(nodesListMakeStrictAppend(&info.pSubplan->pChildren, (SNode*)splCreateScanSubplan(pCxt, (SLogicNode*)info.pAgg, 0)));
  }
