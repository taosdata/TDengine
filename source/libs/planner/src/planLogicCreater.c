...
static int32_t scanAddCol(SLogicNode* pLogicNode, SColRef* colRef, const SSchema* pSchema) {
  int32_t         code = TSDB_CODE_SUCCESS;
  ...
  pRefTableScanCol->colId = colRef->id;
...
static int32_t createVirtualTableLogicNode(SLogicPlanContext* pCxt, SSelectStmt* pSelect,
  ...
  FOREACH(pNode, pVtableScan->pScanCols) {
    ...
    if (pVirtualTable->pMeta->colRef[pCol->colId - 1].hasRef) {
      ...
      PLAN_ERR_JRET(scanAddCol(pRefScan, pColRef, &pVirtualTable->pMeta->schema[pCol->colId - 1]));
