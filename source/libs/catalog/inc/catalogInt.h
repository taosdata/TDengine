...
typedef enum {
  CTG_BATCH_SOURCE_INVALID = 0,
  CTG_BATCH_SOURCE_DIRECT,
  CTG_BATCH_SOURCE_REQ_CTX,
  CTG_BATCH_SOURCE_JOB_DEFAULT,
} SCtgBatchSourceType;
...
typedef struct SCtgTaskReq {
  ...
  SHashObj*        pBatchs;      // batch container used only when batchSource is CTG_BATCH_SOURCE_DIRECT
  SCtgBatchSourceType batchSource;  // explicit batch source for the current task request
} SCtgTaskReq;
