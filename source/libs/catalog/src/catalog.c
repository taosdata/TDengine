...
int32_t catalogGetUserCachedDbs(SCatalog* pCtg, const char* pUser, SArray** pDbList) {
  ...
  if (NULL == pCtg || NULL == pUser || NULL == pDbList) {
    CTG_API_LEAVE(TSDB_CODE_CTG_INVALID_INPUT);
  }

  *pDbList = NULL;
  SCtgUserAuth* pUserAuth = (SCtgUserAuth*)taosHashGet(pCtg->userCache, pUser, strlen(pUser));
  if (NULL == pUserAuth) {
    CTG_API_LEAVE(TSDB_CODE_SUCCESS);  // User not in cache, return empty list
  }

  taosRLockLatch(&pUserAuth->lock);

  SGetUserAuthRsp* pAuth = &pUserAuth->userAuth;
  SHashObj*        readDbs = pAuth->readDbs;
  SHashObj*        writeDbs = pAuth->writeDbs;

  // Count unique databases
  int32_t dbCount = (readDbs ? taosHashGetSize(readDbs) : 0) + (writeDbs ? taosHashGetSize(writeDbs) : 0);
  if (dbCount == 0) {
    taosRUnLockLatch(&pUserAuth->lock);
    CTG_API_LEAVE(TSDB_CODE_SUCCESS);
  }

  *pDbList = taosArrayInit(dbCount, TSDB_DB_FNAME_LEN);
  if (NULL == *pDbList) {
    taosRUnLockLatch(&pUserAuth->lock);
    CTG_API_LEAVE(terrno);
  }

  SSHashObj* addedDbs = tSimpleHashInit(dbCount, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BINARY));
  if (NULL == addedDbs) {
    taosArrayDestroy(*pDbList);
    *pDbList = NULL;
    taosRUnLockLatch(&pUserAuth->lock);
    CTG_API_LEAVE(terrno);
  }
  ...
  void* pIter = NULL;
  // Collect from readDbs
  if (readDbs) {
    while ((pIter = taosHashIterate(readDbs, pIter))) {
      char* dbFName = (char*)taosHashGetKey(pIter, NULL);
      if (NULL == tSimpleHashGet(addedDbs, dbFName, strlen(dbFName) + 1)) {
        char dbName[TSDB_DB_FNAME_LEN] = {0};
        tstrncpy(dbName, dbFName, TSDB_DB_FNAME_LEN);
        if (NULL == taosArrayPush(*pDbList, dbName)) {
          taosHashCancelIterate(readDbs, pIter);
          goto _error;
        }
  ...
  // Collect from writeDbs
  if (writeDbs) {
    while ((pIter = taosHashIterate(writeDbs, pIter))) {
      char* dbFName = (char*)taosHashGetKey(pIter, NULL);
      if (NULL == tSimpleHashGet(addedDbs, dbFName, strlen(dbFName) + 1)) {
        char dbName[TSDB_DB_FNAME_LEN] = {0};
        tstrncpy(dbName, dbFName, TSDB_DB_FNAME_LEN);
        if (NULL == taosArrayPush(*pDbList, dbName)) {
          taosHashCancelIterate(writeDbs, pIter);
          goto _error;
        }
        (void)tSimpleHashPut(addedDbs, dbFName, strlen(dbFName) + 1, NULL, 0);
      }
