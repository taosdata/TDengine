...
static SCtgMsgCtx* ctgGetReqMsgCtx(SCtgTaskReq* pReq);
static SHashObj* ctgGetReqBatchs(SCtgTaskReq* pReq);
static SCtgTaskReq ctgMakeTaskReq(SCtgTask* pTask, int32_t msgIdx, SCtgBatchSourceType batchSource);
...
/*
 * Return the current msg ctx referenced by one task request.
 * Batch-task requests use msgIdx >= 0 to address one per-message ctx, while msgIdx < 0 means the task link ctx.
 * Non-batch tasks always use the task link ctx.
 * Return the resolved msg ctx pointer, or NULL if the request/task is invalid.
 */
static SCtgMsgCtx* ctgGetReqMsgCtx(SCtgTaskReq* pReq) {
  ...
  return (pReq->msgIdx >= 0) ? CTG_GET_TASK_MSGCTX(pReq->pTask, pReq->msgIdx) : ctgGetTaskLinkMsgCtx(pReq->pTask);
}

/*
 * Resolve the batch container carried by one task request from its explicit source.
 * DIRECT means callback-local/explicit request batch, REQ_CTX means the current request msg ctx,
 * and JOB_DEFAULT means the job-owned default launch batch.
 * Return the resolved batch container pointer, or NULL if the chosen source is missing.
 */
static SHashObj* ctgGetReqBatchs(SCtgTaskReq* pReq) {
  ...
  switch (pReq->batchSource) {
    case CTG_BATCH_SOURCE_DIRECT:
      return pReq->pBatchs;
    case CTG_BATCH_SOURCE_REQ_CTX: {
      SCtgMsgCtx* pMsgCtx = ctgGetReqMsgCtx(pReq);
      return pMsgCtx ? pMsgCtx->pBatchs : NULL;
    }
    case CTG_BATCH_SOURCE_JOB_DEFAULT:
...
/*
 * Build one task request with an explicit batch source for the given task and msg index.
 * pTask is the target catalog async task.
 * msgIdx identifies the request-level msg ctx, or -1 for task-level flows.
 * batchSource identifies where downstream code must read the batch container from.
...
static int32_t ctgInitTaskMsgCtxBatchs(SCtgTaskReq* pReq, SCtgMsgCtx* pMsgCtx) {
  ...
  if (NULL == pMsgCtx->pBatchs) {
    qError("fail to resolve batch container for task:%d, type:%d, source:%d", pTask->taskId, pTask->type,
           pReq->batchSource);
    CTG_ERR_RET(TSDB_CODE_CTG_INTERNAL_ERROR);
  }
...
static int32_t ctgInitBatchTaskMsgCtx(SCtgTask* pTask, SCtgMsgCtx* pMsgCtx) {
  ...
  SCtgMsgCtx* pTaskMsgCtx = ctgGetTaskLinkMsgCtx(pTask);
  if (NULL == pTaskMsgCtx) {
    CTG_ERR_RET(TSDB_CODE_CTG_INTERNAL_ERROR);
  }
  ...
  return ctgInitTaskMsgCtxBatchs(&tReq, pMsgCtx);
}
...
int32_t ctgCallSubCb(SCtgTaskReq* pReq) {
  ...
  for (int32_t i = 0; i < parentNum; ++i) {
    ...
    SCtgTaskReq parentReq = {.pTask = pParent,
                             .msgIdx = -1,
                             .autoCreateCtb = 0,
                             .pBatchs = ctgGetReqBatchs(pReq),
                             .batchSource = CTG_BATCH_SOURCE_DIRECT};
...
int32_t ctgLaunchGetTbMetasTask(SCtgTask* pTask) {
  ...
  for (int32_t i = 0; i < pCtx->fetchNum; ++i) {
    ...
    SCtgTaskReq tReq = ctgMakeTaskReq(pTask, pFetch->fetchIdx, CTG_BATCH_SOURCE_REQ_CTX);
...
int32_t ctgSetSubTaskCb(SCtgTask* pSub, SCtgTaskReq* pReq) {
  ...
  if (CTG_TASK_DONE == pSub->status) {
    ...
    CTG_ERR_JRET(pTask->subRes.fp(pReq));
  } else {
...
int32_t ctgLaunchSubTask(SCtgTaskReq* pReq, CTG_TASK_TYPE type, ctgSubTaskCbFp fp, void* param) {
  ...
  if (newTask) {
    ...
    if (NULL == pLaunchBatchs) {
      qError("fail to resolve subtask launch batch, parentTask:%d, subTask:%d, source:%d", pTask->taskId,
             pSub->taskId, pReq->batchSource);
      CTG_ERR_RET(TSDB_CODE_CTG_INTERNAL_ERROR);
    }
