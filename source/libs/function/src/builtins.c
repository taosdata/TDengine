...
static int32_t translateBitChange(SFunctionNode* pFunc, char* pErrBuf, int32_t len) {
  FUNC_ERR_RET(validateParam(pFunc, pErrBuf, len));
  pFunc->node.resType = (SDataType){.bytes = tDataTypes[TSDB_DATA_TYPE_BIGINT].bytes, .type = TSDB_DATA_TYPE_BIGINT};
  return TSDB_CODE_SUCCESS;
}
  ...
  },
  {
    ...
    .parameters = {.minParamNum = 1,
                   ...
                   .inputParaInfo[0][2] = {.isLastParam = true,
                                           ...
                                           .fixedNumValue = {0, 1, 2}},
                   .outputParaInfo = {.validDataType = FUNC_PARAM_SUPPORT_BIGINT_TYPE},},
    .translateFunc = translateBitChange,
    .getEnvFunc   = getBitChangeFuncEnv,
    .initFunc     = bitChangeFunctionSetup,
    .processFunc  = bitChangeFunction,
    .sprocessFunc = bitChangeScalarFunction,
    .finalizeFunc = bitChangeFinalize,
    .pPartialFunc = "bit_change",
    .pMergeFunc   = "sum",
  },
};
// clang-format on
