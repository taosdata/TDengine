...
int32_t blockDistFinalize(SqlFunctionCtx* pCtx, SSDataBlock* pBlock) {
  ...
  int32_t len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE,
  ...
  len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE, "Block_Rows=[%" PRId64 "] MinRows=[%d] MaxRows=[%d] AvgRows=[%" PRId64 "]",
  ...
  len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE, "Inmem_Rows=[%d] Stt_Rows=[%d] ", pData->numOfInmemRows, pData->numOfSttRows);
  varDataSetLen(st, len);
  code = colDataSetVal(pColInfo, row++, st, false);
  if (TSDB_CODE_SUCCESS != code) {
    return code;
  ...
  len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE, "Total_Tables=[%d] Total_Filesets=[%d] Total_Vgroups=[%d]", pData->numOfTables,
  ...
  len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE,
                "--------------------------------------------------------------------------------");
  varDataSetLen(st, len);
  code = colDataSetVal(pColInfo, row++, st, false);
  if (TSDB_CODE_SUCCESS != code) {
  ...
  for (int32_t i = 0; i < tListLen(pData->blockRowsHisto); ++i) {
    len = snprintf(st + VARSTR_HEADER_SIZE, sizeof(st) - VARSTR_HEADER_SIZE, "%04d |", pData->defMinRows + bucketRange * (i + 1));
    ...
    for (int32_t j = 0; j < num; ++j) {
      int32_t x = snprintf(st + VARSTR_HEADER_SIZE + len, sizeof(st) - VARSTR_HEADER_SIZE - len, "%c", '|');
      len += x;
    }

    if (pData->blockRowsHisto[i] > 0) {
      double v = pData->blockRowsHisto[i] * 100.0 / pData->numOfBlocks;
      len += snprintf(st + VARSTR_HEADER_SIZE + len, sizeof(st) - VARSTR_HEADER_SIZE - len, "  %d (%.2f%c)", pData->blockRowsHisto[i], v, '%');
    }
