...
static int32_t strtcSendCalcReq(SSTriggerRealtimeContext *pContext) {
  ...
  for (int32_t i = 0; i < TARRAY_SIZE(pReq->params); ++i) {
    ...
    ST_TASK_ILOG("[calc param %d]: gid=%" PRId64 ", wstart=%" PRId64 ", wend=%" PRId64 ", nrows=%" PRId64 ", create=%d", i, pReq->gid,
                 pParam->wstart, pParam->wend, pParam->wrownum, pReq->createTable);
  }
