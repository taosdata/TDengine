...
static int32_t physiSysTableScanNodeToMsg(const void* pObj, STlvEncoder* pEncoder) {
  ...
  if (TSDB_CODE_SUCCESS == code && pNode->pReadUids != NULL) {
    ...
    int32_t bufLen = sizeof(int32_t) + numKeys * sizeof(int64_t);
    char*   buf = taosMemoryMalloc(bufLen);
    if (NULL == buf) {
      return terrno;
    }
    *(int32_t*)buf = numKeys;
    int64_t* p = (int64_t*)(buf + sizeof(int32_t));
    void*    pIter = NULL;
    int32_t  iter = 0;
    while ((pIter = tSimpleHashIterate(pNode->pReadUids, pIter, &iter)) != NULL) {
      int64_t* key = tSimpleHashGetKey(pIter, NULL);
      *p++ = *key;
    }
    code = tlvEncodeBinary(pEncoder, PHY_SYSTABLE_SCAN_CODE_READ_UIDS, buf, bufLen);
    taosMemoryFree(buf);
  }
    ...
    switch (pTlv->type) {
      ...
      case PHY_SYSTABLE_SCAN_CODE_READ_UIDS: {
        ...
        if (TSDB_CODE_SUCCESS == code) {
          int32_t numKeys = *(int32_t*)buf;
          if (numKeys > 0) {
            pNode->pReadUids = tSimpleHashInit(numKeys, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BIGINT));
            if (NULL == pNode->pReadUids) {
              code = terrno;
            } else {
              int64_t* p = (int64_t*)(buf + sizeof(int32_t));
              for (int32_t i = 0; i < numKeys && TSDB_CODE_SUCCESS == code; ++i) {
                code = tSimpleHashPut(pNode->pReadUids, p, sizeof(int64_t), NULL, 0);
                p++;
              }
            }
          }
