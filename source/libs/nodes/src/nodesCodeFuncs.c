...
static int32_t jsonToSchema(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkSchemaName, pNode->name, sizeof(pNode->name));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToRefCol(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkRefColDbName, pCol->refDbName, sizeof(pCol->refDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkRefColTableName, pCol->refTableName, sizeof(pCol->refTableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkRefColColName, pCol->refColName, sizeof(pCol->refColName));
  }
...
static int32_t jsonToEp(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkEndPointFqdn, pNode->fqdn, sizeof(pNode->fqdn));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToName(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkNameDbName, pNode->dbname, sizeof(pNode->dbname));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkNameTableName, pNode->tname, sizeof(pNode->tname));
  }
...
static int32_t jsonToPhysiAnomalyWindowNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code =
        tjsonGetStringValue(pJson, jkAnomalyWindowPhysiPlanAnomalyOption, pNode->anomalyOpt, sizeof(pNode->anomalyOpt));
  }
...
static int32_t jsonToPhysiQueryInsertNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkQueryInsertPhysiPlanTableFName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToPhysiDeleteNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDeletePhysiPlanTableFName, pNode->tableFName, sizeof(pNode->tableFName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDeletePhysiPlanTsColName, pNode->tsColName, sizeof(pNode->tsColName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToOtableCols(const SJson* pJson, void* pObj) {
  ...
  *pCols = taosArrayInit(colNum, sizeof(SColIdName));
  ...
  for (int32_t i = 0; i < colNum; ++i) {
    ...
    code = tjsonGetStringValue(pCol, jkColOtableName, colName, sizeof(colName));
    if (code < 0) return TSDB_CODE_INVALID_JSON_FORMAT;
  ...
  if (TSDB_CODE_SUCCESS == code && hashSize > 0) {
    *pHash = tSimpleHashInit(hashSize, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BINARY));
    ...
    for (int32_t d = 0; d < hashSize; ++d) {
      ...
      code = tjsonGetStringValue(okeyValue, jkOtableHashName, tbName, sizeof(tbName));
      if (code < 0) return TSDB_CODE_INVALID_JSON_FORMAT;
...
static int32_t jsonToSubplan(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkSubplanDbFName, pNode->dbFName, sizeof(pNode->dbFName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkSubplanUser, pNode->user, sizeof(pNode->user));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToExprNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkExprAliasName, pNode->aliasName, sizeof(pNode->aliasName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkExprUserAlias, pNode->userAlias, sizeof(pNode->userAlias));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToColumnNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnTableAlias, pNode->tableAlias, sizeof(pNode->tableAlias));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnColName, pNode->colName, sizeof(pNode->colName));
  }
  if (TSDB_CODE_SUCCESS == code) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnRefDb, pNode->refDbName, sizeof(pNode->refDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnRefTable, pNode->refTableName, sizeof(pNode->refTableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnRefCol, pNode->refColName, sizeof(pNode->refColName));
  }
  if (TSDB_CODE_SUCCESS == code) {
  ...
  switch (pNode->node.resType.type) {
    ...
    case TSDB_DATA_TYPE_GEOMETRY: {
      ...
      if (TSDB_DATA_TYPE_NCHAR == pNode->node.resType.type) {
        ...
        code = tjsonGetStringValue(pJson, jkValueDatum, buf, pNode->node.resType.bytes * 2 + VARSTR_HEADER_SIZE + 1);
        if (code != TSDB_CODE_SUCCESS) {
          taosMemoryFree(buf);
          break;
        }
    ...
    case TSDB_DATA_TYPE_JSON: {
      ...
      code = tjsonGetStringValue(pJson, jkValueDatum, buf, pNode->node.resType.bytes * 2 + 1);
      if (code != TSDB_CODE_SUCCESS) {
        taosMemoryFree(buf);
        break;
      }
...
static int32_t jsonToFunctionNode(const SJson* pJson, void* pObj) {
  ...
  int32_t code = jsonToExprNode(pJson, pObj);
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkFunctionName, pNode->functionName, sizeof(pNode->functionName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToTableNode(const SJson* pJson, void* pObj) {
  ...
  int32_t code = jsonToExprNode(pJson, pObj);
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkTableDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkTableTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkTableTableAlias, pNode->tableAlias, sizeof(pNode->tableAlias));
  }
...
static int32_t jsonToStreamNode(const SJson* pJson, void* pObj) {
  ...
  int32_t      code = jsonToExprNode(pJson, pObj);
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkStreamDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkStreamStreamName, pNode->streamName, sizeof(pNode->streamName));
  }
  return code;
}
...
static int32_t jsonToStreamTagDefNode(const SJson* pJson, void* pObj) {
  ...
  int32_t            code = tjsonGetStringValue(pJson, jkStreamTagDefTagName, pNode->tagName, sizeof(pNode->tagName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToAnomalyWindowNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAnomalyWindowOption, pNode->anomalyOpt, sizeof(pNode->anomalyOpt));
  }
  return code;
}
...
static int32_t jsonToSlotDescNode(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkSlotDescName, pNode->name, sizeof(pNode->name));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToColumnDefNode(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkColumnDefColName, pNode->colName, sizeof(pNode->colName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDatabaseOptions(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDatabaseOptionsPrecision, pNode->precisionStr, sizeof(pNode->precisionStr));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToTableOptions(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkTableOptionsComment, pNode->comment, sizeof(pNode->comment));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToColumnOptions(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkColumnOptionsEncode, pNode->encode, sizeof(pNode->encode));
  code = tjsonGetStringValue(pJson, jkColumnOptionsCompress, pNode->compress, sizeof(pNode->compress));
  code = tjsonGetStringValue(pJson, jkColumnOptionsLevel, pNode->compressLevel, sizeof(pNode->compressLevel));
  return code;
}
...
static int32_t jsonToColumnReference(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkColumnReferenceColumnName, pNode->colName, sizeof(pNode->colName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnReferenceRefDbName, pNode->refDbName, sizeof(pNode->refDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnReferenceRefTableName, pNode->refTableName, sizeof(pNode->refTableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkColumnReferenceRefColumnName, pNode->refColName, sizeof(pNode->refColName));
  }
  return code;
}
...
static int32_t jsonToSelectStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkSelectStmtStmtName, pNode->stmtName, sizeof(pNode->stmtName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToAlterDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkAlterDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToTrimDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkTrimDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToSsMigrateDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkSsMigrateDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
...
static int32_t jsonToCreateMountStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkMountStmtMountName, pNode->mountName, sizeof(pNode->mountName));
  if (TSDB_CODE_SUCCESS == code) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkMountStmtMountPath, pNode->mountPath, sizeof(pNode->mountPath));
  }
...
static int32_t jsonToCreateTableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateSubTableClause(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateSubTableClauseDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateSubTableClauseTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateSubTableClauseUseDbName, pNode->useDbName, sizeof(pNode->useDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateVSubTableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateVSubTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateVSubTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateVSubTableStmtUseDbName, pNode->useDbName, sizeof(pNode->useDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code =
        tjsonGetStringValue(pJson, jkCreateVSubTableStmtUseTableName, pNode->useTableName, sizeof(pNode->useTableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropTableClause(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDropTableClauseDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropTableClauseTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropStableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDropSuperTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropSuperTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropVtableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDropSuperTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropVirtualTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToAlterTableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkAlterTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetTinyIntValue(pJson, jkAlterTableStmtAlterType, &pNode->alterType);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterTableStmtColName, pNode->colName, sizeof(pNode->colName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterTableStmtNewColName, pNode->newColName, sizeof(pNode->newColName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateUserStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateUserStmtUserName, pNode->userName, sizeof(pNode->userName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateUserStmtPassword, pNode->password, sizeof(pNode->password));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateEncryptAlgrStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, "algorithm_id", pNode->algorithmId, sizeof(pNode->algorithmId));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, "name", pNode->name, sizeof(pNode->name));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, "desc", pNode->desc, sizeof(pNode->desc));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, "type", pNode->algrType, sizeof(pNode->algrType));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, "ossl_algr_name", pNode->osslAlgrName, sizeof(pNode->osslAlgrName));
  }
...
static int32_t jsonToAlterUserStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t         code = tjsonGetStringValue(pJson, jkAlterUserStmtUserName, pNode->userName, sizeof(pNode->userName));
#if 0
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetTinyIntValue(pJson, jkAlterUserStmtAlterType, &pNode->alterType);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterUserStmtPassword, pNode->password, sizeof(pNode->password));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropUserStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkDropUserStmtUserName, pNode->userName, sizeof(pNode->userName));
}
...
static int32_t jsonToCreateRoleStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkRoleStmtRoleName, pNode->name, sizeof(pNode->name));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropEncryptAlgrStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkDropEncryptAlgrStmtUserName, pNode->algorithmId, sizeof(pNode->algorithmId));
}
...
static int32_t jsonToUseDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkUseDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
}
...
static int32_t jsonToCreateDnodeStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateDnodeStmtFqdn, pNode->fqdn, sizeof(pNode->fqdn));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToAlterDnodeStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterDnodeStmtConfig, pNode->config, sizeof(pNode->config));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterDnodeStmtValue, pNode->value, sizeof(pNode->value));
  }
...
static int32_t jsonToCreateIndexStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateIndexStmtIndexDbName, pNode->indexDbName, sizeof(pNode->indexDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateIndexStmtIndexName, pNode->indexName, sizeof(pNode->indexName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateIndexStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateIndexStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropIndexStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropIndexStmtIndexDbName, pNode->indexDbName, sizeof(pNode->indexDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropIndexStmtIndexName, pNode->indexName, sizeof(pNode->indexName));
  }
...
static int32_t jsonToCreateAnodeStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkCreateAnodeStmtUrl, pNode->url, sizeof(pNode->url));
}
...
static int32_t jsonToCreateXnodeTaskStmt(const SJson* pJson, void* pObj) {
  ...
  // Decode options.
  return tjsonGetStringValue(pJson, jkCreateXnodeTaskStmtName, pNode->name, sizeof(pNode->name));
}
...
static int32_t jsonToCreateXnodeStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkCreateXnodeStmtUrl, pNode->url, sizeof(pNode->url));
}
...
static int32_t jsonToDropDnodeStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropDnodeStmtFqdn, pNode->fqdn, sizeof(pNode->fqdn));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToCreateTopicStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkCreateTopicStmtTopicName, pNode->topicName, sizeof(pNode->topicName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateTopicStmtSubscribeDbName, pNode->subDbName, sizeof(pNode->subDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropTopicStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDropTopicStmtTopicName, pNode->topicName, sizeof(pNode->topicName));
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropConsumerGroupStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDropCGroupStmtTopicName, pNode->topicName, sizeof(pNode->topicName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropCGroupStmtConsumerGroup, pNode->cgroup, sizeof(pNode->cgroup));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToAlterClusterStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkAlterClusterStmtConfig, pNode->config, sizeof(pNode->config));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterClusterStmtValue, pNode->value, sizeof(pNode->value));
  }
...
static int32_t jsonToAlterLocalStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkAlterLocalStmtConfig, pNode->config, sizeof(pNode->config));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterLocalStmtValue, pNode->value, sizeof(pNode->value));
  }
...
static int32_t jsonToAlterEncryptKeyStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkAlterEncryptKeyStmtNewKey, pNode->newKey, sizeof(pNode->newKey));
  }
...
static int32_t jsonToDescribeStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkDescribeStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDescribeStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
...
static int32_t jsonToCompactDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkCompactDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
}
...
static int32_t jsonToScanDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkScanDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
}
...
static int32_t jsonToCreateStreamStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code =
      tjsonGetStringValue(pJson, jkCreateStreamStmtStreamDbName, pNode->streamDbName, sizeof(pNode->streamDbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateStreamStmtStreamName, pNode->streamName, sizeof(pNode->streamName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateStreamStmtTargetDbName, pNode->targetDbName, sizeof(pNode->targetDbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code =
        tjsonGetStringValue(pJson, jkCreateStreamStmtTargetTabName, pNode->targetTabName, sizeof(pNode->targetTabName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToGrantStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkGrantStmtUserName, pNode->principal, sizeof(pNode->principal));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkGrantStmtObjName, pNode->objName, sizeof(pNode->objName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkNameTableName, pNode->tabName, sizeof(pNode->tabName));
  }
  if (TSDB_CODE_SUCCESS == code) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    if (pNode->optrType == TSDB_ALTER_ROLE_PRIVILEGES) {
      ...
      code = tjsonGetStringValue(pJson, jkRoleStmtRoleName, pNode->roleName, sizeof(pNode->roleName));
    }
  }
...
static int32_t jsonToShowCreateDatabaseStmt(const SJson* pJson, void* pObj) {
  ...
  return tjsonGetStringValue(pJson, jkShowCreateDatabaseStmtDbName, pNode->dbName, sizeof(pNode->dbName));
}
...
static int32_t jsonToShowCreateTableStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkShowCreateTableStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkShowCreateTableStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
...
static int32_t jsonToShowCreateViewStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkShowCreateViewStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkShowCreateViewStmtViewName, pNode->viewName, sizeof(pNode->viewName));
  }
...
static int32_t jsonToShowTableDistributedStmt(const SJson* pJson, void* pObj) {
  ...
  int32_t code = tjsonGetStringValue(pJson, jkShowTableDistributedStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkShowTableDistributedStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
...
static int32_t jsonToCreateTSMAStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateTSMAStmtTsmaName, pNode->tsmaName, sizeof(pNode->tsmaName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateTSMAStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkCreateTSMAStmtTableName, pNode->tableName, sizeof(pNode->tableName));
  }
  if (TSDB_CODE_SUCCESS == code) {
...
static int32_t jsonToDropTSMAStmt(const SJson* pJson, void* pObj) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropTSMAStmtDbName, pNode->dbName, sizeof(pNode->dbName));
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = tjsonGetStringValue(pJson, jkDropTSMAStmtTsmaName, pNode->tsmaName, sizeof(pNode->tsmaName));
  }
  return code;
}
