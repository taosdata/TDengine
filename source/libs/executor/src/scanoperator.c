...
bool needCountEmpty(STableScanInfo* pTableScanInfo) {
  if (!tsCountAlwaysReturnValue || !pTableScanInfo->hasGroupByTag) {
    return false;
  }
  ...
  if (pSup1) {
    ...
    SqlFunctionCtx* pCtx = pSup1->pCtx;
    for (int32_t i = 0; i < pSup1->numOfExprs; ++i) {
      if (pCtx[i].pExpr->pExpr->nodeType == QUERY_NODE_COLUMN) {
        return false;
      }
      if (!hasCountFunc && pCtx[i].pExpr->pExpr->nodeType == QUERY_NODE_FUNCTION) {
        const char* pName = pCtx[i].pExpr->pExpr->_function.functionName;
        if ((strcmp(pName, "count") == 0)) {
          hasCountFunc = true;
        }
      }
    }
...
SOperatorInfo* createTableScanOperatorInfo(STableScanPhysiNode* pTableScanNode, SReadHandle* readHandle,
  ...
  pInfo->needCountEmptyTable = needCountEmpty(pInfo);
