...
static SSDataBlock* buildStreamPartitionResult(SOperatorInfo* pOperator) {
  ...
  qInfo("numexprs:%d, partName:%s, groupId:%"PRIu64, pInfo->tbnameCalSup.numOfExprs, pDest->info.parTbName, pParInfo->groupId);
  ...
  if (winCode != TSDB_CODE_SUCCESS) {
    ...
    if (pTableSup->numOfExprs > 0) {
      ...
      if (colDataIsNull_s(pTbCol, pDestBlock->info.rows - 1)) {
        ...
        QUERY_CHECK_CODE(code, lino, _end);
        qInfo("%s child_table_name tbName:%s, groupId%"PRIu64, __FUNCTION__, tbName, groupId);
      }
    ...
    qInfo("not get name from rocksdb, partName:%s, groupId:%"PRIu64, pDestBlock->info.parTbName, groupId);
    ...
    qInfo("get name from rocksdb, partName:%s, groupId:%"PRIu64, pSrcBlock->info.parTbName, groupId);
  }
