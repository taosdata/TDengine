...
int32_t buildVirtualSuperTableScanChildTableMap(SOperatorInfo* pOperator) {
  ...
  while (true) {
    ...
    SColumnInfoData *pRefVerCol = taosArrayGet(pChildInfo->pDataBlock, 8);
    ...
    for (int32_t i = 0; i < pChildInfo->info.rows; i++) {
      if (!colDataIsNull_s(pStbNameCol, i)) {
        ...
        if (strncmp(varDataVal(stbrawname), pInfo->vtbScan.tbName, varDataLen(stbrawname)) == 0 &&
            strlen(pInfo->vtbScan.tbName) == varDataLen(stbrawname) &&
...
int32_t buildVirtualNormalChildTableScanChildTableMap(SOperatorInfo* pOperator) {
  ...
  while (true) {
    ...
    SColumnInfoData *pColNameCol = taosArrayGet(pTableInfo->pDataBlock, 3);
    SColumnInfoData *pUidCol = taosArrayGet(pTableInfo->pDataBlock, 4);
    SColumnInfoData *pColIdCol = taosArrayGet(pTableInfo->pDataBlock, 5);
    SColumnInfoData *pRefCol = taosArrayGet(pTableInfo->pDataBlock, 6);
    SColumnInfoData *pVgIdCol = taosArrayGet(pTableInfo->pDataBlock, 7);
    ...
    for (int32_t i = 0; i < pTableInfo->info.rows; i++) {
      ...
      if (!colDataIsNull_s(pTableNameCol, i)) {
        ...
        char* dbrawname = colDataGetData(pDbNameCol, i);
        char* colrawname = colDataGetData(pColNameCol, i);
        QUERY_CHECK_NULL(tbrawname, code, line, _return, terrno)
        QUERY_CHECK_NULL(dbrawname, code, line, _return, terrno)
        QUERY_CHECK_NULL(colrawname, code, line, _return, terrno)

        if (strncmp(varDataVal(tbrawname), pInfo->vtbScan.tbName, varDataLen(tbrawname)) == 0 &&
            strlen(pInfo->vtbScan.tbName) == varDataLen(tbrawname) &&
            strncmp(varDataVal(dbrawname), pInfo->vtbScan.dbName, varDataLen(dbrawname)) == 0 &&
            strlen(pInfo->vtbScan.dbName) == varDataLen(dbrawname)) {
          SColRefInfo info = {0};
          if (colDataIsNull_s(pRefCol, i)) {
            info.colrefName = NULL;
          } else {
            char *refrawname = colDataGetData(pRefCol, i);
            info.colrefName = taosMemoryCalloc(varDataTLen(refrawname), 1);
            QUERY_CHECK_NULL(info.colrefName, code, line, _return, terrno)
            memcpy(info.colrefName, varDataVal(refrawname), varDataLen(refrawname));
            info.colrefName[varDataLen(refrawname)] = 0;
          }

          info.colName = taosMemoryCalloc(varDataTLen(colrawname), 1);
          QUERY_CHECK_NULL(info.colName, code, line, _return, terrno)
          memcpy(info.colName, varDataVal(colrawname), varDataLen(colrawname));
          info.colName[varDataLen(colrawname)] = 0;
          if (!colDataIsNull_s(pUidCol, i)) {
            GET_TYPED_DATA(info.uid, int64_t, TSDB_DATA_TYPE_BIGINT, colDataGetNumData(pUidCol, i), 0);
          }
          if (!colDataIsNull_s(pColIdCol, i)) {
            GET_TYPED_DATA(info.colId, int32_t, TSDB_DATA_TYPE_INT, colDataGetNumData(pColIdCol, i), 0);
          }
          if (!colDataIsNull_s(pVgIdCol, i)) {
            GET_TYPED_DATA(info.vgId, int32_t, TSDB_DATA_TYPE_INT, colDataGetNumData(pVgIdCol, i), 0);
          }
          if (rversion != pVtbScan->rversion || pVtbScan->lastOrgTbVg == NULL) {
            ...
            code = taosHashPut(pVtbScan->curOrgTbVg, &info.vgId, sizeof(info.vgId), NULL, 0);
            QUERY_CHECK_CODE(code, line, _return);
          }
