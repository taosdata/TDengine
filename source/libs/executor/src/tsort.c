 ...
 * */
static size_t tupleAddField(char* t, uint32_t colNum, uint32_t offset, uint32_t colIdx, void* data, size_t length,
                            bool isNull, uint32_t tupleLen) {
  tupleSetOffset(t, colIdx, offset);
  if (isNull) {
    tupleSetNull(t, colIdx, colNum);
  } else {
    if (offset + length > tupleLen + tupleGetDataStartOffset(colNum)) taosMemoryRealloc(t, offset + length);
    tupleSetData(t, offset, data, length);
  }
  return offset + length;
}

static void* tupleGetField(void* t, uint32_t colIdx, uint32_t colNum) {
    ...
    *pSource = NULL;
    ...
    while (1) {
      ...
      if (size > sortBufSize) {
        ...
        if (pHandle->maxRows >= 0)
          blockDataKeepFirstNRows(pHandle->pDataBlock, pHandle->maxRows);
    ...
    for (size_t rowIdx = 0; rowIdx < pBlock->info.rows; ++rowIdx) {
      ...
      for (size_t colIdx = 0; colIdx < colNum; ++colIdx) {
        ...
        offset = tupleAddField(pTuple, colNum, offset, colIdx, colDataGetData(pCol, rowIdx), colLen,
                               colDataIsNull_s(pCol, rowIdx), tupleLen);
      }
      // push into heap
      pqNode.data = pTuple;
