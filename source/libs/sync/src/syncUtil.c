...
void syncPrintNodeLog(const char* flags, ELogLevel level, int32_t dflag, SSyncNode* pNode, const char* format, ...) {
  ...
  if (pNode != NULL) {
    taosPrintLog(flags, level, dflag,
                 ...
                 ", chging:%d, restore:%d, quorum:%d, elect-lc-timer:%" PRId64
                 ...
                 pNode->restoreFinish, syncNodeDynamicQuorum(pNode), pNode->electTimerLogicClock, bufferStatesStr, replMgrStatesStr, cfgStr, hbTimeStr, hbrTimeStr);
  }
}
