  ...
  if (IS_SYS_DBNAME(dbName)) {
    ...
    len +=
        tsnprintf(buf2 + VARSTR_HEADER_SIZE, SHOW_CREATE_DB_RESULT_FIELD2_LEN - VARSTR_HEADER_SIZE,
                  "CREATE DATABASE `%s` BUFFER %d CACHESIZE %d CACHESHARDBITS %d CACHEMODEL '%s' COMP %d DURATION %s "
                  ...
                  dbName, pCfg->buffer, pCfg->cacheSize, pCfg->cacheShardBits, cacheModelStr(pCfg->cacheLast), pCfg->compression, durationStr,
