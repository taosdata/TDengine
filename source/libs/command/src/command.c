...
static int32_t setCreateDBResultIntoDataBlock(SSDataBlock* pBlock, char* dbName, char* dbFName, SDbCfgInfo* pCfg) {
  ...
  if (IS_SYS_DBNAME(dbName)) {
    len += tsnprintf(buf2 + BLOBSTR_HEADER_SIZE, SHOW_CREATE_DB_RESULT_FIELD2_LEN - BLOBSTR_HEADER_SIZE,
    ...
    len += tsnprintf(buf2 + BLOBSTR_HEADER_SIZE, SHOW_CREATE_DB_RESULT_FIELD2_LEN - BLOBSTR_HEADER_SIZE,
                     ...
                     encryptAlgorithmStr(pCfg->encryptAlgorithm), pCfg->ssChunkSize, pCfg->ssKeepLocal, pCfg->ssCompact,
                     compactIntervalStr, compactStartTimeStr, compactEndTimeStr, pCfg->compactTimeOffset);

    if (pRetentions) {
      len += tsnprintf(buf2 + BLOBSTR_HEADER_SIZE + len, SHOW_CREATE_DB_RESULT_FIELD2_LEN - BLOBSTR_HEADER_SIZE,
