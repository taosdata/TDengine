...
cmd ::= GRANT ROLE role_name(A) TO role_name(C).                                  { pCxt->pRootNode = createGrantStmt(pCxt, &A, NULL, &C, NULL, TSDB_GRANT_TYPE_ROLE); }
cmd ::= REVOKE ROLE role_name(A) FROM role_name(C).                               { pCxt->pRootNode = createRevokeStmt(pCxt, &A, NULL, &C, NULL, TSDB_GRANT_TYPE_ROLE); }
/************************************************ grant/revoke ********************************************************/
...
priv_type(A) ::= READ.                                                            { A = PRIV_TYPE(PRIV_TYPE_READ); }
priv_type(A) ::= WRITE.                                                           { A = PRIV_TYPE(PRIV_TYPE_WRITE); }
priv_type(A) ::= ALTER.                                                           { A = PRIV_TYPE(PRIV_TYPE_ALTER); }

priv_type(A) ::= CREATE DATABASE.                                                 { A = PRIV_TYPE(PRIV_DB_CREATE); }
priv_type(A) ::= ALTER DATABASE.                                                  { A = PRIV_TYPE(PRIV_DB_ALTER); }
priv_type(A) ::= DROP DATABASE.                                                   { A = PRIV_TYPE(PRIV_DB_DROP); }
priv_type(A) ::= USE DATABASE.                                                    { A = PRIV_TYPE(PRIV_DB_USE); }
priv_type(A) ::= FLUSH DATABASE.                                                  { A = PRIV_TYPE(PRIV_DB_FLUSH); }
priv_type(A) ::= COMPACT DATABASE.                                                { A = PRIV_TYPE(PRIV_DB_COMPACT); }
priv_type(A) ::= TRIM DATABASE.                                                   { A = PRIV_TYPE(PRIV_DB_TRIM); }
priv_type(A) ::= ROLLUP DATABASE.                                                 { A = PRIV_TYPE(PRIV_DB_ROLLUP); }
priv_type(A) ::= SCAN DATABASE.                                                   { A = PRIV_TYPE(PRIV_DB_SCAN); }
...
priv_type(A) ::= SHOW DATABASES.                                                  { A = PRIV_TYPE(PRIV_SHOW_DATABASES); }
priv_type(A) ::= SHOW VNODES.                                                     { A = PRIV_TYPE(PRIV_SHOW_VNODES); }
priv_type(A) ::= SHOW VGROUPS.                                                    { A = PRIV_TYPE(PRIV_SHOW_VGROUPS); }
priv_type(A) ::= SHOW COMPACTS.                                                   { A = PRIV_TYPE(PRIV_SHOW_COMPACTS); }
priv_type(A) ::= SHOW RETENTIONS.                                                 { A = PRIV_TYPE(PRIV_SHOW_RETENTIONS); }
priv_type(A) ::= SHOW SCANS.                                                      { A = PRIV_TYPE(PRIV_SHOW_SCANS); }
priv_type(A) ::= SHOW SSMIGRATES.                                                 { A = PRIV_TYPE(PRIV_SHOW_SSMIGRATES); }

priv_type(A) ::= CREATE TABLE.                                                    { A = PRIV_TYPE(PRIV_TBL_CREATE); }
priv_type(A) ::= DROP TABLE.                                                      { A = PRIV_TYPE(PRIV_TBL_DROP); }
...
priv_type(A) ::= SHOW FUNCTIONS.                                                  { A = PRIV_TYPE(PRIV_FUNC_SHOW); }

priv_type(A) ::= CREATE INDEX.                                                    { A = PRIV_TYPE(PRIV_IDX_CREATE); }
priv_type(A) ::= DROP INDEX.                                                      { A = PRIV_TYPE(PRIV_IDX_DROP); }
priv_type(A) ::= SHOW INDEXES.                                                    { A = PRIV_TYPE(PRIV_IDX_SHOW); }
...
priv_type(A) ::= SET USER SECURITY INFO.                                          { A = PRIV_TYPE(PRIV_USER_SET_SECURITY); }
priv_type(A) ::= SET USER AUDIT INFO.                                             { A = PRIV_TYPE(PRIV_USER_SET_AUDIT); }
priv_type(A) ::= SET USER BASIC INFO.                                             { A = PRIV_TYPE(PRIV_USER_SET_BASIC); }
priv_type(A) ::= ENABLE USER.                                                     { A = PRIV_TYPE(PRIV_USER_ENABLE); }
priv_type(A) ::= DISABLE USER.                                                    { A = PRIV_TYPE(PRIV_USER_DISABLE); }
priv_type(A) ::= SHOW USERS.                                                      { A = PRIV_TYPE(PRIV_USER_SHOW); }

priv_type(A) ::= CREATE AUDIT DATABASE.                                           { A = PRIV_TYPE(PRIV_AUDIT_DB_CREATE); }
priv_type(A) ::= DROP AUDIT DATABASE.                                             { A = PRIV_TYPE(PRIV_AUDIT_DB_DROP); }
priv_type(A) ::= ALTER AUDIT DATABASE.                                            { A = PRIV_TYPE(PRIV_AUDIT_DB_ALTER); }
priv_type(A) ::= READ AUDIT DATABASE.                                             { A = PRIV_TYPE(PRIV_AUDIT_DB_READ); }
priv_type(A) ::= WRITE AUDIT DATABASE.                                            { A = PRIV_TYPE(PRIV_AUDIT_DB_WRITE); }
...
priv_type(A) ::= GRANT SYSDBA PRIVILEGE.                                          { A = PRIV_TYPE(PRIV_GRANT_SYSDBA); }
priv_type(A) ::= REVOKE SYSDBA PRIVILEGE.                                         { A = PRIV_TYPE(PRIV_REVOKE_SYSDBA); }
priv_type(A) ::= GRANT SYSSEC PRIVILEGE.                                          { A = PRIV_TYPE(PRIV_GRANT_SYSSEC); }
priv_type(A) ::= REVOKE SYSSEC PRIVILEGE.                                         { A = PRIV_TYPE(PRIV_REVOKE_SYSSEC); }
priv_type(A) ::= GRANT SYSAUDIT PRIVILEGE.                                        { A = PRIV_TYPE(PRIV_GRANT_SYSAUDIT); }
priv_type(A) ::= REVOKE SYSAUDIT PRIVILEGE.                                       { A = PRIV_TYPE(PRIV_REVOKE_SYSAUDIT); }
...
priv_type(A) ::= SHOW TOPICS.                                                     { A = PRIV_TYPE(PRIV_TOPIC_SHOW); }
priv_type(A) ::= SHOW CONSUMERS.                                                  { A = PRIV_TYPE(PRIV_CONSUMER_SHOW); }
priv_type(A) ::= SHOW SUBSCRIPTIONS.                                              { A = PRIV_TYPE(PRIV_SUBSCRIPTION_SHOW); }
...
priv_type(A) ::= SHOW TRANS.                                                      { A = PRIV_TYPE(PRIV_TRANS_SHOW); }
priv_type(A) ::= KILL TRANS.                                                      { A = PRIV_TYPE(PRIV_TRANS_KILL); }
priv_type(A) ::= SHOW CONNECTIONS.                                                { A = PRIV_TYPE(PRIV_CONNECTION_SHOW); }
priv_type(A) ::= KILL CONNECTION.                                                 { A = PRIV_TYPE(PRIV_CONNECTION_KILL); }
priv_type(A) ::= SHOW QUERIES.                                                    { A = PRIV_TYPE(PRIV_QUERY_SHOW); }
priv_type(A) ::= KILL QUERY.                                                      { A = PRIV_TYPE(PRIV_QUERY_KILL); }

priv_type(A) ::= USE INFORMATION_SCHEMA.                                          { A = PRIV_TYPE(PRIV_INFO_SCHEMA_USE); }
priv_type(A) ::= USE PERFORMANCE_SCHEMA.                                          { A = PRIV_TYPE(PRIV_PERF_SCHEMA_USE); }
priv_type(A) ::= READ INFORMATION_SCHEMA LIMIT.                                   { A = PRIV_TYPE(PRIV_INFO_SCHEMA_READ_LIMIT); }
priv_type(A) ::= READ INFORMATION_SCHEMA SECURITY.                                { A = PRIV_TYPE(PRIV_INFO_SCHEMA_READ_SEC); }
priv_type(A) ::= READ INFORMATION_SCHEMA AUDIT.                                   { A = PRIV_TYPE(PRIV_INFO_SCHEMA_READ_AUDIT); }
priv_type(A) ::= READ INFORMATION_SCHEMA BASIC.                                   { A = PRIV_TYPE(PRIV_INFO_SCHEMA_READ_BASIC); }
priv_type(A) ::= READ PERFORMANCE_SCHEMA LIMIT.                                   { A = PRIV_TYPE(PRIV_PERF_SCHEMA_READ_LIMIT); }
priv_type(A) ::= READ PERFORMANCE_SCHEMA BASIC.                                   { A = PRIV_TYPE(PRIV_PERF_SCHEMA_READ_BASIC); }
