...
trigger_type(A) ::= STATE_WINDOW NK_LP expr_or_subquery(B) NK_RP state_window_opt(C) true_for_opt(D).                      { A = createStateWindowNode(pCxt, releaseRawExprNode(pCxt, B), C, D); }
...
twindow_clause_opt(A) ::=
  STATE_WINDOW NK_LP expr_or_subquery(B) NK_RP state_window_opt(C) true_for_opt(D). { A = createStateWindowNode(pCxt, releaseRawExprNode(pCxt, B), C, D); }
...
state_window_opt(A) ::= ZEROTH_STATE NK_LP zeroth_literal(B) NK_RP.               { A = createNodeList(pCxt, B); }
state_window_opt(A) ::= ZEROTH_STATE NK_LP NO_ZEROTH NK_RP.                       { A = NULL; }
state_window_opt(A) ::= EXTEND NK_LP extend_literal(B) NK_RP ZEROTH_STATE NK_LP zeroth_literal(C) NK_RP. { A = addNodeToList(pCxt, createNodeList(pCxt, B), C); }
state_window_opt(A) ::= EXTEND NK_LP extend_literal(B) NK_RP ZEROTH_STATE NK_LP NO_ZEROTH NK_RP. { A = createNodeList(pCxt, B); }
