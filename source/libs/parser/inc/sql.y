...
cmd ::= CREATE VTABLE not_exists_opt(A) full_table_name(B)
  ...
  specific_cols_opt(E) TAGS NK_LP tags_literal_list(F) NK_RP                      { pCxt->pRootNode = createCreateVSubTableClause(pCxt, A, B, C, NULL, D, E, F); }
cmd ::= CREATE VTABLE not_exists_opt(A) full_table_name(B)
  NK_LP column_ref_list(C) NK_RP USING full_table_name(D)
  specific_cols_opt(E) TAGS NK_LP tags_literal_list(F) NK_RP                      { pCxt->pRootNode = createCreateVSubTableClause(pCxt, A, B, NULL, C, D, E, F); }
cmd ::= CREATE VTABLE not_exists_opt(A) full_table_name(B) USING full_table_name(C)
  specific_cols_opt(D) TAGS NK_LP tags_literal_list(E) NK_RP                      { pCxt->pRootNode = createCreateVSubTableClause(pCxt, A, B, NULL, NULL, C, D, E); }
