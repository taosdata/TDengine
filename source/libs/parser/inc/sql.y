...
db_options(A) ::= db_options(B) FLUSH_INTERVAL NK_INTEGER (C).                  { A = setDatabaseOption(pCxt, B, DB_OPTION_FLUSH_INTERVAL, &C); }
...
alter_db_option(A) ::= TK_FLUSH_INTERVAL NK_INTEGER(B).                           { A.type = DB_OPTION_FLUSH_INTERVAL, A.val = B; }
