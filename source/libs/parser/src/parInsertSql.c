...
static int parseTimestampOrInterval(const char** end, SToken* pToken, int16_t timePrec, int64_t* ts, int64_t* interval, SMsgBuf* pMsgBuf) {
...
static int parseTime(const char** end, SToken* pToken, int16_t timePrec, int64_t* time, SMsgBuf* pMsgBuf) {
  ...
  if (TSDB_CODE_SUCCESS != parseTimestampOrInterval(&pTokenEnd, pToken, timePrec, &ts, &interval, pMsgBuf)) {
    return buildSyntaxErrMsg(pMsgBuf, "invalid timestamp format", pToken->z);
  }
  *time = ts;
  ...
  if (token.type == TK_NK_MINUS || token.type == TK_NK_PLUS) {
    ...
    if (TSDB_CODE_SUCCESS != parseTimestampOrInterval(&pTokenEnd, &valueToken, timePrec, &tempTs, &tempInterval, pMsgBuf)) {
      return buildSyntaxErrMsg(pMsgBuf, "invalid timestamp format", pToken->z);
    }
    ...
    if (token.type == TK_NK_MINUS) {
      // not support such as 2h - today()
      if (tempInterval <= 0 || tempTs > 0) {
        return buildSyntaxErrMsg(pMsgBuf, "invalid timestamp format", pToken->z);
      }
      *time = ts - interval;
    } else {
