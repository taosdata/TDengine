...
static int32_t createColumnsByTable(STranslateContext* pCxt, const STableNode* pTable, bool igTags, SNodeList* pList,
  ...
  if (QUERY_NODE_REAL_TABLE == nodeType(pTable)) {
    ...
    int32_t           nums = pMeta->tableInfo.numOfColumns +
                   (igTags ? 0
                           : ((TSDB_SUPER_TABLE == pMeta->tableType || inStreamTriggerClause(pCxt) ||
                               ((SRealTableNode*)pTable)->stbRewrite)
...
static EDealRes translateColumn(STranslateContext* pCxt, SColumnNode** pCol) {
  ...
  if ('\0' != (*pCol)->tableAlias[0]) {
    ...
    if (pCxt->currClause == SQL_CLAUSE_ORDER_BY) {
      if ((isSelectStmt(pCxt->pCurrStmt) && !(*pCol)->node.asParam) || isSetOperator(pCxt->pCurrStmt)) {
...
static int32_t createStreamReqBuildCalcPlan(STranslateContext* pCxt, SQueryPlan* calcPlan, SArray* pVgArray,
                                            SCMCreateStreamReq* pReq) {
  int32_t          code = TSDB_CODE_SUCCESS;
