...
static int32_t buildTSMAAst(STranslateContext* pCxt, SCreateTSMAStmt* pStmt, SMCreateSmaReq* pReq, const char* tbName,
  ...
  for (int32_t idx = 0; idx < partitionTagNum; ++idx) {
    ...
    PAR_ERR_JRET(nodesListAppend(pTriggerPartition, pTagCol));
  }
...
static int32_t buildTSMAAst(STranslateContext* pCxt, SCreateTSMAStmt* pStmt, SMCreateSmaReq* pReq, const char* tbName,
  ...
  PAR_ERR_JRET(nodesCloneList(pStmt->pOptions->pFuncs, &info.pFuncs));
  PAR_ERR_JRET(nodesCloneNode(pStmt->pOptions->pInterval, &info.pInterval));

  SFunctionNode* pTbnameFunc = NULL;
  // append partition by tbname
  PAR_ERR_JRET(createTbnameFunction(&pTbnameFunc));
  if (pTbnameFunc) {
    snprintf(pTbnameFunc->node.userAlias, TSDB_COL_NAME_LEN, "tbname");
    PAR_ERR_JRET(nodesListMakeStrictAppend(&info.pPartitionByList, (SNode*)pTbnameFunc));
  }
  ...
  for (int32_t idx = 0; idx < partitionTagNum; ++idx) {
    pTagCol = NULL;
    PAR_ERR_JRET(createColumnNodeWithName(pTags[idx].name, &pTagCol));
    PAR_ERR_JRET(nodesListAppend(info.pPartitionByList, pTagCol));
    SNode* pNew = NULL;
    PAR_ERR_JRET(nodesCloneNode(pTagCol, &pNew));
    PAR_ERR_JRET(nodesListMakeStrictAppend(&info.pTags, pNew));
  }
  ...
  pTagCol = NULL;
  if (pTags && numOfTags > 0) {
    PAR_ERR_JRET(createColumnNodeWithName(pTags[numOfTags - 1].name, &pTagCol));
  }

  PAR_ERR_JRET(buildTSMAAstStreamSubTable(pStmt, pReq, pStmt->pOptions->recursiveTsma ? pTagCol : (SNode*)pTbnameFunc,
                                          (SNode**)&pSubTable));
  info.pSubTable = (SNode*)pSubTable;

  if (pStmt->pOptions->recursiveTsma) {
    PAR_ERR_JRET(nodesListMakeStrictAppend(&info.pTags, pTagCol));
  } else {
    SNode* pNew = NULL;
    PAR_ERR_JRET(nodesCloneNode((SNode*)pTbnameFunc, &pNew));
    PAR_ERR_JRET(nodesListMakeStrictAppend(&info.pTags, pNew));
  }

  if (!pStmt->pOptions->recursiveTsma) {
    PAR_ERR_JRET(fmCreateStateFuncs(info.pFuncs));
  }

  int32_t pProjectionTotalLen = 0;
  PAR_ERR_JRET(buildSampleAst(pCxt, &info, &pReq->ast, &pReq->astLen, &pReq->expr, &pReq->exprLen, &pProjectionTotalLen));
  if (pProjectionTotalLen > TSDB_MAX_BYTES_PER_ROW) {
    PAR_ERR_JRET(TSDB_CODE_PAR_INVALID_ROW_LENGTH);
  }
...
static int32_t buildCreateTSMAReq(STranslateContext* pCxt, SCreateTSMAStmt* pStmt, SMCreateSmaReq* pReq,
  ...
  PAR_ERR_JRET(buildTSMAAst(pCxt, pStmt, pReq, pStmt->pOptions->recursiveTsma ? pRecursiveTsma->targetTb : pStmt->tableName,
                            numOfTags, pTags));
...
static int32_t translateCreateTSMA(STranslateContext* pCxt, SCreateTSMAStmt* pStmt) {
  ...
  if (code == TSDB_CODE_SUCCESS) {
    code = buildCreateTSMAReq(pCxt, pStmt, &pReq, &useTbName);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = collectUseTable(&useTbName, pCxt->pTargetTables);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = buildCmdMsg(pCxt, TDMT_MND_CREATE_TSMA, (FSerializeFunc)tSerializeSMCreateSmaReq, &pReq);
  }
  return code;
}
