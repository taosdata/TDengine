...
static int32_t checkColRef(STranslateContext* pCxt, char* pRefDbName, char* pRefTableName, char* pRefColName,
  ...
  if (pRefTableMeta->tableInfo.precision != precision) {
    PAR_ERR_JRET(generateSyntaxErrMsgExt(&pCxt->msgBuf, TSDB_CODE_PAR_INVALID_REF_COLUMN_TYPE, "virtual table's reference table's time precision and virtual table's time precision not match"));
  }
