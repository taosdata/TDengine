...
static int32_t rewriteCreateVSubTableImpl(STranslateContext* pCxt, SCreateVSubTableStmt* pStmt, SHashObj* pVgroupHashmap) {
  int32_t     code = TSDB_CODE_SUCCESS;
  STableMeta* pSuperTableMeta = NULL;
  SName       name = {0};
  STag*       pTag = NULL;
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = addCreateVCTbReqIntoVgroup(pVgroupHashmap, pStmt, pSuperTableMeta, tagName,
                                      pSuperTableMeta->tableInfo.numOfTags, pTag, &info);
  } else {
    taosMemoryFree(pTag);
  }
