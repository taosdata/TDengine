  ...
  if (TSDB_CODE_SUCCESS == code) {
    ...
    FOREACH(pNode, pStmt->pCols) {
      ...
      if (pOptions && pOptions->hasRef) {
        ...
        if (TSDB_CODE_SUCCESS == code) {
          code = reserveVStbRefDbsInCache(pCxt->pParseCxt->acctId, pOptions->refDb, pOptions->refTable,
                                          pCxt->pMetaCache);
        }
        if (TSDB_CODE_SUCCESS == code) {
          code =
...
static int32_t collectMetaKeyFromCreateInheritedVStable(SCollectMetaKeyCxt* pCxt, SCreateInheritedVStableStmt* pStmt) {
  int32_t code = reserveDbCfgInCache(pCxt->pParseCxt->acctId, pStmt->dbName, pCxt->pMetaCache);
  if (TSDB_CODE_SUCCESS == code && strlen(pStmt->parentDbName) > 0 &&
      strcmp(pStmt->parentDbName, pStmt->dbName) != 0) {
    code = reserveDbCfgInCache(pCxt->pParseCxt->acctId, pStmt->parentDbName, pCxt->pMetaCache);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = reserveTableMetaInCache(pCxt->pParseCxt->acctId, pStmt->parentDbName, pStmt->parentTableName, pCxt->pMetaCache);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = reserveTableVgroupInCache(pCxt->pParseCxt->acctId, pStmt->dbName, pStmt->tableName, pCxt->pMetaCache);
  }
...
static int32_t collectMetaKeyFromShowTags(SCollectMetaKeyCxt* pCxt, SShowStmt* pStmt) {
  ...
  if (TSDB_CODE_SUCCESS == code) {
    code = reserveVStbRefDbsInCache(pCxt->pParseCxt->acctId, ((SValueNode*)pStmt->pDbName)->literal,
                                    ((SValueNode*)pStmt->pTbName)->literal, pCxt->pMetaCache);
  }
  if (TSDB_CODE_SUCCESS == code) {
    code = reserveDbVgInfoInCache(pCxt->pParseCxt->acctId, ((SValueNode*)pStmt->pDbName)->literal, pCxt->pMetaCache);
...
static int32_t collectMetaKeyFromShowVstableInherits(SCollectMetaKeyCxt* pCxt, SShowStmt* pStmt) {
  return reserveTableMetaInCache(pCxt->pParseCxt->acctId, TSDB_INFORMATION_SCHEMA_DB, TSDB_INS_TABLE_VSTABLE_INHERITS,
                                 pCxt->pMetaCache);
}
...
static int32_t collectMetaKeyFromQuery(SCollectMetaKeyCxt* pCxt, SNode* pStmt) {
  ...
  switch (nodeType(pStmt)) {
    ...
    case QUERY_NODE_CREATE_INHERITED_VSTABLE_STMT:
      code = collectMetaKeyFromCreateInheritedVStable(pCxt, (SCreateInheritedVStableStmt*)pStmt);
      break;
