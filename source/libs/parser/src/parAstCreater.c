...
SNode* createStreamNotifyOptions(SAstCreateContext* pCxt, SNodeList* pAddrUrls, SNodeList* pEventTypes) {
  ...
  if (pAddrUrls == NULL || LIST_LENGTH(pAddrUrls) == 0) {
    pCxt->errCode =
    ...
    if (strlen(url) >= TSDB_STREAM_NOTIFY_URL_LEN) {
      pCxt->errCode =
          generateSyntaxErrMsgExt(&pCxt->msgBuf, TSDB_CODE_PAR_SYNTAX_ERROR,
                                  "notification address cannot exceed maximum length %d", TSDB_STREAM_NOTIFY_URL_LEN);
      goto _err;
    }
    if (!validateHttpUrl(url)) {
      pCxt->errCode = generateSyntaxErrMsgExt(&pCxt->msgBuf, TSDB_CODE_PAR_SYNTAX_ERROR, "invalid notification address");
      goto _err;
    }
  }

  if (pEventTypes == NULL || LIST_LENGTH(pEventTypes) == 0) {
    pCxt->errCode = generateSyntaxErrMsgExt(&pCxt->msgBuf, TSDB_CODE_PAR_SYNTAX_ERROR,
