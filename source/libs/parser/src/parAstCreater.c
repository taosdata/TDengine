...
SUserOptions* createDefaultUserOptions(SAstCreateContext* pCxt) {
  ...
  pOptions->sessionPerUser = USER_OPTION_SESSION_PER_USER_DEFAULT;
  pOptions->connectTime = USER_OPTION_CONNECT_TIME_DEFAULT;
  pOptions->connectIdleTime = USER_OPTION_CONNECT_IDLE_TIME_DEFAULT;
  pOptions->callPerSession = USER_OPTION_CALL_PER_SESSION_DEFAULT;
  pOptions->vnodePerCall = -1; // not implemented yet, so use -1 instead of USER_OPTION_VNODE_PER_CALL_DEFAULT;
  pOptions->failedLoginAttempts = USER_OPTION_FAILED_LOGIN_ATTEMPTS_DEFAULT;
  pOptions->passwordLifeTime = USER_OPTION_PASSWORD_LIFE_TIME_DEFAULT;
  pOptions->passwordReuseTime = USER_OPTION_PASSWORD_GRACE_TIME_DEFAULT;
  pOptions->passwordReuseMax = USER_OPTION_PASSWORD_REUSE_MAX_DEFAULT;
  pOptions->passwordLockTime = USER_OPTION_PASSWORD_LOCK_TIME_DEFAULT;
  pOptions->passwordGraceTime = USER_OPTION_PASSWORD_GRACE_TIME_DEFAULT;
  pOptions->inactiveAccountTime = USER_OPTION_INACTIVE_ACCOUNT_TIME_DEFAULT;
  pOptions->allowTokenNum = USER_OPTION_ALLOW_TOKEN_NUM_DEFAULT;
  return pOptions;
...
static bool isValidUserOptions(SAstCreateContext* pCxt, const SUserOptions* opts) {
  ...
  if (opts->hasPasswordReuseTime && (opts->passwordReuseTime < 0 || opts->passwordReuseTime > USER_OPTION_PASSWORD_REUSE_TIME_MAX)) {
    pCxt->errCode = generateSyntaxErrMsg(&pCxt->msgBuf, TSDB_CODE_PAR_INVALID_OPTION_VALUE, "PASSWORD_REUSE_TIME");
    return false;
  }

  if (opts->hasPasswordReuseMax && (opts->passwordReuseMax < 0 || opts->passwordReuseMax > USER_OPTION_PASSWORD_REUSE_MAX_MAX)) {
...
SNode* createCreateUserStmt(SAstCreateContext* pCxt, SToken* pUserName, SUserOptions* opts, bool ignoreExisting) {
  ...
  /*
  CHECK_PARSER_STATUS(pCxt);
  char password[TSDB_USER_PASSWORD_LONGLEN + 3] = {0};
  CHECK_NAME(checkUserName(pCxt, pUserName));
  if (is_import == 0) {
