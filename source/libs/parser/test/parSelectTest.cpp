...
TEST_F(ParserSelectTest, selectFunc) {
  ...
  run("SELECT MAX(c1), c2 FROM t1 STATE_WINDOW(c3, 1, NULL)", TSDB_CODE_PAR_INVALID_STATE_WIN_COL);
  ...
  run("SELECT COUNT(*) FROM t1 STATE_WINDOW(c1, c2, 1)", TSDB_CODE_PAR_INVALID_STATE_WIN_COL);
}
