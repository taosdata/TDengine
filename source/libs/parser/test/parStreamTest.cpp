...
TEST_F(ParserStreamTest, testName) {
  ...
  setCreateStreamReq(&expect, "0.test.s2", "0.test", "0.test", "0.test", "0.test", "t1", "stream_out", "create stream s2 interval(1s) sliding(1s) from t1 into stream_out as select _twstart, avg(c1) from t2", 0);
  run("create stream s2 interval(1s) sliding(1s) from t1 into stream_out as select _twstart, avg(c1) from t2");
