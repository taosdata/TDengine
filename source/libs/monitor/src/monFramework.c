...
void monInitOneGroup(char **gauges, int32_t label_count, const char **sample_labels) {
  ...
  for (int32_t i = 0; i < 3; i++) {
    ...
    if (taosHashPut(tsMonitor.metrics, gauges[i], strlen(gauges[i]), &gauge, sizeof(taos_gauge_t *)) != 0) {
      uError("failed to delete metric from hash");
    }
  }
}
...
int32_t monInitMonitorFW() {
  ...
  char       *dnodes_gauges[] = {UPTIME,         CPU_ENGINE,      CPU_SYSTEM,     CPU_CORE,      MEM_ENGINE, MEM_SYSTEM,
                                 MEM_TOTAL,      DISK_ENGINE,     DISK_USED,      DISK_TOTAL,    NET_IN,     NET_OUT,
...
void monGenClusterInfoTable(SMonInfo *pMonitor) {
  ...
  for (int32_t i = 0; i < 18; i++) {
    ...
    if (gauge != NULL) {
      ...
      if (taosHashPut(tsMonitor.metrics, metric_names[i], strlen(metric_names[i]), &gauge, sizeof(taos_gauge_t *)) != 0)
        uError("failed to put metric into hash");
    }
  }
  ...
  if (metric != NULL) taos_gauge_set(*metric, pInfo->master_uptime, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, DBS_TOTAL, strlen(DBS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->dbs_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, TBS_TOTAL, strlen(TBS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->tbs_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, STBS_TOTAL, strlen(STBS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->stbs_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, VGROUPS_TOTAL, strlen(VGROUPS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->vgroups_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, VGROUPS_ALIVE, strlen(VGROUPS_ALIVE));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->vgroups_alive, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, VNODES_TOTAL, strlen(VNODES_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->vnodes_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, VNODES_ALIVE, strlen(VNODES_ALIVE));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->vnodes_alive, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, CONNECTIONS_TOTAL, strlen(CONNECTIONS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->connections_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, TOPICS_TOTAL, strlen(TOPICS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->topics_toal, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, STREAMS_TOTAL, strlen(STREAMS_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->streams_total, sample_label_values);
  ...
  if (metric != NULL) taos_gauge_set(*metric, dnode_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, DNODES_ALIVE, strlen(DNODES_ALIVE));
  if (metric != NULL) taos_gauge_set(*metric, dnode_alive, sample_label_values);
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pInfo->mnodes); ++i) {
    ...
    if (pMnodeDesc == NULL) break;

    bool dnodeIsOnline = false;
    for (int32_t i = 0; i < taosArrayGetSize(pInfo->dnodes); ++i) {
      SMonDnodeDesc *pDnodeDesc = taosArrayGet(pInfo->dnodes, i);
      if (pDnodeDesc == NULL) break;
  ...
  if (metric != NULL) taos_gauge_set(*metric, mnode_total, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, MNODES_ALIVE, strlen(MNODES_ALIVE));
  if (metric != NULL) taos_gauge_set(*metric, mnode_alive, sample_label_values);

  // grant info
  metric = taosHashGet(tsMonitor.metrics, EXPIRE_TIME, strlen(EXPIRE_TIME));
  if (metric != NULL) taos_gauge_set(*metric, pGrantInfo->expire_time, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, TIMESERIES_USED, strlen(TIMESERIES_USED));
  if (metric != NULL) taos_gauge_set(*metric, pGrantInfo->timeseries_used, sample_label_values);

  metric = taosHashGet(tsMonitor.metrics, TIMESERIES_TOTAL, strlen(TIMESERIES_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pGrantInfo->timeseries_total, sample_label_values);
}
...
void monGenVgroupInfoTable(SMonInfo *pMonitor) {
  ...
  if (tableNumGauge != NULL && taos_collector_registry_register_metric(tableNumGauge) == 1) {
    taos_counter_destroy(tableNumGauge);
  }
  taos_gauge_t *statusGauge = taos_gauge_new(STATUS, "", vgroup_label_count, vgroup_sample_labels);
  if (statusGauge != NULL && taos_collector_registry_register_metric(statusGauge) == 1) {
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pInfo->vgroups); ++i) {
    ...
    if (tableNumGauge != NULL) taos_gauge_set(tableNumGauge, pVgroupDesc->tables_num, sample_labels);

    int32_t status = 0;
    if (strcmp(pVgroupDesc->status, "ready") == 0) {
      status = 1;
    }
    if (statusGauge != NULL) taos_gauge_set(statusGauge, status, sample_labels);
  }
}
...
void monGenDnodeInfoTable(SMonInfo *pMonitor) {
  ...
  if (metric != NULL) taos_gauge_set(*metric, pInfo->uptime, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, CPU_ENGINE, strlen(CPU_ENGINE));
  if (metric != NULL) taos_gauge_set(*metric, cpu_engine, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, CPU_SYSTEM, strlen(CPU_SYSTEM));
  if (metric != NULL) taos_gauge_set(*metric, pSys->cpu_system, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, CPU_CORE, strlen(CPU_CORE));
  if (metric != NULL) taos_gauge_set(*metric, pSys->cpu_cores, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, MEM_ENGINE, strlen(MEM_ENGINE));
  if (metric != NULL) taos_gauge_set(*metric, mem_engine, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, MEM_SYSTEM, strlen(MEM_SYSTEM));
  if (metric != NULL) taos_gauge_set(*metric, pSys->mem_system, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, MEM_TOTAL, strlen(MEM_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pSys->mem_total, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DISK_ENGINE, strlen(DISK_ENGINE));
  if (metric != NULL) taos_gauge_set(*metric, pSys->disk_engine, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DISK_USED, strlen(DISK_USED));
  if (metric != NULL) taos_gauge_set(*metric, pSys->disk_used, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DISK_TOTAL, strlen(DISK_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pSys->disk_total, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, NET_IN, strlen(NET_IN));
  if (metric != NULL) taos_gauge_set(*metric, net_in_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, NET_OUT, strlen(NET_OUT));
  if (metric != NULL) taos_gauge_set(*metric, net_out_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, IO_READ, strlen(IO_READ));
  if (metric != NULL) taos_gauge_set(*metric, io_read_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, IO_WRITE, strlen(IO_WRITE));
  if (metric != NULL) taos_gauge_set(*metric, io_write_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, IO_READ_DISK, strlen(IO_READ_DISK));
  if (metric != NULL) taos_gauge_set(*metric, io_read_disk_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, IO_WRITE_DISK, strlen(IO_WRITE_DISK));
  if (metric != NULL) taos_gauge_set(*metric, io_write_disk_rate, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, VNODES_NUM, strlen(VNODES_NUM));
  if (metric != NULL) taos_gauge_set(*metric, pStat->totalVnodes, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, MASTERS, strlen(MASTERS));
  if (metric != NULL) taos_gauge_set(*metric, pStat->masterNum, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, HAS_MNODE, strlen(HAS_MNODE));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->has_mnode, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, HAS_QNODE, strlen(HAS_QNODE));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->has_qnode, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, HAS_SNODE, strlen(HAS_SNODE));
  if (metric != NULL) taos_gauge_set(*metric, pInfo->has_snode, sample_labels);
  ...
  if (metric != NULL) taos_gauge_set(*metric, numOfErrorLogs, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_INFO, strlen(DNODE_LOG_INFO));
  if (metric != NULL) taos_gauge_set(*metric, numOfInfoLogs, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_DEBUG, strlen(DNODE_LOG_DEBUG));
  if (metric != NULL) taos_gauge_set(*metric, numOfDebugLogs, sample_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_TRACE, strlen(DNODE_LOG_TRACE));
  if (metric != NULL) taos_gauge_set(*metric, numOfTraceLogs, sample_labels);
}
...
void monGenDnodeStatusInfoTable(SMonInfo *pMonitor) {
  ...
  if (gauge != NULL && taos_collector_registry_register_metric(gauge) == 1) {
    taos_counter_destroy(gauge);
  }
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pClusterInfo->dnodes); ++i) {
    ...
    if (gauge != NULL) taos_gauge_set(gauge, status, sample_labels);
  }
}
...
void monGenDataDiskTable(SMonInfo *pMonitor) {
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pInfo->datadirs); ++i) {
    ...
    if (metric != NULL) taos_gauge_set(*metric, pDatadirDesc->size.avail, sample_labels);

    metric = taosHashGet(tsMonitor.metrics, DNODE_DATA_USED, strlen(DNODE_DATA_USED));
    if (metric != NULL) taos_gauge_set(*metric, pDatadirDesc->size.used, sample_labels);

    metric = taosHashGet(tsMonitor.metrics, DNODE_DATA_TOTAL, strlen(DNODE_DATA_TOTAL));
    if (metric != NULL) taos_gauge_set(*metric, pDatadirDesc->size.total, sample_labels);
  }
}
...
void monGenLogDiskTable(SMonInfo *pMonitor) {
  ...
  if (metric != NULL) taos_gauge_set(*metric, pLogDesc->size.avail, sample_log_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_USED, strlen(DNODE_LOG_USED));
  if (metric != NULL) taos_gauge_set(*metric, pLogDesc->size.used, sample_log_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_TOTAL, strlen(DNODE_LOG_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pLogDesc->size.total, sample_log_labels);

  const char *sample_temp_labels[] = {cluster_id, dnode_id, pMonitor->dmInfo.basic.dnode_ep, pTempDesc->name};

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_AVAIL, strlen(DNODE_LOG_AVAIL));
  if (metric != NULL) taos_gauge_set(*metric, pTempDesc->size.avail, sample_temp_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_USED, strlen(DNODE_LOG_USED));
  if (metric != NULL) taos_gauge_set(*metric, pTempDesc->size.used, sample_temp_labels);

  metric = taosHashGet(tsMonitor.metrics, DNODE_LOG_TOTAL, strlen(DNODE_LOG_TOTAL));
  if (metric != NULL) taos_gauge_set(*metric, pTempDesc->size.total, sample_temp_labels);
}
...
void monGenMnodeRoleTable(SMonInfo *pMonitor) {
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pInfo->mnodes); ++i) {
    ...
    if (dnodeIsOnline) {
      if (metric != NULL) taos_gauge_set(*metric, pMnodeDesc->syncState, sample_labels);
    } else {
      if (metric != NULL) taos_gauge_set(*metric, 0, sample_labels);
    }
  }
}
...
void monGenVnodeRoleTable(SMonInfo *pMonitor) {
  ...
  for (int32_t i = 0; i < 1; i++) {
    ...
    if (gauge != NULL) {
      ...
      if (taosHashPut(tsMonitor.metrics, vnodes_role_gauges[i], strlen(vnodes_role_gauges[i]), &gauge,
                      sizeof(taos_gauge_t *)) != 0)
        uError("failed to put metirc into hash");
    }
  }
  ...
  for (int32_t i = 0; i < taosArrayGetSize(pInfo->vgroups); ++i) {
    ...
    for (int32_t j = 0; j < TSDB_MAX_REPLICA; ++j) {
      ...
      if (metric != NULL) taos_gauge_set(*metric, pVnodeDesc->syncState, sample_labels);
    }
  }
}
