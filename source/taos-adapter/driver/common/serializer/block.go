...
var ErrColumnNumberNotMatch = errors.New("number of columns does not match")
var ErrDataTypeWrong = errors.New("wrong data type")
...
func SerializeRawBlock(params []*param.Param, colType *param.ColumnType) ([]byte, error) {
	...
	if len(colTypes) != columns {
		return nil, ErrColumnNumberNotMatch
	}
	var block []byte
	//version int32
