...
func (p *TMQRawDataParser) parseBlockInfos() []*TMQBlockInfo {
	...
	for i := int32(0); i < blockNum; i++ {
		...
		blockInfo.RawBlock = tools.AddPointer(p.block, p.offset)
...
func (p *TMQRawDataParser) parseBool() bool {
	v := *(*int8)(tools.AddPointer(p.block, p.offset))
	p.skip(1)
	return v != 0
}

func (p *TMQRawDataParser) parseUint8() uint8 {
	v := *(*uint8)(tools.AddPointer(p.block, p.offset))
	p.skip(1)
	return v
}
...
func (p *TMQRawDataParser) parseInt32() int32 {
	v := *(*int32)(tools.AddPointer(p.block, p.offset))
	p.skip(4)
	return v
}
...
func (p *TMQRawDataParser) parseName() string {
	...
	for i := 0; i < nameLen-1; i++ {
		name[i] = *(*byte)(tools.AddPointer(p.block, p.offset+uintptr(i)))
	}
	p.skip(nameLen)
	return string(name)
}
