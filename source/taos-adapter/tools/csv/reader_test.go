	...
	for _, tt := range readTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			if wantErr := firstError(tt.Errors, positions, errPositions); wantErr != nil {
				...
				var refOut [][]*string
				if tt.Output != nil {
					refOut = make([][]*string, len(tt.Output))
					for i, iv := range tt.Output {
						for j, refS := range iv {
							if j == 0 {
								refOut[i] = make([]*string, len(iv))
							}
							if refS == "" {
								refOut[i][j] = nil
								if tt.NotNil != nil && tt.NotNil[i][j] {
									refOut[i][j] = &tt.Output[i][j]
								}
							} else {
								refOut[i][j] = &tt.Output[i][j]
							}
			...
			for recNum := 0; ; recNum++ {
				...
				var refOut []*string
				if tt.Output[recNum] != nil {
					refOut = make([]*string, len(tt.Output[recNum]))
					for i, s := range tt.Output[recNum] {
						if s == "" {
							refOut[i] = nil
							if tt.NotNil != nil && tt.NotNil[recNum][i] {
								refOut[i] = &tt.Output[recNum][i]
							}
						} else {
							refOut[i] = &tt.Output[recNum][i]
						}
				...
				if got, want := rec, refOut; !reflect.DeepEqual(got, want) {
