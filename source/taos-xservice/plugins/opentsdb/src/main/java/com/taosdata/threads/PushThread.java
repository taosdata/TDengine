...
public class PushThread implements Runnable {
     ...
     */
    private void push(List<OpentsdbDataEntity> opentsdbDataEntityList) {
        // TODO 获取并判断响应
        try {
            // 判断列表不为空
            if (opentsdbDataEntityList != null && opentsdbDataEntityList.size() > 0) {
                // 获取内存中最新的metric信息
                OpentsdbMetricEntity latestMetricEntity = MetricCache.metricMap.get(opentsdbDataEntityList.get(0).getMetric());
                // 全局变量本地化
                Set<String> latestTagSet = new HashSet<>();
                latestTagSet.addAll(latestMetricEntity.getTagSet());
                // 对比内存中数据与最新数据是否有差异
                if (this.tagSet.isEmpty()) {
                    // 使用最新缓存
                    this.tagSet.addAll(latestTagSet);
                } else if (!this.tagSet.equals(latestTagSet)) {
                    // 需要更新
                    opentsdbDataEntityList.forEach(opentsdbDataEntity -> opentsdbDataEntity.getOpentsdbMetricEntity().getTagSet().addAll(latestTagSet));
                    // 数据写回
