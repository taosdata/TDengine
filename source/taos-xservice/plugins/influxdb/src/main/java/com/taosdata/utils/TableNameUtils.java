...
public class TableNameUtils {
    ...
    public static String generateTableName(String pattern, String measurement, Map<String, Object> tags) {
        String tableName;
        // 如果模板为空，使用默认方式生成表名
        if (StringUtils.isEmpty(pattern)) {
            tableName = generateDefaultTableName(measurement, tags);
        } else {
            ...
            while (matcher.find()) {
                ...
                String replacement = "";
                // 特殊变量：${measurement} 表示使用指标名称
                if ("measurement".equals(variable)) {
                    replacement = measurement != null ? measurement : "";
    ...
    private static String generateDefaultTableName(String measurement, Map<String, Object> tags) {
        // 拼接Metric
        StringBuilder tableName = new StringBuilder(measurement != null ? measurement : "");
        // 判断tags是否存在
