...
export function deleteStableReq(payload) {
  const { selected_db, stableName } = payload;
...
export function getStableStructReq(payload) {
  const { selected_db, stableName, type } = payload;
  return sendSQLReq(`DESCRIBE  \`${selected_db}\`` + '.' + `\`${stableName}\`;`, true)
    .then(list => {
      const ts_field_name = list[0]?.field;
      const encode = list[0]?.encode;
      const compress = list[0]?.compress;
...
export function createStableReq(payload) {
  const { selected_db, stable_form } = payload;
  const { name, columns, tags, rollup } = stable_form;
  let rollupValue = '';
  if (rollup.length) {
...
export function changeStableStructOther(data, stableName) {
  const { operation, first_field = '', second_field = '', encode = '', compress = '', level = '' } = data;
  let sql = '';
