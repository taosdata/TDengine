...
test.describe('DataIn - MySQL datasource', () => {
  test('all required fields show errors when empty and Check Connection is clicked', async ({ page }) => {
    ...
    // Select MySQL as datasource type
    await selectElOptionByText(page, 'type', 'MySQL');

    // Wait for MySQL-specific fields to render
    ...
    await expect(portInput).toHaveAttribute('placeholder', '3306');
  ...
  test('errors clear after filling required fields', async ({ page }) => {
    ...
    // Fill MySQL Host
    await hostInput.scrollIntoViewIfNeeded();
    await hostInput.fill('192.168.1.131');
    ...
    // Fill Database
    await databaseInput.scrollIntoViewIfNeeded();
    await databaseInput.fill('test_ci');
    ...
    // Fill SQL
    await sqlInput.scrollIntoViewIfNeeded();
    await sqlInput.fill('select * from test_ci.tb_test_ci where ts > ${start} and ts < ${end}');
  ...
  test('connectivity check succeeds with reachable MySQL server', async ({ page }) => {
    await gotoDataInTask(page);
    await openAddSourceFromList(page);
  ...
  test('connectivity check fails with unreachable MySQL server', async ({ page }) => {
    await gotoDataInTask(page);
    await openAddSourceFromList(page);
    ...
    // Fill all required fields — use wrong port 3307
    await hostInput.fill('192.168.1.45');
    await portInput.clear();
    await portInput.fill('3307');
  ...
  test('create MySQL task, check connection, configure mapping, submit and verify running', async ({ page }) => {
    test.setTimeout(180_000);
    const ts = Date.now();
    const taskName = `mysql_e2e_${ts}`;
    const stableName = `mysql_stable_${ts}`;
    const subtableName = `mysql_table_\${yyear}`;
    const targetDb = 'ci_db_mysql';
    ...
    try {
      ...
      // Wait for MySQL-specific form fields to render after type change
      const hostInput = page.locator('#data\\.connection_options\\.host');
      await expect(hostInput).toBeVisible({ timeout: 10_000 });
      ...
      const startInput = page.locator('input[id^="data.groups_after."][id$=".start"]').first();
      await startInput.scrollIntoViewIfNeeded();
      await startInput.fill('2024-04-16 00:00:00');

      const endInput = page.locator('input[id^="data.groups_after."][id$=".end"]').first();
      await endInput.scrollIntoViewIfNeeded();
      await endInput.fill('2024-04-18 00:00:00');

      // Query Interval — composeAppend type (InputNumberWithSelect), no id on inner input
      // Default is '1d'. To set 12h: change number to 12, select unit 'h'
      const intervalFormItem = page.locator('.el-form-item').filter({ hasText: 'Query Interval' }).first();
      await intervalFormItem.scrollIntoViewIfNeeded();
      ...
      // Change unit from 'd' (Day) to 'h' (Hours)
      ...
      // Read Concurrency — inside collapsed "Advanced Options", expand first
      const advancedHeader = page.locator('.el-collapse-item__header').filter({ hasText: 'Advanced Options' }).first();
      await advancedHeader.scrollIntoViewIfNeeded();
      await advancedHeader.click();
      await page.waitForTimeout(300);
      ...
      // Delay — composeAppend type (InputNumberWithSelect), no id on inner input
      // Default is '0s'. Leave as default (0).
      const delayFormItem = page.locator('.el-form-item').filter({ hasText: 'Delay' }).first();
      await delayFormItem.scrollIntoViewIfNeeded();
      ...
      // Wait for the transformer section to appear after check-connection succeeds
      ...
      // Click "Retrieve From Server" to fetch sample data (required before Create STable is enabled)
      ...
      // Wait for retrieval to complete — the button shows a loading state, then Create STable becomes enabled
      ...
      // The dialog should have columns pre-populated from the sample data
      ...
      // --- Clean up columns before creating the super table ---
      ...
      // Columns to delete: duplicate ts (skip index 0), tblob, bblob, mblob, lblob
      // Column to move to tag: yyear
      // Must iterate in reverse to avoid index shifting when deleting
