...
import { ColumnStruct, CreateTableProps, CreateTableForm } from '../props';
import { isGte3300, tbNameRule, columnRule } from '../utils';
import { t } from 'locales';
import { ElMessage, ElMessageBox, FormInstance } from 'element-plus';
import { composeType } from 'utils/tdengine';
import {
  getNormalTableStructReq,
  createNormalTableReq,
  changeNormalTableStruct,
  changeNormalTableStructData
} from '../../../api';

const props = withDefaults(defineProps<CreateTableProps>(), {
  showTitle: true,
  columnsArray: () => [],
  isEdit: false
});
...
const formData = reactive<CreateTableForm>({
...
const formTitle = computed(() => {
  if (props.isEdit) {
    ...
    return t('stb.createTableInDb', [currentSelectedDb.value]);
  }
});
...
function handleCreateStable() {
  ...
  formIns.value.validate().then(() => {
    processData();
    createNormalTableReq(formData, currentSelectedDb.value)
      .then(() => {
