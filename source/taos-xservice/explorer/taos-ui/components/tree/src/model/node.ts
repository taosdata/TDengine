...
class Node {
  ...
  pageSize: number = 10;
  total: number = 0;
  expandAllChildren: boolean = false;
  ...
  constructor(options: TreeNodeOptions) {
    ...
    this.childNodes = [];
    this.loading = false;
    this.expandAllChildren = this.parent?.expandAllChildren ?? false;
  ...
  setDataAndUpdateChildren(data: TreeNodeData, newChildNode: Node[]): void {
    if (!Array.isArray(data)) {
      markNodeData(this, data);
    }

    this.data = data;
    this.childNodes = newChildNode;
  }

  filter(value) {
    const filterNodeMethod = this.store.filterNodeMethod;
    const traverse = function (node) {
      ...
      if (node.visible && !node.isLeaf && node.data.typeName != 'table') node.expand();
    };
    traverse(this);
  }
  get label(): string {
  ...
  // 重新加载数据
  reloadData(callback?: AnyFunction) {
    this.loading = false;
    this.loaded = false;
  ...
  loadData(callback?: (node: Node) => void, defaultProps: TreeNodeLoadedDefaultProps = {}) {
    if (
      ...
      const resolve: LoadedCallback = (children: Recordable[], total = children.length) => {
        this.total = total;
        this.loading = false;
        this.loaded = true;
        this.childNodes = [];
        this.pagination = children.length != this.total;
  ...
  getNodePath(): string {
    if (null === this.parent) {
      ...
      const parentPath = this.parent.getNodePath();
      return `${parentPath ? parentPath : ''}${this.data?.name ? '/' + this.data.name : ''}`;
    }
  }

  getNodeIDPath(): string {
    if (null === this.parent) {
