...
use crate::{
    ...
    WritePressureState, pending_ack_fut::PendingAckFuture,
};

fn record_permit_pressure(
    state: &Arc<parking_lot::RwLock<WritePressureState>>,
    permit_wait: Duration,
) {
    let mut guard = state.write();
    if permit_wait > Duration::ZERO {
        guard.record_permit_wait(permit_wait);
    }
}

fn batch_timeout_elapsed(new_polling: Option<Instant>) -> Option<Duration> {
    new_polling.map(|started| started.elapsed())
}
...
pub struct MessagesSender<'a> {
    ...
    pub write_pressure: &'a Arc<parking_lot::RwLock<WritePressureState>>,
...
impl<'a> MessagesSender<'a> {
    ...
    async fn send_batch(
        ...
        record_permit_pressure(self.write_pressure, elapsed);
    ...
    pub async fn send_batch_inner(
        ...
        let fut = InspectTimeoutFuture::new(
            PENDING_ACK_TIMEOUT,
            PendingAckFuture::new(batch_id, rx, PENDING_ACK_TIMEOUT),
    ...
    pub async fn send(
        ...
        if self.polling_timeout_ms > 0
            && self.last_polling.elapsed().as_millis() as i64 > self.polling_timeout_ms
        {
            return self.send_batch(pending_futs, cancel).await;
        }

        // Check batch timeout for the currently cached batch.
        if self
            .new_polling
            .is_some_and(|v| v.elapsed() > self.batch_timeout)
        {
            let elapsed = batch_timeout_elapsed(self.new_polling).unwrap_or_default();
            tracing::debug!(
                cache.len = self.value.len(),
                elapsed = ?elapsed,
                "Batch timeout in send(), send cached batch"
            );
            self.new_polling = None;
            return self.send_batch(pending_futs, cancel).await;
        }
...
mod tests {
    ...
    use super::*;
    use crate::{PendingBatches, WritePressureState, context::CustomContext};
    ...
    fn test_consumer(metrics: Arc<CoreMetrics>) -> LoggingConsumer {
        let context = CustomContext::new(metrics);
        let mut client_config = ClientConfig::new();
        client_config.set("bootstrap.servers", "127.0.0.1:1");
        client_config.set("group.id", "message-sender-tests");
        client_config
            .create_with_context(context)
            .expect("test consumer should be created")
    }

    fn append_cached_row(sender: &mut MessagesSender<'_>) {
        sender.timestamp.append_value(1);
        sender.topic.append_value("topic-a");
        sender.partition.append_value(0);
        sender.offset.append_value(42);
        sender.key.append_value(b"key");
        sender.value.append_value(b"value");
    }

    #[test]
    fn record_permit_pressure_marks_state_for_long_permit_wait() {
        let state = Arc::new(RwLock::new(WritePressureState::new(Duration::from_secs(5))));
        record_permit_pressure(&state, Duration::from_secs(6));

        let snapshot = state.read().snapshot();
        assert!(snapshot.write_blocked);
        assert_eq!(Duration::from_secs(6), snapshot.last_permit_wait);
        assert_eq!(Duration::ZERO, snapshot.last_ack_wait);
    }

    #[test]
    fn batch_timeout_elapsed_returns_none_without_polling_start() {
        assert!(batch_timeout_elapsed(None).is_none());
    }

    #[tokio::test]
    async fn send_batch_clears_new_polling_after_successful_send() {
        let metrics = test_metrics();
        let consumer = test_consumer(metrics.clone());
        ...
        let mut sender = MessagesSender {
            ...
            polling_timeout_ms: 1_000,
            last_polling: Instant::now(),
            new_polling: Some(new_polling),
            codec_processor: None,
            codec_err_count: 0,
            timestamp: TimestampNanosecondBuilder::new(),
            topic: StringBuilder::new(),
            partition: Int32Builder::new(),
        ...
        append_cached_row(&mut sender);

        let mut pending_futs = FuturesOrdered::new();
        let cancel = CancellationToken::new();

        let status = sender.send_batch(&mut pending_futs, &cancel).await.unwrap();

        assert!(matches!(status, ExitStatus::Finished));
        assert!(sender.new_polling.is_none());
        assert_eq!(rx.recv_async().await.unwrap().num_rows(), 1);
    }

    #[tokio::test]
    async fn send_flushes_cached_batch_when_polling_timeout_reached() {
        let metrics = test_metrics();
        let consumer = test_consumer(metrics.clone());
        let (tx, rx) = flume::bounded(1);
        let schema = test_schema();
        ...
        let mut sender = MessagesSender {
            ...
            batch_size: 10,
            batch_timeout: Duration::from_secs(30),
            polling_timeout_ms: 1,
            last_polling,
            new_polling: Some(Instant::now()),
            codec_processor: None,
            codec_err_count: 0,
            timestamp: TimestampNanosecondBuilder::new(),
            topic: StringBuilder::new(),
        ...
        let status = sender.send(&mut pending_futs, &cancel).await.unwrap();
