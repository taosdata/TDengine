...
int32_t smlGetMeta(SSmlHandle *info, const void *measure, int32_t measureLen, STableMeta **pTableMeta) {
  *pTableMeta = NULL;
  ...
  SName pName = {TSDB_TABLE_NAME_T, T_NAME_DIRECT_INSERT, info->taos->acctId, {0}, {0}};
...
static int32_t smlCheckMeta(SSchema *schema, int32_t length, SArray *cols) {
  ...
  for (int32_t i = 0; i < taosArrayGetSize(cols); i++) {
    ...
    if (IS_VAR_DATA_TYPE(kv->type) && kv->length + VARSTR_HEADER_SIZE > sTmp->bytes) {
      uError("column %s (type %s) bytes invalid. db bytes:%d, kv bytes:%zu", sTmp->name, tDataTypes[sTmp->type].name,
             sTmp->bytes, kv->length);
...
static int32_t smlModifyTag(SSmlHandle *info, SHashObj *hashTmp, SRequestConnInfo *conn, SSmlSTableMeta *sTableData,
...
static int32_t smlModifyCols(SSmlHandle *info, SHashObj *hashTmp, SRequestConnInfo *conn, SSmlSTableMeta *sTableData,
                             SName *pName, STableMeta **pTableMeta) {
  ESchemaAction action = SCHEMA_ACTION_NULL;
  SArray       *pColumns = NULL;
  SArray       *pTags = NULL;
  ...
  while (tmp) {
    ...
    if (code == TSDB_CODE_PAR_TABLE_NOT_EXIST || code == TSDB_CODE_MND_STB_NOT_EXIST) {
      ...
      hashTmp = taosHashInit(pTableMeta->tableInfo.numOfTags, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BINARY), true,
                             HASH_NO_LOCK);
      SML_CHECK_NULL(hashTmp);
      SML_CHECK_CODE(smlBuildTempHash(hashTmp, pTableMeta, pTableMeta->tableInfo.numOfColumns,
