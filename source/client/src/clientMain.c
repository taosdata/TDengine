...
int taos_stmt2_exec(TAOS_STMT2 *stmt, int *affected_rows /*, __taos_async_fn_t fp, void *param*/) {
  if (stmt == NULL) {
    tscError("NULL parameter for %s", __FUNCTION__);
    terrno = TSDB_CODE_INVALID_PARA;
    return terrno;
  ...
  // return stmtExec2(stmt, affected_rows, fp, param);
  return stmtExec2(stmt, affected_rows, NULL, NULL);
}
