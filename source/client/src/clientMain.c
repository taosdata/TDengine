...
TAOS_BLOCK_VERSION taos_fetch_block_version(TAOS_RES *res) {
  (void)res;

  if (sizeof(VarDataLenT) == 4) {
    return TAOS_BLOCK_VERSION_2;
  } else if (sizeof(VarDataLenT) == 2) {
    return TAOS_BLOCK_VERSION_1;
  } else {
    return TAOS_BLOCK_VERSION_UNKNOWN;
  }
}
