...
static int buildColumnBindFromRow(TAOS_STMT2* stmt, TAOS_STMT2_BINDV* bindv, ColumnBindOwner* owner) {
  ...
  if (fieldNum <= 0 || fields == NULL) {
    (void)stmtEnsureColumnFieldCache2(stmt);
    fields = pStmt->sql.cachedFields;
    fieldNum = pStmt->sql.cachedFieldNum;
  }
  ...
  if (fieldNum <= 0 || fields == NULL) {
    ...
    pStmt->sql.cachedIsInsert = insert;
    ...
    for (int32_t t = 0; t < tableCount; ++t) {
      ...
      for (int32_t r = 0; r < rowsInTable; ++r, ++outRow) {
        if (!insert) {
          ...
          if (src->is_null) {
            owner->nulls[f][outRow] = src->is_null[srcRow];
            useNulls = true;
          }
...
static bool shouldTryColumnDispatch(TAOS_STMT2* stmt, TAOS_STMT2_BINDV* bindv) {
  ...
  if (pStmt->sql.bindRowFormat || pStmt->sql.status == STMT_BIND_COL) {
    return false;
  }
  return true;
}
...
TEST(stmt2ColumnCase, column_bind_query_rejects_multiple_rows_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_q_multi");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_q_multi");
  do_query(taos, "create table stmt2_testdb_col_q_multi.ntb (ts timestamp, v int)");
  do_query(taos, "insert into stmt2_testdb_col_q_multi.ntb values(1591060628000, 88)");
  do_query(taos, "use stmt2_testdb_col_q_multi");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "select v from stmt2_testdb_col_q_multi.ntb where ts = ?", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  int64_t ts[2] = {1591060628000, 1591060628001};
  int32_t ts_len[2] = {sizeof(int64_t), sizeof(int64_t)};
  TAOS_STMT2_COLUMN_BIND  columnBinds[1] = {{TSDB_DATA_TYPE_TIMESTAMP, ts, ts_len, NULL}};
  TAOS_STMT2_COLUMN_BINDV bindv = {1, 2, 0, columnBinds};
  code = taos_stmt2_bind_param_column(stmt, &bindv);
...
TEST(stmt2ColumnCase, column_bind_short_circuits_sticky_errcode_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_sticky");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_sticky");
  do_query(taos, "create table stmt2_testdb_col_sticky.ntb (ts timestamp, v int)");
  do_query(taos, "use stmt2_testdb_col_sticky");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_sticky.ntb values(?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  ((STscStmt2*)stmt)->errCode = TSDB_CODE_TSC_STMT_API_ERROR;

  int64_t ts = 1591060628000;
  int32_t ts_len = sizeof(int64_t);
  TAOS_STMT2_COLUMN_BIND  columnBind = {TSDB_DATA_TYPE_TIMESTAMP, &ts, &ts_len, NULL};
  TAOS_STMT2_COLUMN_BINDV bindv = {0, 1, 0, &columnBind};
...
TEST(stmt2ColumnCase, column_bind_rejects_dynamic_ntb_values_sql_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_cache_fail");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_cache_fail");
  do_query(taos, "create table stmt2_testdb_col_cache_fail.ntb(ts timestamp, b binary(10))");
  do_query(taos, "use stmt2_testdb_col_cache_fail");
  ...
  int32_t ts_len = sizeof(int64_t);
  TAOS_STMT2_COLUMN_BIND  columnBind = {TSDB_DATA_TYPE_TIMESTAMP, &ts, &ts_len, NULL};
  TAOS_STMT2_COLUMN_BINDV bindv = {1, 1, 0, &columnBind};

  code = taos_stmt2_bind_param_column(stmt, &bindv);
  ASSERT_EQ(code, TSDB_CODE_TSC_STMT_API_ERROR);
  ASSERT_STREQ(taos_stmt2_error(stmt), "Stmt API usage error");
  ASSERT_EQ(((STscStmt2*)stmt)->errCode, TSDB_CODE_TSC_STMT_API_ERROR);
...
TEST(stmt2ColumnCase, column_bind_rejects_dynamic_ntb_column_list_sql_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_cache_fail_cols");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_cache_fail_cols");
  do_query(taos, "create table stmt2_testdb_col_cache_fail_cols.ntb(ts timestamp, b binary(10))");
  do_query(taos, "use stmt2_testdb_col_cache_fail_cols");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "INSERT INTO ? (ts, b) VALUES (?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  int64_t ts = 1591060628000;
  char    b = 'a';
  int32_t ts_len = sizeof(int64_t);
  int32_t b_len = sizeof(char);
  TAOS_STMT2_COLUMN_BIND columns[2] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &ts_len, NULL},
      {TSDB_DATA_TYPE_BINARY, &b, &b_len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV bindv = {2, 1, 0, columns};
...
TEST(stmt2ColumnCase, column_bind_field_cache_failure_sets_sticky_errcode_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_cache_sticky");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_cache_sticky");
  do_query(taos, "use stmt2_testdb_col_cache_sticky");
  ...
  int32_t value = 1;
  int32_t valueLen = sizeof(int32_t);
  TAOS_STMT2_COLUMN_BIND  columnBind = {TSDB_DATA_TYPE_INT, &value, &valueLen, NULL};
  TAOS_STMT2_COLUMN_BINDV bindv = {1, 1, 0, &columnBind};

  int code = taos_stmt2_bind_param_column(stmt, &bindv);
  ASSERT_EQ(code, TSDB_CODE_TSC_STMT_API_ERROR);
  ASSERT_EQ(((STscStmt2*)stmt)->errCode, code);
...
TEST(stmt2ColumnCase, column_bind_error_then_success_return_code_columnar) {
  ...
  int64_t mismatchTs = 1591060628000;
  int32_t mismatchTsLen = sizeof(int64_t);
  TAOS_STMT2_COLUMN_BIND mismatchColumns[1] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &mismatchTs, &mismatchTsLen, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV mismatchBind = {1, 1, 0, mismatchColumns};
  code = taos_stmt2_bind_param_column(stmt, &mismatchBind);
  EXPECT_EQ(code, TSDB_CODE_PAR_INVALID_COLUMNS_NUM);
...
TEST(stmt2ColumnCase, column_bind_rejects_excess_column_count_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_extra_cols");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_extra_cols");
  do_query(taos, "create table stmt2_testdb_col_extra_cols.ntb (ts timestamp, v int)");
  do_query(taos, "use stmt2_testdb_col_extra_cols");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_extra_cols.ntb values(?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  int32_t extra = 2;
  int32_t ts_len = sizeof(int64_t);
  int32_t v_len = sizeof(int32_t);
  int32_t extra_len = sizeof(int32_t);
  TAOS_STMT2_COLUMN_BIND extraColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &ts_len, NULL},
      {TSDB_DATA_TYPE_INT, &v, &v_len, NULL},
      {TSDB_DATA_TYPE_INT, &extra, &extra_len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV extraBind = {3, 1, 0, extraColumns};
  code = taos_stmt2_bind_param_column(stmt, &extraBind);
  EXPECT_EQ(code, TSDB_CODE_PAR_INVALID_COLUMNS_NUM);
...
TEST(stmt2ColumnCase, column_bind_rejects_invalid_column_metadata_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_invalid_meta");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_invalid_meta");
  do_query(taos, "create table stmt2_testdb_col_invalid_meta.ntb (ts timestamp, name binary(16), decv decimal(20,4))");
  do_query(taos, "use stmt2_testdb_col_invalid_meta");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_invalid_meta.ntb values(?,?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_STMT2_COLUMN_BIND invalidTypeColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {999, name, &nameLen, NULL},
      {TSDB_DATA_TYPE_DECIMAL, dec, &decLen, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV invalidTypeBind = {3, 1, 0, invalidTypeColumns};
  code = taos_stmt2_bind_param_column(stmt, &invalidTypeBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  TAOS_STMT2_COLUMN_BIND missingBinaryLengthColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_BINARY, name, NULL, NULL},
      {TSDB_DATA_TYPE_DECIMAL, dec, &decLen, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV missingBinaryLengthBind = {3, 1, 0, missingBinaryLengthColumns};
  code = taos_stmt2_bind_param_column(stmt, &missingBinaryLengthBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  TAOS_STMT2_COLUMN_BIND missingDecimalLengthColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_BINARY, name, &nameLen, NULL},
      {TSDB_DATA_TYPE_DECIMAL, dec, NULL, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV missingDecimalLengthBind = {3, 1, 0, missingDecimalLengthColumns};
  code = taos_stmt2_bind_param_column(stmt, &missingDecimalLengthBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  int32_t negativeNameLen = -1;
  TAOS_STMT2_COLUMN_BIND negativeLengthColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_BINARY, name, &negativeNameLen, NULL},
      {TSDB_DATA_TYPE_DECIMAL, dec, &decLen, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV negativeLengthBind = {3, 1, 0, negativeLengthColumns};
  code = taos_stmt2_bind_param_column(stmt, &negativeLengthBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  TAOS_STMT2_COLUMN_BIND missingBufferColumns[3] = {
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_BINARY, name, &nameLen, NULL},
      {TSDB_DATA_TYPE_DECIMAL, NULL, &decLen, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV missingBufferBind = {3, 1, 0, missingBufferColumns};
  code = taos_stmt2_bind_param_column(stmt, &missingBufferBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);
...
TEST(stmt2ColumnCase, column_bind_insert_stb_rejects_unprocessed_rows_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_stb_unprocessed");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_stb_unprocessed");
  do_query(taos, "create stable stmt2_testdb_col_stb_unprocessed.stb (ts timestamp, v int) tags(t1 int)");
  do_query(taos, "use stmt2_testdb_col_stb_unprocessed");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_stb_unprocessed.stb (tbname,ts,v,t1) values(?,?,?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_STMT2_COLUMN_BINDV bindv = {4, 2, 1, columnBinds};
  code = taos_stmt2_bind_param_column(stmt, &bindv);
...
TEST(stmt2ColumnCase, column_bind_insert_stb_rejects_non_contiguous_tbnames_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_stb_non_contig");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_stb_non_contig");
  do_query(taos, "create stable stmt2_testdb_col_stb_non_contig.stb (ts timestamp, v int) tags(t1 int)");
  do_query(taos, "use stmt2_testdb_col_stb_non_contig");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_stb_non_contig.stb (tbname,ts,v,t1) values(?,?,?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  int64_t ts[3] = {1591060628000, 1591060629000, 1591060630000};
  int32_t v[3] = {11, 22, 33};
  int32_t t1[3] = {1, 2, 3};
  int32_t ts_len[3] = {sizeof(int64_t), sizeof(int64_t), sizeof(int64_t)};
  int32_t v_len[3] = {sizeof(int32_t), sizeof(int32_t), sizeof(int32_t)};
  int32_t t1_len[3] = {sizeof(int32_t), sizeof(int32_t), sizeof(int32_t)};
  int32_t tb_len[3] = {3, 3, 3};
  char    tb_buf[] = "tb1tb2tb1";
  ...
  TAOS_STMT2_COLUMN_BINDV bindv = {4, 3, 2, columnBinds};
  code = taos_stmt2_bind_param_column(stmt, &bindv);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);
...
TEST(stmt2ColumnCase, column_bind_rejects_invalid_tbname_metadata_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_invalid_tbname");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_invalid_tbname");
  do_query(taos, "create stable stmt2_testdb_col_invalid_tbname.stb (ts timestamp, v int) tags(t1 int)");
  do_query(taos, "use stmt2_testdb_col_invalid_tbname");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_invalid_tbname.stb (tbname,ts,v,t1) values(?,?,?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  int32_t t1 = 1;
  int32_t tsLen = sizeof(int64_t);
  int32_t vLen = sizeof(int32_t);
  int32_t t1Len = sizeof(int32_t);
  int32_t tbLen = 3;
  char    tbBuf[] = "tb1";
  char    tbNull = 1;

  TAOS_STMT2_COLUMN_BIND nullMarkedTbnameColumns[4] = {
      {TSDB_DATA_TYPE_BINARY, tbBuf, &tbLen, &tbNull},
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_INT, &v, &vLen, NULL},
      {TSDB_DATA_TYPE_INT, &t1, &t1Len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV nullMarkedTbnameBind = {4, 1, 1, nullMarkedTbnameColumns};
  code = taos_stmt2_bind_param_column(stmt, &nullMarkedTbnameBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);
  ...
  TAOS_STMT2_COLUMN_BIND nullBufferTbnameColumns[4] = {
      {TSDB_DATA_TYPE_BINARY, NULL, &tbLen, &tbNull},
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_INT, &v, &vLen, NULL},
      {TSDB_DATA_TYPE_INT, &t1, &t1Len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV nullBufferTbnameBind = {4, 1, 1, nullBufferTbnameColumns};
  code = taos_stmt2_bind_param_column(stmt, &nullBufferTbnameBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  int32_t emptyTbLen = 0;
  TAOS_STMT2_COLUMN_BIND emptyTbnameColumns[4] = {
      {TSDB_DATA_TYPE_BINARY, tbBuf, &emptyTbLen, NULL},
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_INT, &v, &vLen, NULL},
      {TSDB_DATA_TYPE_INT, &t1, &t1Len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV emptyTbnameBind = {4, 1, 1, emptyTbnameColumns};
  code = taos_stmt2_bind_param_column(stmt, &emptyTbnameBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);

  int32_t negativeTbLen = -1;
  TAOS_STMT2_COLUMN_BIND negativeTbnameColumns[4] = {
      {TSDB_DATA_TYPE_BINARY, tbBuf, &negativeTbLen, NULL},
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &tsLen, NULL},
      {TSDB_DATA_TYPE_INT, &v, &vLen, NULL},
      {TSDB_DATA_TYPE_INT, &t1, &t1Len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV negativeTbnameBind = {4, 1, 1, negativeTbnameColumns};
  code = taos_stmt2_bind_param_column(stmt, &negativeTbnameBind);
  EXPECT_EQ(code, TSDB_CODE_INVALID_PARA);
...
TEST(stmt2ColumnCase, column_bind_insert_old_style_stb_smoke_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_old_style");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_old_style");
  do_query(taos,
           "create stable stmt2_testdb_col_old_style.meters (ts timestamp, current float, voltage int, phase float) "
           "tags(groupid int, location binary(24))");
  do_query(taos, "use stmt2_testdb_col_old_style");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "INSERT INTO ? USING meters TAGS (?, ?) VALUES (?, ?, ?, ?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  char    tb_buf[] = "d1001";
  int32_t tb_len = 5;
  int32_t groupid = 7;
  int32_t groupid_len = sizeof(int32_t);
  char    location[] = "beijing";
  int32_t location_len = 7;
  int64_t ts = 1711958400000;
  int32_t ts_len = sizeof(int64_t);
  float   current = 10.3f;
  int32_t current_len = sizeof(float);
  int32_t voltage = 220;
  int32_t voltage_len = sizeof(int32_t);
  float   phase = 0.31f;
  int32_t phase_len = sizeof(float);

  TAOS_STMT2_COLUMN_BIND columnBinds[7] = {
      {TSDB_DATA_TYPE_BINARY, tb_buf, &tb_len, NULL},
      {TSDB_DATA_TYPE_INT, &groupid, &groupid_len, NULL},
      {TSDB_DATA_TYPE_BINARY, location, &location_len, NULL},
      {TSDB_DATA_TYPE_TIMESTAMP, &ts, &ts_len, NULL},
      {TSDB_DATA_TYPE_FLOAT, &current, &current_len, NULL},
      {TSDB_DATA_TYPE_INT, &voltage, &voltage_len, NULL},
      {TSDB_DATA_TYPE_FLOAT, &phase, &phase_len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV bindv = {7, 1, 1, columnBinds};
  code = taos_stmt2_bind_param_column(stmt, &bindv);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_RES* res =
      taos_query(taos, "select count(*), sum(voltage) from stmt2_testdb_col_old_style.meters where tbname = 'd1001'");
  ASSERT_EQ(taos_errno(res), 0);
  TAOS_ROW row = taos_fetch_row(res);
  ASSERT_NE(row, nullptr);
  ASSERT_EQ(*(int64_t*)row[0], 1);
  ASSERT_EQ(*(int64_t*)row[1], 220);
  taos_free_result(res);
...
TEST(stmt2ColumnCase, column_bind_insert_stb_multi_rows_same_tbname_with_repeated_tags_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_stb_multi_rows");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_stb_multi_rows");
  do_query(taos,
           "create stable stmt2_testdb_col_stb_multi_rows.stb (ts timestamp, v int) tags(t1 int, t2 binary(24))");
  do_query(taos, "use stmt2_testdb_col_stb_multi_rows");
  ...
  int code =
      taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_stb_multi_rows.stb (tbname,ts,v,t1,t2) values(?,?,?,?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);

  int64_t ts[3] = {1711958400000, 1711958460000, 1711958520000};
  int32_t v[3] = {11, 22, 33};
  int32_t t1[3] = {7, 7, 8};
  int32_t ts_len[3] = {sizeof(int64_t), sizeof(int64_t), sizeof(int64_t)};
  int32_t v_len[3] = {sizeof(int32_t), sizeof(int32_t), sizeof(int32_t)};
  int32_t t1_len[3] = {sizeof(int32_t), sizeof(int32_t), sizeof(int32_t)};
  int32_t tb_len[3] = {3, 3, 3};
  int32_t t2_len[3] = {7, 7, 8};
  char    tb_buf[] = "tb1tb1tb2";
  char    t2_buf[] = "beijingbeijingshanghai";

  TAOS_STMT2_COLUMN_BIND columnBinds[5] = {
      {TSDB_DATA_TYPE_BINARY, tb_buf, tb_len, NULL},
      {TSDB_DATA_TYPE_TIMESTAMP, ts, ts_len, NULL},
      {TSDB_DATA_TYPE_INT, v, v_len, NULL},
      {TSDB_DATA_TYPE_INT, t1, t1_len, NULL},
      {TSDB_DATA_TYPE_BINARY, t2_buf, t2_len, NULL},
  };
  TAOS_STMT2_COLUMN_BINDV bindv = {5, 3, 2, columnBinds};
  code = taos_stmt2_bind_param_column(stmt, &bindv);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_RES* res =
      taos_query(taos, "select tbname, t1, t2, v from stmt2_testdb_col_stb_multi_rows.stb order by tbname, ts");
  ASSERT_EQ(taos_errno(res), 0);

  TAOS_ROW row = taos_fetch_row(res);
  ASSERT_NE(row, nullptr);
  ASSERT_EQ(strncmp((char*)row[0], "tb1", 3), 0);
  ASSERT_EQ(*(int32_t*)row[1], 7);
  ASSERT_EQ(strncmp((char*)row[2], "beijing", 7), 0);
  ASSERT_EQ(*(int32_t*)row[3], 11);
  ...
  ASSERT_EQ(*(int32_t*)row[1], 7);
  ASSERT_EQ(strncmp((char*)row[2], "beijing", 7), 0);
  ASSERT_EQ(*(int32_t*)row[3], 22);

  row = taos_fetch_row(res);
  ASSERT_NE(row, nullptr);
  ASSERT_EQ(strncmp((char*)row[0], "tb2", 3), 0);
  ASSERT_EQ(*(int32_t*)row[1], 8);
  ASSERT_EQ(strncmp((char*)row[2], "shanghai", 8), 0);
  ASSERT_EQ(*(int32_t*)row[3], 33);
...
TEST(stmt2ColumnCase, insert_stb_get_fields_Test_columnar) {
...
TEST(stmt2ColumnCase, insert_ctb_using_get_fields_Test_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, insert_ntb_get_fields_Test_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, select_get_fields_Test_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, get_fields_error_Test_columnar) {
  // case 1 :
  {
...
TEST(stmt2ColumnCase, stmt2_init_prepare_Test_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
  {
...
TEST(stmt2ColumnCase, stmt2_stb_insert_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  // normal
...
// TD-35141
// TS-6798
TEST(stmt2ColumnCase, stmt2_insert_non_statndard_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  ...
  {
    // SKIP: Columnar binding does not support "INSERT INTO ? VALUES (?,?)" pattern
    // because field information cannot be obtained through prepare when table name is a parameter.
    // This requires binding table name first, then getting fields, which is not compatible with
    // columnar binding's data organization model.
    printf("[SKIP] Columnar binding does not support 'INSERT INTO ? VALUES (?,?)' pattern\n");
    goto skip_get_fields_insert;
...
// TD-34075
// TD-34504
TEST(stmt2ColumnCase, stmt2_insert_db_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, stmt2_insert_duplicate_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, query_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_7");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_7");
...
TEST(stmt2ColumnCase, query_use_adapter_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, query_error_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, stmt2_ntb_insert_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
...
TEST(stmt2ColumnCase, stmt2_status_Test_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
...
TEST(stmt2ColumnCase, stmt2_nchar_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
...
TEST(stmt2ColumnCase, all_type_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, geometry_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, decimal_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, prepare_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
// TD-34593
TEST(stmt2ColumnCase, prepare_fixedtags_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
// TD-33921
TEST(stmt2ColumnCase, mixed_bind_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
// TD-33582
TEST(stmt2ColumnCase, errcode_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, usage_error_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, column_bind_async_rejects_concurrent_bind_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_col_async");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_col_async");
  do_query(taos, "create table stmt2_testdb_col_async.ntb (ts timestamp, v int)");
  do_query(taos, "use stmt2_testdb_col_async");

  TAOS_STMT2_OPTION option = {0, true, true, NULL, NULL};
  TAOS_STMT2*       stmt = taos_stmt2_init(taos, &option);
  ASSERT_NE(stmt, nullptr);

  int code = taos_stmt2_prepare(stmt, "insert into stmt2_testdb_col_async.ntb values(?,?)", 0);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_STMT2_COLUMN_BINDV bindv = {2, 2, 0, columnBinds};

  bool finish = false;
  code = taos_stmt2_bind_param_column_a(stmt, &bindv, stmtAsyncBindCb2, (void*)&finish);
  checkError(stmt, code, __FILE__, __LINE__);
  ...
  TAOS_RES* res = taos_query(taos, "select count(*), sum(v) from stmt2_testdb_col_async.ntb");
  ASSERT_EQ(taos_errno(res), 0);
  TAOS_ROW row = taos_fetch_row(res);
  ASSERT_NE(row, nullptr);
  ASSERT_EQ(*(int64_t*)row[0], 2);
  ASSERT_EQ(*(int64_t*)row[1], 3);
  taos_free_result(res);
...
TEST(stmt2ColumnCase, rowformat_bind_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, exec_retry_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, core_columnar) {
...
// TS-7067
TEST(stmt2ColumnCase, tbname_columnar) {
  TAOS* taos = taos_connect("127.0.0.1", "root", "taosdata", NULL, 0);
  ASSERT_NE(taos, nullptr);
...
// TS-7074
TEST(stmt2ColumnCase, no_tag_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, vtable_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, bool_bind_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, mixed_literal_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, query_interval_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
...
TEST(stmt2ColumnCase, stmt2_insert_fixed_tag_value_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
  do_query(taos, "drop database if exists stmt2_testdb_6");
  do_query(taos, "create database IF NOT EXISTS stmt2_testdb_6");
...
// 6788971938
TEST(stmt2ColumnCase, query_vtable_core_columnar) {
  TAOS* taos = taos_connect("localhost", "root", "taosdata", "", 0);
  ASSERT_NE(taos, nullptr);
