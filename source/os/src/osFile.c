...
int64_t taosPReadFile(TdFilePtr pFile, void *buf, int64_t count, int64_t offset) {
  ...
  overlapped.OffsetHigh = (uint32_t)((offset & 0xFFFFFFFF00000000LL) >> 0x20);
  overlapped.Offset = (uint32_t)(offset & 0xFFFFFFFFLL);

  HANDLE fHandle = (HANDLE)_get_osfhandle(pFile->fd);
  SetLastError(0);
  bool result = ReadFile(fHandle, buf, count, &ret, &overlapped);
...
int64_t taosPWriteFile(TdFilePtr pFile, const void *buf, int64_t count, int64_t offset) {
  ...
  HANDLE handle = (HANDLE)_get_osfhandle(pFile->fd);
  SetLastError(0);
  bool result = WriteFile(handle, buf, count, &ret, &overlapped);
  if (!result) {
    errno = GetLastError();
    return -1;
  }
