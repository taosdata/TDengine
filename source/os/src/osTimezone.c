...
// Global variable storing UTC offset on Windows (seconds).
...
// Accessor function to ensure reading from the correct storage.
int64_t getWindowsTimezoneOffset(void) {
  // Read TZ from the environment each time (set by taosSetGlobalTimezone).
  char *tz_env = getenv("TZ");

  if (tz_env != NULL && tz_env[0] != '\0') {
    // TZ format can be "+0:00" or "-8:00" (after configuration takes effect).
    // Or "Asia/Shanghai" (configuration not applied yet, system default).
    if (*tz_env == '+' || *tz_env == '-') {
      // Configuration applied; parse the offset.
      char sign = *tz_env;
    ...
    // If TZ is not in offset format, configuration may not be applied yet; continue with system timezone.
  }

  // Get timezone from the system.
...
int32_t resetTimezoneInfo(const char *tzname)
{
  ...
  // Parse timezone offset, formats like "Asia/Shanghai", "UTC+8", or "+08:00".
  // Simplified here: assume users call taosSetGlobalTimezone to set it.
