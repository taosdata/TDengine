...
void clearDiskbasedBuf(SDiskbasedBuf* pBuf) {
  ...
  if (pBuf->pFile != NULL) {
    ...
    if (code != TSDB_CODE_SUCCESS) {
      uWarn("failed to truncate paged buffer file, path:%s, code:%s, fallback to reopen, %s", pBuf->path,
            tstrerror(code), pBuf->id);

      int32_t closeCode = taosCloseFile(&pBuf->pFile);
      if (closeCode != TSDB_CODE_SUCCESS) {
        uError("failed to close paged buffer file during clear fallback, path:%s, closeCode:%d, err:%s, %s",
               pBuf->path, closeCode, terrstr(), pBuf->id);
      } else {
        code = createDiskFile(pBuf);
        if (code != TSDB_CODE_SUCCESS) {
          uError("failed to recreate paged buffer file during clear fallback, path:%s, code:%s, %s", pBuf->path,
                 tstrerror(code), pBuf->id);
        }
      }
    }
