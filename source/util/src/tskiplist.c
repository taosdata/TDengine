...
static FORCE_INLINE void tSkipListWLock(SSkipList *pSkipList) {
  if (pSkipList->lock) {
    int32_t code = taosThreadRwlockWrlock(pSkipList->lock);
    if (code) {
      uError("failed to lock the skip list, reason:%s", tstrerror(code));
    }
  }
  return;
}
...
static FORCE_INLINE void tSkipListRLock(SSkipList *pSkipList) {
  if (pSkipList->lock) {
    int32_t code = taosThreadRwlockRdlock(pSkipList->lock);
    if (code) {
      uError("failed to lock the skip list, reason:%s", tstrerror(code));
    }
  }
...
static FORCE_INLINE void tSkipListUnlock(SSkipList *pSkipList) {
  if (pSkipList->lock) {
    int32_t code = taosThreadRwlockUnlock(pSkipList->lock);
    if (code) {
      uError("failed to unlock the skip list, reason:%s", tstrerror(code));
    }
  }
  return;
}
