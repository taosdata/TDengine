...
static int32_t doGenerateTotp(const char *secret, size_t secretLen, uint64_t tm, int digits) {
  if (secretLen < 16) {
  ...
  for (int j = 0; j < 8; j++) {
    int idx = 7 - j;
    timeBytes[idx] = (tm >> (idx * 8)) & 0xFF;
  }

  // calculate HMAC-SHA1
  uint8_t      hmacResult[EVP_MAX_MD_SIZE];
  unsigned int hmacLen;
  HMAC(EVP_sha1(), secret, secretLen, timeBytes, 8, hmacResult, &hmacLen);

  // use the lower 4 bits of the last byte as offset
  int offset = hmacResult[hmacLen - 1] & 0x0F;

  int32_t binary = ((hmacResult[offset] & 0x7F) << 24) | ((hmacResult[offset + 1] & 0xFF) << 16) |
...
// secret is a byte array, not a base32 string
int32_t taosGenerateTotpCode(const char *secret, size_t secretLen, int digits) {
  if (secretLen < 16) {
    return -1;  // secret is too short
  }
...
// secret is a byte array, not a base32 string
int taosGenerateTotpSecret(const char *seed, size_t seedLen, char *secret, size_t secretLen) {
