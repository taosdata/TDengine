    ...
    <el-collapse-transition>
      ...
      >
        <tree-node
          ...
          @node-loaded="handleChildNodeLoaded"
        />
        <el-pagination
          v-if="node.pagination"
          size="small"
          hide-on-single-page
          :page-sizes="[10, 20, 50, 100]"
          :pager-count="2"
          :current-page="node.currentPage"
          :page-size="node.pageSize"
          layout="prev, pager, next, jumper"
          :total="node.total"
          class="tree-node-page"
          :style="{ 'padding-left': (node.level - 1) * tree.indent + 12 + 'px' }"
...
import { debugWarn, isFunction, isString } from 'element-plus/es/utils/index';
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    watch(
      ...
      val => {
        if (val) {
          ctx.emit('node-loaded', props.node);
        }
      }
    );
    const handleSizeChange = (val: number) => {
      props.node.setPageSize(val);
      props.node.setCurrentPage(1);
      props.node.loadPageData();
    };
    const handlePageChange = (page: number = 1) => {
      props.node.setCurrentPage(page);
      props.node.loadPageData();
    };
    const handleChildNodeLoaded = () => {
      if (props.node.childNodes.every(node => node.loaded)) {
        tree?.ctx.emit('all-child-node-loaded', props.node);
      }
    };
    ...
    const handleDoubleClick = (e: MouseEvent) => {
      if (tree.instance.vnode.props['onNodeDoubleClick']) {
        event.stopPropagation();
        event.preventDefault();
      }
      tree.ctx.emit('node-dblclick', props.node.data, props.node, instance, e);
    };
...
<style lang="scss">
.el-tree-node__children .el-pagination {
  position: sticky;
  bottom: 0;
}
