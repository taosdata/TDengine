...
export const getDefaultValue = (defaultValue: DefaultValue, { lang, unit, unlinkPanels }: GetDefaultValueParams) => {
  let start: Dayjs;
...
export const getValidDateOfMonth = (year: number, month: number, lang: string, disabledDate?: DisabledDateType) => {
...
export const getValidDateOfYear = (value: Dayjs, lang: string, disabledDate?: DisabledDateType) => {
...
export const switchTimezone = function (value1?: any, value2?: any, value3?: any): Dayjs {
  const timezone = localStorage.getItem('timezone') || 'Asia/Shanghai';
  let d = new Date();
  if (value1) {
    d = new Date(value1);
  } else if (value2) {
  ...
  const localTime = d.getTime(); // 获取本地时间的时间戳
  const offset = Number(dayjs().tz(timezone).format('Z').replace(':', '.'));
  const localOffset = d.getTimezoneOffset() * 60000;
  const utcTime = localTime + localOffset; // 计算 UTC 时间戳
  const newTime = utcTime + 3600000 * offset; // 计算目标时区的时间戳
