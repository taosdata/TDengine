<template>
  ...
  >
      ...
      <div :class="ppNs.e('body')">
        ...
        >
          ...
          <span
            ...
            aria-live="polite"
            tabindex="0"
            :class="[dpNs.e('header-label'), { active: currentView === 'month' }]"
    ...
    <div v-show="footerVisible" :class="ppNs.e('footer')">
      ...
      <el-button plain size="small" :class="ppNs.e('link-btn')" :disabled="disabledConfirm" @click="onConfirm">
...
import { ClickOutside as vClickOutside } from 'element-plus/es/directives/index';
import { useLocale, useNamespace } from 'element-plus/es/hooks/index';
import ElInput from 'element-plus/es/components/input/index';
import { TimePickPanel, extractDateFormat, extractTimeFormat } from 'element-plus/es/components/time-picker/index';
import { ElIcon } from 'element-plus/es/components/icon/index';
import { isArray, isFunction } from 'element-plus/es/utils/index';
import { EVENT_CODE } from 'element-plus/es/constants/index';
import { ArrowLeft, ArrowRight, DArrowLeft, DArrowRight } from '@element-plus/icons-vue';
import { TOOLTIP_INJECTION_KEY } from 'element-plus/es/components/tooltip/index';
import { panelDatePickProps } from '../props/panel-date-pick';
import { getValidDateOfMonth, getValidDateOfYear, switchTimezone } from '../utils';
...
const innerDate = ref(switchTimezone(dayjs()).locale(lang.value));
...
const defaultTimeD = computed(() => {
  return switchTimezone(dayjs(defaultTime)).locale(lang.value);
});
...
const handleShortcutClick = (shortcut: Shortcut) => {
  ...
  if (shortcutValue) {
    isShortcut = true;
    emit(switchTimezone(dayjs(shortcutValue)).locale(lang.value));
    return;
  }
  if (shortcut.onClick) {
    shortcut.onClick({
...
const handleMonthPick = async (month: number | MonthsPickerEmits, keepOpen?: boolean) => {
  if (selectionMode.value === 'month') {
    innerDate.value = getValidDateOfMonth(innerDate.value.year(), month as number, lang.value, disabledDate);
...
const handleYearPick = async (year: number | YearsPickerEmits, keepOpen?: boolean) => {
...
const onConfirm = () => {
  if (isMultipleType.value) {
    ...
    if (!result) {
      const defaultTimeD = switchTimezone(dayjs(defaultTime)).locale(lang.value);
...
const disabledNow = computed(() => {
  if (!disabledDate) return false;
  return disabledDate(switchTimezone(dayjs()).locale(lang.value).toDate());
});
const changeToNow = () => {
  // NOTE: not a permanent solution
  //       consider disable "now" button in the future
  const now = switchTimezone(dayjs()).locale(lang.value);
  const nowDate = now.toDate();
  isChangeToNow.value = true;
  if ((!disabledDate || !disabledDate(nowDate)) && checkDateWithinRange(nowDate)) {
    innerDate.value = switchTimezone(dayjs()).locale(lang.value);
...
const handleTimePick = (value: Dayjs, visible: boolean, first: boolean) => {
  ...
  const newDate = props.parsedValue ? (props.parsedValue as Dayjs).hour(hour).minute(minute).second(second) : value;
  innerDate.value = newDate;
  emit(innerDate.value, true);
  if (!first) {
...
const handleVisibleTimeChange = (value: string) => {
  const newDate = switchTimezone(dayjs(value, timeFormat.value)).locale(lang.value);
...
const handleVisibleDateChange = (value: string) => {
  const newDate = switchTimezone(dayjs(value, dateFormat.value)).locale(lang.value);
  if (newDate.isValid()) {
...
const formatToString = (value: Dayjs | Dayjs[]) => {
  return isArray(value) ? (value as Dayjs[]).map(_ => _.format(props.format)) : (value as Dayjs).format(props.format);
};

const parseUserInput = (value: Dayjs) => {
  return switchTimezone(dayjs(value, props.format)).locale(lang.value);
};

const getDefaultValue = () => {
  const parseDate = switchTimezone(dayjs(defaultValue.value)).locale(lang.value);
  if (!defaultValue.value) {
    const defaultTimeDValue = defaultTimeD.value;
    return switchTimezone(dayjs())
...
const handleKeydownTable = (event: KeyboardEvent) => {
  ...
  if (
    [EVENT_CODE.enter, EVENT_CODE.space, EVENT_CODE.numpadEnter].includes(code) &&
    userInputDate.value === null &&
    userInputTime.value === null
  ) {
    event.preventDefault();
...
const handleKeyControl = (code: string) => {
  ...
  type KeyControl = {
    [key: string]: number | KeyControlMappingCallableOffset | ((date: Date, step: number) => any);
  ...
  const mapping: KeyControlMapping = {
    ...
    week: {
      ...
      [left]: -1,
      [right]: 1,
      offset: (date: Date, step: number) => date.setDate(date.getDate() + step * 7)
    },
    date: {
      [up]: -7,
      [down]: 7,
      ...
      [pageUp]: (date: Date) => -new Date(date.getFullYear(), date.getMonth(), 0).getDate(),
      [pageDown]: (date: Date) => new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate(),
  ...
  while (Math.abs(innerDate.value.diff(newDate, 'year', true)) < 1) {
    ...
    const result = switchTimezone(dayjs(newDate)).locale(lang.value);
    innerDate.value = result;
