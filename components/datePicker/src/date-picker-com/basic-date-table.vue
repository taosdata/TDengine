  ...
  >
    <tbody ref="tbodyRef">
      <tr>
        ...
        <th v-for="(week, key) in WEEKS" :key="key" :aria-label="t('el.datepicker.weeksFull.' + week)" scope="col">
          {{ t('el.datepicker.weeks.' + week) }}
        </th>
      </tr>
      <tr v-for="(row, rowKey) in rows" :key="rowKey" :class="getRowKls(row[1])">
        <td
...
import { basicDateTableEmits, basicDateTableProps } from '../props/basic-date-table';
import { useBasicDateTable, useBasicDateTableDOM } from '../composables/use-basic-date-table';
