...
import { castArray, isArray } from 'element-plus/es/utils/index';
import { buildPickerTable, switchTimezone } from '../utils';
...
export const useBasicDateTable = (props: BasicDateTableProps, emit: SetupContext<BasicDateTableEmits>['emit']) => {
  ...
  const selectedDate = computed(() => {
    return props.selectionMode === 'dates' ? (castArray(props.parsedValue) as Dayjs[]) : ([] as Dayjs[]);
  });
  ...
  const setDateText = (cell: DateCell, { count, rowIndex, columnIndex }: CellMeta): boolean => {
  ...
  const setCellMetadata = (cell: DateCell, { columnIndex, rowIndex }: CellCoordinate, count: number) => {
  ...
  const rows = computed(() => {
    ...
    buildPickerTable({ row: 6, column: 7 }, rows_, {
      ...
      now: switchTimezone(dayjs()).locale(unref(lang)).startOf(dateUnit),
      unit: dateUnit,
      relativeDateGetter: (idx: number) => unref(startDate).add(idx - offset, dateUnit),
  ...
  const isCurrent = (cell: DateCell): boolean => {
    return (
      props.selectionMode === 'date' && isNormalDay(cell.type) && cellMatchesDate(cell, props.parsedValue as Dayjs)
    );
  };
  ...
  const isSelectedCell = (cell: DateCell) => {
    return (!unref(hasCurrent) && cell?.text === 1 && cell.type === 'normal') || cell.isCurrent;
  };
  ...
  const handleDatesPick = (newDate: Dayjs, selected: boolean) => {
    const newValue = selected
      ? castArray(props.parsedValue).filter(d => d?.valueOf() !== newDate.valueOf())
      : castArray(props.parsedValue).concat([newDate]);
    emit('pick', newValue);
  };

  const handlePickDate = (event: FocusEvent | MouseEvent, isKeyboardMovement = false) => {
    const target = (event.target as HTMLElement).closest('td');
...
export const useBasicDateTableDOM = (
  ...
  { isCurrent, isWeekActive }: Pick<ReturnType<typeof useBasicDateTable>, 'isCurrent' | 'isWeekActive'>
) => {
  ...
  const getRowKls = (cell: DateCell) => [ns.e('row'), { current: isWeekActive(cell) }];
