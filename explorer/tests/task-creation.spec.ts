...
import { findTaskRow, gotoDataInTask, openAddSourceFromList, selectElOptionByText, startTaskFromRow } from './_utils/datain';
...
test.describe('DataIn - TMQ task E2E (integrated env)', () => {
  test('create TMQ task and start it', async ({ page }) => {
    ...
    await Promise.all([
      page.waitForURL(new RegExp(`${routes.dataInTask}$`), { timeout: 60_000 }),
      submitBtn.click()
    ]);
    ...
    await expect(row).toContainText(/Queued|Started|Running/, { timeout: 60_000 });
  });
});
