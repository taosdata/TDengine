...
function handleAuthentication(authentication, paramsConfig) {
  ...
  alternatives.forEach((item) => {
    ...
    if (item.params) {
      ...
      const keys = Object.keys(item).filter((predicate) =>
        isObject(item[predicate])
      );
      keys.forEach((key) => {
        const {
          display,
      ...
      const config = {
        ...
        required: (currentData) => {
          if (id?.startsWith("opcua")) {
            return ["private_key", "security_policy", "certificate"].includes(
              key
            )
...
function handleParser(
  parser,
  paramsConfig,
  value = cloneDeep(DefaultParserValue),
  id
) {
  if (!parser) return;
...
function handleGroups(groups, paramsConfig, beforeConnectionCheck, id) {
  ...
  paramsConfig.push({
    ...
    field: beforeConnectionCheck
      ? groupsFieldBeforeConnection
      : groupsFieldAfterConnection,
    hide: true,
    children,
  });
  groups.forEach((group) => {
    if (
      (beforeConnectionCheck && !group.connection_option) ||
      (!beforeConnectionCheck && group.connection_option)
    ) {
      return;
    }
    ...
    params.forEach((param) => {
      ...
      const paramConfig = {
        ...
        if: (currentData, originalData) => {
          ...
          if (id == "taos") {
            ...
            const { mode, schema } =
              originalData[groupsFieldAfterConnection][migrateOptionsFiled];
            if (schema == "only") {
              config.hide = ["realtime_settings", "range"].includes(
                config.field
              );
              return ![
                "start",
          ...
          if (id.startsWith("opc")) {
            if (
              datasetsData &&
              datasetsData[valueField] === opcGroupShowValue
            ) {
            ...
            if (currentData.table == historianLiveTable) {
              return ["mode", "table", "tags", "retrieveInterval"].includes(
                name
              );
            } else {
        ...
        required: (currentData, b, c, isEdit) => {
          if (id?.startsWith("kafka") || id?.startsWith("mqtt")) {
            return ["client_id", "group"].includes(name) && isEdit
              ? !isEdit
              : required;
          }
          return required;
      ...
      if (
        (currentType == "influxdb" && paramConfig.field == "bucket") ||
        (currentType == "opentsdb" && paramConfig.field == "metrics")
      ) {
        paramConfig.type = "bucket";
      }
      // 特殊处理 historian 的 mode
      if (
        (currentType == "avevaHistorian" ||
          currentType == "mysql" ||
          currentType == "postgres") &&
        paramConfig.field == "mode"
      ) {
        paramConfig.type = "mode";
      }
      if (
        (currentType == "mqtt" && paramConfig.field == "client_id") ||
        (currentType == "kafka" &&
          (paramConfig.field == "client_id" || paramConfig.field == "group"))
      ) {
...
function handleParams(params, paramsConfig) {
  ...
  params.forEach((param, index) => {
    const { name, display, description, required, placeholder, value, hint } =
      param;
    const config = {
      label: display,
...
function handleProtocolData(protocol, definition) {
  ...
  if (id == "csv") {
    let csvFileConfig = store.state.csvFileListener || {
      file_or_dir: "1",
    ...
    if (csvFileConfig.file_or_dir === "1") {
      return (
        dsn +
        store.state.csvfiles.map((item) => item.response[0]).join(",") +
        "?file_or_dir=" +
        csvFileConfig.file_or_dir +
        "&keep_processed_files=" +
        csvFileConfig.keep_processed_files
      );
    } else {
      return (
        dsn +
        csvFileConfig.fileurl +
        "?file_or_dir=" +
        csvFileConfig.file_or_dir +
        "&file_pattern=" +
        csvFileConfig.file_pattern +
        "&new_file_notify=" +
        csvFileConfig.new_file_notify +
        "&notify_interval=" +
        csvFileConfig.notify_interval +
        "s" + // 固定单位为秒
        "&sort=" +
        csvFileConfig.sort
      );
        ...
        if (k == valueField) {
          ...
          if (TimeFormats.includes(k)) {
            ...
            let type_value = checkValue(
              getQueryParamValue(groups[key][k + "_type"])
            )
              ? getQueryParamValue(groups[key][k + "_type"])
              : "";
            query.push(
              field + "=" + getQueryParamValue(groups[key][k]) + type_value
            );
...
function getDatasetsQuery(datasets, allData, query) {
  ...
  const onlyPoint =
    allData[optionsField].system_configuration != piOptionShowValue;
...
export function getAuthentications(authentication, params, definition) {
  ...
  switch (authentication[valueField]) {
    case "plain":
      ...
      return (
        getQueryParamValue(currentData.username) +
        ":" +
        getQueryParamValue(currentData.password) +
        "@"
      );
    default:
      params.push(
        ...dataFields.map((item) => {
          ...
          return (
            getOriginField(item) + "=" + getQueryParamValue(currentData[item])
          );
        })
      );
      break;
...
function getOptionData(data, queryArr, definition) {
  ...
  if (
    ...
    if (system_configuration && system_configuration != piOptionShowValue)
      return result;
    if (port) {
      result += ":" + port;
    }
    ...
    if (id === "tmq") {
      ...
      if (id == "csv") {
        result += Array.isArray(store.state.csvfiles)
          ? store.state.csvfiles[0].response[0]
          : store.state.csvfiles;
      } else {
        result += endpoint;
      }
    }
...
export function getGroupsObj(data) {
  let groups = Object.assign(
    {},
    data[groupsFieldBeforeConnection] || {},
    data[groupsFieldAfterConnection] || {}
  );
  let obj = {};
  if (!groups) return {};
  for (let key in groups) {
