...
const DefaultParserValue = {
    parse: {
        ...
        name: '', using: '', columns: ['ts'], tags: []
    }
};
export const DefaultOpcTableValue = {
    stable_prefix: '', column_configs: [{
        column_name: 'received_time', column_type: 'timestamp', column_alias: 'received_time', is_primary_key: true
    }, {
        column_name: 'original_time', column_type: 'timestamp', column_alias: 'original_time', is_primary_key: false
    }, {
        column_name: 'value', column_alias: 'value', is_primary_key: false
    }, {
        column_name: 'quality', column_type: 'int', column_alias: 'quality', is_primary_key: false
    }]
};
...
export function getFormConfigByDataSource(dataSource, parserValue) {
    return dataSource.reduce((formConfig, item) => {
        ...
        const paramsConfig = [{
            label: i18n.t('dataIn.connectionConfiguration'), field: optionsField, children: []
        }];
        const config = {
            name, id, type, description, strict, config: paramsConfig, parser
        };
        currentType = id;
...
function handleConnectivityCheck(paramsConfig) {
    paramsConfig.push({
        field: 'checkConnectivity', type: 'checkConnectivity', children: []
    });
}
...
function handleProtocol(protocol, paramsConfig) {
    ...
    paramsConfig[0].children.push({
        ...
        options: choices.map(item => ({
            label: item.display, value: item.name, description: item.description
        }))
    });
}
...
function handleAuthentication(authentication, paramsConfig) {
    ...
    alternatives.forEach(item => {
        ...
        const tabConfig = {
            label: item.display, name: item.name, field: handleField(item.name), children: paramsChildren
        };
        if (item.params) {
            ...
            keys.forEach(key => {
                ...
                const config = {
                    label: display, description, required, field: key, defaultValue: defaultValue ?? ''
                };
...
function handleOptions(options, paramsConfig, id) {
    ...
    if (id === 'kafka') {
        ...
        keys.forEach(key => {
            ...
            if (!display) return;
            const config = {
                label: display, description, field: key, placeholder, // required,
                pattern: pattern || null, patternMsg, defaultValue: value ?? '', if: currentData => {
                    if (!currentData.system_configuration || key == 'host') return true;
                    return currentData.system_configuration == piOptionShowValue;
                }, required: (currentData) => {
                    if (id?.startsWith('opcua')) {
                        return ['private_key', 'security_policy', 'certificate'].includes(key) ? checkValue(currentData.security_mode) && currentData.security_mode !== opcuaSecuritymodeValue : required
                    } else {
                        return required
                    }
                    ...
                    if (id?.startsWith('opcua')) {
                        ...
                        return currentData.security_mode == opcuaSecuritymodeValue && ['security_policy'].includes(key)
                    }
...
function handleCsvData(id, paramsConfig) {
    if (id != 'csv') return;
    paramsConfig.push({
        label: '', field: 'csvData', type: 'csvData', children: []
    });
}
...
function handleDatasets(datasets, paramsConfig) {
    ...
    const datasetsConfig = {
        ...
        multiple: false,
        name: 'datasets',
        children: params ? params?.map((item, index) => {
            ...
            const paramConfig = {
                ...
                children: categoryParams ? categoryParams.map(categoryParam => {
                    const config = {
                        ...categoryParam,
                    ...
                    if (currentType.startsWith('opc')) {
                        ...
                        if (config.field == 'pattern' || config.field == 'browse_name_pattern') {
                            config.type = 'pattern';
                        }
...
function handleGroups(groups, paramsConfig, beforeConnectionCheck, id) {
    ...
    groups.forEach(group => {
        ...
        const {
            name, description: d1, params, collapsible = false, collapsed = true, short_description: d2, display
        } = group;
        const paramChildren = [];
        const config = {
            label: id == 'taos' ? display : name,
            field: id == 'taos' ? name : uuid(),
            description: d1 ?? d2,
        ...
        params.forEach(param => {
            ...
            const paramConfig = {
                ...
                field: handleField(name), // if: collapsible ? data => data[valueField] : true,
                if: (currentData, originalData) => {
                    const datasetsData = originalData[datasetsField];
                    if (collapsible) {
                        if (id === 'kafka' && group.display_order == 1) {
                            return (currentData.sasl_mechanism == "GSSAPI") ? currentData[valueField] && !['sasl_username', 'sasl_password'].includes(name) : currentData[valueField] && ['sasl_mechanism', 'sasl_username', 'sasl_password'].includes(name)
                        } else {
                ...
                required: (currentData, b, c, isEdit) => {
                    if (id?.startsWith('kafka') || id?.startsWith('mqtt')) {
                        return ['client_id', 'group'].includes(name) && isEdit ? !isEdit : required;
                    }
                    return required;
                },
                disabled: (currentData, b, c, isEdit) => {
...
function handleAdvanced(advanced, paramsConfig, id) {
    ...
    params.forEach(group => {
        ...
        const paramChildren = [];
        const config = {
            label: display, field: handleField(name), description: d1 ?? d2, defaultValue: value, // if: !hidden,
            placeholder, required, if: (currentData, originalData) => {
                if (id == 'pi') {
                    const datasetsData = originalData[datasetsField];
