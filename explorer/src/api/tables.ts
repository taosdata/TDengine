import { handleDataKey } from './database';
...
export async function getTableListReq(params) {
  const { currentPage, pageSize, selected_stb, selected_db } = params;
...
export function deleteTableReq(payload) {
  const { selected_db, tableName } = payload;
...
export function createTableReq(payload) {
  const { selected_db, table_form } = payload;
  const { name, stbTmpl, tags, columns } = table_form;
...
export function changeTableStruct(data, tableName) {
  const { operation, first_field = '', second_field = '' } = data;
  let sql = '';
...
export function getMatrixStructReq(payload) {
  const { selected_db, selected_tb } = payload;
...
export function getTableStructReq(payload) {
  ...
  return sendSQLReq(`DESCRIBE \`${selected_db}\`` + '.' + `\`${tableName}\`;`, true)
    .then(list => {
      const ts_field_name = list[0]?.field;
      const columns = [];
      const tags = [];
