...
Windows： <code>C:\\TDengine\\cfg\\</code>`,
    ...
    virtual: {
      grafana: {
        ...
        step2desc: '使用 grafana-cli 命令行工具 进行插件安装，安装后需要重启 Grafana。对于 Linux/Mac，在终端中执行下面命令：',
        step2desc1: '对于 Windows，首先请确保插件安装目录存在（默认是 Grafana 安装目录下 data/plugins）, 然后在 Grafana 安装目录的 bin 目录下以管理员账号执行下面的命令：',
    ...
    tools: {
      ...
      nodered: {
        ...
        brief1: '是由 IBM 开发的基于 Node.js 的开源可视化编程工具，通过图形化界面组装连接各种节点，实现物联网设备、API 及在线服务的连接。同时支持多协议、跨平台，社区活跃，适用于智能家居、工业自动化等场景的事件驱动应用开发，其主要特点是低代码、可视化。',
        brief2: 'TDengine 与 Node-RED 深度融合为工业 IoT 场景提供全栈式解决方案。通过 Node-RED 的 MQTT/OPC UA/Modbus 等协议节点，实现 PLC、传感器等设备毫秒级数据采集。同时 Node-RED 中基于 TDengine 的毫秒级实时查询结果，触发继电器动作、阀门开合等物理控制，实现更实时的联动控制。',
        ...
        step311: '场景介绍',
        step311pre1: '某生产车间有多台智能电表，电表每一秒产生一条数据，数据准备存储在 TDengine 数据库中，要求实时输出每分钟各智能电表平均电流、电压及用电量，同时要对电流 > 25A 或电压 > 230V 负载过大设备进行报警。',
        ...
        step321: '数据采集',
        step321pre1: '示例使用生成随机数方式模拟真实设备生产数据，tdengine-operator 节点配置 TDengine 数据源连接信息，并把数据写入 TDengine，同时使用 debug 节点监控写入成功数据量并展示于界面。',
        step321pre2: '操作步骤如下：',
        ...
        step3212item4: '双击节点打开属性设置，名称填写‘inject1’，下拉列表中选择“周期性执行”，周期选择每隔 1 秒，保存并返回画布。',
        ...
        step322: '数据查询',
        step322pre1: '查询流程由三个节点（inject/tdengine-operator/debug）组成，完成每分钟实时输出各智能电表平均电流、电压及用电量需求。由 inject 节点完成触发查询请求，结果输出至下游 debug 节点中，节点上显示查询执行成功数量。',
        step322pre2: '操作步骤如下：',
        step322item1: '将 inject 节点拖动至画布中，双击节点设置属性，名称填“query”, msg.topic 填写并保存并返回画布：',
        step322item2: '将 tdengine-operator 节点拖动至画布中，双击节点设置属性，“数据库”选择前面已创建好的数据源“db-server”，保存并返回画布。',
        step322item3: '将 debug 节点拖动至画布中，双击节点设置属性，勾选“节点状态”，下拉列表中选择“消息数量”，保存并返回画布。',
        ...
        step323: '数据订阅',
        step323pre1: '数据订阅流程由两个节点（tdengine-consumer/debug）组成，实现过载告警。debug 节点展示向下游节点推送数据次数，生产中可把 debug 节点更换为处理订阅数据的功能节点。',
        step323pre2: '操作步骤如下：',
        ...
        step323item3: '将 debug 节点拖动至画布中，双击节点设置属性，勾选“节点状态”，下拉列表中选择“消息数量”，保存并返回画布。',
