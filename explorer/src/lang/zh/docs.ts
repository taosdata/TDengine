...
Windows： <code>C:\\TDengine\\cfg\\</code>`,
    ...
    virtual: {
      grafana: {
        desc: `${GRAFANA_GDS} 能够与开源数据可视化系统  Grafana 快速集成搭建数据监测报警系统，整个过程无需任何代码开发，${GRAFANA_GDS} 中数据表的内容可以在仪表盘(DashBoard)上进行可视化展现。关于 ${GRAFANA_GDS} 插件的使用您可以在 GitHub`,
        topdesc: `${GRAFANA_GDS} 能够与开源数据可视化系统`,
        topdesc1: `   快速集成搭建数据监测报警系统，整个过程无需任何代码开发。${GRAFANA_GDS} 中数据表的内容可以在仪表盘(DashBoard)上进行可视化展现。关于 ${GRAFANA_GDS} 插件的使用您可以在 GitHub. `,
        ...
        plugin2desc: `2. 在弹出页面的搜索栏内搜索 ${GRAFANA_GDS}，然后会弹出 <code>${GRAFANA_GDS} Data Source</code>"。 `,
        plugin3desc: `3. 最后点击 <code>Install</code> 按钮安装 ${GRAFANA_GDS} 插件。`,
    ...
    tools: {
      ...
      superset: {
        ...
        step2desc:
          'TDengine Python 连接器从 v2.1.18 开始自带 Superset 连接驱动，安装程序会把连接驱动安装到 Superset 相应目录下并向 Superset 提供数据源服务 Superset 与 TDengine 之间使用 WebSocket 协议连接，所以需另安装支持 WebSocket 连接协议的组件 taos-ws-py , 全部安装脚本如下： ',
        step3: '配置',
        step3full: 'Superset 中配置 TDengine 连接',
        step31desc: '进入新建数据库连接页面 "Superset" → "Setting" → "Database Connections" → "+DATABASE"。',
        step32desc:
          '选择 TDengine 数据库连接。"SUPPORTED DATABASES" 下拉列表中选择 "TDengine" 项。注意：若下拉列表中无 "TDengine" 项，请检查安装顺序，确保 `TDengine Python 连接器` 在 `Superset` 安装之后再安装。',
