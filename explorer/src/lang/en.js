...
export default {
  ...
  dataIn: {
    ...
    csvFileProgress: "CSV File Progress",
  ...
  replication: {
    ...
    restoreTip: "Are you sure to {operate} the {id} restore task ?",
  ...
  datasource: {
    ...
    csvFileDesc: 'Please upload the file to the server where the taosx server is located and fill in the directory here; The taosx service will deeply traverse all CSV files in this directory and synchronize data in batches.',
    ...
    csvNewFileNotifyDesc: 'Do you listen for newly added files in the directory? Once enabled, the task will continue to run; Otherwise, only files that already exist at the time of task startup will be processed.',
    csvNotifyInterval: 'Notify Interval',
    csvNotifyIntervalDesc: 'Listening interval, default is 30s. The larger the setting, the worse the timeliness, but the smaller the impact on system performance.',
...
Windows: <code>C:\\TDengine\\cfg\\</code>`,
    ...
    virtual: {
      ...
      gds: {
        desc: `Looker Studio can quickly access ${grafanagds} and create interactive reports and dashboards using its web-based reporting features.The whole process does not require any code development.`,
        topdesc: "Using its ",
        topconnector: "partner connector",
        topdesc1: ` , Looker Studio can quickly access ${grafanagds} and create interactive reports and dashboards using its web-based reporting features.The whole process does not require any code development. Share your reports and dashboards with individuals, teams, or the world. Collaborate in real time. Embed your report on any web page.`,
        topdesc2: "Refer to ",
        topdesc3: `for additional information on utilizing the Looker Studio with ${grafanagds}.`,
    ...
    tools: {
      ...
      superset: {
        ...
        desc1: 'Through the Python connector of TDengine, Superset can support TDengine data sources and provide functions such as data presentation and analysis.',
        ...
        step2desc: "The Python connector of TDengine comes with a connection driver that supports Superset in versions 2.1.18 and later, which will be automatically installed in the Superset directory and provide data source services.The connection uses the WebSocket protocol, so it is necessary to install the `taos-ws-py` component of TDengine separately. The complete installation script is as follows:",
        step3: 'Configure',
        step3full: 'Configure TDengine Connection In Superset',
        step31desc: 'Enter the new database connection page, "Superset" → "Setting" → "Database Connections" → "+DATABASE".',
        step32desc: 'Select TDengine database connection, select the "TDengine" option from the drop-down list of "SUPPORTED DATABASES".If there is no TDengine option in the drop-down list, please confirm that the steps of installing, `Superset` is first and `Python Connector` is second.',
        step33desc: 'Write a name of connection in "DISPLAY NAME".',
        step34desc: 'The "SQLALCHEMY URL" field is a key connection information string, copy and paste the following string.',
        step35desc: 'Click "TEST CONNECTION" to test if the connection can be successful. After passing the test, click the "CONNECT" button to complete the connection.',
        step4: 'Start',
        step4full: 'Start',
        step40desc: 'There is no difference in the use of TDengine data source compared to other data sources. Here is a brief introduction to basic data queries:',
        step41desc: 'Click the "+" button in the upper right corner of the Superset interface, select "SQL query", and enter the query interface.',
        step42desc: 'Select the "TDengine" data source that has been created earlier from the dropdown list of "DATABASES" in the upper left corner.',
        step43desc: 'Select the name of the database to be operated on from the drop-down list of "SCHEMA" (system libraries are not displayed).',
        step44desc: '"SEE TABLE SCHEMA" select the name of the super table or regular table to be operated on (sub tables are not displayed).',
        step45desc: '"Subsequently, the schema information of the selected table will be displayed in the following area.',
        step46desc: 'In the SQL editor area, any SQL statement that conforms to TDengine syntax can be entered for execution.',
        step5: 'Example',
        step5full: 'Example',
        step5desc: 'We chose two popular templates from the Superset Chart template to showcase their effects, using smart meter data as an example:',
        step51desc: '"Aggregate" Type, which displays the maximum voltage value collected per minute during the specified time period in Group 4',
        step52desc: '"RAW RECORDS" Type, which displays the collected values of current and voltage during the specified time period in Group 4',
      },
    },
  },
  component: {
  ...
  taosuser: {
    ...
    conformDeleteRestoreTask: "Are you sure to delete the restore task ",
