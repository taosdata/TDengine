...
export default {
  ...
  datasource: {
    transformer: {
      ...
      expre_convert: '请输入转换字符，示例: {"val1": "convertded_val1", ...}',
    ...
    customcolname: "自定义列",
    customcol: "请输入自定义列",
    uploadcsvtip: "请上传 csv 文件",
    inputcsvdir: "请输入 csv 文件目录",
    ...
    csvNext: "解析",
    csvFileDir: "文件监听目录",
    csvFileDesc: '请将文件上传到 taosx server 所在服务器，并将目录填写到这里；taosx 服务将深度遍历这个目录下所有 csv 文件，批量同步数据。',
    csvFilePattern: '匹配模式',
    ...
    csvFileSort: '文件处理顺序',
    csvFileSortDesc: '系统处理文件列表的先后顺序，对初始文件列表和同一批次监听到的新增文件列表有效，不同监听批次按照时间先后顺序处理；如果监听目录下存在子目录，则同级目录下先处理文件后处理子目录。',
    supportCharacter: '文件名只支持以下字符：中文、大小写字母、数字、空格、%、$、@、点、中横线、下划线、括号（包括圆括号、方括号、大括号和中文括号）',
    csvFileNotExist: '文件已经被删除',
    csvFileCheckError: '获取文件失败：',
    ms: "毫秒",
    μs: "微秒",
    ns: "纳秒",
    getschema: "获取Schema",
...
Windows： <code>C:\\TDengine\\cfg\\</code>`,
    ...
    virtual: {
      ...
      gds: {
        desc: `Looker Studio可以快速访问 ${grafanagds} 并且通过基于网页的报表功能可以快速创建交互式的报表和仪表盘.`,
        topdesc: "使用",
        topconnector: "第三方连接器",
        topdesc1: `，Looker Studio可以快速访问 ${grafanagds} 并且通过基于网页的报表功能可以快速创建交互式的报表和仪表盘。整个过程不需要任何的代码编写过程。可以分享报表和仪表盘给不同的个人，团队以及全世界，还可以跟其他人员实时协作，另外在任何的网页里面嵌入您的报表。`,
        topdesc2: `更多使用 Looker Studio 和 ${grafanagds} 集成可以参考`,
        topdesc3: "。",
        step1: "选择数据源",
        step1desc: "目前的",
        step1desc1: " 连接器",
