...
Windows: <code>C:\\TDengine\\cfg\\</code>`,
    ...
    tools: {
      ...
      superset: {
        ...
        step1: 'Install Apache Superset',
        step11desc: 'Ensure that Apache Superset v2.1.0 or above is installed. If not, please visit ',
        step11desc1: 'official website',
        step11desc2: ' to install.',
        step2: 'Install TDengine Python',
        step2full: 'Install TDengine Python Connector',
        step2desc:
          'The Python connector of TDengine comes with a connection driver that supports Superset in versions 2.1.18 and later, which will be automatically installed in the Superset directory and provide data source services.The connection uses the WebSocket protocol, so it is necessary to install the `taos-ws-py` component of TDengine separately. The complete installation script is as follows:',
        step3: 'Configure',
        step3full: 'Configure TDengine Connection In Superset',
        step31desc:
          'Enter the new database connection page, "Superset" → "Setting" → "Database Connections" → "+DATABASE".',
        step32desc:
          'Select TDengine database connection, select the "TDengine" option from the drop-down list of "SUPPORTED DATABASES".If there is no TDengine option in the drop-down list, please confirm that the steps of installing, `Superset` is first and `Python Connector` is second.',
        step33desc: 'Write a name of connection in "DISPLAY NAME".',
        step34desc:
          'The "SQLALCHEMY URL" field is a key connection information string, copy and paste the following string.',
        step35desc:
          'Click "TEST CONNECTION" to test if the connection can be successful. After passing the test, click the "CONNECT" button to complete the connection.',
        step4: 'Start',
        step4full: 'Start',
        step40desc:
