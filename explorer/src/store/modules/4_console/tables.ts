import { deleteTableReq, getTagValue, createTableReq, getMatrixStructReq } from "@/api/tables";
import { getStableStructReq } from "@/api/stables";
import { VariableTableColumnType } from "@/const";
import { dataType } from "../../../views/3_explorer/views/components/utils";
      ...
      .then(async content => {
        ...
        content.forEach(item => {
          ...
          if (item.typeName == "column") {
            const typeList = handleTypeList(item.dataType)
            columns.push({ 
              ...item, 
        ...
        if (payload.stableName) {
          const tagValueObj = await getTagValue(tags, rootState.dbs.selected_db, payload.stableName, payload.tableName);
          tags.forEach(item => {
