import { getStableListReq, deleteStableReq, createStableReq, getStableStructReq } from "@/api/stables";
...
const actions = {
  ...
  getStatleStruct({ commit, rootState }, payload) {
    const { stableName, type } = payload;
    return getStableStructReq({
      selected_db: rootState.dbs.selected_db,
      stableName: stableName,
      type: type
    }).then(res => {
      const { ts_field_name, columns, tags } = res;
