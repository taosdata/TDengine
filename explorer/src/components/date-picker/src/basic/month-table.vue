...
<script type="text/babel">
  ...
  import { isDate, range, getDayCountOfMonth, nextDate, switchTimezone } from '@/utils/date-util';
  import { hasClass } from '@/utils/dom';
  import { arrayFindIndex, coerceTruthyValueToArray, arrayFind } from '@/utils/util';

  const datesInMonth = (year, month) => {
    const numOfDays = getDayCountOfMonth(year, month);
    const firstDay = switchTimezone(new Date(year, month, 1));
    return range(numOfDays).map(n => nextDate(firstDay, n));
  };

  const clearDate = (date) => {
    return switchTimezone(new Date(date.getFullYear(), date.getMonth()));
  };
  ...
  export default {
    ...
    methods: {
      cellMatchesDate(cell, date) {
        const value = switchTimezone(new Date(date));
        return this.date.getFullYear() === value.getFullYear() && Number(cell.text) === value.getMonth();
      },
      getCellStyle(cell) {
        const style = {};
        const year = this.date.getFullYear();
        const today = switchTimezone(new Date());
      ...
      getMonthOfCell(month) {
        const year = this.date.getFullYear();
        return switchTimezone(new Date(year, month, 1));
      },
      markRange(minDate, maxDate) {
        ...
        for (let i = 0, k = rows.length; i < k; i++) {
          ...
          for (let j = 0, l = row.length; j < l; j++) {
            ...
            const cell = row[j];
            const index = i * 4 + j;
            const time = switchTimezone(new Date(this.date.getFullYear(), index)).getTime();
    ...
    computed: {
      rows() {
        ...
        const selectedDate = [];
        const now = getMonthTimestamp(switchTimezone(new Date()));
        ...
        for (let i = 0; i < 3; i++) {
          ...
          for (let j = 0; j < 4; j++) {
            ...
            cell.text = index;
            let cellDate = switchTimezone(new Date(time));
