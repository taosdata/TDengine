...
// @ts-nocheck
import { hasOwn, isObject, isPropAbsent } from 'element-plus/es/utils/index';
...
export default class TreeStore {
  ...
  updateNodeChildrenByKey(key: TreeKey | TreeNodeData | Node, callback?: AnyFunction): boolean {
    const node = this.getNode(key);
    if (node) {
      node.reloadData(callback);
      return true;
    } else {
      return false;
    }
