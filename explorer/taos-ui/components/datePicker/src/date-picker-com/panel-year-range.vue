      ...
      <div :class="ppNs.e('body')">
        <div :class="leftPanelKls.content">
          <div :class="drpNs.e('header')">
            <button type="button" :class="leftPanelKls.arrowLeftBtn" @click="leftPrevYear">
              <slot name="prev-year">
        ...
        <div :class="rightPanelKls.content">
          <div :class="drpNs.e('header')">
            ...
            <button type="button" :class="rightPanelKls.arrowRightBtn" @click="rightNextYear">
              <slot name="next-year">
                <el-icon><d-arrow-right /></el-icon>
              </slot>
            </button>
...
const leftPanelKls = computed(() => {
  return {
    ...
    arrowRightBtn: [ppNs.e('icon-btn'), { [ppNs.is('disabled')]: !enableYearArrow.value }, 'd-arrow-right']
  };
});
...
const rightPanelKls = computed(() => {
  return {
    ...
    arrowLeftBtn: [ppNs.e('icon-btn'), { 'is-disabled': !enableYearArrow.value }, 'd-arrow-left'],
...
const getDefaultValue = () => {
  ...
  if (isArray(defaultValue.value)) {
    const left = switchTimezone(dayjs(defaultValue.value[0]));
    let right = switchTimezone(dayjs(defaultValue.value[1]));
    if (!props.unlinkPanels) {
      right = left.add(10, unit);
    }
    return [left, right];
  } else if (defaultValue.value) {
    start = switchTimezone(dayjs(defaultValue.value));
  } else {
    start = switchTimezone(dayjs());
  }
