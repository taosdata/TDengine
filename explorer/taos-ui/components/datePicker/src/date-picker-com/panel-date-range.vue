        ...
        <div :class="[ppNs.e('content'), drpNs.e('content')]" class="is-left">
          <div :class="drpNs.e('header')">
            ...
            <button
              ...
              :class="[ppNs.e('icon-btn'), { 'is-disabled': !enableMonthArrow }]"
...
import { getDefaultValue, isValidRange, switchTimezone } from '../utils';
...
const leftDate = ref<Dayjs>(switchTimezone(dayjs()).locale(lang.value));
const rightDate = ref<Dayjs>(switchTimezone(dayjs()).locale(lang.value).add(1, unit));
...
const isValidValue = (date: [Dayjs, Dayjs]) => {
  return (
    isValidRange(date) && (disabledDate ? !disabledDate(date[0].toDate()) && !disabledDate(date[1].toDate()) : true)
  );
};
...
const formatEmit = (emitDayjs: Dayjs | null, index?: number) => {
  if (!emitDayjs) return;
  if (defaultTime) {
    const defaultTimeD = switchTimezone(dayjs(defaultTime[index as number] || defaultTime)).locale(lang.value);
...
const handleDateInput = (value: string | null, type: ChangeType) => {
  ...
  const parsedValueD = switchTimezone(dayjs(value, dateFormat.value)).locale(lang.value);
...
const handleTimeInput = (value: string | null, type: ChangeType) => {
  ...
  const parsedValueD = switchTimezone(dayjs(value, timeFormat.value)).locale(lang.value);
...
const handleMaxTimePick = (value: Dayjs | null, visible: boolean, first: boolean) => {
...
const parseUserInput = (value: Dayjs | Dayjs[]) => {
  return isArray(value)
    ? value.map(_ => switchTimezone(dayjs(_, format.value)).locale(lang.value))
    : switchTimezone(dayjs(value, format.value)).locale(lang.value);
};

function onParsedValueChanged(minDate: Dayjs | undefined, maxDate: Dayjs | undefined) {
  if (props.unlinkPanels && maxDate) {
    ...
    if (maxDate) {
      rightDate.value = rightDate.value.hour(maxDate.hour()).minute(maxDate.minute()).second(maxDate.second());
    }
  }
}
