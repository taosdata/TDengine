...
import { datesInMonth, getValidDateOfMonth, switchTimezone } from '../utils';
...
const tableRows = ref<MonthCell[][]>([[] as MonthCell[], [] as MonthCell[], [] as MonthCell[]]);
const lastRow = ref<number>();
const lastColumn = ref<number>();
const rows = computed<MonthCell[][]>(() => {
  const rows = tableRows.value;

  const now = switchTimezone(dayjs()).locale(lang.value).startOf('month');
...
const handleMonthTableClick = (event: MouseEvent | KeyboardEvent) => {
  ...
  if (props.selectionMode === 'months') {
    ...
    const newValue = hasClass(target, 'current')
      ? castArray(props.parsedValue).filter(d => d?.month() !== newMonth.month())
      : castArray(props.parsedValue).concat([switchTimezone(dayjs(newMonth))]);
    emit('pick', newValue);
