---
description: 车联网场景下的海量采集、存储、乱序治理与查询分析实践
title: 车联网
sidebar_label: 车联网
...
- 极简大数据平台：与传统大数据平台相比，系统将消息队列、流式计算、实时缓存、ETL 工具以及数据库本体集成于一体，构建了极为简洁的架构，同时增强了实时性，大幅减少了验证和维护过程中的工作量。
...
![物流车联网系统架构](assets/distribution.png)
