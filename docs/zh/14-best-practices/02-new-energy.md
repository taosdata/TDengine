---
description: 分布式光伏与储能场景的数据接入、分发与聚合分析实践
title: 新能源
sidebar_label: 新能源
...
![从营销系统接入 TDengine](assets/energy-kafka.png)

针对外部数据源的接入场景，TDengine TSDB Enterprise 提供了专业的 taosX 数据接入组件。用户无须编写任何代码，只须通过配置参数即可迅速接入营销侧 Kafka 消息队列中的分布式光伏电站采集数据，实现提取解析、过滤、数据映射等操作，并将处理后的数据写入 TDengine。这种方法不再依赖第三方 ETL 工具，如下图所示：

![TDengine 外部数据源接入流程](assets/energy-data-in.png)
...
![分布式光伏电站运行数据的分发架构](assets/energy-distribution.png)
