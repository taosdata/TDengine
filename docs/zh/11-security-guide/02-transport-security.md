...
sidebar_label: 传输安全
title: 传输安全
description: TDengine 服务端 SSL/TLS 证书生成与 taosAdapter 配置
---

本指南说明 TDengine 服务端的 SSL/TLS 基础设施配置，包括证书生成与 taosAdapter 服务端配置。

:::note 应用层安全
如需了解客户端 SSL/TLS（TrustStore、`wss`/`useSSL`、REST HTTPS）、Token 认证、动态轮换、连接池管理等应用层安全实践，请参阅 [连接器安全](./04-connector-security.md)。
:::

:::note
WebSocket / REST 对外加密在 **taosAdapter** 上配置。原生协议（`taosc` ↔ `taosd`）的加固说明见 [安全部署配置建议](./06-security-suggestions.md)。
:::
...
本文档适用于 WebSocket 连接，SSL 配置在 **taosAdapter** 服务上进行。taosAdapter 的 `[ssl]` 为企业版能力（配置注释中注明 `Applicable for the Enterprise Edition`）。完整参数说明见 [taosAdapter](../12-operations-and-tooling/03-components/03-taosadapter.md)。
:::
...
- 如果在 1.4 步中将证书复制到了 `/etc/taos/`，则使用上述示例路径。
- 如果使用了其他路径，请相应修改 `certFile` 和 `keyFile` 的值。
...
客户端 SSL/TLS（TrustStore、`wss`/`useSSL`、REST HTTPS）已统一收敛到 [连接器安全](./04-connector-security.md#2-ssltls-配置)。

如需 Token 认证、动态轮换、连接池安全实践，也请参阅该文档。
