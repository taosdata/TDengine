...
| 0x800026AB | Invalid fill mode | 在 interval 窗口中使用 fill(near) 模式 | 使用 interval 窗口支持的 fill 模式 |
| 0x800026AC | Invalid fill values | 错误使用 fill values 参数 | 使用正确的 fill 模式与 fill values 参数配合 |
| 0x800026AD | Invalid surrounding time value | 填写了错误的 surrounding time 值 | 使用正确有效的时间范围和时间单位 |
