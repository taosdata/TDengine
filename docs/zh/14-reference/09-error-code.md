...
| 0x8000010B | Driver was not loaded                   | 未在系统路径中找到 libtaosinternal.so 或 libtaosws.so                      | 重新安装客户端驱动 |
| 0x8000010C | Function was not loaded from the driver | 在 libtaos.so 中定义的一些函数在 libtaosinternal.so 或 libtaosws.so 中未实现 | 重新安装客户端驱动 |
