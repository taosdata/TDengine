...
该示例展示了如何使用 taosgen 工具通过生成器动态生成数据并写入 TDengine 数据库。配置中定义了数据库连接、表结构、数据生成规则，使用 STMT v2 接口进行高效批量写入，包含完整的从数据库创建到数据插入的自动化流程。
...
- 全局配置 (global)
  ...
  - 数据库信息 (database_info): 定义目标数据库 benchdebug，设置时间精度为毫秒，4个 vgroup
  - 超级表信息 (super_table_info): 定义超级表结构，包含3个普通列（电流、电压、相位）和2个标签列（组ID、位置）
  - 子表名生成器 (tbname_generator)：定义生成 10000 张子表名称的规则，格式为 d0 到 d9999。

- 并发控制
  - 作业并发度 (concurrency): 4: 设置全局并发度为4，允许最多4个作业同时执行

- 作业依赖关系
  - 创建数据库 → 创建超级表 → 创建子表 → 插入数据，通过 needs 参数形成有向无环图

- 数据生成配置
  - 子表名称: 使用生成器动态创建 10 万张子表，名称格式为 d0 到 d99999。
  - 标签数据: 同样使用生成器，根据 Schema 为每个子表生成随机的 groupid 和 location 标签值。
  - 时序数据: 普通列数据（current, voltage, phase）由生成器根据定义的规则（如随机数范围）动态生成。
  - 时间戳: 配置了时间戳生成策略，从 1700000000000 (2023-11-14 22:13:20 UTC) 开始，以 1 毫秒的步长递增。

- 高效写入
  - 接口: 在 insert-data Action 中显式指定使用 stmt (参数化写入) 格式及其 v2 版本，极大提升了批量数据插入的性能。
  - 交错模式 (interlace_mode): 已启用，配置为每次为每个子表生成 1 行数据后就切换，这有助于在写入大量子表时获得更平滑的吞吐量。
...
- 测试和评估 TDengine 集群在 海量时间序列数据写入压力下的吞吐量、延迟和稳定性。
- 验证数据库schema设计、资源规划以及不同硬件配置下的性能表现。
- 为电力监控、工业物联网等领域的系统容量规划提供数据支撑
...
该示例展示了如何使用 taosgen 工具从 CSV 文件读取数据并写入 TDengine 数据库。配置中定义了从两个CSV文件分别读取设备标签信息和电表读数数据，使用STMT v2接口进行高效批量写入，包含完整的数据库、超级表创建流程。
...
- 数据生成配置，与生成器方式不同，此配置从CSV文件获取数据
  ...
  - 数据插入 (insert-data Action):
    ...
    - 时间戳策略: 由于CSV源数据中的时间戳列可能不存在或格式不匹配，此处采用生成器策略，从指定时间戳（1700000000000）开始，以1毫秒步长自动生成时间戳。
...
该示例展示了如何使用 taosgen 工具生成模拟设备数据并通过 MQTT 协议进行发布。配置中定义了数据格式、生成规则及 MQTT 连接参数，适用于物联网场景下的设备数据模拟与实时推送，支持动态主题和多种数据类型的灵活生成。

配置详解：
- 全局配置 (global)
  - 超级表信息 (super_table_info): 定义了数据模型结构，虽然不直接创建数据库表，但作为数据生成的 Schema 模板。包含4个普通列（电流、电压、相位、设备状态）。
  - 子表名称生成器 (tbname_generator): 定义生成 10000 张子表名称的规则，格式为 d0 到 d9999。
