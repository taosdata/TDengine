...
场景说明：
适用于性能基准测试和大规模数据模拟场景，如生成海量设备时序数据以测试 TDengine 的写入吞吐量和集群性能。通过动态生成数据，可灵活调整数据量和并发度，验证系统在高负载下的表现。
...
场景说明：
适用于设备数据迁移或批量初始化场景，如将已有的电表历史数据从CSV文件导入TDengine数据库。通过分离标签文件和数据文件，可高效完成大量设备数据的结构化存储，为后续时序数据分析提供基础。
...
场景说明：
适用于物联网设备监控场景，如工厂中的电表数据采集。通过生成模拟数据并推送至 MQTT 主题，可测试实时数据接收、设备状态监控及后端系统处理能力。
