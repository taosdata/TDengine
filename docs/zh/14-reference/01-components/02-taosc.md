...
|参数名称|支持版本|参数含义|
|-----------------|-----------|-|
|tempDir          |           |指定所有运行过程中的临时文件生成的目录，Linux 下缺省值为 /tmp|
...
|tqClientDebugFlag|           |客户端模块的日志开关，取值范围同上|
|simDebugFlag     |           |内部测试工具的日志开关，取值范围同上|
...
|simdEnable       |           |是否启动 SIMD 加速|
|AVX512Enable     |           |是否启动 AVX512 加速|
