...
预配置告警规则导入方法如下图所示，在tdengine-datasource setting界面，打开 “Load Tengine Alert” 开关，点击 “Save & test” 按钮后，插件会自动加载上述告警规则, 规则会放入以数据源名称 + “-alert” 的 grafana 告警目录中。如不需要，关闭Load TDengine Alert开关。点击 “Clear TDengine Alert” 旁边的按钮则会清除此数据源已导入的所有告警规则。 

![TDengine Alert](./assets/TDengine-Alert.webp)  

导入后，点击Grafana左侧Alert rules，可查看当前所有告警规则。 
