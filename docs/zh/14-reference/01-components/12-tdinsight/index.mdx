...
从 TDengine-server 3.3.4.3版本（tdengine-datasource 3.6.3）开始，TDengine Datasource 支持预配置告警规则自动导入功能，用户可将14个告警规则一键导入 Grafana（11及以上版本），直接使用。  
...
Grafana7.5 及以下版本，Dashboards 与 Alert rules 功能合在一起，而之后的新版本两个功能是分开的。为兼容 Grafana7.5及以下版本，TDinsight 面板中增加了 Alert Used Only 面板，仅 Grafana7.5及以下版本需要使用。
