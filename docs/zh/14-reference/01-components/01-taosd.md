...
1. 所有配置参数都将被持久化到本地存储，重启数据库服务后，将默认使用持久化的配置参数列表；对于局部配置参数如果您希望继续使用 config 文件中局部配置参数，需设置 forceReadConfig 为 1。
2. 只有局部配置参数受 forceReadConfig 的控制，而全局配置参数必须通过 alter all dnodes 修改，不受 forceReadConfig 的控制。
3. 在 3.2.0.0 ~ 3.3.0.0（不包含）版本生效，启用该参数后不能回退到升级前的版本。
4. TSZ 压缩算法是通过数据预测技术完成的压缩，所以更适合有规律变化的数据。
5. TSZ 压缩时间会更长一些，如果您的服务器 CPU 空闲多，存储空间小的情况下适合选用。
6. 示例：对 float 和 double 类型都启用有损压缩。
