...
| 局部配置参数  | 每个集群节点本地单独配置的参数  |  单节点    | 1. 通过 SQL 修改。
 2. 通过 taos.cfg 配置文件修改。  | show dnode <dnode_id> variables;|
...
4. 配置参数动态修改方法请您参考 [节点管理](../../reference/taos-sql/node/)。
5. 有些参数在客户端 (taosc) 和服务器端 (taosd) 中都有，在不同地方其作用范围和意义有所不同，详细请参考 [TDengine 配置参数作用范围对比](../../reference/components/config-scope)。
