...
补充说明：
1. 通过 SQL 修改全局配置参数方法：`alter all dnodes ‘参数名’ ‘参数值’;` 
2. 通过 SQL 修改局部配置参数方法，`alter dnode <dnode_id> ‘参数名’ ‘参数值’;`
3. 通过 taos.cfg 配置文件修改局部配置参数，修改后把 `forceReadConfig` 参数设为 1 并重启后方可生效。
4. 配置参数动态修改方法请您参考 [节点管理](https://docs.taosdata.com/reference/taos-sql/node/)。
5. 有些参数在客户端 (taosc) 和服务器端 (taosd) 中都有，在不同地方其作用范围和意义有所不同，详细请参考 [TDengine 配置参数作用范围对比](https://docs.taosdata.com/reference/components/config-scope)。
