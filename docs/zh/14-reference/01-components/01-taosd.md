...
- 说明：慢查询日志阈值，单位为秒，执行时间超过该阈值的查询将记录到慢查询日志
- 类型：整数
- 单位：秒
- 默认值：10
- 最小值：1
...
- 参数类型：全局配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
- 支持版本：从 v3.3.4.6 版本开始引入

#### slowLogMaxLen

- 说明：慢查询日志中 SQL 语句的最大长度
- 类型：整数
- 默认值：4096
- 最小值：1
- 最大值：16384
- 参数类型：全局配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
- 支持版本：从 v3.3.2.6 版本开始引入

#### slowLogScope

- 说明：慢查询日志记录的范围，可选值：all（所有）、query（查询）、insert（插入）、others（其他）、none（不记录）
- 类型：字符串
- 默认值：query
- 参数类型：全局配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
...
- 说明：慢查询日志排除的数据库，该数据库的慢查询不会被记录
- 类型：字符串
- 默认值：空
- 参数类型：全局配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
- 支持版本：从 v3.3.4.2 版本开始引入
...
- 说明：审计数据上报的级别 **`企业版参数`**
- 类型：整数; 1:系统级别，2:集群级别，3:数据库级别，4:子表级别，5:数据级别
- 默认值：3
- 最小值：1
- 最大值：5
- 参数类型：全局配置参数
...
- 说明：UDF 常驻函数列表，这些函数在 udfd 退出时才卸载**`内部参数`**
- 类型：字符串
- 默认值：空
- 参数类型：全局配置参数
- 动态修改：支持通过 SQL 修改，重启生效。
...
- 说明：mnode 流管理线程数
- 类型：整数
- 默认值：CPU 核数的四分之一（不低于2，不超过5）
- 最小值：2
- 最大值：5
- 参数类型：局部配置参数
- 动态修改：不支持
- 支持版本：从 v3.3.6.3 版本开始引入

#### numOfStreamMgmtThreads

- 说明：vnode 流管理线程数
- 类型：整数
- 默认值：CPU 核数的八分之一（不低于2，不超过5）
- 最小值：2
- 最大值：5
- 参数类型：局部配置参数
- 动态修改：不支持
...
- 类型：布尔值；false：按数值类型比较；true：按字符串比较。
- 默认值：true
- 参数类型：局部配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
- 支持版本：从 v3.3.5.10 版本开始引入

#### showFullCreateTableColumn

- 说明：SHOW CREATE TABLE 是否显示完整的列信息
- 类型：布尔值；false：不显示列属性 ENCODE/COMPRESS/LEVEL 信息；true：显示完整创建语句。
- 默认值：false
- 参数类型：局部配置参数
- 动态修改：支持通过 SQL 修改，立即生效。
- 支持版本：从 v3.3.7.0 版本开始引入
...
- 说明：checkpoint 备份目录
- 类型：字符串
- 默认值：/var/lib/taos/backup/checkpoint/ (Linux), C:\TDengine\data\backup\checkpoint\ (Windows)
- 参数类型：局部配置参数
- 动态修改：支持通过 SQL 修改，重启后生效。
- 支持版本：从 v3.2.3.0 版本开始引入
