...
|    shareConnLimit    |          集群间通信一个conn 最多可以挂的req个数，取值范围 1-256，缺省值：10           |
|    readTimeout    |             集群间通信最小超时时间，单个请求如果超过这个值没有任何响应，则可能被主动摘掉，摘掉的请求会主动向业务侧通知，并返回错误码, 取值范围64-603800, 缺省值900,单位为秒         |
