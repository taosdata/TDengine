...
注意：TDengine ODBC 支持 64 位 Windows 系统，支持 32 位 Websocket 连接方式（仅企业版支持），调用 TDengine ODBC 需要通过相应位数的 ODBC 驱动管理器进行。在 32 位 Windows 系统或者 64 位 Windows 系统的 32 位应用程序中，仅支持使用 WebSocket 连接方式访问 TDengine 数据库。
