...
`taospy` 是 TDengine 数据库面向 Python 语言提供的官方连接器，连接器提供对外提供了对数据库写入、查询、订阅等多种操作接口。
安装命令：
``` bash
pip3 install taospy
```

连接器对外开源，源码托管在 Github [taos-connector-python](https://github.com/taosdata/taos-connector-python) 仓库。
...
|:------------------:|----| :----: |
|2.7.16|新增订阅配置 (session.timeout.ms, max.poll.interval.ms)|3.3.2.0|
|2.7.15|新增 VARBINARY 和 GEOMETRY 类型支持|3.2.3.5|
|2.7.14|修复已知问题|3.2.3.5|
|2.7.13|新增 tmq 同步提交 offset 接口|3.1.1.25|
|2.7.12|1. 新增 varbinary 类型支持（STMT暂不支持 varbinary ）<br/> 2. query 性能提升（感谢贡献者[hadrianl](https://github.com/taosdata/taos-connector-python/pull/209)）|3.1.1.2|
|2.7.9|数据订阅支持获取消费进度和重置消费进度|3.0.2.6|
|2.7.8|新增 `execute_many`|3.0.2.2|

WebSocket 版本连接器  

|WebSocket Connector 版本|主要变化|
|:---------------------:|-----|
