...
```text
[+<protocol>]://[<username>:<password>@][<host1>:<port1>[,...<hostN>:<portN>]][/<database>][?<k1>=<v1>[&...<kN>=<vN>]]
|-----------|---|----------|-----------|-------------------------------------|------------|--------------------------|
...
- **addresses**：指定创建连接的一个或多个服务器地址，多个地址用英文逗号分隔。对于 IPv6 地址，必须使用中括号括起来（如 `[::1]` 或 `[2001:db8:1234:5678::1]`），以避免端口号解析冲突。
- **database**：指定默认连接的数据库名，可选参数。
- **params**：其他可选参数。例如 token。
...
- `fn connect(dsn: Option<&str>, args: Option<&PyDict>) -> PyResult<Connection>`
    ...
    - **参数说明**：
        ...
        - `args`：类型 `Option<&PyDict>` 可选，以 Python 字典的形式提供，可用于设置
            ...
            - `port`：端口号
            - `database`：数据库名称
            - `websocket`：指定 WebSocket 连接所用的协议，支持的值为 `http` 或 `https`。
    - **返回值**：连接对象。
