...
```text
<driver>[+<protocol>]://[<username>:<password>@][<host1>:<port1>[,...<hostN>:<portN>]][/<database>][?<k1>=<v1>[&...<kN>=<vN>]]
|------|------------|---|----------|-----------|-------------------------------------|------------|--------------------------|
...
- **addresses**：指定创建连接的一个或多个服务器地址，多个地址用英文逗号分隔。当不指定服务器地址时（`taos://`），原生连接默认为 `localhost:6030`，WebSocket 连接默认为 `localhost:6041` 。
  > 注意：原生连接只支持配置一个地址，WebSocket 连接支持配置多个地址。
