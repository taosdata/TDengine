...
- `int taos_options(TSDB_OPTION option, const void * arg, ...)`
  - **接口说明**：设置客户端选项，支持区域设置（`TSDB_OPTION_LOCALE`）、字符集设置（`TSDB_OPTION_CHARSET`）、时区设置（`TSDB_OPTION_TIMEZONE`）、配置文件路径设置（`TSDB_OPTION_CONFIGDIR`）、驱动类型设置（`TSDB_OPTION_DRIVER`）。区域设置、字符集、时区默认为操作系统当前设置。驱动类型可选内部原生接口(`internal`)和 WebSocket 接口()`websocket`)，默认为 `internal`。
  - **参数说明**：
    - `option`：[入参] 设置项类型。
    - `arg`：[入参] 设置项值。
  - **返回值**：`0`：成功，`-1`：失败。
