...
**WebSocket 连接**  
使用 JDBC WebSocket 连接，不需要依赖客户端驱动。这是一个示例： `jdbc:TAOS-WS://taosdemo.com:6041/power?user=root&password=taosdata&varcharAsString=true`。与 JDBC 原生连接相比，仅需要：
...
对于 REST 连接，url 中的常见配置参数如下（所有参数请参考 Properties 一节）：
- user：登录 TDengine TSDB 用户名，默认值 'root'。
- password：用户登录密码，默认值 'taosdata'。
- batchErrorIgnore：true：在执行 Statement 的 executeBatch 时，如果中间有一条 SQL 执行失败，继续执行下面的 SQL 了。false：不再执行失败 SQL 后的任何语句。默认值为：false。
- httpConnectTimeout：连接超时时间，单位 ms，默认值为 60000。
...
**注意**：部分配置项（比如：locale、charset 和 timezone）在 REST 连接中不生效，返回数据的时间戳取决于应用 JVM 的时区。
