...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]：仅在使用 JDBC WebSocket 连接时生效，令牌信息。其认证优先级高于用户名密码。详见 [连接器安全最佳实践](../../../develop/connector-security-best-practices/#3-token-认证)。
...
- TSDBDriver.PROPERTY_KEY_USE_SSL [`useSSL`]：连接中是否使用 SSL。仅在 WebSocket 连接时生效。详见 [客户端 SSL/TLS 配置](../../../develop/connector-security-best-practices/#22-客户端-ssltls-配置)。
...
- TSDBDriver.PROPERTY_KEY_ADAPTER_HA [`adapterHA`]：是否启用 `adapter` 高可用。仅在使用 WebSocket 连接时生效。true：启用，false：不启用。默认为 false。启用后连接成功就能获取集群所有 `adapter` 列表用于负载均衡，需要 TDengine 版本大于等于 `3.4.2.0` 才能支持此特性。
