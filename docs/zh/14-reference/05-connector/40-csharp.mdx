...
- .Net 6.0 及以上版本，可以统一使用 [IANA](https://www.iana.org/time-zones) 时区格式。

以纽约时区为例：

- .Net 版本小于 6.0 时，Windows 上使用 `Eastern Standard Time`，Linux 和 macOS 上使用 `America/New_York`。
- .Net 版本大于等于 6.0 时，可以统一使用 `America/New_York`。
