...
<font color="red">C# 的原生连接将于 2027-01-01 下线</font>，请迁移到 WebSocket 连接。
...
|:-------------|:---------------------------------|:-----------------|
| 3.2.2        | 支持 taosAdapter 高可用。              | 3.4.1.14 及更高版本 |
| 3.2.1        | 支持 BLOB 类型 SQL/STMT2 写入和查询；支持 DECIMAL 类型 SQL/STMT2 写入和查询。 | 3.4.1.8 及更高版本 |
| 3.2.0        | 支持 WebSocket 多地址故障转移。              | -                |
...
- DECIMAL 类型不支持作为 tag 列。在 C# 中使用 `decimal` 类型表示，支持高精度的十进制数值。因为 C# 的 `decimal` 类型与 TDengine TSDB 的 DECIMAL 类型在精度和范围上有所不同，
  ...
  - stmt 参数绑定写入 DECIMAL 列时，支持 `string` 和 C# `decimal` 两种类型。C# `decimal` 会自动转换为字符串后发送给服务端。如果数值精度超过 29 位，建议使用 `string` 类型以避免精度损失。
...
从 `TDengine.Connector` `3.2.0` 开始，WebSocket 连接中的 `host` 参数支持使用逗号分隔的地址列表来实现故障转移；Native 连接仍然只支持单地址。
...
- `host`：只能填写单个 TDengine 地址。单地址格式支持 `host`、`host:port`、裸 IPv6 `2001:db8::1`、`[2001:db8::1]` 和 `[2001:db8::1]:6030`。Native 连接不支持多地址故障转移。
- `port`：TDengine TSDB 运行实例的端口。如果 `host` 中已经显式写了端口，则以地址中的端口为准。
- `username`：连接的用户名。
- `password`：连接的密码。
...
- `bearerToken`：连接 TDengine TSDB 的 token 验证信息（`3.1.10` 及以上版本支持）。

如果 `protocol=Native` 且 `host` 中配置了多个地址，打开连接时会抛出 `ArgumentException`。

##### WebSocket 连接

单地址示例：`"protocol=WebSocket;host=127.0.0.1;port=6041;useSSL=false;enableCompression=true"`

故障转移示例（`3.2.0` 及以上版本）：`"protocol=WebSocket;host=adapter-a:6041,adapter-b:6041;username=root;password=taosdata;autoReconnect=true;reconnectIntervalMs=2000;reconnectRetryCount=3"`

支持的参数如下：

- `host`：TDengine WebSocket 地址或地址列表。从 `3.2.0` 开始，可以使用逗号分隔多个地址来启用故障转移。单地址格式支持 `host`、`host:port`、裸 IPv6 `2001:db8::1`、`[2001:db8::1]` 和 `[2001:db8::1]:6041`。多地址列表中的 IPv6 必须使用方括号，例如 `host=[::1]:6041,[::1]:6042`。
- `port`：共享兜底端口，只对未在地址中显式指定端口的项生效。如果地址和 `port` 都未指定端口，WebSocket 默认使用 `6041`，`useSSL=true` 时默认使用 `443`。
- `username`：连接的用户名。
- `password`：连接的密码。
- `protocol`：连接协议，可选值为 `Native` 或 `WebSocket`，默认是 `Native`。
- `db`：连接的数据库。
...
- `autoReconnect`：是否自动重连，默认为 false。当在 `3.2.0` 及以上版本中配置了多个 WebSocket 地址时，该参数控制当前连接不可用后的运行时故障转移。
- `reconnectRetryCount`：重连轮数，默认为 3。
- `reconnectIntervalMs`：重连轮之间的间隔毫秒数，默认为 2000。
- `bearerToken`：连接 TDengine TSDB 的 token 验证信息（`3.1.10` 及以上版本支持）。
- `adapterHA`：是否启用 taosAdapter 高可用，默认为 false（`3.2.2` 及以上版本支持）。启用后，连接器在建立连接和订阅时会向 taosAdapter 请求当前可用的 adapter 实例列表，并自动将新发现的节点加入故障转移地址池。当某节点下线时，后续响应中将不再包含该节点，连接器会自动从地址池中移除。该功能需要配合 taosAdapter 的服务注册机制使用。

###### 故障转移说明

- WebSocket 故障转移从 `3.2.0` 开始支持。
- 首次建立 WebSocket 连接时会自动尝试配置的地址；`autoReconnect` 只控制连接建立后中断时的故障转移。
- 连接器使用**最少连接（Least Connections）**算法选择地址：优先连接当前活跃连接数最少的节点。当 `autoReconnect=true` 时，会先尝试最近一次成功连接的地址，再按最少连接策略尝试其他已配置地址。
- 当 `adapterHA=true` 时，连接器会在每次建立连接/订阅成功后，从 taosAdapter 返回的实例列表动态扩展或收缩故障转移地址池。同一进程内多个连接共享全局注册表，任一连接发现的新节点对其他连接也立即生效。
- Native 连接不支持多地址故障转移。
...
- `td.connect.ip`：TMQ 使用的 TDengine WebSocket 地址。从 `3.2.0` 开始，也支持逗号分隔的多地址列表来实现故障转移。多地址列表中的 IPv6 必须使用方括号，例如 `td.connect.ip=[::1]:6041,[::1]:6042`。
- `td.connect.port`：共享兜底端口，会补给 `td.connect.ip` 中未显式写端口的地址。
- `useSSL`：是否使用 SSL 连接，默认为 false。
- `token`：连接 TDengine TSDB cloud 的 token。
- `ws.message.enableCompression`：是否启用 WebSocket 压缩，默认为 false。
- `ws.autoReconnect`：是否自动重连，默认为 false。当 `3.2.0` 及以上版本在 `td.connect.ip` 中配置了多地址时，该参数控制当前连接不可用后的运行时故障转移。
- `ws.reconnect.retry.count`：重连轮数，默认为 3。
- `ws.reconnect.interval.ms`：重连轮之间的间隔毫秒数，默认为 2000。
- `connectionTimezone`：连接级别的时区设置（3.1.8 及以上版本支持，目前只有解析结果集使用的时区功能），仅支持 .NET 6 及以上版本，只支持 IANA 时区格式，详情见[时区说明](#时区说明)。
- `adapterHA`：是否启用 taosAdapter 高可用，默认为 false（`3.2.2` 及以上版本支持）。启用后，消费者在订阅时会向 taosAdapter 请求可用实例列表，并自动扩展或收缩故障转移地址池。

其他参数请参考：[Consumer 参数列表](../../../develop/tmq/#创建参数)，注意 TDengine TSDB 服务端自 3.2.0.0 版本开始消息订阅中的 auto.offset.reset 默认值发生变化。

启用 TMQ 故障转移后，消费者会自动重连并重新订阅。业务侧仍应保证消费处理具备幂等性，因为故障转移后仍可能出现重复消费。
