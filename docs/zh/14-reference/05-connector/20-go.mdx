...
* `timezone` 指定连接使用的时区，sql 解析以及查询结果都会按照此时区进行转换，只支持 IANA 时区格式，特殊字符需要进行编码，以巴黎时区（`Europe/Paris`）为例：`timezone=Europe%2FParis`。
...
- `func (conn *Connector) SetTimezone(timezone string) error`
    - **接口说明**：设置连接时区。
    - **参数说明**：
        - `timezone`：时区字符串，使用 IANA 时区格式，例如：`Europe/Paris`。
    - **返回值**：错误信息。
