...
6. NEAR 填充：使用距离当前时间点最近的有效数据填充。例如 `FILL(NEAR)`。
...
4. FILL 具有连续性，如一列数据中仅第一条不为 NULL，则 fill(prev) 会为后续所有行填充该值。

:::

### SURROUND 子句

用于限制 FILL 子句的填充范围。只能在 PREV、NEXT、NEAR 模式下使用。

Surrounding_time_val 参数用于指定有效数据的时间范围，若拥有有效数据的行距离当前行的时间差超过 surrounding_time_val，则不使用其数据，转而使用 fill_vals 参数指定的值进行填充。其数值必须为正值，单位可选除月（n）、年（y）外的其他时间单位。在 interval 窗口查询中，由于数据间拥有同样的时间间隔，因此 surrounding_time_val 参数的大小必须超过 interval 窗口的时间长度。
