...
**功能说明**：返回满足某个条件的累计连续记录的个数。计算规则：如果条件满足则累计计算加 1，如果条件不满足则重置计数为-1，如果 expr 数据为 NULL，则输出为 NULL。
...
- val : 数值型
- unit : 输出累计时间长度的单位，可取值时间单位： 1b(纳秒), 1u(微秒)，1a(毫秒)，1s(秒)，1m(分)，1h(小时)，1d(天), 1w(周)。如果省略，默认为当前数据库精度。
