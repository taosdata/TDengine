...
**说明**: 当输入类型为DECIMAL类型时, 输入类型也为DECIMAL类型, 输出的precision和scale大小符合数据类型章节中的描述规则, 通过计算SUM类型和UINT64的除法得到结果类型。
...
**说明**: 输入类型为DECIMAL类型时, 输出类型为DECIMAL(38, scale), precision为当前支持的最大值, scale为输入类型的scale.
...
- INTERP在FILL PREV/NEXT/NEAR时, 行为与窗口查询有所区别, 如PREV, 所FILL值为前一个截面的值, 即使是NULL值, 并且当截面存在数据时, 不会进行FILL, 即便当前值为NULL.
