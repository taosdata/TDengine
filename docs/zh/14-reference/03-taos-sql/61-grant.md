...
- 同一表相同类型的权限只能设置一条规则。
- 撤销权限时，系统对 `priv_level`（权限层级）进行精确匹配，不支持递归撤销。例如，在执行 `REVOKE SELECT ON d0.* FROM u1` 时，尽管 `d0.*` 包含通配符，系统也仅撤销 `d0.*` 层级对应的 `SELECT` 权限，而不会递归撤销显式授予特定对象（如 `d0.t1`）的 `SELECT` 权限。
