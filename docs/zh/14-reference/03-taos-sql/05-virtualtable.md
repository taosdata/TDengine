...
5. 表的每行长度不能超过 512KB（注意：每个 VARCHAR/NCHAR/GEOMETRY 类型的列还会额外占用 2 个字节的存储位置）。
6. 使用数据类型 VARCHAR/NCHAR/GEOMETRY，需指定其最长的字节数，如 VARCHAR(20)，表示 20 字节。
7. 创建虚拟表时使用 FROM 来指定列的数据源，支持使用 db_name 跨库指定数据源，不指定 db_name 时默认使用当前 use 的数据库，若不指定 db_name 且未 use 数据库，则会报错。
8. 创建虚拟表时不显式的指定 ts 列的数据源，ts 列的取值是查询虚拟表时查询语句中包含的所有列对应的原始表的主键时间戳合并的结果。
...
13. 创建虚拟子表时，`TAGS (...)` 中的标签既可以使用常量值，也可以使用 tag-ref。当前支持三种 tag-ref 写法：`table.tag`、`FROM table.tag`、`tag_name FROM table.tag`；跨库时可写成 `db_name.table.tag`。
14. tag-ref 只能引用标签列，不能引用数据列；标签类型必须与目标虚拟标签类型一致。

### 当前支持的引用能力

- 虚拟子表支持 tag-ref，查询时会动态解析为源标签的当前值。
- 虚拟表列支持直接引用已有虚拟表的列，因此可以构建 VChild → VChild 的直接引用链。
- 可以在不同虚拟超级表之间分层构建虚拟子表引用链，当前已验证支持同库和跨库场景。
