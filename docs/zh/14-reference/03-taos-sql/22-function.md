...
**说明**：暂停执行指定的秒数。该函数在表达式求值时生效；如果查询中对多行分别计算该表达式，则可能按行分别休眠，因此不保证在表查询中只执行一次。执行 `KILL QUERY` 可在约 100ms 内中断休眠；被中断时，查询通常会以取消错误结束，而不是返回一条值为 `1` 的正常结果行。

**参数**：

- `seconds`：DOUBLE - 休眠的秒数（支持小数，如 0.5）；负数或 NULL 跳过休眠并返回 0

**返回值**：INT - 成功或参数为负数或 NULL 时返回 0；若执行过程中被 `KILL QUERY` 中断，通常返回查询取消错误而非正常结果行
...
-- 在此类查询中，SLEEP 会随结果行一起参与计算，请勿依赖“全查询只执行一次”的行为
