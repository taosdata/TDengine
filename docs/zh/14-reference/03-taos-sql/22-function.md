...
**适用数据类型**: VARCHAR、NCHAR、BOOL、数值类型和 TIMESTAMP。
...
- 如果 `expr` 为 `BOOL` 类型，则按 `0` 或 `1` 转换为字符串后再进行 Base64 编码。
- 如果 `expr` 为 `TIMESTAMP` 类型，则按 `"yyyy-mm-dd hh24:mi:ss"` 格式转换为字符串后再进行 Base64 编码。
- 其他非字符串类型会先转换为其字符串表示，再进行 Base64 编码。
- 输入长度受内部上限 `BASE64_MAX_INPUT_LEN` 限制；当前实现中，超出该上限时会按上限截断后再进行编码。
- 此函数是多字节安全的。
