...
- 若指定 `timezone`，支持 IANA 时区名与固定偏移格式 `[z/Z, +/-hhmm, +/-hh, +/-hh:mm]`，偏移范围为 `-14:00` 到 `+14:00`。
...
- 第三个参数同时支持整数标志位与字符串时区。
  ...
  - 字符串时区：支持 IANA 时区名和固定偏移格式 `[z/Z, +/-hhmm, +/-hh, +/-hh:mm]`，偏移范围为 `-14:00` 到 `+14:00`。
- 未指定第三参数时，优先使用当前连接时区；若连接未设置，则使用服务端默认时区；仍不可用时使用系统默认时区。
- `1w` 截断按 `firstDayOfWeek` 对齐，不再固定按周四对齐。`firstDayOfWeek` 的初始化来源与平台差异，参见 [SET 命令](./53-set.md)。
- 有歧义的时区缩写（如 `CST`）会被拒绝。
