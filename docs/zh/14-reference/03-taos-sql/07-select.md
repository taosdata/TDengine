...
- window_clause 指定数据按照窗口进行切分并进行聚合，是时序数据库特色查询。详细信息可参阅特色查询章节 [TDengine 特色查询](../distinguished)。
  - SESSION(ts_col, tol_val) 会话窗口，ts_col 指定时间戳主键列，tol_val 是时间间隔，正值，时间精度可选 1n、1u、1a、1s、1m、1h、1d、1w，如 SESSION(ts, 12s)。
...
- group_by_expr 指定数据分组聚合规则，支持表达式、位置、列、别名。使用位置语法时必须出现在选择列中，如```select ts, current from meters order by ts desc,2```，2 对应 current 列。
