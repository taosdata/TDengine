...
其中列 `a` 来自源表 A，列 `b` 来自源表 B。朴素执行路径下，引擎需要先将 A 和 B 的全量数据按时间戳归并为一张临时结果集，再在该结果集上执行聚合运算。

这一过程存在双重性能损失：

1. **归并开销**：即使 A 和 B 各有 100 万行且时间戳完全对齐，负责归并的算子仍需逐行完成时间戳比对和列拼接，这一过程本身即为显著的计算开销。
...
在时序数据场景中，状态列通常较为稀疏（设备状态仅在变化时写入，一天可能仅有数十条记录），而测量列则非常密集（传感器每秒采集一次，一天可达数万条）。即使状态列仅有 50 行，朴素方案仍需将其与聚合列的 500 万行进行全量归并，代价极高。
...
以典型场景为例：状态列 `a` 有 50 行（一天内状态变化 50 次），聚合列 `b` 有 500 万行（每秒采集一次的传感器数据）。
