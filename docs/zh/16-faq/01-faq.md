...
如果 FAQ 中的信息不能够帮到你，需要 TDengine 技术团队的技术支持与协助，请将以下两个目录中内容打包：
...
### 1.1 Windows 平台运行 TDengine 出现丢失 msvcp140.dll 解决方法？
...
2. 手动上网下载并替换 msvcp140.dll 文件‌：可以从可靠的源下载 msvcp140.dll 文件，并将其复制到系统的相应目录下。确保下载的文件与你的系统架构（32 位或 64 位）相匹配，并确保来源的安全性‌
...
11. 也可以使用 taos 程序内嵌的网络连通检测功能，来验证服务器和客户端之间指定的端口连接是否通畅：[运维指南](../12-operations-and-tooling/02-operations/index.md)。
...
有关 taosAdapter 组件的详细介绍请看文档：[taosAdapter](../12-operations-and-tooling/03-components/03-taosadapter.md)
...
问题描述：
在使用 TDengine 客户端应用（taos-CLI、taosBenchmark、taosdump 等）或客户端连接器（如 Java、Python、Go 等）时，可能会遇到加载动态链接库`libtaosnative.so`或`libtaosws.so`失败的错误。
...
### 4.2 在服务器上的使用 TDengine CLI 能查到指定时间段的数据，但在客户端机器上查不到？
...
直接查询子表速度更快。使用 TAG 过滤查询超级表是为了方便，可以同时对多个子表的数据进行过滤。如果追求性能且已经明确要查询的子表，直接查询子表可以获得更高的性能。
...
使用到的网络端口请看文档：[运维指南](../12-operations-and-tooling/02-operations/index.md)
...
### 6.13 为什么开源版 TDengine 的主进程会建立一个与公网的连接？
...
你可以随时关闭该参数，只需要在 taos.cfg 中修改 telemetryReporting 为 0，然后重启数据库服务即可。
...
### 7.1 TDengine 3.0 之前的版本升级到 3.0 及以上的版本应该注意什么？
...
### 8.3 在 TDengine CLI 中查询，字段内容不能完全显示出来怎么办？
