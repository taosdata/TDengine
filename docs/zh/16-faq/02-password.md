...
3. 密码字符的取值范围：
    ...
    3. 数字：`0-9`
    4. 特殊字符：`! @ # $ % ^ & * ( ) - _ + = [ ] { } : ; > < ? | ~ , .`
4. 强密码启用（`enableStrongPassword` / SQL `EnableStrongPassword`，默认开启）时，至少包含大写字母、小写字母、数字、特殊字符中的三类；关闭后不对字符种类做强制约束。详见 [用户管理](../05-tdengine-sql/07-user-and-privilege/01-user.md)。

## 各组件特殊字符使用指南

以下以用户名 `user1`、密码 `Ab1!@#$%^&*()-_+=[]{}` 为例。
...
在 [`taos shell`](../12-operations-and-tooling/04-tools/01-taos-cli.md) 中使用时注意：

- 使用参数 `-p` 且不带密码时，会提示输入密码，可输入任意可接受字符。
- 使用参数 `-p` 且直接带密码时，若密码含特殊字符，须使用单引号。
...
在 [taosdump](../12-operations-and-tooling/04-tools/03-taosdump.md) 中使用时注意：

- 使用参数 `-p` 且不带密码时，会提示输入密码，可输入任意可接受字符。
- 使用参数 `-p` 且直接带密码时，若密码含特殊字符，须使用单引号或进行转义。
...
在 [taosBenchmark](../12-operations-and-tooling/04-tools/04-taosbenchmark.md) 中使用时注意：
...
使用 `taosBenchmark -f <JSON>` 时，JSON 文件中的密码无额外限制。

</TabItem>
<TabItem label="taosX" value="taosx">

[taosX](../12-operations-and-tooling/03-components/06-taosx.md) 使用 DSN 表示 TDengine 连接，格式为 `(taos|tmq)[+ws]://<user>:<pass>@<ip>:<port>`，其中 `<pass>` 可包含特殊字符，例如：`taos+ws://user1:Ab1!@#$%^&*()-_+=[]{}@192.168.10.10:6041`。
...
说明：若密码可被 URL decode，则会使用 URL decoded 结果作为密码。例如 `taos+ws://user1:Ab1%21%40%23%24%25%5E%26%2A%28%29-_%2B%3D%5B%5D%7B%7D@localhost:6041` 与 `taos+ws://user1:Ab1!@#$%^&*()-_+=[]{}@localhost:6041` 等价。

在 [taosExplorer](../12-operations-and-tooling/03-components/04-explorer.md) 中无需特殊处理，直接使用即可。

</TabItem>

<TabItem label="Java" value="java">

在 JDBC 中使用含特殊字符的密码时，须对密码做 URL 编码，示例如下：
...
在 Python 中使用含特殊字符的密码时无需特殊处理，示例如下：
...
自 `v3.6.0` 起，Go 连接器支持密码中包含特殊字符，使用时须进行 URL 编码。
...
Rust 连接器使用 DSN 表示 TDengine 连接，格式为 `(taos|tmq)[+ws]://<user>:<pass>@<ip>:<port>`，其中 `<pass>` 可包含特殊字符，例如：`taos+ws://user1:Ab1!@#$%^&*()-_+=[]{}@192.168.10.10:6041`。
...
自 `v3.1.5` 起，Node.js 连接器支持密码中包含特殊字符，无需特殊处理。
...
在 C# 中使用密码时注意：连接字符串不支持分号（分号为分隔符）。此时可先用不带密码的字符串构建 `ConnectionStringBuilder`，再设置用户名和密码。
...
- 密码采用 Basic Auth，格式为 `Authorization: Basic base64(<user>:<pass>)`。
- 不支持密码中包含冒号 `:`。
