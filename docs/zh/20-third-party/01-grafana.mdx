...
- taosKeeper TDengine 可以通过 taosKeeper 将监控数据写入指定数据库中。此项仅在连接 TDengine 3.0 及以上版本需要。taosKeeper 部署细节请参考：[TaosKeeper](../../reference/taosKeeper)

记录以下信息：

- TDengine 集群 REST API 地址，如：`http://tdengine.local:6041`。
- TDengine 集群认证信息，可使用用户名及密码。
- taosKeeper 配置监控日志写入的数据库名称。此项仅在连接 TDengine 3.0 及以上版本需要。
...
- INPUT SQL：输入要查询的语句（该 SQL 语句的结果集应为两列多行），例如：`select avg(mem_system) from log.dn where ts >= $from and ts < $to interval($interval)` ，其中，from、to 和 interval 为 TDengine 插件的内置变量，表示从 Grafana 插件面板获取的查询范围和时间间隔。除了内置变量外，`也支持可以使用自定义模板变量`。当 TDengine 服务端版本为 3.0 版本时 SQL 修改为 `select avg(mem_system) from $logdb.dnodes_info where ts >= $from and ts < $to interval($interval)` ，其中，logdb 为 taosKeeper 配置的数据库名称。
- ALIAS BY：可设置当前查询别名。
