...
之后，用户可以直接通过 [http://localhost:3000](http://localhost:3000) 的网址，登录 Grafana 服务器（用户名/密码：admin/admin），通过左侧 `Configuration -> Data Sources` 可以添加数据源，
...
![TDengine TSDB Database Grafana plugin create dashboard](./create_dashboard1.webp)
...
![TDengine TSDB Database Grafana plugin create dashboard](./create_dashboard2.webp)
...
![TDengine TSDB Database Grafana plugin create dashboard](./create_dashboard3.webp)
...
![TDengine TSDB Database Grafana plugine import dashboard](./import_dashboard.webp)
...
![TDengine TSDB Database Grafana plugin alert email](./alert-email.webp)
...
![TDengine TSDB Database Grafana plugin feishu robot](./alert-feishu1.webp)

在 Grafana 页面找到“Home” -> “Alerting” -> “Contact points”创建新联络点  
“Name”：Feishu Contact Point  
“Integration”：选择联络类型，这里选择 Webhook，并填写 URL (在飞书机器人助手的 Grafana 触发器 Webhook 地址)，完成后保存联络点  

![TDengine TSDB Database Grafana plugin feishu contact point](./alert-feishu2.webp)

### 配置告警通知策略

配置好联络点后，可以看到已有一个 Default Policy

![TDengine TSDB Database Grafana plugin Notification default policy](./alert-notification1.webp)

点击右侧“...” -> ”Edit”，然后编辑默认通知策略，弹出配置窗口：

![TDengine TSDB Database Grafana plugin Notification](./alert-notification2.webp)
...
![TDengine TSDB Database Grafana plugin Alert Rules](./alert-rules1.webp)
...
![TDengine TSDB Database Grafana plugin Alert Evaluation Behavior](./alert-evaluation.webp)
...
![TDengine TSDB Database Grafana plugin Alert Labels and Notifications](./alert-labels.webp)
...
![TDengine TSDB Database Grafana plugin Alert Labels and Notifications](./alert-annotations.webp)
