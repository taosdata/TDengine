...
我们使用 Node-RED + TDengine 来实现需求，使用 Inject + function 节点模拟设备每秒产生一条数据，tdengine 节点负责写入数据，实时查询统计使用 tdengine 节点查询功能，设备过载报警使用 tdengine-consumer 订阅异常数据。
...
- <b>增加写入节点</b> 
  1. 节点选择区域内选中 tdengine 节点，拖动至画布中。
