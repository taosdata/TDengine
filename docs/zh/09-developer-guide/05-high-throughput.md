...
其他配置参数请参考 [高效写入配置](08-connectors-reference/02-java.mdx#properties)。
...
1. JDBC URL 通过环境变量配置，例如：`export TDENGINE_JDBC_URL="jdbc:TAOS-WS://localhost:6041?user=root&password=taosdata&varcharAsString=true"`
...
其他参数请参考 [数据库管理](../04-tdengine-sql/02-ddl/01-database.md)。
...
其他参数请参考 [服务端配置](../11-operations-and-tooling/03-components/01-taosd.md)。
...
下面是连接器实现高效写入特性的原理图：
![TDengine 高效写入示例场景的线程模型](assets/highvolume.png)
