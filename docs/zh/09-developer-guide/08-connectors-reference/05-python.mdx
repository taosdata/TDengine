...
连接方式的详细介绍请参考：[连接方式](../01-connect/index.md#连接方式)
...
- 对于 `taos-ws-py` 0.6.6 及以下版本，如需使用 SQLAlchemy，需要依赖 `taospy` 2.8.5 至 2.8.9 版本。
- 从 `taos-ws-py` 0.6.7 版本开始，SQLAlchemy dialect 已迁移至 `taos-ws-py`，使用 SQLAlchemy 时不再依赖 `taospy`。
...
| ----------------------- | ------------------------------------------------------------------------------------ | ----------------- |
|          0.6.9           | 新增对 riscv64 架构的支持                                                              | - |
|          0.6.8           | 支持 DECIMAL 数据类型                                                                 | - |
|          0.6.7           | 将 SQLAlchemy dialect 迁移至 `taos-ws-py`，使用 SQLAlchemy 时不再依赖 `taospy`          | - |
|          0.6.6           | 数据订阅支持令牌身份验证                                                                | - |
...
- `fn connect(dsn: Option<&str>, args: Option<&PyDict>) -> PyResult<Connection>`
  ...
  - **参数说明**：
    ...
    - `args`：可选参数，类型为 `Option<&PyDict>`。以 Python 字典形式提供，字典中的值均为字符串类型。可用于配置以下选项：
      ...
      - `retry_backoff_max_ms`：连接失败时的最大等待时间（毫秒），默认为 2000。
      - `read_timeout`：WebSocket 连接的响应超时时间（秒），不含数据订阅，默认为 300（5 分钟）。
      - `totp_code`：用于基于时间的一次性密码（TOTP）身份验证
      - `bearer_token`：用于令牌身份验证
  - **返回值**：连接对象。
...
- `fn __iter__(slf: PyRef<Self>) -> PyRef<Self>`
  - **接口说明**：使 `TaosResult` 对象支持迭代器协议，返回迭代器本身。可以通过 `for` 循环遍历查询结果集中的每一行数据。
  - **返回值**：`PyRef<Self>` 迭代器对象本身。
- `fn __next__(mut slf: PyRefMut<Self>) -> PyResult<Option<PyObject>>`
  - **接口说明**：获取结果集中的下一行数据，当所有数据遍历完成后结束迭代。每行数据以元组（tuple）形式返回，元组中的元素顺序与字段顺序一致。
  - **返回值**：`Option<PyObject>` 包含一行字段值的元组。
