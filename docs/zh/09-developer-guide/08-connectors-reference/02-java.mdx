...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]：仅在使用 JDBC WebSocket 连接时生效，令牌信息。其认证优先级高于用户名密码。详见 [连接器安全最佳实践](../../10-security-guide/04-connector-security.md#3-token-认证)。
...
- TSDBDriver.PROPERTY_KEY_USE_SSL [`useSSL`]：连接中是否使用 SSL。仅在 WebSocket 连接时生效。详见 [客户端 SSL/TLS 配置](../../10-security-guide/04-connector-security.md#22-客户端-ssltls-配置)。
