...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| PI 服务名 | 是 | PI Data Archive Server 的主机名。**推荐使用主机名而非 IP 地址**（见下文「为什么推荐使用主机名而非 IP」） |
...
- 认证通过与否，取决于 taosX-Agent 主机上的 Windows 身份是否被 PI 系统信任，以及 PI 侧是否配置了对应的 Mapping（见下文「PI 侧权限配置」）。

### 连接器以哪个 Windows 身份访问 PI
...
在大多数企业域环境中，taosX-Agent 的服务账户已通过 Windows 集成认证与 PI 系统建立信任关系，只需在 PI 侧为其 Windows 身份配置好 Mapping（见下文「PI 侧权限配置」），连接即可成功。
:::
...
| 禁止该 taosX-Agent 访问 PI | 删除对应 Mapping（会导致连接时认证失败） |
...
2. **先用 PI 工具手工验证**：在 taosX-Agent 主机上打开 PI System Management Tools (SMT) 或 PI System Explorer (PSE)，直接连接 PI。如果手工连接也失败，说明是环境问题（网络/权限/SDK），而非 taosX 配置问题。
