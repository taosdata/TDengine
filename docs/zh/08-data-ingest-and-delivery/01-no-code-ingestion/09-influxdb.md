...
本节讲述如何通过 taosExplorer 界面创建数据写入任务，从 InfluxDB 将数据接入当前 TDengine 集群。
...
在数据写入页面中点击 **+新增数据源** 按钮进入新增数据源页面，如下图所示：

![Common-zh00-EnterDataSourcePage.png](../../assets/kinghist-01.png "进入新增数据源页面")
...
![InfluxDB-02zh-SelectTheTypeAsInfluxDB.png](../../assets/InfluxDB-02zh-SelectTheTypeAsInfluxDB.png "选择数据源类型为InfluxDB")

### 配置连接信息

在 **连接配置** 区域填写 *`源 InfluxDB 数据库的连接信息`*，如下图所示：

![InfluxDB-03zh-FillInTheConnectionInformation.png](../../assets/InfluxDB-03zh-FillInTheConnectionInformation.png "填写源InfluxDB数据库的连接信息")
...
在 **认证** 区域有两个选项卡，*`1.x 版本`* 与 *`2.x 版本`* ，这是由于不同版本的 InfluxDB 数据库的鉴权参数不同且 API 存在较大差异，请根据实际情况进行选择：  
  ...
  **密码** 输入源 InfluxDB 数据库中上方用户的登录密码。  
  ![InfluxDB-04zh-SelectVersion1.x.png](../../assets/InfluxDB-04zh-SelectVersion1.x.png "选择1.x版本")  
  *`2.x 版本`*  
  **版本** 在下拉框中选择源 InfluxDB 数据库的版本。  
  ...
  **添加数据库保留策略** 这是一个 *`是/否`* 的开关项，InfluxQL 需要数据库与保留策略（DBRP）的组合才能查询数据，InfluxDB 的 Cloud 版本及某些 2.x 版本需要人工添加这个映射关系，打开这个开关，连接器可以在执行任务时自动添加。  
  ![InfluxDB-05zh-SelectVersion2.x.png](../../assets/InfluxDB-05zh-SelectVersion2.x.png "选择2.x版本")

在 **认证** 区域的下方有一个 **连通性检查** 按钮，用户可以点击此按钮检查上方填写的信息是否可以正常获取源 InfluxDB 数据库的数据，检查结果如下图所示：  
  **失败**  
  ![InfluxDB-06zh-ConnectivityCheckFailed.png](../../assets/InfluxDB-06zh-ConnectivityCheckFailed.png "连通性检查失败")  
  **成功**  
  ![InfluxDB-07zh-ConnectivityCheckSuccessful.png](../../assets/InfluxDB-07zh-ConnectivityCheckSuccessful.png "连通性检查成功")

### 配置任务信息

**桶 Bucket** 是 InfluxDB 数据库中存储数据的一个命名空间，每个任务需要指定一个 Bucket，用户需要先点击右侧的 **获取 Schema** 按钮获取当前源 InfluxDB 数据库的数据结构信息，然后在下拉框中进行选择，如下图所示：
![InfluxDB-08zh-GetSchemaAndSelectOneBucket.png](../../assets/InfluxDB-08zh-GetSchemaAndSelectOneBucket.png "获取数据结构信息并选择一个Bucket")
