...
- 采集间隔是任务级参数（`observe` 模式）：同一 OPC UA Data In 任务内的普通点位共用该间隔，不支持在同一任务内按单个点位或点位分组设置不同轮询频率。`subscribe` 模式由服务器按变化上报，不按固定间隔扫描全部点位。
- 若部分点位需要不同的轮询周期，请拆成多个任务（可共用同一 OPC 服务器与 Agent），分别设置采集间隔。
- 修改采集参数、点位映射时，在 taosExplorer 中编辑任务即可；无需为改点位而重装 taosX-Agent。Agent 安装与复用说明见 [安装 taosX-Agent](../01-install-agent.md)。
...
在任务运行中，点击 **编辑**，点击 **增加数据点位** 按钮，可以手动在 CSV 配置文件中，追加一个 OPC UA 数据点位的规则。增加数据点位不需要任务重启，不会产生数据丢失。适用于只需新增少量点位、不必重导全量点表的场景。
...
已入库的子表标签值（含 BrowseName / Description / Path 等自动生成的 TAG，以及 Property→TAG）的日常维护，建议优先在 TDengine IDMP 中完成；未使用 IDMP 时，可用 SQL [`ALTER TABLE … SET TAG`](../../../05-tdengine-sql/02-ddl/02-table.md#修改标签值) 批量修改，无需重导 CSV。CSV 列与自动 TAG 说明见 [OPC UA CSV 映射参考](./02-csv-reference.md)。

## 相关文档

- [安装 taosX-Agent](../01-install-agent.md)：远程代理安装；任务变更一般无需重装 Agent
- [OPC-DA](../06-opcda.md)：DA 侧元数据、采集间隔与任务维护说明（与 UA 对照）
