...
TDengine 可以通过 MQTT 连接器从外部 MQTT Broker 订阅数据并将其写入 TDengine，以实现实时数据流入库。订阅主题的 QoS 可为 0、1、2（由 Broker 侧能力决定）。这与 [MQTT 订阅](../../06-data-subscription/03-mqtt.md)（客户端连接 TDengine Bnode）不是同一功能，后者仅支持 QoS 0/1。
...
![mqtt-collect.png](../../assets/mqtt-06.png)
...
![mqtt-09.png](../../assets/mqtt-10.png)

#### 数据过滤

在 **过滤** 中，填写过滤条件，例如：填写`id != 1`，则只有 id 不为 1 的数据才会被写入 TDengine。

![mqtt-10.png](../../assets/mqtt-11.png)

点击 **删除**，可以删除当前过滤规则。

点击 **放大镜图标** 可查看预览过滤结果。

![mqtt-11.png](../../assets/mqtt-12.png)
...
![mqtt-17.png](../../assets/mqtt-13.png)

在 **映射** 中，填写目标超级表中的子表名称，例如：`t_{id}`。根据需求填写映射规则，其中 mapping 支持设置缺省值。

![mqtt-12.png](../../assets/mqtt-14.png)

点击 **预览**，可以查看映射的结果。

![mqtt-13.png](../../assets/mqtt-15.png)

如果超级表列为模板变量，在子表映射时会进行 pivot 操作，其中模板变量的值展开为列名，列的值为对应的映射列

例如：

![mqtt-18.png](../../assets/mqtt-16.png)

预览结果为：

![mqtt-19.png](../../assets/mqtt-17.png)
