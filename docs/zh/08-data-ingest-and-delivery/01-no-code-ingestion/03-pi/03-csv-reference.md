...
在 taosExplorer 中创建 PI 任务时，点击 **下载默认配置** 按钮可以自动生成默认的模型配置文件。你可以下载后编辑，再上传覆盖默认配置。
...
更多关于映射规则和数据转换表达式的语法，请参阅 [零代码第三方数据接入](../index.md) “数据提取、过滤和转换”部分。
...
在 taosExplorer 中填写过滤条件后再点击 **下载默认配置**，可以只生成匹配过滤条件的点位/模板的配置。过滤条件的语法请参阅 [Dataset Filter 配置](./07-dataset-filter.md)。
