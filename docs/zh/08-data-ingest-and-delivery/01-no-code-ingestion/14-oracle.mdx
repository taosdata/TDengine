...
本节讲述如何通过 Explorer 界面创建数据迁移任务，从 Oracle 迁移数据到当前 TDengine TSDB 集群。

## 功能概述

Oracle 数据库系统是世界上流行的关系数据库管理系统，系统可移植性好、使用方便、功能强，适用于各类大、中、小微机环境。它是一种高效率的、可靠性好的、适应高吞吐量的数据库方案。

TDengine TSDB 可以高效地从 Oracle 读取数据并将其写入 TDengine TSDB，以实现历史数据迁移或实时数据同步。
...
![oracle-05.png](assets/oracle-05.png)
...
点击 **提交** 按钮，完成创建 Oracle 到 TDengine TSDB 的数据同步任务，回到**数据源列表**页面可查看任务执行情况。
