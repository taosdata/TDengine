...
本节讲述如何通过 taosExplorer 界面创建数据写入任务，从 MongoDB 将数据接入当前 TDengine 集群。
...
![mongodb-01.png](../../assets/mongodb-01.png "选择数据源类型为 MongoDB")

### 配置连接信息

在 **连接配置** 区域填写 *`源 MongoDB 数据库的连接信息`*，如下图所示：

![mongodb-02.png](../../assets/mongodb-02.png "填写源 MongoDB 数据库的连接信息")
...
![mongodb-03.png](../../assets/mongodb-03.png "配置认证信息")
...
![ mongodb-04.png](../../assets/mongodb-04.png "配置连接选项")
...
2. 在 **查询模板** 中添加子表字段占位符，例如 `{"ddate":{"$gte":${start_datetime},"$lt":${end_datetime}}, ${col_name1}, ${col_name2}}`。运行时 `${col_name1}`、`${col_name2}` 会展开为 JSON 字段过滤条件（如 `"col_name1":"deviceA"`），而不是裸字段名
3. 在 **transform** 中配置 `col_name1` 与 `col_name2` 两个 tag 映射

**查询模板** 用于查询数据的查询语句，JSON 格式，语句中必须包含时间范围条件，且开始时间和结束时间必须成对出现。模板中定义的时间范围由源数据库中的某个代表时间的列和下面定义的占位符组成。
...
**查询排序** 执行查询时的排序条件，JSON 格式，它必须符合 MongoDB 排序条件的格式规范，使用示例如下：
...
![ mongodb-05.png](../../assets/mongodb-05.png "配置数据查询")
...
在 **从列中提取或拆分** 中填写从消息体中提取或拆分的字段，例如：将 vValue 字段拆分成 `vValue_0` 和 `vValue_1` 这 2 个字段，选择 split 提取器，separator 填写分割符 `,`, number 填写 2，配置完成后点击右侧的 **预览** 按钮可以查看转换的结果。
...
![mongodb-06.png](../../assets/mongodb-01.png)
