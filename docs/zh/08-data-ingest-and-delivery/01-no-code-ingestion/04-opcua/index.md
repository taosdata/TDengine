...
OPC 是工业自动化领域和其他行业中安全可靠地交换数据的互操作标准之一。OPC UA 是经典 OPC 规范的下一代标准，是一个平台无关的面向服务的架构规范，集成了现有 OPC Classic 规范的所有功能，提供了一条迁移到更安全和可扩展解决方案的路径。
...
**点位集** 通过 tab 切换，提供三种互斥的配置模式：

1. **选择数据点位**（默认）：浏览并勾选要采集的 OPC UA 数据点位。
2. **上传 CSV 配置文件**：通过 CSV 文件批量配置点位。
3. **告警与事件**：采集 OPC UA Server 推送的告警与事件（A&E），详见 [OPC UA 告警与事件采集](./06-alarm-event.md)。

前两种为普通点位采集（Value），第三种为告警与事件采集（A&E），下文分别说明。
...
选择「**告警与事件**」tab 后，进入 OPC UA 告警与事件（A&E）采集模式，把 Server 推送的告警与事件采集写入 TDengine。该模式的配置项包括：

- **事件类型**（必选）：树形多选要订阅的 OPC UA 事件类型，选中某类型即自动包含其所有子类。
- **事件源过滤**（可选）：树形多选要采集的事件源（SourceNode），不选则采集所有源。
- **最小严重度**（可选）：仅采集严重度 ≥ 该值（1–1000）的事件，留空则不过滤。
- **配置规则预览**：正式提交前静态预览将建的超级表结构与生效过滤条件，预览所见即实际所建。

A&E 采集的数据入库规则（超级表 / 子表命名、字段类型映射、状态机拆列、查询示例）与常见问题，详见 [OPC UA 告警与事件采集](./06-alarm-event.md)。

:::note
「告警与事件」为企业版功能，且与采集模式联动：选择该 tab 后，下方「采集配置」中的采集模式会自动锁定为 `event`。
:::
