...
TDengine 可以通过 MQTT 连接器从外部 MQTT Broker 订阅数据并将其写入 TDengine，以实现实时数据流入库。订阅主题的 QoS 可为 0、1、2（由 Broker 侧能力决定）。这与 [MQTT 订阅](../../07-data-subscription/03-mqtt.md)（客户端连接 TDengine Bnode）不是同一功能，后者仅支持 QoS 0/1。
...
![mqtt-collect.png](assets/mqtt-collect.png)

### 7. 配置 MQTT Payload 解析
...
![mqtt-17.png](assets/mqtt-17.png)
...
![mqtt-18.png](assets/mqtt-18.png)

预览结果为：

![mqtt-19.png](assets/mqtt-19.png)
