...
title: 零代码数据写入
description: 通过 taosExplorer 与 taosX 零代码接入第三方数据源
...
| Sparkplug B | Sparkplug B 3.0 | 基于 MQTT 的工业互操作规范，定义设备/边/应用之间的主题命名与载荷格式，常用于 SCADA 与 IIoT 数据采集。 |
| KingHistorian | - | 亚控科技实时历史数据库，用于工业过程实时与历史数据的采集、存储与分析。 |
| Pulsar | - | Apache Pulsar 分布式消息与流平台，支持多租户与持久化消息。 |
| Pulsar-Tuya | - | 基于 Pulsar 的涂鸦（Tuya）设备数据接入方式。 |
| pSpace | - | 工业实时数据库/中间件，用于采集与转发过程数据。 |
| TDengine 查询 | 2.4+, 3.0+ | TDengine 旧版本数据查询方式写入新集群。 |
| TDengine 数据订阅 | 3.0+ | 使用 TMQ 订阅指定数据库或超级表的数据。 |
...
仅非结构化的数据源需要这个步骤，目前 MQTT 和 Kafka 数据源会使用这个步骤提供的规则来解析非结构化数据，以初步获取结构化数据，即可以以字段描述的行列数据。在 taosExplorer 中你需要提供示例数据和解析规则，来预览解析出以表格呈现的结构化数据。
...
1. 直接在输入框中输入示例数据；
2. 点击右侧按钮“从服务器检索”则从配置的服务器获取示例数据，并追加到示例数据输入框中；
3. 上传文件，将文件内容追加到示例数据输入框中。
...
从 `v3.3.5.0` 开始，在任务管理列表中，增加了一项“健康状态”，用于指示当前任务运行过程中的健康状态。
