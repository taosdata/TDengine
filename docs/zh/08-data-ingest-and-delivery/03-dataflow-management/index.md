...
sidebar_label: 边云协同
title: 边云协同
description: TDengine 边云协同数据同步与汇聚
toc_max_heading_level: 4
---

## 为什么需要边云协同

在工业互联网场景中，边缘设备只用于处理局部数据，决策者无法只根据边缘设备采集的信息，形成对整个系统的全局认知。在实际应用中，边缘设备需要将数据上报给云计算平台（公有云或私有云），在云计算平台实现数据的汇聚与信息的融合，让决策者对整个数据有个全局的洞察。这种边云协同的架构，逐渐成为支撑工业互联网发展的重要支柱。
...
TDengine 企业版致力于提供强大的边云协同能力，具备以下显著特性：
...
TDengine 支持边缘节点主动向云端推送数据，也支持云端从边缘节点拉取数据；两种方向均可使用数据订阅或查询。

- 边缘节点较少且云端可以直接访问边缘节点时，适合由云端拉取。
- 边缘节点较多或不允许从外部访问边缘节点时，适合由边缘节点推送。
- 要求近实时同步时，使用数据订阅。
- 要求定期或按需同步时，使用查询。
...
1. 在左侧主菜单中选择 **管理**。
1. 打开 **数据复制** 标签页，点击 **新增数据复制**。
1. 选择要同步到云端的数据库。
1. 在 **目标 DSN** 中输入云端节点的 DSN。
1. 点击 **确认**。
...
1. 执行以下命令，将数据推送到云端：
   ...
   例如，以下命令使用默认用户名和密码，把 `192.0.2.1:6030` 上边缘节点的 `sync_test` 数据库同步到 `198.51.100.1:6030` 上云端节点的 `edge_data` 数据库：

   ```shell
   taosx run -f 'taos://root:taosdata@192.0.2.1:6030/sync_test' -t 'taos://root:taosdata@198.51.100.1:6030/edge_data' -v
   ```

更多信息见 [taosX 数据迁移](../../12-operations-and-tooling/03-components/06-taosx.md#从旧版本迁移数据)。

### 云端拉取与数据订阅

1. 在浏览器中打开边缘节点的 taosExplorer。
1. 在左侧主菜单中选择 **主题**。
1. 找到要同步到云端的数据库，在 **获取 DSN** 列点击 **复制**，保存该 DSN。
1. 打开云端节点的 taosExplorer。
1. 在左侧主菜单中选择 **数据写入**。
1. 打开 **数据写入任务** 标签页，点击 **新增数据源**。
1. 输入任务名称。
1. 在 **类型** 下拉列表中选择 **TDengine 数据订阅**。
1. 在 **目标** 下拉列表中选择接收边缘数据的数据库；如果没有合适的数据库，点击 **创建数据库**。
1. 在 **连接配置** 中，把之前复制的 DSN 粘贴到 **Topic DSN**。
1. 点击 **检查连通性**，确认云端可以访问边缘节点。
1. 在 **订阅选项** 的 **Client ID** 中输入唯一标识，并按需配置其他选项。
1. 点击 **提交**。

云端会订阅边缘节点的指定数据库并将其数据写入云端目标数据库。可以重复上述步骤，从更多边缘数据库或节点同步数据。详情见 [TDengine 数据订阅数据源](../01-no-code-ingestion/03-tmq.md)。
...
1. 打开 **数据写入任务** 标签页，点击 **新增数据源**。
1. 输入任务名称。
1. 在 **类型** 下拉列表中选择 **TDengine 查询**。
1. 在 **目标** 下拉列表中选择接收边缘数据的数据库；如果没有合适的数据库，点击 **创建数据库**。
1. 在 **连接配置** 中选择 `WS` 协议，填写边缘节点 taosAdapter 的主机名或 IP 地址、端口，以及要同步的数据库。
1. 在 **认证** 中填写有权访问该数据库的边缘节点用户名和密码。
1. 点击 **检查连通性**，确认云端可以访问边缘节点。
1. 按需配置其他选项，点击 **提交**。

云端会查询边缘节点的指定数据库并将数据写入云端目标数据库。详情见 [TDengine 查询数据源](../01-no-code-ingestion/02-migrate.md)。

下图以一个具体的生产车间的实例介绍了在 TDengine 企业版中实现边云协同架构。在生产车间，设备产生的实时数据存储至部署在边缘侧的 TDengine。部署在分厂的 TDengine 会去订阅生产车间的 TDengine 中的数据。为了更好地满足业务需求，需要由数据分析师设置一些订阅规则，例如将数据进行降采样，或者只同步超过指定阈值的数据。同理，部署在集团侧的 TDengine 再订阅来自各座分厂的数据，实现集团维度的数据汇聚后，即可进行下一步的分析和处理。

![边云协同示意图](../../assets/dataflow-management-01.png)
...
制造业企业通常面临的一个痛点问题就是数据同步。很多企业目前采用离线方式来同步数据，但 TDengine 企业版实现了数据的实时同步，而且规则可配置。这种方式能够避免定期传输大数据量导致的资源浪费和带宽阻塞风险。
