...
title: 引擎发布历史及下载链接
description: TDengine 引擎发布历史、版本说明及下载链接
---
...
- `Major`：有重大新特性，升级时不支持滚动升级，且升级后不可回退，如从 `v3.2.3.0` 升级到 `v3.3.0.0` 后不能回退
- `Feature`：有新特性，升级时不支持滚动升级，但从相同的 Major Release 不同的 Feature Release 升级后可以回退，如从 `v3.3.0.0` 升级到 `v3.3.1.0` 后可以回退到 `v3.3.0.0`。客户端驱动（`libtaos.so`）与服务端需要同步升级。
...
TDengine `v3.3.7.0` 及以上版本请访问 [下载中心](https://www.taosdata.com/download-center)
