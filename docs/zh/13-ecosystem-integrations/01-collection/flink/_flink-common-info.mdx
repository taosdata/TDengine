...
| ------------------| ------------------------------------ | ---------------- |
|        2.1.4      | 升级 jdbc 驱动到 3.7.3 版本，详细参见 [JDBC 历史版本](../../../10-developer-guide/08-connectors-reference/02-java.mdx#版本历史)| - |
|        2.1.3      | 增加数据转换异常信息输出 | - |
|        2.1.2      | 写入字段增加反引号过滤 | - |
...
|        2.0.0      | 1. Sink 支持自定义数据结构序列化，写入 TDengine <br/> 2. 支持 Table SQL 方式写入 TDengine 数据库 | `v3.3.5.1` 及以上版本 |
|        1.0.0      | 支持 Sink 功能，将来自其他数据源的数据写入到 TDengine| `v3.3.2.0` 及以上版本|
