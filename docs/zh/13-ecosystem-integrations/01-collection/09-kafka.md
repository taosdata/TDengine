...
description: 使用 Kafka Connect 插件在 Kafka 与 TDengine 之间双向同步数据
---
...
企业版也可在 taosExplorer 中配置零代码 Kafka 写入与数据发布，详见 [零代码数据写入 · Kafka](../../08-data-ingest-and-delivery/01-no-code-ingestion/07-kafka.mdx) 与 [数据发布 · Kafka](../../08-data-ingest-and-delivery/02-no-code-delivery/03-Kafka.md)。基于主题的消费语义还可对照 [数据订阅](../../07-data-subscription/index.md)。
...
3. 已安装 Git、curl、vi
4. 已安装并启动 TDengine。如果还没有，可参考 [用安装包快速体验](../../04-quick-start/01-download-and-install/02-package.md)
...
TDengine Sink Connector 内部使用 TDengine [无模式写入接口](../../10-developer-guide/04-schemaless.md) 写数据到 TDengine，目前支持三种格式的数据：InfluxDB 行协议格式，OpenTSDB Telnet 协议格式，和 OpenTSDB JSON 协议格式。
...
TDengine Source Connector 的作用是将 TDengine 某个数据库某一时刻之后的数据推送到 Kafka。其实现原理是：先分批拉取历史数据，再用定时查询的策略同步增量数据；同时会监控表的变化，可以自动同步新增的表。如果重启 Kafka Connect，一般会从上次中断的位置继续同步。

TDengine Source Connector 会将 TDengine 数据表中的数据转换成 InfluxDB Line 协议格式或 OpenTSDB JSON 协议格式，然后写入 Kafka。
...
此时会显示所有历史数据。切换到 `taos` shell，插入两条新的数据：
...
1. `name`：connector 名称。
1. `connector.class`：connector 的完整类名，例如 `com.taosdata.kafka.connect.sink.TDengineSinkConnector`。
...
1. `connection.backoff.ms`：创建连接失败后的重试间隔时间，单位为 ms。默认 5000。
