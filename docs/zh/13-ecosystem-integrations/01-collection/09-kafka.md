...
![TDengine TSDB Database Kafka Connector -- Kafka Connect structure](kafka/assets/Kafka_Connect.webp)
...
![TDengine TSDB Database Kafka Connector -- streaming integration with kafka connect](kafka/assets/streaming-integration-with-kafka-connect.webp)
...
3. 已安装 Git、curl、vi
4. 已安装并启动 TDengine TSDB。如果还没有可参考 [安装和卸载](../../04-quick-start/index.md)
...
TDengine TSDB Sink Connector 内部使用 TDengine TSDB [无模式写入接口](../../10-developer-guide/04-schemaless.md) 写数据到 TDengine TSDB，目前支持三种格式的数据：InfluxDB 行协议格式，OpenTSDB Telnet 协议格式，和 OpenTSDB JSON 协议格式。
