...
在 TDengine 中为接收 MQTT 数据创建相应数据库和表结构。进入 `taos` shell 复制并执行以下 SQL 语句：
...
由于 EMQX 不同版本配置界面均有不同，这里仅以 v4.4.5 为例，其他版本请参考相应官网文档。

### 登录 EMQX Dashboard

使用浏览器打开网址 `http://IP:18083` 并登录 EMQX Dashboard。初次安装用户名为 `admin` 密码为：`public`。

![TDengine Database EMQX login dashboard](emqx/assets/login-dashboard.webp)

### 创建规则（Rule）

选择左侧“规则引擎（Rule Engine）”中的“规则（Rule）”并点击“创建（Create）”按钮：

![TDengine Database EMQX rule engine](emqx/assets/rule-engine.webp)
...
![TDengine Database EMQX create rule](emqx/assets/create-rule.webp)

### 新增“动作（action handler）”

![TDengine Database EMQX](emqx/assets/add-action-handler.webp)

### 新增“资源（Resource）”

![TDengine Database EMQX create resource](emqx/assets/create-resource.webp)
...
![TDengine Database EMQX edit resource](emqx/assets/edit-resource.webp)
...
相关文档请参考 [TDengine REST API 文档](../../10-developer-guide/08-connectors-reference/10-rest-api.mdx)。
...
![TDengine Database EMQX edit action](emqx/assets/edit-action.webp)
...
![TDengine Database EMQX client num](emqx/assets/client-num.webp)
...
![TDengine Database EMQX run-mock](emqx/assets/run-mock.webp)

## 验证 EMQX 接收到数据

在 EMQX Dashboard 规则引擎界面进行刷新，可以看到有多少条记录被正确接收到：

![TDengine Database EMQX rule matched](emqx/assets/check-rule-matched.webp)

## 验证数据写入到 TDengine

使用 `taos` shell 程序登录并查询相应数据库和表，验证数据是否被正确写入到 TDengine 中：

![TDengine Database EMQX result in taos](emqx/assets/check-result-in-taos.webp)
