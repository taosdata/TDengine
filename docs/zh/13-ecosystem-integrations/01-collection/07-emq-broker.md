...
![TDengine TSDB Database EMQX login dashboard](emqx/assets/login-dashboard.webp)

### 创建规则（Rule）

选择左侧“规则引擎（Rule Engine）”中的“规则（Rule）”并点击“创建（Create）”按钮：

![TDengine TSDB Database EMQX rule engine](emqx/assets/rule-engine.webp)
...
![TDengine TSDB Database EMQX create rule](emqx/assets/create-rule.webp)

### 新增“动作（action handler）”

![TDengine TSDB Database EMQX](emqx/assets/add-action-handler.webp)

### 新增“资源（Resource）”

![TDengine TSDB Database EMQX create resource](emqx/assets/create-resource.webp)
...
![TDengine TSDB Database EMQX edit resource](emqx/assets/edit-resource.webp)
...
相关文档请参考[TDengine TSDB REST API 文档](../../10-developer-guide/08-connectors-reference/10-rest-api.mdx)。
...
![TDengine TSDB Database EMQX edit action](emqx/assets/edit-action.webp)
...
![TDengine TSDB Database EMQX client num](emqx/assets/client-num.webp)
...
![TDengine TSDB Database EMQX run-mock](emqx/assets/run-mock.webp)

## 验证 EMQX 接收到数据

在 EMQX Dashboard 规则引擎界面进行刷新，可以看到有多少条记录被正确接收到：

![TDengine TSDB Database EMQX rule matched](emqx/assets/check-rule-matched.webp)

## 验证数据写入到 TDengine TSDB

使用 TDengine TSDB CLI 程序登录并查询相应数据库和表，验证数据是否被正确写入到 TDengine TSDB 中：

![TDengine TSDB Database EMQX result in taos](emqx/assets/check-result-in-taos.webp)
