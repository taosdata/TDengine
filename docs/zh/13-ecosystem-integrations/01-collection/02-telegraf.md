...
import Telegraf from "../../12-operations-and-tooling/03-components/resources/_telegraf.mdx"
...
使用 `taos` shell 验证从 Telegraf 向 TDengine 写入数据并能够正确读出：
...
举例如下：配置 smlChildTableName=tname 插入数据为 st,tname=cpu1,t1=4 c1=3 1626006833639000000 则创建的表名为 cpu1。如果多行数据 tname 相同，但是后面的 tag_set 不同，则使用第一行自动建表时指定的 tag_set，其他的行会忽略。[TDengine 无模式写入参考指南](../../10-developer-guide/04-schemaless.md)
