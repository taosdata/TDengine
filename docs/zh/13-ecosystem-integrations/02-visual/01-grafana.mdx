...
本文档介绍如何将 TDengine TSDB 数据源与开源数据可视化系统 [Grafana](https://www.grafana.com/) 集成，以实现数据的可视化和监测报警系统的搭建。通过 TDengine TSDB 插件，您可以轻松地将 TDengine TSDB 数据表的数据展示在 Grafana 仪表盘上，且无需进行复杂的开发工作。
...
- taosAdapter 已经安装并正常运行。具体细节请参考 [taosAdapter 的使用手册](../../12-operations-and-tooling/03-components/03-taosadapter.md)

记录以下信息：

- TDengine TSDB 集群 REST API 地址，如：`http://tdengine.local:6041`。
- TDengine TSDB 集群认证信息，可使用用户名及密码。
...
grafana-cli --pluginUrl \
      https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip \
...
```bash
./grafana-cli.exe --pluginUrl https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip plugins install tdengine-datasource
```
...
- Host：TDengine TSDB 集群中提供 REST 服务的 IP 地址与端口号，默认 `http://localhost:6041`
- User：TDengine TSDB 用户名。
- Password：TDengine TSDB 用户密码。

点击 `Save & Test` 进行测试，成功会提示：`TDengine Data source is working`

</TabItem>
<TabItem value="manual" label="手动安装">
下载 [tdengine-datasource.zip](https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip) 文件到本地并解压到 Grafana 插件目录。命令行下载示例如下：

```bash
wget https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip
```
...
参考 [Grafana 容器化安装说明](https://grafana.com/docs/grafana/next/setup-grafana/installation/docker/#install-plugins-in-the-docker-container)。使用如下命令启动一个容器，并自动安装 TDengine TSDB 插件：

```bash
docker run -d \
  -p 3000:3000 \
  --name=grafana \
  -e "GF_INSTALL_PLUGINS=https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip;tdengine-datasource" \
  grafana/grafana
```

使用 docker-compose，配置 Grafana Provisioning 自动化配置，体验 TDengine TSDB + Grafana 组合的零配置启动：
    ...
    services:
      ...
      grafana:
        ...
        environment:
          ...
          GF_INSTALL_PLUGINS: "https://downloads.taosdata.com/tdengine-tsdb-datasource/tdengine-datasource.zip;tdengine-datasource"
...
3. 使用 docker-compose 命令启动 TDengine TSDB + Grafana：`docker-compose up -d`。
...
1. 介绍基础知识，包括 Grafana 的内置变量和自定义变量，TDengine TSDB 对与时序查询的特色语法支持。
2. 介绍如何使用 TDengine TSDB 数据源在 Grafana 中创建 Dashboard，然后给出使用时序查询的特色语法和如何分组展示数据。
3. 由于配置的 Dashboard 在页面显示时会定时查询 TDengine TSDB 来刷新显示，如果 SQL 编写不当会导致严重的性能问题，我们给出了性能优化建议。
4. 最后我们以 TDengine TSDB 监控面板 TDinsight 为例介绍了如何导入我们提供的 DashBoard。
...
对于每个查询都建议设置查询范围的起始时间和结束时间，可以有效的减少 TDengine TSDB 服务端执行查询扫描的数据量。`interval` 是窗口切分的大小，在 Grafana 11 版本中，其大小为时间间隔和返回点数计算而得。
...
上述特性详细介绍可以参考 [特色查询](../../05-tdengine-sql/04-data-query/06-distinguished.md)。

### 创建 Dashboard

有了前面的基础知识，我们可以配置基于 TDengine TSDB 数据源的时间序列数据展示 Dashboard。  
在 Grafana 主界面创建 Dashboard，点击【Add Query】进入面板查询页面：

![TDengine TSDB Database Grafana plugin create dashboard](assets/create_dashboard1.webp)
...
![TDengine TSDB Database Grafana plugin create dashboard](assets/create_dashboard2.webp)
...
![TDengine TSDB Database Grafana plugin create dashboard](assets/create_dashboard3.webp)
...
在数据源配置页面，您可以为该数据源导入 TDinsight 面板，作为 TDengine TSDB 集群的监控可视化工具。如果 TDengine TSDB 服务端为 3.0 版本请选择 `TDinsight for 3.x` 导入。注意 TDinsight for 3.x 需要运行和配置 taoskeeper。

![TDengine TSDB Database Grafana plugine import dashboard](assets/import_dashboard.webp)

其中适配 TDengine TSDB 2.* 的 Dashboard 已发布在 Grafana：[Dashboard 15167 - TDinsight](https://grafana.com/grafana/dashboards/15167)) 。

使用 TDengine TSDB 作为数据源的其他面板，可以[在此搜索](https://grafana.com/grafana/dashboards/?dataSource=tdengine-datasource)。以下是一份不完全列表：

- [15146](https://grafana.com/grafana/dashboards/15146)：监控多个 TDengine TSDB 集群
- [15155](https://grafana.com/grafana/dashboards/15155)：TDengine TSDB 告警示例
...
TDengine TSDB Grafana 插件支持告警，如果要配置告警，需要以下几个步骤：
...
![TDengine TSDB Database Grafana plugin alert email](assets/alert-email.webp)
...
![TDengine TSDB Database Grafana plugin feishu robot](assets/alert-feishu1.webp)

在 Grafana 页面找到“Home” -> “Alerting” -> “Contact points”创建新联络点  
“Name”：Feishu Contact Point  
“Integration”：选择联络类型，这里选择 Webhook，并填写 URL (在飞书机器人助手的 Grafana 触发器 Webhook 地址)，完成后保存联络点  

![TDengine TSDB Database Grafana plugin feishu contact point](assets/alert-feishu2.webp)

### 配置告警通知策略

配置好联络点后，可以看到已有一个 Default Policy

![TDengine TSDB Database Grafana plugin Notification default policy](assets/alert-notification1.webp)

点击右侧“...” -> ”Edit”，然后编辑默认通知策略，弹出配置窗口：

![TDengine TSDB Database Grafana plugin Notification](assets/alert-notification2.webp)
...
![TDengine TSDB Database Grafana plugin Alert Rules](assets/alert-rules1.webp)
...
![TDengine TSDB Database Grafana plugin Alert Evaluation Behavior](assets/alert-evaluation.webp)
...
![TDengine TSDB Database Grafana plugin Alert Labels and Notifications](assets/alert-labels.webp)
...
![TDengine TSDB Database Grafana plugin Alert Labels and Notifications](assets/alert-annotations.webp)
