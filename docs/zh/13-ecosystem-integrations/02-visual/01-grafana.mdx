...
title: 与 Grafana 集成
description: 安装 TDengine 数据源插件并在 Grafana 中查询与展示时序数据
...
本文介绍如何将 TDengine 与开源可视化系统 [Grafana](https://www.grafana.com/) 集成，用于时序数据展示与监测告警。通过 TDengine 数据源插件，即可把库表数据展示在 Grafana 仪表盘上，而无需额外开发。

最小可用流程也可先参阅快速上手中的 [Grafana 集成](../../04-quick-start/09-grafana-integration.md)。

## 前置条件

要让 Grafana 能正常添加 TDengine 数据源，需要以下几方面的准备工作。

- Grafana 服务已经部署并正常运行。当前 TDengine 支持 Grafana 8.0 及以上版本，建议使用最新版本。  
    **注意**：要确保启动 Grafana 的账号有该安装目录的写权限，否则可能后面无法安装插件。
...
对于使用 Grafana 8.x 及以上版本或使用 [Grafana Provisioning](https://grafana.com/docs/grafana/latest/administration/provisioning/) 配置的用户，可以在 Grafana 服务器上使用安装脚本自动安装插件并添加数据源 Provisioning 配置文件。
...
1. 介绍基础知识，包括 Grafana 的内置变量和自定义变量，以及 TDengine 对时序查询的特色语法支持。
2. 介绍如何使用 TDengine 数据源在 Grafana 中创建 Dashboard，并说明时序查询特色语法与分组展示方式。
3. 由于配置的 Dashboard 在页面显示时会定时查询 TDengine 来刷新显示，如果 SQL 编写不当会导致严重的性能问题，下文给出性能优化建议。
4. 最后以 TDengine 监控面板 TDinsight 为例，说明如何导入提供的 Dashboard。
...
Grafana 提供了内置变量和自定义变量，它们都可以在编写 SQL 时引用，引用的方式是 `$variableName`，`variableName` 是变量的名字，其他引用方式请参考 [引用方式](https://grafana.com/docs/grafana/latest/dashboards/variables/variable-syntax/)。
...
对于每个查询都建议设置查询范围的起始时间和结束时间，可以有效地减少 TDengine 服务端执行查询扫描的数据量。`interval` 是窗口切分的大小，在 Grafana 11 版本中，其大小为时间间隔和返回点数计算而得。
...
![TDengine Database Grafana plugin create dashboard](assets/create_dashboard1.webp)
...
![TDengine Database Grafana plugin create dashboard](assets/create_dashboard2.webp)
...
![TDengine Database Grafana plugin create dashboard](assets/create_dashboard3.webp)
...
在数据源配置页面，你可以为该数据源导入 TDinsight 面板，作为 TDengine 集群的监控可视化工具。如果服务端为 3.x，请选择 `TDinsight for 3.x` 导入。注意 TDinsight for 3.x 需要运行并配置 taoskeeper，详见 [taosKeeper](../../12-operations-and-tooling/03-components/05-taoskeeper.md)。

![TDengine Database Grafana plugin import dashboard](assets/import_dashboard.webp)
...
![TDengine Database Grafana plugin alert email](assets/alert-email.webp)
...
![TDengine Database Grafana plugin feishu robot](assets/alert-feishu1.webp)

在 Grafana 页面找到“Home” -> “Alerting” -> “Contact points”创建新联络点  
“Name”：Feishu Contact Point  
“Integration”：选择联络类型，这里选择 Webhook，并填写 URL (在飞书机器人助手的 Grafana 触发器 Webhook 地址)，完成后保存联络点  

![TDengine Database Grafana plugin feishu contact point](assets/alert-feishu2.webp)

### 配置告警通知策略

配置好联络点后，可以看到已有一个 Default Policy

![TDengine Database Grafana plugin Notification default policy](assets/alert-notification1.webp)

点击右侧“...” -> ”Edit”，然后编辑默认通知策略，弹出配置窗口：

![TDengine Database Grafana plugin Notification](assets/alert-notification2.webp)
...
![TDengine Database Grafana plugin Alert Rules](assets/alert-rules1.webp)
...
![TDengine Database Grafana plugin Alert Evaluation Behavior](assets/alert-evaluation.webp)
...
![TDengine Database Grafana plugin Alert Labels and Notifications](assets/alert-labels.webp)
...
![TDengine Database Grafana plugin Alert Labels and Notifications](assets/alert-annotations.webp)
