...
Ignition 通过其 SQL Bridge 模块支持多种 SQL 数据库，你可以通过 JDBC 在 Ignition 部署中添加数据库连接。
...
- 安装 TDengine，请参考 [下载与安装](../../04-quick-start/01-download-and-install/02-package.md)。
- 安装 Ignition，请参考 [官方文档](https://www.docs.inductiveautomation.com/)。
- JDBC 驱动能力说明见 [Java 连接器](../../10-developer-guide/08-connectors-reference/02-java.mdx)。

## 配置数据源

1. 从 [Maven](https://central.sonatype.com/artifact/com.taosdata.jdbc/taos-jdbcdriver) 下载 TDengine JDBC 连接器。
...
现在，你可以在 Ignition 的 SQL Bridge 模块中使用 TDengine。Ignition 的数据流入 TDengine 后，还可以在 TDengine IDMP 中利用这些数据构建资产模型、进行实时分析和生成 AI 驱动的可视化。
