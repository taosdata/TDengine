...
[Apache Spark](https://spark.apache.org/) 是开源大数据处理引擎，基于内存计算，可用于批处理、流处理、机器学习、图计算等场景，支持 MapReduce 计算模型及丰富的算子与函数，适合大规模分布式数据处理。

通过 [TDengine Java 连接器](../../10-developer-guide/08-connectors-reference/02-java.mdx)，Spark 可以读取 TDengine 中的数据做扩展计算，也可将结果写回 TDengine，或结合数据订阅消费增量数据。

## 前置条件

准备以下环境：

- TDengine `v3.3.6.0` 及以上版本集群已部署并正常运行（企业及社区版均可）。
...
- JDBC 驱动建议使用较新版本（文档示例基于 3.6.2 及以上）。可从 [maven.org](https://central.sonatype.com/artifact/com.taosdata.jdbc/taos-jdbcdriver) 下载。
...
详细参数见：[URL 参数介绍](../../10-developer-guide/08-connectors-reference/02-java.mdx#url-规范)。
...
数据接入需注册 TDengine 方言，方言中主要处理反引号，数据类型映射与 JDBC 相同，无需额外处理，参见：[JDBC 数据类型映射](../../10-developer-guide/08-connectors-reference/02-java.mdx#数据类型映射)
