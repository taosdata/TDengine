...
创建文件 `tdengine.yaml`，其中 replicas 定义集群节点的数量为 3。节点时区为中国（Asia/Shanghai），每个节点分配 10G 标准（[storageClass](https://kubernetes.io/docs/concepts/storage/storage-classes/)：standard）存储。你也可以根据实际情况进行相应修改。
