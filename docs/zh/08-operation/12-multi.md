...
3. 向已有某一级存储添加新挂载点时，**必须将新的 `dataDir` 行追加到该级所有现有条目的末尾**，不可插入到中间位置（详见下方说明）。

**关于注意事项 3 的说明**：

TDengine 启动时，按 `taos.cfg` 中声明的顺序，为同一级别的每块磁盘依次分配一个数字 ID（`did_id`）：0 级的主挂载点固定分配 `id=0`，其余磁盘按出现顺序依次编号为 `1, 2, 3 …`。这些 ID 会被持久化写入每个 vnode 的 `tsdb/current.json` 元数据文件中，并在下次启动时用于还原各数据文件的绝对路径。

若将新磁盘插入到该级别列表的中间，其后的所有磁盘都会获得新的 ID，导致 TDengine 在启动时按错误的磁盘路径去寻找现有数据文件。
...
dataDir /mnt/data3 0 0   # 新磁盘添加在末尾 ✓
```
