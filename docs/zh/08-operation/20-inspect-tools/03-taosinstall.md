...
Options:
  ...
  -sh, --set-hostname      set hostname, default is False
  -si, --set-ips           set ips to /etc/hosts, default is False
...
- `set-hostname`：根据配置文件的FQDN更新集群节点的hostname。默认为False
- `set-ips`：根据配置文件的FQDN对应IP更新集群节点的/etc/hosts。默认为False
- `version`：打印安装工具版本信息。

#### 安装流程
> **💡 Note:** 在安装前需确认TDengine各服务对应的配置文件中预设定参数是否配置正确，特别是数据和日志路径等信息。
...
停服升级数据库
```
./taosinstall upgrade -m ssh -u
```
滚动升级数据库
```
./taosinstall upgrade -m ssh -ru
```