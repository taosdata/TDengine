...
1. `column_name`：进行时序数据异常检测的输入数据列，当前只支持单列，且只能是数值类型，不能是字符类型（例如：`NCHAR`、`VARCHAR`、`VARBINARY`等类型），**不支持函数表达式**。3.4.1.0 开始支持多列数据输入异常检测，对于只能处理单列的模型，只识别第一列，多余的列将自动忽略。
...
--- 使用 ksigma 算法进行异常检测，输入参数 k 值为 2，检测列 i32 列, i8列会自动忽略，
--- 在 3.4.1.x 之前的版本会报告错误，3.4.1.x 之后的版本自动忽略 i8 列
SELECT _wstart, _wend, SUM(i32) 
FROM foo
