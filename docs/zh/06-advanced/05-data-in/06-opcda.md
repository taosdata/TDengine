...
| 4    | tbname                  | String   | 符合 TDengine 子表命名规范的任何字符串；如果存在特殊字符`.`，使用下划线替换。对于 OPC UA：如果存在`{ns}`，使用 point_id 中的 ns 替换，如果存在`{id}`，使用 point_id 中的 id 替换。对于 OPC DA：如果存在`{tag_name}`，使用 tag_name 中最后一个`.`之后的字符串替换。如果存在`{/tag_name}`，使用 tag_name 中最后一个`/`之后的字符串替换；如果存在`{id}`，使用 tag_name 替换；如果存在`{_id}`，所有`/`用`_`替换。 | 是       |                          |
