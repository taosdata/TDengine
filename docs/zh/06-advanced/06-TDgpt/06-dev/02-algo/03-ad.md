...
`execute` 是算法处理的核心方法。框架调用该方法之前，会在对象属性 `self.list` 中设置好用于异常检测的时间序列数据。

从 3.4.1.0 版本开始，对于每个从 `AbstractAnomalyDetectionService` 继承的异常检测类，还新增了一个 Python 成员变量 `input_data_lists`。该变量是一个二维数组，用于存储多列输入数据。为保持兼容性，`self.list` 仍表示第一列输入数据，而 `input_data_lists` 包含全部输入列。因此，算法作者在 `execute` 中如果只处理单列数据，可继续读取 `self.list`；如果需要处理多列数据，则应读取 `input_data_lists`。 
