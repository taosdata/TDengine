...
```shell
docker pull tdengine/tdgpt:3.4.0.2
```
...
```shell
docker pull tdengine/tdgpt-full:3.4.0.2
```
...
- Python: 3.10、3.11、3.12 版本。部分依赖库与 Python 3.13 及以上版本存在冲突，暂不支持。
- TDengine TSDB：3.3.6.0 及以上版本。
- C 编译器：因依赖 uWSGI，部署环境需包含 C 编译器（注： 3.4.1 版本不再使用 uWSGI）。但是，PyTorch 进行深度学习推理的时候会动态生成C++代码，优化模型执行效率，该过程需要调用 cc1x ，因此运行环境需具备完整编译运行能力。
...
1. 从列表中下载获得 tar.gz 安装包
   ...
   <PkgListV37 productName="TDengine TDgpt-OSS" version="3.4.0.2" platform="Linux-Generic" pkgType="Server"/>
...
```bash
tar -zxvf tdengine-tdgpt-oss-3.4.0.2-linux-x64.tar.gz
```
