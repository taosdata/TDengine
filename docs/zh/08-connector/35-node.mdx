...
**Node.js Websocket 连接器**， 通过 taosAdapter 提供的 Websocket 接口连接 TDengine 实例。

连接方式的详细介绍请参考：[连接器建立连接的方式](../../develop/connect/#连接器建立连接的方式)

## 支持的平台

Node.js Websocket 连接器，支持 Node.js 14以上版本。
...
## 处理异常
在报错后，通过 try catch 可以获取到错误的信息和错误码。

错误说明：Node.js Websocket 连接器错误码在 100 到 110 之间，之外的错误为 TDengine 其他功能模块的报错。
...
| 102     | received data, but no find callback                             | 接收到服务端数据但没有找到上层回调                                      |
| 103     | invalid message type                                            | 接收到的消息类型无法识别，请检查服务端是否正常。               |
| 104     | connection not created                                          | 连接没有创建，请检查连接是否正常。                           |
...
| 108     | connection already closed                                       | 连接已经关闭，请检查 Connection 是否关闭后再次使用，或是连接是否正常。                                                          |
| 109     | parse fetch block data fail                                     | 获取到的查询数据，解析失败                    |
| 110     | Websocket connector arrived limited                             | Websocket连接达到上限                                                   |

## TDengine DataType 和 Node.js DataType
...
- 新建安装验证目录，例如：`~/tdengine-test`，下载 GitHub 上 [nodejsChecker.js 源代码](https://github.com/taosdata/TDengine/tree/3.0/docs/examples/node/Websocketexample/nodejsChecker.js)到本地。
...
其他参数请参考：[Consumer 参数列表](../../develop/tmq#创建-consumer-以及consumer-group)， 注意 TDengine 服务端自3.2.0.0版本开始消息订阅中的 auto.offset.reset 默认值发生变化。
...
- Node.js Websocket 连接器（`@tdengine/websocket`）支持 Node.js 14 以上版本，其他版本可能存在包兼容性的问题。
- 使用链接器结束后，需要调用 taos.connectorDestroy(); 释放连接器资源。


## 常见问题

1. 使用 Node.js Websocket 连接器需要启动 taosadapter。

   ```bash
   sudo systemctl start taosadapter
   ```

2. "Unable to establish connection"或"Unable to resolve FQDN"
