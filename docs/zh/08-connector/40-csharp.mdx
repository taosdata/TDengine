...
也可以修改当前项目的 `.csproj` 文件，添加一下 ItemGroup。
...
需要修改目标项目的 `.csproj` 项目文件，将 nupkg 中的 `runtimes/` 下的动态库复制到当前项目的 `$(OutDir)`下。一下示例是复制到 `$(OutDir)\dep_lib`。
...
注意：`TDengine.Connector` 自 version>= 3.0.2 的 nuget package 中才会有动态库。
