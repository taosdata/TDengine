...
---
本节讲述如何通过 Explorer 界面创建数据迁移任务, 从 OPC-DA 服务器迁移数据到当前 TDengine 集群。
...
如果 taosX 服务运行在 OPC-DA 所在服务器上，**代理** 是不必须的，否则，需要配置 **代理** ：在下拉框中选择指定的代理，也可以先点击右侧的 **+创建新的代理** 按钮创建一个新的代理 ，跟随提示进行代理的配置。
...
在 **连接配置** 区域填写 **OPC-DA 服务地址**，例如：`127.0.0.1:5000`，并配置认证方式。
...
- 信息点编码：用作点位子表名。
- TAG 点地址：通常是 `id.name` 格式的字符串，可使用工具从 OPC-DA 服务器导出，或从本页面 **下载所有位点的列表** 下载后再编辑。
