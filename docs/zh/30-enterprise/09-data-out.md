...
- topic_suffix: TMQ topic 名字的后缀，此项为必填字段；
- cols: 超级表中要同步的列，默认为所有字段，此项为可选字段；
- tags: 超级表中要同步的标签，默认为所有标签，此项为可选字段；
...
从 192.168.2.19 上的 TDengine 输出数据到 192.168.2.13 服务器的 Kafka 实例中。
