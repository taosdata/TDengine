...
import swin from '../05-tdengine-sql/04-data-query/assets/session_window.png';
import ewin from '../05-tdengine-sql/04-data-query/assets/event_window.png';
...
- [设置当前会话时区](../05-tdengine-sql/10-time/01-timezone.md#设置时区)
- [了解夏令时（DST）对写入与查询的影响](../05-tdengine-sql/10-time/02-dst.md)
...
![时间窗口示意图](../05-tdengine-sql/04-data-query/assets/time_window.webp)
...
INTERVAL 子句支持使用 FILL 子句来指定数据缺失时的数据填充方法。关于 FILL 子句如何使用请参考 [FILL 子句](#fill-子句)。
...
- `TRUE_FOR(...)` 指定窗口过滤条件，以及（仅事件窗口）开窗/关窗连续满足门限。支持基于持续时间、记录条数或两者组合过滤，也可用 `start(...)` 指定开窗连续满足条件、`end(...)` 指定关窗连续满足条件。详见 [事件窗口 TRUE_FOR 参数](../05-tdengine-sql/04-data-query/06-distinguished.md#事件窗口)。

详细说明参见 [TDengine TSDB 特色查询](../05-tdengine-sql/04-data-query/06-distinguished.md#状态窗口)。
...
关于 FILL 子句的通用语法请参考 [FILL 子句](#fill-子句)。
...
EXTERNAL_WINDOW 也支持在空窗口上使用 `FILL` 子句。默认情况下，空窗口不会产出结果行；使用 `FILL` 后，可以决定是否保留该窗口以及如何填充聚合列。在 EXTERNAL_WINDOW 查询中，支持的模式为 `NONE`、`NULL`、`NULL_F`、`VALUE`、`VALUE_F`、`PREV`、`NEXT`；暂不支持 `LINEAR`、`NEAR`、`SURROUND`。FILL 产生的填充行会参与 `HAVING` 判断，因此执行顺序是"先填充，再过滤"。关于 FILL 子句的通用语法请参考 [FILL 子句](#fill-子句)。

#### 约束与限制

关于外部窗口的核心特性（分组对齐、窗口属性列引用规则、嵌套调用等）和约束限制的详细说明，请参考 [TDengine TSDB 特色查询 - 外部窗口](../05-tdengine-sql/04-data-query/06-distinguished.md#外部窗口)。
...
窗口函数只能出现在 `SELECT` 列表和 `ORDER BY` 中，如需基于窗口结果再做过滤或聚合，请把窗口查询写成子查询。完整的语法、窗口帧规则、命名窗口和函数列表见[窗口函数](../05-tdengine-sql/04-data-query/09-window-function.md)。
