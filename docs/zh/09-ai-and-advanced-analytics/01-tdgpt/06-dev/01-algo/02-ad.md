...
自 `v3.4.1.0` 起，对于每个从 `AbstractAnomalyDetectionService` 继承的异常检测类，还新增了一个 Python 成员变量 `self.input_data_lists`。该变量是一个二维数组，用于存储多列输入数据。为保持兼容性，`self.list` 仍表示第一列输入数据，而 `self.input_data_lists` 包含全部输入列。因此，算法作者在 `execute` 中如果只处理单列数据，可继续读取 `self.list`；如果需要处理多列数据，则应读取 `self.input_data_lists`。
...
对于 PyOD 这类第三方检测器，需要将 `self.input_data_lists` 转换为 `(n_samples, n_features)` 形状的特征矩阵。PyOD 使用 `0` 表示正常点、`1` 表示异常点，适配代码需要将 PyOD 标签映射为 TDgpt 的结果约定：正常点使用 `self.valid_code`，异常点使用 `-1`。
...
如果是第一次启动该 Anode, 请按照 [Anode 管理](../../03-management.md) 中的步骤先将该 Anode 添加到 TDengine 系统中。
