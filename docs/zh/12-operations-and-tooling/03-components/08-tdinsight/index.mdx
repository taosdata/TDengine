...
- TDengine 已经安装并正常运行，此仪表盘需要 TDengine `v3.0.0.0` 及以上，并开启监控上报配置，具体配置请参考：[taosd 监控相关](../01-taosd.md#监控相关)。
- taosAdapter 已经安装并正常运行。具体细节请参考：[taosAdapter 参考手册](../03-taosadapter.md)
- taosKeeper 已安装并正常运行。具体细节请参考：[taosKeeper 参考手册](../05-taoskeeper.md)
- Grafana 服务已安装并正常运行。建议使用最新的 Grafana 版本，TDinsight 支持 Grafana 8.0 及以上版本。
  :::info
...
安装 Grafana TDengine 数据源插件和配置数据源的步骤请参考：[与 Grafana 的集成](../../../13-ecosystem-integrations/02-visual/01-grafana.mdx#安装-grafana-插件并配置数据源)

## 导入 TDinsightV3 仪表盘

- **推荐方式**：在配置好数据源后，点击“Settings”右侧的“Dashboards”，会显示数据源自带的仪表盘，然后选择“TDengine for 3.x”右侧的“Import”进行导入。  
![TDinsight V3 import](../../../assets/tdinsight-01.webp)  
...
![TDengine Database TDinsight mnodes overview](../../../assets/tdinsight-04.webp)
...
![TDengine Database TDinsight requests](../../../assets/tdinsight-05.webp)
...
![TDengine Database TDinsight database](../../../assets/tdinsight-06.webp)
...
![TDengine Database TDinsight dnode-usage](../../../assets/tdinsight-07.webp)
...
![TDengine Database TDinsight monitor taosadapter](../../../assets/tdinsight-08.webp)
...
假设你在主机 `tdengine` 上启动 TDengine 数据库，HTTP API 端口为 `6041`，用户为 `root1`，密码为 `pass5ord`。执行脚本：
...
请注意，配置数据源、通知 Channel 和仪表盘在前端是不可更改的。你应该再次通过此脚本更新配置或手动更改 `/etc/grafana/provisioning` 目录（这是 Grafana 的默认目录，根据需要使用`-P`选项更改）中的配置文件。

特别地，当你使用 Grafana Cloud 或其他组织时，`-O` 可用于设置组织 ID。 `-G` 可指定 Grafana 插件安装目录。 `-e` 参数将仪表盘设置为可编辑。
