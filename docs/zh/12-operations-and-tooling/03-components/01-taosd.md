...
- 说明：安全删除物理覆写填充方式。仅当数据库开启 `SECURE_DELETE` 时生效。`0` 表示用零填充覆写；`1` 表示对整块可直接覆写的数据块使用随机字节填充。部分重叠块为保证就地写回，始终使用零填充。详见 [数据安全 · 安全删除](../../11-security-guide/03-data-security.md#安全删除)。
- 类型：整数
- 默认值：0
- 最小值：0
- 最大值：1
