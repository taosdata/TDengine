...
- 任何已经加入集群的 dnode 都可以作为后续待加入节点的 `firstEp`。`firstEp` 参数仅仅在该 dnode 首次加入集群时起作用，加入集群后，该 dnode 会保存最新的 mnode 的 endpoint 列表，后续不再依赖这个参数。之后配置文件中的 `firstEp` 参数主要用于客户端连接，如果没有为 `taos` shell 设置参数，则默认连接由 `firstEp` 指定的节点。
...
TDengine 提供了可视化管理 TDengine 集群的能力，要想使用图形化界面需要部署 taosExplorer 服务，关于它的详细说明和部署请参考 [taosExplorer 参考手册](../03-components/04-explorer.md)
