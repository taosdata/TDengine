...
TDengine 数据源插件已提交至 Grafana 官网，如何安装 TDengine 数据源插件和配置数据源请参考 [安装 Grafana 插件并配置数据源](../../13-ecosystem-integrations/02-visual/01-grafana.mdx#安装-grafana-插件并配置数据源)。完成插件的安装和数据源的创建后，可以进行 TDinsight 仪表盘的导入。
...
2. taosX 服务监控配置正确，如何配置可以参考下文“配置 taosX 监控”，服务启动成功。  
   **注意**：TDengine TSDB Enterprise `v3.2.3.0` 或以上版本包含的 taosX 才包含此功能。如果单独安装 taosX，需要 taosX 1.5.0 或以上版本。
3. 部署 Grafana，安装 TDengine Datasource 插件，配置好数据源。可以参考：[安装 Grafana 插件并配置数据源](../../13-ecosystem-integrations/02-visual/01-grafana.mdx#安装-grafana-插件并配置数据源)。  
   **注意**：需要安装 Grafana 插件 [TDengine Datasource v3.5.0](https://grafana.com/grafana/plugins/tdengine-datasource/) 或以上版本。

### 配置 taosX 监控

taosX 的配置文件（默认 `/etc/taos/taosx.toml`）中与 monitor 相关的配置如下：
...
| interval       | --monitor-interval | MONITOR_INTERVAL  | taosX 发送 metrics 数据到 taosKeeper 的时间间隔，单位秒 | 1-10     | 10                                       |

### 基于 TDinsight 监控 taosX
...
taosExplorer 支持集成已有的 Grafana dashboard。

### 配置 Grafana

编辑 `grafana.ini`，修改以下配置项。配置 `root_url` 可能对现有的 Grafana 使用习惯有所影响；为了集成到 taosExplorer，需要如此配置，以便通过 taosExplorer 做服务代理。
...
修改 `explorer.toml`，其中 dashboard 配置的 URL 中的 IP，应配置为 taosExplorer 服务器能够访问到的 Grafana 服务内网地址。
