...
TDengine TSDB 完整的软件包包括服务端（taosd）、用于与第三方系统对接并提供 RESTful 接口的 taosAdapter、应用驱动（taosc）、命令行程序 (TDengine TSDB CLI) 和一些工具软件。TDengine TSDB 除了提供多种语言的连接器之外，还通过 [taosAdapter](../reference/components/taosadapter) 提供 [RESTful 接口](../reference/connector/rest-api)。

本章主要介绍如何快速设置 TDengine TSDB 环境并体验其高效写入和查询。
...
## 加入 TDengine TSDB 官方社区

微信扫描以下二维码，学习了解 TDengine TSDB 的最新技术，与大家共同交流物联网大数据技术应用、TDengine TSDB 使用问题和技巧等话题。
