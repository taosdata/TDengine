...
1. 从列表中下载获得 tar.gz 安装包：
    <PkgListV37 productName="TDengine TSDB-Enterprise" version="3.4.0.2" platform="Linux-Generic" pkgType="Server"/>
2. 进入到安装包所在目录，使用 `tar` 解压安装包，以 x64 架构为例：
    ```bash
    tar -zxvf tdengine-tsdb-enterprise-3.4.0.2-linux-x64.tar.gz
    ```
...
1. 从列表中下载获得最新 Windows 安装程序：
    <PkgListV37 productName="TDengine TSDB-Enterprise" version="3.4.0.2" platform="Windows" pkgType="Server" />
