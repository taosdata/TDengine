...
本节讲述如何部署 Agent (for taosX)。在安装了 taosX 安装包后，系统中就具备了 Agent，细节请参考 [安装与配置](../install)。
...
可以通过 `journalctl` 查看 Agent 的日志或通过日志文件 `/var/log/taos/agent.log` 或 `C:\\TDengine\\log\agent.log` 文件查看。
