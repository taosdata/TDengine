...
进入消费后，在固定间隔（每 10 秒或每条消息后）检查当前时间是否达到 `refreshAt`（`80% TTL + 最多 10% TTL 的随机抖动`）；达到后触发轮换。轮换核心顺序为：**先创建并订阅新 Consumer** → 对旧 Consumer 做 best-effort commit offset → unsubscribe 并 close 旧 Consumer → 切换到新 Consumer。
...
stateDiagram-v2
    ...
    消费 --> 检查TokenAge : poll/commit 周期
    消费 --> 鉴权失败恢复轮换 : 认证失败（0x80000357）
    检查TokenAge --> 判定Token是否过期
    state 判定Token是否过期 <<choice>>
    判定Token是否过期 --> 消费 : now < refreshAt
    判定Token是否过期 --> 主动轮换 : now >= refreshAt（80% TTL + jitter）

    主动轮换 --> 构建并订阅新Consumer
    鉴权失败恢复轮换 --> 构建并订阅新Consumer
    构建并订阅新Consumer --> 判定轮换构建结果
    state 判定轮换构建结果 <<choice>>
    判定轮换构建结果 --> commitOffset : 新Consumer就绪
    判定轮换构建结果 --> 消费 : 主动轮换失败（保留旧Consumer并重试）
    判定轮换构建结果 --> 退出循环 : 鉴权失败恢复轮换仍失败

    commitOffset --> 取消订阅并关闭旧Consumer : best effort
    取消订阅并关闭旧Consumer --> 切换引用
    切换引用 --> 消费
    退出循环 --> [*]
```
...
**本状态机中的轮换触发条件**：周期检查 `now >= refreshAt`（`80% TTL + jitter`），或出现认证失败（`0x80000357`）。
