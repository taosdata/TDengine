...
通过 [TDengine TSDB Java connector](../../09-developer-guide/08-connectors-reference/02-java.mdx)，Spark 可快速读取 TDengine TSDB 数据，利用 Spark 强大引擎，扩展 TDengine TSDB 数据处理计算能力，同时通过它，Spark 亦可把数据写入 TDengine TSDB 及从 TDengine TSDB 订阅数据。
...
详细参数见：[URL 参数介绍](../../09-developer-guide/08-connectors-reference/02-java.mdx#url-规范)。
...
数据接入需注册 TDengine TSDB 方言，方言中主要处理反引号，数据类型映射与 JDBC 相同，无需额外处理，参见：[JDBC 数据类型映射](../../09-developer-guide/08-connectors-reference/02-java.mdx#数据类型映射)
