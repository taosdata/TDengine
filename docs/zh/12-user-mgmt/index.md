...
TDengine Cloud 提供了一个简单而易用的用户管理，包括用户、用户组、角色、权限和资源。TDengine Cloud 默认提供了 4 个级别的权限和 8 种默认角色，其中包括组织级别、实例级别、数据库级别和权限管理级别，默认的角色是数据库管理员、账户、数据生产者、数据消费者、开发人员、实例管理员、超级管理员和组织所有者。您还可以用 TDengine Cloud 已经定义的权限创建自定义角色。
