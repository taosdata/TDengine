...
import xiaot from './assets/xiaot.webp'
...
TDengine TSDB 完整的软件包包括服务端（taosd）、用于与第三方系统对接并提供 RESTful 接口的 taosAdapter、应用驱动（taosc）、命令行程序 (TDengine TSDB CLI) 和一些工具软件。TDengine TSDB 除了提供多种语言的连接器之外，还通过 [taosAdapter](../../11-operations-and-tooling/03-components/03-taosadapter.md) 提供 [RESTful 接口](../../09-developer-guide/08-connectors-reference/10-rest-api.mdx)。
