...
本节讲述如何通过 Explorer 界面创建数据迁移任务，从 InfluxDB 迁移数据到当前 TDengine TSDB 集群。

## 功能概述

InfluxDB 是一种流行的开源时间序列数据库，它针对处理大量时间序列数据进行了优化。TDengine TSDB 可以通过 InfluxDB 连接器高效地读取 InfluxDB 中的数据，并将其写入 TDengine TSDB，以实现历史数据迁移或实时数据同步。
...
点击 **提交** 按钮，完成创建 InfluxDB 到 TDengine TSDB 的数据同步任务，回到**数据源列表**页面可查看任务执行情况。
