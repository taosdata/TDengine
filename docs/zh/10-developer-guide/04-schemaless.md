...
sidebar_label: 无模式写入
title: 无模式写入
description: 使用 InfluxDB / OpenTSDB 行协议向 TDengine 无模式写入
...
TDengine 提供数据写入的幂等性保证，即可反复调用 API 进行出错数据的写入操作。但是不提供多行数据写入的原子性保证。即在多行数据一批次写入过程中，会出现部分数据写入成功，部分数据写入失败的情况。
...
8. 为了提高写入的效率，默认假设同一个超级表中 field_set 的顺序是一样的（第一条数据包含所有的 field，后面的数据按照这个顺序），如果顺序不一样，需要配置参数 `smlDataFormat` 为 `false`，否则，数据写入按照相同顺序写入，库中数据会异常，从 `v3.0.3.0` 开始自动检测顺序是否一致，该配置已废弃。
9. 由于 SQL 建表表名不支持点号（`.`），所以 schemaless 也对点号（.）做了处理，如果 schemaless 自动建表的表名如果有点号（.），会自动替换为下划线（\_）。如果手动指定子表名的话，子表名里有点号（.），同样转化为下划线（\_）。
10. `taos.cfg` 增加 `smlTsDefaultName` 配置（字符串类型），只在客户端起作用，配置后，schemaless 自动建表的时间列名字可以通过该配置设置。不配置的话，默认为 `_ts`。
11. 无模式写入的数据超级表或子表名区分大小写。
12. 无模式写入仍然遵循 TDengine 对数据结构的底层限制，例如每行数据的总长度不能超过 64KB（`TSDB_MAX_BYTES_PER_ROW`，`v3.0.5.0` 之前为 48KB），标签值的总长度不超过 16KB。
...
推荐使用 `ws/unified` 无模式写入接口（Go 连接器 `v3.8.0` 起）：

```go
{{#include docs/examples/go/schemaless/unified/main.go}}
```

`ws/schemaless` 兼容接口（下方示例）自 `v3.8.0` 起标记为废弃，当前仍有效，建议尽快迁移：
...
运行上节的样例代码，会在 power 数据库中自动建表，可通过 `taos` shell 或应用程序查询数据。下面给出用 `taos` shell 查询超级表和 `meters` 表的示例。
