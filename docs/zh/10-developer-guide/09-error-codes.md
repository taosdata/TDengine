...
| 0x80000237 | Operation not supported in windows | 当前操作在 Windows 平台不支持 | 在支持该功能的操作系统上执行，或避免在 Windows 平台执行该操作                   |
...
| 0x800003F9 | Stream has no stored CREATE statement (created before this feature or metadata missing)      | 不存在                                                                            | 确认建流版本是否过低导致不兼容                                                                                     |
...
| 0x800026B4 | Function does not support DISTINCT                                                                     | 对不支持 DISTINCT 的函数使用了 DISTINCT 关键字，或对 `*` 使用了 DISTINCT，或在 SESSION/STATE/EVENT 窗口中使用了 DISTINCT | 仅 COUNT/SUM/AVG/MIN/MAX 支持 DISTINCT，且不能与 SESSION/STATE/EVENT 窗口同时使用 |
| 0x800026B5 | Window function is not allowed in this clause                                                          | 窗口函数出现在非法子句中                               | 将窗口函数移到 SELECT 或 ORDER BY 中                        |
| 0x800026B6 | Window function requires ORDER BY                                                                      | 需要排序的窗口函数未指定 ORDER BY                      | 在窗口规范中补充 ORDER BY                                   |
| 0x800026B7 | Referenced window name is not defined                                                                  | OVER 引用的命名窗口不存在                              | 在当前查询块中定义该窗口名                                 |
| 0x800026B8 | Duplicate window name                                                                                  | 多个 WINDOW 项使用了同名窗口                           | 重命名其中一个 WINDOW 定义                                 |
| 0x800026B9 | RANGE offset frame requires exactly one ORDER BY expression                                            | 带偏移的 RANGE 帧使用了多个 ORDER BY 表达式             | 将 ORDER BY 缩减为一个表达式                                |
| 0x800026BA | Invalid window frame bound                                                                             | 帧起点或终点无效                                        | 修正为合法的 PRECEDING/FOLLOWING 边界                      |
| 0x800026BB | Invalid window function argument                                                                       | 窗口函数参数非法或超出范围                              | 修正函数参数个数、类型或取值                                |
| 0x800026BC | Invalid window function argument                                                                       | 窗口函数参数非法或超出范围                              | 修正函数参数个数、类型或取值                                |
...
| 0x80002816 | Invalid timeline in function                 | 函数输入时间线无效。对依赖时间线顺序的函数，如果输入数据缺少有效时间线、时间线包含 NULL、乱序或重复时间戳，函数无法得到有意义的计算结果。                                                                                                                                          | 检查 SQL 输入，确保函数使用有效且满足函数要求的时间线；必要时按时间列排序或过滤非法时间戳数据。              |
