...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]：仅在使用 JDBC WebSocket 连接时生效，令牌信息。其认证优先级高于用户名密码。详见 [连接器安全最佳实践](../../11-security-guide/04-connector-security.md#3-token-认证)。
...
- TSDBDriver.PROPERTY_KEY_USE_SSL [`useSSL`]：连接中是否使用 SSL。仅在 WebSocket 连接时生效。详见 [客户端 SSL/TLS 配置](../../11-security-guide/04-connector-security.md#22-客户端-ssltls-配置)。
...
JDBC 标准不支持数据订阅，因此本章所有接口都是扩展接口。主题与 `WITH META` 等概念见 [数据订阅](../../07-data-subscription/index.md) 与 [主题语法](../../07-data-subscription/01-topic.md)；多语言用法见 [数据订阅编程接口](../07-subscription-api.md)。TaosConsumer 类提供了消费者相关接口，ConsumerRecord 提供了消费记录相关接口，TopicPartition 和 OffsetAndMetadata 提供了分区信息以及偏移量元数据接口。最后 ReferenceDeserializer 和 MapDeserializer 提供了反序列化的支持。
