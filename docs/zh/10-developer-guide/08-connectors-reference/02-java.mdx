...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| 3.9.0                | 优化参数绑定缓存和序列化，以提升 TDengine TSDB Enterprise 3.4.1.16 及更高版本的写入性能                                                                                                                                                                                                                              | -                  |
...
- TSDBDriver.PROPERTY_KEY_STMT_BIND_MODE [`stmtBindMode`]：参数绑定序列化模式，支持 `column` （列模式，TDengine 3.4.1.13 版本后支持，序列化效率高且字节数少）、`traditional` （之前的按表分组绑定模式）和 `auto` (根据版本自动选择)。仅在使用 WebSocket 连接时生效，支持高效写入模式。默认值为 `auto`。
...
其他参数请参考：[Consumer 参数列表](../07-subscription-api.md#创建参数)，注意 TDengine TSDB 服务端自 3.2.0.0 版本开始消息订阅中的 auto.offset.reset 默认值发生变化。
