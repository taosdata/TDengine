...
description: TDengine Java 连接器基于标准 JDBC API 实现，并提供原生连接与 WebSocket 连接两种连接器。
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import RequestId from "./resources/_request_id.mdx";
...
详细的迁移指引请参考：[连接方式说明](./index.md#连接方式)
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| 3.9.0                | 优化参数绑定缓存和序列化，以提升 TDengine TSDB Enterprise `v3.4.1.16` 及更高版本的写入性能                                                                                                                                                                                                                              | -                  |
| 3.8.4                | 1. 支持 taosadapter 的高可用（HA）端点发现。<br/>2. 连接前对端点进行乱序。<br/>3. 修复 WebSocket 连接属性初始化 Catalog。                                                                                                                                                                                                                           | -                  |
...
| 3.6.0                | 在 WebSocket 连接上支持高效写入和 Decimal 数据类型                                                                                                                                                                                                                                     | `v3.3.6.0` 及更高版本 |
| 3.5.3                | 在 WebSocket 连接上支持无符号数据类型                                                                                                                                                                                                                                                  | -                  |
| 3.5.2                | 解决了 WebSocket 查询结果集释放 bug                                                                                                                                                                                                                                                    | -                  |
| 3.5.1                | 解决了数据订阅获取时间戳对象类型问题                                                                                                                                                                                                                                                   | -                  |
| 3.5.0                | 1. 优化了 WebSocket 连接参数绑定性能，支持参数绑定查询使用二进制数据 <br/> 2. 优化了 WebSocket 连接在小查询上的性能 <br/> 3. WebSocket 连接上支持设置时区和应用信息                                                                                                                    | `v3.3.5.0` 及更高版本 |
...
| 3.3.0                | 1. 优化 WebSocket 连接下的数据传输性能 <br/> 2. 支持跳过 SSL 验证，默认关闭                                                                                                                                                                                                            | `v3.3.2.0` 及更高版本 |
...
| 3.2.7                | 1. 支持 VARBINARY 和 GEOMETRY 类型 <br/> 2. 增加 native 连接的时区设置支持 <br/> 3. 增加 WebSocket 自动重连功能                                                                                                                                                                        | `v3.2.0.0` 及更高版本 |
| 3.2.5                | 数据订阅增加 committed()、assignment() 方法                                                                                                                                                                                                                                            | `v3.1.0.3` 及更高版本 |
| 3.2.4                | 数据订阅在 WebSocket 连接下增加 enable.auto.commit 参数，以及 unsubscribe 方法                                                                                                                                                                                                         | -                  |
| 3.2.3                | 修复 ResultSet 在一些情况数据解析失败                                                                                                                                                                                                                                                  | -                  |
| 3.2.2                | 数据订阅支持 seek 功能                                                                                                                                                                                                                                                                 | `v3.0.5.0` 及更高版本 |
| 3.2.1                | 1. WebSocket 连接支持 schemaless 与 prepareStatement 写入 <br/> 2. consumer poll 返回结果集为 ConsumerRecord，可通过 value() 获取指定结果集数据                                                                                                                                        | `v3.0.3.0` 及更高版本 |
| 3.2.0                | 存在连接问题，不推荐使用                                                                                                                                                                                                                                                               | -                  |
| 3.1.0                | WebSocket 连接支持订阅功能                                                                                                                                                                                                                                                             | -                  |
| 3.0.1 - 3.0.4        | 修复一些情况下结果集数据解析错误的问题。3.0.1 在 JDK 11 环境编译，JDK 8 环境下建议使用其他版本                                                                                                                                                                                         | -                  |
| 3.0.0                | 支持 TDengine `v3.0`                                                                                                                                                                                                                                                                 | `v3.0.0.0` 及更高版本 |
...
错误码信息请参考 [错误码](../09-error-codes.md)  
...
GEOMETRY 类型是 little endian 字节序的二进制数据，符合 WKB 规范。详细信息请参考 [数据类型](../../05-tdengine-sql/01-datatype.md)  
WKB 规范请参考[Well-Known Binary (WKB)](https://libgeos.org/specifications/wkb/)  
对于 java 连接器，可以使用 jts 库来方便的创建 GEOMETRY 类型对象，序列化后写入 TDengine，这里有一个样例[Geometry 示例](https://github.com/taosdata/TDengine/blob/main/docs/examples/JDBC/JDBCDemo/src/main/java/com/taos/example/GeometryDemo.java)  
...
**原因**：taos-jdbcdriver 3.\* 版本仅支持 TDengine `v3.0` 及以上版本。
...
`jdbc:TAOS://taosdemo.com:6030/power?user=root&password=taosdata`，使用了 JDBC 原生连接的 TSDBDriver，建立了到 host 为 taosdemo.com，端口为 6030（TDengine 的默认端口），数据库名为 power 的连接。这个 URL
...
当使用 JDBC 原生连接连接 TDengine 集群时，可以使用 TDengine 客户端驱动配置文件，在配置文件中指定集群的 firstEp、secondEp 等参数。此时在 JDBC url 中不指定 `host` 和 `port`。
...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]：仅在使用 JDBC WebSocket 连接时生效，令牌信息。其认证优先级高于用户名密码。详见 [客户端与连接器安全 · Token 认证](../../11-security-guide/05-client-connector-security.md#3-token-认证)。
...
- TSDBDriver.PROPERTY_KEY_USE_SSL [`useSSL`]：连接中是否使用 SSL。仅在 WebSocket 连接时生效。详见 [客户端与连接器安全 · SSL/TLS](../../11-security-guide/05-client-connector-security.md#22-客户端-ssltls-配置)。
- TSDBDriver.PROPERTY_KEY_DISABLE_SSL_CERT_VALIDATION [`disableSSLCertValidation`]：关闭 SSL 证书验证。仅在使用 WebSocket 连接时生效。true：启用，false：不启用。默认为 false。⚠️ **警告**：关闭证书验证存在中间人攻击风险，严禁在生产环境使用。
...
- TSDBDriver.PROPERTY_KEY_ADAPTER_HA [`adapterHA`]：是否启用 `adapter` 高可用。仅在使用 WebSocket 连接时生效。true：启用，false：不启用。默认为 false。启用后连接成功即可获取集群所有 `adapter` 端点列表用于负载均衡，需要 TDengine 版本大于等于 `v3.4.2.0` 才能支持此特性。
...
- TSDBDriver.PROPERTY_KEY_PBS_MODE [`pbsMode`]：**已废弃**。
- TSDBDriver.PROPERTY_KEY_STMT_BIND_MODE [`stmtBindMode`]：参数绑定序列化模式，支持 `column` （列模式，TDengine `v3.4.1.13` 及之后支持，序列化效率高且字节数少）、`traditional` （之前的按表分组绑定模式）和 `auto` (根据版本自动选择)。仅在使用 WebSocket 连接时生效，支持高效写入模式。默认值为 `auto`。
...
JDBC 标准不支持数据订阅，因此本章所有接口都是扩展接口。主题与 `WITH META` 等概念见 [数据订阅](../../06-data-subscription/index.md) 与 [主题语法](../../06-data-subscription/01-topic.md)；多语言用法见 [数据订阅编程接口](../07-subscription-api.md)。TaosConsumer 类提供了消费者相关接口，ConsumerRecord 提供了消费记录相关接口，TopicPartition 和 OffsetAndMetadata 提供了分区信息以及偏移量元数据接口。最后 ReferenceDeserializer 和 MapDeserializer 提供了反序列化的支持。
...
- bootstrap.servers：
  ...
  2. 如果使用 Native 连接，则为 TDengine 服务端所在的`ip:port`，Native 连接不支持多个端点。
...
- `td.connect.token`：TMQ 订阅的 Bearer Token。
- `td.connect.user` + `td.connect.pass`：用户名密码认证（不使用 Token 时需要）。

其他参数请参考：[创建参数](../07-subscription-api.md#创建参数)，注意：TDengine 服务端自 `v3.2.0.0` 起，消息订阅中 `auto.offset.reset` 的默认值变为 `latest`。
