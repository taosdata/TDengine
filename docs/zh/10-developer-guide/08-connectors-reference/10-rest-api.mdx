...
RESTful 接口不依赖于任何 TDengine TSDB 的库，因此客户端不需要安装任何 TDengine TSDB 的库，只要客户端的开发语言支持 HTTP 协议即可。TDengine TSDB 的 RESTful API 由 [taosAdapter](../../12-operations-and-tooling/03-components/03-taosadapter.md) 提供，在使用 RESTful API 之前需要确保 `taosAdapter` 正常运行。
