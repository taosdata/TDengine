...
RESTful 接口不依赖于任何 TDengine 的库，因此客户端不需要安装任何 TDengine 的库，只要客户端的开发语言支持 HTTP 协议即可。TDengine 的 RESTful API 由 [taosAdapter](../../12-operations-and-tooling/03-components/03-taosadapter.md) 提供，在使用 RESTful API 之前需要确保 `taosAdapter` 正常运行。
...
- row_with_meta：可选参数，指定是否每行数据都携带列名，缺省为 false。（自 `v3.3.2.0` 起支持）

例如：`http://h1.taos.com:6041/rest/sql/test` 是指向地址为 `h1.taos.com:6041` 的 URL，并将默认使用的数据库库名设置为 `test`。

HTTP 请求的 Header 里需带有身份认证信息，TDengine 支持 Basic 认证、Token 认证与自定义认证三种机制，后续版本将提供标准安全的数字签名机制来做身份验证。
...
默认情况下，`taosAdapter` 对大多数 C 接口调用出错时也会返回 200 响应码，但是 HTTP body 中包含错误信息。从 TDengine `v3.0.3.0` 开始 `taosAdapter` 提供配置参数 `httpCodeServerError` 用来设置当 C 接口返回错误时是否返回非 200 的 HTTP 响应码。无论是否设置此参数，响应 body 里都有详细的错误码和错误信息，具体请参考 [错误](#错误) 。
...
TDengine 从 `v3.4.0.0` 开始支持 token 认证机制，token 认证机制与 Basic 认证机制类似，区别在于 token 认证机制使用的授权码 `<TOKEN>` 是由 TDengine 服务器端生成并分发给客户端的，而不是由客户端通过 Base64 编码生成的。
...
TOKEN 使用 SQL 语句 `CREATE TOKEN` 创建，具体见 [令牌管理](../../05-tdengine-sql/07-user-and-privilege/01-user.md#令牌管理)。
...
## TDengine 2.x 和 `v3.0` 之间 REST API 的差异

### URI

| URI                  | TDengine 2.x         | TDengine `v3.0`                                         |
| :--------------------| :------------------: | :--------------------------------------------------: |
...
| HTTP code            | TDengine 2.x         | TDengine `v3.0` | 备注                                  |
| :--------------------| :------------------: | :----------: | :-----------------------------------: |
