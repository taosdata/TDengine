...
|:-------------|:---------------------------------|:-----------------|
| 3.2.2        | 支持 taosAdapter 高可用。              | `v3.4.1.14` 及更高版本 |
| 3.2.1        | 支持 BLOB 类型 SQL/STMT2 写入和查询；支持 DECIMAL 类型 SQL/STMT2 写入和查询。 | `v3.4.1.8` 及更高版本 |
| 3.2.0        | 支持 WebSocket 多地址故障转移。              | -                |
| 3.1.10       | 支持 TDengine Token 验证。       | -                |
| 3.1.9        | stmt 对外接口不变，内部重构为 stmt2 实现。      | -                |
| 3.1.8        | 支持连接级别时区设置，严格校验 stmt 绑定类型与数据库类型。 | -                |
| 3.1.7        | 支持 IPv6 连接，支持 DECIMAL 类型。        | `v3.3.6.0` 及更高版本    |
| 3.1.6        | 优化 WebSocket 连接接收消息处理。           | -                |
| 3.1.5        | 修复 WebSocket 协议编码中文时长度错误。        | -                |
| 3.1.4        | 提升 WebSocket 查询和写入性能。            | `v3.3.2.0` 及更高版本    |
| 3.1.3        | 支持 WebSocket 自动重连。               | -                |
| 3.1.2        | 修复 schemaless 资源释放。              | -                |
| 3.1.1        | 支持 varbinary 和 geometry 类型。      | -                |
| 3.1.0        | WebSocket 使用原生实现。                | `v3.2.1.0` 及更高版本    |
...
- DECIMAL 类型不支持作为 tag 列。在 C# 中使用 `decimal` 类型表示，支持高精度的十进制数值。因为 C# 的 `decimal` 类型与 TDengine 的 DECIMAL 类型在精度和范围上有所不同，
...
- `port`：TDengine 运行实例的端口。如果 `host` 中已经显式写了端口，则以地址中的端口为准。
- `username`：连接的用户名。
- `password`：连接的密码。
...
- `bearerToken`：连接 TDengine 的 token 验证信息（`3.1.10` 及以上版本支持）。
...
主题概念见 [数据订阅](../../06-data-subscription/index.md) 与 [主题语法](../../06-data-subscription/01-topic.md)；多语言用法见 [数据订阅编程接口](../07-subscription-api.md)。`ConsumerBuilder` 类提供了消费者构建相关接口，`ConsumeResult` 类提供了消费结果相关接口，`TopicPartitionOffset` 类提供了分区偏移量相关接口。`ReferenceDeserializer` 和 `DictionaryDeserializer` 提供了反序列化的支持。
