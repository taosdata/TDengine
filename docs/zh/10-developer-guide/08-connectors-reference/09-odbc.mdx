---
sidebar_label: ODBC
title: TDengine TSDB ODBC
---

TDengine TSDB ODBC 是为 TDengine TSDB 实现的 ODBC 驱动程序，支持 Windows 系统的应用（如 [PowerBI](https://powerbi.microsoft.com/zh-cn/) 等）以及用户自定义开发的应用程序，通过 ODBC 标准接口访问本地、远程和云服务的 TDengine TSDB 数据库。

TDengine TSDB ODBC 提供基于 WebSocket（推荐）和 原生连接两种方式连接 TDengine TSDB 数据库，使用时可以为 TDengine TSDB 数据源设置不同的连接方式。访问云服务时必须使用 WebSocket 连接方式。

TDengine TSDB ODBC 提供 64 位 和 32 位 两种驱动程序。但 32 位仅企业版支持，且仅支持 WebSocket 连接方式。
...
想更多了解 TDengine TSDB 时序时序数据库的使用，可访问 [TDengine TSDB 官方文档](https://docs.taosdata.com/intro/)。

:::warning 原生连接即将废弃

<font color="red">ODBC 的原生连接将于 2027-01-01 下线</font>，请迁移到 WebSocket 连接。
...
ODBC 驱动的**原生连接（Native）**直接调用 TDengine 客户端驱动（`taosnative` 库）通过 TDengine 私有协议与服务端通信，与 **WebSocket 连接**通过 `taosAdapter` 的 WebSocket 接口访问服务端不同。

- 原生连接通常拥有更好的性能，但要求客户端驱动版本与服务端版本一致。
- WebSocket 连接兼容性更好，一般无需随服务端升级而更新客户端，且支持云服务和 32 位应用程序。
- 建议一般场景优先使用 WebSocket 连接。详细的连接方式说明请参考：[连接方式说明](../#连接方式)

:::

:::warning 原生连接与 WebSocket 连接不可混用

原生连接（Native）和 WebSocket 连接在同一进程中**不支持混合使用，也不允许在运行时切换**。一个进程只能使用其中一种连接方式。如果需要同时使用两种连接方式，请在不同的进程中分别使用。
...
2. 安装 TDengine TSDB Windows 客户端。版本在 3.2.1.0 或以上，包含 TDengine TSDB 的 ODBC 64 位驱动；版本在 3.3.3.0 或以上，包含 TDengine TSDB 的 ODBC 32/64 位驱动。

## 配置数据源

### 数据源连接类型与区别

TDengine TSDB ODBC 支持两种连接 TDengine TSDB 数据库方式：WebSocket 连接与 Native 连接，其区别如下：

1. 访问云服务仅支持使用 WebSocket 连接方式。

2. 32 位应用程序仅支持使用 WebSocket 连接方式。

3. WebSocket 连接的兼容性更好，一般不需要随着 TDengine TSDB 数据库服务端升级而升级客户端的库。

4. Native 连接通常性能更好一点，但是必须与 TDengine TSDB 数据库服务端的版本保持一致。

5. 对于一般用户，建议使用 **WebSocket** 连接方式，性能与 Native 差别不大，兼容性更好。

6. 原生连接（Native）和 WebSocket 连接在同一进程中**不支持混合使用，也不允许在运行时切换**。一个进程只能使用其中一种连接方式，请在创建数据源时确定所需的连接类型。

7. WebSocket 连接支持配置传输压缩开关（`COMPRESSION=0/1`），用于控制传输是否启用压缩；Native 连接不支持该参数。
...
3. 选择想要添加的数据源，这里我们选择【TDengine TSDB】

4. 点击完成，进入 TDengine TSDB ODBC 数据源配置页面，填写如下必要信息
    ...
    4.5【用户名】仅供第 5 步测试连接使用，选填，数据库用户名，如果不填，TDengine TSDB 默认 root

    4.6【密码】仅供第 5 步测试连接使用，选填，数据库用户密码，如果不填，TDengine TSDB 默认 taosdata

    4.7【兼容软件】支持对 ADO 和工业软件（KingSCADA、Kepware 等）的兼容性适配，通常情况下，选择默认值 General 即可

    4.8【启用传输压缩】仅 WebSocket 连接可用，选填。勾选后等价于设置 `COMPRESSION=1`，不勾选等价于 `COMPRESSION=0`。该参数用于控制 WebSocket 传输是否启用压缩。
...
- 原生连接方式支持的平台和 TDengine TSDB Windows X64 版 客户端驱动支持的平台一致。
- WebSocket 连接方式除此之外还支持 Windows X64 系统上运行的 32 位应用程序上使用。

## 版本历史

| taos_odbc 版本 | 主要变化                                                                                             |   TDengine TSDB 版本    |
| -----------  | --------------------------------------------------------------------------------------------------  | ----------------  |
|      v1.1.1   | 支持 ADO 访问 TDengine ODBC 32/64接口。                                                                              | 3.3.3.0 及更高版本 |
...
| TDengine TSDB Type      | SQL Type          | C Type            |
|--------------------|-------------------|-------------------|
...
| SQLBrowseConnect | ❌ | ❌ | ❌ | |
| SQLBulkOperations | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLCloseCursor | ✅ | ✅ | ✅ | |
| SQLColAttribute | ✅ | ✅ | ✅ | |
| SQLColumnPrivileges | ❌ | ❌ | ❌ | TDengine TSDB 没有严格对应的 |
| SQLColumns | ✅ | ✅ | ✅ | |
| SQLCompleteAsync | ❌ | ❌ | ❌ | |
| SQLConnect | ✅ | ✅ | ✅ | |
| SQLCopyDesc | ❌ | ❌ | ❌ | |
...
| SQLDisconnect | ✅ | ✅ | ✅ | |
| SQLDriverConnect | ✅ | ✅ | ✅ | |
| SQLEndTran | ✅ | ✅ | ✅ | TDengine TSDB 是非事务性的，因此这最多是模拟 |
| SQLExecDirect | ✅ | ✅ | ✅ | |
| SQLExecute | ✅ | ✅ | ✅ | |
| SQLExtendedFetch | ❌ | ❌ | ❌ | |
| SQLFetch | ✅ | ✅ | ✅ | |
| SQLFetchScroll | ✅ | ✅ | ✅ | TDengine TSDB 没有对应的，仅仅实现 SQL_FETCH_NEXT |
| SQLForeignKeys | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLFreeHandle | ✅ | ✅ | ✅ | |
| SQLFreeStmt | ✅ | ✅ | ✅ | |
| SQLGetConnectAttr | ✅ | ✅ | ✅ | 支持部分属性，下面列出了不支持的属性 |
| SQLGetCursorName | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLGetData | ✅ | ✅ | ✅ | |
| SQLGetDescField | ❌ | ❌ | ❌ | |
| SQLGetDescRec | ❌ | ❌ | ❌ | |
| SQLGetDiagField | ✅ | ✅ | ✅ | |
...
| SQLGetTypeInfo | ✅ | ✅ | ✅ | |
| SQLMoreResults | ✅ | ✅ | ✅ | |
| SQLNativeSql | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLNumParams | ✅ | ✅ | ✅ | |
| SQLNumResultCols | ✅ | ✅ | ✅ | |
| SQLParamData | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLPrepare | ✅ | ✅ | ✅ | |
| SQLPrimaryKeys | ✅ | ✅ | ✅ | |
| SQLProcedureColumns | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLProcedures | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLPutData | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLRowCount | ✅ | ✅ | ✅ | |
| SQLSetConnectAttr | ✅ | ✅ | ✅ | 支持部分属性，下面列出了不支持的属性 |
| SQLSetCursorName | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLSetDescField | ❌ | ❌ | ❌ | |
| SQLSetDescRec | ❌ | ❌ | ❌ | |
| SQLSetEnvAttr | ✅ | ✅ | ✅ | |
| SQLSetPos | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLSetStmtAttr | ✅ | ✅ | ✅ | 支持部分属性，下面列出了不支持的属性 |
| SQLSpecialColumns | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLStatistics | ❌ | ❌ | ❌ | TDengine TSDB 没有对应的 |
| SQLTablePrivileges | ❌ | ❌ | ❌ | TDengine TSDB 没有严格对应的 |
| SQLTables | ✅ | ✅ | ✅ | |
...
| Attribute | Note |
| :----- | :---- |
| SQL_ATTR_CONCURRENCY | TDengine TSDB 没有 updatable-CURSOR 机制 |
| SQL_ATTR_FETCH_BOOKMARK_PTR | TDengine TSDB 没有  BOOKMARK 机制 |
...
- **允许任何带有 ODBC 库的编程语言与 TDengine TSDB 通信** ：
...
- API：SQLEndTran
  - **是否支持**：支持
  - **标准**：ISO 92
  - **作用**：用于提交或回滚事务。TDengine TSDB 是非事务性的，因此 SQLEndTran 函数最多只能模拟提交或回滚操作。如果有任何未完成的连接或语句，无论是提交（commit）还是回滚（rollback）都不会成功
