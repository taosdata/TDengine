...
- 建议一般场景优先使用 WebSocket 连接。详细的连接方式说明请参考：[连接方式说明](./index.md#连接方式)
...
2. 安装 TDengine Windows 客户端。版本在 `v3.2.1.0` 或以上，包含 TDengine 的 ODBC 64 位驱动；版本在 `v3.3.3.0` 或以上，包含 TDengine 的 ODBC 32/64 位驱动。
...
| -----------  | --------------------------------------------------------------------------------------------------  | ----------------  |
|      v1.1.1   | 支持 ADO 访问 TDengine ODBC 32/64接口。                                                                              | `v3.3.3.0` 及更高版本 |
|      v1.1.0   | 1. 支持视图功能；<br/>2. 支持 VARBINARY/GEOMETRY 数据类型；<br/>3. 支持 ODBC 32 位 WebSocket 连接方式（仅企业版支持）；<br/>4. 支持 ODBC 数据源配置对话框设置对工业软件 KingSCADA、Kepware 等的兼容性适配选项（仅企业版支持）； | `v3.3.3.0` 及更高版本 |
|      v1.0.2   | 支持 CP1252 字符编码；                                                                                 | `v3.2.3.0` 及更高版本 |
...
|    v1.0.0.0   | 发布初始版本，支持与 TDengine 数据库交互以读写数据，具体请参考“API 参考”一节                                | `v3.2.2.0` 及更高版本 |
