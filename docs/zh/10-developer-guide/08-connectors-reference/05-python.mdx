...
连接方式的详细介绍请参考：[连接方式](./index.md#连接方式)
...
|        2.7.12         | 1. 新增 varbinary 类型支持（STMT 暂不支持 varbinary）<br/> 2. query 性能提升（感谢贡献者[hadrianl](https://github.com/taosdata/taos-connector-python/pull/209)）         | `v3.1.1.2` 及更高版本 |
|         2.7.9         | 数据订阅支持获取消费进度和重置消费进度                                                                                                                                 | `v3.0.2.6` 及更高版本 |
|         2.7.8         | 新增 `execute_many`                                                                                                                                              | `v3.0.0.0` 及更高版本 |

WebSocket Connector 历史版本：

| WebSocket Connector 版本 | 主要变化                                                                              |   TDengine 版本    |
| ----------------------- | ------------------------------------------------------------------------------------ | ----------------- |
|          0.7.0           | 1. 支持 taosAdapter 高可用 <br/> 2. 支持设置应用程序名称                                 | - |
...
|          0.3.4           | 支持 VARBINARY 和 GEOMETRY 数据类型                                                    | `v3.3.0.0` 及更高版本 |
|          0.3.2           | 优化 WebSocket sql 查询和插入性能，修改 readme 和 文档，修复已知问题                        | `v3.2.3.0` 及更高版本 |
|          0.2.9           | 已知问题修复                                                                           | - |
|          0.2.5           | 1. 数据订阅支持获取消费进度和重置消费进度 <br/> 2. 支持 schemaless <br/> 3. 支持 STMT        | - |
|          0.2.4           | 数据订阅新增取消订阅方法                                                                 | `v3.0.5.0` 及更高版本 |
...
- **params**：
  ...
  - `adapter_ha`：是否启用 taosAdapter 高可用。启用后，连接器会向 taosAdapter 请求当前可用的实例列表，并将发现的节点加入地址池，用于负载均衡和故障转移。默认值为 `false`。
