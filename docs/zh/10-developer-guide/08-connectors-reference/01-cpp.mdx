...
- `TAOS *taos_connect(const char *ip, const char *user, const char *pass, const char *db, uint16_t port);`
  ...
  - **返回值**：返回数据库连接，返回值为空表示失败。应用程序需要保存返回的参数，以便后续使用。
    :::info
    同一进程可以根据不同的 host/port 连接多个 TDengine TSDB 集群。用户启用 TOTP 双因认证后，调用此函数返回的错误代码是 `TSDB_CODE_MND_WRONG_TOTP_CODE`，应用程序此时应提示用户输入 TOTP 验证码并通过 `taos_connect_totp` 创建连接。
    :::
...
- `void taos_set_option(OPTIONS *options, const char *key, const char *value)`
  ...
  - **支持的配置项**：
    ...
    | adapterHa         | WebSocket         | 是否启用 taosAdapter 高可用。启用后，连接器会向 taosAdapter 请求当前可用的实例列表，并将发现的节点加入地址池，用于负载均衡和故障转移。默认值为 false。v3.4.2.0 开始支持。 |
...
除了使用 SQL 方式或者使用参数绑定 API 写入数据外，还可以使用 Schemaless 的方式完成写入。Schemaless 可以免于预先创建超级表/数据子表的数据结构，而是可以直接写入数据，TDengine TSDB 系统会根据写入的数据内容自动创建和维护所需要的表结构。Schemaless 的使用方式详见 [Schemaless 写入](../04-schemaless.md) 章节，这里介绍与之配套使用的 C/C++ API。
...
- `tmq_conf_t *tmq_conf_new()`
  ...
  - **C WebSocket 连接器专有配置项**：
    ...
    | 配置项 | 类型 | 取值范围 | 说明 | 备注 |
    | ------ | ---- | -------- | ---- | ---- |
    | `ws.tls.mode` | integer | `0`：禁用 TLS 加密。若服务端启用 TLS，客户端会自动升级连接。<br/>`1`：启用 TLS 加密，但不验证服务端证书。<br/>`2`：启用 TLS 加密，验证服务端证书，但不验证主机名。<br/>`3`：启用 TLS 加密，验证服务端证书及主机名（服务端证书需包含 SAN，CN 会被忽略）。 | C WebSocket 连接支持的 TLS 加密模式 | 默认值为 `0`。v3.3.8.12 开始支持。 |
    | `ws.tls.version` | string | `TLSv1.2`、`TLSv1.3` | C WebSocket 连接支持的 TLS 协议版本列表，多个版本使用英文逗号分隔。 | 默认值为 `TLSv1.3`。v3.3.8.12 开始支持。 |
    | `ws.tls.ca` | string | CA 证书文件路径或 PEM 格式的证书内容 | C WebSocket 连接用于验证服务端证书的 CA 证书。该证书应为签发服务端证书的 CA 证书。 | v3.3.8.12 开始支持。 |
    | `ws.adapter.ha` | boolean | `true`、`false` | C WebSocket 连接是否启用 taosAdapter 高可用。启用后，连接器会向 taosAdapter 请求当前可用的实例列表，并将发现的节点加入地址池，用于负载均衡和故障转移。 | 默认值为 `false`。v3.4.2.0 开始支持。 |
