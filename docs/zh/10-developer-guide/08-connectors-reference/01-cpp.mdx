---
sidebar_label: C/C++
title: TDengine C/C++ Connector
description: C/C++ 原生与 WebSocket 连接器参考
...
TDengine 客户端驱动支持多种平台，具体支持的平台列表请参考：[支持的平台列表](./index.md#支持的平台)
...
| ------------------- | --------------------------------------------------------------- | ------------------ |
| `v3.3.6.0`             | 提供了 SQL 执行、参数绑定、无模式写入和数据订阅等全面功能支持。 | `v3.3.2.0` 及更高版本 |
...
- `TAOS *taos_connect(const char *ip, const char *user, const char *pass, const char *db, uint16_t port);`
  ...
  - **返回值**：返回数据库连接，返回值为空表示失败。应用程序需要保存返回的参数，以便后续使用。
    :::info
    同一进程可以根据不同的 host/port 连接多个 TDengine 集群。用户启用 TOTP 双因认证后，调用此函数返回的错误代码是 `TSDB_CODE_MND_WRONG_TOTP_CODE`，应用程序此时应提示用户输入 TOTP 验证码并通过 `taos_connect_totp` 创建连接。
    :::

- `TAOS *taos_connect_totp(const char *ip, const char *user, const char *pass, const char* totp, const char *db, uint16_t port);`
  - **接口说明**：功能同 `taos_connect`，但支持使用 [TOTP](https://www.rfc-editor.org/rfc/rfc6238) 进行双因认证。
  - **参数说明**：
    - ip：[入参] TDengine 集群中任一节点的 FQDN。
    - user：[入参] 用户名。
    - pass：[入参] 密码。
    - totp：[入参] TOTP 验证码，如果此参数的值为 NULL，则此函数的行为与 `taos_connect` 完全相同。
    - db：[入参] 数据库名字，如果用户没有提供，也可以正常连接，用户可以通过该连接创建新的数据库，如果用户提供了数据库名字，则说明该数据库用户已经创建好，缺省使用该数据库。
    - port：[入参] taosd 程序监听的端口。
  - **返回值**：返回数据库连接，返回值为空表示失败。应用程序需要保存返回的参数，以便后续使用。
  - **支持版本**：`v3.4.0.0` 及以上
...
- `TAOS *taos_connect_token(const char *ip, const char *token, const char *db, uint16_t port);`
  - **接口说明**：功能同 `taos_connect`，但使用令牌而不是用户名/密码进行身份验证。
  - **参数说明**：
    - ip：[入参] TDengine 集群中任一节点的 FQDN。
    - token：[入参] 令牌。
    - db：[入参] 数据库名字，如果用户没有提供，也可以正常连接，用户可以通过该连接创建新的数据库，如果用户提供了数据库名字，则说明该数据库用户已经创建好，缺省使用该数据库。
...
- `int taos_get_connection_info(TAOS *taos, TSDB_CONNECTION_INFO info, char* buffer, int* len);`
  ...
  - **参数说明**：
    - taos：[入参] 数据库连接。
    - info：[入参] 一个枚举值，用于指定要获取哪项连接信息。
    - buffer：[出参] 用于接收实际数据的缓冲区，内存由调用方负责管理。由于此函数是用于获取连接信息的通用函数，故 buffer 是一个字节数组，也就是说，即使实际数据是字符串，buffer 也不包含结尾 0。
    - len：[出入参] 作为入参，代表 buffer 的大小，作为出参，代表实际数据的大小。
  - **支持的信息项**:
  
    | 名称                       |  说明                                                            |
    |----------------------------|------------------------------------------------------------------|
    | TSDB_CONNECTION_INFO_USER  |  连接对应的用户名                                                |
    | TSDB_CONNECTION_INFO_TOKEN |  连接对应的令牌名称（如果连接使用令牌创建）                      |
...
- `void taos_set_option(OPTIONS *options, const char *key, const char *value)`
  ...
  - **接口说明**：向 OPTIONS 结构体中添加一个连接配置项（键值对）。该函数用于构建连接配置，配置项将在调用 `taos_connect_with()` 时生效。
  - **参数说明**：
    - `options`：[入参] 指向一个有效的 OPTIONS 结构体指针，用于存储连接配置项。
    - `key`：[入参] 配置项的键名（字符串），支持的键名见下表。
    - `value`：[入参] 配置项的值（字符串），与 `key` 对应的具体配置内容。
  - **返回值**：无。
  - **支持的配置项**：

    | 键名                | 适用连接方式        | 说明                                                                                    |
    |---------------------|-------------------|----------------------------------------------------------------------------------------|
    | ip                | Native/WebSocket  | TDengine 集群中任一节点的 FQDN 或 IP 地址                                                   |
    | user              | Native/WebSocket  | 用户名                                                                                   |
    | pass              | Native/WebSocket  | 密码                                                                                     |
    | db                | Native/WebSocket  | 数据库名称                                                                                |
    | port              | Native/WebSocket  | 服务端口号。Native 连接默认端口为 6030，WebSocket 连接默认端口为 6041。                         |
    | charset           | Native            | 字符集                                                                                   |
    | timezone          | Native/WebSocket  | 时区                                                                                     |
    | userIp            | Native/WebSocket  | 用户 IP 地址                                                                              |
    | userApp           | Native/WebSocket  | 用户 APP 名称                                                                             |
    | connectorInfo     | Native            | 连接器信息                                                                                |
    | adapterList       | WebSocket         | taosAdapter 地址列表，用于负载均衡和故障转移。多个地址使用英文逗号分隔，格式为 `host1:port1,host2:port2,...`。优先级高于 `ip` 参数。 |
    | adapterHa         | WebSocket         | 是否启用 taosAdapter 高可用。启用后，连接器会向 taosAdapter 请求当前可用的实例列表，并将发现的节点加入地址池，用于负载均衡和故障转移。默认值为 false。自 `v3.4.2.0` 起支持。 |
    | compression       | WebSocket         | 数据压缩开关。0：禁用，1：启用。默认值为 0。                                                    |
    | connRetries       | WebSocket         | 连接失败时的最大重试次数。默认值为 5。                                                         |
    | retryBackoffMs    | WebSocket         | 连接失败时的初始等待时间（毫秒）。此值会随着连续失败而指数增长，直到达到最大等待时间。默认值为 200。      |
    | retryBackoffMaxMs | WebSocket         | 连接失败时的最大等待时间（毫秒）。默认值为 2000。                                                |
    | token             | WebSocket         | 云服务认证 token                                                                           |
    | wsTlsMode         | WebSocket         | TLS 加密模式：<br/> - 0：禁用 TLS 加密。若服务端启用 TLS，客户端会自动升级连接。<br/> - 1：启用 TLS 加密，但不验证服务端证书。<br/> - 2：启用 TLS 加密，验证服务端证书，但不验证主机名。<br/> - 3：启用 TLS 加密，验证服务端证书及主机名（服务端证书需包含 SAN，CN 会被忽略）。<br/> 默认值为 0。 |
    | wsTlsVersion      | WebSocket         | TLS 协议版本列表，多个版本使用英文逗号分隔。可选值：TLSv1.2、TLSv1.3。默认值为 TLSv1.3。            |
    | wsTlsCa           | WebSocket         | 客户端用于验证服务端证书的 CA 证书文件路径或 PEM 格式的证书内容。该证书应为签发服务端证书的 CA 证书。    |

  - **注意事项**：
    - OPTIONS 结构体最多支持 256 个配置项
    - 配置项的键名区分大小写
    - 相同键名多次设置时，后面的值会覆盖前面的值
    - WebSocket 连接专有配置项在 Native 连接方式下会被忽略，反之亦然
  - **支持版本**：`v3.3.8.12` 及以上

- `TAOS *taos_connect_with(const OPTIONS *options)`

  - **接口说明**：使用 OPTIONS 结构体中的连接配置项创建数据库连接，并初始化连接上下文。
  - **参数说明**：
    - `options`：[入参] 指向一个有效的 OPTIONS 结构体指针，包含通过 `taos_set_option()` 添加的连接配置项。如果为 NULL 或配置项为空，将使用默认连接参数。
  - **返回值**：返回数据库连接，返回值为空表示失败。应用程序需要保存返回的参数，以便后续使用。
  - **支持版本**：`v3.3.8.12` 及以上
...
从 `v3.3.5.0` 开始，TDengine 大幅简化了参数绑定（STMT2）接口。这样在通过参数绑定接口写入数据时，可以避免了 SQL 语法解析的资源消耗，通过批量绑定的方式，在绝大多数情况下显著提升写入性能。此时的典型操作步骤如下：
...
从 `v2.1.1.0` / `v2.1.2.0` 开始，TDengine 大幅改进了参数绑定接口对数据写入（`INSERT`）场景的支持。这样在通过参数绑定接口写入数据时，就避免了 SQL 语法解析的资源消耗，从而在绝大多数情况下显著提升写入性能。此时的典型操作步骤如下：
...
除了使用 SQL 方式或者使用参数绑定 API 写入数据外，还可以使用 Schemaless 的方式完成写入。Schemaless 可以免于预先创建超级表/数据子表的数据结构，而是可以直接写入数据，TDengine 系统会根据写入的数据内容自动创建和维护所需要的表结构。Schemaless 的使用方式详见 [Schemaless 写入](../04-schemaless.md) 章节，这里介绍与之配套使用的 C/C++ API。
...
主题概念见 [数据订阅](../../07-data-subscription/index.md) 与 [主题语法](../../07-data-subscription/01-topic.md)；多语言用法见 [数据订阅编程接口](../07-subscription-api.md)。C/C++ 可通过 `tmq_get_res_type` 区分数据与元数据消息。
...
- `tmq_conf_t *tmq_conf_new()`
  ...
  - **C WebSocket 连接器专有配置项**：

    | 配置项 | 类型 | 取值范围 | 说明 | 备注 |
    | ------ | ---- | -------- | ---- | ---- |
    | `ws.tls.mode` | integer | `0`：禁用 TLS 加密。若服务端启用 TLS，客户端会自动升级连接。<br/>`1`：启用 TLS 加密，但不验证服务端证书。<br/>`2`：启用 TLS 加密，验证服务端证书，但不验证主机名。<br/>`3`：启用 TLS 加密，验证服务端证书及主机名（服务端证书需包含 SAN，CN 会被忽略）。 | C WebSocket 连接支持的 TLS 加密模式 | 默认值为 `0`。自 `v3.3.8.12` 起支持。 |
    | `ws.tls.version` | string | `TLSv1.2`、`TLSv1.3` | C WebSocket 连接支持的 TLS 协议版本列表，多个版本使用英文逗号分隔。 | 默认值为 `TLSv1.3`。自 `v3.3.8.12` 起支持。 |
    | `ws.tls.ca` | string | CA 证书文件路径或 PEM 格式的证书内容 | C WebSocket 连接用于验证服务端证书的 CA 证书。该证书应为签发服务端证书的 CA 证书。 | 自 `v3.3.8.12` 起支持。 |
    | `ws.adapter.ha` | boolean | `true`、`false` | C WebSocket 连接是否启用 taosAdapter 高可用。启用后，连接器会向 taosAdapter 请求当前可用的实例列表，并将发现的节点加入地址池，用于负载均衡和故障转移。 | 默认值为 `false`。自 `v3.4.2.0` 起支持。 |
