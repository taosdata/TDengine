...
- **addresses**：地址列表，支持一个或多个 `host:port`，多个地址之间使用英文逗号分隔（例如 `localhost:6041,localhost:6042`）。`@tdengine/websocket` 同时支持 IPv4 和 IPv6 两种地址格式。对于 IPv6 地址，必须使用中括号包裹（例如 `[::1]` 或 `[2001:db8:1234:5678::1]`），以避免与端口号解析冲突。
- **database**：数据库名称。
- **params**：
