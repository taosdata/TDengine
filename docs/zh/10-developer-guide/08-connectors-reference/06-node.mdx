...
title: TDengine TSDB Node.js Connector
---
...
| TDengine TSDB DataType | Node.js DataType|
|-------------------|-------------|
| TIMESTAMP         | bigint      |
| TINYINT           | number      |
| SMALLINT          | number      |
...
1. "Unable to establish connection" 或 "Unable to resolve FQDN"
   ...
   **原因**：一般都是因为配置 FQDN 不正确。可以参考[如何彻底搞懂 TDengine TSDB 的 FQDN](https://www.taosdata.com/blog/2021/07/29/2741.html) 。

## API 参考

Node.js 连接器（`@tdengine/websocket`）通过 taosAdapter 提供的 WebSocket 接口连接 TDengine TSDB 实例。
...
- **params**：
  ...
  - `adapter_ha`：是否启用 `taosAdapter` 高可用。`true` 为启用，`false` 为不启用，默认值为 `false`。启用后，连接成功即可自动获取集群中所有 `taosAdapter` 端点并加入地址池用于负载均衡和故障转移，需要 TDengine TSDB 版本大于等于 `3.4.2.0`。
