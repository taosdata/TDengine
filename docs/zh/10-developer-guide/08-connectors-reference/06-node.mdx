...
| ------------------| ----------------------| ----------------|
|        3.5.0       | 支持 taosAdapter HA（高可用） | - |
|        3.4.0       | 1. 支持 BLOB 数据类型 <br/> 2. 参数绑定支持 DECIMAL 数据类型 <br/> 3. 支持上报用户 APP 名称和 IP 地址 | - |
|        3.3.0       | 支持负载均衡和故障转移 | - |
|        3.2.3       | 1. 支持令牌身份认证 <br/> 2. 支持上报连接器版本信息 | - |
|        3.2.2       | 修复 Windows 系统上的时区处理问题 | - |
|        3.2.1       | 修复 SQL 查询结果排序问题 | - |
|        3.2.0       | 优化 stmt 参数绑定，提升写入效率 | - |
|        3.1.9       | 修复 WebSocket 连接的时区设置问题 | - |
...
调用连接器 API 报错后，可通过 `try...catch` 获取错误信息和错误码。  
...
| 示例程序                                                                                                                                    | 示例程序描述                           |
| ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| [sql_example](https://github.com/taosdata/TDengine/tree/main/docs/examples/node/websocketexample/sql_example.js)                           | 基本使用示例，如建立连接、执行 SQL 等操作。                     |
...
Node.js 连接器（`@tdengine/websocket`）通过 taosAdapter 提供的 WebSocket 接口连接 TDengine 实例。
...
- **addresses**：地址列表，支持一个或多个 `host:port`，多个地址之间使用英文逗号分隔（例如 `node1:6041,node2:6041`）。`@tdengine/websocket` 同时支持 IPv4 和 IPv6 两种地址格式。对于 IPv6 地址，必须使用中括号包裹（例如 `[::1]` 或 `[2001:db8:1234:5678::1]`），以避免与端口号解析冲突。
- **database**：数据库名称。
- **params**：
  - `token`：用于 TDengine 云服务身份认证。
  - `bearer_token`：用于 TDengine 身份认证，优先级高于用户名/密码。
  - `adapter_ha`：是否启用 `taosAdapter` 高可用。`true` 为启用，`false` 为不启用，默认值为 `false`。启用后，连接成功即可自动获取集群中所有 `taosAdapter` 端点并加入地址池用于负载均衡和故障转移，需要 TDengine 版本大于等于 `v3.4.2.0`。
...
// 启用 taosAdapter HA，连接成功后自动发现集群中的其他 taosAdapter 端点：
...
- setBearerToken(token: string) 设置 TDengine 认证 token，认证优先级高于用户名密码。
- setUserApp(userApp: string) 设置用户上报的 APP 名称。
- setUserIp(userIp: string) 设置用户上报的 IP 地址。
...
主题概念见 [数据订阅](../../06-data-subscription/index.md) 与 [主题语法](../../06-data-subscription/01-topic.md)；多语言用法见 [数据订阅编程接口](../07-subscription-api.md)。
