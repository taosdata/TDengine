...
JDBC 驱动从 `3.6.0` 起，在 WebSocket 连接上提供高效写入特性。JDBC 驱动高效写入特性有如下特点：
...
3. **stt_trigger**：用于控制 TSDB 数据落盘策略以及触发后台合并文件的文件个数。默认值为 `2`。当 `stt_trigger = 1` 时，适用于表数量少但写入频率高的场景；当 `stt_trigger > 1` 时，更适合表数量多但写入频率低的场景。  
