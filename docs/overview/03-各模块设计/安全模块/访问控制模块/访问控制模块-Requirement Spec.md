...
| **SYSDBA** | 系统数据库管理员 (System Database Administrator) |
| **SYSSSO** | 系统安全管理员 (System Security Officer) |
| **SYSAUDITOR** | 系统审计员 (System Auditor) |
| **TOTP** | 基于时间的一次性密码 (Time-based One-Time Password) |
| **SMA** | 预聚合 (Small Materialized Aggregation) |
| **RSMA** | 降采样预聚合 (Rollup Small Materialized Aggregation) |
| **TSMA** | 窗口预聚合 (Time-window Stream Media Aggregation) |
| **DNODE** | 数据节点 (Data Node) |
| **MNODE** | 管理节点 (Management Node) |
| **QNODE** | 查询节点 (Query Node) |
| **SNODE** | 流计算节点 (Stream Node) |
| **BNODE** | 备份节点 (Backup Node) |
| **VGROUP** | 虚拟组 (Virtual Group) |
| **VNODE** | 虚拟节点 (Virtual Node) |
