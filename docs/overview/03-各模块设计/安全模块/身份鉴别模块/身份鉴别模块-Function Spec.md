...
| --- | --- | --- | --- | --- |
| 2025-10-21 | 2025-11-24 | 1.0 | 张博民 | 新建 |
| 2025-12-09 | 2025-12-09 | 1.1 | 程洪泽 | 修订及完善内容 |
...
创建用户时，需要为用户指定用户名、口令、资源限制等信息。调整后的语法如下（择一实现 TOTP 信息和功能限制中的 TOTP，具体见后文“TOTP 认证”一节）：
```plaintext {wrap}
...
4. TOTP: 是否启用 TOTP 认证。`0` 表示不启用，`1` 表示启用，默认为 `0`。此项可选，根据 TOTP 的实现方式决定，参见 TOTP 一节。
...
4. CALL_PER_SESSION ：单会话最大并发子调用数量。默认 `10`，最小 `1`，`-1` 代表 `UNLIMITED`。
5. VNODE_PER_CALL：单调用最大涉及 vnode 数量。默认 `10`，`-1` 代表 `UNLIMITED`。此项为可选功能，开发过程中根据进度和实现难度决定是否实现。
...
由于用户信息中增加了很多可选项，`ALTER USER` 语句也需做相应调整，新的语法如下（择一实现 `TOTP 信息`和功能限制中的 `TOTP`，具体见后文“TOTP 认证”一节）：：
```plaintext {wrap}
...
1. 可以在客户端连接注册允许禁止 HOST 和时间的变化通知
```cpp {wrap}
typedef enum {
...
用户配置被修改后，新配置项生效时机如下：
1. **立即生效（立即中断不符合限制条件的连接）：**锁定、允许和禁止的 HOST、允许和禁止的时间段、CONNECT_IDLE_TIME、CONNECT_TIME
2. **下次使用相关功能或进行相关检查前生效：**功能限制项、SESSION_PER_USER、CALL_PER_SESSION、VNODE_PER_CALL、FAILED_LOGIN_ATTEMPTS、PASSWORD_LIFE_TIME、PASSWORD_LOCK_TIME、PASSWORD_GRACE_TIME、INACTIVE_ACCOUNT_TIME
3. **PASSWORD_REUSE_TIME 和 PASSWORD_REUSE_MAX：**在下次修改密码前生效，如存在超出限制范围的历史密码，则清除
4. **ALLOW_TOKEN_NUM：**在下次创建 TOKEN 前生效，不影响已经创建的 TOKEN 的有效性，但在 TOKEN 数量小于限额前，不允许创建新的 TOKEN

### 4.4 TOTP 认证

启用 TOTP 认证需要生成 TOTP 密钥，此密钥的生成方式有两种，实现时根据难度选择：
1. 生成方式 1：
  ```sql {wrap}
  CREATE USER <username> TOTP { 0 | 1 };
  UPDATE TOTP FOR USER <username>;
  ```

   - 用户在功能限制中启用 TOTP 认证后，执行上面的命令时，如果尚未创建 TOTP 密钥则创建密钥，如已创建则更新该密钥，命令执行完毕后返回新创建或更新后的密钥。
   - 如果用户启用了 TOTP 认证且已经创建 TOTP 密钥，则该用户必须同时使用密码和 TOTP 验证码登录。
   - 如果用户启用了 TOTP 认证但尚未创建 TOTP 密钥，则该用户可通过密码登录，但登录后仅能执行上述命令创建密钥。但是，如果用户密码处于过期状态，则用户必须先修改密码，然后创建 TOTP 密钥。
   - 服务端使用安全的随机数生成器生成密钥。
   - 这种方式的难点在于 UPDATE 语句目前在 taosc 中无法返回信息，代码修改量较大；另外，创建用户时也无法直接生成 TOTP 密钥。
1. 生成方式 2：
  ```sql {wrap}
  CREATE USER <username> TOTPSEED <totpseed>;
  ALTER USER <username> TOTPSEED <totpseed>;
  SELECT GENERATE_TOTP_SECRET( <totpseed> );
  ```

   - totpseed 是用户可以随意输入的字符，系统根据它的值生成一个 TOTP 密钥。
   - 增加一个 `GENERATE_TOTP_SECRET` 函数，对确定的 totpseed 返回确定的 TOTP 密钥，用户可以通过执行此函数查看实际的密钥。
   - 由于用户使用的 totpseed 可能过于简单，实现中按口令信息的标准强制要求其具有一定的复杂性。
   - 这种方式实现简单，但生成和查看 TOTP 密钥分成了两步。

### 4.5 TOKEN 认证

令牌信息存储在系统表 ins_tokens 表中，包括：令牌对应的 client_id（此 id 可以唯一标识一个令牌）、令牌对应的用户名、令牌（自动生成）、创建时间、有效期、授权码、禁用状态以及其他可扩展的字符串等。
使用下列命令管理 TOKEN，但使用 TOKEN 登录时，即使用户本身有权限修改 TOKEN 的有效期，其在该会话中也无权修改。
```sql {wrap}
CREATE token 
        FROM USER <username> 
        auth_code <code> 
...
ALTER token SET enable=1 WHERE token_id = <token_id>
```
...
2. 增加 `taos_connect_totp`，用于 TOTP 认证，此 API 兼容 `taos_connect`，即在调用方未提供 TOTP 验证码时，其行为和 `taos_connect` 相同。
...
![](./images/wb_OP98wSZGFhDuF2bcB5yc9UnAnPd.png)

##### 4.6.2.2 服务端 TOKEN 登录流程

![](./images/wb_RHpMwsmMhhGlAVbQuzocRA1vnGe.png)

##### 4.6.2.3 总体登录流程

![](./images/wb_Fgn9wSvJ0hgHN4bZas1cdGWSnch.png)
...
2. 除通过使用生成 TOTP 密钥时的 totpseed 调用 `GENERATE_TOTP_SECRET` 查看密钥外，用户的不能直接查看已经生成的 TOTP 密钥。
3. 使用 TOKEN 登录时，即使 TOKEN 对应的用户有权限修改 TOKEN 的有效期，也无权修改 TOKEN 的有效期。
...
1. JIRA [TS-7231](https://jira.taosdata.com:18080/browse/TS-7231)
2. [身份鉴别 RS](https://taosdata.feishu.cn/wiki/GZNPwH62SiiRtQkQHTvcM73YnDh)
3. [IP 白名单用户手册](https://taosdata.feishu.cn/wiki/TEQlwg19hizT7ukPcWscRRYunub)
4. [IP 白名单-Function Spec](https://taosdata.feishu.cn/wiki/WwJiwYgrwisPTxkm2NXc73VNnMd)
