The reqId is very like TraceID in distributed tracing systems. In a distributed system, a request may need to pass through multiple services or modules to be completed. The reqId is used to identify and associate all related operations of this request, allowing us to track and understand the complete path of the request.
...
If the user does not set a reqId, the client library will also generate one randomly internally, but it is still recommended for the user to set it, as it can better associate with the user's request.
