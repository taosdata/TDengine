...
After an error is reported, the error message and error code can be obtained through try catch.

Error Description: The Node.js client library error code is between 100 and 110, and code of other TDengine function modules.

For Node.js client library error codes, please refer to.
...
| 102     | received data, but no find callback                             | Client waiting timeout, please check network and taos adapter status.                                     |
| 103     | invalid message type                                            | Please check if the client version and server version match.               |
| 104     | connection not created                                          | Connection creation failed. Please check the network and taos adapter status.                           |
| 105     | websocket request timeout                                       | Increase the execution time by adding the messageWaitTimeout parameter, or check the connection to the taos adapter.|
...
| 108     | connection already closed                                       | The connection has been closed, check the connection status, or recreate the connection to execute the relevant instructions.      |
| 109     | parse fetch block data fail                                     | Please check if the client version and server version match.                     |
| 110     | websocket connector arrived limited                             | Please check if the connection has been closed after use                                                 |

## TDengine DataType vs. Node.js DataType

| TDengine DataType | Node.js Type|
|-------------------|-------------|
| TIMESTAMP         | bigint      |
| TINYINT           | number      |
| SMALLINT          | number      |
...
- Create an installation test folder such as `~/tdengine-test`. Download the [nodejsChecker.js source code](https://github.com/taosdata/TDengine/tree/3.0/docs/examples/node/websocketexample/nodejsChecker.js) to your local machine.
...
**Note**：
- After using the Node.js client library, it is necessary to call taos.destroy() Release connector resources
```javascript
...
```javascript
await wsSchemaless.schemalessInsert([influxdbData], SchemalessProto.InfluxDBLineProtocol, Precision.NANO_SECONDS, 0, 1);
```
...
- taos.TMQConstants.AUTO_OFFSET_RESET: To determine whether the consumption location is based on the latest or earliest data.
...
- After using the connect, you need to call taos.connectorDestroy(); Release connector resources.

## Frequently Asked Questions

1. Using Node.js client library requires starting taosadapter.

   ```bash
   sudo systemctl start taosadapter
   ```

2. "Unable to establish connection" or "Unable to resolve FQDN" 

   **Solution**: Usually, the root cause is an incorrect FQDN configuration. You can refer to this section in the [FAQ](https://docs.tdengine.com/2.4/train-faq/faq/#2-how-to-handle-unable-to-establish-connection) to troubleshoot.
