...
For errors in other TDengine modules, please refer to [Error Codes](../09-error-codes.md).
...
The GEOMETRY type is binary data in little endian byte order, conforming to the WKB standard. For more details, please refer to [Data Types](../../05-tdengine-sql/01-datatype.md)
...
For other parameters, please refer to: [Consumer Parameter List](../07-subscription-api.md), note that starting from version 3.2.0.0 of the TDengine server, the default value of `auto.offset.reset` in message subscriptions has changed.
