...
The RESTful interface does not depend on any TDengine libraries, so the client does not need to install any TDengine libraries as long as the client's development language supports the HTTP protocol. TDengine's RESTful API is provided by [taosAdapter](../../12-operations-and-tooling/03-components/03-taosadapter.md), and `taosAdapter` must be running before using the RESTful API.
...
For error codes and descriptions, please refer to [Error Codes](../09-error-codes.md)
...
TOKEN is created using the SQL statement `CREATE TOKEN`, detailed usage can be found in the [Token Management](../../05-tdengine-sql/07-user-and-privilege/01-user.md#token-management) section.
