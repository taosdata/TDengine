...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------ |
| 3.9.0                   | Optimize parameter binding caching and serialization to improve write performance for TDengine TSDB Enterprise 3.4.1.16 and later.                                                                                                                                                                                                                                                                                                                                       | -                  |
| 3.8.4                   | 1. Support HA endpoint discovery for taosadapter. <br/> 2. Shuffle the endpoints before connecting. <br/> 3. Initialize the WebSocket catalog from connection properties.                                                                                                                                                                                                                                                                                                                                             | -                  |
...
For error code information please refer to [Error Codes](../09-error-codes.md)
...
GEOMETRY type is binary data in little endian byte order, complying with the WKB standard. For more details, please refer to [Data Types](../../05-tdengine-sql/01-datatype.md)  
...
For other issues, please refer to [FAQ](../../16-faq/01-faq.md)
...
- TSDBDriver.PROPERTY_KEY_BEARER_TOKEN [`bearerToken`]: bearerToken information that takes effect only when using the JDBC WebSocket connection. Its authentication priority is higher than that of username and password. For details, see [Client and Connector Security](../../11-security-guide/05-client-connector-security.md#3-token-authentication).
...
- TSDBDriver.PROPERTY_KEY_USE_SSL [`useSSL`]: Whether to use SSL in the connection. Effective only in WebSocket connections. For details, see [Client and Connector Security](../../11-security-guide/05-client-connector-security.md#2-ssltls-configuration).
...
For other parameters, please refer to: [Consumer parameter list](../07-subscription-api.md), note that the default value of auto.offset.reset in message subscription has changed starting from TDengine server version 3.2.0.0.
