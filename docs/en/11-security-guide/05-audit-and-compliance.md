...
TDengine records user operations and sends them to `taosKeeper`, which stores the audit logs in a TDengine cluster. Starting with `v3.4.1.0`, audit records can instead be stored in the local cluster by enabling `auditSaveInSelf`. For authoritative parameter details, see [taosd](../12-operations-and-tooling/03-components/01-taosd.md).

In this document, “compliance” means supporting internal audits and operational traceability with configurable audit trails, and applying fixes published in [Security Advisories](./07-security-advisories.md). It does not claim compliance with a specific external certification or regulation.
...
For types, ranges, and dynamic-modification support, see the [taosd reference](../12-operations-and-tooling/03-components/01-taosd.md).
