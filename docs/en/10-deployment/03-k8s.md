...
Create the `tdengine.yaml` file and set `replicas` to 3. In this example, the region is set to Asia/Shanghai and 10 GB of [standard](https://kubernetes.io/docs/concepts/storage/storage-classes/) storage are allocated per node. You can change the configuration based on your environment and business requirements. 
