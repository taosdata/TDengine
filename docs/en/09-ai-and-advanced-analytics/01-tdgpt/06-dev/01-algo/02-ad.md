...
For third-party detectors such as PyOD, convert `self.input_data_lists` to a
feature matrix in the shape `(n_samples, n_features)`. PyOD uses `0` for normal
points and `1` for outliers, so adapters must map PyOD labels to TDgpt result
codes: normal points use `self.valid_code`, and outliers use `-1`.
