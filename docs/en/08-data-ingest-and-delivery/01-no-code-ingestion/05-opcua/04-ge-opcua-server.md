...
This page describes how to configure the OPC UA connection in taosExplorer when the data source is a **GE CSS OPC UA Server**.
...
- Configured via **Secure Channel Certificate** + **Certificate's Private Key** in taosExplorer.
...
## Generate the taosX client certificate
...
## Configure the OPC UA Connection in taosExplorer

In taosExplorer, go to **Data In → Create New Data In Task**, choose **OPC UA**, and apply the following settings.

### Connection Configuration

| taosExplorer field | Value |
| --- | --- |
...
| User auth enabled on server          | taosExplorer setting                                                                                                                |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
...
## Trust and Map the Client Certificate

The first **Check Connection** attempt in taosExplorer will most likely fail. This is expected — the GE OPC UA Server has not yet trusted or mapped the new taosX client certificate.
...
After the certificate is associated with the user, return to taosExplorer and run **Check Connection** again.
...
| Authentication-related error | The taosX Authentication tab does not match the user-auth mode actually enabled on the server. | Confirm with the server administrator whether the server enabled Anonymous / Username / OPC UA user certificate, then switch to the matching tab in taosExplorer and provide the right credentials. |
...
- After the server-side certificate association is complete, rerun **Check Connection** in taosExplorer.
