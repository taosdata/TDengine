...
This section describes how to create data migration tasks through the taosExplorer interface, migrating data from Kafka to the current TDengine cluster.
...
![Configure connection information](../../assets/no-code-ingestion-kafka-01.png)
...
![Configure plain authentication](../../assets/no-code-ingestion-kafka-02.png)

#### SCRAM (SCRAM-SHA-256) Authentication

Select the `SCRAM-SHA-256` authentication mechanism and enter the username and password:

![Configure SCRAM authentication](../../assets/no-code-ingestion-kafka-03.png)
...
![Configure GSSAPI authentication](../../assets/no-code-ingestion-kafka-04.png)
...
![Configure SSL certificate](../../assets/no-code-ingestion-kafka-05.png)
...
![Configure collection settings](../../assets/no-code-ingestion-kafka-06.png)

### Configure Payload Parsing

Fill in the configuration parameters related to Payload parsing in the **Payload Parsing** area.

Kafka supports multiple parsing rule blocks for different payload structures. Each block has a match expression that uses the same syntax as a filter. The default expression is `true`, which matches every message. Extraction, splitting, filtering, and mapping rules work the same way inside each block. You can add or delete rule blocks, subject to the table-column limit.
