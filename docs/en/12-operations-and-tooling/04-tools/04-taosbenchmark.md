...
taosBenchmark is the default installation component in the TDengine server and client installation package. It can be used after installation, refer to [TDengine Installation](../../04-quick-start/index.md)
...
Running in JSON configuration file mode provides a rich set of configuration options, and most command-line parameters can be configured in the configuration file.
...
- **-I/--interface \<insertMode>:**
  Insert mode, options include taosc, stmt, stmt2, sml, corresponding to normal write, parameter binding interface (v1 version) write, parameter binding interface (v2 version) write, schemaless interface write. Default is taosc.
...
- **-N/--normal-table:** A toggle parameter, specifies to only create basic tables, not supertables. Default is false. Only available when the insertion mode is taosc, stmt, or stmt2.
...
- **-g/--debug:** A toggle parameter, enables debug information output. Default is off.

- **-G/--performance:** A toggle parameter, enables performance statistics output. Default is off.

- **-H/--angle-step \<NUMBER>:** Angle step factor, in milliseconds, default is 1.

- **-Q/--nodrop:** A toggle parameter, does not delete existing databases. Default is to delete.

- **-X/--dsn \<DSN>:** DSN (Data Source Name) for connecting to cloud services, used to connect to TDengine Cloud.

- **-W/--cloud_dsn \<DSN>:** Alias for `-X/--dsn`, DSN for connecting to cloud services.

- **-Z/--driver \<DRIVER>:** Specify the connection driver, options are "Native" (native connection) or "WebSocket" (WebSocket connection), default is Native.
...
- **auto_create_table**: Effective only when insert_mode is taosc, stmt, stmt2 and child_table_exists is "no", "yes" means taosBenchmark will automatically create non-existent tables during data insertion; "no" means all tables are created in advance before insertion.
...
- **insert_mode**: Insertion mode, options include taosc, stmt, stmt2, sml corresponding to normal write, parameter binding interface (v1 version) write, parameter binding interface (v2 version) write, schemaless interface write. Default is taosc.
...
- **line_protocol:** Use line protocol to insert data, effective only when insert_mode is sml, options include line, telnet, json.
...
- **partial_col_num:** If this value is a positive number n, then only the first n columns are written to, effective only when insert_mode is taosc, if n is 0 then all columns are written to.
...
For the following parameters, see the description of [Subscription](../../07-data-subscription/index.md):
