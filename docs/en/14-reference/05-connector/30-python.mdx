...
The connector requires Python 3.0 or above, and the installation command is as follows:
``` bash
...
For a detailed introduction of the connection method, please refer to: [Connection Method](../../../development/connect/# Connection Method)
...
In addition to encapsulating native and REST interfaces, taopsy also provides compliance with the Python Data Access Specification (PEP 249)(https://peps.python.org/pep-0249/) The programming interface.
 This makes it easy to integrate taopsy with many third-party tools, such as QLAlchemy(https://www.sqlalchemy.org/) And [pandas](https://pandas.pydata.org/).
...
Supports Python 3.0 and above versions.
...
| :-------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------- |
|2.7.16 | Add subscription configuration (session. timeout. ms, Max. roll. interval. ms)                            | 3.1.1.2 and higher versions|
|2.7.15 | Added support for VARBINRY and GEOMETRY types                                                             | 3.1.1.2 and higher versions|
|2.7.14 | Fix Known Issues                                                                                          | 3.1.1.2 Higher Version|
|2.7.13 | Added tmq synchronous submission offset interface                                                         | 3.1.1.2 and higher versions|
|2.7.12 | 1. Added support for varbinary type (STMT currently does not support varbinary)<br/> 2 Query performance improvement (thanks to contributor [hadrianl])(https://github.com/taosdata/taos-connector-python/pull/209)) | 3.1.1.2 and higher versions|
|2.7.9 | Data subscription supports obtaining and resetting consumption progress                                    | 3.0.2.6 and higher versions|
|2.7.8 | Added 'executioner_many'                                                                                   | 3.0.0.0 and higher versions|

WebSocket Connector Historical Versions:

|WebSocket Connector Version | Major Changes                                                                                    | TDengine Version|
| :----------------------- | -------------------------------------------------------------------------------------------------- | :----------------- |
|0.3.5 | Added support for VARBINARY and GEOMETRY types, fixed known issues                                                     | 3.3.0.0 and higher versions|
|0.3.2 | Optimize WebSocket SQL query and insertion performance, modify readme and documentation, fix known issues              | 3.2.3.0 and higher versions|
|0.2.9 | Known issue fixes                                                                                                      | 3.0.5.0 and higher versions|
|0.2.5 | 1. Data subscription supports obtaining and resetting consumption progress<br/>2 Support schemaless<br/>3 Support STMT | 3.0.5.0 and higher versions|
|0.2.4 | Data Subscription Addition/Unsubscribe Method                                                                          | 3.0.5.0 and higher versions|
