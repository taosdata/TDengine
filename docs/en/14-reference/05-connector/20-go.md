...
- `timezone` specifies the timezone used for the connection. Both SQL parsing and query results will be converted according to this timezone. Only IANA timezone formats are supported, and special characters need to be encoded. Taking the Paris timezone (`Europe/Paris`) as an example: `timezone=Europe%2FParis`.
...
- `func (conn *Connector) SetTimezone(timezone string) error`
  ...
  - **Parameter Description**:
    - `timezone`: Timezone string, using the IANA timezone format, for example: `Europe/Paris`.
