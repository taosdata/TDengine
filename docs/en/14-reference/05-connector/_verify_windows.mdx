In the CMD, navigate to the `C:\TDengine` directory and execute `taos.exe` to connect to the TDengine service, entering the TDengine CLI interface. Here is an example:
