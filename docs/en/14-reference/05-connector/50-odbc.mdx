...
TDengine ODBC is an ODBC driver implemented for TDengine, allowing applications on Windows systems (such as [PowerBI](https://powerbi.microsoft.com/zh-cn/)) to access tables/views in the TDengine database through the ODBC standard interface, whether they are local, remote, or cloud-based.
...
4. Click Finish to enter the TDengine ODBC data source configuration page and fill in the following necessary information.
    ...
    4.3 **URL**: (mandatory), ODBC data source URL, for example: `http://localhost:6041`, for cloud service the URL example is: `https://gw.cloud.taosdata.com?token=your_token`.
