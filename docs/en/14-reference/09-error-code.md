...
| 0x800026B4 | Function does not support DISTINCT                                                                     | DISTINCT used with an unsupported function, with `*`, or with SESSION/STATE/EVENT windows | Only COUNT/SUM/AVG/MIN/MAX support DISTINCT; cannot be used with SESSION/STATE/EVENT windows |
| 0x800026B5 | Window function is not allowed in this clause                                                          | Window function appears in an illegal clause                               | Move the window function to SELECT or ORDER BY                |
| 0x800026B6 | Window function requires ORDER BY                                                                      | Window function uses a form that requires ordering but no ORDER BY is present | Add ORDER BY to the window specification                      |
| 0x800026B7 | Referenced window name is not defined                                                                  | The named window referenced by OVER does not exist                         | Define the window name in the current query block             |
| 0x800026B8 | Duplicate window name                                                                                  | More than one WINDOW item uses the same name                               | Rename one of the WINDOW definitions                          |
...
| 0x80002816 | Invalid timeline in function                 | The function input timeline is invalid. For functions that depend on timeline order, missing valid timeline input, NULL timestamps, out-of-order timestamps, or duplicate timestamps can make the calculation meaningless. | Check the SQL input and ensure that the function uses a valid timeline that meets the function requirements. Sort by the time column or filter invalid timestamp data when necessary. |
