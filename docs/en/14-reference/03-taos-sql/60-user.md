...
- `SESSION_PER_USER` The maximum allowed simultaneous connections of the user. The default value is `-1` when `enableAdvancedSecurity` is `1` and `UNLIMITED` otherwise, with a minimal of `1`, set to `UNLIMITED` disables the restriction. Support in Enterprise Edition v3.4.0.0 and above.
