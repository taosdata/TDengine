...
- `HOST` and `NOT_ALLOW_HOST` IP address whitelist and blacklist. Entries can be a single IP address, such as `192.168.1.1`, or a subnet range in [CIDR](https://www.rfc-editor.org/rfc/rfc4632) format, such as `192.168.1.1/24`. Support in Enterprise Edition v3.4.0.0 and above.
  - If neither `HOST` nor `NOT_ALLOW_HOST` is set, the user is allowed to log in from any address. **Note:** For security reasons, if both `HOST` and `NOT_ALLOW_HOST` are omitted during user creation, the system automatically adds `127.0.0.1` and `::1` to the `HOST` list. This means the user can only log in from localhost. Therefore, the scenario described in this item will only occur if you use `ALTER USER` to remove all `HOST` and `NOT_ALLOW_HOST` entries.
