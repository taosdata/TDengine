...
- When revoking privileges, the `priv_level` is matched exactly; cascading revocation is not supported. For example, `REVOKE SELECT ON d0.* FROM u1` only removes the specific privilege for `d0.*`, and does not remove the privileges explicitly granted on individual objects, such as `d0.t1`.
