...
- null_ratio (float): Ratio of NULL values to generate, range [0.0, 1.0], default 0.0. NULL represents an empty value that replaces the column's latest value with null.
- none_ratio (float): Ratio of NONE values to generate, range [0.0, 1.0], default 0.0. NONE represents missing data that does not overwrite the column's latest value (preserves previous value). The sum of null_ratio and none_ratio must not exceed 1.0.
