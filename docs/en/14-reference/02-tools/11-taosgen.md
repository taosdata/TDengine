...
This example demonstrates how to use the taosgen tool to simulate 10,000 smart meters. Each smart meter collects three physical quantities: current, voltage, and phase. A record is generated every 5 minutes, with current data generated randomly and voltage simulated using a sine wave. The generated data is written to the meters super table in the tsbench database of TDengine TSDB via WebSocket.

Configuration details:
- Global configuration (global)
  - Connection info (connection_info): Defines database connection parameters, including connection pool settings (maximum 10 connections, minimum 2 connections, connection timeout 1000ms).
  - Data format info (data_format): Sets the data format to SQL.
  - Data channel info (data_channel): Uses the WebSocket interface for communication.
  - Database info (database_info): Defines the target tsbench database, sets time precision to milliseconds, and 4 vgroups.
...
- Job dependencies
  - Uses the needs parameter to indicate dependencies, constructing an execution flow of: create database → create super table → create child table → insert data, forming a directed acyclic graph (DAG).

- Data generation configuration
  - Subtable names: Dynamically creates 10,000 subtable names using the generator, with names ranging from d0 to d9999.
  - Tag data: Uses the generator to create random groupid and location tag values for each subtable according to the schema.
  ...
  - Timestamp strategy: Configures a timestamp generation strategy, starting from the specified timestamp 1700000000000 (2023-11-14 22:13:20 UTC), incrementing by 5 minutes each step.
...
- Efficient writing
  ...
  - Interlace mode (interlace_mode): Enabled, configured to switch after generating 1 row of data for each subtable, simulating real data generation.
...
This configuration is specifically designed for performance benchmarking of the TDengine database. It is suitable for simulating scenarios where large-scale IoT devices (such as meters and sensors) continuously generate high-frequency data, and can be used for:
...
- Providing data support for capacity planning in fields such as industrial IoT.
...
This example demonstrates how to use the taosgen tool to simulate 10,000 smart meters, each collecting three physical quantities: current, voltage, and phase. Each meter generates a record every 5 minutes, with measurement data read from a CSV file and written to the meters super table in the tsbench database of TDengine TSDB via WebSocket.

Configuration details:
- Global configuration (global)
  - Connection info (connection_info): Defines database connection parameters, including connection pool settings (maximum 10 connections, minimum 2 connections, connection timeout 1000 ms).
  - Data format info (data_format): Sets the data format to SQL.
...
- Data generation configuration (different from the generator method, this configuration obtains data from CSV files)
    - Subtable names: Reads subtable names from the 2nd column (index starts from 0, so tbname_index: 2) of the CSV file ctb-tags.csv.
    - Tag data: Reads tag data from the same CSV file ctb-tags.csv, excluding the 2nd column (since the 2nd column is the subtable name, not a tag).
    - Time series data: Reads normal column data (current, voltage, phase) from the CSV file ctb-data.csv. The 0th column (tbname_index: 0) in this file is the subtable name, used to associate data with the subtable.
    - Timestamp strategy: Since the timestamp column in the CSV source data may not exist or may not match the required format, a generator strategy is used here, starting from the specified timestamp 1700000000000 (2023-11-14 22:13:20 UTC), incrementing by 5 minutes each step.
...
- Efficient writing
  ...
  - Insert threads: Uses a single thread for insertion (insert_threads: 1), suitable for scenarios with small data volumes or requiring sequential processing.
...
csv file format:
- The content format of the `ctb-tags.csv` file is:
...
- The content format of the `ctb-data.csv` file is:
...
This example demonstrates how to use the taosgen tool to simulate 10,000 smart meters, each collecting four physical quantities: current, voltage, phase, and state. Each meter generates a record every 5 minutes. The current data is generated randomly, the voltage is simulated using a sine wave, and the generated data is published via the MQTT protocol.
...
- Data generation (source)
  ...
  - Measurement data: Normal column data (current, voltage, phase, state) is dynamically generated by the generator according to defined rules. The state column is randomly selected from three predefined enum values.
  - Timestamp strategy: Starts from the specified timestamp 1700000000000 (2023-11-14 22:13:20 UTC), incrementing by 5 minutes each step.
...
- MQTT target configuration (target)
  ...
  - Topic configuration (topic): Uses dynamic topic factory/`{table}`/`{state}`/data, where:
    - The `{table}` placeholder will be replaced with the actual generated subtable name.
    - The `{state}` placeholder will be replaced with the generated state column value, enabling publishing to different topics by device state.
...
- Control strategy (control)
  ...
  - Data generation strategy (data_generation):
    ...
    - Rows per table (per_table_rows): Generates 100 rows of data for each subtable.
