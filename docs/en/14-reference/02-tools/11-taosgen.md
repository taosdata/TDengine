...
Tip: If no parameters are specified when running taosgen, the default behavior is:
- Create TDengine database tsbench.
- Create super table meters.
- Create 10,000 child tables d0 ~ d9999.
- Batch write 10,000 rows of data to each child table.
...
- schema: Describes configuration parameters for data definition and generation patterns.
  ...
  - columns (list): Pattern definition for normal columns in the table.
  - tags (list): Pattern definition for tag columns in the table.
...
- expression: Generated by expression (supports integer, float, double, string).
  - formula (string): Expression content, syntax uses Lua language, built-in variables:
