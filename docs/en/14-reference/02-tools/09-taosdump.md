...
Starting from v3.4.1.1, taosdump has been fully upgraded with higher performance, smaller backup data size, and more practical features.
...
The new version of taosdump supports the vast majority of old version command-line parameters (with a few exceptions). It also adds features such as resumable execution, Parquet export, restoring only specified databases, and optimized backup and restore performance for low-frequency multi-table scenarios.

## Getting the Tool

taosdump is included in both the server and client installation packages of TDengine TSDB v3.4.1.1 and later. For installation details, see [TDengine TSDB Installation](../../../get-started/).
...
| `-C, --checkpoint` | Flag. Enable resumable execution. taosdump always writes checkpoint files. When this option is enabled, rerunning the command skips completed items |
