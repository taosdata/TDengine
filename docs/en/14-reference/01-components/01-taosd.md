...
| enableGrantLegacySyntax | After 3.4.0.11    | Supported, effective immediately   | Whether to enable compatibility with 3.3.x.y version grant/revoke syntax; when enabled (1), the grant/revoke authorization syntax will adaptively expand privileges to database/table/view/index/tsma/rsma/topic/stream according to the privilege type and priv_level when no privilege object is explicitly specified; when disabled (0), it will only adaptively expand privileges to table/view; 0: disable, 1: enable; default value 0 |
