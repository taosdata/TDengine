...
**Return Value**: A null-terminated static string pointer. The string lifetime must cover the whole plugin lifetime. The plugin must not allocate this return value on the heap and require taosX to release it.
...
`uint32_t parser_abi_version(void)`: Returns the parser plugin ABI version. taosX currently supports only parser plugins that return `2`.

`ParserMutateStatus`: The return status of `parser_mutate`.

- `PARSER_MUTATE_OK`: Parsing succeeded, and the output has been written to `out_ptr`.
- `PARSER_MUTATE_BUFFER_TOO_SMALL`: The output buffer is too small. The caller should resize the buffer to at least `required_cap` and retry.
- `PARSER_MUTATE_ERROR`: Parsing failed or the arguments are invalid.
...
`uint8_t* out_ptr`: Pointer to the output buffer allocated and owned by taosX. The plugin may only write the result into this buffer, and must not return, cache, or free output memory allocated by the plugin.

`uint32_t out_cap`: Maximum number of bytes that can be written to `out_ptr`.

`uint32_t* out_len`: Actual length of the JSON bytes written to `out_ptr`.

`uint32_t* required_cap`: When returning `PARSER_MUTATE_BUFFER_TOO_SMALL`, the plugin must write the minimum capacity required for the complete output. taosX will use this value to resize the buffer and retry.

**Memory Ownership**:

- Output memory is always owned and released by taosX.
- The plugin is only responsible for writing the complete JSON bytes into the buffer pointed to by `out_ptr`.
- The plugin must not return output memory allocated by the plugin, and must not require taosX to call an extra release function for parser output.
- The old `parser_free_output` API has been removed. ABI v2 no longer supports that ownership model.

**Return Value Constraints**:

- Return `PARSER_MUTATE_OK` on success, and guarantee `out_len <= out_cap`.
- Return `PARSER_MUTATE_BUFFER_TOO_SMALL` when the output buffer is too small, and write `required_cap`.
- Return `PARSER_MUTATE_ERROR` for other errors.
