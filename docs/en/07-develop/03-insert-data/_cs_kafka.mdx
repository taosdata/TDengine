...
[Confluent.Kafka](https://www.nuget.org/packages/Confluent.Kafka/1.9.4-RC1#readme-body-tab) is Confluent's .NET client for Apache Kafka and the Confluent Platform. More infomation reference [Kafka .NET Client](https://docs.confluent.io/kafka-clients/dotnet/current/overview.html#dotnet-installation)
...
Up to now, Every time the [Confluent.Kafka](https://www.nuget.org/packages/Confluent.Kafka/1.9.4-RC1#readme-body-tab) consumer consumes one message (one record) from topic. The following content include following example of writting data to TDengine: 
