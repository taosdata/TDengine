...
- [Modify the server default timezone configuration](../12-operations-and-tooling/03-components/01-taosd.md)
- [Modify the client default timezone configuration](../12-operations-and-tooling/03-components/02-taosc.md)
- [Set the current session timezone](../05-tdengine-sql/10-time/01-timezone.md#set-timezone)
- [Understand how DST affects writes and queries](../05-tdengine-sql/10-time/02-dst.md)
...
![Windowing logic](../assets/query-and-aggregate-01.png)
...
![Sliding window logic](../assets/query-and-aggregate-02.png)
...
The INTERVAL clause supports using the FILL clause to specify the data filling method when data is missing. For how to use the FILL clause, please refer to [FILL Clause](../05-tdengine-sql/04-data-query/01-query.md#fill-clause).
...
- `TRUE_FOR(...)` filters windows by duration, row count, or both. For `EVENT_WINDOW`, also supports `start(...)` / `end(...)` for open/close streak thresholds. See [Event Window TRUE_FOR](../05-tdengine-sql/04-data-query/06-distinguished.md#event-window).

For detailed information, see [TDengine Distinctive Queries](../05-tdengine-sql/04-data-query/06-distinguished.md#state-window).
...
![Session window example](../assets/query-and-aggregate-03.png)
...
![Event window example](../assets/query-and-aggregate-04.png)
...
3. The week-related ranges here do not take the first day of the week into account. TDengine supports configuring the first day of the week; see [firstDayOfWeek](../12-operations-and-tooling/03-components/02-taosc.md#region-related).
