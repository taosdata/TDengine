...
- Install JDBC driver. Download the `TDengine JDBC` connector file `taos-jdbcdriver-3.6.2-dist.jar` from `maven.org`.
...
**Step 1**, Create a Spark Interaction Instance.
``` java
  // create connect
  SparkSession spark = SparkSession.builder()
      .appName("appSparkTest")
...
**Step 3**, Map the Table and Display the Data in the Table.
``` java
  // map table
...
**Step 3**, Subscribe to the Topic, Consume the Data, and Display It in Spark.
``` java
    // poll
...
**Step 4**, Unsubscribe and Release Resources.
``` java
  // close
...
**Step 2**, Use Spark SQL to query the data in the Spark View and calculate the weekly voltage change rate. The SQL is as follows:
``` sql
