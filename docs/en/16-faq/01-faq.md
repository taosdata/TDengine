...
1. You can also use the network connectivity test feature embedded in the taos program to verify whether the specified port connection between the server and client is clear: [Operation Guide](../12-operations-and-tooling/02-operations/index.md).
...
4. **Stmt object reuse**: Frequently creating new `PreparedStatement` (Stmt) objects significantly hurts performance. Create Stmt objects at application startup and reuse them throughout. Refer to the [high-throughput write example](../10-developer-guide/05-high-throughput.md). Use `show queries;` to verify long-running parameter-binding statements are present, confirming Stmt reuse.
...
For the network ports used, please refer to the document: [Operation Guide](../12-operations-and-tooling/02-operations/index.md)
...
Problem Solution:
Upgrade to a recent JDBC driver version and add the appropriate parameter to the DBeaver JDBC connection URL. Refer to the [official documentation](../13-ecosystem-integrations/04-tool/01-dbeaver.md) for details.
