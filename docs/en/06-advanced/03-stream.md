...
- Show streams can be used to view the progress of background stream creation (ready status indicates success, init status indicates stream creation in progress or failed, if it remains in init status for a long time, please delete the stream and rebuild it).
