...
- `ENCODE` and `COMPRESS`: See [Data Compression](../03-data-write/03-compress.md).

- `COMMENT` and `SMA`: See [Tables](./02-table.md).

- `KEEP`: See [Databases](./01-database.md) for details. However, note the following differences between supertable-level `KEEP` and database-level `KEEP`:
...
- `VIRTUAL`: Specify 1 to create a virtual supertable. The following restrictions apply:
  ...
  - `BASE ON`: A virtual supertable can inherit columns and tags from one or more parent virtual supertables using the `BASE ON` clause. For details, see [Virtual Table Inheritance](./04-virtualtable.md#virtual-supertable-inheritance).
