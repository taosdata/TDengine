...
:::note
`lag`/`lead` can also be used **without** an `OVER` clause, in which case they are evaluated on the row order of the input result set. See [Sequential Analysis Functions](./03-function.md#lag). The parameter rules differ slightly: without `OVER`, `offset` must be an integer greater than 0; with `OVER`, `offset` may be 0.
:::
