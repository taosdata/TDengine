...
Since TDengine Enterprise Edition v3.4.2.1, a user's `SYSINFO` attribute (see [User Management](./01-user.md)) is linked with the `SYSINFO_0`/`SYSINFO_1` roles and elevated system roles, eliminating the need to configure them separately. The rules are as follows:
...
> - **3.4.0.11+**: The expansion behavior when `priv_obj` is omitted is controlled by the parameter [`enableGrantLegacySyntax`](../../12-operations-and-tooling/03-components/01-taosd.md#security-related) (default `0`). To avoid ambiguity, it is recommended to always explicitly specify `priv_obj`.
...
**Pre-activation Check:** Before activation, the system scans **all users who hold any system role** (SYSSEC, SYSAUDIT, SYSAUDIT_LOG, SYSDBA) and **all users who directly hold `ALTER SECURITY POLICY` privilege** (including disabled users).
...
| Feature | 3.3.x.y- | 3.4.0.0+ | 3.4.0.11+ | 3.4.1.6+ |
|---------|---------|----------|-----------|----------|
...
When upgrading from 3.3.x.y or earlier to 3.4.0.0+, the system automatically completes the following migration operations:

#### 1. Automatic Permission Conversion

Permissions set in lower versions are automatically converted to the 3.4.0.0+ permission format:

- Use `SHOW USER PRIVILEGES` before and after the upgrade to view and verify permission conversion results

#### 2. User Role Mapping

Users created in lower versions are automatically mapped based on their permission status:

- The root user has full permissions of SYSDBA, SYSSEC, and SYSAUDIT system roles by default
- After the upgrade, use `SHOW USERS` to view the user list and their role information

#### 3. System Role Initialization

System built-in roles are automatically created and configured with corresponding permissions during the upgrade:

- Automatically creates SYSDBA, SYSSEC, SYSAUDIT, SYSAUDIT_LOG, SYSINFO_0, SYSINFO_1, and other system roles
- System role permissions are predefined by the system and cannot be modified via the command line (supports automatic updates during upgrades)
- After the upgrade, use `SHOW ROLES` and `SHOW ROLE PRIVILEGES` to view system roles and their permissions

#### 4. Post-Upgrade Permission Configuration

After upgrading to 3.4.0.0+, when configuring permissions for users or roles, refer to the new permission management model:

- For system permission configuration, refer to the [System Permission Management](#system-permission-management) section
- For object permission configuration, refer to the [Object Permission Management](#object-permission-management) section
- For permission configuration examples, refer to the [Best Practices](#best-practices) section
- After configuration, use `SHOW USER PRIVILEGES` and `SHOW ROLE PRIVILEGES` to verify permission settings

#### Upgrade Precautions

| Item | Description |
|------|-------------|
| ✓ Automatic upgrade | Supports automatic upgrade from lower versions after shutdown to 3.4.0.0+ |
| ✗ Rolling upgrade | Rolling upgrade is not supported; shutdown upgrade is required |
| ✗ Version rollback | Cannot downgrade to a lower version after upgrade |
| ⚠️ Pre-upgrade backup | Recommended to run `SHOW USER PRIVILEGES` and save the output |
| ⚠️ Post-upgrade verification | Recommended to verify that permission conversion meets expectations, especially for critical business users |
| ⚠️ Three-power separation | After upgrade, it is recommended to immediately configure [Separation of Three Powers](#overview-of-separation-of-three-powers) and stop using root for daily operations |
