...
2. taosAdapter (business) provides RESTful and WebSocket access interfaces to the business cluster. It can be set up as a cluster of multiple instances behind a reverse proxy server such as nginx.

3. taosKeeper collects monitoring data from the business cluster and stores it in the monitoring cluster.

4. The monitoring cluster is the target cluster for storing monitoring data. This cluster can use the same physical machines as the business cluster if desired. If you are monitoring multiple business clusters, set up a monitoring cluster that is physically independent from any business cluster. However, if hardware resources are limited, you can set up your monitoring cluster and business cluster on the same hardware.

5. taosAdapter (monitoring) provides RESTful and WebSocket access interfaces to the monitoring cluster. If your business and monitoring clusters use the same physical machines, you can use a single taosAdapter cluster for business and monitoring. However, if your business and monitoring clusters use independent hardware, you must set up independent taosAdapter clusters for your business and monitoring clusters.. taosAdapter (monitoring) can also be set up as a cluster of multiple instances behind a reverse proxy server such as nginx.
...
8. The rules when setting up taosX and Agent, there are a few different scenarios:
  ...
  2) taosX can't connect to the data source directly
    In this case, Agent is required and needs to be set up in a network that can connect to the data source directly and can connect to the gRPC port listened by taosX (default is 6055).
