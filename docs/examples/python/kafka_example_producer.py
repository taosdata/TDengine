...
def produce_total(workers: int, broker: str, topic: str, message_type: str, total: int, table_count: int):
    if len(producers) == 0:
        lock.acquire()
        if len(producers) == 0:
...
def _produce_total(topic: str, message_type: str, total: int, table_count: int):
...
def _init_kafka_producers(broker: str, count: int):
...
def _get_fake_date(table_count: int, message_type: str = 'json') -> str:
...
def _get_json_message(table_count: int) -> str:
    return json.dumps({
        'ts': _get_timestamp(),
...
def _get_line_message(table_count: int) -> str:
...
def _random_table_name(table_count: int) -> str:
