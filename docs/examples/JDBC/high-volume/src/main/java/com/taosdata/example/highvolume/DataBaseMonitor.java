 ...
 * Count total records in database periodically so that we can estimate the
 * writing speed.
 */
public class DataBaseMonitor {
    ...
    public long getTableCount() throws SQLException {
        try (ResultSet result = stmt
                .executeQuery("select count(*) from information_schema.ins_tables where db_name = 'test';")) {
            result.next();
            return result.getLong(1);
        }
    }
