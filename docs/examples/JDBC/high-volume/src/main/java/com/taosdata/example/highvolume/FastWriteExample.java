...
public class FastWriteExample {
    ...
    public static void main(String[] args) throws InterruptedException, SQLException {
        ...
        int rowsPerTable = args.length > 3 ? Integer.parseInt(args[3]) : 10000;
        int maxBatchSize = args.length > 4 ? Integer.parseInt(args[4]) : 3000;
        ...
        for (int i = 0; i < readTaskCount; ++i) {
            ReadTask task = new ReadTask(i, taskQueues, tableCountPerTask, rowsPerTable);
            Thread t = new Thread(task);
        ...
        while (true) {
            ...
            if (count == rowsPerTable * numberOfTable){
                break;
            }
        }
    }
