# TDengine TSDB 全链路传输安全与压缩

> 版本：v3.4+（TLS/SASL 相关功能为企业版特性 🔒；存储压缩为社区版和企业版通用特性）
> 存储压缩完整 SQL 语法参见 [TDengine 用户手册 - 数据压缩](https://docs.tdengine.com/operation/compress/)。

---

## 概述

TDengine TSDB 的传输安全（TLS/SASL）与压缩（WebSocket/gRPC/TCP/落盘）按 **6 层分层架构** 逐层展开——从用户侧的入口（应用、Web UI、CLI），经接入层的两条路径（WebSocket/REST 与 taosc 私有协议），延伸到数据采集链路、集群内部，再到运维侧的可观测性接入，最后进入存储层落盘压缩。传输安全与传输压缩共享同一条物理链路，本文按链路统一讲解。
...
| ③ 数据采集链路 | taosX→Adapter WSS + deflate；taosxAgent→taosX gRPC TLS + gzip；外部源 TLS + 消息体压缩（gzip/snappy/lz4/zstd）|
...
| taosxAgent → taosX | gRPC TLS | gzip | 不启用 | 不启用 |
| 外部源 → taosX / taosxAgent | TLS（视源端协议）| gzip/snappy/lz4/zstd（消息体）| 视源端 | 视源端 |
...
App / 连接器侧的 TLS 与压缩都落在 **App ↔ taosAdapter** 这一跳（详见 §2.1 的服务端配置），客户端侧只需在 DSN / 连接字符串中开启。
...
4 个 CLI 工具在传输层都落到 §2 的两条接入路径之一。统一的思路：
...
**taosAdapter 服务端**（`/etc/taos/taosadapter.toml`）：

```toml
[ssl]
enable   = true
certFile = "/etc/taos/certs/adapter.pem"
keyFile  = "/etc/taos/certs/adapter-key.pem"
```

#### 2.1.2 传输压缩（permessage-deflate）

taosAdapter 服务端默认接受客户端的 permessage-deflate 协商请求（硬编码启用），并对所有 HTTP 响应自动 gzip 压缩。客户端按需在 DSN 中开启即可（参见 §1.1 各连接器示例）。
...
taosc 作为独立的原生客户端库，使用 TDengine **私有协议**（TCP :6030）与 taosd 通信；私有协议之上承载 TLS 传输加密、SASL/SCRAM-SHA-256 认证与 zlib 消息压缩。原生连接器（C/Java JNI/Python native/Go cgo/Rust native/C#）将 taosc 作为动态库嵌入应用进程，其传输行为与 taosAdapter 内部调用的 taosc 完全一致。
...
### 3.2 taosxAgent → taosX（gRPC TLS + gzip）

taosxAgent 通过 Arrow Flight RPC（gRPC / HTTP2）上报数据。
...
**taosxAgent**（`/etc/taos/agent.toml`）：
...
DNode 间的 RPC 复用 `compressMsgSize` 配置（参见 §2.2.5）——一旦在 `taos.cfg` 中打开，客户端 ↔ taosd 与 DNode ↔ DNode 共同受控。集群内部通常走万兆内网，建议保持默认关闭以避免 CPU 开销。

---

## 5. 可观测性接入传输（taosKeeper）

taosKeeper 作为被动接收方汇聚 taosd / taosAdapter / taosX / taosExplorer 推送的指标，再写回 TSDB。它只负责可观测性链路，不承载审计日志语义。其自身暴露 HTTPS 接收端口，并通过 HTTPS 向 taosAdapter 回写。
...
> Explorer 自身的 HTTPS 配置已归入 §1.2（因为它是 Web UI **入口**，而非“向内”的运维接入）。

---

## 6. 存储层：落盘压缩

> 经过 §1-§5 的链路传输之后，数据最终写入 TSDB 存储，在这里再经过一轮**落盘压缩**（三级压缩）以显著降低占用。存储压缩独立于传输链路，对客户端透明。详细 SQL 参见 [TDengine 用户手册 - 数据压缩](https://docs.tdengine.com/operation/compress/)。
...
| 场景 | TLS | App→Adapter 压缩 | taosc→taosd 压缩 | taosX→Adapter 压缩 | taosxAgent→taosX 压缩 |
|------|-----|------------------|------------------|---------------------|--------------------|
...
- [ ] taosxAgent `ca` 参数指向正确 CA
- [ ] taosxAgent `compression = true`（跨 WAN 场景）
