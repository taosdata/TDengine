...
| 日期 | 版本 | 负责人 | 主要修改内容 |
| --- | --- | --- | --- |
| 2024/11/4 | 0.1 | 司马靖 | 初稿 |
| 2024/11/14 | 0.2 | 司马靖 | 根据 11/08 讨论的结果更新一版 |
| 2024/11/18 | 0.3 | 司马靖 | 根据 11/15 讨论的结果更新一版 |
| 2024/11/19 | 0.4 | 司马靖 | 根据 11/18 讨论的结果更新一版 |
| 2024/11/22 | 1.0 | 司马靖 | 正式版第一稿 |
| 2025/2/11 | 1.1 | 司马靖 | 去掉企业版的限制 |

## 3. 定义

### 3.1 虚拟表 (Virtual table)

**虚拟表** 是一种动态数据结构，允许从多个表中选择列，将数据按照时间戳排序，并根据对齐规则生成一张新的逻辑表。它的主要功能如下：
1. **列选择与拼接**
...
**原始表**是虚拟表数据的来源，通常包括时间戳列及其他属性列。

## 4. 行为说明

### 4.1 数据模型

以[需求说明：增强视图](https://taosdata.feishu.cn/wiki/NPJ8w7FI6i5UimkxBeCcpa1yn7e)中 4.1 节的数据模型为例，
首先采用单列模型建模。共创建三个超级表，标签列包括设备 ID、采集点类型、点号。
```sql {wrap}
...
创建虚拟表时支持跨库指定数据源。
虚拟超级表下只支持创建虚拟子表，虚拟子表也只能 `USING` 虚拟超级表来创建。
...
若在创建虚拟表时，只想指定其中某几列，可以使用以下方法创建，其他列可以在在创建之后通过 `ATLER VTABLE ... ALTER` 的方式手动指定：
```sql
create vtable d1 (
    switch from p10.val,
    wind_speed from p19.val)
...
用户在查询时，虚拟表和正常的物理表在用户看来应该是一样的，所以虚拟表的查询与正常表的查询是相同的，没有区别。
...
![](./images/img_Ny95b4okZo4hGkxRw0McYuY0nDC.png)
...
得到的结果如下图所示：
![](./images/img_H7Tzbb96VoOAXKxNyHPcigUKnOb.png)
...
得到的结果如下图所示：
![](./images/img_ESrVbTso9oZA4UxMDsYcMkwpnac.png)

因为 `c1`, `c2` 列对应的原始表 `t1`, `t2` 中没有 `0:00:03` 这个时间戳，所以最后的结果也不会包含这个时间戳。

#### 4.3.2 空值的处理与插值方法

在构建虚拟表时，由于不同表的时间戳不完全一致，数据合并后可能会产生空值（`NULL`）。为了解决这一问题，新增了填充函数：`padding` 用于对空值进行处理和插值。
函数的定义可见**附录。**
1. `padding(col, 'prev')` 
  用同原始表同一列中**时间戳之前最近的****非空值**填充空值。
  例如执行如下查询：
  ```sql
  select padding(c1, 'prev') as c1, c2, c3 from v1; 
  ```

  可以得到如下结果：
  ![](./images/img_DpMJb2kHnojq09xhnaacmMTCnfd.png)

  对于查询：
  ```sql
  select padding(c1, 'prev') as c1, padding(c2, 'prev') as c2 from v1;
  ```

  查询结果如下：
![](./images/img_YxoRbDcnGo6UC3xaY7NcF4OFnKc.png)

由于 `c2` 列在 `0:00:01` 时间点之前并没有数据，所以在 `0:00:01` 时间点没有填充值，仍然是 `NULL`。
1. `padding(col, 'next')` 
  用同原始表同一列中**时间戳之后最近的非空值**填充空值。
  例如执行如下查询：
  ```sql
  select padding(c1, 'next') as c1, c2, c3 from v1; 
  ```

  可以得到如下结果：
  ![](./images/img_Oe34bc3kMoNPvexSXDbc9uOvnSd.png)

  由于 `c1` 列在 `0:00:05` 时间点之后并没有数据，所以在 `0:00:05` 时间点没有填充值，仍然是 `NULL`。
  对于查询：
  ```sql
  select padding(c1, 'next') as c1, padding(c2, 'next') as c2 from v1;
  ```

  查询结果如下：
![](./images/img_G9I2bUNzFoMg9FxrCVaceHOnnHd.png)

1. `padding(col, 'linear')` 
  基于时间戳之间的间隔，使用**线性插值**方法计算空值。
  例如执行如下查询：
  ```sql
  select padding(c1, 'linear') as c1, c2, c3 from v1; 
  ```

  可以得到如下结果：
  ![](./images/img_YSmKbZ9dlon5ZnxWDpGcAxBJnfb.png)

  由于 `c1` 列在 `0:00:05` 时间点之后并没有数据，计算线性插值需要结合该时间戳**之前的最近非空值**和**之后的最近非空值**来计算，所以在 `0:00:05` 时间点没有填充值，仍然是 `NULL`。
  对于查询：
  ```sql
  select padding(c1, 'linear') as c1, padding(c2, 'linear') as c2 from v1;
  ```

  查询结果如下：
  ![](./images/img_H0Kyb9hgdogJTgxwZTVc8it2n9b.png)

1. `padding(col, 'value', padding_value)` 
  基于时间戳之间的间隔，使用**固定值插值**方法填充空值。
  例如执行如下查询：
  ```sql
  select padding(c1, 'value', 10) as c1, c2, c3 from v1; 
  ```

  可以得到如下结果：
  ![](./images/img_CnT6bKhGUoJbPNx62PzclW3Hnze.png)

  对于查询：
  ```sql
  select padding(c1, 'value', 10) as c1, padding(c2, 'value', 10) as c2 from v1;
  ```

  查询结果如下：
  ![](./images/img_Lc6AbC0NsoYALxxV94qcTsQgnmg.png)
...
表结构没有变化，表类型 `type` 新增 `V_NORMAL_TABLE` 和 `V_CHILD_TABLE`
...
**限制**：
1. 第一版（三月底版本）
   - 不包括填充函数，若需插值，可以对虚拟表查询结果使用 interp 函数来对某个时间点进行插值，但不支持使用函数查询整列插值后的结果。
   - 不支持 4.2.2.1 中创建虚拟子表时指定 tag 为原始表的 tag 值功能。
...
```sql
PADDING(expr, {'prev' ｜ 'next' | 'linear'})
PADDING(expr, 'value', padding_value)
```
...
**适用于**：表和超级表。
**使用说明**：
- 在参数列中，只要是为 NULL 的值都会被填充。
- 作用于超级表时, 会先对每个超级表下的子表进行填充，然后再将结果汇总。
- 填充方式为 `prev` 时表示用同表同一列中**时间戳之前最近的非空值**填充空值。
- 填充方式为 `next` 时表示用同表同一列中**时间戳之后最近的非空值**填充空值。
- 填充方式为 `linear` 时表示基于时间戳之间的间隔，结合该时间戳**之前的最近非空值**和**之后的最近非空值**来使用**线性插值**方法计算空值。
- 填充方式为 `value` 时需要指定填充值，表示使用**固定值**来填充空值。
