...
| --- | --- | --- | --- | --- |
| 2026-04-24 | 2026-04-24 | 1.0 | 贾靖斌 | 编写 3.4.1.0 性能测试报告 |

### 2. 概述

本报告旨在验证 TDengine TSDB 3.4.1.0 版本在标准性能基准下的表现。测试通过 TestNG_V3 性能框架，分别在"无缓存（None）"和"最新行缓存（Lastrow）"两种缓存模型下，对数据写入及多种复杂查询场景进行压力测试，以评估系统的吞吐量及响应时延。同时针对 v3.4.1.0 新增的 Interval 窗口插值、External Window、批量标签修改等功能的性能表现进行了专项验证。
...
- **新增功能性能**：涵盖 Interval 窗口插值时间范围查询、External Window 查询、批量标签修改操作性能。
...
- **软件配置**：
  - 操作系统：Linux x64
  - 数据库版本：TDengine TSDB Enterprise 3.4.1.0
...
|  | **描述** | **测试项数量** | **结论** |
| --- | --- | --- | --- |
| **benchmark_query_none** | 无缓存查询模型 | 1项写入 + 19项查询 | 达标 |
| **benchmark_query_lastrow** | 最新行缓存模型 | 1项写入 + 19项查询 | 达标 |
| **benchmark_new_features** | 新增功能性能测试 | 3项专项测试 | 达标 |

### 5. 测试结果

http://192.168.0.204:3000/d/f39f4b6c-7243-44ee-817f-a8a52b5fe516/baseline-all?orgId=1&refresh=1m&var-base_type=release&var-base_label=3.4.1.0&var-target_type=release&var-target_label=3.4.0.0
...
| **测试 ID** | **测试场景描述** | **验证点** |
| --- | --- | --- |
| **N10001** | Interval 窗口插值时间范围查询 | 验证不同插值算法（线性插值、前值填充）在大数据量下的查询响应时间 |
| **N10002** | External Window 查询 | 验证外部窗口函数在复杂窗口定义下的计算性能 |
| **N10003** | 批量标签修改操作 | 验证批量修改百万级子表标签值的操作吞吐量和响应时间 |

### 7. 版本性能对比分析 (v3.4.1.0 vs v3.4.0.0)

根据 Grafana 性能对比看板（Baseline-All）数据分析，TDengine TSDB v3.4.1.0 在核心性能指标上相对于 v3.4.0.0 表现稳健，整体呈现稳中有升的趋势。

#### 7.1 写入性能对比分析

- **吞吐量 (Throughput)**: v3.4.1.0 整体写入吞吐量与 v3.4.0.0 持平，在高并发 STMT 写入场景下，表现稳定。
- **延迟 (Latency)**: 写入平均延迟维持在毫秒级，在高负载压力下，v3.4.1.0 的 P99 延迟表现与 v3.4.0.0 一致。

#### 7.2 查询性能对比分析

- **点查询性能 (Q10001)**: v3.4.1.0 与 v3.4.0.0 性能高度一致，QPS 波动在 ±2% 以内。
- **最新行查询**: 在 `lastrow` 模式下 QPS 表现优秀，满足实时业务需求。
- **复杂查询**: 左关联查询 (Q60002) 延迟稳定，版本间性能高度一致。
- **插值查询 (Q30004)**: v3.4.1.0 在 Interval 窗口插值时间范围场景下，得益于查询优化器改进，响应时间有所提升。

#### 7.3 新增功能性能分析

- **Interval 窗口插值**：在百万行数据集上的窗口插值查询响应时间满足性能基线要求。
- **External Window**：外部窗口函数计算性能稳定，资源消耗合理。
- **批量标签修改**：百万级子表批量标签修改操作在预期时间内完成，事务开销可控。

### 8. 测试结论

- **策略有效性**：Lastrow 缓存模型针对高频访问场景的优化通过验证。
- **版本迭代一致性**：对比 v3.4.0.0，新版本在核心读写性能上保持高度一致，无性能回退。
- **新增功能性能达标**：Interval 窗口插值、External Window、批量标签修改等新增功能的性能表现均满足基线要求。
- **读写均衡性**：在高并发读写混合压力下，响应曲线依然平滑。
- **性能基准达标**：全量 19 项查询及写入基准均达标，新增功能性能测试达标，符合安可发布要求。

**最终结论：TDengine TSDB v3.4.1.0 性能表现与 v3.4.0.0 历史基准持平，新增功能性能达标，建议发布。**
