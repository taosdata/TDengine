...
| --- | --- | --- | --- | --- |
| 2025-10-15 | - | 0.1 | 鲍之骁 | 初稿 |
| 2025-11-19 | 2025-11-19 | 1.0 | 关胜亮 | 评审后发布 |

## 2. 背景

在数据安全日益重要的背景下，尤其是在金融、政府和医疗等行业，静态数据（Encryption at Rest）的保护已成为合规要求的核心。TDengine作为时序数据库，存储大量敏感时序数据、配置和元数据，面临潜在泄露风险。该特性源于JIRA TS-7230的需求，旨在通过引入透明数据加密和密钥管理机制，实现重要数据的加密存储，支持国密算法，从而提升系统的机密性和完整性。目标是让用户无需手动干预即可自动加密文件，同时提供密钥生命周期管理，确保数据在存储层的安全，同时最小化性能影响。
JIRA [TS-7230](https://jira.taosdata.com:18080/browse/TS-7230)

## 3. 定义

1. **SVR_KEY (服务器主密钥)**: 用于加密数据库主密钥和系统级信息，与机器硬件绑定，防止跨机移植。
2. **DB_KEY (数据库主密钥)**: 用于加密数据库密钥。
3. **CFG_KEY (配置加密密钥)**: 专用于加密配置文件和非数据文件，一旦生成不可更改。
4. **META_KEY (元数据加密密钥)**: 用于加密元数据文件，如权限和密码相关信息，一旦生成不可更改。
5. **DATA_KEY (时序数据加密密钥)**: 用于加密时序数据文件和相关日志，一旦生成不可更改。
6. **透明加密**: 数据库系统自动处理的加解密过程，用户无需感知或手动操作。
7. **国密算法**: 中国国家密码管理局批准的算法，包括SM2 (非对称加密)、SM3 (哈希)、SM4 (对称加密)，用于符合本土合规要求。
8. **taosk**: 新增的可执行程序，用于密钥生成、管理和文件查看。
9. **encrypt.bin**: 存储加密密钥的文件，位于dnode/config目录下，结合机器码实现绑定。

## 4. 密钥生成与管理

本特性引入了密钥管理和加密机制，影响配置、部署和SQL操作。加密过程透明，用户通过taosk工具和SQL交互。以下详细说明变化。
...
2. --set-algorithm: 设置加密算法（sm4/sm3/sm2），默认sm4。
...
1. TDinsight：配置密钥到期告警策略，监控接口推送通知。

## 16. 安装和卸载

需要将编译生成的 taosk 二进制增加打包，安装以及卸载的流程。
