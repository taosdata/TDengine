...
| 2026-05-19 | - | 0.3 | Tony Zhang | 补充 `JOIN ON TIMETRUNCATE(...)` 在 merge join / hash join 下的一致性与 DST 语义 |
| 2026-05-23 | 2026-5-26 | 1.0 | Tony Zhang | 补充固定偏移兼容语义：`SET TIMEZONE` / `TO_ISO8601` 保持 legacy ISO 解释，`ALTER LOCAL` / `TO_CHAR` / `TIMETRUNCATE` 使用通用校验路径并按 POSIX 符号解释；补充 `GMT` 拒绝与 `ALTER LOCAL timezone` 统一校验说明 |
...
| `JOIN ON TIMETRUNCATE(..., 1d)` | 无 | L2 → L4 → L5 |
...
**参数**：
- `timezone_string`：支持 IANA 时区名或固定偏移字符串
...
- `SET TIMEZONE` 保持历史固定偏移解释：`+08:00` 表示 `UTC+08:00`，`-05:00` 表示 `UTC-05:00`
...
- `ALTER LOCAL 'timezone'` 走通用时区校验路径，除 IANA 名称外，还接受 Windows 标准时区名称、`UTC±H[:MM]` / `UTC±HH[:MM]` 与裸固定偏移。
- 该通用路径中的固定偏移保留符号并按 POSIX 语义解释，因此 `+08:00`、`UTC+08:00`、`UTC+8` 都表示 `UTC-08:00`。
- `GMT` / `GMT±...` 在该路径中统一拒绝。
...
对于 `JOIN ON TIMETRUNCATE(ts, 1d)` 这类“先按本地日历截断、再做等值连接”的场景，连接两侧都应按同一回退链解析时区与日边界；默认优化器选择的 merge join 路径与显式 `/*+ hash_join() */` 路径都应得到一致结果。
...
| 单位 | 含义 | 示例 | 说明 |
| --- | --- | --- | --- |
| `n` | 月 | `1n` | 仅支持 `1n`，不支持 `2n` 等倍数 |
| `q` | 季度 | `1q` | 仅支持 `1q`，不支持 `2q` 等倍数 |
| `y` | 年 | `1y` | 仅支持 `1y`，不支持 `2y` 等倍数 |

原有单位 `b`/`u`/`a`/`s`/`m`/`h`/`d`/`w` 行为不变；所有单位均只接受 `1x` 格式，`Nx`（N>1）一律返回 `TSDB_CODE_FUNC_TIME_UNIT_INVALID`。
...
相同的 DST 感知日边界语义也适用于 `JOIN ON TIMETRUNCATE(ts, 1d)`。也就是说，在 IANA 时区下，如果两个绝对时刻落在同一本地自然日，则无论执行计划最终走 merge join 还是 hash join，都应能够命中；若落在不同本地自然日，则两条路径都应返回不命中。
...
| `TIMETRUNCATE(..., n/q/y)` | 新增 | 新增自然时间单位（仅 `1n`/`1q`/`1y`），不影响旧 SQL；`Nx`（N>1）报参数无效错误 |
