...
| --- | --- | --- | --- | --- |
| 2026-05-19 | - | 0.1 | Tony Zhang | 初稿 |
| 2026-05-19 | - | 0.2 | Tony Zhang | 补充 `JOIN ON TIMETRUNCATE(...)` 在 hash join 路径下的 planner/executor 设计说明 |
...
- planner/executor：INTERVAL 周窗口与季度窗口行为；`JOIN ON TIMETRUNCATE(...)` 的主键表达式下推与执行
...
4. JOIN 相关补充：
- `JOIN ON TIMETRUNCATE(ts, 1d)` 需要保证 merge join 与 hash join 使用同一套 DST 感知日边界语义
- hash join 不能直接把 `TIMETRUNCATE(...)` 函数节点当作普通列 key 使用，planner 需要先物化成 slot-backed 列引用，再交给 hash key 构建路径消费
...
#### 5.6.3 `JOIN ON TIMETRUNCATE(...)` 一致性

目标：
- `JOIN ON TIMETRUNCATE(ts, 1d)` 在 merge join 与 hash join 下结果一致
- IANA 时区下按目标时刻解析 DST，不允许 hash join 退化为 epoch 时刻固定偏移

实现点：
- planner：hash join 在提取 join key 时，允许 `TIMETRUNCATE(column, ...)` 形式的主键表达式；对这类表达式先补齐 data block slot，再转换为 slot-backed 列节点加入 `pOnLeft/pOnRight`
- executor：hash join 的主键表达式上下文补齐 `timezone_t`、`firstDayOfWeek`、precision 等信息，`TIMETRUNCATE` 计算逻辑与 merge join 对齐
- 验收：默认 merge join 与 `/*+ hash_join() */` 两条路径在 `TestJoinDst` 中都应覆盖“同一本地日命中 / 跨本地日不命中”
