...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < numOfThreads; i++) {
    int err = pthread_create(&threads[i], NULL, func, (void *)(argv[1]));
    if (err != 0) {
...
void Test(char *qstr,  const char *input, int index)  {
  ...
  //queryDB(taos, "create table if not exists m1 (ts timestamp, ti tinyint, si smallint, i int, bi bigint, f float, d double, b binary(10))");
