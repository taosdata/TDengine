...
class TDTestCase:
    ...
    def run(self):
        ...
        os.system("taosdemo -f wal/insertDataDb1.json -y " % binPath)
        tdSql.execute("drop database if exists db1")
        os.system("taosdemo -f wal/insertDataDb2.json -y " % binPath)
        tdSql.execute("drop table if exists db2.stb0")
        os.system("taosdemo -f wal/insertDataDb2Newstab.json -y " % binPath)
