...
class TDTestCase:
    ...
    def executeQueries(self):
        ...
        tdSql.query("select last(c1) from %s%d where ts <= %d" %
        ...
        tdSql.query("select last(c1) from %s%d interval(1m)" %
                    (self.perfix, 1))
        tdSql.checkData(1, 1, 1)

        tdSql.query("select last(c1) from %s%d interval(1d)" %
                    (self.perfix, 1))
        tdSql.checkData(0, 1, 19)

        tdSql.query("select last(c1) from %s%d where ts <= %d interval(1m)" %
                    (self.perfix, 1, self.ts + 4 * 60000))
        tdSql.checkRows(5)
        tdSql.checkData(1, 1, 1)
        ...
        tdSql.query("select last(c1) as c from st where ts <= %d" %
                    (self.ts + 4 * 60000))
        tdSql.checkData(0, 0, 4)
        ...
        tdSql.query("select last(c1) as c from st where t1 <= 5 and ts <= %d" %
                    (self.ts + 4 * 60000))
        tdSql.checkData(0, 0, 4)
