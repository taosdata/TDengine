...
class TDTestCase:
    ...
    def run(self):
        ...
        # TSIM: sql use $db
        # TSIM: sql create table %s (ts timestamp, tbcol int) TAGS(tgcol1
        # smallint, tgcol2 bigint, tgcol3 float, tgcol4 double)
        tdLog.info(
            "create table %s (ts timestamp, tbcol int) TAGS(tgcol1 smallint, tgcol2 bigint, tgcol3 float, tgcol4 double)" %
            mt)
        tdSql.execute(
            'create table %s (ts timestamp, tbcol int) TAGS(tgcol1 smallint, tgcol2 bigint, tgcol3 float, tgcol4 double)' %
            mt)
        ...
        # TSIM: sql create table $tb using %s tags( 0, 0, 0, 0 )
            tdLog.info(
                "create table %s using %s tags( 0, 0, 0, 0 )" %
                (tb, mt))
            tdSql.execute(
                'create table %s using %s tags( 0, 0, 0, 0 )' %
                (tb, mt))
        ...
        # TSIM: sql create table $tb using %s tags( 1, 1, 1, 1 )
            tdLog.info(
                "create table %s using %s tags( 1, 1, 1, 1 )" %
                (tb, mt))
            tdSql.execute(
                'create table %s using %s tags( 1, 1, 1, 1 )' %
                (tb, mt))
        ...
        tdLog.info('=============== step19')
        # TSIM: sql select count(tbcol), avg(tbcol), sum(tbcol), min(tbcol),
        # max(tbcol), first(tbcol), last(tbcol) from %s where tgcol1 = 1
        tdLog.info(
        ...
        tdLog.info('=============== step22')
        # TSIM: sql select count(tbcol), avg(tbcol), sum(tbcol), min(tbcol),
        # max(tbcol), first(tbcol), last(tbcol) from %s where tgcol1 = 1 group
        # by tgcol1
        tdLog.info(
