...
class TDTestCase:
    ...
    def run(self):
        ...
        # binary(10), tgcol2 bigint, tgcol3 smallint, tgcol4 bigint, tgcol5
        # binary(30), tgcol6 binary(20))
        tdLog.info(
            'create table %s (ts timestamp, tbcol int) TAGS(tgcol1 binary(10), tgcol2 bigint, tgcol3 smallint, tgcol4 bigint, tgcol5 binary(30), tgcol6 binary(20))' %
            (mt))
        tdSql.execute(
            'create table %s (ts timestamp, tbcol int) TAGS(tgcol1 binary(10), tgcol2 bigint, tgcol3 smallint, tgcol4 bigint, tgcol5 binary(30), tgcol6 binary(20))' %
            (mt))
        ...
        while (i < 5):
            ...
            # )
            tdLog.info(
                'create table %s using %s tags( "0", 0, 0, 0, "0", "0" )' %
                (tb, mt))
            tdSql.execute(
                'create table %s using %s tags( "0", 0, 0, 0, "0", "0" )' %
                (tb, mt))
        ...
        while (i < 10):
            ...
            # )
            tdLog.info(
                'create table %s using %s tags( "1", 1, 1, 1, "1", "1" )' %
                (tb, mt))
            tdSql.execute(
                'create table %s using %s tags( "1", 1, 1, 1, "1", "1" )' %
                (tb, mt))
        ...
        # TSIM: sql select count(tbcol), avg(tbcol), sum(tbcol), min(tbcol),
        # max(tbcol), first(tbcol), last(tbcol) from $mt where tgcol1 = 1 and
        # tgcol2 = 1 and tgcol3 = 1  and tgcol4 = 1  and tgcol5 = 1 and tgcol6
        # = 1
        ...
        tdLog.info(
            'select count(tbcol), avg(tbcol), sum(tbcol), min(tbcol), max(tbcol), first(tbcol), last(tbcol) from %s group by tgcol6' %
            (mt))
        tdSql.query(
            'select count(tbcol), avg(tbcol), sum(tbcol), min(tbcol), max(tbcol), first(tbcol), last(tbcol) from %s group by tgcol6' %
            (mt))
