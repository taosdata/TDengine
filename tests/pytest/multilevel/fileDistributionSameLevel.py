...
class TDTestCase:
    ...
    def run(self):
        ...
        for i in range(self.ntables):
            ...
            tdSql.execute("insert into tb%d values(%d, 1)" %
                          (i, self.ts + int(i / 100) * 86400000))
        ...
        for i in range(50):
            ...
            for j in range(10):
                tdSql.execute("insert into tb values(%d, 1)" %
                              (self.ts + int(i / 100) * 86400000))
        tdDnodes.stopAll()
        tdDnodes.start()
        flag = True
        for i in range(4):
            if dataDict[dataDir[i]] == dataDict[dataDir[i + 1]]:
                ...
                break
        if not flag:
            tdLog.exit("%s failed, expect  not occured" % (sys.argv[0]))
