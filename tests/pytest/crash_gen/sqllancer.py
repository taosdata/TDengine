...
class SQLLancer:
    ...
    def formatHistogram(self, expr):
        funcList = [f'HISTOGRAM({expr}, "user_input", "[1, 3, 5, 7]", {random.choice([0, 1])})'] + \
                   [f'HISTOGRAM({expr}, "linear_bin", \'{"start": 0.0, "width": 5.0, "count": 5, "infinity": true}\', {random.choice(DataBoundary.HISTOGRAM_BOUNDARY.value)})'] + \
    ...
    def formatFunc(self, func, colname, **kwarg):
        ...
        elif func in ['AVG', 'COUNT', 'SPREAD', 'STDDEV', 'SUM', 'HYPERLOGLOG']:
            return f"{func}({colname})"
        elif func in ['INTERP', 'DIFF']:
        ...
        elif func in ['FIRST', 'LAST', 'LAST_ROW', 'MAX', 'MIN', 'MODE', 'UNIQUE']:
            return f"{func}({colname})"
        ...
        elif func in ['CSUM', 'IRATE', 'MAVG', 'STATECOUNT', 'STATEDURATION', 'TWA']:
            return f"{func}({colname})"
            pass
        # if func == 'CAST':
...
sqllancer = SQLLancer()
colDict = {"c1": "INT", "c2": "VARCHAR", "c3": "TIMESTAMP"}
tbname = "stb"
print("####", DataBoundary.INTERP_BOUNDARY.value)
