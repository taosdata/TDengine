...
class TDTestCase:
    ...
    def inTest(self, dbname="db"):
        ...
        tdSql.execute('CREATE TABLE `t11` USING `st1` (`t1`) TAGS (11);')
        tdSql.execute('CREATE TABLE `t12` USING `st1` (`t1`) TAGS (12);')
        tdSql.execute('CREATE TABLE `t21` USING `st2` (`t1`) TAGS (21);')
        tdSql.execute('CREATE TABLE `t22` USING `st2` (`t1`) TAGS (22);')
        tdSql.execute('CREATE TABLE `ta` (`ts` TIMESTAMP, `v1` INT);')
        ...
        tdSql.query("select last(*) from ta where tbname in ('ta');")  
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, '2025-01-21 00:00:03')
        tdSql.checkData(0, 1, 3) 
  
        tdSql.query("select last(*) from ta where tbname in ('ta', 't21');")  
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, '2025-01-21 00:00:03')
        tdSql.checkData(0, 1, 3) 
        
        tdSql.query("select last(*) from ta where tbname in ('t21');")  
        tdSql.checkRows(0)
        
        tdSql.query("select last(*) from ta where tbname in ('ta') and tbname in ('ta');")  
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, '2025-01-21 00:00:03')
        tdSql.checkData(0, 1, 3) 
        ...
        tdSql.query("select last(*) from ta where tbname in ('t21') or tbname in ('ta');")  
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, '2025-01-21 00:00:03')
        tdSql.checkData(0, 1, 3) 

        tdLog.debug(f"--------------  step2:  super table test   ------------------")
        tdSql.query("select last(*) from st1 where tbname in ('t11');")  
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, '2025-01-21 00:11:03')
        tdSql.checkData(0, 1, 113)

        tdSql.query("select last(*) from st1 where tbname in ('t21');")  
        tdSql.checkRows(0)

        tdSql.query("select last(*) from st1 where tbname in ('ta', 't21');")
        tdSql.checkRows(0)

        tdSql.query("select last(*) from st1 where tbname in ('t21', 't12');")
        tdSql.checkRows(1)
