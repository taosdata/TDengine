...
class TDTestCase:
    ...
    def queryOrderByAgg(self):
        ...
        tdSql.error(f"SELECT last(ts), first(ts) FROM t1 order by last(ts)")  # ambiguous ts column
        tdSql.no_error(f"SELECT last(ts) lts, first(ts) fts FROM t1 order by last(ts)")
        tdSql.no_error(f"SELECT last(ts) lts, first(ts) fts FROM t1 order by lts")
        tdSql.error(f"SELECT last(c1), first(c1) FROM t1 order by last(c1)")  # ambiguous c1 column
        tdSql.error(f"SELECT last(ts) as t, first(ts) as t FROM t1 order by last(t)")  # ambiguous alias t
