...
class FillResValidator:
    ...
    def validate_fill_prev_one_group(self, fill_res_one_group, interval_res_one_group, desc_res_one_group: List):
                ...
                if fill_ts < interval_row[0]:
                    ...
                    for colIdx in range(1, len(fill_row)):
                        if interval_row[colIdx] is None:
                            ...
                            if val is not None:
                                #tdLog.debug(f"interval col is None: ({j}, {colIdx}) found prev valid val: {val}")
                                pass
                            else:
                                #tdLog.debug(f"interval col is None: ({j}, {colIdx}) not found prev valid val: {val}")
                                pass
                        else:
        ...
        for row_asc, row_desc in zip(fill_res_one_group, desc_res_one_group):
            for colIdx in range(1, len(row_asc)):
                if row_asc[colIdx] != row_desc[colIdx]:
                    tdLog.exit(f"got different val for fill_res: {row_asc} rowIdx: {i} colIdx: {colIdx}, expect: {row_desc[colIdx]} got: {row_asc[colIdx]}")
