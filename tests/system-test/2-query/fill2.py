...
class FillQueryGenerator:
    ...
    def get_agg_funcs(self) ->list:
        res: list[str] = []
        funcs = ['avg', 'min', 'max', 'sum', 'last', 'first']
        for col in self.columns:
            if not col.type_.is_varchar_type() and not col.type_.is_decimal_type():
    ...
    def generate_interval(self):
        if self.interval is None:
            self.interval = random.randint(1, 100000)
    ...
    def fix_where_out_of_interval_range(self):
        if self.where_end < self.ts_start:
            self.where = None
            self.generate_where()
    ...
    def no_fill_sql(self)-> str:
        return f'select {self.generate_select_list()} from {self.dbname}.{self.tbname} WHERE {self.generate_where()} {self.generate_partition_by()} INTERVAL({self.generate_interval()}a)'

    def generate_sql(self)-> str:
        return f'select {self.generate_select_list()} FROM {self.dbname}.{self.tbname} WHERE {self.generate_where()} {self.generate_partition_by()} INTERVAL({self.generate_interval()}a) FILL({self.generate_fill()})'
...
class FillResValidator:
    ...
    def get_row_with_ts(self, res, rowIdx):
        if len(res) > rowIdx:
            return res[rowIdx]
        return None
    
    def find_prev_valid_val_for_col(self, res, fromRowIdx, forward: bool, colIdx):
        if forward:
            step = 1
            end = len(res)
        else:
    ...
    def validate_fill_prev_one_group(self, fill_res_one_group, interval_res_one_group, desc_res_one_group):
        ...
        while i < len(fill_res_one_group):
            ...
            fill_ts = fill_row[0]
            interval_row = self.get_row_with_ts(interval_res_one_group, j)
            if interval_row is None:
                for colIdx in range(1, len(fill_row)):
                ...
                if fill_ts < interval_row[0]:
                    for colIdx in range(1, len(fill_row)):
                        val = self.find_prev_valid_val_for_col(interval_res_one_group, j-1, False, colIdx)
                    ...
                    for colIdx in range(1, len(fill_row)):
                        if interval_row[colIdx] is None:
                            val = self.find_prev_valid_val_for_col(interval_res_one_group, j, False, colIdx)
                        else:
