...
class TDTestCase:
    updatecfgDict = {'debugFlag': 135, 'asynclog': 0, 'checkpointinterval':60, 'snodeAddress': '127.0.0.1:873'}
    ...
    def initstream(self):
        ...
        # os.system("taosBenchmark -y -B 1 -t 4000 -S 500 -n 100  -v 3")
        # os.system("taosBenchmark -y -B 1 -t 40 -S 500 -n 100  -v 3 -a 3")
        os.system("taosBenchmark -y -B 1 -t 4 -S 500 -n 10  -v 2")
        # os.system("nohup taosBenchmark -y -B 1 -t 40000 -S 500 -n 10000  -v 3 -a 3 > /dev/null 2>&1 &")
        time.sleep(5)
        tdSql.execute("create snode on dnode 1")
        tdSql.execute("use test")
        tdSql.execute("create stream if not exists s1 trigger force_window_close watermark 5s into st1 tags(location varchar(255)) subtable(concat('yggl_base_',location))  as select _wstart,sum(voltage),groupid from meters partition by tbname,location interval(1s)")
        tdLog.debug("========create stream using snode and insert data ok========")
        self.get_dnode_info()

    def insertData(self):
        tdLog.debug("========insert data start========")
        tdSql.execute("insert into test.d1 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d2 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d3 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d4 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d5 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d6 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d7 values(now, 1, 1, 1)")
        tdSql.execute("insert into test.d8 values(now, 1, 1, 1)")
    ...
    def restartTaosd(self):
        ...
        for result in tdSql.queryResult:
            if result[2] == 'test' and result[3] == 'leader':
