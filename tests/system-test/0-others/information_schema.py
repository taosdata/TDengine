...
class TDTestCase:
    def init(self, conn, logSql, replicaVar=1):
        ...
        self.ins_list = ['ins_dnodes','ins_mnodes','ins_qnodes','ins_snodes','ins_cluster','ins_databases','ins_functions',\
            'ins_indexes','ins_stables','ins_tables','ins_tags','ins_columns','ins_users','ins_grants','ins_vgroups','ins_configs','ins_dnode_variables',\
                'ins_topics','ins_subscriptions','ins_streams','ins_stream_tasks','ins_vnodes','ins_user_privileges','ins_compacts', 'ins_compact_details']
    ...
    def ins_columns_check(self):
        ...
        tdLog.info(f'len(tdSql.queryResult) = %d' % len(tdSql.queryResult))
        tdSql.checkEqual(True, len(tdSql.queryResult) in(203, 204))
