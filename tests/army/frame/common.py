...
class TDCom:
    ...
    def generate_query_result(self, inputfile, test_case):
        if not os.path.exists(inputfile):
            ...
            os.system(f"~/dev/tdengine/debug/build/bin/taos -f {inputfile} | grep -v 'Query OK'|grep -v 'Copyright'| grep -v 'Welcome to the TDengine Command' > {self.query_result_file}  ")
