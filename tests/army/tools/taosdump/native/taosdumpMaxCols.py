...
# Tag types: same as column types (all 15 are valid as tags)
ALL_TAG_TYPES = ALL_COL_TYPES
...
class TDTestCase(TBase):
    ...
    def _testExportWideTable(self, binPath):
        ...
        dump_cmd = "%s -c %s --databases %s -o %s -T 1" % (
            binPath, self.cfgPath, dbName, dumpdir)
        tdLog.info("Dump command: %s" % dump_cmd)
        rc = os.system(dump_cmd)
        if rc != 0:
            tdLog.exit("taosdump export failed with rc=%d" % rc)
        ...
        for af in avro_files:
            ...
            if "ERROR" in output or "Unable" in output or "Cannot" in output:
                tdLog.exit("Inspect error on %s: %s"
                           % (os.path.basename(af), output[:500]))
    ...
    def _testRoundtripMaxCols(self, binPath):
        ...
        orig_stb_cnt = tdSql.getData(0, 0)
        tdSql.query("SELECT COUNT(*) FROM `%s`.`%s`" % (dbName, ntbName))
        orig_ntb_cnt = tdSql.getData(0, 0)
        ...
        for af in avro_files:
            ...
            if fsize < 100:
                tdLog.exit("Corrupted avro: %s (size=%d)" % (af, fsize))

        # -- Drop + restore --
        tdSql.execute("DROP DATABASE %s" % dbName)

        restore_cmd = "%s -c %s -i %s" % (binPath, self.cfgPath, dumpdir)
        tdLog.info("Restore command: %s" % restore_cmd)
        rc = os.system(restore_cmd)
        if rc != 0:
            tdLog.exit("taosdump import failed with rc=%d" % rc)
        ...
        if tdSql.queryRows != num_cols + num_tags:
            tdLog.exit("STB schema: expected %d rows, got %d"
                       % (num_cols + num_tags, tdSql.queryRows))
        ...
        for i in range(tdSql.queryRows):
            fname = tdSql.getData(i, 0)
            ftype = tdSql.getData(i, 1)
        ...
        for i in range(min(num_cols - 1, len(ALL_COL_TYPES))):
            ...
            if expected_type not in actual and actual not in expected_type:
                ...
                type_errors += 1
        if type_errors > 0:
            tdLog.exit("STB column type mismatches: %d" % type_errors)
        ...
        if case_check_name not in desc_map:
            tdLog.exit("Case-sensitive column name `%s` not found after restore!"
                       % case_check_name)
        tdLog.info("Case-sensitive column names preserved after round-trip")

        # Row counts
        tdSql.query("SELECT COUNT(*) FROM `%s`" % stbName)
        if tdSql.getData(0, 0) != orig_stb_cnt:
            tdLog.exit("STB count: expected %s, got %s"
                       % (orig_stb_cnt, tdSql.getData(0, 0)))
        tdLog.info("STB row count: %s" % orig_stb_cnt)

        tdSql.query("SELECT COUNT(*) FROM `%s`" % ntbName)
        if tdSql.getData(0, 0) != orig_ntb_cnt:
            tdLog.exit("NTB count: expected %s, got %s"
                       % (orig_ntb_cnt, tdSql.getData(0, 0)))
        ...
        for cname, (type_str, _, verify_fn) in verify_cols.items():
            ...
            if str(restored) != str(original):
                if not verify_fn(None, restored):
                    ...
                    mismatch += 1
        if mismatch > 0:
            tdLog.exit("STB column value mismatches: %d" % mismatch)
        ...
        for tname, (type_str, _, verify_fn) in verify_tags.items():
            ...
            if str(restored) != str(original):
                if not verify_fn(None, restored):
                    ...
                    mismatch += 1
        if mismatch > 0:
            tdLog.exit("STB tag value mismatches: %d" % mismatch)
        tdLog.info("STB tag values verified for all %d types" % len(verify_tags))

        # Normal table schema
        tdSql.query("DESCRIBE `%s`" % ntbName)
        if tdSql.queryRows != ntb_cols:
            tdLog.exit("NTB schema: expected %d, got %d"
                       % (ntb_cols, tdSql.queryRows))
        ...
        for cname, original in orig_ntb_vals.items():
            ...
            if str(restored) != str(original):
                ...
                if not verify_fn(None, restored):
                    ...
                    mismatch += 1
        if mismatch > 0:
            tdLog.exit("NTB column value mismatches: %d" % mismatch)
    ...
    def run(self):
        tdSql.prepare()
        binPath = frame.etool.taosDumpFile()
        if binPath == "":
            tdLog.exit("taosdump not found!")
        tdLog.info("taosdump found: %s" % binPath)

        # client cfg for taosdump to connect to framework taosd
        # binPath() returns .../debug/build/bin/, go up 3 levels to project root
        self.cfgPath = os.path.normpath(os.path.join(
            frame.epath.binPath(), "..", "..", "..", "sim", "psim", "cfg"))
        tdLog.info("cfgPath: %s" % self.cfgPath)
