...
@pytest.mark.sanity
def test_case_base_1(input_data):
    """
    用例概述: mongodb 用例, 测试分库分表，${Y}, ${M}
    数据源信息: 192.168.1.45:27017, 用户名密码:admin/tbase125!
    用例步骤：
    1. 使用 agent
    2. 配置数据库名: test_db1_${Y}, 表名: tb_${M}
    3. 查询模版: {"createtime":{"$gte":${start_datetime},"$lt":${end_datetime}}}
    4. 数据起始时间: '2020-01-01 00:00:00+08:00' - '2024-07-31 00:00:00+08:00'
    ...
    env_data, case_data_orig = input_data
    case_data = copy.deepcopy(case_data_orig)
    case_data["from"]["database"] = "test_db1_${Y}"
    case_data["from"]["collection"] = "tb_${M}"
    case_data["from"][
        "sql"
    ] = '{"createtime":{"$gte":${start_datetime},"$lt":${end_datetime}}}'
    case_data["from"]["start"] = "2020-01-01T00:00:00+08:00"
    case_data["from"]["end"] = "2024-07-31T00:00:00+08:00"
    mongodb_payload = Util.read_yaml("mongodb/payload-basic.yaml")["parser"]
    case_data["parser"] = mongodb_payload
    ...
    assert rows_count == 360, "入库的数据量应等于 360"
    TaosAdapter.drop_db(ENV.taosd_host, case_data["to"]["target_dbname"])


@pytest.mark.sanity
def test_case_base_2(input_data):
    """
    用例概述: mongodb 用例, 测试分库分表，${Y}, ${M}, ${D}
    数据源信息: 192.168.1.45:27017, 用户名密码:admin/tbase125!
    用例步骤：
    1. 使用 agent
    2. 配置数据库名: test_db2_${Y}, 表名: tb_${M}_${D}
    3. 查询模版: {"createtime":{"$gte":${start_datetime},"$lt":${end_datetime}}}
    4. 数据起始时间: '2020-01-01 00:00:00+08:00' - '2024-07-31 00:00:00+08:00'
    ...
    env_data, case_data_orig = input_data
    case_data = copy.deepcopy(case_data_orig)
    case_data["from"]["database"] = "test_db2_${Y}"
    case_data["from"]["collection"] = "tb_${M}_${D}"
    case_data["from"][
        "sql"
    ] = '{"createtime":{"$gte":${start_datetime},"$lt":${end_datetime}}}'
    case_data["from"]["start"] = "2020-01-01T00:00:00+08:00"
    ...
    assert rows_count == 1793, "入库的数据量应等于 1793"
    TaosAdapter.drop_db(ENV.taosd_host, case_data["to"]["target_dbname"])
