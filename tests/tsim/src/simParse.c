...
int32_t simCheckExpression(char *exp1) {
  char *  op1, *op2, *op, *rest;
  int32_t op1Len, op2Len, opLen;

  rest = paGetToken(exp1, &op1, &op1Len);
  if (op1Len == 0) {
  ...
  if (opLen == 0) return (int32_t)(rest - exp1);

  /* if it is key word "then" */
  if (strncmp(op, "then", 4) == 0) return (int32_t)(op - exp1);
