...
if [ "$build_no_asan" = "true" ]; then
    ...
    --rm --ulimit core=-1 ${DOCKER_IMAGE_NAME:-tdengine-ci:0.3} sh -c "cd $REP_DIR;rm -rf debug;mkdir -p debug;cd debug;cmake .. $BUILD_HTTP_OPT -DBUILD_TOOLS=true -DBUILD_TEST=true -DWEBSOCKET=true -DBUILD_TAOSX=false -DJEMALLOC_ENABLED=0 -DCMAKE_EXPORT_COMPILE_COMMANDS=1 ;make -j|| exit 1"
