...
class TDTestCase:
    ...
    def restartTaosd(self, index=1, dbname="db"):
        ...
        tdSql.execute(f"use stable_fun_iii")

    def run(self):
        print("running {}".format(__file__))
        tdSql.execute("drop database if exists stable_fun_iii")
        tdSql.execute("create database if not exists stable_fun_iii")
        tdSql.execute('use stable_fun_iii')
        ...
        tdSql.query('select * from (select SAMPLE(q_binary,396) from regular_table_2  SESSION(ts,19w) order by ts desc);')
        tdSql.checkRows(200)
        for i in range(0,200):
            assert((tdSql.getData(i, 1)).startswith('binary'))

        #tdSql.execute('drop database stable_fun_iii')
    def stop(self):
        tdSql.close()
