...
class TestStrBoundary(TDCase):
    ...
    def str_type_boundary_check(self):
        ...
        for data_type, data_value in self.boundary_dict.items():
            ...
            self.tdSql.checkEqual(self.tdRest.resp['data'][2][2], data_value)
            self.tdRest.request(f'select t1, c1 from {dbname}.tb')
            self.tdSql.checkEqual(str(self.tdRest.resp['data'][0][0]), max_length)
            self.tdSql.checkEqual(str(self.tdRest.resp['data'][0][1]), max_length)
            ...
            self.tdRest.error(f'create table if not exists {dbname}.tb using {dbname}.stb tags (now-2h, "{exceed_length}")')
            self.tdRest.error(f'insert into {dbname}.tb values (now-1h, "{exceed_length}")')
