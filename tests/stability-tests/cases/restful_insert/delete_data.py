...
class DeleteData(TDCase):
    ...
    def insert_base_data(self,col_type,tbname,rows,base_data,dbname):
        for i in range(rows):
            if col_type.lower() == 'tinyint':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["tinyint"]})')
            elif col_type.lower() == 'smallint':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["smallint"]})')
            elif col_type.lower() == 'int':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["int"]})')
            elif col_type.lower() == 'bigint':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["bigint"]})')
            elif col_type.lower() == 'tinyint unsigned':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["tinyint unsigned"]})')
            elif col_type.lower() == 'smallint unsigned':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["smallint unsigned"]})')
            elif col_type.lower() == 'int unsigned':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["int unsigned"]})')
            elif col_type.lower() == 'bigint unsigned':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["bigint unsigned"]})')
            elif col_type.lower() == 'bool':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["bool"]})')    
            elif col_type.lower() == 'float':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["float"]})')      
            elif col_type.lower() == 'double':
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts+i},{base_data["double"]})')
            elif 'binary' in col_type.lower():
                self.tdRest.request(f'''insert into {dbname}.{tbname} values({self.ts+i},"{base_data['binary']}")''')
            elif 'nchar' in col_type.lower():
                self.tdRest.request(f'''insert into {dbname}.{tbname} values({self.ts+i},"{base_data['nchar']}")''')        
    def delete_all_data(self,tbname,col_type,row_num,base_data,dbname,tb_type,tb_num=1):
        self.tdRest.request(f'delete from {dbname}.{tbname}')
        self.tdRest.request(f'flush database {dbname}')
        self.tdRest.request('reset query cache')
        self.tdRest.request(f'select * from {dbname}.{tbname}')
    ...
    def delete_one_row(self,tbname,column_type,column_name,base_data,row_num,dbname,tb_type,tb_num=1):
        self.tdRest.request(f'delete from {dbname}.{tbname} where ts={self.ts}')
        self.tdRest.request(f'flush database {dbname}')
        self.tdRest.request('reset query cache')
        ...
        if tb_type == 'ntb' or tb_type == 'ctb':
            if 'binary' in column_type.lower():
                self.tdRest.request(f'''insert into {dbname}.{tbname} values({self.ts},"{base_data['binary']}")''')
            elif 'nchar' in column_type.lower():
                self.tdRest.request(f'''insert into {dbname}.{tbname} values({self.ts},"{base_data['nchar']}")''')
            else:
                self.tdRest.request(f'insert into {dbname}.{tbname} values({self.ts},{base_data[column_type]})')
        elif tb_type == 'stb':
            for i in range(tb_num):
                if 'binary' in column_type.lower():
                    self.tdRest.request(f'''insert into {dbname}.{tbname}_{i} values({self.ts},"{base_data['binary']}")''')
                elif 'nchar' in column_type.lower():
                    self.tdRest.request(f'''insert into {dbname}.{tbname}_{i} values({self.ts},"{base_data['nchar']}")''')
                else:
                    self.tdRest.request(f'insert into {dbname}.{tbname}_{i} values({self.ts},{base_data[column_type]})')
        self.tdSql.query(f'select {column_name} from {dbname}.{tbname} where ts={self.ts}')
    ...
    def delete_rows(self,dbname,tbname,col_name,col_type,base_data,row_num,tb_type,tb_num=1):
        for i in range(row_num):
            self.tdRest.request(f'delete from {dbname}.{tbname} where ts>{self.ts+i}')
            self.tdRest.request(f'flush database {dbname}')
            self.tdRest.request('reset query cache')
        ...
        for i in range(row_num):
            ...
            if tb_type == 'ntb' or tb_type == 'ctb':
                ...
                for j in range(tb_num):
                    self.insert_base_data(col_type,f'{tbname}_{j}',row_num,base_data,dbname)
        for i in range(row_num):
            self.tdRest.request(f'delete from {dbname}.{tbname} where ts between {self.ts} and {self.ts+i}')
            self.tdRest.request(f'flush database {dbname}')
            self.tdRest.request('reset query cache')
            self.tdSql.query(f'select {col_name} from {dbname}.{tbname}')
            if tb_type == 'ntb' or tb_type == 'ctb':
                ...
                for j in range(tb_num):
                    self.insert_base_data(col_type,f'{tbname}_{j}',row_num,base_data,dbname)
            self.tdRest.request(f'delete from {dbname}.{tbname} where ts between {self.ts+i+1} and {self.ts}')
            self.tdSql.query(f'select {col_name} from {dbname}.{tbname}')
            if tb_type == 'ntb' or tb_type == 'ctb':
                self.tdSql.checkRow(row_num)
            elif tb_type == 'stb':
    ...
    def delete_data_ntb(self):
        ...
        for col_name,col_type in self.column_dict.items():
            self.tdRest.request(f'create table {self.dbname}.{self.ntbname} (ts timestamp,{col_name} {col_type})')
            ...
            self.tdRest.request(f'drop table {self.dbname}.{self.ntbname}')
    ...
    def delete_data_ctb(self):
        ...
        self.tdRest.request(f'use {self.dbname}')
        for col_name,col_type in self.column_dict.items():
            self.tdRest.request(f'create table {self.dbname}.{self.stbname} (ts timestamp,{col_name} {col_type}) tags(t1 int)')
            for i in range(self.tbnum):
                self.tdRest.request(f'create table {self.dbname}.{self.stbname}_{i} using {self.stbname} tags(1)')
            ...
            self.tdRest.request(f'drop table {self.stbname}')
    def delete_data_stb(self):
        self.tdRest.request(f'drop database if exists {self.dbname}')
        self.tdCom.createDb(self.dbname)
        self.tdRest.request(f'use {self.dbname}')
        for col_name,col_type in self.column_dict.items():
            self.tdRest.request(f'create table {self.stbname} (ts timestamp,{col_name} {col_type}) tags(t1 int)')
            for i in range(self.tbnum):
                self.tdRest.request(f'create table {self.stbname}_{i} using {self.stbname} tags(1)')
                self.insert_base_data(col_type,f'{self.stbname}_{i}',self.rowNum,self.base_data,self.dbname)
            ...
            self.tdRest.request(f'drop table {self.dbname}.{self.stbname}')
        self.tdRest.request(f'drop database {self.dbname}')
    def run(self):
