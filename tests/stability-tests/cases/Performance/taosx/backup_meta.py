...
class FullBackup(TDCase):
    ...
    def full_backup_db_stb(self):
        for source_task in ['']:
            for target_task in ['']:
                ...
                self.Insert_file.get_process_exporter_info(env_setting, 1, timestamp_start, timestamp_end)
                self.Insert_file.get_node_exporter_info(env_setting, 1, timestamp_start, timestamp_end)
                time.sleep(5)
                thread_list_target = []
                for source in range(len(self.source_taosd_list)):
                    timestamp_start = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
    ...
    def run(self):
        ...
        self.tdTaosx.data_insert(self.source_taosd_list,self.dbname,self.stbname,self.tbname_m,self.tb_num,self.row_num,self.start_timestamp,self.drop_flag,self.child_table_exist_flag,self.taosBenchmark_fqdn,self.test_root,self.replica,case_root_flag='perf')
