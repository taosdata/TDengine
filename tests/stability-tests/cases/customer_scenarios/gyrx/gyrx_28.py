...
class Demo(TDCase):
    def init(self):
        ...
        self.restart_interval = 10  # 节点重启间隔(秒)
    ...
    def node_restart_operations(self):
        ...
        while time.time() - start_time < self.test_duration:
            try:
                ...
                if taosd_iplist:
                    target_node = taosd_iplist[1]  # 选择第一个节点
                    dnode_name = f"{target_node}:6030"  # 构造dnode名称，按照你提供的格式
                    
                    self.logger.info(f"准备重启节点: {dnode_name}")
