...
class Demo(TDCase):
    ...
    def run(self):
        ...
        # 执行查询并获取结果
        self.tdSql.query(f"show table distributed {self.dbname}.{self.stbname}")
        result = self.tdSql.query_data  # 修复：使用正确的属性名
        
        # 提取压缩比
        compression_ratio = 0
        if result and len(result) > 0:
            result_str = str(result[0])
            if "Compression_Ratio=[" in result_str:
                ratio_str = result_str.split("Compression_Ratio=[")[1].split("%]")[0]
                try:
                    compression_ratio = float(ratio_str)
                except:
                    self.logger.warning("无法解析压缩比")
        
        # 记录到日志
        self.json_log["compression_ratio"] = f"表 {self.dbname}.{self.stbname} 的压缩比为 {compression_ratio}%"
