...
class Container:
    ...
    def destroy_docker_net(self, config):
        self.remote.cmd(config["fqdn"][0], [f'docker stop $(docker ps -q)'])
    ...
    def run_container(self, config, container_name, host, net_ip):
        #cfg  fqdn    ,kill  taosd rm data restart tassd
        ...
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sh -c \"ps -ef | grep taosd | grep -v grep | awk \'{{print \$2}}\' | xargs -r kill -9\"']) 
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sh -c \"ps -ef | grep taosadapter | grep -v grep | awk \'{{print \$2}}\' | xargs -r kill -9\"']) 
        time.sleep(2)    
        #self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/# firstEp/firstEp/g" /etc/taos/taos.cfg && sed -i "s/hostname/taostest_net_host_0/g" /etc/taos/taos.cfg && sed -i "s/buildkitsandbox/{container_name}/g" /etc/taos/taos.cfg '])
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/# firstEp/firstEp/g" /etc/taos/taos.cfg'])
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/hostname/taostest_net_host_0/g" /etc/taos/taos.cfg'])
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/buildkitsandbox/{container_name}/g" /etc/taos/taos.cfg '])
        #self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sed -i "s/buildkitsandbox/{container_name}/g" /etc/taos/taoskeeper.toml '])
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} rm -rf /var/lib/taos/'])
        time.sleep(2)  
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d {container_name} sh -c \'taosd\''])
        time.sleep(2)  
        #删除错误节点
        #query_result=self.remote.cmd(config["fqdn"][0], [f'docker exec -ti taostest_net_host_0 sh -c \'taos -s \"select id from information_schema.ins_dnodes where support_vnodes =0\";\''])
        #print(query_result)
        dnode_id = str(self.remote.cmd(config["fqdn"][0], [f'docker exec -ti taostest_net_host_0 sh -c \'taos -s \"select id from information_schema.ins_dnodes where support_vnodes =0\";\''])).split("==============")[1].split("|")[0]
        self.remote.cmd(config["fqdn"][0], [f'docker exec -d taostest_net_host_0 sh -c \'taos -s \"drop dnode {dnode_id}";\''])
    ...
    def gen_host_setting_dict(self, config, container_count):
        ...
        for i in range(container_count):
            host_setting_dict[f'{config["net_name"]}_host_{i}'] = f'{ip_prefix}.{ip_suffix+i+2}'
...
class XTYTest:
    ...
    def start_container_dnodes(self):
        ...
        # res = self._remote.cmd(self.container_setting["fqdn"][0], [f'docker exec -ti {host_info} sh -c \'taos -s \"select distinct(status) from information_schema.ins_dnodes where endpoint not like "\\\"build%\\\"\"\''])
        res = self._remote.cmd(self.container_setting["fqdn"][0], [f'docker exec -ti {host_info} sh -c \'taos -s \"select status from information_schema.ins_dnodes where support_vnodes >=0";\''])
        ready_flag = 0
    ...
    def restart_container_dnodes(self):
        ...
        self.common.multi_thread_run(run_taosd_tlist)
        res = self._remote.cmd(self.container_setting["fqdn"][0], [f'docker exec -ti {host_info} sh -c \'taos -s \"select distinct(status) from information_schema.ins_dnodes where support_vnodes >=1";\''])
        ready_flag = 0
...
if  __name__ == '__main__':
    container_config = {"fqdn": ["xintongyuan"], "net_name":"taostest_net", "subnet": "172.12.0.0/16", "image": "tdengine/tdengine", "dnode_count": 100}
