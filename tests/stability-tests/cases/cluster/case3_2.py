...
class Case3_1(ClusterCase):
    ...
    def install_test_frame_dnode_5(self):

        self.envMgr._remote.cmd("dnode_5", [
                                "pip3 install --extra-index-url http://192.168.1.131:8080/simple --trusted-host 192.168.1.131  taostest"])
        self.envMgr._remote.cmd("dnode_5", ["export TEST_ROOT={}".format(os.environ["TEST_ROOT"])])
    ...
    def start_case(self , case_cmd):
        ...
        result = self.envMgr._remote.cmd2("dnode_5", ["export TEST_ROOT={}".format(os.environ["TEST_ROOT"]),case_cmd])
    ...
    def run(self):
        ...
        # prepare test_frame_work for run case at dnode_5 ,default dnode_5 has not deploy
        self.install_test_frame_dnode_5()
    ...
    def desc(self) -> str:
        case_description = '''
            [test]<wenzhouwww> test case for cluster about 3.2 , restart taosd and basic query task and restart taosd of master mnode ;
        '''
