...
int buildDatabase(TAOS* pConn, TAOS_RES* pRes) {
  pRes = taos_query(
      pConn,
      "create stable if not exists st1 (ts timestamp, c1 int, c2 float, c3 binary(16), b blob) tags(t1 int, t3 "
      "nchar(8), t4 bool)");
  if (taos_errno(pRes) != 0) {
  ...
  pRes = taos_query(pConn, "insert into ct3 using st1(t1) tags(3000) values(1626006833600, 5, 6, 'c', 'aaaaaaa')");
  if (taos_errno(pRes) != 0) {
