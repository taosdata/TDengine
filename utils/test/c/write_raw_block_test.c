...
int32_t test_write_raw_block_with_fields(char* query, char* dst) {
  ...
  TdFilePtr pFile = taosOpenFile("../../../utils/test/c/data_3460", TD_FILE_READ);
  ASSERT(pFile != NULL);
  int32_t size = 3460;
  ...
  error_code = taos_write_raw_block_with_fields(pConn, numOfRows, pMemBuf, dst, fields, numFields);
  taos_free_result(pRes);
