...
int buildDatabase(TAOS* pConn, TAOS_RES* pRes) {
  pRes = taos_query(
      pConn,
      "create stable if not exists st1 (ts timestamp, c1 int, c2 float, c3 binary(16), c4 decimal(10,3), b blob) "
      "tags(t1 int, t3 "
      "nchar(8), t4 bool)");
  if (taos_errno(pRes) != 0) {
  ...
  pRes = taos_query(
      pConn, "insert into ct0 using st1 tags(1000, \"ttt\", true) values(1626006833400, 1, 2, 'a', 3.2, 'aaaaaaaa')");
  if (taos_errno(pRes) != 0) {
  ...
  pRes = taos_query(pConn,
                    "insert into ct1 using st1(t1) tags(2000) values(1626006833600, 3, 4, 'b', 4.32, 'bbbbbbbbbb')");
  if (taos_errno(pRes) != 0) {
  ...
  pRes = taos_query(
      pConn,
      "insert into ct0 using st1 tags(1000, \"ttt\", true) values(1626006833400, 1, 2, 'a', 23.23, 'cccccccccc')");
  if (taos_errno(pRes) != 0) {
    printf("failed to insert into ct0, reason:%s\n", taos_errstr(pRes));
    return -1;
  }
  ...
  pRes = taos_query(pConn,
                    "insert into ct1 using st1(t1) tags(2000) values(1626006833600, 3, 4, 'b', 43.53, 'ddddddddd')");
  if (taos_errno(pRes) != 0) {
    printf("failed to insert into ct1, reason:%s\n", taos_errstr(pRes));
    return -1;
  }
  ...
  pRes = taos_query(
      pConn,
      "insert into ct3 using st1(t1) tags(3000) values(1626006833600, 5, 6, 'c', 43.53, 'eeeeeee') ct1 using "
      "st1(t1) tags(2000) values(1626006833601, 2, 3, 'sds', 43.53, 'ffffffff') (1626006833602, 4, 5, "
      "'ddd', 43.53, 'gggggggg') ct0 using st1 tags(1000, \"ttt\", true) values(1626006833603, 4, 3, 'hwj', 43.53, "
      "'xxxxxxxx') "
      "ct1 using st1(t1) tags(2000) values(now+5s, 23, 32, 's21ds', 43.53, 'zzzzzzzzzzzzz')");
  if (taos_errno(pRes) != 0) {
