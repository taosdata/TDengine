...
- taosinfo    存放重要信息日志, 不包括 DEBUG、TRACE级别以外的信息。包括：INFO/ERROR/WARNING
- taosdlog    服务器端生成的日志，存储DEBUG、TRACE级别日志以外，根据设置的日志输出级别，记录DEBUG、TRACE级别的日志（如果设置的日志级别是 135）。
- taoslog     客户端（driver）生成的日志，存储DEBUG、TRACE级别日志以外，根据设置的日志输出级别，记录DEBUG、TRACE级别的日志（如果设置的客户端配置文件中日志级别是 135）。
