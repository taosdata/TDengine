...
### 在 Docker 容器中运行 TDengine server 用于开发验证（不对容器外部提供服务）
...
### 在 Docker 容器中运行 TDengine server 对容器外部提供服务
...
注意：如果容器中运行 taosAdapter，需要根据需要增加映射其他端口，具体端口请参考[taosAdapter文档](https://github.com/taosdata/taosadapter/blob/develop/README-CN.md)。
