...
`taos-jdbcdriver` 的实现包括两种形式： JDBC 本地连接和 JDBC-RESTful（taos-jdbcdriver-2.0.18 开始支持 JDBC-RESTful）。 JDBC 本地连接通过调用客户端驱动 libtaos.so（或 taos.dll ）的本地方法实现， JDBC-RESTful 则在内部封装了 RESTful 接口实现。
...
2. Java 1.8 以上运行时环境
3. maven 3.6 以上版本
4. TDengine-client（使用 JDBC 本地连接时必须，使用 JDBC-RESTful 时非必须）
