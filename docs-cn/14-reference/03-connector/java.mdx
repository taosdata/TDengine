...
`taos-jdbcdriver` 的实现包括两种形式： JDBC 本地连接和 JDBC-REST（taos-jdbcdriver-2.0.18 开始支持 JDBC-RESTful）。 JDBC 本地连接通过调用客户端驱动 libtaos.so（或 taos.dll ）的本地方法实现， JDBC-RESTful 则通过封装 TDengine REST 服务接口建立连接，REST 连接实现的功能集合和本地连接有少量不同。
...
4. 已安装 TDengine-client（使用 JDBC 本地连接必须安装，使用 JDBC-RESTful 无需安装）
...
**注意**：部分配置项（比如：locale、timezone）在 RESTful 连接中不生效。
...
- TSDBDriver.PROPERTY_KEY_CONFIG_DIR：仅在使用 JDBC 本地连接时生效。客户端配置文件目录路径，Linux OS 上默认值 `/etc/taos`，Windows OS 上默认值 `C:/TDengine/cfg`。
- TSDBDriver.PROPERTY_KEY_CHARSET：仅在使用 JDBC 本地连接时生效。 客户端使用的字符集，默认值为系统字符集。
- TSDBDriver.PROPERTY_KEY_LOCALE：仅在使用 JDBC 本地连接时生效。 客户端语言环境，默认值系统当前 locale。
- TSDBDriver.PROPERTY_KEY_TIME_ZONE：仅在使用 JDBC 本地连接时生效。 客户端使用的时区，默认值为系统当前时区。
