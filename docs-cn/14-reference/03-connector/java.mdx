...
`taos-jdbcdriver` 中 TSDBDriver 实现了 JDBC driver 标准接口，支持通过客户端驱动程序（taosc）原生连接 TDengine 集群，支持数据写入、查询、订阅、schemaless 接口和参数绑定接口等功能。`taos-jdbcdriver` 丛 2.0.18 版本增加了 RestfulDriver，支持使用 REST 连接 TDengine 集群。REST 连接实现的功能集合和原生连接有少量不同。
