...
| develop | database/sql（cgo & restful）、订阅、schemaless写入、stmt、restful | 2.4.0.4及以上  |
...
`master` 分支建议只使用 `database/sql` 功能。

### RESTful 连接

RESTful 连接指连接器通过 taosAdapter 组件提供的 RESTful API 建立与 taosd 的连接。
