...
C# connector 目前是通过 TDengine 客户端驱动建立本地连接，进而可以访问 TDegnine 数据库。目前已支持 TDengine 的绝大部分特性例如：同步查询、异步查询、管理连接、订阅消费，schemaless 等。
通过阅读本文可以了解如何在 Windows 或者 Linx 环境中安装 TDengine C# connector，以及如何连接 TDengine 数据库、查询数据、插入数据等基本操作。
但是对于 RESTful 连接暂未封装，需要用户参考 [RESTful APIs](https://docs.taosdata.com//reference/restful-api/) 自己封装。
...
|--------------------|:-------------------------------------:|--------------------------------|
|        1.0.2       | v2.5.0.x,v2.4.0.x,v2.3.4+ | 支持连接管理、同步查询、错误信息   |
|        1.0.3       | v2.5.0.x,v2.4.0.x,v2.3.4+ | 支持参数绑定、schemaless, json tag |
|        1.0.4       | v2.5.0.x,v2.4.0.x,v2.3.4+ | 支持异步查询，订阅、修复绑定参数     |
|        1.0.5       | v2.5.0.x,v2.4.0.x,v2.3.4+ | 修复 Windows 同步查询中文报错   |
|        1.0.6       | v2.5.0.x,v2.4.0.x,v2.3.4+ | 修复schemaless |
...
**注意**：由于 TDengine 的应用驱动是使用 C 语言开发的，使用 C# 驱动包时需要依赖系统对应的客户端驱动。

- TDengine 客户端驱动在 Linux 系统中成功安装 TDengine 客户端之后，依赖的本地客户端驱动 libtaos.so 文件会被自动拷贝至 /usr/lib/libtaos.so，该目录包含在 Linux 自动扫描路径上，无需单独指定。
- TDengine 客户端驱动在 Windows 系统中安装完 TDengine 客户端之后，驱动包依赖的 taos.dll 文件会自动拷贝到系统默认搜索路径 C:/Windows/System32 下，同样无需要单独指定。
...
|示例程序                                                                                                               | 示例程序描述                                       |
|--------------------------------------------------------------------------------------------------------------------|--------------------------------------------|
| [C#checker](https://github.com/taosdata/TDengine/tree/develop/examples/C%23/C%23checker)                           | 使用 C# Driver 可以根据 help 命令给出的参数列表，测试C# Driver的同步插入查询    |
| [TDengineTest](https://github.com/taosdata/TDengine/tree/develop/examples/C%23/TDengineTest)                       | 使用 C# Driver 的简单插入查询示例 |
...
| [schemaless](https://github.com/taosdata/TDengine/tree/develop/examples/C%23/schemaless)                           | 使用 C# Driver 使用 schemaless 的示例 |
| [benchmark](https://github.com/taosdata/TDengine/tree/develop/examples/C%23/taosdemo)                               | C# 版简易 benchmark |
...
* "Unable to establish connection"，"Unable to resolve FQDN"， 一般都是应为为配置 FQDN 可以参考[如何彻底搞懂 TDengine 的 FQDN](https://www.taosdata.com/blog/2021/07/29/2741.html)解决。

* Unhandled exception. System.DllNotFoundException: Unable to load DLL 'taos' or one of its dependencies: 找不到指定的模块。 原因：程序没有找到依赖的客户端驱动。 解决方法：Windows 下可以将 C:\TDengine\driver\taos.dll 拷贝到 C:\Windows\System32\ 目录下，Linux 下将建立如下软链 ln -s /usr/local/taos/driver/libtaos.so.x.x.x.x /usr/lib/libtaos.so 即可。
