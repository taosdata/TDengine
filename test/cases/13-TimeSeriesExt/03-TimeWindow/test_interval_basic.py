...
class TestInterval:
    ...
    def check_surround_stream(self):
        ...
        stream = StreamItem(
            ...
            res0 as select _twstart, _twend, _wstart, _wend, first(c1), max(c1) from ntb
            where ts >= _twstart and ts < _twend
            interval(1d) fill(prev) """
        )
        ...
        stream = StreamItem(
            ...
            res1 as select _twstart, _twend, _wstart, _wend, first(c1), max(c1) from stb
            where ts >= _twstart and ts < _twend
        ...
        stream = StreamItem(
            ...
            res3 as select _twstart, _twend, count(*) from ntb
            where ts >= _twstart and ts < _twend"""
        )
        streams.append(stream)
