...
class TestWindowProjection:
    ...
    def test_window_projection(self):
        """Window Projection

        1. Test the window query with projection select list

        Catalog:
            - Timeseries:WindowProjection
        ...
        History:
            - 2026-05-11 Jing Sima Created file

        """
        self.fill_error_case()
        self.agg_mixed_scalar_error()
        self.scalar_agg_mode()
        self.edge_inans()
        self.small_data_inans()
        self.fill_inans()
        self.largedata_inans()
        self.partition_inans()
    ...
    def fill_error_case(self):
        self.prepare_small_data()

        # Verify FILL(PREV) is rejected in projection-mode window queries that contain raw columns without aggregate functions.
        tdSql.error(
        ...
        # Verify FILL(NEXT) is rejected in projection-mode window queries that contain raw columns without aggregate functions.
        tdSql.error(
            "SELECT _wstart, ts, current FROM d1001 "
            "WHERE ts >= '2024-01-01 00:00:00' AND ts < '2024-01-01 00:00:15' "
        ...
        # Verify FILL(LINEAR) is rejected in projection-mode window queries that contain raw columns without aggregate functions.
        tdSql.error(
            "SELECT _wstart, ts, current FROM d1001 "
            "WHERE ts >= '2024-01-01 00:00:00' AND ts < '2024-01-01 00:00:15' "
        ...
        # Verify FILL(NEAR) is rejected in projection-mode window queries that contain raw columns without aggregate functions.
        tdSql.error(
            "SELECT _wstart, ts, current FROM d1001 "
            "WHERE ts >= '2024-01-01 00:00:00' AND ts < '2024-01-01 00:00:15' "
    ...
    def agg_mixed_scalar_error(self):
        """Aggregate mixed with raw column/scalar func error

        Verify that mixing raw columns with aggregate functions in a window query is correctly rejected.
        """
        self.prepare_small_data()
    ...
    def scalar_agg_mode(self):
        """Small-data in/ans validation with scalar/agg mode

        Run 82 queries on 3x10 rows + null table covering 3 window mode,
        basic 5 window types, super table, scalar expressions, pseudo cols,
        FILL, WHERE, HAVING, ORDER BY, LIMIT, subqueries, csum, child table HAVING,
        ...
        sqlFile = os.path.join(
            ...
            "test_window_projection_mode.in"
        )
        ansFile = os.path.join(
            os.path.dirname(__file__), "ans",
            "test_window_projection_mode.ans"
        )
        tdCom.compare_testcase_result(
            sqlFile, ansFile, "test_window_projection_mode"
        )

    def small_data_inans(self):
    ...
    def fill_inans(self):
        """Fill in projection mode in/ans validation

        Run fill queries (NULL/NULL_F/NONE/VALUE/VALUE_F) on the small
        dataset covering single/multi columns, scalar expressions, pseudo
        cols, partition by, HAVING, ORDER BY, LIMIT, subqueries, csum,
        NULL-data tables, leading/trailing fill, and edge ranges.
        """
        self.prepare_small_data()
        tdSql.execute("USE test_wp_small")
        tdSql.execute("CREATE TABLE IF NOT EXISTS d_null USING meters TAGS('NullCity', 9)")
        tdSql.execute(
            "INSERT INTO d_null VALUES"
            "('2024-01-01 00:00:00', NULL, 220, 0.300),"
            "('2024-01-01 00:00:01', 1.50, NULL, 0.300),"
        ...
        sqlFile = os.path.join(
            os.path.dirname(__file__), "in",
            "test_window_projection_fill.in"
        )
        ansFile = os.path.join(
            os.path.dirname(__file__), "ans",
            "test_window_projection_fill.ans"
        )
        tdCom.compare_testcase_result(
            sqlFile, ansFile, "test_window_projection_fill"
        )

    def largedata_inans(self):
