...
class TestTbname:
    ...
    def ts6532(self, dbname="db"):
        ...
        tdSql.checkData(0, 1, "CREATE STABLE ```s``t``` (`ts` TIMESTAMP ENCODE 'delta-i' COMPRESS 'lz4' LEVEL 'medium', ```v1``` INT ENCODE 'simple8b' COMPRESS 'lz4' LEVEL 'medium') TAGS (```t``1``` INT) SECURE_DELETE 0")
        showCreateResult = tdSql.getData(0, 1)
        ...
        tdSql.checkData(0, 1, "CREATE STABLE ```vstb``100``` (```ts``` TIMESTAMP ENCODE 'delta-i' COMPRESS 'lz4' LEVEL 'medium', ```c``0``` INT ENCODE 'simple8b' COMPRESS 'lz4' LEVEL 'medium', ```c``1``` INT ENCODE 'simple8b' COMPRESS 'lz4' LEVEL 'medium') TAGS (```t0``` INT, `t``1``` VARCHAR(20)) VIRTUAL 1 SECURE_DELETE 0")
