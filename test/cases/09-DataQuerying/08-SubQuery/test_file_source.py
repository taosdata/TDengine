...
class TestFileSource:
    ...
    def test_file_source_no_ts(self):
        ...
        Contrast with TEXT(): TEXT no longer requires the first column to be TIMESTAMP
        (aligned with FILE behaviour; tested in test_text_source_no_ts).

        Positive cases (file_source_no_ts.csv: id INT, name VARCHAR(16), score FLOAT):
        - SELECT all columns ORDER BY id ASC
        - ORDER BY non-ts column (score DESC)
        - GROUP BY name, COUNT(id)

        Negative cases:
        - INTERVAL window on non-ts FILE: rejected (no primary timestamp column)
        - JOIN with non-ts FILE: rejected (JOIN requires primary timestamp column)
