...
class TestQuantifiedSubQuery2:
    ...
    sqlIdx = 0
    maxColumnIdx = 20
    querySql = "select f1 from {srcTableName} where {row} {compareExpr} {quantifiedExpr} (select {row2} from {targetTableName} {filterExpr});"
    queryConstSql = "select f1 from {srcTableName} where {constExpr} {compareExpr} {quantifiedExpr} (select {row2} from {targetTableName} {filterExpr});"
    ...
    def test_quantified_sub_query2(self):
        ...
        1. Prepare data.
        2. Execute various where clause with different kind of quantified expr sub queries.
