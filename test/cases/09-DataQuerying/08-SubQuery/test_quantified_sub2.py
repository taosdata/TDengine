...
class TestQuantifiedSubQuery2:
    ...
    caseName = "test_quantified_sub_query2"
    ...
    subSqls = [
        ...
        "select case when f1 = 1 then 2 / {quantifiedSql} else 2 end, sum(f1) from {tableName} group by f1",
        "select case when f1 = 1 then 1 else {quantifiedSql} is null end, sum(f1) from {tableName} group by f1",
        "select _wstart, {quantifiedSql}, avg(f1) from {tableName} interval(1d)",
        "select ({quantifiedSql}) + 1, avg(f1) from {tableName} interval(1d)",
        "select min({quantifiedSql}), avg(f1) from {tableName} interval(1d)",
        "select max(({quantifiedSql}) + 1), avg(f1) from {tableName} interval(1d)",
        ...
        "select ({quantifiedSql}) + 1, avg(f1) from {tableName} SESSION(ts, 1d)",
        "select {quantifiedSql} > 0 from {tableName} SESSION(ts, 1d)",
        "select sum(case when {quantifiedSql} > 0 then 1 else 2 end) from {tableName} STATE_WINDOW(f1)",
        "select sum(case when f1 = 1 then cast(2 / {quantifiedSql} as bigint) else 2 end) from {tableName} EVENT_WINDOW START WITH f1 >= 1 END WITH f1 <= 2",
        "select sum(case when f1 = 1 then 1 else {quantifiedSql} is null end + 1) from {tableName} COUNT_WINDOW(2)",
        "select sum({quantifiedSql}) from {tableName} partition by tbname interval(1d)",
        "select avg(abs({quantifiedSql} * -1)) from {tableName} partition by f1 interval(1d)",
        "select f1, {quantifiedSql} from {tableName} group by f1",
        "select f1, {quantifiedSql}, avg({quantifiedSql}) from {tableName} group by f1",

        # from
        "select b.f1, {quantifiedSql} + 1 from {tableName} a join {tableName} b on a.ts = b.ts and {quantifiedSql} order by b.f1",
        "select {quantifiedSql} + 1, avg({quantifiedSql} + 2) from {tableName} a left join {tableName} b on a.ts = b.ts and {quantifiedSql}",
        ...
        "select f1 from {tableName} where f1 in ({quantifiedSql}) order by f1",
        "select f1 from {tableName} where f1 not in ({quantifiedSql}) order by f1",
        ...
        # group
        "select sum(f1) from {tableName} group by {quantifiedSql}, f1 having(({quantifiedSql}) > 2) order by 1",
    ...
    def test_quantified_sub_query2(self):
