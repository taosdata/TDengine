...
class TestFq18SourceSpecific(FederatedQueryVersionedMixin):
    ...
    def test_fq_source_specific_queries(self):
        """Source-specific positive query cases: InfluxDB tags + PARTITION BY TBNAME.
        ...
        failed = self._run_specific_cases(
            [(cid, sql, opts)
             ...
             if pos and "min_count" not in opts]
        )
        if failed:
            all_errors = "\n".join(
    ...
    def test_fq_source_specific_infoschema(self):
        """MySQL INFORMATION_SCHEMA introspection via federated query.

        Verifies that TDengine federated query can access MySQL
        INFORMATION_SCHEMA.TABLES as a virtual external table.  The expected
        count is ≥ 1 (not an exact value) because INFORMATION_SCHEMA may
        contain additional system tables beyond the user-created one.
        ...
        History:
            - 2026-06-xx wpan  Migrated from fq_04 test_fq_sql_infoschema
        """
        for case_id, sql_template, pos, _, opts in _CASES:
            if not pos or "min_count" not in opts:
                continue
            min_count = opts["min_count"]
            sql = self._fmt(sql_template)
            tdSql.query(sql)
            tdSql.checkRows(1)
            actual = int(tdSql.getData(0, 0))
            assert actual >= min_count, (
                f"[{case_id}] Expected count ≥ {min_count}, got {actual}\n  SQL: {sql}"
            )
            tdLog.info(
                f"[{case_id:<9s} POS] PASS  count={actual} ≥ {min_count}  "
                f"(db={self._M_INFO_DB!r})"
            )

    def test_fq_source_specific_errors(self):
        """Source-specific error cases: PARTITION BY TBNAME on MySQL/PG + InfluxDB 1.8.
