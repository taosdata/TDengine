...
class TestFq10Performance(FederatedQueryVersionedMixin):
    ...
    def setup_class(self):
        ...
        # (inline SQL instead of self._teardown_data() which fails when self is the class)
        ...
        tdSql.execute("drop database if exists perf_timeout_src")
        tdSql.execute("drop external source if exists perf_timeout_src")
        tdSql.execute("drop database if exists perf_retry_src")
        tdSql.execute("drop external source if exists perf_retry_src")
    ...
    def teardown_class(self):
        ...
        try:
            ...
            TestFq10Performance._print_summary(self)
