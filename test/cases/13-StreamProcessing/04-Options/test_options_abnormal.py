...
class TestStreamOptionsTrigger:
    ...
    class Basic0(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                f"create stream sn7 state_window(cint) from ntb options(pre_filter(cint < 5 and cvarchar like '%abc%')) into res_ntb (firstts, lastts, cnt_v, sum_v, avg_v) as select first(_c0), last_row(_c0), count(cint), sum(cint), avg(cint) from %%trows;"
            )
