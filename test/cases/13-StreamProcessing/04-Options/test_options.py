...
class TestStreamOptionsTrigger:
    ...
    class Basic1(StreamCheckItem):
        ...
        def create(self):
            ...
            # tdSql.execute(
            #     f"create stream s1_g state_window(cint) from {self.vstbName} partition by tbname, tint stream_options(expired_time(10s)) into res_vstb OUTPUT_SUBTABLE(CONCAT('res_vstb_', tbname)) (firstts, lastts, cnt_v, sum_v, avg_v) as select first(_c0), last_row(_c0), count(cint), sum(cint), avg(cint) from %%trows;"
            # )
