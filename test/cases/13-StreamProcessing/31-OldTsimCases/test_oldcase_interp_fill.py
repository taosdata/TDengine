...
class TestStreamOldCaseInterpFill:
    ...
    def streamInterpLarge(self):
        ...
        tdSql.execute(
            f"create stream streams1 interval(1s) sliding(1s) from t1 stream_options(max_delay(1s)) into streamt as select _irowts, interp(a), interp(b), interp(c), interp(d) from t1 range(_twstart) fill(prev);"
        )
