...
class TestStreamOldCaseConcat:
    ...
    class Col00(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.error(
                "create stream streams13 interval(10s) sliding(10s) from st partition by tbname, tc stream_options(max_delay(3s)) into streamt13(a, b, c, d) tags(tx varchar(60)) as select _twstart, count(*) c1, max(a) c2, max(b) from %%trows where ts >= _twstart and ts < _twend;"
            )
            tdSql.error(
    ...
    class Tag00(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams1 interval(10s) sliding(10s) from st partition by tbname stream_options(max_delay(3s)) into result.streamt OUTPUT_SUBTABLE(concat("aaa-", %%tbname)) as select _twstart, count(*) c1 from %%tbname;'
            )
    ...
    class Tag01(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams2 interval(10s) sliding(10s) from st partition by tbname stream_options(max_delay(3s)) into result2.streamt2 output_subtable(concat("tag-", %%1)) TAGS(cc varchar(100) as concat("tag-", %%tbname)) as select _twstart, count(*) c1 from %%trows;'
            )
    ...
    class Tag02(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams3 interval(10s) sliding(10s) from st partition by tbname stream_options(max_delay(3s)) into result3.streamt3 output_subtable(concat("tbn-", %%tbname)) tags(dd varchar(100) as concat("tag-", %%tbname)) as select _twstart, count(*) c1 from %%trows;'
            )
    ...
    class Tag03(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams4 interval(10s) sliding(10s) from st partition by tbname stream_options(max_delay(3s)) into result4.streamt4 OUTPUT_SUBTABLE(concat("tbn-", %%tbname)) TAGS(dd varchar(100) as concat("tag-", %%tbname)) as select _twstart, count(*) c1 from %%trows;'
            )
    ...
    class Tag04(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams6 interval(10s) sliding(10s) from st partition by ta, tbname stream_options(max_delay(3s)) into result6.streamt6 TAGS(dd int as cast(concat(%%1, "0") as int)) as select _twstart, count(*) c1 from %%trows;'
            )
    ...
    class Tag20(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                'create stream streams1 interval(10s) sliding(10s) from st partition by tbname stream_options(max_delay(3s)) into result.streamt OUTPUT_SUBTABLE("aaa") as select _twstart, count(*) c1 from st ;'
            )
