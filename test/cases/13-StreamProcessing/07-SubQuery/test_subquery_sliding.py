...
class TestStreamSubquerySliding:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=37,
            stream="create stream rdb.s37 interval(5m) sliding(5m) from tdb.triggers into rdb.r37 as select _tcurrent_ts tc,  _tlocaltime - _tcurrent_ts tx1, _tcurrent_ts-_tprev_ts tx2, _tnext_ts-_tcurrent_ts tx3, sum(cint) c1, avg(v1) c2, _tnext_ts + 1 as ts2, _tgrpid from qdb.meters where cts >= _twstart and cts < _twend;",
            res_query="select * from rdb.s37",
            exp_query="select _wstart, sum(cint), count(cint), tbname from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:35:00.000' and tbname='t1' partition by tbname interval(5m);",
        )
        # self.streams.append(stream) cases/13-StreamProcessing/07-SubQuery/test_subquery_sliding_bug4.py
