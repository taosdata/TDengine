...
class TestStreamSubqueryState:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=109,
            stream="create stream rdb.s109 state_window(c1) from tdb.v2 into rdb.r109 as select _twstart, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2) from tdb.v2 ta join tdb.t2 tb on ta.ts = tb.ts where ta.ts >= _twstart and ta.ts < _twend + 4m group by ta.c2 having sum(tb.c2) > 130;",
