...
class TestStreamSubqueryEvent:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=0,
            stream="create stream rdb.s0 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r0 as select _twstart ts, _twstart + 5m te, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=5,
            stream="create stream rdb.s5 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r5 as select _twstart ts, _twend te, _twduration td, _twrownum tw, %%tbname as tb, count(c1) c1, avg(c2) c2 from %%tbname where ts >= _twstart and ts < _twend",
        ...
        stream = StreamItem(
            id=6,
            stream="create stream rdb.s6 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r6 as select _twstart ts, count(c1), avg(c2) from %%trows where ts >= _twstart and ts < _twend partition by tbname",
        ...
        stream = StreamItem(
            id=7,
            stream="create stream rdb.s7 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r7 as select _twstart ts, count(c1), avg(c2) from %%tbname where ts >= _twstart and ts < _twend and %%tbname = tbname",
        ...
        stream = StreamItem(
            id=8,
            stream="create stream rdb.s8 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r8 as select _twstart ts, count(c1), avg(c2) from %%trows where ts >= _twstart and ts < _twend partition by %%1",
        ...
        stream = StreamItem(
            id=9,
            stream="create stream rdb.s9 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r9 tags(gid bigint as _tgrpid, id int as %%1) as select _twstart ts, _twend te, _twduration td, _twrownum tw, _tgrpid tg, _tlocaltime tl, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and _twduration is not null and _twrownum is not null and _tgrpid is not null and _tlocaltime is not null;",
        ...
        stream = StreamItem(
            id=11,
            stream="create stream rdb.s11 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 into rdb.r11 as select _tcurrent_ts tc, _tprev_ts tp, _tnext_ts tn, _tgrpid tg, _tlocaltime tl, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _tcurrent_ts and cts < _tnext_ts and _tgrpid is not null and _tlocaltime is not null and tbname != 't1';",
        ...
        stream = StreamItem(
            id=12,
            stream="create stream rdb.s12 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r12 as select _twstart ts, %%tbname tb, %%1, count(*) v1, avg(c1) v2, first(c1) v3, last(c1) v4 from %%trows where c2 > 0;",
        ...
        stream = StreamItem(
            id=14,
            stream="create stream rdb.s14 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, tbname into rdb.r14 as select _twstart ts, %%tbname t1, %%1 t2, count(*) c1, avg(c1) c2, first(c1) c3, last(c1) c4 from %%trows;",
        ...
        stream = StreamItem(
            id=15,
            stream="create stream rdb.s15 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r15 as select _wstart, sum(cint), FIRST(cint) from qdb.meters where cts >= _twstart and cts <_twend interval(150s)",
        ...
        stream = StreamItem(
            id=16,
            stream="create stream rdb.s16 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 into rdb.r16 as select _tcurrent_ts ts, count(*) c1, avg(cint) c2, _tcurrent_ts + 1 as ts2, _tgrpid from qdb.meters where tbname = 't1' partition by tbname;",
        ...
        stream = StreamItem(
            id=17,
            stream="create stream rdb.s17 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r17 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(5m)",
        ...
        stream = StreamItem(
            id=19,
            stream="create stream rdb.s19 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r19 as select _twend tw, sum(c1) sumcnt from (select tbname, count(*) c1 from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1 partition by tbname)",
        ...
        stream = StreamItem(
            id=20,
            stream="create stream rdb.s20 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r20 as select _twend tw, count(*) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1;",
        ...
        stream = StreamItem(
            id=21,
            stream="create stream rdb.s21 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 into rdb.r21 as select _tcurrent_ts tw, count(*) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000'",
        ...
        stream = StreamItem(
            id=22,
            stream="create stream rdb.s22 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r22 as select _twend tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=23,
            stream="create stream rdb.s23 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r23 as select _twend tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname=%%tbname partition by tbname",
        ...
        stream = StreamItem(
            id=24,
            stream="create stream rdb.s24 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r24 as select _twend tw, sum(cint) c1, %%1 c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1 partition by tint",
        ...
        stream = StreamItem(
            id=25,
            stream="create stream rdb.s25 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r25 as select _twstart tw, sum(cint) c1, %%1 c2, %%2 c3 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1",
        ...
        stream = StreamItem(
            id=26,
            stream="create stream rdb.s26 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r26 as select _twstart tw, sum(cint) c1, tbname from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname='t18' partition by tbname",
        ...
        stream = StreamItem(
            id=27,
            stream="create stream rdb.s27 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r27 as select _tcurrent_ts tw, sum(cint) c1, count(cint) c2 from qdb.meters where cts >= _tcurrent_ts and cts < _tnext_ts and tbname=%%1",
        ...
        stream = StreamItem(
            id=28,
            stream="create stream rdb.s28 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r28 as select _twstart ts, `name` as cname, `super` csuper, create_time cctime, %%1 from information_schema.ins_users",
        ...
        stream = StreamItem(
            id=29,
            stream="create stream rdb.s29 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r29 as select _wstart, sum(cint), avg(cint) from qdb.meters where cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=30,
            stream="create stream rdb.s30 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 into rdb.r30 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(2m)",
        ...
        stream = StreamItem(
            id=32,
            stream="create stream rdb.s32 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r32 as select _wstart, sum(cint), avg(cint) from qdb.meters where tbname != %%tbname and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=33,
            stream="create stream rdb.s33 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r33 tags(tbn varchar(128) as cast(%%1 as varchar)) as select _twstart, sum(cint), avg(cint) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=34,
            stream="create stream rdb.s34 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r34 as select _tprev_ts tc, TIMEDIFF(_tprev_ts, _tnext_ts) tx, %%tbname tb, %%1 tg1, sum(c1) c1, avg(c2) c2, first(c1) c3, last(c2) c4 from %%trows where c1 > 0;",
        ...
        stream = StreamItem(
            id=35,
            stream="create stream rdb.s35 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r35 as select _wstart ts, count(c1) c1, sum(c2) c2, %%1 c3, %%2 c4, cast(_tlocaltime / 1000000 as timestamp) c5, _tgrpid c6 from %%trows count_window(2);",
        ...
        stream = StreamItem(
            id=37,
            stream="create stream rdb.s37 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r37 as select _twstart ts, _twend, count(c1), sum(c2) from %%trows",
        ...
        stream = StreamItem(
            id=38,
            stream="create stream rdb.s38 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r38 as select _twstart ts, _irowts, _isfilled, interp(c1), _irowtsorigin, _twend, _twduration, _twrownum from %%trows RANGE(_twend) FILL(linear);",
        ...
        stream = StreamItem(
            id=39,
            stream="create stream rdb.s39 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r39 as select _tnext_ts tn, TIMETRUNCATE(_tnext_ts, 1d) tnt, sum(cint) c1, _tgrpid tg, TIMETRUNCATE(cast(_tlocaltime /1000000 as timestamp), 1d) tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint = 1 partition by tint",
        ...
        stream = StreamItem(
            id=40,
            stream="create stream rdb.s40 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r40 as select _twstart, table_name, db_name, stable_name, tag_name, tag_value from information_schema.ins_tags where table_name='j0' and stable_name='jmeters' and tag_name='tjson'",
        ...
        stream = StreamItem(
            id=41,
            stream="create stream rdb.s41 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r41 as select cts, cint, cbool, cast(tjson->'k1' as varchar(8)) cjson, _twstart from qdb.j0 where cts >= _twstart and cts < _twstart + 5m and cbool = %%1 order by cts limit 2, 3",
        ...
        stream = StreamItem(
            id=46,
            stream="create stream rdb.s46 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r46 as select _twstart ts, count(c1) ccnt, sum(c2) csum, first(id) cfirst from %%trows",
        ...
        stream = StreamItem(
            id=47,
            stream="create stream rdb.s47 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r47 as select _twstart ts, sum(`vgroups`) c1, sum(ntables) c2 from information_schema.ins_databases where name != 'information_schema' and name != 'performance_schema'",
        ...
        stream = StreamItem(
            id=56,
            stream="create stream rdb.s56 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r56 as select _wstart ws, _wend we, _twstart tws, _twend twe, first(c1) cf, last(c1) cl, count(c1) cc from %%trows where ts >= _twstart and ts < _twend interval(1m) fill(prev)",
        ...
        stream = StreamItem(
            id=58,
            stream="create stream rdb.s58 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r58 as select interp(cbigint) from qdb.v1 where ctinyint > 0 and cint > 2 RANGE('2025-01-01 00:02:00.000', '2025-01-01 00:08:00.000') EVERY (1m) FILL(linear) limit 50;",
        ...
        stream = StreamItem(
            id=59,
            stream="create stream rdb.s59 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r59 as select interp(csmallint) from qdb.meters partition by tbname where ctinyint > 0 and cint > 2 RANGE('2025-01-01 00:02:00.000') EVERY (1m) FILL(linear)",
        ...
        stream = StreamItem(
            id=60,
            stream="create stream rdb.s60 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r60 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=61,
            stream="create stream rdb.s61 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r61 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=62,
            stream="create stream rdb.s62 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r62 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=65,
            stream="create stream rdb.s65 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r65 as select _twstart ts, HISTOGRAM(cfloat, 'user_input', '[1, 7]', 1) th from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=66,
            stream="create stream rdb.s66 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r66 as select _twstart ts, FIRST(cuint), LAST(cbigint), LAST_ROW(cubigint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=67,
            stream="create stream rdb.s67 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r67 as select _twstart ts, MAX(ctinyint), MIN(cutinyint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=68,
            stream="create stream rdb.s68 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r68 as select _twstart ts, mode(cint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=69,
            stream="create stream rdb.s69 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r69 as select _twstart ts, SAMPLE(cdecimal8, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=70,
            stream="create stream rdb.s70 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r70 as select _twstart ts, BOTTOM(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=71,
            stream="create stream rdb.s71 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r71 as select _twstart ts, TAIL(cbigint, 1)  from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=72,
            stream="create stream rdb.s72 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r72 as select _twstart ts, TOP(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=73,
            stream="create stream rdb.s73 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r73 as select _rowts, _twstart ts, CSUM(cint) + CSUM(cuint) from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=75,
            stream="create stream rdb.s75 event_window(start with c1>= 5 end with c1< 10） from tdb.vtriggers partition by id into rdb.r75 as select _wstart, twa(k),avg(k),count(1) from tdb.vtriggers where id=%%1 where ts>='2015-8-18 00:00:00' and ts<='2015-8-18 00:07:00' interval(1m) fill(prev)",
        ...
        stream = StreamItem(
            id=77,
            stream="create stream rdb.s77 event_window(start with c1>= 5 end with c1< 10） from tdb.vtriggers partition by id into rdb.r77 as select _twstart ts, count(*) c1 from information_schema.ins_streams where stream_name='s77'",
        ...
        stream = StreamItem(
            id=78,
            stream="create stream rdb.s78 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r78 as select _twstart, _twrownum, count(*), sum(cdecimal8) from qdb.meters where tbname !=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=79,
            stream="create stream rdb.s79 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r79 as select _twstart, sum(cdecimal8), count(*), _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=80,
            stream="create stream rdb.s80 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r80 as select _twstart, sum(v1), sum(v2) from (select count(c1) v1, sum(c2) v2 from %%trows)",
        ...
        stream = StreamItem(
            id=81,
            stream="create stream rdb.s81 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r81 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend and cint > 5 and _twrownum > 0 ",
        ...
        stream = StreamItem(
            id=82,
            stream="create stream rdb.s82 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r82 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend and cint > 5",
        ...
        stream = StreamItem(
            id=83,
            stream="create stream rdb.s83 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r83 as select ts, diff(c1) c3 from %%trows ",
        ...
        stream = StreamItem(
            id=84,
            stream="create stream rdb.s84 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r84 as select cts, top(cint, 5) from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=85,
            stream="create stream rdb.s85 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r85 as select cts, top(cint, 5) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=86,
            stream="create stream rdb.s86 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 into rdb.r86 as select ts, top(c1, 5) from %%trows limit 1 offset 1;",
        ...
        stream = StreamItem(
            id=87,
            stream="create stream rdb.s87 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r87 as select last(cts, cint, cuint) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=88,
            stream="create stream rdb.s88 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r88 as select from select cast(avg(f) as int) b  from view1 group by a having avg(f) > 100111 order by b",
        ...
        stream = StreamItem(
            id=89,
            stream="create stream rdb.s89 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r89 as select rand() from %%trows where rand() >= 0 and rand() < 1 _c0, _rowts;",
        ...
        stream = StreamItem(
            id=91,
            stream="create stream rdb.s91 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r91 as select _twstart, id cid, name cname, sum(c1) amount from %%trows where ts between _twstart and _twend and id=%%1 and name=%%1 partition by id, name having sum(c1) <= 5;",
        ...
        stream = StreamItem(
            id=92,
            stream="create stream rdb.s92 event_window(start with c1>= 5 end with c1< 10） from tdb.n1 as  select _twstart, tint, %%tbname, sum(cint) amount from qdb.meters where tbname=%%tbname and ts between _twstart and _twend partition by tbname having sum(cint) > 50",
        ...
        stream = StreamItem(
            id=94,
            stream="create stream rdb.s94 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r94 as select diff(faev) from (select _ts, faev, deviceid from demo union all select _ts + 1s, faev, deviceid from %%trows order by faev, _ts, deviceid) partition by faev",
        ...
        stream = StreamItem(
            id=95,
            stream="create stream rdb.s95 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r95 as select _twstart, _twend, last(cts, cint, cuint) from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=98,
            stream="create stream rdb.s98 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r98 as select last(cts, cint, cbool) from qdb.jmeters where cts >= _twstart and cts < _twstart + 5m and tjson->'k1'='v1' partition by tjson->'k1'",
        ...
        stream = StreamItem(
            id=100,
            stream="create stream rdb.s100 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r100 as select json, to_json from %%trows where BETWEEN AND",
        ...
        stream = StreamItem(
            id=101,
            stream="create stream rdb.s101 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r101 as select 1&2, 2|3  and or && || case from tdb.v1 limit 1 offset 1 ",
        ...
        stream = StreamItem(
            id=102,
            stream="create stream rdb.s102 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r102 as select cols(last(ts), ts, c0), count(1) {t1} from {from_table} group by t1 order by t1",
        ...
        stream = StreamItem(
            id=103,
            stream="create stream rdb.s103 event_window(start with c1>= 5 end with c1< 10） from tdb.vtriggers partition by tbname into rdb.r103 as select cvarchar like 'a', not like, regexp, not regexp from tdb.vtriggers where tbname=%%tbname limit 1 offset 1 ",
        ...
        stream = StreamItem(
            id=104,
            stream="create stream rdb.s104 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r104 as select _wstart ts, CASE c1 WHEN 0 THEN 'Running' WHEN 5 THEN 'Warning' ELSE 'Unknown' END t1, c1 & c2 t2, c1 | c2 t3, (c1 != 0 or c2 <> 0) t4  from %%trows",
        ...
        stream = StreamItem(
            id=106,
            stream="create stream rdb.s106 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r106 as select _twstart, cts, concat(cvarchar, cnchar), cint + cuint, ctinyint - cdouble, cfloat * cdouble, cbigint * 12, -ctinyint from qdb.meters where tbname=%%tbname limit 1 offset 1;",
        ...
        stream = StreamItem(
            id=107,
            stream="create stream rdb.s107 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers into rdb.r107 as select _twstart, cts, cvarchar like 'SanFrancisco', cnchar not like 'SanFrancisco', cvarchar regexp '%co', cnchar not regexp '%co' from qdb.t5 limit 1 offset 2;",
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twstart + 5m partition by tbname slimit 1 soffset 2",
        ...
        stream = StreamItem(
            id=111,
            stream="create stream rdb.s111 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r111 as select _wstart, _wend, _twstart, _twend, csmallint, count(cts), count(cbool) from qdb.meters where cts >= _twstart and cts < _twstart + 5m partition by tbname state_window(csmallint)",
        ...
        stream = StreamItem(
            id=113,
            stream="create stream rdb.s113 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r113 as select _wstart, _wend, _twstart, _twend, first(csmallint), count(cts), count(cbool) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m and csmallint = 2 session(cts, 40s);",
        ...
        stream = StreamItem(
            id=114,
            stream="create stream rdb.s114 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r114 as show dnode 1 variables like 'bypassFlag'",
        ...
        stream = StreamItem(
            id=115,
            stream="create stream rdb.s115 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r115 as SELECT a.voltage, sum(cint) FROM ct_join_1 a left JOIN ct_join_2 b ON a.ts = b.ts group by a.voltage having b.voltage > 14;",
        ...
        stream = StreamItem(
            id=117,
            stream="create stream rdb.s117 event_window(start with c1>= 5 end with c1< 10） from tdb.t1 into rdb.r117 as select t1.ts, t2.ts from db1_st1 t1 right asof join db1_st2 t2 where t1.v_int >= 0 and (cos(t2.t_double) >= 0 or cos(t2.t_double) < 0) order by t1.ts, t2.ts;",
        ...
        stream = StreamItem(
            id=118,
            stream="create stream rdb.s118 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r118 as select t1.ts, t2.ts from qdb.v1 t1 right asof join from qdb.v2 t2 where t1.v_int >= 0 and (cos(t2.t_double) >= 0 or cos(t2.t_double) < 0) order by t1.ts, t2.ts;",
        ...
        stream = StreamItem(
            id=119,
            stream="create stream rdb.s119 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r119 as select dictintc from qdb.meters where tbname in(%%tbname) _qstart, _qend, _c0, _rowts",
        ...
        stream = StreamItem(
            id=123,
            stream="create stream rdb.s123 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id, name into rdb.r123 as select _wstart tw, _wend te, _twstart, _twend, count(c1) c1, sum(c2) c2 from %%trows interval(1m)",
        ...
        stream = StreamItem(
            id=124,
            stream="create stream rdb.s124 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r124 as select _wstart ts, count(c1), sum(c2) from %%trows where ts >= _twstart and ts < _twend interval(1m) fill(prev)",
        ...
        stream = StreamItem(
            id=125,
            stream="create stream rdb.s125 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r125 as select _rowts, _twstart ts, DERIVATIVE(cbigint, 5, 0) from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=126,
            stream="create stream rdb.s126 event_window(start with c1>= 5 end with c1< 10） from tdb.v1 into rdb.r126 as select _rowts, _twstart ts, IRATE(cubigint) from %%trows where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=127,
            stream="create stream rdb.s127 event_window(start with c1>= 5 end with c1< 10） from tdb.vtriggers into rdb.r127 as select _rowts, _twstart ts, MAVG(cfloat, 1) from %%trows where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=128,
            stream="create stream rdb.s128 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by tbname into rdb.r128 as select _rowts, _twstart ts, STATECOUNT(cdouble, 'LT', 5) from vdb.triggers where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=129,
            stream="create stream rdb.s129 event_window(start with c1>= 5 end with c1< 10） from tdb.triggers partition by id into rdb.r129 as select _c0, _twstart ts, STATEDURATION(cusmallint, 'LT', 5, 1m) from vdb.triggers where tint=%%1 and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=130,
            stream="create stream rdb.s130 event_window(start with c1>= 5 end with c1< 10） from tdb.vtriggers partition by tbname into rdb.r130 as select _c0, _twstart ts, TWA(ctinyint) from vdb.triggers where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m;",
