...
class TestStreamSubquerySession:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=27,
            stream="create stream rdb.s27 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r27 as select _twstart tw, sum(cint) c1, count(cint) c2 from qdb.vmeters where cts >= _twstart and cts < _twstart + 5m and tbname=%%1",
        ...
        # self.streams.append(stream) TD-36397  TD-36470
        ...
        stream = StreamItem(
            id=131,
            stream="create stream rdb.s131 session(ts, 2m) from tdb.v1 partition by id, tbname into rdb.r131 as select ta.ts tats, tb.cts tbts, ta.c1 tac1, ta.c2 tac2, tb.cint tbc1, tb.cuint tbc2, _twstart, _twend from %%trows ta right join qdb.t1 tb on ta.ts=tb.cts where ta.ts >= _twstart and ta.ts < _twend + 4m;",
