...
class TestPeriodInterval:
    ...
    def do_exec(
        ...
        while True:
            tdSql.query(f"select status from information_schema.ins_streams where stream_name='{localQueryResult[0][0]}'")
        ...
        while True:
            tdSql.query(f"select status from information_schema.ins_streams where stream_name='{localQueryResult[1][0]}'")
            if tdSql.getData(0, 0) != "Running":
                print("stream not running, waiting....")
                time.sleep(10)
            else:
        ...
        for tbname in [self.stb_name, self.tb_name]:
            ...
            if tbname == self.stb_name:
                sql = f'select * from (select _wstart AS wstart, {fwc_stb_source_select_str}  from {tbname} {where_elm} {query_partition_elm} interval({self.tdCom.dataDict["interval"]}s) {query_stb_fill_elm} order by wstart) {query_where_elm}',
                print('--------------', sql)

                sql1 = f'select wstart, {fwc_stb_output_select_str} from {tbname}{self.tdCom.des_table_suffix} {stream_where_elm} order by wstart',
                print('================', sql1)
