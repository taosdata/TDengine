...
class TestIntervalInterpCases:
    ...
    def get_stream_first_ts(self, table_name1, table_name2):
        tdSql.query(f'select  * from  {table_name1}{table_name2} order by 1 ')

        res_ts = tdSql.getData(0, 0)
        return res_ts

    def do_exec(self, interval, partition="tbname", funciton_name="", funciton_name_alias="", fill_value=None, ):
        # partition must be tbname, and not NONE.
        tdLog.info(
            f"testing stream sliding+interp+every: every: {interval}, partition: {partition}, fill: {fill_value} "
        )
        ...
        tdSql.execute(
            f"create stream itp_force_error_0 {trigger_type} from {self.stb_name} into itp_force_error_0_s as select _irowts,tbname,_isfilled,interp(c1,1) from {self.stb_name} partition by tbname   every(5s)   fill(prev) ;"
        )

        tdSql.execute(
            f"create stream itp_force_error_1 {trigger_type} from {self.stb_name} into itp_force_error_1_s as select _irowts,tbname,_isfilled,interp(c1,1) from {self.stb_name} partition by tbname   every(5s)   fill(prev) ;"
        )
        
        tdSql.error(
            f"create stream itp_force_error_2 {trigger_type} from {self.stb_name} into itp_force_error_2_s as  select _irowts,tbname,_isfilled,interp(c1,1) from {self.stb_name} partition by tbname   every(5s)   fill(prev) ;"
        )
        tdSql.error(
            f"create stream itp_force_error_3 {trigger_type} from {self.stb_name} into itp_force_error_3_s as  select _irowts,tbname,_isfilled,interp(c11,1) from {self.stb_name}   partition by tbname   every(5s)   fill(prev) ;"
        )
        tdSql.error(
            f"create stream itp_1d_next_error_0 {trigger_type} from {self.stb_name} into itp_1d_next_error_t0 as select _irowts,tbname,_isfilled,interp(current) from {self.stb_name} where groupid=100 partition by every(5s) fill(next) ;"
        )
        # tdSql.error(
        #     f"create stream itp_1d_next_error_1  trigger at_once into itp_1d_next_error_t1 as select _irowts,tbname,_isfilled,interp(current) from {self.stb_name} where groupid=100 partition by every(5s) fill(next) ;"
        # )
        # tdSql.error(
        #     f"create stream itp_1d_next_error_2  trigger window_close into itp_1d_next_error_t2 as select _irowts,tbname,_isfilled,interp(current) from {self.stb_name} where groupid=100 partition by every(5s) fill(next) ;"
        # )
        # tdSql.error(
        #     f"create stream itp_1d_next_error_3  trigger max_delay 5s into itp_1d_next_error_t3 as select _irowts,tbname,_isfilled,interp(current) from {self.stb_name} where groupid=100 partition by every(5s) fill(next) ;"
        # )
        ...
        # create stream set filter of  tag and tbname
    ...
    def test_do_exec(self):
        ...
        # not support yet.
        # self.do_exec(interval=10, partition="tbname", funciton_name="interp(c1)", funciton_name_alias="intp_c1", fill_value="PREV",)
        # self.do_exec(interval=8, partition="tbname", funciton_name="interp(c1)", funciton_name_alias="intp_c1", fill_value="VALUE",)
        # self.do_exec(interval=8, partition="tbname", funciton_name="interp(c1)", funciton_name_alias="intp_c1", fill_value="NULL",)
