...
############ virtual normal table + expired_time
    ...
    class Basic13(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.execute(
                f"create stream s13 sliding(10s) from ct1 options(ignore_nodata_trigger) into res_ct1 (firstts, lastts, cnt_v, sum_v, avg_v) as select first(_c0), last_row(_c0), count(cint), sum(cint), avg(cint) from %%trows;"
            )
            tdSql.execute(
                f"create stream s13_g sliding(10s) from {self.stbName} partition by tbname, tint options(ignore_nodata_trigger) into res_stb OUTPUT_SUBTABLE(CONCAT('res_stb_', tbname)) (firstts, lastts, cnt_v, sum_v, avg_v) as select first(_c0), last_row(_c0), count(cint), sum(cint), avg(cint) from %%trows;"
            )
