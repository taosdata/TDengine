...
class TestStreamStateTrigger:
    ...
    def test_stream_state_trigger(self):
        ...
        sql1 = "create stream s1 state_window(cint) from ct1 into res_ct1 (firstts, lastts, num_v, cnt_v, avg_v) as select first(_c0), last_row(_c0), _twrownum, count(*), avg(cuint) from %%trows;"
        sql2 = "create stream s2 state_window(cint) from ct2 into res_ct2 (firstts, lastts, num_v, cnt_v, avg_v) as select first(_c0), last_row(_c0), _twrownum, count(*), avg(cuint) from %%trows;"
        sql3 = "create stream s3 state_window(cint) from stb partition by tbname into res_stb OUTPUT_SUBTABLE(CONCAT('res_stb_', tbname)) (firstts, lastts, num_v, cnt_v, avg_v) tags (nameoftbl varchar(128) as tbname, gid bigint as _tgrpid) as select first(_c0), last_row(_c0), _twrownum, count(*), avg(cuint) from %%trows partition by tbname;"
        sql4 = "create stream s4 state_window(cint) from stb partition by tbname, tint into res_stb_mtag OUTPUT_SUBTABLE(CONCAT('res_stb_mtag_', tbname, '_', cast(tint as varchar))) (firstts, lastts, num_v, cnt_v, avg_v) tags (nameoftbl varchar(128) as tbname, gid bigint as _tgrpid) as select first(_c0), last_row(_c0), _twrownum, count(*), avg(cuint) from %%trows partition by %%1, %%2;"
        ...
        ############ add one new sub table of stb  
        tdLog.info(f"=============== new create one sub table of stb, and write new data")
        sqls = [
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:00', 0, 0);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:01', 0, 0);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:02', 1, 1);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:03', 1, 1);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:04', 1, 1);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:05', 2, 2);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:06', 2, 2);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:07', 2, 2);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:08', 2, 2);",
            "insert into cta using stb tags(1) values ('2025-01-01 00:00:09', 3, 3);",
        ]
        tdSql.executes(sqls)
        # tdSql.query("select _wstart, count(cint), avg(cint) from stb partition by tbname state_window(cint)")
        # tdSql.printResult()
        ...
        ############ true_for 
        sql5 = "create stream s5 state_window(cint) true_for(5s) from ct1 into res_truefor_ct1 (firstts, lastts, num_v, cnt_v, avg_v) as select first(_c0),last_row(_c0), _twrownum, count(*), avg(cuint) from %%trows;"
    ...
    def checks3(self):
        result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_stb_ct1"
        tdSql.checkResultsByFunc(
            sql=result_sql,
            func=lambda: tdSql.getRows() == 3
            and tdSql.compareData(0, 0, "2025-01-01 00:00:00.000")
    ...
    def checksNew(self, check_idx):
        if 0 == check_idx:
            result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_stb_cta"
        elif 1 == check_idx:
            result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_stb_mtag_cta_1"
        tdSql.checkResultsByFunc(
            sql=result_sql,
            func=lambda: tdSql.getRows() == 3
            and tdSql.compareData(0, 0, "2025-01-01 00:00:00.000")
        ...
        tdLog.info(f"=============== check s3-new sub result success !!!!!!!! =====================")
        return
    ...
    def checks4(self):
        ...
        tdSql.query("desc sdb.res_stb_mtag")
        tdSql.printResult()
        tdSql.checkRows(7)
        tdSql.checkData(0, 0, "firstts")
        tdSql.checkData(1, 0, "lastts")
    ...
    def checks5(self, check_idx):
        result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_truefor_ct1"
        if 0 == check_idx: 
            tdSql.checkResultsByFunc(
                sql=result_sql,
    ...
    def checks6(self, check_idx):
        ...
        if 0 == check_idx: 
            result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_fill_all_ct1"
            tdSql.checkResultsByFunc(
                sql=result_sql,
                func=lambda: tdSql.getRows() == 0,
            )
            ...
            tdSql.checkResultsByFunc(
                ...
                and tdSql.compareData(2, 2, 4)
                and tdSql.compareData(2, 3, 4)
                and tdSql.compareData(10, 0, "2025-01-02 00:00:22.000")
                and tdSql.compareData(10, 1, "2025-01-02 00:00:29.000")
                and tdSql.compareData(10, 2, 2)
            ...
            result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_fill_part_ct1"
            tdSql.checkResultsByFunc(
                sql=result_sql,
            ...
            result_sql = "select firstts, lastts, num_v, cnt_v, avg_v from res_max_delay_ct5"
            tdSql.checkResultsByFunc(
                sql=result_sql,
