...
class TestStreamSlidingTrigger:
    ...
    queryResults = {
        ...
        "0-0-0-2-0": [3, None, True, [], "order by cts, tag_tbname"],#FAILED, 某个子表结果计算为 NULL，应该为 0 
        ...
        "0-0-0-2-17": [40, None, True, [], ""],#FAILED ,_tgrpid每次执行可能变化不一致
        "0-0-0-2-18": [3513, None, True, [], ""], #success
        "0-0-0-2-23": [-1, None, True, [], ""], 
    ...
    def execCase(self):
        ...
        createStreamSqls = [ #(SQL, (minPartitionColNum, partitionByTbname))
            (f"create stream stName sliding({self.sliding}s) from {self.trigTbname} into outTbname as querySql;", (0, False)),#0
