...
class Test_Period:
    ...
    def createStreams(self):
        ...
        sqls = [
            ...
            "CREATE STREAM IF NOT EXISTS test.stream1_sub3 PERIOD(1h, 0s) FROM test.d0   NOTIFY('ws://idmp:6042/eventReceive') ON(WINDOW_OPEN|WINDOW_CLOSE) INTO test.result_stream1_sub2 AS SELECT CAST(_tlocaltime/1000000 as timestamp) AS ts, COUNT(*) AS cnt, AVG(voltage) AS avg_voltage, SUM(power) AS sum_power FROM %%trows",
            "CREATE STREAM IF NOT EXISTS test.stream1_sub4 PERIOD(1d, 0s) FROM test.d0   NOTIFY('ws://idmp:6042/eventReceive') ON(WINDOW_OPEN|WINDOW_CLOSE) INTO test.result_stream1_sub3 AS SELECT CAST(_tlocaltime/1000000 as timestamp) AS ts, COUNT(*) AS cnt, AVG(voltage) AS avg_voltage, SUM(power) AS sum_power FROM %%trows",
        ]
