...
class TestSnodeMgmt:
    caseName = "test_three_gorges_case1"
    ...
    def createStream(self):
        ...
        stream = (
                    f"""create stream {self.dbname}.str_station_alarmmsg_systemalarm_test sliding(10m) from {self.dbname}.stb_sxny_cn partition by tbname,cnstationno stream_options(fill_history(0)|expired_time(1d))
            into {self.dbname}.str_station_alarmmsg_systemalarm_test output_subtable(concat('str_station_alarmmsg_systemalarm_test_',tbname)) tags(cnstationno varchar(255) as cnstationno)
            as select
                first(dt) alarmdate,
                val alarmstatus
            from
                {self.dbname}.stb_sxny_cn t
            where
                index_code in ('index_a1', 'index_a2')
                and dt >= today() - 1d
                and cz_flag = 'cz_z1' 
                partition by tbname,
                cnstationno,
                ...
                (
                    case
                        when index_code = 'index_a1' then '01'
                        when index_code = 'index_a2' then '02'
                    end
                ) as alarmtype,
