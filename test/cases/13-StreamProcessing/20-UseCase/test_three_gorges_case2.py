...
class TestSnodeMgmt:
    caseName = "test_three_gorges_case2"
    ...
    def test_snode_mgmt(self):
        ...
        Catalog:
            - Streams:str_station_alarmmsg_systemalarm_test
        ...
        tdSql.query(f"select * from {self.dbname}.str_station_alarmmsg_inverteralarm_test")
        if tdSql.getRows() == 0:
    ...
    def createStream(self):
        ...
        stream = (
                    f"""create stream {self.dbname}.str_station_alarmmsg_inverteralarm_test state_window(cast(val as int)) from {self.dbname}.stb_sxny_cn partition by tbname,cnstationno,ps_code,index_name,point,country_equipment_code,index_code,point_path stream_options(low_latency_calc|fill_history|pre_filter(index_code in ('index_a0', 'index_a1','index_a2') and dt >= today() - 1d and blq_flag = 'blq_a0' ))
                    into {self.dbname}.str_station_alarmmsg_inverteralarm_test output_subtable(concat_ws('_','str_station_alarmmsg_systemalarm_test',cnstationno,country_equipment_code,point)) 
                    tags(tablename varchar(255) as tbname,
                    cnstationno varchar(255) as cnstationno,
                    gpstationno varchar(255) as ps_code,
                    cninverterno varchar(255) as country_equipment_code,
                    gpinverterno varchar(255) as country_equipment_code,
                    ...
                    alarmtype varchar(255) as (case when index_code = 'index_a1' then '01' when index_code = 'index_a2' then '02' end) ,
