...
Parameters/参数:
    ...
    --histroy-rows: Number of rows per table / 每个表的历史行数
...
class StreamStarter:
    def __init__(self, runtime=None, perf_file=None, table_count=500, 
                ...
                stream_sql=None, sql_type='s2_2', stream_perf_test_dir=None, monitor_interval=1,
                create_data=False, restore_data=False, deployment_mode='single',
                debug_flag=131, num_of_log_lines=500000) -> None:
        ...
        self.histroy_rows = histroy_rows      # 插入历史记录数
        self.real_time_batch_rows = real_time_batch_rows      # 后续每轮插入记录数
    ...
    def backup_cluster_data(self):
        ...
        try:
            ...
            # 直接复制整个节点目录
            for dnode in ['dnode1', 'dnode2', 'dnode3']:
                src_dir = os.path.join(self.stream_perf_test_dir, dnode)
    ...
    def restore_cluster_data(self):
        ...
        try:
            ...
            # 清理现有目录
            for dnode in ['dnode1', 'dnode2', 'dnode3']:
                cluster_dir = os.path.join(self.stream_perf_test_dir, dnode)
    ...
    def prepare_env(self):
        ...
        try:
            ...
            if os.path.exists(self.stream_perf_test_dir):
                print(f"清理已存在的集群目录: {self.stream_perf_test_dir}")
                subprocess.run(f'rm -rf {self.stream_perf_test_dir}/*', shell=True)
            else:
                print(f"创建新的集群目录: {self.stream_perf_test_dir}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例用法:
  %(prog)s -m 1 --sql-type s2_7 --table-count 1000
  %(prog)s -m 2 --deployment-mode cluster --debug-flag 135
  %(prog)s --create-data --table-count 500 --histroy-rows 100
        """
    )
    ...
    # SQL相关参数
    sql_group = parser.add_argument_group('SQL参数')
    ...
    system_group.add_argument('--stream-perf-test-dir', type=str, 
                            ...
                            help='集群根目录, 默认/home/stream_perf_test_dir')
    system_group.add_argument('--monitor-interval', type=int, default=1,
    ...
    system_group.add_argument('--debug-flag', type=int, default=131,
                            help='TDengine调试标志位, 默认131\n'
                                 '常用值: 131(默认), 135(详细), 143(更详细)')
    system_group.add_argument('--num-of-log-lines', type=int, default=500000,
                            help='TDengine日志文件最大行数, 默认500000')
    ...
    #                     help='子表数量,默认500')
    # parser.add_argument('--histroy-rows', type=int, default=1,
    #                     help='初始插入记录数,默认1')
    # parser.add_argument('--real-time-batch-rows', type=int, default=200,
    #                    help='后续每轮插入记录数')
    # parser.add_argument('--disorder-ratio', type=int, default=0,
    #                     help='数据乱序率,默认0')
    # parser.add_argument('--vgroups', type=int, default=10,
    #                     help='vgroups,默认10')
    # parser.add_argument('--sql-type', type=str, default='s2_7',
    #                    choices=['s2_2', 's2_3', 's2_4', 's2_5', 's2_6', 's2_7', 's2_8', 's2_9', 's2_10', 's2_11', 's2_12', 's2_13', 's2_14', 's2_15','all'],
    #                    help='实时流计算SQL-CASE-ID')
    # parser.add_argument('--stream-sql', type=str,
    #                     help='自定义流计算SQL(优先级高于sql-type)')
    # parser.add_argument('--sql-file', type=str,
    #                     help='从文件读取流式查询SQL')
    # parser.add_argument('--stream-perf-test-dir', type=str, default='/home/stream_perf_test_dir',
    #                     help='集群根目录,默认/home/stream_perf_test_dir')
    # parser.add_argument('--monitor-interval', type=int, default=1,
    #                     help='性能数据采集间隔(秒),默认1秒')
    # parser.add_argument('--deployment-mode', type=str, default='single',
    #                     choices=['single', 'cluster'],
    #                     help='部署模式: single(单节点) 或 cluster(3节点集群), 默认single')
    # parser.add_argument('--debug-flag', type=int, default=131,
    #                     help='TDengine调试标志位,默认131,可以设置135、143等')
    # parser.add_argument('--num-of-log-lines', type=int, default=500000,
    #                     help='TDengine日志文件最大行数,默认500000')
