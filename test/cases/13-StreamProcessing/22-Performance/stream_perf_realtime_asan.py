...
class StreamInserter(threading.Thread):
    """持续写入数据的线程类"""
...
class StreamStarter:
    ...
    def prepare_env(self):
        ...
        try:
            # 停止已存在的taosd进程
            print("停止现有taosd进程")
            #self.stop_taosd() #asan
            
            # # 清理已存在的进程
            # cleanup_cmds = [
            #     "pkill taosd",
            #     "pkill while_stream_insert.sh",
            # ]
...
""", shell=True)
            ...
            for instance in self.instances:
                cfg_file = f"{self.cluster_root}/{instance['name']}/conf/taos.cfg"
                #cmd = f'nohup taosd -c {cfg_file} > /dev/null 2>&1 &'
                cmd = f"nohup taosd -c {cfg_file} > /tmp/taosd_nohup_log/{instance['name']}_{time.strftime('%Y%m%d_%H%M%S')}.log &"
                    ...
                    "super_tables": [
                        {
                            ...
                            "timestamp_step": 50,
                            "trying_interval": 10,
                            "start_timestamp": "now",   #"2025-06-01 00:00:00",
                            "sample_format": "csv",
                            "sample_file": "./sample.csv",
                            "tags_file": "",
                            "columns": [
    ...
    def do_start(self):
        ...
        try:
            ...
            # script_path = "/mnt/merged_disk/guoxy/while_stream_insert.sh"
            # inserter = StreamInserter(script_path)
            # inserter.start()
            ...
            try:            
                ...
                while True:
                    ...
                    if cycle_count > 1:
                        ...
                        #     raise Exception("写入新数据失败-update_insert_config")
                        ...
                        #     raise Exception("写入新数据失败-run-benchmark")
