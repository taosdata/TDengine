...
class TestFunLag:
    ...
    def test_func_win_lag(self):
        ...
        History:
        History:
            - 2025-11-28 Stephen Jin Create the lag function
    ...
    def basic(self):
        ...
        tdSql.query(f"SELECT _rowts, lag(f1) f1l, lag(f2) f2l, lag(f1)*lag(f2) p, lag(f3), lag(f3)*lag(f2), lag(f3)*lag(f1), lag(f4), lag(f4)*lag(f1), lag(f4)*lag(f2), lag(f4)*lag(f3), lag(f4)*lag(f4) from {dbName}.{tbName}")
        tdSql.checkRows(4)
        ...
        tdSql.query(f"SELECT _rowts, lag(f1) f1l, lag(f2) f2l, lag(f1)*lag(f2) p from {dbName}.{tbName}")
        tdSql.checkRows(4)
        tdSql.checkData(0, 0, '2025-12-2 1:1:1')
        tdSql.checkData(0, 1, 1)
        tdSql.checkData(0, 2, 2)
