...
INIT_SQLS = [
    ...
    # Duplicate values with NULL event_time for unique() tie-break coverage
    "CREATE TABLE t_event_unique_null_dup (ts TIMESTAMP, event_time TIMESTAMP, val INT, st NCHAR(10))",
    "INSERT INTO t_event_unique_null_dup VALUES "
    "('2024-01-01 00:00:01', NULL, 10, 'null1') "
    "('2024-01-01 00:00:02', '2024-06-01 00:00:02', 10, 'time1') "
    "('2024-01-01 00:00:03', NULL, 20, 'null2a') "
    "('2024-01-01 00:00:04', NULL, 20, 'null2b') "
    "('2024-01-01 00:00:05', '2024-06-01 00:00:05', 30, 'time3') "
    "('2024-01-01 00:00:06', '2024-06-01 00:00:06', 40, 'time4') "
    "('2024-01-01 00:00:07', NULL, 40, 'null4')",
    ...
    # Full join source for degraded timeline coverage. Projecting the right
    # timestamp produces NULL timeline rows for left-only matches.
    "CREATE TABLE t_join_l (ts TIMESTAMP, val INT)",
    "INSERT INTO t_join_l VALUES "
    "('2024-07-01 00:00:00', 1) "
    "('2024-07-01 00:00:02', 3) "
    "('2024-07-01 00:00:03', 4) "
    "('2024-07-01 00:00:04', 5)",
    "CREATE TABLE t_join_r (ts TIMESTAMP, val INT)",
    "INSERT INTO t_join_r VALUES "
    "('2024-07-01 00:00:00', 2) "
    "('2024-07-01 00:00:01', 3) "
    "('2024-07-01 00:00:03', 5) "
    "('2024-07-01 00:00:05', 7)",
]
...
class TestTimelineFallback:
    ...
    def test_degraded_timeline_join_subquery_matrix(self):
        """Degraded timeline from JOIN and UNION ALL subqueries.

        Sections HH-HI: behavior matrix coverage for subquery sources that
        project a timestamp column but contain NULL timestamp rows. FULL JOIN
        covers the original regression path; UNION ALL covers the same matrix
        through a non-JOIN temp table branch.
        ...
        History:
            - 2026-07-02 Added JOIN and UNION ALL degraded timeline matrix
        """
        _init_data()
        case_dir = os.path.dirname(os.path.abspath(__file__))
        input_file = os.path.join(
            case_dir, "in", "test_degraded_timeline_subquery_matrix.in"
        )
        expected_file = os.path.join(
            case_dir, "ans", "test_degraded_timeline_subquery_matrix.ans"
        )
        tdCom.compare_testcase_result(
            input_file, expected_file, "test_degraded_timeline_subquery_matrix"
        )
