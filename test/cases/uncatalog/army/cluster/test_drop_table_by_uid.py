...
class TestDropTableByUid:
    ...
    def get_uid_by_db_table_name(self, db_name, table_name, table_type=TableType.STABLE):
        ...
        # get table uid list
        if table_type == self.TableType.STABLE:
            return [item[10] for item in tdSql.queryResult]
        else:
            return [item[5] for item in tdSql.queryResult]
    ...
    def run_drop_single_table_by_uid(self):
    ...
    def run_drop_multiple_tables_by_uid(self):
    ...
    def run_uid_as_table_name(self):
    ...
    def run_abnormal_non_exist_uid(self):
        ...
        tdSql.error(f"drop stable with if exists `1234567890`;", expectErrInfo="STable not exist:", fullMatched=False)
        tdSql.error(f"drop table with `1234567890`;", expectErrInfo="Table does not exist:", fullMatched=False)
        tdSql.execute(f"drop database {db_name};")
        tdLog.info("Finish test case: test_abnormal_non_exist_uid")

    def run_abnormal_incorrect_table_type(self):
        ...
        try:
            ...
            tdSql.error(f"drop stable with `{uid_ct[0]}`;", expectErrInfo="STable not exist", fullMatched=False)
            tdSql.error(f"drop stable with `{uid_t[0]}`;", expectErrInfo="STable not exist", fullMatched=False)
    ...
    def run_abnormal_mixed_uid(self):
        ...
        uid_t = self.get_uid_by_db_table_name(db_name, "t", self.TableType.REGULAR_TABLE)
        # drop table with incorrect sql
        tdSql.error(f"drop stable with `{uid_st[0]}`,`{uid_ct[0]}`;", expectErrInfo="syntax error", fullMatched=False)
        tdSql.error(f"drop table with `{uid_st[0]}`,`{uid_ct[0]}`,`{uid_t[0]}`;", expectErrInfo="Cannot drop super table in batch", fullMatched=False)
        tdSql.execute(f"drop database {db_name};")
        tdLog.info("Finish test case: test_abnormal_mixed_uid")

    def run_abnormal_system_tables(self):
    ...
    def run_abnormal_drop_table_with_non_root_user(self):
    ...
    def test_drop_table_by_uid(self):
        ...
        self.run_drop_multiple_tables_by_uid()
        self.run_uid_as_table_name()
        # abnormal cases
        self.run_abnormal_non_exist_uid()
        self.run_abnormal_incorrect_table_type()
        self.run_abnormal_mixed_uid()
