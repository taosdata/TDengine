...
# Constants
# ---------------------------------------------------------------------------
TARGET_DNODE = 3          # Follower dnode whose WAL we corrupt
WAL_ROLL_PERIOD = 1
...
def _setup_db(db: str) -> None:
    ...
    tdSql.execute(
        f"create database {db} replica 3 vgroups 1 "
...
class TestWalCorruptionThreeReplica:

    def setup_class(cls):
        tdLog.debug(f"start to execute {__file__}")

    def test_wal_corruption_three_replica(self):
        """taosd starts normally and queries succeed after WAL corruption
           on a follower dnode in a 3-replica database
        ...
        for position in positions:
            for corruption_name, corruption_fn in corruptions:
                db = f"wal_3r_{position[:2]}_{corruption_name[:3]}"
                ...
                time.sleep(5)  # allow raft sync to complete
        ...
        tdLog.info("[3-replica] All sub-scenarios passed")
