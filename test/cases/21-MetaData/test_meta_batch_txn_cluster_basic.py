...
"""Cluster batch meta txn: basic failover tests (s40-s51).

Requires: ./ci/pytest.sh pytest ... -N 3 -M 3

Tests cover:
  - MNode leader switch during COMMIT/ROLLBACK (s40-s41)
  - Client disconnect auto-rollback (s42)
  - VNode follower/leader restart during txn (s43-s44)
  - Cluster restart, MNode restart committing recovery (s45-s46)
  - VNode leader switch fencing COMMIT/ROLLBACK (s47-s48)
  - Cross-VNode multi-dnode COMMIT/ROLLBACK (s49-s50)
  - Cross-VNode fencing COMMIT (s51)
"""

from new_test_framework.utils import tdLog, tdSql, tdCom, sc, clusterComCheck
import time

class TestBatchMetaTxnClusterBasic:
    """Cluster batch meta txn: basic failover (s40-s51, 3 dnode, 3 mnode)."""
    ...
    def test_meta_batch_txn_cluster_basic(self):
        """Cluster batch meta txn: basic failover (s40-s51)

        40. mnode_leader_switch_commit
        41. mnode_leader_switch_rollback
        42. client_disconnect_auto_rollback
        43. vnode_follower_restart_commit
        44. vnode_leader_restart_commit
        45. cluster_restart_txn_survives
        46. mnode_restart_committing_recovery
        47. fencing_vnode_leader_switch
        48. fencing_vnode_leader_switch_rollback
        49. cross_vnode_multi_dnode_commit
        50. cross_vnode_multi_dnode_rollback
        51. cross_vnode_fencing_commit
