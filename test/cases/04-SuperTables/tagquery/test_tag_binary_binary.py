...
class TestTagBinaryBinary:

    def setup_class(cls):
        tdLog.debug(f"start to execute {__file__}")

    def test_tag_binary_binary(self):
        """两个 Bianry 类型标签的超级表查询

        1. 创建一个包含两个 Binary 类型标签的超级表
        2. 创建子表并插入数据
        ...
        History:
            - 2025-4-17 Simon Guan Migrated to new test framework, from tests/script/tsim/tag/binary_binary.sim
