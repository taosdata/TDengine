...
class TestColumnBigInt:

    def setup_class(cls):
        tdLog.debug(f"start to execute {__file__}")

    def test_column_bigint(self):
        """BigInt 数据列的超级表查询

        1. 创建包含 1 个 BigInt 类型普通数据列和 1 个 BigInt 类型标签列的超级表
        2. 创建子表并写入数据
        ...
        History:
            - 2025-4-28 Simon Guan Migrated to new test framework, from tests/script/tsim/field/bigint.sim
