...
class TestVtableNcharLength:
    ...
    def test_vtable_nchar_len_mix(self):
        ...
        # Check length on mixed columns
        # binary_32_col: binary(16) referencing binary(32) source
        tdSql.query(f"select length(binary_32_col), length(binary_16_col) "
                    f"from {db}.vtb_nchar_mix order by ts;")
        tdSql.checkRows(5)
        for i in range(5):
            if self.SRC_32_BINARY[i] is None:
                tdSql.checkData(i, 0, None)
            else:
                tdSql.checkData(i, 0, len(self.SRC_32_BINARY[i]))
            if self.SRC_16_BINARY[i] is None:
                tdSql.checkData(i, 1, None)
            else:
                tdSql.checkData(i, 1, len(self.SRC_16_BINARY[i]))

        # char_length on nchar columns
        tdSql.query(f"select char_length(nchar_32_col), char_length(nchar_16_col) "
                    f"from {db}.vtb_nchar_mix order by ts;")
        tdSql.checkRows(5)
        for i in range(5):
            if self.SRC_32_NCHAR[i] is None:
                tdSql.checkData(i, 0, None)
            else:
                tdSql.checkData(i, 0, len(self.SRC_32_NCHAR[i]))
            if self.SRC_16_NCHAR[i] is None:
                tdSql.checkData(i, 1, None)
            else:
                tdSql.checkData(i, 1, len(self.SRC_16_NCHAR[i]))

        # Concat across mixed-length columns
        tdSql.query(f"select concat(binary_32_col, binary_16_col) "
                    f"from {db}.vtb_nchar_mix order by ts;")
        tdSql.checkRows(5)
        for i in range(5):
            if self.SRC_32_BINARY[i] is None or self.SRC_16_BINARY[i] is None:
                tdSql.checkData(i, 0, None)
            else:
                tdSql.checkData(i, 0, self.SRC_32_BINARY[i] + self.SRC_16_BINARY[i])
