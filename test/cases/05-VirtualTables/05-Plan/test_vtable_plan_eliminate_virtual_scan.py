...
class TestVtablePlanEliminateVirtualScan:
    ...
    def setup_class(cls):
        ...
        for i in range(4):
            ...
            datafile = etool.getFilePath(os.path.dirname(__file__), "data", "data1.csv")
            tdSql.execute("insert into vtb_org_normal_0 file '%s';" % datafile)
            tdSql.execute("insert into vtb_org_child_0 file '%s';" % datafile)

            datafile = etool.getFilePath(os.path.dirname(__file__), "data", "data2.csv")
            tdSql.execute("insert into vtb_org_normal_0 file '%s';" % datafile)
            tdSql.execute("insert into vtb_org_child_0 file '%s';" % datafile)

            datafile = etool.getFilePath(os.path.dirname(__file__), "data", "data3.csv")
            tdSql.execute("insert into vtb_org_normal_0 file '%s';" % datafile)
            tdSql.execute("insert into vtb_org_child_0 file '%s';" % datafile)
