...
class TestVtableTagRef:
    ...
    def setup_class(cls):
        ...
        tdSql.execute("CREATE DATABASE ts_a VGROUPS 2 BUFFER 16;")
        tdSql.execute("CREATE DATABASE ts_b VGROUPS 3 BUFFER 16;")
        tdSql.execute("CREATE DATABASE ts_c VGROUPS 1 BUFFER 16;")
        tdSql.execute("CREATE DATABASE ts_tag VGROUPS 2 BUFFER 16;")
        tdSql.execute("CREATE DATABASE ts_data VGROUPS 3 BUFFER 16;")
        tdSql.execute("CREATE DATABASE td_main VGROUPS 4 BUFFER 16;")
        tdSql.execute("CREATE DATABASE td_aux VGROUPS 2 BUFFER 16;")
        tdSql.execute("CREATE DATABASE td_split VGROUPS 2 BUFFER 16;")
        tdSql.execute(f"CREATE DATABASE {DB_DDL} BUFFER 16;")
        tdSql.execute(f"CREATE DATABASE {DB_DDL_CROSS} BUFFER 16;")
    ...
    def _check_values(self, sql, expected, desc=""):
        ...
        if nrows != len(expected):
            actual_dump = [tuple(str(tdSql.getData(i, j)) for j in range(ncols)) for i in range(nrows)]
            assert False, (
                f"{desc}: expected {len(expected)} rows, got {nrows}\n"
                f"  Actual rows: {actual_dump}")
    ...
    def test_query_ddl_db_data(self):
        ...
        # vstb super table count (use information_schema to avoid VStbRefDbs
        # catalog race condition that can occur under ASAN+Q3)
        tdSql.query(
            f"SELECT COUNT(*) FROM information_schema.ins_tables "
            f"WHERE db_name = '{DB_DDL}' AND stable_name = 'vstb';"
        )
