...
class TestVtableTagRefShowDesc:
    ...
    def setup_class(cls):
        ...
        tdSql.execute("CREATE DATABASE td_src_other BUFFER 16")
    ...
    def test_show_create_after_drop_source(self):
        ...
        Verify SHOW CREATE, DESCRIBE, and SHOW TAGS still work after the source
        table is dropped.  These commands use stored colRef metadata from the
        vnode and do not require the source table to be alive.
        ...
        # DESCRIBE succeeds using stored colRef metadata
        tdSql.query(f"DESCRIBE {DB}.v_tmp")
        # SHOW TAGS also succeeds (uses stored tag metadata)
        tdSql.query(f"SHOW TAGS FROM {DB}.v_tmp")

        # Clean up
        tdSql.execute(f"DROP TABLE IF EXISTS {DB}.v_tmp")
