...
class TestStreamSubqueryEvent:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=34,
            stream="create stream rdb.s34 event_window(start with c2=0 end with c2=10) from tdb.triggers partition by tbname into rdb.r34 as select _twstart - 5m tc, TIMEDIFF(_twstart - 5m, _twstart + 5m) tx, %%tbname tb, %%1 tg1, sum(c1) c1, avg(c2) c2, first(c1) c3, last(c2) c4 from %%trows;",
        ...
        stream = StreamItem(
            id=38,
            stream="create stream rdb.s38 event_window(start with c2=0 end with c2=10) from tdb.triggers partition by tbname into rdb.r38 as select _twstart ts, _irowts, _isfilled, interp(c1), _twend, _twduration, _twrownum from %%trows RANGE(_twend) FILL(linear);",
            res_query="select `_irowts`, `_isfilled`, `interp(c1)`, `_twrownum` from rdb.r38 where tag_tbname='t1' limit 1",
            exp_query="select _irowts , _isfilled , interp(c1), 2 from tdb.t1 RANGE('2025-01-01 00:01:00.000') FILL(linear);",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=58,
            stream="create stream rdb.s58 event_window(start with c2=0 end with c2=10) from tdb.triggers partition by tbname into rdb.r58 as select _irowts, interp(cbigint), _isfilled from qdb.v1 where ctinyint > 0 and cint > 2 RANGE('2025-01-01 00:02:00.000', '2025-01-01 00:08:00.000') EVERY (1m) FILL(linear) limit 50;",
        ...
        stream = StreamItem(
            id=59,
            stream="create stream rdb.s59 event_window(start with c2=0 end with c2=10) from tdb.triggers partition by tbname into rdb.r59 as select _irowts, interp(csmallint), _isfilled from qdb.meters where ctinyint > 0 and cint > 2 partition by tbname RANGE('2025-01-01 00:02:01.000') EVERY (1m) FILL(next)",
            res_query="select * from rdb.r59 where tag_tbname='t1'",
            exp_query="select timestamp '2025-01-01 00:02:01.000', last(csmallint), true, tbname from qdb.meters where cts >= '2025-01-01 00:02:00.000' and cts <= '2025-01-01 00:02:30.000' and tbname='t1' partition by tbname;",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=83,
            stream="create stream rdb.s83 event_window(start with c2=0 end with c2=10) from tdb.t1 into rdb.r83 as select ts, diff(c1) c3 from %%trows ",
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 event_window(start with c2=0 end with c2=10) from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twstart + 5m partition by tbname order by tbname limit 1 offset 2",
        ...
        stream = StreamItem(
            id=119,
            stream="create stream rdb.s119 event_window(start with c2=0 end with c2=10) from tdb.vtriggers partition by id, tbname into rdb.r119 as select _twstart ts, count(tac1), sum(tbcint) from (select ta.ts tats, tb.cts tbts, ta.c1 tac1, tb.cint tbcint from qdb.meters tb right asof join tdb.t1 ta on ta.ts < tb.cts jlimit 10 where ta.ts >= _twstart and ta.ts < _twend + 4m and cos(ta.c1) > 0);",
            res_query="select * from rdb.r119 where id =2 limit 1",
            exp_query="select cast('2025-01-01 00:15:00.000' as timestamp) ts, count(tac1), sum(tbcint), 2, 'v2' from (select ta.ts tats, tb.cts tbts, ta.c1 tac1, tb.cint tbcint from qdb.meters tb right asof join tdb.t1 ta on ta.ts < tb.cts jlimit 10 where ta.ts >= '2025-01-01 00:15:00.000' and ta.ts < '2025-01-01 00:20:00.000' and cos(ta.c1) > 0);",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=127,
            stream="create stream rdb.s127 event_window(start with c2=0 end with c2=10) from tdb.v1 into rdb.r127 as select _rowts, MAVG(c1, 1) from %%trows;",
