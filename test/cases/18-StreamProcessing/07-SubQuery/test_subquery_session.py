...
class TestStreamSubquerySession:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=5,
            stream="create stream rdb.s5 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r5 as select _twstart ts, _twstart + 5m te, _twduration td, _twrownum tw, %%tbname as tb, count(c1) c1, avg(c2) c2 from %%tbname where ts >= _twstart and ts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=7,
            stream="create stream rdb.s7 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r7 as select _twstart ts, count(c1), avg(c2) from %%tbname where ts >= _twstart and ts <= _twend and %%tbname = tbname",
        ...
        stream = StreamItem(
            id=9,
            stream="create stream rdb.s9 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r9 tags(gid bigint as _tgrpid, id int as %%1) as select _twstart ts, _twstart + 5m te, _twduration td, _twrownum tw, _tgrpid tg, _tlocaltime tl, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and _twduration is not null and _twrownum is not null and _tgrpid is not null and _tlocaltime is not null;",
        ...
        stream = StreamItem(
            id=14,
            stream="create stream rdb.s14 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r14 as select _twstart ts, %%tbname t1, %%1 t2, count(*) c1, avg(c1) c2, first(c1) c3, last(c1) c4 from %%trows;",
        ...
        stream = StreamItem(
            id=15,
            stream="create stream rdb.s15 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r15 as select _wstart, sum(cint), FIRST(cint) from qdb.meters where cts >= _twstart and cts < _twstart + 5m interval(150s)",
        ...
        stream = StreamItem(
            id=17,
            stream="create stream rdb.s17 session(ts, 2m) from tdb.triggers into rdb.r17 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(5m)",
        ...
        stream = StreamItem(
            id=19,
            stream="create stream rdb.s19 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r19 as select _twend + 4m tw, sum(c1) sumcnt from (select tbname, count(*) c1 from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1 partition by tbname)",
        ...
        stream = StreamItem(
            id=20,
            stream="create stream rdb.s20 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r20 as select _twend + 4m tw, count(*) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1;",
        ...
        stream = StreamItem(
            id=22,
            stream="create stream rdb.s22 session(ts, 2m) from tdb.triggers into rdb.r22 as select _twend + 4m tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=23,
            stream="create stream rdb.s23 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r23 as select _twend + 4m tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname=%%tbname partition by tbname",
        ...
        stream = StreamItem(
            id=24,
            stream="create stream rdb.s24 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r24 as select _twend + 4m tw, sum(cint) c1, %%1 c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1 partition by tint",
        ...
        stream = StreamItem(
            id=25,
            stream="create stream rdb.s25 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r25 as select _twstart tw, sum(cint) c1, %%1 c2, %%2 c3 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1",
        ...
        stream = StreamItem(
            id=26,
            stream="create stream rdb.s26 session(ts, 2m) from tdb.triggers into rdb.r26 as select _twstart tw, sum(cint) c1, tbname from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname='t18' partition by tbname",
        ...
        stream = StreamItem(
            id=27,
            stream="create stream rdb.s27 session(ts, 2m) from tdb.v2 partition by tbname into rdb.r27 as select _twstart tw, sum(cint) c1, count(cint) c2 from qdb.vmeters where cts >= _twstart and cts < _twstart + 5m and tbname=%%1",
        ...
        stream = StreamItem(
            id=28,
            stream="create stream rdb.s28 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r28 as select _twstart ts, `name` as cname, `super` csuper, create_time cctime, %%1 from information_schema.ins_users",
        ...
        stream = StreamItem(
            id=29,
            stream="create stream rdb.s29 session(ts, 2m) from tdb.t1 into rdb.r29 as select _wstart ts, sum(cint), avg(cint) from qdb.meters where cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=30,
            stream="create stream rdb.s30 session(ts, 2m) from tdb.n1 into rdb.r30 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(2m)",
        ...
        stream = StreamItem(
            id=32,
            stream="create stream rdb.s32 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r32 as select _wstart ts, sum(cint), avg(cint) from qdb.meters where tbname != %%tbname and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=33,
            stream="create stream rdb.s33 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r33 tags(tbn varchar(128) as cast(%%1 as varchar)) as select _twstart, sum(cint), avg(cint) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=35,
            stream="create stream rdb.s35 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r35 as select _wstart ts, count(c1) c1, sum(c2) c2, %%1 c3, %%2 c4, cast(_tlocaltime / 1000000 as timestamp) c5, _tgrpid c6 from %%trows count_window(2);",
        ...
        stream = StreamItem(
            id=37,
            stream="create stream rdb.s37 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r37 as select _twstart ts, _twend, count(c1), sum(c2) from %%trows",
        ...
        stream = StreamItem(
            id=38,
            stream="create stream rdb.s38 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r38 as select _twstart ts, _irowts, _isfilled, interp(c1), _twend, _twduration, _twrownum from %%trows RANGE(_twend) FILL(prev);",
        ...
        stream = StreamItem(
            id=40,
            stream="create stream rdb.s40 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r40 as select _twstart, table_name, db_name, stable_name, tag_name, tag_value from information_schema.ins_tags where table_name='j0' and stable_name='jmeters' and tag_name='tjson'",
        ...
        stream = StreamItem(
            id=41,
            stream="create stream rdb.s41 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r41 as select cts, cint, cbool, cast(tjson->'k1' as varchar(8)) cjson, _twstart from qdb.j0 where cts >= _twstart and cts < _twstart + 5m and cbool = %%1 order by cts limit 2, 3",
        ...
        stream = StreamItem(
            id=46,
            stream="create stream rdb.s46 session(ts, 2m) from tdb.v1 into rdb.r46 as select _twstart ts, count(c1) ccnt, sum(c2) csum, first(id) cfirst from %%trows",
            res_query="select ts, ccnt, csum, cfirst from rdb.r46 limit 4",
            exp_query="select _wstart, count(*), sum(c2), first(id) from tdb.v1 session(ts, 2m) limit 4",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=49,
            stream="create stream rdb.s49 session(ts, 2m) from tdb.triggers into rdb.r49 as select _twstart ts, ABS(cint), ACOS(cuint), ASIN(cbigint), ATAN(cubigint), CEIL(cfloat), COS(cdouble), DEGREES(csmallint), EXP(cusmallint), FLOOR(ctinyint), LN(cutinyint), LOG(cfloat), MOD(cbigint, cint), PI(), POW(cuint, 2), RADIANS(cbigint), ROUND(cfloat), SIGN(cdouble), SQRT(csmallint), TAN(cfloat), TRUNCATE(cdouble, 1), CRC32(cvarchar) from qdb.meters where cts >=_twstart and cts <= _twend and tbname='t1' order by cts limit 1",
        ...
        stream = StreamItem(
            id=58,
            stream="create stream rdb.s58 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r58 as select _irowts, interp(cbigint), _isfilled from qdb.v1 where ctinyint > 0 and cint > 2 RANGE('2025-01-01 00:02:00.000', '2025-01-01 00:08:00.000') EVERY (1m) FILL(linear) limit 50;",
        ...
        stream = StreamItem(
            id=59,
            stream="create stream rdb.s59 session(ts, 2m) from tdb.triggers partition by id into rdb.r59 as select _irowts, interp(csmallint), _isfilled from qdb.meters where ctinyint > 0 and cint > 2 partition by tbname RANGE('2025-01-01 00:02:01.000') EVERY (1m) FILL(next)",
        ...
        stream = StreamItem(
            id=60,
            stream="create stream rdb.s60 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r60 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=61,
            stream="create stream rdb.s61 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r61 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=62,
            stream="create stream rdb.s62 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r62 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=64,
            stream="create stream rdb.s64 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r64 as select cts, cint, cuint, cbigint, cubigint, cfloat, cdouble, cvarchar, csmallint, cusmallint, ctinyint, cutinyint, cbool, cnchar, cvarbinary from qdb.view1 where cts >= _twstart and cts < _twstart + 5m limit 1",
        ...
        stream = StreamItem(
            id=65,
            stream="create stream rdb.s65 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r65 as select _twstart ts, HISTOGRAM(cfloat, 'user_input', '[1, 7]', 1) th from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=66,
            stream="create stream rdb.s66 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r66 as select _twstart ts, FIRST(cuint), LAST(cbigint), LAST_ROW(cubigint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=67,
            stream="create stream rdb.s67 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r67 as select _twstart ts, MAX(ctinyint), MIN(cutinyint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=68,
            stream="create stream rdb.s68 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r68 as select _twstart ts, mode(cint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=69,
            stream="create stream rdb.s69 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r69 as select _twstart ts, SAMPLE(cdecimal8, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=70,
            stream="create stream rdb.s70 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r70 as select _twstart ts, BOTTOM(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=71,
            stream="create stream rdb.s71 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r71 as select _twstart ts, TAIL(cbigint, 1)  from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=72,
            stream="create stream rdb.s72 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r72 as select _twstart ts, TOP(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=73,
            stream="create stream rdb.s73 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r73 as select _rowts ts, _twstart tws, _twend twe, CSUM(cint) + CSUM(cuint) val from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=75,
            stream="create stream rdb.s75 session(ts, 2m) from tdb.vtriggers partition by id, tbname into rdb.r75 as select _wstart, twa(c1), avg(c1), count(c1) from tdb.vtriggers where id=%%1 and ts>=_twstart and ts < _twend + 4m interval(1m) fill(prev)",
        ...
        stream = StreamItem(
            id=77,
            stream="create stream rdb.s77 session(ts, 2m) from tdb.vtriggers partition by id, tbname into rdb.r77 as select _twstart ts, count(*) c1 from information_schema.ins_streams where stream_name='s77'",
        ...
        stream = StreamItem(
            id=78,
            stream="create stream rdb.s78 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r78 as select _twstart, _twrownum, count(*), sum(cdecimal8) from qdb.meters where tbname !=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=79,
            stream="create stream rdb.s79 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r79 as select _twstart, sum(cdecimal8), count(*), _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=80,
            stream="create stream rdb.s80 session(ts, 2m) from tdb.triggers into rdb.r80 as select _twstart, sum(v1), sum(v2) from (select count(c1) v1, sum(c2) v2 from %%trows)",
        ...
        stream = StreamItem(
            id=81,
            stream="create stream rdb.s81 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r81 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 4m and cint > 5 and _twrownum > 0 ",
        ...
        stream = StreamItem(
            id=82,
            stream="create stream rdb.s82 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r82 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 4m and cint > 5",
        ...
        stream = StreamItem(
            id=83,
            stream="create stream rdb.s83 session(ts, 2m) from tdb.t1 into rdb.r83 as select ts, diff(c1) c3 from %%trows ",
        ...
        stream = StreamItem(
            id=84,
            stream="create stream rdb.s84 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r84 as select cts, top(cint, 5) from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 4m order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=85,
            stream="create stream rdb.s85 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r85 as select cts, top(cint, 5) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=86,
            stream="create stream rdb.s86 session(ts, 2m) from tdb.n1 into rdb.r86 as select ts, top(c1, 5) from %%trows limit 1 offset 1;",
        ...
        stream = StreamItem(
            id=87,
            stream="create stream rdb.s87 session(ts, 2m) from tdb.t1 into rdb.r87 as select last(cts, cint, cuint) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=88,
            stream="create stream rdb.s88 session(ts, 2m) from tdb.v1 into rdb.r88 as select _twstart tw, cbool, cast(avg(cint) as int) from qdb.view1 where cts >= _twstart and cts < _twend + 4m and cbool = true group by cbool having avg(cint) > 0",
        ...
        stream = StreamItem(
            id=89,
            stream="create stream rdb.s89 session(ts, 2m) from tdb.v1 into rdb.r89 as select _twstart tw, _c0 ta, _rowts tb, c1, c2, rand() c3 from %%trows order by _c0 limit 1",
        ...
        stream = StreamItem(
            id=92,
            stream="create stream rdb.s92 session(ts, 2m) from tdb.n1 into rdb.r92 as select _twstart, tint, sum(cint) amount from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname='t1' partition by tbname",
        ...
        stream = StreamItem(
            id=93,
            stream="create stream rdb.s93 session(ts, 2m) from tdb.v1 into rdb.r93 as select _twstart, sum(cnt) from (select _twstart ts, count(*) cnt from information_schema.ins_tables where db_name='qdb' union all select _twstart ts, count(*) cnt from information_schema.ins_tables where db_name='tdb')",
        ...
        stream = StreamItem(
            id=94,
            stream="create stream rdb.s94 session(ts, 2m) from tdb.v1 into rdb.r94 as select ts, c1, c2 from (select ts, c1, c2 from tdb.t3 union all select ts, c1, c2 from %%trows)",
        ...
        stream = StreamItem(
            id=95,
            stream="create stream rdb.s95 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r95 as select _twstart, _twend + 4m, last(cts, cint, cuint) from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=96,
            stream="create stream rdb.s96 session(ts, 2m) from tdb.t1 into rdb.r96 (ts, ti composite key, te, tx, ty) as select _twstart, tint, _twend + 4m, count(cint), sum(cint) from qdb.meters where cts >= _twstart and cts < _twend + 4m and tint is not NULL partition by tint",
        ...
        stream = StreamItem(
            id=98,
            stream="create stream rdb.s98 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r98 as select last(cts, cint, cbool) from qdb.jmeters where cts >= _twstart and cts < _twstart + 5m and tjson->'k1'='v1' partition by tjson->'k1'",
        ...
        stream = StreamItem(
            id=100,
            stream="create stream rdb.s100 session(ts, 2m) from tdb.v1 into rdb.r100 as select _twstart, count(*) from qdb.jmeters where cts >= _twstart and cts < _twend + 4m and tjson->'k1' match 'v1'",
        ...
        stream = StreamItem(
            id=102,
            stream="create stream rdb.s102 session(ts, 2m) from tdb.t1 into rdb.r102 as select last(ts), cols(last(ts), c1, c2), count(1) from %%trows",
        ...
        stream = StreamItem(
            id=106,
            stream="create stream rdb.s106 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r106 as select _twstart, cts, concat(cvarchar, cnchar), cint + cuint, ctinyint - cdouble, cfloat * cdouble, cbigint * 12, -ctinyint from qdb.meters where tbname=%%tbname limit 1 offset 1;",
        ...
        stream = StreamItem(
            id=107,
            stream="create stream rdb.s107 session(ts, 2m) from tdb.v1 into rdb.r107 as select _twstart, cts, cvarchar like 'SanFrancisco', cnchar not like 'SanFrancisco', cvarchar regexp '%co', cnchar not regexp '%co' from qdb.t5 limit 1 offset 2;",
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 session(ts, 2m) from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twstart + 5m partition by tbname order by tbname limit 1 offset 2",
        ...
        stream = StreamItem(
            id=109,
            stream="create stream rdb.s109 session(ts, 2m) from tdb.v2 into rdb.r109 as select _twstart, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2) from tdb.v2 ta join tdb.t2 tb on ta.ts = tb.ts where ta.ts >= _twstart and ta.ts < _twend + 4m group by ta.c2 having sum(tb.c2) > 130;",
        ...
        stream = StreamItem(
            id=111,
            stream="create stream rdb.s111 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r111 as select _wstart, _wend, _twstart, _twend + 4m, csmallint, count(cts), count(cbool) from qdb.meters where cts >= _twstart and cts < _twstart + 5m partition by tbname state_window(csmallint)",
        ...
        stream = StreamItem(
            id=113,
            stream="create stream rdb.s113 session(ts, 2m) from tdb.t1 into rdb.r113 as select _wstart, _wend, _twstart, _twend + 4m, first(csmallint), count(cts), count(cbool) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m and csmallint = 2 session(cts, 40s);",
        ...
        stream = StreamItem(
            id=115,
            stream="create stream rdb.s115 session(ts, 2m) from tdb.triggers partition by tbname into rdb.r115 as select ta.cts tats, tb.cts tbts, ta.cint tat1, tb.cint tbt1, _twstart, _twend + 4m from qdb.t1 ta left join qdb.t2 tb on ta.cts=tb.cts where ta.cts >= _twstart and ta.cts < _twend + 4m;",
        ...
        stream = StreamItem(
            id=117,
            stream="create stream rdb.s117 session(ts, 2m) from tdb.t1 into rdb.r117 as select _twstart, count(tac1), sum(tbcint) from (select ta.ts tats, tb.cts tbts, ta.c1 tac1, tb.cint tbcint from tdb.t1 ta left asof join qdb.t1 tb on ta.ts < tb.cts jlimit 5 where ta.ts >= _twstart and ta.ts < _twend + 4m);",
        ...
        stream = StreamItem(
            id=123,
            stream="create stream rdb.s123 session(ts, 2m) from tdb.triggers partition by id, name, tbname into rdb.r123 as select _wstart tw, _wend te, _twstart, _twend + 4m, count(c1) c1, sum(c2) c2 from %%trows interval(1m)",
        ...
        stream = StreamItem(
            id=125,
            stream="create stream rdb.s125 session(ts, 2m) from tdb.v1 into rdb.r125 as select _rowts, _twstart ts, DERIVATIVE(cbigint, 5, 0) from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=126,
            stream="create stream rdb.s126 session(ts, 2m) from tdb.vtriggers partition by id, tbname into rdb.r126 as select _twstart ts, IRATE(cbigint) from qdb.vmeters where tbname=%%tbname;",
        ...
        stream = StreamItem(
            id=127,
            stream="create stream rdb.s127 session(ts, 2m) from tdb.v1 into rdb.r127 as select _rowts, MAVG(c1, 1) from %%trows;",
        ...
        stream = StreamItem(
            id=128,
            stream="create stream rdb.s128 session(ts, 2m) from tdb.vtriggers partition by tbname into rdb.r128 as select _rowts, STATECOUNT(cdouble, 'LT', 5) from qdb.vmeters where tbname=%%tbname and cts >= _twstart and cts < _twend + 4m;",
        ...
        stream = StreamItem(
            id=129,
            stream="create stream rdb.s129 session(ts, 2m) from tdb.triggers partition by id, tbname into rdb.r129 as select _c0, STATEDURATION(cusmallint, 'LT', 5, 1m) from qdb.meters where tbname=%%2 and cts >= _twstart and cts < _twend + 4m;",
        ...
        stream = StreamItem(
            id=130,
            stream="create stream rdb.s130 session(ts, 2m) from tdb.vtriggers partition by tbname into rdb.r130 as select _twstart ts, _twend + 4m, TWA(ctinyint) from qdb.vmeters where tbname=%%1 and cts >= _twstart and cts < _twend + 4m;",
        ...
        stream = StreamItem(
            id=132,
            stream="create stream rdb.s132 session(ts, 2m) from tdb.t1 partition by id into rdb.r132 as select tb.cts tbts, ta.ts tats, ta.c1 tac1, ta.c2 tac2, tb.cint tbc1, tb.cuint tbc2, _twstart, _twend from tdb.t1 ta right join qdb.t1 tb on ta.ts=tb.cts where tb.cts >= _twstart and tb.cts < _twend + 4m;",
    ...
    def check80(self):
        tdSql.checkResultsByFunc(
            ...
            and tdSql.compareData(3, 2, 10)
            and tdSql.compareData(4, 0, "2025-01-01 00:20:00.000")
            and tdSql.compareData(4, 1, 2)
            and tdSql.compareData(4, 2, 420)
            and tdSql.compareData(5, 0, "2025-01-01 00:30:00.000")
            and tdSql.compareData(5, 1, 2)
