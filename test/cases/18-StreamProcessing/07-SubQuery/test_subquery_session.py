...
class TestStreamSubquerySession:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 session(ts, 2m) from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twstart + 5m partition by tbname slimit 1 soffset 2",
