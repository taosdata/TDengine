...
class TestStreamSubquerySliding:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=38,
            stream="create stream rdb.s38 interval(5m) sliding(5m) from tdb.triggers partition by tbname into rdb.r38 as select _twstart ts, _irowts, _isfilled, interp(c1), _twend, _twduration, _twrownum from %%trows RANGE(_twend) FILL(prev);",
            res_query="select `_irowts`, `_isfilled`, `interp(c1)`, `_twrownum` from rdb.r38 where tag_tbname='t1' limit 1",
            exp_query="select _irowts , _isfilled , interp(c1), 1 from tdb.t1 where _c0 >= '2025-01-01 00:00:00.000' and _c0 < '2025-01-01 00:05:00.000' RANGE('2025-01-01 00:05:00.000') FILL(prev);",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=58,
            stream="create stream rdb.s58 interval(5m) sliding(5m) from tdb.triggers partition by tbname into rdb.r58 as select _irowts, interp(cbigint), _isfilled from qdb.v1 where ctinyint > 0 and cint > 2 RANGE('2025-01-01 00:02:00.000', '2025-01-01 00:08:00.000') EVERY (1m) FILL(linear) limit 50;",
        ...
        stream = StreamItem(
            id=59,
            stream="create stream rdb.s59 interval(5m) sliding(5m) from tdb.triggers partition by id into rdb.r59 as select _irowts, interp(csmallint), _isfilled from qdb.meters where ctinyint > 0 and cint > 2 partition by tbname RANGE('2025-01-01 00:02:01.000') EVERY (1m) FILL(next)",
            res_query="select `_irowts`, `interp(csmallint)`, `_isfilled` from rdb.r59 where id=1",
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 interval(5m) sliding(5m) from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twend partition by tbname order by tbname limit 1 offset 2",
