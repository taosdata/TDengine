...
class TestStreamSubqueryCount:
    ...
    def createStreams(self):
        ...
        stream = StreamItem(
            id=0,
            stream="create stream rdb.s0 count_window(1, c1) from tdb.v1 into rdb.r0 as select _twstart ts, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=5,
            stream="create stream rdb.s5 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r5 as select _twstart ts, _twstart + 5m te, _twduration td, _twrownum tw, %%tbname as tb, count(c1) c1, avg(c2) c2 from %%tbname where ts >= _twstart and ts < _twstart + 5m",
            res_query="select ts, te, td, tw, tb, c1, c2, tag_tbname from rdb.r5 where tag_tbname='t1' limit 3;",
            exp_query="select _wstart, _wend, 0, count(c1), 't1', count(c1), avg(c2), 't1' from tdb.t1 where ts >= '2025-01-01 00:00:00' and ts < '2025-01-01 00:15:00' interval(5m) fill(value, 0, 0, null);",
        ...
        stream = StreamItem(
            id=7,
            stream="create stream rdb.s7 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r7 as select _twstart ts, count(c1), avg(c2) from %%tbname where ts >= _twstart and ts <= _twend and %%tbname = tbname",
        ...
        stream = StreamItem(
            id=9,
            stream="create stream rdb.s9 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r9 tags(gid bigint as _tgrpid, id int as %%1) as select _twstart ts, _twstart + 5m te, _twduration td, _twrownum tw, _tgrpid tg, _tlocaltime tl, count(cint) c1, avg(cint) c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and _twduration is not null and _twrownum is not null and _tgrpid is not null and _tlocaltime is not null;",
        ...
        stream = StreamItem(
            id=14,
            stream="create stream rdb.s14 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r14 as select _twstart ts, %%tbname t1, %%1 t2, count(*) c1, avg(c1) c2, first(c1) c3, last(c1) c4 from %%trows;",
        ...
        stream = StreamItem(
            id=15,
            stream="create stream rdb.s15 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r15 as select _wstart, sum(cint), FIRST(cint) from qdb.meters where cts >= _twstart and cts < _twstart + 5m interval(150s)",
        ...
        stream = StreamItem(
            id=16,
            stream="create stream rdb.s16 count_window(1, c1) from tdb.n1 into rdb.r16 as select _twstart ts, count(*) c1, avg(cint) c2, _twstart + 1 as ts2, case when _tgrpid != 0 then 1 else 0 end from qdb.meters where tbname = 't1' partition by tbname;",
        ...
        stream = StreamItem(
            id=17,
            stream="create stream rdb.s17 count_window(1, c1) from tdb.v1 into rdb.r17 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(5m)",
        ...
        stream = StreamItem(
            id=19,
            stream="create stream rdb.s19 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r19 as select _twend + 5m tw, sum(c1) sumcnt from (select tbname, count(*) c1 from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1 partition by tbname)",
        ...
        stream = StreamItem(
            id=20,
            stream="create stream rdb.s20 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r20 as select _twend + 5m tw, count(*) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' and tint=%%1;",
        ...
        stream = StreamItem(
            id=21,
            stream="create stream rdb.s21 count_window(1, c1) from tdb.n1 into rdb.r21 as select _twstart tw, count(*) c1, case when _tgrpid != 0 then 1 else 0 end tg, _tlocaltime tl from qdb.meters where cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000'",
        ...
        stream = StreamItem(
            id=22,
            stream="create stream rdb.s22 count_window(1, c1) from tdb.v1 into rdb.r22 as select _twend + 5m tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=23,
            stream="create stream rdb.s23 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r23 as select _twend + 5m tw, sum(cint) c1, _tgrpid tg, _tlocaltime tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname=%%tbname partition by tbname",
        ...
        stream = StreamItem(
            id=24,
            stream="create stream rdb.s24 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r24 as select _twend + 5m tw, sum(cint) c1, %%1 c2 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1 partition by tint",
        ...
        stream = StreamItem(
            id=25,
            stream="create stream rdb.s25 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r25 as select _twstart tw, sum(cint) c1, %%1 c2, %%2 c3 from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint=%%1",
        ...
        stream = StreamItem(
            id=26,
            stream="create stream rdb.s26 count_window(1, c1) from tdb.v1 into rdb.r26 as select _twstart tw, sum(cint) c1, tbname from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname='t18' partition by tbname",
        ...
        stream = StreamItem(
            id=27,
            stream="create stream rdb.s27 count_window(1, c1) from tdb.v2 partition by tbname into rdb.r27 as select _twstart tw, sum(cint) c1, count(cint) c2 from qdb.vmeters where cts >= _twstart and cts < _twstart + 5m and tbname=%%1",
        ...
        stream = StreamItem(
            id=28,
            stream="create stream rdb.s28 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r28 as select _twstart ts, `name` as cname, `super` csuper, create_time cctime, %%1 from information_schema.ins_users",
        ...
        stream = StreamItem(
            id=29,
            stream="create stream rdb.s29 count_window(1, c1) from tdb.t1 into rdb.r29 as select _wstart ts, sum(cint), avg(cint) from qdb.meters where cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=30,
            stream="create stream rdb.s30 count_window(1, c1) from tdb.n1 into rdb.r30 as select _wstart, sum(cint), FIRST(cint) from qdb.meters interval(2m)",
        ...
        stream = StreamItem(
            id=32,
            stream="create stream rdb.s32 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r32 as select _wstart ts, sum(cint), avg(cint) from qdb.meters where tbname != %%tbname and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=33,
            stream="create stream rdb.s33 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r33 tags(tbn varchar(128) as cast(%%1 as varchar)) as select _twstart, sum(cint), avg(cint) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m interval(1m);",
        ...
        stream = StreamItem(
            id=35,
            stream="create stream rdb.s35 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r35 as select _wstart ts, count(c1) c1, sum(c2) c2, %%1 c3, %%2 c4, cast(_tlocaltime / 1000000 as timestamp) c5, _tgrpid c6 from %%trows count_window(2);",
        ...
        stream = StreamItem(
            id=37,
            stream="create stream rdb.s37 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r37 as select _twstart ts, _twend, count(c1), sum(c2) from %%trows",
            res_query="select * from rdb.r37 where id == 1 limit 3",
            exp_query="select _wstart, _wstart, count(c1), sum(c2), 1, '1', 't1' from tdb.triggers where ts >= '2025-01-01 00:00:00.000' and ts < '2025-01-01 00:15:00.000' and id = 1 interval(5m) fill(value, 0, NULL);",
        )
        self.streams.append(stream)
        ...
        stream = StreamItem(
            id=39,
            stream="create stream rdb.s39 count_window(1, c1) from tdb.v1 into rdb.r39 as select _twstart + 5m tn, TIMETRUNCATE(_twstart + 5m, 1d) tnt, sum(cint) c1, _tgrpid tg, TIMETRUNCATE(cast(_tlocaltime /1000000 as timestamp), 1d) tl from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tint = 1 partition by tint",
        ...
        stream = StreamItem(
            id=40,
            stream="create stream rdb.s40 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r40 as select _twstart, table_name, db_name, stable_name, tag_name, tag_value from information_schema.ins_tags where table_name='j0' and stable_name='jmeters' and tag_name='tjson'",
        ...
        stream = StreamItem(
            id=41,
            stream="create stream rdb.s41 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r41 as select cts, cint, cbool, cast(tjson->'k1' as varchar(8)) cjson, _twstart from qdb.j0 where cts >= _twstart and cts < _twstart + 5m and cbool = %%1 order by cts limit 2, 3",
        ...
        stream = StreamItem(
            id=46,
            stream="create stream rdb.s46 count_window(1, c1) from tdb.v1 into rdb.r46 as select _twstart ts, count(c1) ccnt, sum(c2) csum, first(id) cfirst from %%trows",
            res_query="select ts, csum from rdb.r46 limit 4",
            exp_query="select ts, c2 from tdb.v1;",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=47,
            stream="create stream rdb.s47 count_window(1, c1) from tdb.v1 into rdb.r47 as select _twstart ts, sum(`vgroups`) c1, sum(ntables) c2 from information_schema.ins_databases where name != 'information_schema' and name != 'performance_schema'",
        ...
        stream = StreamItem(
            id=49,
            stream="create stream rdb.s49 count_window(1, c1) from tdb.v1 into rdb.r49 as select _twstart ts, ABS(cint), ACOS(cuint), ASIN(cbigint), ATAN(cubigint), CEIL(cfloat), COS(cdouble), DEGREES(csmallint), EXP(cusmallint), FLOOR(ctinyint), LN(cutinyint), LOG(cfloat), MOD(cbigint, cint), PI(), POW(cuint, 2), RADIANS(cbigint), ROUND(cfloat), SIGN(cdouble), SQRT(csmallint), TAN(cfloat), TRUNCATE(cdouble, 1), CRC32(cvarchar) from qdb.meters where cts >=_twstart and cts <= _twend and tbname='t1' order by cts limit 1",
        ...
        stream = StreamItem(
            id=60,
            stream="create stream rdb.s60 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r60 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=61,
            stream="create stream rdb.s61 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r61 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=62,
            stream="create stream rdb.s62 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r62 as select _twstart ts, RAND(), NOW(), TODAY(), TIMEZONE() from qdb.n2 where _tgrpid != 0 order by cts desc limit 1",
        ...
        stream = StreamItem(
            id=64,
            stream="create stream rdb.s64 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r64 as select cts, cint, cuint, cbigint, cubigint, cfloat, cdouble, cvarchar, csmallint, cusmallint, ctinyint, cutinyint, cbool, cnchar, cvarbinary from qdb.view1 where cts >= _twstart and cts < _twstart + 5m limit 1",
        ...
        stream = StreamItem(
            id=65,
            stream="create stream rdb.s65 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r65 as select _twstart ts, HISTOGRAM(cfloat, 'user_input', '[1, 7]', 1) th from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=66,
            stream="create stream rdb.s66 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r66 as select _twstart ts, FIRST(cuint), LAST(cbigint), LAST_ROW(cubigint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=67,
            stream="create stream rdb.s67 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r67 as select _twstart ts, MAX(ctinyint), MIN(cutinyint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=68,
            stream="create stream rdb.s68 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r68 as select _twstart ts, mode(cint) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=69,
            stream="create stream rdb.s69 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r69 as select _twstart ts, SAMPLE(cdecimal8, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=70,
            stream="create stream rdb.s70 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r70 as select _twstart ts, BOTTOM(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=71,
            stream="create stream rdb.s71 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r71 as select _twstart ts, TAIL(cbigint, 1)  from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=72,
            stream="create stream rdb.s72 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r72 as select _twstart ts, TOP(cint, 1) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=73,
            stream="create stream rdb.s73 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r73 as select _rowts ts, _twstart tws, _twend twe, CSUM(cint) + CSUM(cuint) val from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=75,
            stream="create stream rdb.s75 count_window(1, c1) from tdb.vtriggers partition by id, tbname into rdb.r75 as select _wstart, twa(c1), avg(c1), count(c1) from tdb.vtriggers where id=%%1 and ts>=_twstart and ts < _twend + 5m interval(1m) fill(prev)",
        ...
        stream = StreamItem(
            id=77,
            stream="create stream rdb.s77 count_window(1, c1) from tdb.vtriggers partition by id, tbname into rdb.r77 as select _twstart ts, count(*) c1 from information_schema.ins_streams where stream_name='s77'",
        ...
        stream = StreamItem(
            id=78,
            stream="create stream rdb.s78 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r78 as select _twstart, _twrownum, count(*), sum(cdecimal8) from qdb.meters where tbname !=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=79,
            stream="create stream rdb.s79 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r79 as select _twstart, sum(cdecimal8), count(*), _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m and _twrownum > 0;",
        ...
        stream = StreamItem(
            id=80,
            stream="create stream rdb.s80 count_window(1, c1) from tdb.v2 into rdb.r80 as select _twstart, sum(v1), sum(v2) from (select count(c1) v1, sum(c2) v2 from %%trows)",
        ...
        stream = StreamItem(
            id=81,
            stream="create stream rdb.s81 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r81 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 5m and cint > 5 and _twrownum > 0 ",
        ...
        stream = StreamItem(
            id=82,
            stream="create stream rdb.s82 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r82 as select cts, diff(cint) c3, _twstart, _twend, _twrownum from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 5m and cint > 5",
        ...
        stream = StreamItem(
            id=83,
            stream="create stream rdb.s83 count_window(1, c1) from tdb.t1 into rdb.r83 as select _twstart ts, count(*) from (select ts, diff(c1) c3 from %%trows)",
            res_query="select * from rdb.r83",
            exp_query="select ts, 0 from tdb.t1;",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=84,
            stream="create stream rdb.s84 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r84 as select cts, top(cint, 5) from qdb.meters where tbname=%%1 and cts >= _twstart and cts <= _twend + 5m order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=85,
            stream="create stream rdb.s85 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r85 as select cts, top(cint, 5) from qdb.meters where tint=%%1 and cts >= _twstart and cts < _twstart + 5m order by cint limit 2 offset 2;",
        ...
        stream = StreamItem(
            id=86,
            stream="create stream rdb.s86 count_window(1, c1) from tdb.n1 into rdb.r86 as select ts, top(c1, 5) from %%trows limit 1 offset 0;",
            res_query="select * from rdb.r86",
            exp_query="select ts, c1 from tdb.n1;;",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=87,
            stream="create stream rdb.s87 count_window(1, c1) from tdb.t1 into rdb.r87 as select last(cts, cint, cuint) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=88,
            stream="create stream rdb.s88 count_window(1, c1) from tdb.v1 into rdb.r88 as select _twstart tw, cbool, cast(avg(cint) as int) from qdb.view1 where cts >= _twstart and cts < _twend + 5m and cbool = true group by cbool having avg(cint) > 0",
        ...
        stream = StreamItem(
            id=89,
            stream="create stream rdb.s89 count_window(1, c1) from tdb.v1 into rdb.r89 as select _twstart tw, _c0 ta, _rowts tb, c1, c2, rand() c3 from %%trows order by _c0 limit 1",
        ...
        stream = StreamItem(
            id=90,
            stream="create stream rdb.s90 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r90 as select _twstart ts, sum(tint), sum(t1), sum(t2) from (select _twstart, tint, count(val) t1, sum(val) t2 from (select _wstart, tint, count(cint) val from qdb.meters where tint=%%1 and (cts between _twstart and _twend + 5m) partition by tint interval(1m) having (count(cint) > 0)) tb partition by tint having count(tint) > 0)",
        ...
        stream = StreamItem(
            id=92,
            stream="create stream rdb.s92 count_window(1, c1) from tdb.n1 into rdb.r92 as select _twstart, tint, sum(cint) amount from qdb.meters where cts >= _twstart and cts < _twstart + 5m and tbname='t1' partition by tbname",
        ...
        stream = StreamItem(
            id=93,
            stream="create stream rdb.s93 count_window(1, c1) from tdb.v1 into rdb.r93 as select _twstart, sum(cnt) from (select _twstart ts, count(*) cnt from information_schema.ins_tables where db_name='qdb' union all select _twstart ts, count(*) cnt from information_schema.ins_tables where db_name='tdb')",
        ...
        stream = StreamItem(
            id=94,
            stream="create stream rdb.s94 count_window(1, c1) from tdb.v1 into rdb.r94 as select ts, c1, c2 from (select ts, c1, c2 from tdb.t3 union all select ts, c1, c2 from %%trows)",
        ...
        stream = StreamItem(
            id=95,
            stream="create stream rdb.s95 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r95 as select _twstart, _twend + 5m, last(cts, cint, cuint) from qdb.meters where tbname=%%1 and cts >= _twstart and cts < _twstart + 5m",
        ...
        stream = StreamItem(
            id=96,
            stream="create stream rdb.s96 count_window(1, c1) from tdb.t1 into rdb.r96 (ts, ti composite key, te, tx, ty) as select _twstart, tint, _twend + 5m, count(cint), sum(cint) from qdb.meters where cts >= _twstart and cts < _twend + 5m and tint is not NULL partition by tint",
        ...
        stream = StreamItem(
            id=98,
            stream="create stream rdb.s98 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r98 as select last(cts, cint, cbool) from qdb.jmeters where cts >= _twstart and cts < _twstart + 5m and tjson->'k1'='v1' partition by tjson->'k1'",
        ...
        stream = StreamItem(
            id=99,
            stream="create stream rdb.s99 count_window(1, c1) from tdb.v1 into rdb.r99 as (select cts, cint from qdb.t1 where cts >= _twstart and cts <= _twend + 5m limit 1 offset 0) union all (select cts, cint from qdb.t2 where cts >= _twstart and cts <= _twend + 5m limit 1 offset 2) union all (select cts, cint from qdb.t3 where cts >= _twstart and cts <= _twend + 5m limit 1 offset 4)",
        ...
        stream = StreamItem(
            id=100,
            stream="create stream rdb.s100 count_window(1, c1) from tdb.v1 into rdb.r100 as select _twstart, count(*) from qdb.jmeters where cts >= _twstart and cts < _twend + 5m and tjson->'k1' match 'v1'",
        ...
        stream = StreamItem(
            id=102,
            stream="create stream rdb.s102 count_window(1, c1) from tdb.t1 into rdb.r102 as select last(ts), cols(last(ts), c1, c2), count(1) from %%trows",
            res_query="select * from rdb.r102",
            exp_query="select last(ts), cols(last(ts), c1, c2), count(1) from tdb.t1 interval(5m);",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=103,
            stream="create stream rdb.s103 count_window(1, c1) from tdb.vtriggers partition by tbname into rdb.r103 as select ts, cast(c1 as varchar) like '0' t1, cast(c1 as varchar) not like '0' t2, cast(c1 as varchar) regexp '[0-9]+' t3, cast(c1 as varchar) not regexp '[0-9]+' t4 from tdb.vtriggers where tbname=%%tbname and ts >= _twstart and ts < _twend + 5m limit 1",
        ...
        stream = StreamItem(
            id=105,
            stream="create stream rdb.s105 count_window(1, c1) from tdb.v2 partition by tbname into rdb.r105 as select t1, c11, c12, t2, c21, c22, _twstart, _twend + 5m from (select cols(last_row(cint), cts as t1, cuint as c11), last_row(cint) c12,  cols(first(cint), cts as t2, cuint c21), first(cint) c22  from qdb.meters where tbname='t1' and cts >= _twstart and cts < _twstart + 5m)",
        ...
        stream = StreamItem(
            id=106,
            stream="create stream rdb.s106 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r106 as select _twstart, cts, concat(cvarchar, cnchar), cint + cuint, ctinyint - cdouble, cfloat * cdouble, cbigint * 12, -ctinyint from qdb.meters where tbname=%%tbname limit 1 offset 1;",
        ...
        stream = StreamItem(
            id=107,
            stream="create stream rdb.s107 count_window(1, c1) from tdb.v1 into rdb.r107 as select _twstart, cts, cvarchar like 'SanFrancisco', cnchar not like 'SanFrancisco', cvarchar regexp '%co', cnchar not regexp '%co' from qdb.t5 limit 1 offset 2;",
        ...
        stream = StreamItem(
            id=108,
            stream="create stream rdb.s108 count_window(1, c1) from tdb.t1 into rdb.r108 as select _twstart, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= _twstart and cts < _twstart + 5m partition by tbname order by tbname limit 1 offset 2",
            res_query="select * from rdb.r108 limit 1",
            exp_query="select cast('2025-01-01 00:00:00.000' as timestamp) ts, count(cts), first(cuint), last(cuint), sum(cuint), tbname from qdb.meters where tbname in ('t1', 't2', 't3', 't4') and cts >= '2025-01-01 00:00:00.000' and cts < '2025-01-01 00:05:00.000' partition by tbname order by tbname limit 1 offset 2",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=109,
            stream="create stream rdb.s109 count_window(1, c1) from tdb.v2 into rdb.r109 as select _twstart, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2) from tdb.v2 ta join tdb.t2 tb on ta.ts = tb.ts where ta.ts >= _twstart and ta.ts < _twend + 5m group by ta.c2 having sum(tb.c2) > 130;",
        ...
        stream = StreamItem(
            id=111,
            stream="create stream rdb.s111 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r111 as select _wstart, _wend, _twstart, _twend + 5m, csmallint, count(cts), count(cbool) from qdb.meters where cts >= _twstart and cts < _twstart + 5m partition by tbname state_window(csmallint)",
        ...
        stream = StreamItem(
            id=112,
            stream="create stream rdb.s112 count_window(1, c1) from tdb.vtriggers partition by tbname into rdb.r112 as select _twstart, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2) from %%tbname ta join tdb.t2 tb on ta.ts = tb.ts where ta.ts >= _twstart and ta.ts < _twend + 5m group by ta.c2 having sum(tb.c2) > 130;",
        ...
        stream = StreamItem(
            id=113,
            stream="create stream rdb.s113 count_window(1, c1) from tdb.t1 into rdb.r113 as select _wstart, _wend, _twstart, _twend + 5m, first(csmallint), count(cts), count(cbool) from qdb.t1 where cts >= _twstart and cts < _twstart + 5m and csmallint = 2 session(cts, 40s);",
        ...
        stream = StreamItem(
            id=114,
            stream="create stream rdb.s114 count_window(1, c1) from tdb.vtriggers partition by tbname into rdb.r114 as select _twstart, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2) from tdb.t2 ta join %%tbname tb on ta.ts = tb.ts where ta.ts >= _twstart and ta.ts < _twend + 10m group by ta.c2 having sum(tb.c2) > 130;",
            res_query="select * from rdb.r114 where tag_tbname='v2'",
            exp_query="select cast('2025-01-01 00:10:00.000' as timestamp) ts, count(ta.c1), count(ta.c2), sum(ta.c2), count(tb.c1), count(tb.c2), sum(tb.c2), 'v2' from tdb.t2 ta join tdb.v2 tb on ta.ts = tb.ts where ta.ts >= '2025-01-01 00:10:00.000' and ta.ts < '2025-01-01 00:20:00.000' group by ta.c2 having sum(tb.c2) > 130;",
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=115,
            stream="create stream rdb.s115 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r115 as select ta.cts tats, tb.cts tbts, ta.cint tat1, tb.cint tbt1, _twstart, _twend + 5m from qdb.t1 ta left join qdb.t2 tb on ta.cts=tb.cts where ta.cts >= _twstart and ta.cts < _twend + 5m;",
        ...
        stream = StreamItem(
            id=116,
            stream="create stream rdb.s116 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r116 as select _wstart, _wend, _twstart, _twend + 5m, _wduration, first(csmallint), count(cts), count(cbool) from qdb.meters where tbname=%%tbname and cts >= _twstart and cts < _twstart + 5m partition by tbname event_window start with csmallint = 1 end with csmallint = 2;",
        ...
        stream = StreamItem(
            id=117,
            stream="create stream rdb.s117 count_window(1, c1) from tdb.t1 into rdb.r117 as select _twstart, count(tac1), sum(tbcint) from (select ta.ts tats, tb.cts tbts, ta.c1 tac1, tb.cint tbcint from tdb.t1 ta left asof join qdb.t1 tb on ta.ts < tb.cts jlimit 5 where ta.ts >= _twstart and ta.ts < _twend + 5m);",
        ...
        stream = StreamItem(
            id=118,
            stream="create stream rdb.s118 count_window(1, c1) from tdb.vtriggers partition by id, tbname into rdb.r118 as select _twstart ts, count(tac1), sum(tbcint) from (select ta.ts tats, tb.cts tbts, ta.c1 tac1, tb.cint tbcint from qdb.t1 tb right asof join tdb.t1 ta on ta.ts < tb.cts jlimit 10 where ta.ts >= _twstart and ta.ts < _twend + 5m and cos(tb.cint) >= 0 and cos(ta.c1) > 0);",
        ...
        stream = StreamItem(
            id=122,
            stream="create stream rdb.s122 count_window(1, c1) from tdb.triggers partition by tbname into rdb.r122 as select first(tats), last(tbts), count(tat1), sum(tat1), first(tbt1), last(tbt1) from (select ta.cts tats, tb.cts tbts, ta.cint tat1, tb.cint tbt1 from qdb.t1 ta left join qdb.t2 tb on ta.cts=tb.cts and (ta.cint >= tb.cint) order by ta.cts) where tats >= _twstart and tats < _twend + 5m",
        ...
        stream = StreamItem(
            id=123,
            stream="create stream rdb.s123 count_window(1, c1) from tdb.triggers partition by id, name, tbname into rdb.r123 as select _wstart tw, _wend te, _twstart, _twend + 5m, count(c1) c1, sum(c2) c2 from %%trows interval(1m)",
        ...
        stream = StreamItem(
            id=125,
            stream="create stream rdb.s125 count_window(1, c1) from tdb.v1 into rdb.r125 as select _rowts, _twstart ts, DERIVATIVE(cbigint, 5, 0) from qdb.v1 where cts >= _twstart and cts < _twstart + 5m;",
        ...
        stream = StreamItem(
            id=126,
            stream="create stream rdb.s126 count_window(1, c1) from tdb.vtriggers partition by id, tbname into rdb.r126 as select _twstart ts, IRATE(cbigint) from qdb.vmeters where tbname=%%tbname;",
        ...
        stream = StreamItem(
            id=127,
            stream="create stream rdb.s127 count_window(1, c1) from tdb.v1 into rdb.r127 as select _rowts, MAVG(c1, 1) from %%trows;",
            res_query="select `_rowts`, `mavg(c1, 1)` from rdb.r127 limit 1 offset 1",
        ...
        stream = StreamItem(
            id=128,
            stream="create stream rdb.s128 count_window(1, c1) from tdb.vtriggers partition by tbname into rdb.r128 as select _rowts, STATECOUNT(cdouble, 'LT', 5) from qdb.vmeters where tbname=%%tbname and cts >= _twstart and cts < _twend + 5m;",
        ...
        stream = StreamItem(
            id=129,
            stream="create stream rdb.s129 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r129 as select _c0, STATEDURATION(cusmallint, 'LT', 5, 1m) from qdb.meters where tbname=%%2 and cts >= _twstart and cts < _twend + 5m;",
        ...
        stream = StreamItem(
            id=130,
            stream="create stream rdb.s130 count_window(1, c1) from tdb.vtriggers partition by tbname into rdb.r130 as select _twstart ts, _twend + 5m, TWA(ctinyint) from qdb.vmeters where tbname=%%1 and cts >= _twstart and cts < _twend + 5m;",
        ...
        stream = StreamItem(
            id=131,
            stream="create stream rdb.s131 count_window(1, c1) from tdb.triggers partition by id, tbname into rdb.r131 as select ta.ts tats, tb.cts tbts, ta.c1 tac1, ta.c2 tac2, tb.cint tbc1, tb.cuint tbc2, _twstart, _twend + 5m from tdb.triggers ta right join qdb.t1 tb on ta.ts=tb.cts where ta.ts >= _twstart and ta.ts < _twend + 5m and ta.id=%%1;",
        ...
        stream = StreamItem(
            id=132,
            stream="create stream rdb.s132 count_window(1, c1) from tdb.t1 partition by id into rdb.r132 as select tb.cts tbts, ta.ts tats, ta.c1 tac1, ta.c2 tac2, tb.cint tbc1, tb.cuint tbc2, _twstart, _twend from tdb.t1 ta right join qdb.t1 tb on ta.ts=tb.cts where tb.cts >= _twstart and tb.cts < _twend + 5m;",
        ...
        stream = StreamItem(
            id=134,
            stream="create stream rdb.s134 count_window(1, c1) from tdb.v1 into rdb.r134 as select _twstart, sum(v1), sum(v2) from (select count(c1) v1, sum(c2) v2 from %%trows)",
        ...
        stream = StreamItem(
            id=135,
            stream="create stream rdb.s135 count_window(1, c1) from tdb.v1 partition by tbname into rdb.r135 as select t1, c11, c12, t2, c21, c22, _twstart, _twend + 5m from (select cols(last_row(cint), cts as t1, cuint as c11), last_row(cint) c12,  cols(first(cint), cts as t2, cuint c21), first(cint) c22  from qdb.meters where tbname='t1' and cts >= _twstart and cts < _twstart + 5m)",
            check_func=self.check135,
        )
        self.streams.append(stream)

        stream = StreamItem(
            id=136,
            stream="create stream rdb.s136 count_window(1) from tdb.v1 partition by id, tbname into rdb.r136 as select ta.ts tats, tb.cts tbts, ta.c1 tac1, ta.c2 tac2, tb.cint tbc1, tb.cuint tbc2, _twstart, _twend from %%trows ta right join qdb.t1 tb on ta.ts=tb.cts where ta.ts >= _twstart and ta.ts < _twend + 5m;",
    ...
    def check133(self):
        tdSql.checkResultsByFunc(
            sql="select tats, tbts, tac1, tac2, tbc1, tbc2 from rdb.r133",
            ...
            and tdSql.compareData(0, 5, 0)
            and tdSql.compareData(1, 0, "2025-01-01 00:05:00.000")
            and tdSql.compareData(1, 1, "2025-01-01 00:05:00.000")
            and tdSql.compareData(1, 2, 5)
            and tdSql.compareData(1, 3, None)
            and tdSql.compareData(1, 4, 10)
            and tdSql.compareData(2, 5, 0)
            and tdSql.compareData(2, 0, "2025-01-01 00:10:00.000")
            and tdSql.compareData(2, 1, "2025-01-01 00:10:00.000")
            and tdSql.compareData(2, 2, 10)
            and tdSql.compareData(2, 3, None)
            and tdSql.compareData(2, 4, 20)
            and tdSql.compareData(2, 5, 0)
            and tdSql.compareData(3, 0, "2025-01-01 00:15:00.000")
            and tdSql.compareData(3, 1, "2025-01-01 00:15:00.000")
            and tdSql.compareData(3, 2, None)
            and tdSql.compareData(3, 3, 150)
            and tdSql.compareData(3, 4, 30)
            and tdSql.compareData(3, 5, 2)
            and tdSql.compareData(4, 0, "2025-01-01 00:30:00.000")
            and tdSql.compareData(4, 1, "2025-01-01 00:30:00.000")
            and tdSql.compareData(4, 2, 30)
            and tdSql.compareData(4, 3, None)
            and tdSql.compareData(4, 4, 60)
            and tdSql.compareData(4, 5, 0),
        )

    def check134(self):
        tdSql.checkResultsByFunc(
            sql="select * from rdb.r134;",
            func=lambda: tdSql.getRows() == 4
            and tdSql.compareData(0, 0, "2025-01-01 00:00:00.000")
            and tdSql.compareData(0, 1, 1)
            and tdSql.compareData(0, 2, 0)
            ...
            and tdSql.compareData(3, 0, "2025-01-01 00:30:00.000")
            and tdSql.compareData(3, 1, 1)
            and tdSql.compareData(3, 2, 300),
        )

    def check135(self):
        tdSql.checkResultsByFunc(
            sql="select t1, c11, c12, t2, c21, c22 from rdb.r135 where tag_tbname='v1';",
            func=lambda: tdSql.getRows() == 4
            and tdSql.compareData(0, 0, "2025-01-01 00:04:30.000")
            and tdSql.compareData(0, 1, 1)
            and tdSql.compareData(0, 2, 9)
    ...
    def check136(self):
        tdSql.checkResultsByFunc(
            ...
            and tdSql.compareData(2, 1, "2025-01-01 00:10:00.000")
            and tdSql.compareData(2, 2, 10)
            and tdSql.compareData(2, 3, 100)
            and tdSql.compareData(2, 4, 20)
            and tdSql.compareData(2, 5, 0)
            and tdSql.compareData(3, 0, "2025-01-01 00:30:00.000")
            and tdSql.compareData(3, 1, "2025-01-01 00:30:00.000")
            and tdSql.compareData(3, 2, 30)
            and tdSql.compareData(3, 3, 300)
            and tdSql.compareData(3, 4, 60)
            and tdSql.compareData(3, 5, 0),
        )
