...
class Test_ThreeGorges:
    caseName = "test_three_gorges_second_case4"
    ...
    tblRowNum = 10
    tableList = []
    outTbname = "stb_sxny_cn_test_v004"
    streamName = "test_str_sxny_cn_test_v004"
    tableList = []
    resultIdx = "1"
    ...
    def test_three_gorges_second_case4(self):
    ...
    def createStream(self):
        ...
        stream = (
                    f"""create stream test1.test_str_sxny_cn_test_v004 interval(1d) sliding(1d) from test1.stb_sxny_cn 
                    partition by index_code,ps_code
                    stream_options(expired_time(3d)|fill_history|pre_filter(index_code in ('index_a0') and dt >= today())|event_type(window_close) )
                    into test1.stb_sxny_cn_test_v004 output_subtable(concat_ws('_','sxny_cn_test_v004',ps_code)) 
                    tags(
