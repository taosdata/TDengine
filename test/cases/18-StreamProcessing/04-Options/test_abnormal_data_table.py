...
class TestStreamDisorderTable:
    ...
    class Basic4(StreamCheckItem):
        ...
        def create(self):
            ...
            tdSql.executes(
                [
                f"create stream if not exists s1 interval(1h) sliding(1h) from {self.db}.{self.stbName} partition by site, zone stream_options(watermark(10m) | ignore_disorder | fill_history('2024-10-04 09:00:00.000')) into res1 output_subtable(concat('res_1_', cast(site as varchar),  cast(zone as varchar))) tags(tag_site nchar(32) as site, tag_zone nchar(32) as zone) as select _twend as window_end, %%1 as site, %%2 as zone, case when last(_c0) is not null then 1 else 0 end as zone_online from {self.db}.{self.stbName} where _c0 >= _twstart and _c0 < _twend and site=%%1 and zone=%%2;",
                f"create stream if not exists s2 interval(1h) sliding(1h) from {self.db}.{self.stbName} partition by site, zone stream_options(watermark(10m) | ignore_disorder | fill_history('2024-10-04 09:00:00.000')) into res2 output_subtable(concat('res_2_', cast(site as varchar),  cast(zone as varchar))) tags(tag_site nchar(32) as site, tag_zone nchar(32) as zone) as select _twend as window_end, %%1 as site, %%2 as zone, last(_c0) from {self.db}.{self.stbName} where _c0 >= _twstart and _c0 < _twend and site=%%1 and zone=%%2;",
                f"create stream if not exists s3 interval(1h) sliding(1h) from {self.db}.{self.stbName} partition by site, zone stream_options(watermark(10m) | ignore_disorder | fill_history('2024-10-04 09:00:00.000')) into res3 output_subtable(concat('res_3_', cast(site as varchar),  cast(zone as varchar))) tags(tag_site nchar(32) as site, tag_zone nchar(32) as zone) as select _twend as window_end, %%1 as site, %%2 as zone, count(*) from {self.db}.{self.stbName} where _c0 >= _twstart and _c0 < _twend and site=%%1 and zone=%%2;",
                f"create stream if not exists s4 interval(1h) sliding(1h) from {self.db}.{self.stbName} partition by site, zone stream_options(watermark(10m) | ignore_disorder | fill_history('2024-10-04 09:00:00.000')) into res4 output_subtable(concat('res_4_', cast(site as varchar),  cast(zone as varchar))) tags(tag_site nchar(32) as site, tag_zone nchar(32) as zone) as select _c0, _twend as window_end, %%1 as site, %%2 as zone from {self.db}.{self.stbName} where _c0 >= _twstart and _c0 < _twend and site=%%1 and zone=%%2;"
                ]
            )
