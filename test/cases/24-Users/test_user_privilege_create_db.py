...
class TestUserPrivilegeCreateDb:
    ...
    def test_user_privilege_create_db(self):
        ...
        tdSql.execute(f"grant create database to u2")
        ...
        tdSql.query(
            f'select * from ins_user_privileges where user_name == "u1" order by priv_type'
        )
        tdSql.checkRows(1) # the owner of u1_d1 is u1, not show in privileges since 3.4.0.0
        tdSql.checkData(0, 0, "u1")
        tdSql.checkData(0, 1, "CREATE DATABASE")
        tdSql.checkData(0, 2, "CLUSTER")
        ...
        tdSql.execute(f"use information_schema")
        tdSql.query(
            f'select * from ins_user_privileges where user_name == "u2" order by priv_type'
        )
        tdSql.checkRows(1) # the owner of u2_d1 is u2, not show in privileges since 3.4.0.0
        ...
        tdLog.info(f"=============connect with root, revoke select from u1, all from u2")
        tdSql.connect("root")
        tdSql.execute(f"revoke select on u1_d1.* from u1")
        ...
        tdSql.connect("root")
        tdSql.execute(f"grant select,insert on u1_d1.* to u1")
