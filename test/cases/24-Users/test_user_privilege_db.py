...
class TestUserPrivilegeDb:
    ...
    def test_user_privilege_db(self):
        ...
        tdSql.error(f"GRANT select ON d1.* to a;")
        tdSql.error(f"GRANT select ON d0.* to user1;")

        tdSql.execute(f"GRANT select ON d1.* to user1;")
        tdSql.execute(f"GRANT select ON d2.* to user1;")
        tdSql.execute(f"GRANT select ON *.* to user1;")

        tdSql.execute(f"REVOKE select ON d1.* from user1;")
        tdSql.execute(f"REVOKE select ON d2.* from user1;")
        tdSql.execute(f"REVOKE select ON *.* from user1;")
    
        tdLog.info(f"=============== test write")
        tdSql.error(f"GRANT insert ON d1.* to a;")
        tdSql.error(f"GRANT insert ON d0.* to user1;")

        tdSql.execute(f"GRANT insert ON d1.* to user1;")
        tdSql.execute(f"GRANT insert ON d2.* to user1;")
        tdSql.execute(f"GRANT insert ON *.* to user1;")

        tdSql.execute(f"REVOKE insert ON d1.* from user1;")
        tdSql.execute(f"REVOKE insert ON d2.* from user1;")
        tdSql.execute(f"REVOKE insert ON *.* from user1;")
        ...
        tdSql.error(f"GRANT select,insert ON d1.* to a;")
        tdSql.error(f"GRANT select,insert ON d0.* to user1;")

        tdSql.execute(f"GRANT select,insert ON d1.* to user1;")
        tdSql.execute(f"GRANT select,insert ON d2.* to user1;")
        tdSql.execute(f"GRANT select,insert ON *.* to user1;")

        tdSql.execute(f"REVOKE select,insert ON d1.* from user1;")
        tdSql.execute(f"REVOKE select,insert ON d2.* from user1;")
        ...
        tdSql.execute(f"grant use,create topic on database d1 to u1;")
        tdSql.execute(f"grant use,create table,create topic on database d2 to u1;")
        tdSql.execute(f"GRANT select ON d1.* to u1;")
        tdSql.execute(f"GRANT insert ON d2.* to u1;")
        tdSql.execute(f"GRANT select ON d1.d1_stb to u1")
        tdSql.execute(f"GRANT insert ON d1.d1_stb to u1")
        tdSql.execute(f"GRANT insert ON d1.d1_stb1 to u1")
        ...
        tdSql.execute(f"GRANT subscribe ON topic d1.d1_topic_1 to u1")
        tdSql.execute(f"GRANT select ON view d1.d1_view1 to u1")
        tdSql.execute(f"GRANT select ON view d1.d1_view2 to u1")
        tdSql.execute(f"GRANT select ON view d1.d1_view3 to u1")
        tdSql.error(f"GRANT insert ON view d1.d1_view1 to u1")
        tdSql.error(f"GRANT insert ON view d1.d1_view2 to u1")
        tdSql.execute(f"GRANT alter ON view d1.d1_view3 to u1")
        tdSql.execute(f"GRANT all ON view d1.d1_view3 to u1")
        ...
        tdSql.query(f"select * from information_schema.ins_user_privileges where user_name = 'u1'")
        # tdSql.checkRows(1) # PRIV_TODO: clean privilege after drop db
