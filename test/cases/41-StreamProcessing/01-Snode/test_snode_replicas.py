...
class TestSnodeReplicas:
    ...
    def checkSnodeIdList(self, idList, excludeList):
        for i in idList:
            if (i in excludeList):
                raise Exception(f"snode idList error:{idList}, excludeList:{excludeList}")

    def checkSnodeIdListMatch(self, idList, expectedList):
        if len(idList) != len(expectedList):
            raise Exception(f"snode idList length error:{idList}, expectedList:{expectedList}")
        
        for i in idList:
            if (i not in expectedList):
                raise Exception(f"snode idList error:{idList}, expectedList:{expectedList}")
            
    def checkSnodeReplica(self, replicaList, expectedList):
        if 1 == len(replicaList) and 0 != replicaList[0]:
            raise Exception(f"snode replicaList error:{replicaList}, expectedList:{expectedList}")
    ...
    def checkSnodeAsReplicaOf(self, asReplicaOf, expectedList):
        ...
        if len(asReplicaOfList) != len(set(asReplicaOfList)):
            raise Exception(f"snode asReplicaOfList contains duplicates: {asReplicaOfList}")
    
        asReplicaOfList.sort()
        if (len(expectedList) > 1 and asReplicaOfList != expectedList):
            raise Exception(f"snodes asReplicaOf error:{asReplicaOf}, list:{asReplicaOfList}, expectedList:{expectedList}, nonVal:{noneVal}")
