...
class Test_ThreeGorges:
    ...
    tblRowNum = 10
    tableList = []
    outTbname = "stb_sxny_cn_test_v006"
    streamName = "test_str_sxny_cn_test_v006"
    tableList = []
    resultIdx = "1"
    ...
    def createStream(self):
        ...
        stream = (
                    f"""create stream test1.test_str_sxny_cn_test_v006 interval(1d) sliding(1d) from test1.stb_sxny_cn 
                    partition by index_code,ps_code
                    stream_options(fill_history|pre_filter(index_code in ('index_a0') and val>0 and dt >= today())|event_type(window_close) )
                    into test1.stb_sxny_cn_test_v006 output_subtable(concat_ws('_','sxny_cn_test_v006',ps_code)) 
                    tags(
