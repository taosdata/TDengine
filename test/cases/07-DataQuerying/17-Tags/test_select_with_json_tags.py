...
class TestSelectWithJsonTags:
    ...
    def test_select_with_json_tags(self):
        """Select With Json Tag Test

        1.Create db
        2.Create supper table
        4.Inset data with json tags
        5.Select data and check the result
        ...
        tdSql.execute(f"create stable stb(ts timestamp, value int) tags (info json)")
        ...
        tdSql.query(f"select info->'info' from stb")
        tdSql.checkRows(5)
        tdSql.checkData(0, 1, "大连\n123")
        tdSql.checkData(0, 1, "大连\t123")
        tdSql.checkData(0, 1, "大连\b123")
        tdSql.checkData(0, 1, "大连\r123")
        tdSql.checkData(0, 1, "大连\f123")
