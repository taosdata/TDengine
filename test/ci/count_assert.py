...
def check_list_result(result_list,detaild_list):
    ...
    remove_detail_items = [
            ...
            f"{TD_project_path}/community/source/libs/tdb/src/db/tdbPager.c:ASSERT_CORE(pgno > 0 && pgno <= pPager->dbFileSize, \"invalid pgno:%u, db file size %u.\", pgno, pPager->dbFileSize);",
            f"{TD_project_path}/community/source/libs/tdb/src/db/tdbPager.c:ASSERT_CORE(*ppgno > 0 && *ppgno <= pPager->dbFileSize, \"invalid pgno:%u, db file size %u.\", *ppgno, pPager->dbFileSize);",
        ]
    expected_strings = [
