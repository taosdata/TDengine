...
if [[ $archOs =~ "aarch64" ]]; then
  ...
  memory_leak=$(cat ${LOG_DIR}/*.asan | grep "Direct leak" | grep -v "Direct leak of 32 byte" | grep -v "taosws.abi3.so" | wc -l)
  ...
  memory_leak=$(cat ${LOG_DIR}/*.asan | grep "Direct leak" | grep -v "taosws.abi3.so" | wc -l)
fi
