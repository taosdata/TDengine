...
function run_thread() {
    ...
    local count=0
    local script="${workdirs[index]}/TDengine/test/ci/cov/run_coverage_container_internal.sh"
    if [ $ent -ne 0 ]; then
        local script="${workdirs[index]}/TDinternal/community/test/ci/cov/run_coverage_container_internal.sh -e"
    fi
