...
if [ ! `echo $1 | grep :` ];then
        ...
        docker run -itd --name $2 -h $2 -p $1:9100 node_exporter:v1 /bin/bash
else
	...
	end_port=`echo $1 | cut -d ':' -f 2`
		...
		docker run -itd --name $perfix$i -h $perfix$i -p $i:9100 node_exporter:v1 /bin/bash
        done
fi
#docker run -itd --name node_exporter_agent1 -h node_exporter_agent1 -p 10000:9100 node_exporter:v1 /bin/bash
