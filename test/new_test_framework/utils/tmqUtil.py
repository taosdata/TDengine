...
class TMQCom:
    ...
    def selectConsumeResult(self, expectRows, cdbName="cdb"):
        ...
        for i in range(expectRows):
            tdLog.info(
                f"consume id: {tdSql.getData(i, 1)}, consume msgs: {tdSql.getData(i, 2)}, consume rows: {tdSql.getData(i, 3)}"
            )
    ...
    def startTmqSimProcess(
        ...
        if platform.system().lower() == "windows":
            ...
            shellCmd += " -y %d -d %s -g %d -r %d -w %s -e %d " % (
                pollDelay,
                dbName,
                showMsg,
                showRow,
    ...
    def stopTmqSimProcess(self, processorName):
        ...
        if platform.system().lower() == "windows":
            psCmd = "ps -ef|grep -w %s|grep -v grep | awk '{print $2}'" % (
                processorName
            )
    ...
    def create_database(self, tsql, dbName, dropFlag=1, vgroups=4, replica=1):
        ...
        tsql.execute(
            f"create database if not exists {dbName} vgroups {vgroups} replica {replica}"
        )
    ...
    def create_stable(self, tsql, dbName, stbName):
        ...
        tsql.execute(
            "create table if not exists %s.%s %s" % (dbName, stbName, schemaString)
        )
    ...
    def create_ctable(
        ...
        for i in range(ctbNum):
            ...
            sql += " %s.%s%d using %s.%s tags(%d, %d, %d, '%s', '%s')" % (
                dbName,
                ctbPrefix,
                i + ctbStartIdx,
                dbName,
    ...
    def drop_ctable(
        self, tsql, dbname=None, count=1, default_ctbname_prefix="ctb", ctbStartIdx=0
    ):
        for _ in range(count):
            create_ctable_sql = (
                f"drop table if exists {dbname}.{default_ctbname_prefix}{ctbStartIdx};"
            )
            ctbStartIdx += 1
    ...
    def insert_data(
        self, tsql, dbName, stbName, ctbNum, rowsPerTbl, batchNum, startTs=None
    ):
    ...
    def insert_data_1(
        self, tsql, dbName, ctbPrefix, ctbNum, rowsPerTbl, batchNum, startTs
    ):
        tdLog.debug("start to insert data ............")
        tsql.execute("use %s" % dbName)
        pre_insert = "insert into "
    ...
    # schema: (ts timestamp, c1 int, c2 bigint, c3 double, c4 binary(32), c5 nchar(32), c6 timestamp) tags (t1 int, t2 bigint, t3 double, t4 binary(32), t5 nchar(32))
    def insert_data_interlaceByMultiTbl(
        self,
        tsql,
        dbName,
        ctbPrefix,
        ...
        while rowsOfCtb < rowsPerTbl:
            ...
            for i in range(ctbNum):
                ...
                for k in range(batchNum):
                    if k % 2 == 0:
                        sql += "(%d, %d, %d, %d, 'binary_%d', 'nchar_%d', now) " % (
                            startTs + ctbDict[i],
                            ctbDict[i],
                            ctbDict[i],
                            ctbDict[i],
    ...
    def threadFunctionForInsertByInterlace(self, **paraDict):
        ...
        self.insert_data_interlaceByMultiTbl(
            newTdSql,
            paraDict["dbName"],
            paraDict["ctbPrefix"],
            paraDict["ctbNum"],
            ...
            paraDict["batchNum"],
            paraDict["startTs"],
            paraDict["ctbStartIdx"],
        )
        return

    def asyncInsertDataByInterlace(self, paraDict):
        pThread = threading.Thread(
            target=self.threadFunctionForInsertByInterlace, kwargs=paraDict
        )
        pThread.start()
        return pThread

    def insert_data_with_autoCreateTbl(
        self,
        tsql,
        dbName,
        stbName,
        ...
        for i in range(ctbNum):
            ...
            sql += " %s.%s%d using %s.%s tags (%d, %d, %d, '%s', '%s') values " % (
                dbName,
                ctbPrefix,
                i + ctbStartIdx,
                dbName,
    ...
    def syncCreateDbStbCtbInsertData(self, tsql, paraDict):
        ...
        tdCom.create_stable(
            ...
            column_elm_list=paraDict["colSchema"],
            tag_elm_list=paraDict["tagSchema"],
        )
        tdCom.create_ctable(
            tsql,
            dbname=paraDict["dbName"],
            stbname=paraDict["stbName"],
            tag_elm_list=paraDict["tagSchema"],
            count=paraDict["ctbNum"],
            default_ctbname_prefix=paraDict["ctbPrefix"],
        )
        ...
        for i in range(ctbNum):
            ...
            tdCom.insert_rows(
                ...
                dbname=paraDict["dbName"],
                tbname=tbName,
                start_ts_value=paraDict["startTs"],
                count=paraDict["rowsPerTbl"],
            )
        return
    ...
    def checkFileContent(self, consumerId, queryString, skipRowsOfCons=0):
        ...
        cmdStr = '%s/build/bin/taos -c %s -s "%s >> %s"' % (
            buildPath,
            cfgPath,
            queryString,
            dstFile,
        ...
        while True:
            ...
            if len(dstSplit) != len(srcSplit):
                tdLog.exit(
                    "consumerId %d consume rows len is not match the rows by direct query,len(dstSplit):%d != len(srcSplit):%d, dst:%s, src:%s"
    ...
    def insert_rows_into_ntbl(
        ...
        for tblIdx in range(tblNum):
            for rowIdx in range(rows):
                column_value_list = tdCom.gen_column_value_list(
                    column_ele_list, f"{startTs}+{rowIdx}s"
                )
                column_value_str = ""
                idx = 0
    ...
    def killProcesser(self, processerName):
        if platform.system().lower() == "windows":
            killCmd = (
                'wmic process where name="%s.exe" call terminate > NUL 2>&1'
                % processerName
            )
            psCmd = 'wmic process where name="%s.exe" | findstr "%s.exe"' % (
                processerName,
                processerName,
            )
        else:
            ...
            psCmd = (
                "ps -efww|grep -w %s| grep -v grep | awk '{print $2}'" % processerName
            )
