...
class TDDnode:
    ...
    def stoptaosd(self):
        ...
        if (not self.remoteIP == ""):
            tdLog.info("not remote stop dnode%d"%self.index)
            self.remoteExec(self.cfgDict, "tdDnodes.dnodes[%d].running=1\ntdDnodes.dnodes[%d].stop()"%(self.index-1,self.index-1))
            tdLog.info("stop dnode%d"%self.index)
            return
        if self.valgrind == 0:
        ...
        if self.running != 0:
            if platform.system().lower() == 'windows':
                ...
                psCmd = "ps -ef|grep -w %s| grep %s|grep -v grep | awk '{print $2}' | xargs" % (toBeKilled,self.cfgDir)
            processID = subprocess.check_output(
                psCmd, shell=True).decode("utf-8").strip()
